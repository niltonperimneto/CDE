# XmTextGetSubstring
library call`XmTextGetSubstring`A Text function that retrieves a copy
of a portion of the internal text bufferXmTextGetSubstringText functionsXmTextGetSubstring#include <Xm/Text.h>int`XmTextGetSubstring`WidgetwidgetXmTextPositionstartintnum_charsintbuffer_sizechar *buffer
## DESCRIPTION
...
per character (`MB_CUR_MAX`) for the current locale.`MB_CUR_MAX`is a macro defined instdlib.h. The buffer
...
size of buffer the application should provide:buffer_size= (num_chars* MB_CUR_MAX) + 1
...
&cdeman.XmText; and &cdeman.XmTextGetSubstringWcs;.