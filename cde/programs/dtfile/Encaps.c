...
static DialogResource commonResources[] = {
    {"displayed", XmRBoolean, sizeof(Boolean),
     XtOffset(DialogInstanceDataPtr, displayed), (caddr_t)False,
     (WriteResourceProc)_DtBooleanToString},

    {"x", XmRPosition, sizeof(Position), XtOffset(DialogInstanceDataPtr, x),
     (caddr_t)0, (WriteResourceProc)_DtPositionToString},

    {"y", XmRPosition, sizeof(Position), XtOffset(DialogInstanceDataPtr, y),
     (caddr_t)0, (WriteResourceProc)_DtPositionToString},

    {"width", XmRHorizontalDimension, sizeof(Dimension),
     XtOffset(DialogInstanceDataPtr, width), (caddr_t)0,
     (WriteResourceProc)_DtDimensionToString},

    {"height", XmRVerticalDimension, sizeof(Dimension),
     XtOffset(DialogInstanceDataPtr, height), (caddr_t)0,
     (WriteResourceProc)_DtDimensionToString},
};
...
static void DialogStructureNotifyHandler(Widget w, XtPointer client_data,
                                         XEvent *event);
static void SetIconifyState(Widget shell, Boolean iconify);
static void DataChangeCallback(Widget widget, XtPointer client_data,
                               XtPointer call_data);
static void DataCloseCallback(Widget widget, XtPointer client_data,
                              XtPointer call_data);
static void TimerEvent(Widget widget, XtIntervalId *id);
static void IntDialogGetResources(XrmDatabase database, char *base,
                                  DialogResource *resource, XrmName *xrmName,
                                  int nameCount, XrmQuark stringQuark);
static void IntDialogPutResources(int fd, char **nameList, char *dialogName,
                                  char *base, DialogResource *resource);
#ifdef USE_XINERAMA
static Boolean GetXineramaScreenDimensions(Widget w, int *xorg, int *yorg,
...
void _DtInitializeEncapsulation(Display *display, char *name, char *class)

{
  ...
  encap_parent_shell = XtAppCreateShell(
      name, class, applicationShellWidgetClass, display, NULL, 0);
  ...
  timerId = XtAppAddTimeOut(
      XtWidgetToApplicationContext(encap_parent_shell), initialTimeoutLength,
      (XtTimerCallbackProc)TimerEvent, (caddr_t)encap_parent_shell);
...
int _DtInstallDialog(DialogClass *dialog_class, Boolean cache,
                     Boolean destroyPopups)

{
  ...
  if (num_classes == class_set_size) {
    ...
    class_set = (ClassSet *)XtRealloc((char *)class_set,
...
DialogData *_DtGetDialogData(DialogData *dialog_data)
...
DialogData *_DtGetDefaultDialogData(int dialog_type)
...
DialogData *_DtGetResourceDialogData(int dialog_type, XrmDatabase data_base,
                                     char **name_list)
 ...
 *
 ************************************************************************/
void _DtShowDialog(Widget parent, Widget map_parent, XtPointer top_rec,
                   DialogData *dialog_data, DialogChangedProc change_proc,
                   XtPointer change_data, DialogClosedProc close_proc,
                   XtPointer close_data, char *workspaces,
                   Boolean iconify_state, Boolean ignoreCache, char *title,
...

{
  ...
  if ((!ignoreCache) && (class_set[dialog_type].cache) &&
    ...
    timerId = XtAppAddTimeOut(
        XtWidgetToApplicationContext(encap_parent_shell), activeTimeoutLength,
        (XtTimerCallbackProc)TimerEvent, (XtPointer)encap_parent_shell);
  }
  ...
  if (XtIsSubclass(XtParent(dialog->dialog_widget),
    ...
    if (XtIsRealized(XtParent(dialog->dialog_widget)) == False) {
      if (instance_data->displayed == True) {
        (void)sprintf(geometry, "=%dx%d+%d+%d", instance_data->width,
                      instance_data->height, instance_data->x,
                      instance_data->y);
        ...
        n = 0;
        XtSetArg(args[n], XmNwidth, instance_data->width);
        n++;
        XtSetArg(args[n], XmNheight, instance_data->height);
        n++;
      ...
      if (classHints) {
        XSetClassHint(XtDisplay(dialog->dialog_widget),
                      XtWindow(XtParent(dialog->dialog_widget)), classHints);
      }
  ...
  (*(class_set[dialog_data->type].class->set_values))(dialog->dialog,
                                                      dialog_data->data);
  ...
  if (class_set[dialog_data->type].class->set_focus) {
    (*(class_set[dialog_data->type].class->set_focus))(dialog->dialog,
                                                       dialog_data->data);
  }

  XtAddEventHandler(XtParent(dialog->dialog_widget), StructureNotifyMask, False,
                    (XtEventHandler)DialogStructureNotifyHandler, NULL);
}
...
void _DtHideDialog(DialogData *dialog_data, Boolean call_callbacks)
  ...
  while (dialog != NULL) {
...

    {
      ...
      if (call_callbacks && dialog->close) {
        ...
        (*(dialog->close))(dialog->close_client_data, dialog->dialog_data,
                           new_data);
      } else {
        dialog->in_use = False;
        ((DialogInstanceData *)(dialog->dialog_data->data))->displayed = False;
...
Widget _DtGetDialogShell(DialogData *dialog_data)
...
XtPointer _DtGetDialogInstance(DialogData *dialog_data)
...
DialogData *_DtGetInstanceData(XtPointer instance)
...
Boolean _DtIsDialogShowing(DialogData *dialog_data)
...
void _DtWriteDialogData(DialogData *dialog_data, int fd, char **name_list)
...
void _DtFreeDialogData(DialogData *dialog_data)
...
void _DtDialogGetResources(XrmDatabase database, char **name_list,
                           char *dialog_name, char *base,
                           DialogResource *resources, int resource_count)
    ...
    if (XrmQGetResource(database, xrmName, xrmName, &repType, &value)) {
      if (repType == stringQuark)
        if (strcmp(resource->type, XmRString) != 0) {
          XtConvert(encap_parent_shell, XmRString, &value, resource->type,
                    &convertedValue);
        } else {
...
void _DtDialogPutResources(int fd, char **nameList, char *dialogName,
                           char *base, DialogResource *resources,
                           int resourceCount)
...

{
  ...
  (*(resource->write_resource))(fd, (XtPointer)(base + resource->offset),
                                outBuf);
}
 ...
 *
 ************************************************************************/
void _DtEncapSetWorkSpaceHints(Widget shell, char *workspaces)

{
  ...
  if (workspaces) {
    ...
    if (DtWsmGetCurrentWorkspace(XtDisplay(shell), rootWindow, &pCurrent) ==
        Success) {
      DtWsmSetWorkspacesOccupied(XtDisplay(shell), XtWindow(shell), &pCurrent,
                                 1);
    }
  }
}
...

{
  ...
  if (dialog->change)
    (*(dialog->change))(dialog->change_client_data, dialog->dialog_data,
                        new_data, call_data);
}
...

{
  ...
  for (i = 0; i < num_classes; i++) {
    ...
    if (dialog == NULL) {
      ...
      timerState = TIMER_ACTIVE_STATE;
      timerId = XtAppAddTimeOut(
          XtWidgetToApplicationContext(widget), activeTimeoutLength,
          (XtTimerCallbackProc)TimerEvent, (XtPointer)widget);
  ...
  timerId =
      XtAppAddTimeOut(XtWidgetToApplicationContext(widget), idleTimeoutLength,
                      (XtTimerCallbackProc)TimerEvent, (XtPointer)widget);
}
...
void _DtIntToString(int fd, XtPointer value, char *out_buf)

{
  int *val = (int *)value;
  (void)sprintf(resourceBuf, "%d", *val);
  _DtStringToString(fd, &resourceBuf, out_buf);
}

void _DtShortToString(int fd, XtPointer value, char *out_buf)
...
void _DtPositionToString(int fd, XtPointer value, char *out_buf)

{
  short val = *(Position *)value;
  (void)sprintf(resourceBuf, "%d", val);
  _DtStringToString(fd, &resourceBuf, out_buf);
}

void _DtDimensionToString(int fd, XtPointer value, char *out_buf)

{
  short val = *(Dimension *)value;
  (void)sprintf(resourceBuf, "%d", val);
  _DtStringToString(fd, &resourceBuf, out_buf);
}

void _DtBooleanToString(int fd, XtPointer value, char *out_buf)
...
void _DtXmStringToString(int fd, XtPointer value, char *out_buf)

{
  XmString *val = (XmString *)value;
  char *out_value = NULL;

  if (*val != NULL) {
    out_value = (char *)_XmStringUngenerate(*val, XmFONTLIST_DEFAULT_TAG,
                                            XmCHARSET_TEXT, XmCHARSET_TEXT);
    if (out_value != NULL) {
      if (strlen(out_value) != 0) {
        (void)write(fd, out_value, strlen(out_value));
        _DtStringToString(fd, (XtPointer)&out_value, out_buf);
...
void _DtXmStringTableToString(int fd, XtPointer value, char *out_buf)

{
  XmStringTable *val = (XmStringTable *)value;
  int i;
  char *out_value = NULL;
  Boolean first = True;
  ...
  if ((val != NULL) && (*val != NULL)) {
    i = 0;
    while ((*val)[i] != NULL) {
      out_value = (char *)_XmStringUngenerate((*val)[i], XmFONTLIST_DEFAULT_TAG,
                                              XmCHARSET_TEXT, XmCHARSET_TEXT);
      if (out_value != NULL) {
        if (first) {
...
void _DtStringToString(int fd, XtPointer value, char *out_buf)
...
/* ARGSUSED */
void _DtmapCB(Widget w, XtPointer client_data, XtPointer call_data) {
  Arg args[2];
  Widget parent;
  Position newX, newY;
...
void _DtBuildDialog(Widget parent, Widget map_parent, XtPointer top_rec,
                    DialogData *dialog_data, DialogChangedProc change_proc,
                    XtPointer change_data, DialogClosedProc close_proc,
                    XtPointer close_data, char *workspaces,
                    Boolean iconify_state, Boolean ignoreCache, char *title,
...
void _DtShowBuiltDialog(Widget parent, Widget map_parent,
                        DialogData *dialog_data, char *workspaces,
                        Boolean iconify_state, XClassHint *classHints)
  ...
  if (event->type == MapNotify) {
    if (NumberOfDialogMapped == 0) {
      ...
      if (timeOut) {
        ...
        timerId =
            XtAppAddTimeOut(XtWidgetToApplicationContext(w), timeOut,
...
void _DtChangeTo(XtPointer client_data, char *directory) {
  Dialog *dialog = (Dialog *)client_data;

  ChangeDirectoryToParent(dialog->change_client_data, directory);
}

void _DtFreeDialog(DialogData *dialog_data) {
 ...
 */
static Boolean GetXineramaScreenDimensions(Widget w, int *org_x, int *org_y,
