 ...
 *
 ************************************************************************/
int CheckAccess(char *fname, int what) {
  int access_priv;
  uid_t save_ruid;
  gid_t save_rgid;
 ...
 *
 ************************************************************************/
void FileOperationError(Widget w, char *message1, char *message2) {
  char *message_buf;
  char *title;
  char *tmpStr;
 ...
 *
 ************************************************************************/
static int Check(Widget w, char *spth, ino_t dinode, int mode,
...
static Boolean MoveDir(Widget w, char *source, char *target,
                       struct stat *sourceStatInfo,
                       void (*errorHandler)(Widget, char *, char *),
                       char **targetRtn, int type) {
  ...
  *targetRtn = NULL;
  ...
  /*
    ...
    folder\n%s\nWrite permission is required."); msg = XtMalloc(strlen(tmpStr) +
    strlen(source) + 2); sprintf(msg, tmpStr, source);
    (*errorHandler) (w, msg, source);
    XtFree(msg);
    }
    XtFree(targetDir);
      ...
      if (errorHandler) {
        tmpStr =
            GETMESSAGE(11, 39,
                       ...
                       "processes for this system has been reached.\nStop some "
                       "of the processes or programs that are "
    ...
    if (child_pid == 0) {
      ...
      if (type == TRASH_DIRECTORY)
        rc = execlp(DTCOPY, "dtfile_copy", "-move", "-confirmReplace",
                    "-confirmErrors", "-popDown", "-checkPerms", source, target,
                    NULL);
      else
        rc = execlp(DTCOPY, "dtfile_copy", "-move", "-confirmReplace",
  ...
  if (s1.st_ino != s2.st_ino) { /*  different parent inodes  */
    (void)lstat(source, &s1);   /* get source dir ino */

    if (Check(w, _DtPName(target), s1.st_ino, MOVE_FILE,
              errorHandler)) { /* move into self */
      XtFree(targetDir);
      return (False);
    }
  }
 ...
 *
 ************************************************************************/
static Boolean CopyDir(Widget w, int mode, char *from, char *to,
                       Boolean isContainer, struct stat *s1,
                       void (*errorHandler)(Widget, char *, char *),
  ...
  if (child_pid == -1) {
    if (errorHandler) {
      tmpStr = GETMESSAGE(
          ...
          "this system has been reached.\nStop some of the processes or "
  ...
  if (child_pid == 0) {
    ...
    if (mode == MERGE_DIR)
      ...
      rc = execlp(DTCOPY, "dtfile_copy", "-dontDelete", "-forceCopies",
                  "-copyTop", "-confirmReplace", "-confirmErrors", "-popDown",
                  from, target, (char *)NULL);
    else
      /* replace target dir */
      rc = execlp(DTCOPY, "dtfile_copy", "-forceCopies", "-copyTop",
                  "-confirmErrors", "-popDown", from, target, (char *)NULL);
 ...
 *
 ************************************************************************/
Boolean FileManip(Widget w, int mode, char *from, char *to,
    ...
    if (mode == MOVE_FILE) {
      if (checkForBusyDir && DirectoryBusy(from)) {
        if (errorHandler) {
          ...
          tmpStr = (GETMESSAGE(
              11, 30,
              "Cannot move or rename the folder %s.\nAll File Manager views "
              "displayed for a folder or its sub-folders\nmust be closed "
              "before a folder can be moved or renamed."));
      ...
      move_dir_return =
          MoveDir(w, from, to, &s1, errorHandler, &realTarget, type);
      ...
      if ((link_len = readlink(from, link_path, MAX_PATH)) > 0) {
        ...
        if (mode == MOVE_FILE)
          if (RunFileCommand(MOVE_CMD, from, to, NULL) == 0)
            return (True);
          else if (RunFileCommand(LINK_CMD, "-s", from, to) == 0)
            return (True);
      ...
      if (errno) {
        ...
        case EDQUOT: {
          if (mode == COPY_FILE)
            tmpStr = (GETMESSAGE(
                11, 51,
                "Unable to copy the file/folder because\nthe disk quota will "
                "be exceeded on the disk\nyou are copying it to."));
          else
            tmpStr = (GETMESSAGE(
                11, 52,
                "Unable to move the file/folder because\nthe disk quota will "
        ...
        if (catmsg == NULL) {
          if (mode == COPY_FILE)
            tmpStr =
                (GETMESSAGE(11, 53,
                            "The copy of the file/folder failed\ndue to some "
                            "internal error. The internal\nerror given is:"));
          else
            tmpStr =
                (GETMESSAGE(11, 54,
                            "The move of the file/folder failed\ndue to some "
                            "internal error. The internal\nerror given is:"));
          catmsg = XtNewString(tmpStr);
        ...
        bufLen = (strerrormsg
                      ? strlen(strerrormsg) + strlen(errnoMsg) + strlen(samsg)
                      : 0) +
                 strlen(catmsg) + 10;
        msg = XtMalloc(bufLen);
        strcpy(msg, catmsg);
  ...
  if (mode == MOVE_FILE) {
    if (unlink(from) < 0) {
      if (errorHandler) {
        ...
        tmpStr = (GETMESSAGE(
            11, 38,
            "You do not have permission to move %s\nHowever, you can copy the "
            "object.\nTo copy an object:\n  - press and hold the <Ctrl> key, "
            "and\n  - drag the object with your mouse.\nOr\n  - use 'Copy To' "
            "in the 'Selected' menu popup of the menu bar."));
        msg = XtNewString(tmpStr);
