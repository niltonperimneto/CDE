...
extern Widget _DtDuplicateIcon(Widget, Widget, XmString, String, XtPointer,
                               Boolean);
...
static int FileNameAscending(const void *t1, const void *t2);
static int FileNameDescending(const void *t1, const void *t2);
static int FileTypeAscending(const void *t1, const void *t2);
static int FileTypeDescending(const void *t1, const void *t2);
static int FileSizeAscending(const void *t1, const void *t2);
static int FileSizeDescending(const void *t1, const void *t2);
static int FileDateAscending(const void *t1, const void *t2);
static int FileDateDescending(const void *t1, const void *t2);
static void MakeReuseList(Widget *children, int num_children,
                          FileViewData **order_list, int order_count,
                          Widget **reuse_icons, Widget **reuse_btns);
static void CompressObjectList(ObjectPosition **object_positions,
                               int num_objects, int starting_index);
static ObjectPosition *GetPositionalData(FileMgrData *file_mgr_data,
                                         FileViewData *object,
                                         Dimension max_width, Boolean create);
static void RedisplayUsingStackingOrder(FileMgrData *file_mgr_data, Widget w,
                                        XEvent *event, Region region);
static void ReorderChildrenList(XmManagerWidget file_window, Widget *manage,
                                int manageCount, Widget *unmanage,
                                int unmanageCount);
static Boolean IsDesktopPtr(FileViewData *fileViewData,
                            FileMgrData **fileMgrData, DesktopRec **desktopRec);
static Boolean IntersectRects(Position x1, Position y1, Dimension w1,
                              Dimension h1, Position x2, Position y2,
                              Dimension w2, Dimension h2);
static void InitiateIconDrag(FileViewData *fileViewData, int rootX, int rootY,
                             XEvent *event);
static void RelocateDesktopIcon(DesktopRec *desktopRec, int root_x, int root_y);
static void BuildObjectPositions(FileMgrData *file_mgr_data);
static void moveCopyLinkCancel(Widget w, XtPointer client_data,
                               XtPointer call_data);
static void moveCopyLinkOK(Widget w, XtPointer client_data,
                           XtPointer call_data);
static void DropOnRootCB(Widget w, XtPointer client_data, XtPointer call_data);
static void _UpdateFileIcons(FileMgrRec *file_mgr_rec,
                             FileMgrData *file_mgr_data, Boolean new_directory,
                             DirectorySet *add_dir_set);
static void CreateTreeIcons(Widget w);
static void TreeBtnCallback(Widget w, XtPointer clientData,
                            XmAnyCallbackStruct *callData);
static void LayoutDesktopIcons(FileMgrRec *file_mgr_rec,
                               FileMgrData *file_mgr_data,
                               FileViewData **order_list, int order_count,
                               Boolean turn_off_hourglass);
static XmGadget InputInGadget(Widget w, int x, int y);
static XmGadget InputForGadget(Widget cw, int x, int y);
static ObjectPtr GetBottomOfStack(FileMgrData *file_mgr_data);
...
#define YSPACING(fmd)                                                          \
  (fmd->layout_data                                                            \
       ? (fmd->view != BY_NAME                                                 \
              ? -((IconLayoutData *)fmd->layout_data)->highlight               \
...
static struct TreePx {
  ...
  Pixmap px[3]; /*   small, medium, and large pixmap */
    ...
    {"Dttvnor", 0}, {"Dttverr", 0}, {"Dttvmor", 0}, {"Dttvlss", 0},
    {"Dttvbth", 0}, {"Dttvemp", 0}, {"Dttvnil", 0},
};
...
static Boolean SpecialCases(FileViewData **t1, FileViewData **t2, int *rc) {
  /* Tree mode */
  ...
  if (FMStrcoll((*t2)->file_data->file_name, ".") == 0 ||
      FMStrcoll((*t2)->file_data->file_name, "..") == 0) {
    *rc = 1;
    return True;
  }
...
static int FileNameAscending(const void *t1, const void *t2) {
  int rc;
  FileViewData **f1 = (FileViewData **)t1;
  FileViewData **f2 = (FileViewData **)t2;

  if (SpecialCases(f1, f2, &rc))
    return rc;

  if ((*f1)->file_data->action_name == NULL) {
    if ((*f2)->file_data->action_name == NULL)
      rc = FMStrcoll((*f1)->file_data->file_name, (*f2)->file_data->file_name);
    else
      rc =
          FMStrcoll((*f1)->file_data->file_name, (*f2)->file_data->action_name);
  } else {
    if ((*f2)->file_data->action_name != NULL)
      rc = FMStrcoll((*f1)->file_data->action_name,
                     (*f2)->file_data->action_name);
    else
      rc =
          FMStrcoll((*f1)->file_data->action_name, (*f2)->file_data->file_name);
  }
...
static int FileTypeAscending(const void *t1, const void *t2) {
  ...
  rc = strcoll((*f1)->file_data->logical_type, (*f2)->file_data->logical_type);
  if (rc == 0)
...
static int FileSizeAscending(const void *t1, const void *t2) {
  ...
  if ((*f1)->file_data->stat.st_size < (*f2)->file_data->stat.st_size)
    return -1;

  else if ((*f1)->file_data->stat.st_size > (*f2)->file_data->stat.st_size)
    return 1;
...
static int FileDateAscending(const void *t1, const void *t2) {
  ...
  if ((*f1)->file_data->stat.st_mtime < (*f2)->file_data->stat.st_mtime)
    return -1;
...
void OrderFiles(FileMgrData *file_mgr_data, DirectorySet *directory_set) {
  FileViewData **file_view_data;
  int file_count;
  FileViewData **order_list;
  int (*sort)(const void *, const void *) = NULL;
  int (*sub_sort)(const void *, const void *) = NULL;
  int i;
  int start;
  ...
  if (file_mgr_data->order == ORDER_BY_FILE_TYPE) {
    if (file_mgr_data->direction == DIRECTION_ASCENDING) {
      sort = FileTypeAscending;
      sub_sort = FileNameAscending;
    } else {
      sort = FileTypeDescending;
      sub_sort = FileNameDescending;
    }
  } else if (file_mgr_data->order == ORDER_BY_ALPHABETICAL) {
    if (file_mgr_data->direction == DIRECTION_ASCENDING)
      sort = FileNameAscending;
    else
      sort = FileNameDescending;
  } else if (file_mgr_data->order == ORDER_BY_DATE) {
    if (file_mgr_data->direction == DIRECTION_ASCENDING)
      sort = FileDateAscending;
    else
      sort = FileDateDescending;

  } else if (file_mgr_data->order == ORDER_BY_SIZE) {
    if (file_mgr_data->direction == DIRECTION_ASCENDING)
      sort = FileSizeAscending;
    else
      sort = FileSizeDescending;
  } else {
    /* Unknown sort order? */
    return;
  }
  ...
  qsort(order_list, file_count, sizeof(FileViewData *), sort);
  ...
  if (sub_sort != NULL) {
    ...
    while (i < file_count) {
      if (order_list[start]->file_data->logical_type !=
        ...
        qsort(order_list + start, i - start, sizeof(FileViewData *), sub_sort);
        start = i;
      }
...
void FilterFiles(FileMgrData *file_mgr_data, DirectorySet *directory_set) {
  FileViewData **file_view_data;
  ...
  for (i = 0; i < directory_set->file_count; i++) {
    ...
    /* field set to false                                            */
    if ((_DtCheckForDataTypeProperty(file_view_data[i]->file_data->logical_type,
                                     "invisible")) &&
    ...
    if (file_view_data[i]->file_data->file_name[0] == '.') {
      ...
      if (file_mgr_data->restricted_directory != NULL) {
        if ((strcmp(file_mgr_data->restricted_directory,
             ...
             (strcmp(file_view_data[i]->file_data->file_name, ".") == 0 ||
              strcmp(file_view_data[i]->file_data->file_name, "..") == 0)) ||
            (strncmp(file_mgr_data->restricted_directory,
    ...
    matches = False;
    if (strcmp(filter, "") != 0 && file_mgr_data != trashFileMgrData &&
        ...
        if (fnmatch((const char *)filter,
          ...
          if (mustMatch) {
            ...
            matches = True;
                         ...
                         (const char *)file_view_data[i]->file_data->file_name,
                         0) == 0) {
        if (mustMatch) {
          filterCount++;
          file_view_data[i]->filtered = True;
...
void GetAncestorInfo(FileMgrData *file_mgr_data, FileViewData *ip, int *levelp,
                     char *path, Bool *morep)
/*
...
void GetFullName(FileMgrData *file_mgr_data, FileViewData *ip, char *path) {
  GetAncestorInfo(file_mgr_data, ip, NULL, path, NULL);
}

void GetDirName(FileMgrData *file_mgr_data, FileViewData *ip, char *path) {
  char *p;
...
void GetLevel(FileViewData *ip, int *levelp) {
  int level = 0;
...
static void CopyOrderedEntries(FileViewData *ip, FileViewData *fvd_array[],
                               int *index)
/*
...
void FlattenTree(FileMgrData *file_mgr_data, FileViewData ***file_view_data,
                 int *file_count) {
  int i;
...
static Bool IsButtonOrMotion(Display *disp, XEvent *ep, char *arg) {
  return (ep->type == MotionNotify || ep->type == ButtonRelease);
}

void IconCallback(Widget w, XtPointer clientData, XtPointer callData) {
  ...
  if (adjustState == -1) {
    if (XrmGetResource(XrmGetDatabase(XtDisplay(w)), "*enableBtn1Transfer",
                       "*EnableBtn1Transfer", &str_type_return,
                       &value_return) &&
  ...
  run_default_action:
    ...
    if (event->type == ButtonPress) {
      ...
      XGrabPointer(XtDisplay(w), root, False,
                   ButtonReleaseMask | PointerMotionMask, GrabModeAsync,
                   GrabModeAsync, None, loc_cursor1, CurrentTime);
      do {
        XIfEvent(XtDisplay(w), &bevent, IsButtonOrMotion, NULL);
        if (!dragged && bevent.type == MotionNotify) {
          dx = event->x_root - bevent.xmotion.x_root;
          if (dx < 0)
            dx = -dx;
          dy = event->y_root - bevent.xmotion.y_root;
          if (dy < 0)
            dy = -dy;
          if (dx > DBLCLICK_DRAG_THRESHOLD || dy > DBLCLICK_DRAG_THRESHOLD) {
            ...
            XGrabPointer(XtDisplay(w), root, False,
                         ButtonReleaseMask | PointerMotionMask, GrabModeAsync,
                         GrabModeAsync, None, loc_cursor2, CurrentTime);
            dragged = True;
          }
        }
      ...
      if ((openDirType == NEW && strcmp(command, openInPlace) == 0) ||
        ...
        if (command) {
          if ((fileMgrData->show_type == MULTIPLE_DIRECTORY || dragged) &&
            ...
            RunCommand(command, fileMgrData, fileViewData, NULL, NULL, NULL);
        } else
          Error = True;
      }
    }
...
static int GetSelectedCount(FileViewData *fileViewData,
                            FileMgrData *fileMgrData, DesktopRec *desktopRec,
                            int *dt) {
  int selectedCount = 0;
  char *wsName;
...
static Pixmap GetDragIconMask(Widget w, unsigned int wid, unsigned int hei) {
  Pixmap dragMask;
  ...
  dragMask =
      XCreatePixmap(dpy, RootWindowOfScreen(XtScreenOfObject(w)), wid, hei, 1);
  ...
  if (flags & XmPIXMAP_RECT)
    XFillRectangle(
        dpy, dragMask, fillGC, pRect.x - minX + shadowThickness + marginWidth,
        pRect.y - minY + shadowThickness + marginHeight,
        pRect.width - 2 * marginWidth, pRect.height - 2 * marginHeight);
  if (flags & XmLABEL_RECT) {
    XFillRectangle(
        dpy, dragMask, fillGC, lRect.x - minX + shadowThickness + marginWidth,
        lRect.y - minY + shadowThickness + marginHeight,
        lRect.width - 2 * marginWidth, lRect.height - 2 * marginHeight);
  }
  XFreeGC(dpy, fillGC);
...
static Widget GetDragIcon(Widget w, FileViewData *fileViewData) {
  ...
  if (initiating_view &&
    ...
    fileNameString =
        XmStringCreateLocalized(fileViewData->file_data->file_name);
    wid = XmStringWidth(fontList, fileNameString) + g->icon.pixmap_width +
          g->icon.cache->margin_width + g->icon.cache->spacing +
          G_ShadowThickness(g) + G_HighlightThickness(g);
  ...
  n = 0;
  XtSetArg(args[n], XmNhotX, 0);
  n++;
  XtSetArg(args[n], XmNhotY, 0);
  n++;
  XtSetArg(args[n], XmNwidth, wid);
  n++;
  XtSetArg(args[n], XmNheight, hei);
  n++;
  XtSetArg(args[n], XmNmaxWidth, wid);
  n++;
  XtSetArg(args[n], XmNmaxHeight, hei);
  n++;
  XtSetArg(args[n], XmNdepth, d);
  n++;
  XtSetArg(args[n], XmNpixmap, dragPixmap);
  n++;
  XtSetArg(args[n], XmNmask, dragMask);
  n++;
  XtSetArg(args[n], XmNforeground, mw->core.background_pixel);
  n++;
  XtSetArg(args[n], XmNbackground, mw->manager.foreground);
  n++;
  dragIcon = XtCreateWidget("drag_icon", xmDragIconObjectClass, screen_object,
                            args, n);
...
static void NewConvertDelete(Widget w, FileViewData *fileViewData,
                             char **fileList, int numFiles) {
  ...
  if (child_pid == -1) {
    ...
    tmpStr = GETMESSAGE(
        11, 59, "Cannot create a child process to delete the dropped files.");
...
static void NewConvertFileName(Widget w, FileViewData *fileViewData,
                               char **fileList, Cardinal *numFiles) {
  FileMgrData *fileMgrData;
  DesktopRec *desktopRec;
  int selectedCount;
  int dt, i, count;
...
/* ARGSUSED */
static void FileConvertCB(Widget w, XtPointer client, XtPointer call) {
  ...
  if (cb->reason == DtCR_DND_CONVERT_DATA) {
    ...
    NewConvertFileName(w, fileViewData, cb->dragData->data.files,
                       &cb->dragData->numItems);
  } else if (cb->reason == DtCR_DND_CONVERT_DELETE) {
    NewConvertDelete(w, fileViewData, cb->dragData->data.files,
                     cb->dragData->numItems);
  }
}
...
/* ARGSUSED */
static void DragFinishCB(Widget w, XtPointer client, XtPointer call) {
...
/* ARGSUSED */
void StartDrag(Widget w, FileViewData *fileViewData, XEvent *event) {
  static XtCallbackRec fileConvertCB[] = {{FileConvertCB, NULL}, {NULL, NULL}};
  static XtCallbackRec dragFinishCB[] = {{DragFinishCB, NULL}, {NULL, NULL}};
  static XtCallbackRec dropOnRootCB[] = {{DropOnRootCB, NULL}, {NULL, NULL}};
  Widget drag_icon;
  Arg args[2];
  int numArgs;
  ...
  numArgs = 0;
  XtSetArg(args[numArgs], DtNsourceIcon, drag_icon);
  numArgs++;
  XtSetArg(args[numArgs], DtNdropOnRootCallback, dropOnRootCB);
  numArgs++;
  ...
  if (DtDndDragStart(w, event, DtDND_FILENAME_TRANSFER, selectedCount,
                     operations, fileConvertCB, dragFinishCB, args,
                     numArgs) == NULL) {
...
void FileIconMotion(Widget w, XtPointer clientData, XEvent *event) {
  int diffX, diffY;
...
static void PositionDesktopIcon(int drop_x, int drop_y, int *root_x,
                                int *root_y)

{
  ...
  if (desktop_data != NULL &&
    ...
    pixmap_offset_x = pixmap_offset_y =
        3 + 1 + G_HighlightThickness(g) + G_ShadowThickness(g);
...

{
  ...
  RegisterInGrid((int)width, (int)height, desktopRec->root_x,
                 desktopRec->root_y, desktopRec->workspace_num, False);
...
Boolean FileIsSelected(FileMgrData *file_mgr_data,
                       FileViewData *file_view_data) {
...
static void SetFileSelected(FileMgrData *file_mgr_data,
                            FileViewData *file_view_data) {
  /* mark selected */
  ...
  if (!file_view_data->need_update) {
    if (file_view_data->file_data->link == NULL)
      SetToSelectColors(
          ...
          (Widget)((FileMgrRec *)file_mgr_data->file_mgr_rec)->file_window, 0);
    else
      SetToSelectColors(
          file_view_data->widget,
...
static void SetFileUnselected(FileMgrData *file_mgr_data,
                              FileViewData *file_view_data) {
  /* mark selected */
  ...
  if (!file_view_data->need_update) {
    if (file_view_data->file_data->link != NULL)
      SetToNormalColors(
          ...
          ((FileMgrRec *)(file_mgr_data->file_mgr_rec))->main, LINK_FILE);
    else
      SetToNormalColors(
          file_view_data->widget,
          ((FileMgrRec *)(file_mgr_data->file_mgr_rec))->file_window,
          ((FileMgrRec *)(file_mgr_data->file_mgr_rec))->main, 0);
...
void SelectFile(FileMgrData *file_mgr_data, FileViewData *file_view_data) {
  int selection_count;
  int i;
  ...
  file_mgr_data->selection_list = (FileViewData **)XtRealloc(
      (char *)file_mgr_data->selection_list,
...
void DeselectFile(FileMgrData *file_mgr_data, FileViewData *file_view_data,
                  Boolean valid) {
  int selection_count;
  int i;
  int j;
  ...
  file_mgr_data->selection_list =
      (FileViewData **)XtRealloc((char *)file_mgr_data->selection_list,
...
void DeselectAllFiles(FileMgrData *file_mgr_data) {
  FileViewData *file_view_data;
  int i, j, k;
  ObjectPtr bottom;
  ...
  if (!PositioningEnabledInView(file_mgr_data)) {
    ...
    repaint_list = (FileViewData **)XtMalloc(
...
void SelectAllFiles(FileMgrData *file_mgr_data) {
  ...
  if (PositioningEnabledInView(file_mgr_data)) {
    ...
    for (i = 0; i < file_mgr_data->num_objects; i++) {
      ...
      */
      if (top->file_view_data != NULL && strcmp(top->name, "..") != 0) {
...
void SetToSelectColors(Widget widget, Widget file_window, int type) {
  int j;
  ...
  j = 0;
  XtSetArg(args[j], XmNbackground, &background_color);
  j++;
  XtGetValues(file_window, args, j);

  j = 0;
  if (background_color == white_pixel) {
    XtSetArg(args[j], XmNbackground, black_pixel);
    j++;
    XtSetArg(args[j], XmNforeground, white_pixel);
    j++;
  } else if (background_color == black_pixel) {
    XtSetArg(args[j], XmNbackground, white_pixel);
    j++;
    XtSetArg(args[j], XmNforeground, black_pixel);
    j++;
  } else {
    XtSetArg(args[j], XmNbackground, white_pixel);
    j++;
...
void SetToNormalColors(Widget widget, Widget bg_fg_colors,
                       Widget top_shadow_colors, int type) {
  int j;
  Pixel background_color;
  ...
  XtSetArg(args[j], XmNbackground, &background_color);
  j++;
  XtSetArg(args[j], XmNforeground, &foreground_color);
  j++;
  XtGetValues(bg_fg_colors, args, j);

  j = 0;
  XtSetArg(args[j], XmNtopShadowColor, &pixmap_background);
  j++;
...
#ifdef _SHOW_LINK
  if (type == LINK_FILE)
    XtSetArg(args[j], XmNforeground, pixmap_background);
  j++;
  else
#endif
      XtSetArg(args[j], XmNforeground, foreground_color);
...
void DestroyIconName(Widget w, XtPointer client_data, XtPointer call_data) {
  char *str;
  Arg args[1];
...
int GetInsertPosition(int x1, int x2, XmFontList fontList, char *name) {
...
void CreateNameChangeDialog(Widget w, FileViewData *file_view_data,
                            XtPointer client_data, int type) {
  ...
  if (type != DESKTOP && file_mgr_data == trashFileMgrData)
    return;
  ...
  if (DtDtsDataTypeIsAction(file_view_data->file_data->logical_type)) {
    ...
    tmpStr = GETMESSAGE(10, 40,
                        "This object is an Action.\nAction icon labels cannot "
                        "be directly renamed.");
    msg = XtNewString(tmpStr);
  ...
  */
  {
    ...
    n = 0;
    XtSetArg(args[n], XmNuserData, name);
    n++;
    XtSetArg(args[n], XmNmarginHeight, 0);
    n++;
    XtSetArg(args[n], XmNmarginWidth, 0);
    n++;
    XtSetArg(args[n], XmNvalue, name);
    n++;
    XtSetArg(args[n], XmNwidth, stringWidth);
    n++;
  }
  ...
  if (type == DESKTOP) {
    ...
    if (file_mgr_data) {
      ...
      XtAddCallback(text, XmNmotionVerifyCallback,
                    (XtCallbackProc)ChangeIconName, (XtPointer)file_mgr_data);
      XtAddCallback(text, XmNmodifyVerifyCallback,
                    (XtCallbackProc)ChangeIconName, (XtPointer)file_mgr_data);
      XtAddCallback(text, XmNactivateCallback, (XtCallbackProc)ChangeIconName,
...

  {
    ...
    XQueryPointer(XtDisplay(parent), XtWindow(file_view_data->widget),
                  &rootWindow, &childWindow, &rootX, &rootY, &pX, &pY, &mask);
    if (pX == 0)
    ...
    n = 0;
    XtSetArg(args[n], XmNcursorPosition, insertPosition);
    n++;
  ...
  if (shapeExtension)
    if (type == DESKTOP) {
      ...
      _DtIconGetIconRects((DtIconGadget)desktopWindow->iconGadget, &flags,
                          &rect[0], &rect[1]);
      /* 1 */
      rect[0].x += 1;
      rect[0].y += 1;
      ...
      XShapeCombineRectangles(XtDisplay(desktopWindow->shell),
                              XtWindow(desktopWindow->shell), ShapeBounding, 0,
                              0, &rect[0], 2, ShapeSet, Unsorted);
    }
#endif
  XtManageChild(text);
...
void SavePositionalData(int fd, FileMgrData *file_mgr_data, char **name_list,
                        char *name) {
  int i;
  ...
  if (file_mgr_data->positionEnabled == RANDOM_ON &&
    ...
    for (i = 0; i < file_mgr_data->num_objects; i++) {
      ...
      fprintf(fd_stream, "*%s.%s.%s.%s%d:   %s %d %d %d\n", name_list[0],
              name_list[1], name, "object", i, ptr->name, ptr->x, ptr->y,
              ptr->stacking_order);
    }
  } else {
    /* Number of object positions */
...
void RestorePositionalData(XrmDatabase db, char **name_list,
                           FileMgrData *file_mgr_data, char *name) {
  XrmName xrm_name[10];
  int i = 0;
  int j;
  char objectName[20];
  ...
  if (XrmQGetResource(db, xrm_name, NULL, &rep_type, &value)) {
    if ((num_objects = atoi(value.addr)) <= 0)
      ...
      file_mgr_data->object_positions =
          (ObjectPosition **)XtMalloc(sizeof(ObjectPosition *) * num_objects);
    }
  }
...
void UnpostTextField(FileMgrData *file_mgr_data)
...
void UnpostTextPath(FileMgrData *file_mgr_data)
...
void RedrawOneGadget(Widget child, XEvent *event, Region region)
...
void RepaintDesktop(FileMgrData *file_mgr_data, XEvent *event, Region region)
...

{
   ...
   */
  while (
      XCheckWindowEvent(XtDisplay(w), XtWindow(w), ExposureMask, &expEvent)) {
...
void DrawingAreaRedisplay(Widget wid, XEvent *event, Region region)
...
ObjectPtr GetBottomOfStack(FileMgrData *file_mgr_data)
...
ObjectPtr GetTopOfStack(FileMgrData *file_mgr_data) {
  int i;
  ObjectPtr top;
...
void RepositionUpInStack(FileMgrData *file_mgr_data, int oldPos, int newPos)

{
  ...
  file_window = (XmManagerWidget)(((FileMgrRec *)file_mgr_data->file_mgr_rec)
                                      ->file_window);

  /* Find the affect child */
  for (i = 0;
       (i < file_window->composite.num_children) &&
...
ObjectPtr FindCurrentPosition(FileMgrData *file_mgr_data, char *file_name)
...
void RepairStackingPointers(FileMgrData *file_mgr_data)
...
void OrderChildrenList(FileMgrData *file_mgr_data)
...
void SetHotRects(FileViewData *file_view_data, XtCallbackProc callback,
...

{
  ...
  if (file_view_data->displayed) {
    ...
    operations = TypeToDropOperations(file_view_data->file_data->logical_type);
    ...
    if (operations) {
      ...
      if (numRects) {
        XtSetArg(args[n], XmNdropRectangles, rects);
        n++;
        XtSetArg(args[n], XmNnumDropRectangles, numRects);
        n++;
      }
      XtSetArg(args[n], XmNdropSiteOperations, operations);
      n++;
      ...
      XtAddCallback(file_view_data->widget, XmNdropCallback, callback,
                    callback_data);
    } else {
      /*********************************************/
...
unsigned char TypeToDropOperations(char *file_type)

{
  ...
  if (action = DtDtsDataTypeToAttributeValue(file_type, DtDTS_DA_MOVE_TO_ACTION,
                                             NULL)) {
    operations = operations | XmDROP_MOVE;
    DtDtsFreeAttributeValue(action);
  }
  if (action = DtDtsDataTypeToAttributeValue(file_type, DtDTS_DA_COPY_TO_ACTION,
                                             NULL)) {
    operations = operations | XmDROP_COPY;
    DtDtsFreeAttributeValue(action);
  }
  if (action = DtDtsDataTypeToAttributeValue(file_type, DtDTS_DA_LINK_TO_ACTION,
                                             NULL)) {
...
char *TypeToAction(unsigned char dropOperation, char *logical_type) {
  char *action;
  ...
  case XmDROP_MOVE:
    action = DtDtsDataTypeToAttributeValue(logical_type,
                                           DtDTS_DA_MOVE_TO_ACTION, NULL);
    break;
  case XmDROP_COPY:
    action = DtDtsDataTypeToAttributeValue(logical_type,
                                           DtDTS_DA_COPY_TO_ACTION, NULL);
    break;
  case XmDROP_LINK:
    action = DtDtsDataTypeToAttributeValue(logical_type,
                                           DtDTS_DA_LINK_TO_ACTION, NULL);
    break;
  default:
    action = NULL;
    break;
...
static Boolean IsDesktopPtr(FileViewData *fileViewData,
  ...
  *fileMgrData = (FileMgrData *)((DirectorySet *)fileViewData->directory_set)
                     ->file_mgr_data;
  return (False);
}
 ...
 *
 ********************************************************************/
void PositionFileView(FileViewData *file_view_data,
                      FileMgrData *file_mgr_data) {
  Position x, y;
  Arg args[1];
  ...
  p = XtVaCreateManagedWidget(
      "positionicon", xmGadgetClass, file_mgr_rec->file_window,
      XmNmappedWhenManaged, False, XmNx, file_view_data->x, XmNy, y, XmNwidth,
      file_mgr_data->grid_width, XmNheight, file_mgr_data->grid_height, NULL);

  XmScrollVisible(file_mgr_rec->scroll_window, p, XSPACING,
                  YSPACING(file_mgr_data));

  XtDestroyWidget(p);

  XmScrollBarGetValues(file_mgr_rec->horizontal_scroll_bar, &value, &size,
                       &increment, &page);
  x = -((Position)value);

  XmScrollBarGetValues(file_mgr_rec->vertical_scroll_bar, &value, &size,
                       &increment, &page);
  y = -((Position)value);
...
static Boolean InMultipleObjectRegion(FileMgrData *file_mgr_data,
                                      FileViewData *fileViewData) {
  int i;
  for (i = 0; i < file_mgr_data->selected_file_count; ++i)
    if (strcmp(file_mgr_data->selection_list[i]->file_data->file_name,
               fileViewData->file_data->file_name) == 0)
      return True;
  return False;
}

void FmPopup(Widget w, XtPointer client_data, XEvent *event,
             FileMgrData *file_mgr_data) {
  FileMgrRec *file_mgr_rec;
  Arg args[2];
  FileViewData *fileViewData = NULL;
  ...
  if ((w == NULL) && (client_data == NULL)
    ...
    if (file_mgr_data == trashFileMgrData) {
      ...
      label_string =
          XmStringCreateLocalized((GETMESSAGE(33, 2, "Current Folder")));
      ...
      if (fileViewData != NULL) {
        ...
        UpdateActionMenuPane((XtPointer)fileViewData, file_mgr_rec,
                             fileViewData->file_data->logical_type, FM_POPUP,
                             num_of_children, fileMgrPopup.menu,
  ...
  /* we are dealing with an object popup */
    ...
    if (file_mgr_data == trashFileMgrData) {
      ...
      if (file_mgr_data->selected_file_count > 1 &&
        ...
        label_string =
            XmStringCreateLocalized((GETMESSAGE(33, 1, "Multiple Objects")));
      } else {
        char *tmp_label;
      ...
      if (file_mgr_data->selected_file_count > 1 &&
        ...
        XtRemoveAllCallbacks(fileMgrPopup.objPopup[BTN_HELP],
                             XmNactivateCallback);
        XtAddCallback(fileMgrPopup.objPopup[BTN_HELP], XmNactivateCallback,
                      ObjectHelp, (XtPointer)fileViewData->widget);
  ...
  if (event == NULL) {
    ...
    x = file_mgr_rec->shell->core.x + file_mgr_rec->scroll_window->core.x +
        w->core.x + gWidth / 2;
    ;

    y = file_mgr_rec->shell->core.y + file_mgr_rec->header_frame->core.y +
        file_mgr_rec->header_frame->core.height +
        file_mgr_rec->scroll_window->core.y + w->core.y + gHeight / 2;
    ;
...
static void DropOnRootCB(Widget w, XtPointer client_data, XtPointer call_data) {
  ...
  if (fileList->reason != DtCR_DND_DROP_ANIMATE) {
    ...
    if (initiating_view != NULL) {
      file_mgr_rec =
          (FileMgrRec *)(((FileMgrData *)initiating_view)->file_mgr_rec);
    ...
    if (numFiles > 1) {
      ...
      effScreenHeight =
          HeightOfScreen(XtScreen(w)) - (4 + 4 + 48 + 3 + 15 + 4 + 5);
    ...
    if (file_mgr_data == NULL) {
          ...
          if (DTFileIsSelected(
            ...
            for (j = 0; j < desktop_data->numWorkspaces; j++)
              if (desktop_data->workspaceData[j]->number ==
                ...
                break;
            wsNum = j;
            for (j = 0; j < desktop_data->workspaceData[wsNum]->files_selected;
                 j++) {
              RelocateDesktopIcon(
                  desktop_data->workspaceData[wsNum]->selectedDTWindows[j],
                  root_x, root_y);
              root_x += step;
              root_y += step;
            ...
            RelocateDesktopIcon(desktop_data->desktopWindows[i], root_x,
                                root_y);
            break;
          }
        }
      ...
      */
      ...
      tmpStr = (GETMESSAGE(
          11, 37,
          "You can't drop files from\nthe Trash Can on to the Workspace.\nTo "
          "remove an object from Trash,\n   -Select the object, and\n   "
          "-Choose 'Put Back'\nfrom the File menu or the object's popup "
          "menu.\nYou can then drag the object out of File Manager and drop it "
    ...
    } else /* it was dropped on the root window so lets process it */
    {
      ...
      for (i = 0; i < numFiles; i++) {
        ...
        if (strcmp(file_mgr_data->current_directory, file_set[i]) == 0) {
          SetupDesktopWindow(XtDisplay(file_mgr_rec->shell), file_mgr_data,
                             file_mgr_rec, ".", host_set[i], file_set[i],
                             root_x, root_y, NULL, EndIndex);
        } else if (strncmp(file_mgr_data->current_directory, file_set[i],
                           strlen(file_set[i])) == 0) {
          SetupDesktopWindow(XtDisplay(file_mgr_rec->shell), file_mgr_data,
                             file_mgr_rec, "..", host_set[i],
                             file_mgr_data->current_directory, root_x, root_y,
                             NULL, EndIndex);
        } else {
          *ptr = '\0';
          if (*(file_set[i]) == 0) {
            SetupDesktopWindow(XtDisplay(file_mgr_rec->shell), file_mgr_data,
                               file_mgr_rec, ptr + 1, host_set[i], "/", root_x,
                               root_y, NULL, EndIndex);
          } else {
            SetupDesktopWindow(XtDisplay(file_mgr_rec->shell), file_mgr_data,
                               file_mgr_rec, ptr + 1, host_set[i], file_set[i],
                               root_x, root_y, NULL, EndIndex);
          }
        }
        root_x += step;
...
void FreeLayoutData(XtPointer p) {
...
void UnmanageFileIcons(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
                       FileViewData *file_view_data) {
  XmManagerWidget file_window;
  FileViewData **order_list;
  int order_count;
...
void UpdateFileIcons(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
                     Boolean new_directory) {
  _UpdateFileIcons(file_mgr_rec, file_mgr_data, new_directory, NULL);
}

void AddFileIcons(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
                  DirectorySet *add_dir_set) {
...
static int WidgetCmp(const void *v1, const void *v2) {
  Widget w1 = *(Widget *)v1;
  Widget w2 = *(Widget *)v2;
  return (w1 > w2) - (w1 < w2);
}
    ...
    for (i = 0; i < order_count; i++) {
      if (order_list[i]->filtered &&
        ...
        if (order_list[i]->treebtn)
          del_btn++;
#endif
        order_list[i]->widget = order_list[i]->treebtn = NULL;
      } else {
        if (order_list[i]->widget) {
          p = bsearch(&order_list[i]->widget, sorted_chilren, num_children,
                      sizeof(Widget), WidgetCmp);
          if (p) {
        ...
        if (order_list[i]->treebtn) {
          p = bsearch(&order_list[i]->treebtn, sorted_chilren, num_children,
                      sizeof(Widget), WidgetCmp);
          if (p) {
            /* don't reuse this widget for any other file */
            reuse[p - sorted_chilren] = False;
...
static void UpdateOneIconLabel(FileMgrData *file_mgr_data,
                               FileViewData *file_view_data) {
  char *label;
  char *s;
  ...
  if (file_mgr_data->view != BY_ATTRIBUTES) {
    if (strcmp(file_view_data->file_data->file_name, "..") == 0) {
      ...
      /* label = "name/" */
      label =
          (char *)XtMalloc(strlen(file_view_data->file_data->file_name) + 2);
...
static void UpdateOneFileIcon(FileMgrRec *file_mgr_rec,
                              FileMgrData *file_mgr_data,
                              FileViewData *file_view_data) {
  XmString icon_label;
  char *logical_type;
  ...
  if (file_mgr_data->view == BY_NAME)
    pixmapData = NULL;
           ...
           file_view_data->file_data->physical_type == DtDIRECTORY) {
    pixmapData = CheckForOpenDirectory(file_view_data, directory_set,
                                       file_mgr_data, logical_type);
  } else {
    if (file_mgr_data->view == BY_NAME_AND_ICON)
      pixmapData = _DtRetrievePixmapData(
          logical_type, file_view_data->file_data->file_name,
          directory_set->name, (Widget)file_window, LARGE);
    else
      pixmapData = _DtRetrievePixmapData(
          logical_type, file_view_data->file_data->file_name,
          directory_set->name, (Widget)file_window, SMALL);
  }
  ...
  if (is_instance_icon) {
    if (file_view_data->icon_mtime !=
        file_view_data->file_data->stat.st_mtime) {
  ...
  if (layout_data->background == white_pixel) {
    if (file_view_data->selected) {
      ...
      n++;
      XtSetArg(args[n], XmNforeground, layout_data->foreground);
      n++;
    }
    ...
    if (file_view_data->selected) {
      ...
      XtSetArg(args[n], XmNbackground, layout_data->background);
      n++;
      XtSetArg(args[n], XmNforeground, layout_data->foreground);
      n++;
      XtSetArg(args[n], XmNpixmapBackground, layout_data->pixmap_back);
      n++;
      XtSetArg(args[n], XmNpixmapForeground, layout_data->pixmap_fore);
      n++;
    }
  }
  ...
  if (icon_widget) {
    ...
    if (layout_data->dup_icon_widget == NULL) {
      ...
      n++;
#endif
      XtSetArg(args[n], XmNx, -999);
      n++;
      XtSetArg(args[n], XmNy, -999);
      n++;
      ...
      icon_widget = _DtDuplicateIcon(
          (Widget)file_window, layout_data->dup_icon_widget, icon_label,
    ...
    XtAddCallback(icon_widget, XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
                  NULL);
    file_view_data->registered = False;
  }

  if (file_mgr_data->view != BY_NAME)
    _DtCheckAndFreePixmapData(logical_type, (Widget)file_window,
                              (DtIconGadget)icon_widget, pixmapData);
  ...
  if (file_mgr_data->show_type != MULTIPLE_DIRECTORY ||
    ...
    n++;
    XtSetArg(args[n], XmNlabelPixmap, px);
    n++;
    XtSetArg(args[n], XmNbackground, layout_data->background);
    n++;
    XtSetArg(args[n], XmNtraversalOn, False);
    ...
    XtSetArg(args[n], XmNmarginHeight, 0);
    n++;
    XtSetArg(args[n], XmNuserData, file_mgr_data);
    n++;
    XtSetArg(args[n], XmNx, -999);
    n++;
    XtSetArg(args[n], XmNy, -999);
    ...
    if (btn_widget) {
      ...
      btn_widget =
          XmCreatePushButtonGadget((Widget)file_window, "tree_button", args, n);
    }
...
static void _UpdateFileIcons(FileMgrRec *file_mgr_rec,
  ...
  DPRINTF(("_UpdateFileIcons(\"%s\", new_dir %c, add_dir_set %p) ...\n",
           file_mgr_data->current_directory, new_directory ? 'T' : 'F',
           add_dir_set));
  ...
  if (XtIsManaged(file_mgr_rec->file_window)) {
    ...
    DPRINTF(("  focus_widget = %p (%s)\n", file_mgr_rec->focus_widget,
             file_mgr_rec->focus_widget ? XtName(file_mgr_rec->focus_widget)
                                        : "nil"));
  ...
  MakeReuseList(file_window->composite.children,
                file_window->composite.num_children, order_list, order_count,
                &layout_data->reuse_icons, &layout_data->reuse_btns);
...
static void CreateTreeIcons(Widget w) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0; j < tpxN; j++) {
      ...
      TreePxTab[j].px[i] =
          _DtGetPixmap(XtScreen(w), pxname, foreground_color, background_color);
      width = height = 0;
...
Pixmap GetTreebtnPixmap(FileMgrData *file_mgr_data,
                        FileViewData *file_view_data) {
...
static void GetIconLayoutParms(FileMgrRec *file_mgr_rec,
                               FileMgrData *file_mgr_data, IconLayoutData *ld) {
  DirectorySet *directory_set;
  int file_count;
  ...
  if (g) {
    ...
    ld->width =
        ((Widget)g)->core.width + ld->pixmap_width - g->icon.pixmap_width;
    ld->height =
        ((Widget)g)->core.height + ld->pixmap_height - g->icon.pixmap_height;
    ...
    if (file_mgr_data->view == BY_NAME_AND_ICON &&
      ...
      ld->width =
          2 * ld->margin + ld->pixmap_width + ld->spacing + ld->char_width;
...
static void EstimateIconSize(FileMgrRec *file_mgr_rec,
                             FileMgrData *file_mgr_data,
                             IconLayoutData *layout_data,
                             FileViewData *file_view_data, Dimension *width,
                             Dimension *height) {
  int label_len;
  int label_width;
...
static Dimension GetExtraHeight(FileMgrData *file_mgr_data,
                                FileViewData *file_view_data,
                                int treebtn_size) {
  if (showEmptySet && file_view_data->file_data->is_subdir &&
      file_view_data->ts >= tsDirs && file_view_data->ndir == 0 &&
      (file_view_data->nfile == 0 ||
       file_mgr_data->tree_files == TREE_FILES_NEVER)) {
...
void EraseTreeLines(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
                    FileViewData *file_view_data) {
  XmManagerWidget file_window;
  ...
  DPRINTF2(("EraseTreeLines(\"%s\"): x/y %d/%d (widget %d/%d)\n",
            ...
            file_view_data->widget->core.x, file_view_data->widget->core.y));

  /* clear area from icon widget to bottom of file window */
  XClearArea(XtDisplay(file_window), XtWindow(file_window), x, y, fw_width - x,
             fw_height - y, False);
  ...
  if (y > 0 && (Dimension)(x + file_mgr_data->grid_width) < fw_width) {
    ...
    XClearArea(XtDisplay(file_window), XtWindow(file_window), x, 0,
               fw_width - x, y, False);
  }
}
...
void RedrawTreeLines(Widget w, int ex, int ey, int ewidth, int eheight,
                     int ecount, FileMgrRec *file_mgr_rec,
                     FileMgrData *file_mgr_data) {
  ...
  for (k = 0; k < order_count; k++) {
    ...
    if (x <= ex + ewidth && x + TreeWd(level, sz) > ex && y0 <= ey + eheight &&
      ...
      for (i = 0; i < level; i++) {
        if (more[i])
          XDrawLine(XtDisplay(w), XtWindow(w), solid_gc, x + TreeLX(i, sz), y0,
                    x + TreeLX(i, sz), y1);
      }
      ...
      if (level > 0) {
        XDrawLine(XtDisplay(w), XtWindow(w), solid_gc, xl, y0, xl,
                  more[level] ? y1 : y + grid_height / 2);
          ...
          XDrawLine(XtDisplay(w), XtWindow(w),
                    (file_view_data->file_data->is_subdir &&
                     file_view_data->ts == tsNotRead)
                        ? dash_gc
                        : solid_gc,
                    xl, y + grid_height / 2, xl + TreeOffset,
                    y + grid_height / 2);
        }
      }
      ...
      if (extra_height) {
        ...
        XDrawLine(XtDisplay(w), XtWindow(w), solid_gc, xl, y1 + e_height / 2,
                  xl + TreeOffset, y1 + e_height / 2);

        xl = x + TreeWd(level, sz) + TreeBtnWd[sz];
        XCopyArea(XtDisplay(w), TreePxTab[tpxNil].px[sz], XtWindow(w), solid_gc,
                  0, 0, TreeNilWd[sz], TreeNilHt[sz], xl, y1);
        /*
                    if (empty_msg == NULL)
                    ...
                    XDrawImageString(XtDisplay(w), XtWindow(w), solid_gc,
                              xl + TreeNilWd[sz] + 2,
                              y1 + (TreeNilHt[sz] - font_height)/2 +
           font_yoffset, empty_msg, strlen(empty_msg));
        */
      }
    }
...
static Boolean ToBeManaged(IconLayoutData *layout_data,
                           FileViewData *file_view_data) {
  int i;
...
static int DisplaySomeIcons(FileMgrRec *file_mgr_rec,
                            FileMgrData *file_mgr_data, int ex, int ey, int ewd,
                            int eht, int workLimit, Boolean doAll) {
#ifdef DT_PERFORMANCE
  struct timeval update_time_s;
  struct timeval update_time_f;
#endif
  ...
  DPRINTF2(("DisplaySomeIcons(\"%s\", x/y %d/%d, wd/ht %d/%d, i %d:%d) ...\n",
  ...
  for (; k < order_count && workCount1 < workLimit; k++) {
    ...
    if (!file_view_data->displayed ||
        ...
        file_view_data->x >= ex + ewd || file_view_data->y < ey ||
  ...
  do_this_entry:
      ...
      if (PositioningEnabledInView(file_mgr_data)) {
        ...
        if (position_data->late_bind) {
          ...
          */
           ...
           */
          /*
                         position_data->y = file_view_data->y -
             child->core.height;
          */
        }
      }
    ...
    } else /* file_mgr_data->show_type == MULTIPLE_DIRECTORY */
    {
      ...
      if (file_view_data->treebtn) {
        ...
        x = file_view_data->x + TreeLX(level, layout_data->treebtn_size) +
            TreeOffset;
        y = file_view_data->y - grid_height +
            (Dimension)(grid_height - child->core.height) / (Dimension)2;

        if (child->core.x != x || child->core.y != y)
          XmeConfigureObject(child, x, y, child->core.width, child->core.height,
        ...
        for (i = 0; i < file_window->composite.num_children; i++) {
          ...
          if (XmIsTextField(child)) {
            ...
            if (!child->core.being_destroyed) {
              ...
              x = textExtent.x;
              y = textExtent.y -
                  (Dimension)(child->core.height - textExtent.height) /
                      (Dimension)2;
  ...
  for (wp = layout_data->next_icon_to_use; (child = *wp) != NULL; wp++) {
  ...
  if (!PositioningEnabledInView(file_mgr_data) &&
    ...
    for (k = 0; k < changeCount; k++) {
      SetHotRects(change[k], (XtCallbackProc)DropOnObject,
                  (XtPointer)change[k]);
    }
  }
...
static void CommitWorkProcUpdates(FileMgrRec *file_mgr_rec,
                                  FileMgrData *file_mgr_data, Boolean reset) {
  XmManagerWidget file_window;
  IconLayoutData *layout_data;
  Widget w;
  int i;
  ...
  DPRINTF(
      ("CommitWorkProcUpdates: %d+%d updates (m %d, n %d, i %d:%d, reset %d)\n",
       g_workCount1, g_workCount2, layout_data->manage_count, g_callCount,
       layout_data->i_do_next_vis, layout_data->i_do_next_all, reset));
...
static Boolean DisplayWorkProc(XtPointer client_data) {
  ...
  if (layout_data->visible_done)
    ...
    n = DisplaySomeIcons(file_mgr_rec, file_mgr_data, ex, ey, ewd, eht, n1,
                         False);
    if (n == 0) {
  ...
  if (layout_data->visible_done && n < n2) {
    n = DisplaySomeIcons(file_mgr_rec, file_mgr_data, 0, 0, 32767, 32767,
                         n2 - n, True);
  ...
  if (commit_updates) {
     ...
     */
    if (PositioningEnabledInView(file_mgr_data)) {
      for (bottom = GetBottomOfStack(file_mgr_data); bottom;
           bottom = (ObjectPosition *)bottom->prev) {
        if (bottom->file_view_data != NULL &&
...
void LayoutFileIcons(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
  ...
  fileType =
      GetDirectoryLogicalType(file_mgr_data, file_mgr_data->current_directory);
  ...
  if (!file_mgr_data->dropSite) {
    ...
    DtDndVaDropRegister((Widget)file_mgr_rec->file_window,
                        ...
                        operations, dropCB, DtNdropAnimateCallback, dropCB,
                        DtNregisterChildren, True, DtNtextIsBuffer, True,
                        XmNanimationStyle, XmDRAG_UNDER_SHADOW_IN, NULL);
  ...
  if (PositioningEnabledInView(file_mgr_data)) {
    if (file_mgr_data->object_positions) {
      LayoutDesktopIcons(file_mgr_rec, file_mgr_data, order_list, order_count,
                         turn_off_hourglass);
  ...
  } else /* show_type == SINGLE_DIRECTORY, view != BY_ATTRIBUTES */
  {
    ...
    if (tmp <= MAXWINSIZE)
      ...
      row_count = (total_icon_count + column_count - 1) / column_count;
      file_window_height =
          (Dimension)(2 * MARGIN - YSPACING(file_mgr_data) +
                      row_count * ((int)grid_height + YSPACING(file_mgr_data)));
    }

    DPRINTF(("  %d columns, %d rows (scroll_window width %d)\n", column_count,
             row_count, scrolled_window_width));

    /* assign positions to all icons */
    y = MARGIN;
    file_window_width =
        (Dimension)(2 * MARGIN - XSPACING +
                    column_count * ((int)grid_width + XSPACING));
    ...
    for (i = 0; i < row_count; i++) {
      ...
      for (j = 0; j < column_count && k < order_count;) {
        ...
        if (layout_data->alignment == XmALIGNMENT_CENTER &&
          ...
          order_list[k]->x =
              x + (Dimension)(grid_width - icon_width) / (Dimension)2;
        } else
          order_list[k]->x = x;
        order_list[k]->y = y + grid_height;
  ...
  if (VerticalScrollbarIsVisible(file_mgr_rec->vertical_scroll_bar,
                                 file_mgr_rec->scroll_window)) {
    if (file_window_width >=
        (Dimension)(scrolled_window_width - 4 - vert_scrollbar_width))
  ...
  if (VerticalScrollbarIsVisible(file_mgr_rec->vertical_scroll_bar,
                                 file_mgr_rec->scroll_window)) {
    if (file_window_width <=
        (Dimension)(scrolled_window_width - (vert_scrollbar_width + 4)))

      file_window_width = scrolled_window_width - (vert_scrollbar_width + 4);
  } else {
  ...
  if (HorizontalScrollbarIsVisible(file_mgr_rec->horizontal_scroll_bar,
    ...
    if (file_window_height <
        (Dimension)(scrolled_window_height - (horiz_scrollbar_height + pad)))
      file_window_height =
          scrolled_window_height - (horiz_scrollbar_height + pad);
  } else {
...
layout_done:
  ...
  if (file_mgr_data->scrollToThisFile) {
    ...
    for (i = 0; i < file_mgr_data->directory_count; ++i) {
      if (strcmp(((DirectorySet *)file_mgr_data->directory_set[i])->name,
                 file_mgr_data->scrollToThisDirectory) == 0) {
    ...
    if (file_view_data && file_view_data->filtered == False) {
      FileMgrRec *file_mgr_rec = (FileMgrRec *)file_mgr_data->file_mgr_rec;
...
static void TreeBtnCallback(Widget w, XtPointer clientData,
  ...
  DrawCurrentDirectory(file_mgr_rec->current_directory, file_mgr_rec,
                       file_mgr_data);
  LayoutFileIcons(file_mgr_rec, file_mgr_data, False, True);
  XmDropSiteEndUpdate(file_mgr_rec->file_window);
  RedrawTreeLines(file_mgr_rec->file_window, -file_mgr_rec->file_window->core.x,
                  -file_mgr_rec->file_window->core.y,
                  XtParent(file_mgr_rec->file_window)->core.width,
                  XtParent(file_mgr_rec->file_window)->core.height, 0,
...
void RepairFileWindow(FileMgrData *file_mgr_data)

{
  ...
  XUnionRectWithRegion(&rect, redrawRegion, redrawRegion);
  (*file_window->core.widget_class->core_class.expose)((Widget)file_window,
                                                       NULL, redrawRegion);
...

{
  ...
  for (i = 0; i < order_count; i++) {
    ...
    } else if (position_data =
                   GetPositionalData(file_mgr_data, object, 0, False)) {
      /*
  ...
  for (i = 0; i < file_mgr_data->num_objects;) {
    if (!file_mgr_data->object_positions[i]->in_use) {
      ...
      if (file_mgr_data->object_positions[i]->prev) {
        file_mgr_data->object_positions[i]->prev->next =
            (ObjectPtr)file_mgr_data->object_positions[i]->next;
      }
      if (file_mgr_data->object_positions[i]->next) {
        file_mgr_data->object_positions[i]->next->prev =
            (ObjectPtr)file_mgr_data->object_positions[i]->prev;
      }
  ...
  for (i = 0; i < order_count; i++) {
    ...
    if (object->displayed) {
      position_data =
          GetPositionalData(file_mgr_data, object, file_window_width, True);
  ...
  if (XtIsManaged(file_mgr_rec->horizontal_scroll_bar)) {
    ...
    (void)XmScrollBarGetValues(file_mgr_rec->horizontal_scroll_bar, &value,
                               &size, &increment, &page);
  }

  if (VerticalScrollbarIsVisible(file_mgr_rec->vertical_scroll_bar,
                                 file_mgr_rec->scroll_window)) {
    if ((Dimension)largest_x >=
        (Dimension)(file_mgr_rec->scroll_window->core.width -
  ...
  } else if ((Dimension)largest_x >=
             (Dimension)(file_mgr_rec->scroll_window->core.width - 4))
    XtManageChild(file_mgr_rec->horizontal_scroll_bar);
  else {
    /* set scroll bar values changing its position */
    if (value != 0 && XtIsManaged(file_mgr_rec->horizontal_scroll_bar))
  ...
  if ((Dimension)(current_ht) <
      (Dimension)(file_mgr_rec->scroll_window->core.height - 4)) {
  ...
  do {
    ...
    for (i = 0; i < file_mgr_data->num_objects; i++) {
      ...
      if (oP->file_view_data->displayed &&
          IntersectRects(x, y, objWidth, objHeight, oP->x, oP->y, oWidth,
                         oHeight)) {
        /* Try next grid spot */
        x += grid_width + XSPACING;
...
static void BuildObjectPositions(FileMgrData *file_mgr_data) {
  ...
  file_mgr_data->object_positions = (ObjectPosition **)XtMalloc(
      sizeof(ObjectPosition *) * file_mgr_data->directory_set[0]->file_count);
  ...
  for (i = 0, j = 0; i < file_mgr_data->directory_set[0]->file_count; i++) {
    ...
    ptr->y = file_view_data->y - (file_view_data->need_update
                                      ? file_mgr_data->grid_height
...
void RepositionIcons(FileMgrData *file_mgr_data, char **file_list,
                     int file_count, Position drop_x, Position drop_y,
...

{
  ...
  for (i = file_count - 1; i >= 0; i--) {
    ...
    if (strcmp(file_mgr_data->current_directory, file_list[i]) == 0)
      name = dot_dir;
    else if (strncmp(file_mgr_data->current_directory, file_list[i],
                     strlen(file_list[i])) == 0)
      name = dotdot_dir;
    else {
    ...
    for (j = 0; j < file_mgr_data->num_objects; j++) {
      if (strcmp(name, file_mgr_data->object_positions[j]->name) == 0) {
        ...
        for (k = 0; strcmp(name, file_mgr_data->directory_set[0]
                                     ->file_view_data[k]
                                     ->file_data->file_name);
             k++)
          ;
    ...
    if (newEntry) {
      ...
      RepositionUpInStack(
          file_mgr_data, file_mgr_data->object_positions[j]->stacking_order, 1);
    }
   ...
   */
  if (!late_binding_needed || (late_binding_needed && (newEntryCount == 0))) {
    RegisterDesktopHotspots(file_mgr_data, file_mgr_rec);
  }
}

void RegisterDesktopHotspots(FileMgrData *file_mgr_data,
                             FileMgrRec *file_mgr_rec) {
  XmManagerWidget file_window;
  FileViewData *file_view_data;
  int ex, ey, ewd, eht;
  ...
  while (top) {
    ...
    if (file_view_data != NULL && file_view_data->displayed &&
      ...
      if ((Position)(icon->core.x + icon->core.width) >= (Position)ex &&
          ...
          icon->core.y < ey + eht) {
        SetHotRects(file_view_data, (XtCallbackProc)DropOnObject,
...
Boolean VerticalScrollbarIsVisible(Widget vertSB, Widget scrolledWin)
...
Boolean HorizontalScrollbarIsVisible(Widget hortSB, Widget scrolledWin)
