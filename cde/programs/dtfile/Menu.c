...
static void GetMainMenuData(Widget mbar, XtPointer *mainRecordPtr,
                            MenuDesc **menuDescPtr, int *menuItemCountPtr,
                            unsigned int **globalMenuStatesPtr,
                            unsigned int **viewMenuStatesPtr);
static void RereadDirectoryMenu(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void ShowTrash(Widget w, XtPointer client_data, XtPointer call_data);
static void NewView(Widget w, XtPointer client_data, XtPointer call_data);
static void SetMenuSensitivity(Widget w, XtPointer client_data,
                               XtPointer call_data);
static void RestoreMenuSensitivity(Widget w, XtPointer client_data,
                                   XtPointer call_data);
static void TerminalCWD(Widget w, XtPointer client_data, XtPointer call_data);
...
Widget CreateMenu(Widget main, FileMgrRec *file_mgr_rec) {
...
void ActivateSingleSelect(FileMgrRec *file_mgr_rec, char *file_type) {
  ...
  if ((strcmp(file_mgr_data->selection_list[0]->file_data->file_name, "..") ==
       0) ||
      (strcmp(file_mgr_data->selection_list[0]->file_data->file_name, ".") ==
       0)) {
  ...
  UpdateActionMenuPane(
      (XtPointer)file_mgr_data, file_mgr_rec, file_type, NOT_DESKTOP, 0,
      (Widget)NULL, file_mgr_data->selection_list[0]->file_data->physical_type);
}

static void UnmanageAllActionItems(FileMgrRec *file_mgr_rec) {
  int i, children;
...
void ActivateMultipleSelect(FileMgrRec *file_mgr_rec) {
  file_mgr_rec->menuStates |= TRASH | PUT_ON_DESKTOP;
  file_mgr_rec->menuStates &= ~(RENAME | MOVE | DUPLICATE | LINK | MODIFY);
...
void ActivateNoSelect(FileMgrRec *file_mgr_rec) {
  file_mgr_rec->menuStates &=
      ~(RENAME | MOVE | DUPLICATE | LINK | TRASH | MODIFY | PUT_ON_DESKTOP);
...
void TrashFiles(Widget w, XtPointer client_data, XtPointer call_data) {
  FileMgrRec *file_mgr_rec;
  DialogData *dialog_data;
  FileMgrData *file_mgr_data;
  ...
  if (!TrashIsInitialized()) {
    ...
    msg =
        XtMalloc(strlen(GETMESSAGE(
                     27, 95,
                     "Unable to access the Trash directory:\n   %s\n   All "
  ...
  if (file_view_data)
    _DtBuildActionArgsWithSelectedFiles(&file_view_data, 1, &action_args,
                                        &arg_count);
  else
  ...
  DtActionInvoke(file_mgr_rec->shell, TRASH_ACTION, action_args, arg_count,
                 NULL, NULL, trash_dir, True, NULL, NULL);
...
void ChangeToHome(Widget w, XtPointer client_data, XtPointer call_data) {
  FileMgrRec *file_mgr_rec;
  DialogData *dialog_data;
  FileMgrData *file_mgr_data;
  char *host;
...
void ChangeToParent(Widget w, XtPointer client_data, XtPointer call_data) {
  FileMgrRec *file_mgr_rec;
  DialogData *dialog_data;
  FileMgrData *file_mgr_data;
  Arg args[1];
...
static void SetMenuSensitivity(Widget w, XtPointer client_data,
  ...
  if ((dialog_data = _DtGetInstanceData((XtPointer)file_mgr_rec)) != NULL) {
    ...
    if (stat(file_mgr_data->current_directory, &file_info) == 0) {
      if (!(file_info.st_mode & S_IXUSR) || !(file_info.st_mode & S_IWUSR)) {
    ...
    */
    ...
    if (file_mgr_data->toolbox) {
      if (geteuid() == root_user ||
        ...
        file_mgr_rec->menuStates &=
            ~(CREATE_DIR | CREATE_FILE | MOVE | LINK | RENAME | TRASH);
        _DtGlobalUpdateMenuItemState(*create_dataBtn, CREATE_FILE, False,
                                     &currentMenuStates);
        _DtGlobalUpdateMenuItemState(*create_directoryBtn, CREATE_DIR, False,
                                     &currentMenuStates);
        _DtGlobalUpdateMenuItemState(*moveBtn, MOVE, False, &currentMenuStates);
        _DtGlobalUpdateMenuItemState(*linkBtn, LINK, False, &currentMenuStates);
...
static void RestoreMenuSensitivity(Widget w, XtPointer client_data,
  ...
  if (restrictMode) {
    ...
    *ptr = '/';
  } else if ((currentMenuStates & MOVE_UP) == 0)
    _DtGlobalUpdateMenuItemState(*upBarBtn, MOVE_UP, True, &currentMenuStates);

  if ((currentMenuStates & CLEAN_UP) == 0)
    _DtGlobalUpdateMenuItemState(*cleanUpBtn, CLEAN_UP, True,
                                 &currentMenuStates);
}
...
static void TerminalCWD(Widget w, XtPointer client_data, XtPointer call_data) {
  ...
  DtActionInvoke(file_mgr_rec->shell, "Terminal", NULL, 0, NULL, NULL, pwd_dir,
                 True, NULL, NULL);
