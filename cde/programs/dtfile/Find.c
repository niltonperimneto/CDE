...
static DialogResource resources[] = {
    {"folders", XmRString, sizeof(String), XtOffset(FindDataPtr, directories),
     (XtPointer)NULL, _DtStringToString},

    {"name", XmRString, sizeof(String), XtOffset(FindDataPtr, filter),
     (XtPointer)NULL, _DtStringToString},

    {"selectedItem", XmRInt, sizeof(int), XtOffset(FindDataPtr, selected_item),
     (XtPointer)-1, _DtIntToString},

    {"content", XmRString, sizeof(String), XtOffset(FindDataPtr, content),
     (XtPointer)NULL, _DtStringToString},
};

static DialogResource match_resources[] = {
    {"matchData", XmRString, sizeof(String), XtOffset(DummyPtr, string),
     (XtPointer)NULL, _DtStringToString},
};
...
static void Create(Display *display, Widget parent, Widget *return_widget,
                   XtPointer *dialog);
static void InstallChange(FindRec *find_rec, XtCallbackProc callback,
                          XtPointer client_data);
static void InstallClose(FindRec *find_rec, XtCallbackProc callback,
                         XtPointer client_data);
static void Destroy(FindRec *find_rec);
static XtPointer GetValues(FindRec *find_rec);
static XtPointer GetDefaultValues(void);
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list);
static void SetValues(FindRec *find_rec, FindData *find_data);
static void WriteResourceValues(DialogData *values, int fd, char **name_list);
static void FreeValues(FindData *find_data);
static Boolean GetFindValues(FindRec *find_rec, FindData *find_data,
                             Boolean validate);
static void InvalidFindMessage(FindRec *find_rec, int messageIndex,
                               String extra_string);
static void FreeMatchInfo(String *matches, int numMatches);
static void LeaveStopBttn(Widget w, FindRec *find_rec, XEvent *event);
static void EnterStopBttn(Widget w, FindRec *find_rec, XEvent *event);
static void StopSearch(Widget w, FindRec *find_rec);
static void StartSearch(Widget w, FindRec *find_rec);
static void StartCallback(Widget w, XtPointer client_data, XtPointer call_data);
static void StopCallback(Widget w, XtPointer client_data, XtPointer call_data);
static String GrowBuffer(String buf, int *size, int extra);
static String MakeAbsolute(String current_directory, String path);
static Boolean FindProcessStarted(FindRec *find_rec, FindData *find_data);
static Boolean ExecuteFind(FindRec *find_rec, FindData *find_data,
                           FileMgrData *file_mgr_data);
static Boolean ExecuteGrep(FindRec *find_rec);
static void AlternateInputHandler(XtPointer client_data, int *source,
                                  XtInputId *id);
static void AlternateInputHandler2(XtPointer client_data, int *source,
                                   XtInputId *id);
static void AlternateInputHandler3(XtPointer client_data, int *source,
                                   XtInputId *id);
static void GetFileName(Widget list, int selectedItem, String *host,
                        String *path, FileMgrData *file_mgr_data);
static Boolean ExtractDirectory(String host, String path, char **file_name);
static void NewView(Widget widget, XtPointer client_data, XtPointer call_data);
static void FindPutOnDesktop(Widget widget, XtPointer client_data,
                             XtPointer call_data);
static void SetActiveItem(Widget widget, XtPointer client_data,
                          XtPointer call_data);
static void SetFocus(FindRec *find_rec, FindData *find_data);
...
static void Create(Display *display, Widget parent, Widget *return_widget,
  ...
  label_string = XmStringCreateLocalized((GETMESSAGE(
      15, 47, "Fill in one or more fields to specify which items to find:")));
  n = 0;
  ...
  n++;
  XtSetArg(args[n], XmNtopOffset, 14);
  n++;
  ...
  label_string =
      XmStringCreateLocalized((GETMESSAGE(15, 31, "File or Folder Name: ")));
  n = 0;
  XtSetArg(args[n], XmNlabelString, label_string);
  n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
  ...
  n++;
  XtSetArg(args[n], XmNtopWidget, headLabel);
  n++;
  ...
  n++;
  XtSetArg(args[n], XmNleftWidget, filterLabel);
  n++;
  ...
  label_string =
      XmStringCreateLocalized((GETMESSAGE(15, 32, "File Contents:")));
  n = 0;
  XtSetArg(args[n], XmNlabelString, label_string);
  n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
  ...
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, filterText);
  n++;
  XtSetArg(args[n], XmNtopOffset, 15);
  n++;
  XtSetArg(args[n], XmNtraversalOn, False);
  ...
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNleftWidget, contentLabel);
  n++;
  XtSetArg(args[n], XmNleftOffset, 0);
  n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  ...
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, contentText);
  n++;
  ...
  n++;
  find_rec->widgArry[0] = XmCreatePushButtonGadget(followLinkPD, "On", args, 3);
  XmStringFree(label_string);
  ...
  label_string =
      XmStringCreateLocalized((GETMESSAGE(15, 35, "Follow Links: ")));
  ...
  n++;
  XtSetArg(args[n], XmNsubMenuId, followLinkPD);
  n++;
  XtSetArg(args[n], XmNmenuHistory, find_rec->widgArry[OFF]);
  n++;
  ...
  n++;
#if defined(sun)
  XtSetArg(args[n], XmNtopWidget, form1);
  n++;
#else
  XtSetArg(args[n], XmNtopWidget, contentText);
  n++;
  ...
  label_string =
      XmStringCreateLocalized(((char *)GETMESSAGE(15, 42, "Search Folder: ")));
  n = 0;
  XtSetArg(args[n], XmNlabelString, label_string);
  n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
  ...
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, outputSeparator);
  n++;
  XtSetArg(args[n], XmNtopOffset, 15);
  n++;
  XtSetArg(args[n], XmNtraversalOn, False);
  ...
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNleftWidget, dirLabel);
  n++;
  XtSetArg(args[n], XmNleftOffset, 0);
  n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
  ...
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, dirLabel);
  n++;
  XtSetArg(args[n], XmNtopOffset, 15);
  n++;
  ...
  n++;
  XtSetArg(args[n], XmNvisibleItemCount, 5);
  n++;
  scrolledList = XmCreateScrolledList(form, "file_list", args, n);
  XtManageChild(scrolledList);

  XtAddCallback(scrolledList, XmNbrowseSelectionCallback, SetActiveItem,
                (XtPointer)find_rec);
  XtAddCallback(scrolledList, XmNdefaultActionCallback, NewView,
                (XtPointer)find_rec);
  XtAddCallback(XtParent(scrolledList), XmNhelpCallback,
                (XtCallbackProc)HelpRequestCB, HELP_FIND_DIALOG_STR);

  n = 0;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, listLabel);
  n++;
  XtSetArg(args[n], XmNtopOffset, 5);
  n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
  ...
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
  n++;
  XtSetArg(args[n], XmNrightPosition, 10);
  n++;
  ...
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, scrolledList);
  n++;
  XtSetArg(args[n], XmNtopOffset, 10);
  n++;
  ...
  */
  ...
  label_string =
      XmStringCreateLocalized((GETMESSAGE(15, 37, "Put In Workspace")));
  n = 0;
  XtSetArg(args[n], XmNlabelString, label_string);
  n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
  ...
  n++;
  XtSetArg(args[n], XmNrightPosition, 24);
  n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  ...
  XtAddCallback(start, XmNactivateCallback, StartCallback, (XtPointer)find_rec);
  ...
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
  n++;
  XtSetArg(args[n], XmNleftPosition, 26);
  n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
  n++;
  ...
  XtAddCallback(stop, XmNactivateCallback, StopCallback, (XtPointer)find_rec);
  ...
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
  n++;
  XtSetArg(args[n], XmNrightPosition, 74);
  n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, separator);
  ...
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
  n++;
  XtSetArg(args[n], XmNleftPosition, 76);
  n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
  n++;
...
static void InstallClose(FindRec *find_rec, XtCallbackProc callback,
  ...
  delete_window_atom =
      XmInternAtom(XtDisplay(find_rec->shell), "WM_DELETE_WINDOW", True);
...
static void SetValues(FindRec *find_rec, FindData *find_data) {
  ...
  if (find_data->num_matches > 0) {
    ...
    matches = (XmString *)XtMalloc(sizeof(XmString) * find_data->num_matches);
    ...
    if (find_data->selected_item != -1) {
      XmListSelectPos(find_rec->matchList, find_data->selected_item + 1, False);
    }
  } else {
...
static void WriteResourceValues(DialogData *values, int fd, char **name_list) {
  ...
  if (find_data->num_matches > 0) {
    ...
    dummy.string =
        CvtStringListToString(find_data->matches, find_data->num_matches);
...
static Boolean GetFindValues(FindRec *find_rec, FindData *find_data,
  ...
  find_data->filter =
      (char *)_DtStripSpaces(XmTextFieldGetString(find_rec->fileNameFilter));
  if (validate) {
  ...
  if (find_data->num_matches == 0) {
    ...
    for (i = 0; i < find_data->num_matches; i++) {
      find_data->matches[i] =
          (char *)_XmStringUngenerate(stringTable[i], XmFONTLIST_DEFAULT_TAG,
...
static void InvalidFindMessage(FindRec *find_rec, int messageIndex,
  ...
  if (noExistanceMessage == NULL) {
    tmpStr =
        GETMESSAGE(15, 44,
                   "Search Folder name argument is missing.\nType in the name "
                   "of the folder where you want the search to begin.");
    ...
    tmpStr = GETMESSAGE(
        15, 50,
        "Search Folder name or File Content argument is missing\nType in the "
        "name of the folder where you want the search to begin.\nOr type in "
        "the string that you want to search.");
    noSearchArgumentMessage = XtNewString(tmpStr);

    tmpStr = GETMESSAGE(15, 45,
                        "The selected file no longer exists.\n\nSomeone "
                        "deleted the file after the search process completed.");
...
static void StopSearch(Widget w, FindRec *find_rec) {
  ...
  XtRemoveEventHandler(find_rec->stop, LeaveWindowMask, FALSE,
                       (XtEventHandler)LeaveStopBttn, find_rec);
  XtRemoveEventHandler(find_rec->stop, EnterWindowMask, FALSE,
                       (XtEventHandler)EnterStopBttn, find_rec);
...
static void StartSearch(Widget w, FindRec *find_rec) {
  ...
  if (!FindProcessStarted(find_rec, find_data)) {
    ...
    */
    XtAddEventHandler(find_rec->stop, LeaveWindowMask, FALSE,
                      (XtEventHandler)LeaveStopBttn, find_rec);
    XtAddEventHandler(find_rec->stop, EnterWindowMask, FALSE,
                      (XtEventHandler)EnterStopBttn, find_rec);
  }
...
static void StartCallback(Widget w, XtPointer client_data,
                          XtPointer call_data) {
...
static Boolean ExecuteFind(FindRec *find_rec, FindData *find_data,
  ...
  if (access((char *)path, R_OK) == -1) {
#else
  if ((((access_priv = getaccess(path, UID_EUID, NGROUPS_EGID_SUPP, 0,
                                 (void *)0, (void *)0)) == -1) ||
       !(access_priv & R_OK)) &&
      (geteuid() != root_user)) {
#endif /* BLS */
...
#if defined(sun)
  {
    ...
    if (menuHistory == find_rec->widgArry[ON]) {
      ...
      command = GrowBuffer(command, &commandLen, (int)strlen(FOLLOW_OPTION));
...
static void AlternateInputHandler(XtPointer client_data, int *source,
  ...
  while (1) {
    ...
    if (offset > 0) {
      ...
      if ((stat(buf, &stat_data) == 0) || (lstat(buf, &stat_data) == 0)) {
        findptr = buf;

        /* Strip out any invisible files */
        if (findptr) {
          file_type = (char *)DtDtsDataToDataType(findptr, NULL, 0, NULL, NULL,
                                                  NULL, NULL);
        ...
        if (findptr &&
          ...
          if (file_mgr_data->restricted_directory != NULL)
            string = XmStringCreateLocalized(
                findptr + strlen(file_mgr_data->restricted_directory));
          else
...
static void GetFileName(Widget list, int selectedItem, String *host,
  ...
  temp =
      (char *)_XmStringUngenerate(items[selectedItem], XmFONTLIST_DEFAULT_TAG,
                                  XmCHARSET_TEXT, XmCHARSET_TEXT);
  *host = NULL;
  *path = temp;
  if (file_mgr_data->restricted_directory != NULL) {
    tmpptr = (char *)XtMalloc(strlen(file_mgr_data->restricted_directory) +
                              strlen(*path) + 1);
...
static Boolean ExtractDirectory(String host, String path, char **file_name) {
  ...
  realPath = (String)ResolveLocalPathName(host, path, NULL, home_host_name,
                                          &tt_status);
  if (TT_OK != tt_status)
    return (False);
  else {
    ...
    if (ExtractDirectory(file_mgr_data->host, path, &file_name)) {
      ...
      if (file_mgr_data->restricted_directory == NULL)
        ...
        if (file_mgr_data->toolbox)
          dialog_data =
              GetNewView(file_mgr_data->host, path,
...
static void FindPutOnDesktop(Widget widget, XtPointer client_data,
  ...
  if (host == NULL) {
    SetupDesktopWindow(XtDisplay(widget), NULL,
                       (FileMgrRec *)file_mgr_data->file_mgr_rec, file_name,
                       file_mgr_data->host, path, -1, -1,
                       file_mgr_data->restricted_directory, EndIndex);
  } else {
    SetupDesktopWindow(XtDisplay(widget), NULL,
                       (FileMgrRec *)file_mgr_data->file_mgr_rec, file_name,
                       host, path, -1, -1, file_mgr_data->restricted_directory,
                       EndIndex);
  }
  XtFree((char *)path);
  XtFree((char *)host);
...
FILE *findpopen(char *cmd, char *mode, int *childpid) {
