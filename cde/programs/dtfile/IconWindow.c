...
static Boolean CheckForBtnUp(Display *display, XEvent *event, XtPointer arg);
static void GetSelectionRectangle(Display *display, Window window,
                                  XButtonEvent *event, FileMgrRec *file_mgr_rec,
                                  FileMgrData *file_mgr_data,
                                  Boolean toggleSelect);
static void DrawRectangle(Display *display, Window window, GC gc, int x1,
                          int y1, int x2, int y2);
static void ProcessSelection(FileMgrData *file_mgr_data, Widget icon,
                             Boolean toggleSelect, XButtonEvent *event);

static void FM_XmHighlightBorder(Widget w);
static void FM_XmUnhighlightBorder(Widget w);

static XmGadget InputForGadget(Widget wid, int x, int y);
...
void FileWindowExposeCallback(Widget w, XtPointer client_data,
                              XtPointer call_data) {
...
void FileWindowResizeCallback(Widget w, XtPointer client_data, XEvent *event) {
  FileMgrRec *file_mgr_rec;
  DialogData *dialog_data;
  FileMgrData *file_mgr_data;
...
void FileWindowMapCallback(Widget w, XtPointer client_data, XEvent *event) {
  FileMgrRec *file_mgr_rec;
  DialogData *dialog_data;
  FileMgrData *file_mgr_data;
...
void FileWindowInputCallback(Widget w, XtPointer client_data,
                             XtPointer call_data) {
  ...
  if (new_event->type == ButtonPress) {
    ...
    if (first) {
      ...
      NumSelectKeys = XmeVirtualToActualKeysyms(XtDisplay(w), osfXK_Select,
                                                &SelectBindings);
  ...
  if (!desktopRec && new_event->type == KeyPress &&
    ...
    if (found && fileViewData->file_data->is_subdir) {
      ...
      RedrawTreeLines(w, -w->core.x, -w->core.y, XtParent(w)->core.width,
                      XtParent(w)->core.height, 0, file_mgr_rec, file_mgr_data);
    }
  }

  else if (new_event->type == ButtonPress ||
           (new_event->type == KeyPress && keybdFocusPolicy == XmEXPLICIT)) {
    ...
    if (new_event->type == KeyPress) {
      ...
      if (keysym == XK_Escape) {
        ...
        if (desktopRec)
          ...
          UnpostTextField(file_mgr_data);
        return;
      } else if (keysym != ActivateKeysym && keysym != SpaceKeysym &&
                 keysym != osfXK_Delete && keysym != osfXK_EndLine &&
                 keysym != osfXK_BeginLine && keysym != osfXK_PageLeft &&
                 keysym != osfXK_PageUp && keysym != osfXK_PageDown &&
                 keysym != osfXK_PageRight && keysym != osfXK_Menu) {
    ...
    if (eventIcon != NULL) {
      ...
      if (((file_mgr_rec && FileIsSelected(file_mgr_data, fileViewData)) ||
        ...
        if ((file_mgr_rec && FileIsSelected(file_mgr_data, fileViewData)) &&
          ...
          if (PositioningEnabledInView(file_mgr_data)) {
            ...
            for (j = 0; i < file_mgr_data->selected_file_count; j++, i++) {
              if (top->file_view_data != file_mgr_data->selection_list[j])
        ...
        case osfXK_Delete: {
          ...
          if (desktopRec == NULL && file_mgr_rec != NULL) {
            if (file_mgr_data->selected_file_count && fileViewData &&
                FileIsSelected(file_mgr_data, fileViewData)) {
              _DtBuildActionArgsWithSelectedFiles(
                  file_mgr_data->selection_list,
                  file_mgr_data->selected_file_count, &action_args, &arg_count);
            } else {
              _DtBuildActionArgsWithSelectedFiles(&fileViewData, 1,
                                                  &action_args, &arg_count);
            }
            DtActionInvoke(file_mgr_rec->shell, TRASH_ACTION, action_args,
                           arg_count, NULL, NULL, trash_dir, True, NULL, NULL);
          } else if (desktopRec != NULL) {
            int selectedCount =
                desktop_data->workspaceData[desktopRec->workspace_num - 1]
                    ->files_selected;
            if (selectedCount) {
              _DtBuildActionArgsWithDTSelectedFiles(
                  desktop_data->workspaceData[desktopRec->workspace_num - 1]
                      ->selectedDTWindows,
                  selectedCount, &action_args, &arg_count);
            } else {
              _DtBuildActionArgsWithSelectedFiles(&fileViewData, 1,
                                                  &action_args, &arg_count);
            }
            DtActionInvoke(desktopRec->shell, TRASH_ACTION, action_args,
                           arg_count, NULL, NULL, trash_dir, True, NULL, NULL);
          }
        ...
        case osfXK_Menu:
          if (file_mgr_data) /* if not, then it is a desktop object */
            FmPopup(NULL, (XtPointer)fileViewData, (XEvent *)event,
                    file_mgr_data);
          break;
        case osfXK_EndLine:
          if (!desktopRec && file_mgr_data->show_type != MULTIPLE_DIRECTORY) {
            Widget child = NULL;
            ...
            if (child) {
              ...
              order_list =
                  ((IconLayoutData *)file_mgr_data->layout_data)->order_list;
              order_count =
                  ((IconLayoutData *)file_mgr_data->layout_data)->order_count;
  ...
  } else if (new_event->type == ButtonRelease ||
             (new_event->type == KeyRelease &&
    ...
    if (new_event->type == KeyRelease) {
      ...
      if (event->button == Button4 || event->button == Button5) {
        ...
        String q = event->button == Button4 ? "IncrementUpOrLeft"
                                            : "IncrementDownOrRight";
        if (VerticalScrollbarIsVisible(file_mgr_rec->vertical_scroll_bar,
                                       file_mgr_rec->scroll_window))
          XtCallActionProc(file_mgr_rec->vertical_scroll_bar, q, new_event, &p,
                           1);
      }
      if (event->button != Button1)
        return;
    ...
    if (((initialDragX >= 0) && (initialDragY >= 0)) &&
        (eventIcon = (Widget)InputForGadget(w, initialDragX, initialDragY))) {
      /*
      ...
      if (_DtIconSelectInTitle(eventIcon, (Position)initialDragX,
        ...
        if (desktopRec) {
          CreateNameChangeDialog(eventIcon, fileViewData, (XtPointer)desktopRec,
                                 DESKTOP);
          ...
          */
          if (file_mgr_data->view != BY_ATTRIBUTES ||
              (file_mgr_data->view == BY_ATTRIBUTES && TurnOnTextName))
...
static void GetSelectionRectangle(Display *display, Window window,
  ...
  if (vsbVisible) {
    XmScrollBarGetValues(vb, &vb_val_ret, &vb_slide_size, &vb_inc_ret,
                         &vb_pg_inc_ret);
  ...
  if (hsbVisible) {
    XmScrollBarGetValues(hb, &hb_val_ret, &hb_slide_size, &hb_inc_ret,
                         &hb_pg_inc_ret);
  ...
  {
    XQueryPointer(display, window, &root, &sub_win, &win_x, &win_y, &x2, &y2,
                  &mask);
    ...
    if ((mask & Button1Mask) == 0) {
       ...
       */
      if (XPeekIfEvent(display, &btnUpEvent,
                       (Bool (*)(Display *, XEvent *, XPointer))CheckForBtnUp,
                       NULL)) {
    ...
    if ((x2 > old_x && !hsbVisible && (Dimension)old_x < file_window_width) ||
        (x2 > old_x && hsbVisible &&
         (Dimension)(hb_val_ret + hb_slide_size) <
             (Dimension)(file_window_width + 1)) ||
        (x2 < old_x && !hsbVisible && old_x > 0) ||
        (x2 < old_x && hsbVisible && hb_val_ret >= 0) ||
        (y2 > old_y && !vsbVisible && (Dimension)old_y < file_window_height) ||
        (y2 > old_y && vsbVisible &&
         (Dimension)(vb_val_ret + vb_slide_size) <
             (Dimension)(file_window_height + 1)) ||
      ...
      if ((Dimension)x2 >= max_width) {
        if (hsbVisible) {
          ...
          if ((Dimension)(hb_val_ret + hb_slide_size) <=
              (Dimension)(file_window_width + 1)) {
            XmScrollBarSetValues(hb, hb_val_ret, hb_slide_size, hb_inc_ret,
                                 hb_pg_inc_ret, True);
            min_width = (x2 - max_width) + min_width;
            max_width = x2;
            XmUpdateDisplay((Widget)file_window);
          } else if ((Dimension)(hb_val_ret + hb_slide_size) >
                         (Dimension)(file_window_width + 1) &&
                     (Dimension)(hb_val_ret - (x2 - max_width)) <
                         (Dimension)(file_window_width - hb_slide_size)) {
            XmScrollBarSetValues(hb, file_window_width - hb_slide_size + 1,
                                 hb_slide_size, hb_inc_ret, hb_pg_inc_ret,
                                 True);
        ...
        if (hsbVisible) {
          if (min_width > 0) {
            ...
            if (hb_val_ret >= 0) {
              ...
              XmScrollBarSetValues(hb, 0, hb_slide_size, hb_inc_ret,
                                   hb_pg_inc_ret, True);
              max_width = current_file_window_width;
              XmUpdateDisplay((Widget)file_window);
      ...
      if ((Dimension)y2 > max_height) {
        if (vsbVisible) {
          ...
          if ((Dimension)(vb_val_ret + vb_slide_size) <=
              (Dimension)(file_window_height + 1)) {
            XmScrollBarSetValues(vb, vb_val_ret, vb_slide_size, vb_inc_ret,
                                 vb_pg_inc_ret, True);
            min_height = (y2 - max_height) + min_height;
            max_height = y2;
            XmUpdateDisplay((Widget)file_window);
          } else if ((Dimension)(vb_val_ret + vb_slide_size) >
                         (Dimension)(file_window_height + 1) &&
                     (Dimension)(vb_val_ret - (y2 - max_height)) <
                         (Dimension)(file_window_height + 1 - vb_slide_size)) {
            XmScrollBarSetValues(vb, file_window_height + 1 - vb_slide_size,
        ...
        if (vsbVisible) {
          if (min_height > 0) {
            ...
            if (vb_val_ret >= 0) {
              ...
              XmScrollBarSetValues(vb, 0, vb_slide_size, vb_inc_ret,
                                   vb_pg_inc_ret, True);
              max_height = current_file_window_height;
              XmUpdateDisplay((Widget)file_window);
        ...
        for (j = 0; j < directory_set[i]->file_count; j++) {
          ...
          case XmPIXMAP_TOP:
            if (alignment == XmALIGNMENT_BEGINNING)
              ...
              fprintf(stderr,
                      "GetSelectionRectangle: unsupported alignment %d , "
                      "defaulting to Beginning\n",
                      alignment);
              pixmap_x = x + highlightThickness;
            }
            break;
...
void WidgetRectToRegion(FileMgrData *file_mgr_data, Widget w, Region region) {
...
static void ProcessSelection(FileMgrData *file_mgr_data, Widget icon,
  ...
  if (icon == NULL && event->type == ButtonPress) {
    ...
    GetSelectionRectangle(XtDisplay(file_mgr_rec->file_window),
                          (Window)XtWindow(file_mgr_rec->file_window), event,
                          file_mgr_rec, file_mgr_data, toggleSelect);
  } else {
          ...
          if (!XEmptyRegion(intersect_region)) {
            if (toggleSelect == True &&
              ...
              if (file_mgr_data->view == BY_ATTRIBUTES) {
                ...
                XtVaGetValues(file_view_data->widget, XmNfontList, &fontList,
                              NULL);

                fileNameString = XmStringCreateLocalized(
                ...
                XUnionRectWithRegion(&textExtent, tmp_region, tmp_region);
...
void DrawHighlight(Widget w, FileViewData *file_view_data,
  ...
  n++;
  XtSetArg(args[n], XmNmarginWidth, &marginWidth);
  n++;
  XtSetArg(args[n], XmNmarginHeight, &marginHeight);
  n++;
  XtSetArg(args[n], XmNpixmapPosition, &pixmapPosition);
  n++;
  XtSetArg(args[n], XmNalignment, &alignment);
  n++;
  ...
  case XmPIXMAP_TOP:
    if (alignment == XmALIGNMENT_BEGINNING) {
      ...
      if (stringWidth > pixmapWidth) {
        ...
        rect[num_rectangles].y = y;
        rect[num_rectangles].width = highlightThickness;
        rect[num_rectangles++].height =
            pixmapHeight + (2 * adj_y) - highlightThickness;

        /* 3 */
        rect[num_rectangles].x =
            x + stringWidth + (2 * adj_x) - highlightThickness;
        rect[num_rectangles].y =
      ...
      /* 5 */
      rect[num_rectangles].x = x;
      rect[num_rectangles].y = y + pixmapHeight + stringHeight + spacing +
                               (2 * adj_y) - highlightThickness;
  ...
  case XmPIXMAP_LEFT:
    if ((Dimension)(stringHeight + 4) >= pixmapHeight)
      ...
      /* 4 */
      rect[num_rectangles].x = x + adj_x + pixmapWidth + spacing + stringWidth +
...
void DrawUnhighlight(Widget w, int type) {
  int n;
  Position x, y;
  Dimension highlightThickness, shadowThickness;
  Dimension half_shadowThickness;
  ...
  case XmPIXMAP_TOP:
    if (alignment == XmALIGNMENT_BEGINNING) {
      /* 1 */
      XClearArea(XtDisplay(w), XtWindow(w), x, y, pixmapWidth + (2 * adj_x),
                 highlightThickness, False);

      if (stringWidth > pixmapWidth) {
        /* 2 */
        XClearArea(XtDisplay(w), XtWindow(w),
                   x + pixmapWidth + (2 * adj_x) - highlightThickness, y,
                   highlightThickness, pixmapHeight + spacing, False);
        ...
        XClearArea(XtDisplay(w), XtWindow(w),
                   ...
                   stringHeight + (2 * adj_y), False);
      } else {
        /* 2 */
        XClearArea(XtDisplay(w), XtWindow(w),
                   x + pixmapWidth + (2 * adj_x) - highlightThickness, y,
                   highlightThickness,
                   pixmapHeight + (2 * adj_y) - half_shadowThickness, False);
        ...
        XClearArea(XtDisplay(w), XtWindow(w),
                   x + stringWidth + (2 * adj_x) - highlightThickness,
                   y + pixmapHeight + (2 * adj_y) - half_shadowThickness,
                   highlightThickness,
                   spacing + stringHeight + highlightThickness, False);
      }

      /* 5 */
      XClearArea(XtDisplay(w), XtWindow(w), x,
                 y + pixmapHeight + stringHeight + spacing + (2 * adj_y) -
                     highlightThickness,
                 stringWidth + (2 * adj_y), highlightThickness, False);

      /* 6 */
      XClearArea(XtDisplay(w), XtWindow(w), x, y, highlightThickness,
                 pixmapHeight + stringHeight + spacing + (2 * adj_y), False);
      ...
      if (stringWidth > pixmapWidth) {
        ...
        XClearArea(XtDisplay(w), XtWindow(w),
                   x + (Dimension)(stringWidth - pixmapWidth) / (Dimension)2 +
                       pixmapWidth + (2 * adj_x) - highlightThickness,
                   y, highlightThickness, pixmapHeight + spacing, False);
        ...
        /* 6 */
        XClearArea(XtDisplay(w), XtWindow(w), x, y + pixmapHeight + spacing,
                   highlightThickness, stringHeight + (2 * adj_y), False);
        ...
        XClearArea(XtDisplay(w), XtWindow(w),
                   x + pixmapWidth + (2 * adj_x) - highlightThickness, y,
                   highlightThickness,
                   pixmapHeight + spacing + 2 * highlightThickness, False);
        ...
        XClearArea(XtDisplay(w), XtWindow(w),
                   ...
                   y + pixmapHeight + spacing + highlightThickness + 1,
                   highlightThickness,
                   stringHeight + (2 * adj_y) - highlightThickness - 1, False);
  ...
  case XmPIXMAP_LEFT:
    if ((Dimension)(stringHeight + 4) >= pixmapHeight)
      ...
      XClearArea(
          ...
          x + adj_x + pixmapWidth + (Dimension)(spacing + 1) / (Dimension)2, y,
          highlightThickness,
          (Dimension)(pixmapHeight - stringHeight) / (Dimension)2, False);

      /* 3 */
      XClearArea(XtDisplay(w), XtWindow(w),
                 x + adj_x + pixmapWidth +
                     (Dimension)(spacing + 1) / (Dimension)2,
                 y + (Dimension)(pixmapHeight - stringHeight) / (Dimension)2,
                 spacing / 2 + stringWidth + adj_x, highlightThickness, False);

      /* 4 */
      XClearArea(XtDisplay(w), XtWindow(w),
                 x + adj_x + pixmapWidth + spacing + stringWidth + adj_x -
                     highlightThickness,
                 y + (Dimension)(pixmapHeight - stringHeight) / (Dimension)2,
                 highlightThickness, stringHeight + 2 * adj_y, False);
...
void DrawShadowTh(Widget w, GC gc, int type) {
  int n;
  Position x, y;
  Dimension highlightThickness, shadowThickness;
  Dimension width, height;
  ...
  Boolean s_out = False, s_etched = False;
  ...
  XtSetArg(args[n], XmNalignment, &alignment);
  n++;
  XtSetArg(args[n], XmNshadowType, &shadowType);
  n++;
  XtGetValues(w, args, n);
  ...
  s_out = (shadowType == XmSHADOW_OUT || shadowType == XmSHADOW_ETCHED_OUT)
              ...
              : False;
  s_etched =
      (shadowType == XmSHADOW_ETCHED_IN || shadowType == XmSHADOW_ETCHED_OUT)
          ? True
          : False;
  ...
  if (gc == NULL) {
    if (shadowType == XmSHADOW_ETCHED_OUT) {
      top_gc =
          (s_out) ? mgr->manager.top_shadow_GC : mgr->manager.bottom_shadow_GC;
      bottom_gc =
          (s_out) ? mgr->manager.bottom_shadow_GC : mgr->manager.top_shadow_GC;
    } else {
      bottom_gc =
          (s_out) ? mgr->manager.top_shadow_GC : mgr->manager.bottom_shadow_GC;
      top_gc =
  ...
  if (!s_etched) {
    XmeDrawShadows(
        XtDisplay(w), XtWindow(w), top_gc, bottom_gc, x + highlightThickness,
        y + highlightThickness, width - 2 * highlightThickness,
        height - 2 * highlightThickness, shadowThickness, XmSHADOW_IN);
  } else {
    ...
    if (shadowThickness <= 0)
      return;
    if (shadowThickness > (Dimension)(width / 2))
      shadowThickness = width / 2;
    if (shadowThickness > (Dimension)(height / 2))
      shadowThickness = height / 2;
    if (shadowThickness <= 0)
      return;

    shadowThickness =
        (shadowThickness % 2) ? (shadowThickness - 1) : (shadowThickness);
    ...
    case XmPIXMAP_TOP:
      if (alignment == XmALIGNMENT_BEGINNING) {
        if (type == NOT_DESKTOP) {
          ...
          rect[num_rects++].height = pixmapHeight + marginHeight + spacing / 2 +
          ...
          if (stringWidth > pixmapWidth) {
            ...
            rect[num_rects].y =
                y + pixmapHeight + 2 * marginHeight + shadowThickness;
            rect[num_rects].width =
                stringWidth - pixmapWidth + marginWidth - half_shadowThickness;
            rect[num_rects++].height = half_shadowThickness;
            ...
            rect[num_rects].y =
                y + pixmapHeight + 2 * marginHeight + shadowThickness;
            rect[num_rects].width = half_shadowThickness;
            rect[num_rects++].height =
                stringHeight + (2 * marginHeight) + half_shadowThickness;
          } else {
            /* 3 */
            ...
            rect[num_rects].y = y + pixmapHeight + 2 * marginHeight +
                                shadowThickness + half_shadowThickness;
            rect[num_rects].width =
                pixmapWidth - stringWidth + marginHeight - half_shadowThickness;
            rect[num_rects++].height = half_shadowThickness;
          ...
          rect[num_rects].y = y + highlightThickness;
          rect[num_rects].width = half_shadowThickness;
          rect[num_rects++].height = pixmapHeight + stringHeight + spacing +
                                     (2 * marginHeight) + shadowThickness;

          XFillRectangles(XtDisplay(w), XtWindow(w), top_gc, &rect[0],
                          num_rects);

          num_rects = 0;
          /* 1 */
          rect[num_rects].x = x + highlightThickness + half_shadowThickness;
          rect[num_rects].y = y + highlightThickness + half_shadowThickness;
          rect[num_rects].width =
              pixmapWidth + (2 * marginWidth) + half_shadowThickness;
          rect[num_rects++].height = half_shadowThickness;
          ...
          if (stringWidth > pixmapWidth) {
            ...
            if (highlightThickness == 0)
              ...
              rect[num_rects++].height =
                  pixmapHeight + marginHeight + shadowThickness;
            ...
            rect[num_rects].y = y + pixmapHeight + 2 * marginHeight +
                                shadowThickness + half_shadowThickness;
            rect[num_rects].width =
                stringWidth - pixmapWidth + marginWidth - shadowThickness;
            rect[num_rects++].height = half_shadowThickness;
            ...
            rect[num_rects].x = x + highlightThickness + stringWidth +
                                ...
                                half_shadowThickness;
            rect[num_rects].y =
                y + pixmapHeight + 2 * marginHeight + 2 * shadowThickness;
            rect[num_rects].width =
                pixmapWidth - stringWidth + marginHeight - half_shadowThickness;
            rect[num_rects++].height = half_shadowThickness;
          ...
          if (stringWidth > pixmapWidth)
            ...
            rect[num_rects].width =
                stringWidth + (2 * marginWidth) + (2 * shadowThickness);
          rect[num_rects++].height = half_shadowThickness;
          ...
          XFillRectangles(XtDisplay(w), XtWindow(w), bottom_gc, &rect[0],
                          num_rects);
        } else {
          num_rects = 0;
          /* 1 */
          ...
          rect[num_rects].y = y + highlightThickness + half_shadowThickness;
          rect[num_rects].width =
              pixmapWidth + (2 * marginWidth) + shadowThickness;
          rect[num_rects++].height = half_shadowThickness;
          ...
          if (stringWidth > pixmapWidth) {
            ...
            rect[num_rects].width = stringWidth - pixmapWidth + marginWidth;
            rect[num_rects++].height = half_shadowThickness;
          }
          ...
          if (stringWidth > pixmapWidth) {
            if (highlightThickness == 0)
              rect[num_rects++].height =
                  pixmapHeight + marginHeight + spacing / 2;
            else
              rect[num_rects++].height =
                  pixmapHeight + marginHeight + spacing / 2;
          } else {
          ...
          if (stringWidth > pixmapWidth) {
            if (highlightThickness == 0)
              ...
              rect[num_rects++].height =
                  pixmapHeight + marginHeight + half_shadowThickness;
          } else {
            if (highlightThickness == 0)
              rect[num_rects++].height = pixmapHeight + marginHeight +
                                         shadowThickness + (spacing / 2) +
          ...
          rect[num_rects].y =
              y + highlightThickness + pixmapHeight + 2 * marginHeight;
          rect[num_rects].width = half_shadowThickness;
          rect[num_rects++].height = stringHeight + highlightThickness +
                                     (2 * marginHeight) + shadowThickness +
                                     half_shadowThickness;
          ...
          if (stringWidth > pixmapWidth)
            rect[num_rects].width =
                stringWidth + (2 * marginWidth) + shadowThickness;
          else
            rect[num_rects].width =
                pixmapWidth + (2 * marginWidth) + shadowThickness;
          rect[num_rects++].height = half_shadowThickness;
          ...
          if (stringWidth > pixmapWidth)
            ...
            rect[num_rects].width =
                pixmapWidth + (2 * marginWidth) + (2 * shadowThickness);
          rect[num_rects++].height = half_shadowThickness;
        ...
        if (stringWidth > pixmapWidth) {
          ...
          rect[num_rects].x =
              x + (Dimension)(stringWidth - pixmapWidth) / (Dimension)2 +
              highlightThickness + shadowThickness + pixmapWidth +
              (2 * marginWidth);
          rect[num_rects].y = y + highlightThickness + pixmapHeight +
                              marginHeight + half_shadowThickness;
          rect[num_rects].width =
          ...
          /* 6 */ rect[num_rects].x =
              x + highlightThickness + half_shadowThickness;
          rect[num_rects].y =
              y + highlightThickness + pixmapHeight + 2 * marginHeight;
          rect[num_rects].width = half_shadowThickness;
          rect[num_rects++].height = stringHeight + highlightThickness +
          ...
          /* 1 */
          rect[num_rects].x =
              x + highlightThickness + pixmapWidth +
              (Dimension)(stringWidth - pixmapWidth) / (Dimension)2 +
              (2 * marginWidth) + shadowThickness;
          rect[num_rects].y = y + highlightThickness + half_shadowThickness;
          rect[num_rects].width = half_shadowThickness;
          ...
          /* 2 */
          rect[num_rects].x =
              x + highlightThickness + pixmapWidth + shadowThickness +
              (2 * marginWidth) + half_shadowThickness +
              (Dimension)(stringWidth - pixmapWidth) / (Dimension)2;
          rect[num_rects].y = y + highlightThickness;
          rect[num_rects].width = half_shadowThickness;
          ...
          rect[num_rects].y = y + highlightThickness;
          rect[num_rects].width = half_shadowThickness;
          rect[num_rects++].height = 2 * highlightThickness + pixmapHeight +
                                     stringHeight + 2 * marginWidth + spacing;
          ...
          /* 3 */
          rect[num_rects].x = x + highlightThickness + half_shadowThickness;
          rect[num_rects].y = y + 2 * highlightThickness + pixmapHeight +
                              stringHeight + spacing + 2 * marginHeight;
          rect[num_rects].width = pixmapWidth + (2 * shadowThickness) +
                                  (2 * marginWidth) + half_shadowThickness;
          rect[num_rects++].height = half_shadowThickness;
        ...
        XmeDrawShadows(XtDisplay(w), XtWindow(w), top_gc, bottom_gc,
                       x + highlightThickness, y + highlightThickness,
                       width - 2 * highlightThickness,
                       height - 2 * highlightThickness, shadowThickness,
                       XmSHADOW_IN);
      break;
    case XmPIXMAP_LEFT:
      if (type == NOT_DESKTOP) {
        if ((Dimension)(stringHeight + 5) >= (Dimension)pixmapHeight) {
          ...
          /* 2 */
          rect[num_rects].x = x + highlightThickness + half_shadowThickness +
                              pixmapWidth + stringWidth + spacing +
                              shadowThickness + 2 * marginWidth;
          rect[num_rects].y = y + highlightThickness;
          rect[num_rects].width = half_shadowThickness;
          rect[num_rects++].height =
              stringHeight + 2 * marginHeight + 2 * shadowThickness;
          ...
          rect[num_rects].y =
              ...
              ((Dimension)(pixmapHeight - stringHeight) / (Dimension)2) +
              marginHeight - shadowThickness;
          rect[num_rects].width = stringWidth + marginWidth + shadowThickness;
          rect[num_rects++].height = half_shadowThickness;
          ...
          rect[num_rects].y =
              y + highlightThickness +
              ((Dimension)(pixmapHeight - stringHeight) / (Dimension)2) +
              stringHeight + 2 * marginHeight + shadowThickness;
          rect[num_rects].width = stringWidth + marginWidth + shadowThickness;
          rect[num_rects++].height = half_shadowThickness;
          ...
          rect[num_rects].y =
              ...
              half_shadowThickness;
          rect[num_rects].width =
              stringWidth + marginWidth + half_shadowThickness;
          rect[num_rects++].height = half_shadowThickness;
          ...
          rect[num_rects].y =
              y + highlightThickness +
              ((Dimension)(pixmapHeight - stringHeight) / (Dimension)2) +
              2 * marginHeight + shadowThickness + stringHeight +
              half_shadowThickness;
          rect[num_rects].width = stringWidth + marginWidth + shadowThickness;
          rect[num_rects++].height = half_shadowThickness;
          ...
          rect[num_rects].width = half_shadowThickness;
          rect[num_rects++].height =
              pixmapHeight + 2 * marginHeight + half_shadowThickness;
        ...
        if ((Dimension)(stringHeight + 5) >= pixmapHeight) {
          ...
          if (pixmapHeight > stringHeight)
            rect[num_rects++].height =
                pixmapHeight + 2 * marginHeight + 2 * shadowThickness;
          else
            rect[num_rects++].height =
                stringHeight + 2 * marginHeight + 2 * shadowThickness;
          ...
          rect[num_rects++].height =
              ((Dimension)(pixmapHeight - stringHeight) / (Dimension)2) +
              marginHeight - half_shadowThickness - shadowThickness;
