...
static void ProcessMoveCopyLinkDT(char *command, DesktopRec *desktopWindow,
                                  DtDndDropCallbackStruct *drop_parameters);
static void
ProcessBufferDropOnFolderDT(char *command, DesktopRec *desktopWindow,
                            DtDndDropCallbackStruct *drop_parameters);
static FileViewData *BuildNewOrderlist(FileViewData *order_list, Widget icon,
                                       char *hostName, char *dirName,
                                       char *file, Boolean is_registered,
                                       Boolean IsToolBox);
...
static void popupMenu(Widget w, XtPointer client_data, XtPointer call_data);
static void OpenFolder(Widget w, XtPointer client_data, XtPointer call_data);
static void renameDT(Widget w, XtPointer client_data, XtPointer call_data);
static void BuildDesktopLinks(Display *display);
static int InitializeNewWorkspaces(Display *display, char *workspaceName);
static void TimerEvent(Widget widget, XtIntervalId *id);
static void DropOnDesktopObject(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void DrawAInput(Widget w, XtPointer client_data, XEvent *event,
                       Boolean *contDispatch);
static void CalculateRootCoordinates(int ws_num, int width, int height,
                                     int *root_x, int *root_y);

static int GetWorkspaceNum(char *workspace_name, int NumWorkspaces,
                           int *workspace_index);
static void FreeUpOldWorkspace(int oldNumWorkspaces, int old_ws_num, int ws_num,
...
void InitializeDesktopWindows(int number_cache, Display *display) {
  DesktopRec *desktop;
  ...
  if (desktop_data == NULL) {
    ...
    desktop_data->desktopWindows = (DesktopRec **)XtRealloc(
        (char *)desktop_data->desktopWindows, sizeof(DesktopRec *) * new_size);
  }
  ...
  for (i = desktop_data->numCachedIcons + desktop_data->numIconsUsed;
    ...
    n = 0;
    XtSetArg(args[n], XmNmwmFunctions, NULL);
    n++;
    XtSetArg(args[n], XmNmwmDecorations, NULL);
    n++;
    XtSetArg(args[n], XmNallowShellResize, True);
    n++;
    XtSetArg(args[n], XmNvisual, CopyFromParent);
    n++;
    desktop->shell = pu_shell = XtCreatePopupShell(
        popup_name, topLevelShellWidgetClass, toplevel, args, n);
    ...
    XtAddEventHandler(drawA, ButtonPressMask, False, DrawAInput,
                      (XtPointer)desktop);

    /* Callback for handling selection */
    XtAddCallback(drawA, XmNinputCallback, FileWindowInputCallback,
                  (XtPointer)desktop);

    /* Event handler for detecting drag threshold surpassed */
    XtAddEventHandler(drawA, Button1MotionMask | Button2MotionMask, False,
                      (XtEventHandler)FileIconMotion, NULL);
    ...
    n++;
    XtSetArg(args[n], "drawShadow", True);
    n++;
...
void RegisterIconDropsDT(DesktopRec *desktopWindow) {
  XSync(XtDisplay(desktopWindow->shell), 0);

  SetHotRects(desktopWindow->file_view_data, DropOnDesktopObject,
              (XtPointer)desktopWindow);

  XtRemoveAllCallbacks(desktopWindow->iconGadget, XmNcallback);
  XtAddCallback(desktopWindow->iconGadget, XmNcallback,
                (XtCallbackProc)IconCallback, desktopWindow->file_view_data);
}
...
static FileViewData *BuildNewOrderlist(FileViewData *order_list, Widget icon,
  ...
  file_view_data->directory_set = (XtPointer)XtMalloc(sizeof(DirectorySet));
  ...
  file_view_data->parent = file_view_data->next = file_view_data->desc = NULL;
  ...
  file_view_data->ndir = file_view_data->nfile = file_view_data->nnew = 0;
...
void DesktopObjectRemoved(DesktopRec *desktopWindow) {
    ...
    for (k = 0; k < workSpace->files_selected; ++k) {
      if (workSpace->selectedDTWindows[k] == desktopWindow) {
        for (l = k; l < workSpace->files_selected; ++l) {
          workSpace->selectedDTWindows[l] = workSpace->selectedDTWindows[l + 1];
        }
...
void DesktopObjectChanged(DesktopRec *desktopWindow) {
  FileData *file_data;
  Arg args[1];
  ...
  pixmapData = _DtRetrievePixmapData(
      file_data->logical_type, file_data->file_name,
      desktopWindow->dir_linked_to, desktopWindow->shell, desktopIconType);
  ...
  _DtCheckAndFreePixmapData(file_data->logical_type, desktopWindow->shell,
...
static void MakeDesktopWindow(DesktopRec *desktopWindow, int rX, int rY) {
  Colormap colormap;
  Pixel background;
  Pixel foreground;
  Pixel top_shadow;
  ...
  XmGetColors(XtScreen(desktopWindow->shell), colormap, background, &foreground,
  ...
  if (rX == -1) {
    CalculateRootCoordinates(desktopWindow->workspace_num, width, height, &rX,
                             &rY);
  }
  ...
  RegisterInGrid((int)width, (int)height, rX, rY, desktopWindow->workspace_num,
                 True);
}

void SaveDesktopInfo(int session) {
  ...
  case NORMAL_RESTORE:
    ...
    fileName =
        ResolveLocalPathName(home_host_name, desktop_dir, desktopFileName,
                             home_host_name, &tt_status);
    break;
  case HOME_DIR_RESTORE:
    full_path = (char *)XtMalloc(strlen(dt_path) + strlen("/home") + 1);
    sprintf(full_path, "%s/home", dt_path);
    fileName = ResolveLocalPathName(home_host_name, full_path, desktopFileName,
                                    home_host_name, &tt_status);
    XtFree(full_path);
    full_path = NULL;
    break;
  ...
  for (i = 0; i < desktop_data->numIconsUsed; i++) {
    ...
    fprintf(fptr, "%s\n%s\n%s\n%s\n%s\n%s\n%d %d %d %d %d %d %d\n",
            desktopWindow->file_name, desktopWindow->workspace_name,
            desktopWindow->dir_linked_to, rDir, title, helpVol,
            desktopWindow->toolbox, desktopWindow->view, desktopWindow->order,
            desktopWindow->direction, desktopWindow->positionEnabled,
            desktopWindow->root_x, desktopWindow->root_y);
...
void LoadDesktopInfo(char *session) {
  static char *desktopFileName = NULL;
  ...
  for (i = count; i < numWindows + count; i++) {
    int len = 0, toolbox = False, view = UNSET_VALUE, order = UNSET_VALUE,
        direction = UNSET_VALUE, positionEnabled = UNSET_VALUE;
    ...
    { /* Construct the full path and check for the file or link
      ...
      */
      ...
      if (stat(path, &stat_buf) != 0 && lstat(path, &stat_buf) != 0) {
    ...
    if (fgets(message, bufferSize, fptr)) {
      sscanf(message, "%d %d %d %d %d %d %d\n", &toolbox, &view, &order,
             &direction, &positionEnabled, &rX, &rY);
    } else
      message = NULL;
    ...
    for (j = 0; j < desktop_data->numWorkspaces; j++) {
      if (strcmp(workSpace, desktop_data->workspaceData[j]->name) == 0) {
        ...
        desktopWindow->workspace_num = desktop_data->workspaceData[j]->number;
        break;
      }
    }
    ...
    desktopWindow->file_view_data = BuildNewOrderlist(
        NULL, desktopWindow->iconGadget, desktopWindow->host,
        desktopWindow->dir_linked_to, desktopWindow->file_name,
        desktopWindow->registered, desktopWindow->toolbox);
...
static void FreeDesktopWindow(DesktopRec *desktopWindow) {
  ...
  for (; i < desktop_data->numIconsUsed - 1; i++) {
    desktop_data->desktopWindows[i] = desktop_data->desktopWindows[i + 1];
  }
...
static void DrawAInput(Widget w, XtPointer client_data, XEvent *event,
  ...
  if (num_of_files > 1) {
    ...
    XtRemoveAllCallbacks(desktop_data->popupMenu->removeDT,
                         XmNactivateCallback);
    XtAddCallback(
        desktop_data->popupMenu->removeDT, XmNactivateCallback, RemoveDT,
    ...
    XtRemoveAllCallbacks(desktop_data->popupMenu->rename, XmNactivateCallback);
    ...
    if (desktopWindow->toolbox == False) { /* Sensitize rename
                                            */
      XtSetSensitive(desktop_data->popupMenu->rename, True);
      XtAddCallback(desktop_data->popupMenu->rename, XmNactivateCallback,
                    renameDT, (XtPointer)desktopWindow);
    } else { /* Don't allow user to rename the objects in the tool box.
    ...
    desktop_data->popupMenu->action_pane_file_type =
        XtNewString(desktopWindow->file_view_data->file_data->logical_type);

    UpdateActionMenuPane(
        (XtPointer)desktopWindow, (FileMgrRec *)NULL,
        desktop_data->popupMenu->action_pane_file_type, DESKTOP, 7,
        desktop_data->popupMenu->popup,
        desktopWindow->file_view_data->file_data->physical_type);

    if (desktopWindow->file_view_data->file_data->physical_type ==
            DtDIRECTORY &&
        desktopWindow->file_name) {
      char *ptr;
      ...
      if (ptr != NULL) {
        ptr++;
        if (strcmp(root_title, ptr) ==
            0) { /* The desktop object is a ROOT directory */
          XtSetSensitive(desktop_data->popupMenu->openFolder, False);
          XtSetSensitive(desktop_data->popupMenu->rename, False);
  ...
  if (event == NULL) {
    ...
    if (beventy == -1)
      beventy =
          desktopWindow->root_y + desktopWindow->iconGadget->core.height / 2;
...
void RemoveDT(Widget w, XtPointer client_data, XtPointer call_data) {
...
#ifdef SUN_PERF
void RemoveMovedObjectFromDT(Widget w, XtPointer client_data, int file_cnt,
                             char **file_list) {
  DesktopRec *desktopWindow = (DesktopRec *)client_data;
...
static void OpenFolder(Widget w, XtPointer client_data, XtPointer call_data) {
  ...
  if (desktopWindow->toolbox) {
    ...
    if (desktopWindow->restricted_directory != NULL) {
      ...
      if (desktopWindow->toolbox)
        dialog_data =
            GetNewView(desktopWindow->host, desktopWindow->dir_linked_to,
                       desktopWindow->restricted_directory, NULL, 0);
      else
        dialog_data = GetNewView(desktopWindow->host,
                                 desktopWindow->dir_linked_to, NULL, NULL, 0);
    } else {
      dialog_data = GetNewView(desktopWindow->host,
                               desktopWindow->dir_linked_to, NULL, NULL, 0);
    }
...
void SelectDTFile(DesktopRec *desktopWindow) {
  int selection_count;
  int i, j;
  ...
  for (i = 0; i < desktop_data->numWorkspaces; i++) {
    if (strcmp(workspace_name, desktop_data->workspaceData[i]->name) == 0) {
      ...
      desktop_data->workspaceData[i]->selectedDTWindows =
          (DesktopRec **)XtRealloc(
              (char *)desktop_data->workspaceData[i]->selectedDTWindows,
              sizeof(DesktopRec *) *
  ...
  SetToSelectColors(desktopWindow->iconGadget, desktopWindow->frame, LINK_FILE);
  XSync(display, False);
}

void DeselectAllDTFiles(WorkspaceRec *workspaceData) {
  int selection_count;
  int i;
  ...
  for (i = 0; i < selection_count; i++) {
    SetToNormalColors(workspaceData->selectedDTWindows[i]->iconGadget,
                      ...
                      workspaceData->selectedDTWindows[i]->frame, LINK_FILE);
  }
...
void DeselectDTFile(WorkspaceRec *workspaceData, DesktopRec *desktopWindow) {
  int selection_count;
  int i;
  int j;
  ...
  workspaceData->selectedDTWindows =
      (DesktopRec **)XtRealloc((char *)workspaceData->selectedDTWindows,
...
void DTActionCallback(Widget w, XtPointer client_data, XtPointer call_data) {
  DesktopRec *desktopWindow;
  char *command;
  Arg args[1];
...
void RunDTCommand(char *command, DesktopRec *desktopWindow,
                  DtDndDropCallbackStruct *drop_parameters)
  ...
  if ((strcmp(openNewView, command) == 0) ||
    ...
    if (strcmp(ltype, LT_FOLDER_LOCK) == 0) {
      ...
      msg = (char *)XtMalloc(
          ...
          strlen(desktopWindow->file_view_data->file_data->file_name) + 1);
      sprintf(msg, GETMESSAGE(30, 1, "Cannot read from %s"),
              desktopWindow->file_view_data->file_data->file_name);
      _DtMessage(desktopWindow->shell, title, msg, NULL, HelpRequestCB);
      XtFree(title);
      title = NULL;
      XtFree(msg);
      msg = NULL;
    ...
    if ((strcmp(ltype, LT_AGROUP) == 0) || (strstr(ltype, LT_AGROUP_SUBDIR)) ||
        (strcmp(ltype, LT_TRASH) == 0)) {
      ProcessAction(command, desktopWindow->file_view_data, drop_parameters,
                    desktopWindow->host, desktopWindow->dir_linked_to,
                    desktopWindow->restricted_directory, desktopWindow->shell);
    } else {
...
static void ProcessMoveCopyLinkDT(char *command, DesktopRec *desktopWindow,
  ...
  if (!error)
    CheckMoveType((FileMgrData *)NULL, file_view_data,
                  ...
                  desktopWindow, file_set, host_set, modifiers, numFiles,
                  drop_parameters->x, drop_parameters->y, DESKTOP);
...
ProcessBufferDropOnFolderDT(char *command, DesktopRec *desktopWindow,
  ...
  MakeFilesFromBuffersDT(file_view_data, directory, file_set, host_set,
                         buffer_set, num_of_buffers, desktopWindow, NULL, NULL);
...
char *IsAFileOnDesktop2(char **file_set, int file_count, int *number,
                        Boolean *IsToolBox) {
  int i, j;
  DesktopRec *desktopWindow;
  ...
  for (i = 0; i < file_count; i++) {
    ...
    for (j = 0; j < desktop_data->numIconsUsed; j++) {
      ...
      */
      ...
      filename2 = (char *)XtRealloc(
          filename2,
          strlen(filename2) +
              strlen(desktopWindow->file_view_data->file_data->file_name) + 2);
      strcat(filename2, "/");
      ...
      if (strcmp(filename1, filename2) == 0) {
        *IsToolBox = desktopWindow->toolbox;
        if (onDesktop == False) {
          ...
          strcat(filesOnDesktop, "   ");
          strcat(filesOnDesktop, filename1);
          strcat(filesOnDesktop, "\n");
        }
        *number += 1;
        break;
...
void ProcessDTSelection(DesktopRec *desktopRec, XButtonEvent *event) {
  Window rootWindow;
  Atom pCurrent;
  Screen *currentScreen;
  int screen;
...
Boolean DTFileIsSelected(DesktopRec *desktopRec, FileViewData *fileViewData) {
  Window rootWindow;
  Atom pCurrent;
  Screen *currentScreen;
  int screen;
...
#define EXPECTED_PANEL_HEIGHT 83
  RegisterInGrid(EXPECTED_PANEL_WIDTH, EXPECTED_PANEL_HEIGHT,
                 ...
                 (displayWidth > EXPECTED_PANEL_WIDTH)
                     ? (displayWidth - EXPECTED_PANEL_WIDTH) / 2
                     : 0,
...
void InitializeDesktopGrid(int displayWidth, int displayHeight) {
  int i, j, k;
...
void RegisterInGrid(int width, int height, int rX, int rY, int workspace,
                    Boolean type) {
  int row, column;
...
void PutOnDTCB(Widget w, XtPointer client_data, XtPointer call_data) {
  Arg args[1];
  FileMgrRec *file_mgr_rec;
  DialogData *dialog_data;
  FileMgrData *file_mgr_data;
  ...
  if (file_view_data == NULL) {
    for (i = 0; i < file_mgr_data->selected_file_count; i++) {
      if (file_mgr_data->toolbox) {
        ...
        *ptr = '\0';
        ...
        SetupDesktopWindow(XtDisplay(w), file_mgr_data, file_mgr_rec, file_name,
                           file_mgr_data->host, directory, -1, -1, NULL,
                           EndIndex);
        ...
        *ptr = '/';
        ...
        directory =
            ((DirectorySet *)(file_mgr_data->selection_list[i]->directory_set))
                ->name;
    ...
    if (file_mgr_data->toolbox) {
      ...
      *ptr = '/';
      ...
      SetupDesktopWindow(XtDisplay(w), file_mgr_data, file_mgr_rec, file_name,
                         file_mgr_data->host, file_mgr_data->current_directory,
                         -1, -1, NULL, EndIndex);
    }
  }
}
...
void SetupDesktopWindow(Display *display, FileMgrData *file_mgr_data,
                        FileMgrRec *file_mgr_rec, char *file_name,
                        char *hst_name, char *dir_name, int root_x, int root_y,
                        char *type, int EndIndex) {
  char *title;
  char title_buf[256];
  ...
  directory_name = ResolveLocalPathName(hst_name, dir_name, file_name,
                                        home_host_name, &tt_status);
  if (TT_OK != tt_status) { /* Should pop up an error dialog.
                             */
    return;
  }
  ...
  restricted_top =
      file_mgr_data && file_mgr_data->restricted_directory &&
      strcmp(directory_name, file_mgr_data->restricted_directory) == 0;
    ...
    if (strcmp(desktop_data->desktopWindows[i]->title, title) == 0 &&
      ...
      tmpStr =
          (GETMESSAGE(28, 11,
                      ...
                      "backdrop in this Workspace.\nYou cannot put another "
                      "object with the same name on the Workspace.\nTo have "
                      "both objects on the Workspace, rename one of them."));
      msg = XtNewString(tmpStr);
  ...
  desktopWindow->file_view_data = BuildNewOrderlist(
      NULL /* @@@ use cached info if possible! */, desktopWindow->iconGadget,
      home_host_name, desktopWindow->dir_linked_to, file_name,
      desktopWindow->registered, desktopWindow->toolbox);
  ...
  if (checkBrokenLink != 0 && checkBrokenLinkTimerId == 0) {
    checkBrokenLinkTimerId =
        XtAppAddTimeOut(XtWidgetToApplicationContext(toplevel),
                        checkBrokenLink * 1000, TimerEventBrokenLinks, NULL);
  }
}
...
static void FreeCachedIcons(int iconsToBeFreed) {
  ...
  for (i = desktop_data->numCachedIcons + desktop_data->numIconsUsed;
    ...
    if (desktop == sacredDesktop) {
      ...
      desktop_data->desktopWindows[desktop_data->numIconsUsed] = sacredDesktop;
    }
...
static void CreatePopupMenu(Widget drawA) {
  ...
  n++;
  desktop_data->popupMenu->popup = XmCreatePopupMenu(drawA, "DTPopup", args, n);

  XtAddCallback(desktop_data->popupMenu->popup, XmNmapCallback, popupMenu,
                (XtPointer)NULL);
  XtAddCallback(desktop_data->popupMenu->popup, XmNhelpCallback,
                (XtCallbackProc)DTHelpRequestCB, HELP_DESKTOP_POPUP_MENU_STR);

  label_string =
      XmStringCreateLocalized((GETMESSAGE(28, 4, "Workspace Object")));
  XtSetArg(args[0], XmNlabelString, label_string);
  popupBtns[0] = (Widget)XmCreateLabelGadget(desktop_data->popupMenu->popup,
                                             "title", args, 1);
  popupBtns[1] =
      XmCreateSeparatorGadget(desktop_data->popupMenu->popup, "sep1", NULL, 0);
  popupBtns[2] =
      XmCreateSeparatorGadget(desktop_data->popupMenu->popup, "sep2", NULL, 0);
  XmStringFree(label_string);

  label_string =
      XmStringCreateLocalized((GETMESSAGE(28, 5, "Remove From Workspace")));
  XtSetArg(args[0], XmNlabelString, label_string);
  mnemonic = ((char *)GETMESSAGE(28, 6, "R"));
  XtSetArg(args[1], XmNmnemonic, mnemonic[0]);
  desktop_data->popupMenu->removeDT = popupBtns[3] =
      XmCreatePushButtonGadget(desktop_data->popupMenu->popup, "rmDT", args, 2);
  XmStringFree(label_string);
  XtAddCallback(desktop_data->popupMenu->removeDT, XmNhelpCallback,
                (XtCallbackProc)DTHelpRequestCB, HELP_DESKTOP_POPUP_MENU_STR);

  label_string =
      XmStringCreateLocalized((GETMESSAGE(28, 7, "Open Parent Folder")));
  XtSetArg(args[0], XmNlabelString, label_string);
  mnemonic = ((char *)GETMESSAGE(28, 8, "O"));
  XtSetArg(args[1], XmNmnemonic, mnemonic[0]);
  desktop_data->popupMenu->openFolder = popupBtns[4] =
      XmCreatePushButtonGadget(desktop_data->popupMenu->popup, "info", args, 2);
  XmStringFree(label_string);
  ...
  XtSetArg(args[1], XmNmnemonic, mnemonic[0]);
  desktop_data->popupMenu->rename = popupBtns[5] = XmCreatePushButtonGadget(
      desktop_data->popupMenu->popup, "rename", args, 2);
  XmStringFree(label_string);
...
static void popupMenu(Widget w, XtPointer client_data, XtPointer call_data) {
  ...
  if (found) {
    DrawAInput(desktop_data->popupMenu->popup,
               (XtPointer)desktop_data->desktopWindows[i], event,
               (Boolean *)NULL);
  }
}

#ifdef SHAPE
void GenerateShape(DesktopRec *desktopWindow) {
  ...
  _DtIconGetIconRects((DtIconGadget)desktopWindow->iconGadget, &flags, &rect[0],
                      &rect[1]);
    ...
    if ((Dimension)(stringHeight + 5) >= pixmapHeight) {
      if (pixmapHeight == 0) {
        ...
        if (pixmapHeight > stringHeight)
          rect[1].height += 2 * shadowThickness + (pixmapHeight - stringHeight);
        else
          rect[1].height += 2 * shadowThickness;
  ...
  XShapeCombineRectangles(XtDisplay(desktopWindow->shell),
                          XtWindow(desktopWindow->shell), ShapeBounding, 0, 0,
                          &rect[0], 2, ShapeSet, Unsorted);
}
#endif
...
static void BuildDesktopLinks(Display *display) {
  ...
  desktop_data->workspaceData = (WorkspaceRec **)XtMalloc(
      sizeof(WorkspaceRec *) * desktop_data->numWorkspaces);
  for (i = 0; i < desktop_data->numWorkspaces; i++) {
...
static int InitializeNewWorkspaces(Display *display, char *workspaceName) {
  ...
  if (DtWsmGetWorkspaceList(display, rootWindow, &paWS, &numInfo) == Success) {
    ...
    desktop_data->workspaceData = (WorkspaceRec **)XtRealloc(
        (char *)desktop_data->workspaceData, sizeof(WorkspaceRec *) * numInfo);
  ...
  for (i = desktop_data->numWorkspaces; i < numInfo; i++) {
    for (j = 0; j < numColumns; j++)
      for (k = 0; k < numRows; k++)
        desktop_grid[(i * numRows * numColumns) + (j * numRows) + k] = 0;

    RegisterPanelInGrid(i + 1, DisplayWidth(display, screen),
 ...
 *
 *********************************************************************/
void CleanUpWSName(char *workspace_name) {
  char *ptr;
 ...
 *
 ***********************************************************************/
void CheckDesktopMarquee(int x, int y, int width, int height) {
  int i, j;
  Window rootWindow;
  Atom pCurrent;
  Screen *currentScreen;
  ...
  for (i = 0; i < desktop_data->numIconsUsed; i++) {
    ...
    case XmPIXMAP_LEFT: /* 2 */
      if (flags & XmPIXMAP_RECT) {
        pixmapRect.x =
            pixmapRect.x + desktop_data->desktopWindows[i]->root_x + 1;
        pixmapRect.y =
            pixmapRect.y + desktop_data->desktopWindows[i]->root_y + 1;
        ...
        pointInRegion = (XPointInRegion(region, pixmapRect.x, pixmapRect.y) &&
                 ...
                 XmLABEL_RECT) { /* For the ones that don't have pixmap */
      ...
      if (pointInRegion) {
        ...
        workspaceData->selectedDTWindows = (DesktopRec **)XtRealloc(
            (char *)workspaceData->selectedDTWindows,
            sizeof(DesktopRec *) * (workspaceData->files_selected + 2));
        ...
        workspaceData->selectedDTWindows[0] = desktop_data->desktopWindows[i];
        SetToSelectColors((Widget)g, desktop_data->desktopWindows[i]->frame,
                          LINK_FILE);
        XSync(display, False);
      } else {
        ...
        for (j = 0; j < workspaceData->files_selected; j++) {
          if (workspaceData->selectedDTWindows[j] ==
              desktop_data->desktopWindows[i]) {
            DeselectDTFile(workspaceData, desktop_data->desktopWindows[i]);
            XSync(display, False);
            break;
          }
        }
...
static void ProcessDropOnDesktopObject(Widget w,
  ...
  if (dropInfo->reason != DtCR_DND_DROP_ANIMATE) {
    ...
    if (FileFromTrash(dropInfo->dropData->data.files[0])) {
      if (InvalidTrashDragDrop(dropInfo->operation, FROM_TRASH,
                               desktopWindow->shell)) {
  ...
  /******************************************/
    ...
    command =
        TypeToAction(dropInfo->operation,
                     desktopWindow->file_view_data->file_data->logical_type);
    if (command) {
      RunDTCommand(command, desktopWindow, dropInfo);
...
void WorkSpaceRemoved(Widget w, Atom deleted_ws_atom, int type,
  ...
  if (type == DtWSM_MODIFY_WORKSPACE_TYPE_DELETE) {
    ...
    old_ws_num =
        GetWorkspaceNum(old_workspace_name, oldNumWorkspaces, &old_index);
    ...
    for (i = 0; i < desktop_data->numIconsUsed;
         i++) { /* Those objects whose workspace number matches the deleted
                   workspace number */
      if (desktop_data->desktopWindows[i]->workspace_num ==
          old_ws_num) { /* Look up in all of the workspace objects */
        for (j = 0; j < desktop_data->numIconsUsed;
             j++) { /* Those objects in the current workspace */
          if (j != i &&
              desktop_data->desktopWindows[j]->workspace_num == ws_num) {
            /* If there is an object matching the old workspace object delete
             * the old */
            if (strcmp(desktop_data->desktopWindows[j]->file_name,
                       desktop_data->desktopWindows[i]->file_name) == 0) {
              ws_num = GetWorkspaceNum(workspace_name,
                                       desktop_data->numWorkspaces, &new_index);
              for (k = 0;
                   k < desktop_data->workspaceData[new_index]->files_selected;
                   k++) {
                /* Adjust the selectedDTWindows of current to accommadate the
                 * old list */
                if (desktop_data->workspaceData[new_index]
                        ->selectedDTWindows[k] ==
                    desktop_data->desktopWindows[i]) {
                  desktop_data->workspaceData[new_index]->selectedDTWindows[k] =
                      NULL;
                  for (; k <
              ...
              XWithdrawWindow(XtDisplay(desktop_data->desktopWindows[i]->shell),
                              ...
                              XDefaultScreen(XtDisplay(
                                  desktop_data->desktopWindows[i]->shell)));
...
static void FreeUpOldWorkspace(int oldNumWorkspaces, int old_ws_num, int ws_num,
  ...
  if (numoldfiles) {
    desktop_data->workspaceData[new_index]
        ->selectedDTWindows = (DesktopRec **)XtRealloc(
        (XtPointer)desktop_data->workspaceData[new_index]->selectedDTWindows,
    ...
    for (j = numnewfiles; j < numnewfiles + numoldfiles; j++) {
      desktop_data->workspaceData[new_index]->selectedDTWindows[j] =
          desktop_data->workspaceData[old_index]
              ->selectedDTWindows[j - numnewfiles];
    }
    XtFree(
        (XtPointer)desktop_data->workspaceData[old_index]->selectedDTWindows);
    desktop_data->workspaceData[old_index]->selectedDTWindows = NULL;
    desktop_data->workspaceData[new_index]->files_selected =
        numnewfiles + numoldfiles;
  }

  numnewfiles =
      desktop_data->workspaceData[new_index]->secondaryHelpDialogCount;
  numoldfiles =
      desktop_data->workspaceData[old_index]->secondaryHelpDialogCount;
  if (numoldfiles) {
    desktop_data->workspaceData[new_index]->secondaryHelpDialogList =
        (DialogData **)XtRealloc(
            (XtPointer)desktop_data->workspaceData[old_index]
                ->secondaryHelpDialogList,
            sizeof(DialogData *) * (numnewfiles + numoldfiles));
    for (j = numnewfiles; j < numnewfiles + numoldfiles; j++) {
      desktop_data->workspaceData[new_index]->secondaryHelpDialogList[j] =
          desktop_data->workspaceData[old_index]
              ->secondaryHelpDialogList[j - ws_num];
    }
    desktop_data->workspaceData[new_index]->secondaryHelpDialogCount =
        numnewfiles + numoldfiles;
  }
...
void PutOnWorkspaceHandler(Tt_message ttMsg) {
  DesktopRec *desktopWindow;
  ...
  if (tt_is_err(tt_ptr_error(fullName))) { /* No file name */
  ...
  if (NULL == fullName ||
    ...
    template = XtNewString(GETMESSAGE(
  ...
  for (i = 0; i < desktop_data->numIconsUsed; ++i) {
    if (strcmp(desktop_data->desktopWindows[i]->title, title) == 0 &&
        strcmp(desktop_data->desktopWindows[i]->workspace_name, workspace) ==
            0) {
      ...
      if (invalidWorkspace)
        template = XtNewString(GETMESSAGE(
            28, 12,
            "An invalid workspace was passed. The default workspace will "
            "be\nworkspace one and an object named:\n\n   %s\n\nalready exists "
            "on the backdrop in this Workspace.\nYou cannot put another object "
            "with the same name on the Workspace.\nTo have both objects on the "
            "Workspace, rename one of them."));
      else
        template = XtNewString(
            GETMESSAGE(28, 11,
                       ...
                       "object with the same name on the Workspace.\nTo have "
                       "both objects on the Workspace, rename one of them."));
      errorMsg = (char *)XtMalloc(strlen(template) + strlen(title) + 1);
  ...
  desktopWindow->workspace_name =
      XtNewString(desktop_data->workspaceData[ws]->name);
  ...
  desktopWindow->file_view_data = BuildNewOrderlist(
      NULL /* @@@ use cached info if possible! */, desktopWindow->iconGadget,
      home_host_name, desktopWindow->dir_linked_to, fileName,
      desktopWindow->registered, desktopWindow->toolbox);
