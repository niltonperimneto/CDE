...
typedef void (*ChangeDirCallbackProc)(FileMgrData *file_mgr_data,
                                      char *dir_name);

static DialogResource resources[] = {
    {"string_path", XmRXmString, sizeof(XmString),
     XtOffset(ChangeDirDataPtr, string_path), NULL,
     (WriteResourceProc)_DtXmStringToString},

    {"historyList", XmRXmStringTable, sizeof(XmStringTable),
     XtOffset(ChangeDirDataPtr, history_list), NULL,
     (WriteResourceProc)_DtXmStringTableToString},

    {"visibleCount", XmRInt, sizeof(int),
     XtOffset(ChangeDirDataPtr, visible_count), (XtPointer)8,
     (WriteResourceProc)_DtIntToString},

    {"listCount", XmRInt, sizeof(int), XtOffset(ChangeDirDataPtr, list_count),
     NULL, (WriteResourceProc)_DtIntToString},
};

/********    Static Function Declarations    ********/

static void Create(Display *display, Widget parent, Widget *shell,
                   XtPointer *dialog);
static void InstallChange(ChangeDirRec *change_dir_rec, XtCallbackProc callback,
                          XtPointer client_data);
static void InstallClose(ChangeDirRec *change_dir_rec, XtCallbackProc callback,
                         XtPointer client_data);
...
static Boolean TryToChangeDir(Boolean isFromDialog, char *incoming_string,
                              FileMgrData *file_mgr_data,
                              FileMgrRec *file_mgr_rec,
                              ChangeDirData *change_dir_data,
                              Widget selection_box, ChangeDirApply *apply_data,
                              XmSelectionBoxCallbackStruct *callback_data,
                              XtPointer callback);
static void OkCallback(Widget selection_box, ChangeDirApply *apply_data,
                       XmSelectionBoxCallbackStruct *callback_data);
static int CheckRestrictedDir(FileMgrRec *file_mgr_rec,
                              FileMgrData *file_mgr_data, char **value,
                              Boolean relative_name);
static void GoToNewDir(FileMgrData *file_mgr_data, char *dir_name);
static void SetFocus(ChangeDirRec *change_dir_rec,
                     ChangeDirData *change_dir_data);
...
static void Create(Display *display, Widget parent, Widget *shell,
  ...
  prompt = XmStringCreateLocalized(
      ((char *)GETMESSAGE(2, 14, "Destination Folder:")));

  trans_table = XtParseTranslationTable(translations_space);
  n = 0;
  XtSetArg(args[n], XmNselectionLabelString, prompt);
  n++;
  XtSetArg(args[n], XmNokLabelString, okXmString);
  n++;
  XtSetArg(args[n], XmNcancelLabelString, cancelXmString);
  n++;
  XtSetArg(args[n], XmNhelpLabelString, helpXmString);
  n++;
  ...
  w = XmSelectionBoxGetChild(change_dir_rec->change_dir, XmDIALOG_HELP_BUTTON);
  XtSetValues(w, args, 2);
...
static void InstallClose(ChangeDirRec *change_dir_rec, XtCallbackProc callback,
  ...
  close_widget = XmSelectionBoxGetChild(change_dir_rec->change_dir,
                                        XmDIALOG_CANCEL_BUTTON);

  XtAddCallback(close_widget, XmNactivateCallback, callback, client_data);

  delete_window_atom =
      XmInternAtom(XtDisplay(change_dir_rec->shell), "WM_DELETE_WINDOW", True);
...
static XtPointer GetValues(ChangeDirRec *change_dir_rec) {
  ...
  n = 0;
  XtSetArg(args[n], XmNx, &change_dir_data->x);
  n++;
  XtSetArg(args[n], XmNy, &change_dir_data->y);
  n++;
  XtSetArg(args[n], XmNwidth, &change_dir_data->width);
  n++;
  XtSetArg(args[n], XmNheight, &change_dir_data->height);
  n++;
  XtSetArg(args[n], XmNlistItemCount, &change_dir_data->list_count);
  n++;
  XtSetArg(args[n], XmNlistVisibleItemCount, &change_dir_data->visible_count);
  n++;
  XtSetArg(args[n], XmNtextString, &string_path);
  n++;
  XtSetArg(args[n], XmNlistItems, &history_list);
  n++;
  XtGetValues(change_dir_rec->change_dir, args, n);

  /*  Extract and make local copies of the XmString data.  */

  change_dir_data->history_list = (XmStringTable)XtMalloc(
...
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list) {
  ...
  _DtDialogGetResources(data_base, name_list, CHANGEDIRECTORY,
                        (char *)change_dir_data, resources,
                        changeDirClass->resource_count);
...
static void SetValues(ChangeDirRec *change_dir_rec,
  ...
  if (restrictMode && file_mgr_data->toolbox == False)
    change_dir_data->string_path = XmStringCreateLocalized(users_home_dir);
  ...
  if (change_dir_data->string_path) {
    ...
    XtAddCallback(text, XmNmodifyVerifyCallback, (XtCallbackProc)TextChange,
                  (XtPointer)file_mgr_data);
    XtAddCallback(text, XmNmotionVerifyCallback, (XtCallbackProc)TextChange,
                  (XtPointer)file_mgr_data);
    XtAddCallback(text, XmNvalueChangedCallback, (XtCallbackProc)TextChange,
                  (XtPointer)file_mgr_data);
  }
  ...
  XtSetValues(
      XmSelectionBoxGetChild(change_dir_rec->change_dir, XmDIALOG_LIST_LABEL),
      args, 1);
...
static void WriteResourceValues(DialogData *values, int fd, char **name_list) {
  ...
  _DtDialogPutResources(fd, name_list, CHANGEDIRECTORY, values->data, resources,
                        changeDirClass->resource_count);
}
...
static Boolean TryToChangeDir(Boolean isFromDialog, char *incoming_string,
  ...
  if ((new_directory) && (strcmp(new_directory, "") != 0)) {
    _DtPathFromInput(new_directory, file_mgr_data->current_directory, &host,
                     &path);
  } else {
    tmpStr = GetSharedMessage(CHANGE_DIR_ERROR_TITLE);
    title = XtNewString(tmpStr);
    tmpStr = GETMESSAGE(2, 18,
                        "Destination Folder name is missing.\nType in a folder "
                        "name or select a folder from the list.");
  ...
  */
  if ((path) && (stat(path, &stat_buf) == 0) &&
    ...
    if (CheckAccess(path, R_OK | X_OK)) {
      ...
      */
      ...
      if (selection_box) {
        ...
        message_buf =
            XtMalloc(strlen(tmpStr) + strlen(change_dir_data->host_name) + 1);
        sprintf(message_buf, "%s%s", tmpStr, change_dir_data->host_name);
        host_string = XmStringCreateLocalized(message_buf);
        XtSetArg(args[0], XmNlabelString, host_string);
      ...
      if (isFromDialog)
        ((XtCallbackProc)callback)(selection_box, apply_data->client_data,
                                   path);
      else
        ((ChangeDirCallbackProc)callback)(file_mgr_data, path);
    }
  } else {
    tmpStr = GetSharedMessage(CHANGE_DIR_ERROR_TITLE);
    title = XtNewString(tmpStr);
    ...
    if (isFromDialog)
      ...
      _DtMessage(file_mgr_rec->current_directory_text, title, message_buf, NULL,
                 HelpRequestCB);
...
static void OkCallback(Widget selection_box, ChangeDirApply *apply_data,
  ...
  if (result == 0) {
    TryToChangeDir(True, (char *)strValue, file_mgr_data, file_mgr_rec,
                   change_dir_data, selection_box, apply_data, callback_data,
                   (XtPointer)apply_data->callback);
  }
  ...
  *value = (char *)DtEliminateDots(*value);
  ...
  if (strncmp(*value, directory, len) != 0 ||
    ...
    if (restrictMode)
      tmpBuffer =
          GETMESSAGE(2, 8,
                     "You cannot switch to this folder.  You are\nallowed to "
                     "view only folders beneath your\ncurrent folder. You "
                     "cannot specify an absolute\npath to the new folder.");
    else
      tmpBuffer = GETMESSAGE(2, 9,
                             "This view is in restricted mode.  You cannot "
                             "go\nto the specified folder because it is not in "
...
void ChangeToNewDir(Widget text_field, XtPointer client_data,
...
void DropOnChangeView(Widget w, XtPointer client_data, XtPointer call_data) {
  ...
  if (numFiles > 0 && fileList->reason == DtCR_DND_DROP_ANIMATE) {
    ...
    if (CheckRestrictedDir(file_mgr_rec, file_mgr_data, &full_path_name,
                           False) == 0) {
      change_dir_data = (ChangeDirData *)file_mgr_data->change_dir->data;
      change_dir_rec =
          (ChangeDirRec *)_DtGetDialogInstance(file_mgr_data->change_dir);
...
static void GoToNewDir(FileMgrData *file_mgr_data, char *dir_name) {
  ...
  if (file_mgr_data->change_dir) {
    ...
    if (!found) {
      ...
      change_dir_data->history_list = (XmStringTable)XtRealloc(
          (char *)change_dir_data->history_list,
          sizeof(XmStringTable) * (change_dir_data->list_count + 1));
...
void TextChange(Widget text, XtPointer client_data,
      ...
      if (callback_data->newInsert < maxlength) {
        if (callback_data->event == NULL)
          _XmTextFieldSetDestination(
              (Widget)text, (Position)callback_data->currInsert, CurrentTime);
        else {
...
void ChangeDirectoryToParent(FileMgrRec *file_mgr_rec, char *directory) {
  DialogData *dialog_data;
  ChangeDirData *change_dir_data;
  ChangeDirRec *change_dir_rec;
  FileMgrData *file_mgr_data;
