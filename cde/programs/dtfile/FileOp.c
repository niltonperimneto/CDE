...
static Boolean CreateFileFromBuffer(int pipe_s2m, char *directory,
                                    char *fully_qualified_name, void *buffer,
                                    int size);
static String appendErrorMessage(String message, String new, String arg);
static void DisplayErrorMessage(int pipe_s2m, char *errMsg, char *from,
                                char *dir);
...
static void DisplayDuplicateOpError(FileOpCBData *cb_data, int index);
  ...
  while (n < len) {
    ...
    if (rc > 0)
      ...
      if (rc == 0) {
        if (whined_fd != fd) {
          whined_fd = fd;
          fprintf(stderr, "PipeRead: broken pipe, ppid=%d pid=%d fd=%d\n",
                  getppid(), getpid(), fd);
        }
      } else {
...
int PipeWriteString(int fd, char *s) {
  short len, sent = 0;
  void (*oldPipe)(int);

  oldPipe = (void (*)(int))signal(SIGPIPE, SIG_IGN);
...
static int PipeWriteErrmsg(int fd, int rc, char *msg, char *arg) {
...
void FileOpError(Widget w, char *message1, char *message2) {
...
static void SendModifyMsg(int pipe_fd, int mode, Boolean first, char *to,
                          DirUpdate *updates, int i, char *target_file) {
  short pipe_msg;
  char *dir_path;
  long modify_time;
  ...
  if (mode == MOVE_FILE && !updates[j].time_sent) {
    ...
    dir_path = ResolveLocalPathName(updates[i].host, updates[i].directory, NULL,
...
static int FileMoveCopyProcess(int pipe_s2m, int pipe_m2s, char *to_file,
                               char *directory, char *host, DirUpdate *updates,
  ...
  target_dir = (char *)ResolveLocalPathName(host, directory, NULL,
                                            home_host_name, &tt_status);
  if (TT_OK != tt_status) {
    char msg1[2 * MAX_PATH];
#ifdef sun
    sprintf(msg1, GetSharedMessage(CANNOT_CONNECT_ERROR_1), directory, host,
            host);
#else
    sprintf(msg1, GetSharedMessage(CANNOT_CONNECT_ERROR_2), directory, host,
            host);
#endif
  ...
  for (i = 0; i < file_count; i++) {
    ...
    if (mode != MAKE_BUFFER) {
      from = ResolveLocalPathName(updates[i].host, updates[i].directory,
                                  updates[i].file, home_host_name, &tt_status);
      if (from == NULL) {
        char msg1[2 * MAX_PATH];
      ...
      if (updates[i].app_man_dir) {
        ...
        msg = XtNewString(
            GETMESSAGE(11, 30,
                       ...
                       "be closed before a folder can be moved or renamed."));
    ...
    if (to_file == NULL) {
      if (strcmp(target_dir, "/") == 0) {
        target_file =
            (char *)XtMalloc(strlen(target_dir) + strlen(updates[i].file) + 1);
        sprintf(target_file, "%s%s", target_dir, updates[i].file);
      } else {
        target_file =
            (char *)XtMalloc(strlen(target_dir) + strlen(updates[i].file) + 2);
        sprintf(target_file, "%s/%s", target_dir, updates[i].file);
      }
    } else {
      if (strcmp(target_dir, "/") == 0) {
        target_file =
            (char *)XtMalloc(strlen(target_dir) + strlen(to_file) + 1);
        sprintf(target_file, "%s%s", target_dir, to_file);
      } else {
        target_file =
            (char *)XtMalloc(strlen(target_dir) + strlen(to_file) + 2);
        sprintf(target_file, "%s/%s", target_dir, to_file);
      }
    }
    if (updates[i].app_man_dir)
      target_file = _DtResolveAppManPath(target_file, updates[i].app_man_dir);
    ...
    if (mode == COPY_FILE) {
      if (CheckAccess(target_dir, W_OK) == -1) {
        ...
        if (CopyString == NULL)
          ...
          CopyString = (char *)XtRealloc(CopyString,
                                         strlen(CopyString) + strlen(from) + 2);
          strcat(CopyString, "\n");
    ...
    if (stat(target_file, &stat_buf) == 0) {
      ...
      if (mode == MAKE_BUFFER) {
        ...
        if (S_ISDIR(from_stat_buf.st_mode) && S_ISDIR(stat_buf.st_mode) &&
            mode == COPY_FILE) {
    ...
    if (mode == MAKE_BUFFER) {
      ...
      return_val = CreateFileFromBuffer(pipe_s2m, to, target_file,
      ...
      if (strncmp(directory, desktop_dir, strlen(desktop_dir)) == 0)
        return_val = FileManip((Widget)(intptr_t)pipe_s2m, mode, from, to,
                               isContainer, FileOpError, True, DESKTOP);
      else
        return_val = FileManip((Widget)(intptr_t)pipe_s2m, mode, from, to,
                               isContainer, FileOpError, True, NOT_DESKTOP);
      XtFree((char *)from);
    }
  ...
  if (CopyError == TRUE) {
    ...
    errMsg = GETMESSAGE(11, 48,
                        "Cannot copy the following objects to folder "
                        "\"%s\"\n\n%s\n\nThe most likely cause is that you do "
  ...
  if (MoveError1 == TRUE) {
    ...
    errMsg = GETMESSAGE(11, 49,
                        "Cannot move the following objects to folder "
                        "\"%s\"\n\n%s\n\nThe most likely cause is that you do "
                        "not have\nwrite permission for the target folder");
  ...
  if (sameCount != 0) {
    ...
    /* send message to main process to display dialog (and possibly
     * remove/rename files) */
    if (file_count == 1) {
      if (to_file == NULL) /* note target_file here is only file name, above it
                              is full path */
      ...
      write(pipe_s2m, &pipe_msg, sizeof(short));
      DPRINTF(("FileMoveCopyProcess: sending: mode %d directory \"%s\" file "
               "\"%s\" \n",
      ...
      DPRINTF(("FileMoveCopyProcess: sending msg %d\n", pipe_msg));
      write(pipe_s2m, &pipe_msg, sizeof(short));
      DPRINTF(("FileMoveCopyProcess: sending: mode %d processCount %d "
               "sameCount %d directory \"%s\"\n",
      ...
      PipeWriteString(pipe_s2m, directory);
      DPRINTF(
          ("FileMoveCopyProcess: sending %d filename strings\n", sameCount));
    ...
    if (rc != PIPEMSG_CANCEL) {
      ...
      for (i = 0; i < sameCount; i++) {
        ...
        if (mode != MAKE_BUFFER) {
          from =
              ResolveLocalPathName(updates[j].host, updates[j].directory,
                                   updates[j].file, home_host_name, &tt_status);
          if (TT_OK != tt_status) {
            char msg1[2 * MAX_PATH];

#ifdef sun
            sprintf(msg1, GetSharedMessage(CANNOT_CONNECT_ERROR_1),
                    updates[j].directory, updates[j].host, updates[j].host);
#else
            sprintf(msg1, GetSharedMessage(CANNOT_CONNECT_ERROR_2),
                    updates[j].directory, updates[j].host, updates[j].host);
#endif
        ...
        case PIPEMSG_MERGE:
          ...
          if (strncmp(directory, desktop_dir, strlen(desktop_dir)) == 0)
            return_val = FileManip((Widget)(intptr_t)pipe_s2m, MERGE_DIR, from,
                                   to, isContainer, FileOpError, True, DESKTOP);
          else
            return_val =
                FileManip((Widget)(intptr_t)pipe_s2m, MERGE_DIR, from, to,
                          isContainer, FileOpError, True, NOT_DESKTOP);
          break;

        case PIPEMSG_REPLACE_BUFFER:
          target_file =
              (char *)XtMalloc(strlen(to) + strlen(updates[j].file) + 2);
        ...
        case PIPEMSG_RENAME_BUFFER:
          ...
          target_file = (char *)XtMalloc(strlen(to) + strlen(renamed_file) + 2);
        ...
        default:
          if (strncmp(directory, desktop_dir, strlen(desktop_dir)) == 0)
            ...
            if (Same) {
              ...
              toFile = (char *)XtMalloc(strlen(newDir) + strlen(newFile) + 3);
              strcpy(toFile, newDir);
              strcat(toFile, "/");
              strcat(toFile, newFile);

              return_val =
                  FileManip((Widget)(intptr_t)pipe_s2m, mode, from, toFile,
                            False, FileOpError, True, NOT_DESKTOP);
              XtFree(path);
              XtFree(toFile);
            } else
              return_val =
                  FileManip((Widget)(intptr_t)pipe_s2m, mode, from, to,
                            isContainer, FileOpError, True, NOT_DESKTOP);
          }
        } /* endswitch */
...
static int FileMoveCopyProcessDesktop(int pipe_s2m, int pipe_m2s,
                                      char *directory, char *host,
                                      DirUpdate *updates, int file_count,
                                      int mode, DesktopRec *desktopWindow) {
  char *to, *from;
  int i, rc;
  Boolean first_op = True;
  Boolean return_val = False;
  ...
  for (i = 0; i < file_count; i++) {
    ...
    if (mode == MAKE_BUFFER) {
      ...
      if (strcmp(to, "/") == 0) {
        target_file =
            (char *)XtMalloc(strlen(to) + strlen(updates[i].file) + 1);
        sprintf(target_file, "%s%s", to, updates[i].file);
      } else {
        target_file =
            (char *)XtMalloc(strlen(to) + strlen(updates[i].file) + 2);
        sprintf(target_file, "%s/%s", to, updates[i].file);
      }
...
static void RemoveIconFromWorkspace(char *iconName, char *targetDir) {
  ...
  for (i = 0; i < desktop_data->numIconsUsed; i++) {
    ...
    if (strcmp(fileName, iconName) == 0) {
      ...
      currentScreen = XScreenOfDisplay(XtDisplay(desktopWin->shell), screen);
      rootWindow = RootWindowOfScreen(currentScreen);

      if (DtWsmGetCurrentWorkspace(XtDisplay(desktopWin->shell), rootWindow,
                                   &pCurrent) == Success)
        workspace_name = XGetAtomName(XtDisplay(desktopWin->shell), pCurrent);
      else
...
static void ChangeWorkspaceIconLink(FileMgrData *fmd, char *iconName,
                                    char *targetDir, char *iconPdir) {
  DesktopRec *desktopWin;
  char *dirp = NULL;
  int i;
  char fileName[MAX_PATH];
  ...
  for (i = 0; i < desktop_data->numIconsUsed; i++) {
    ...
    if (dirp) {
      ...
      full_dir_name = ResolveLocalPathName(desktopWin->host, dirp, NULL,
                                           home_host_name, &tt_status);
      if (TT_OK == tt_status) {
      ...
      ((DirectorySet *)file_view_data->directory_set)->name = XtNewString(dirp);
...
static void FileOpPipeCB(XtPointer client_data, int *fd, XtInputId *id) {
  ...
  ActionAreaDefn replace_renameActions = {XtNumber(replace_rename_actionItems),
  ...
  static ActionAreaItem replace_merge_actionItems[] = {
      ...
      {"Cancel", 9, 28, replace_merge_cancel_callback,
       NULL}, /* changed below depending on mode */
      {"Help", 9, 29, HelpRequestCB, HELP_FILE_MANAGER_REP_MRG},
  };

  ActionAreaDefn replace_mergeActions = {XtNumber(replace_merge_actionItems),
                                         1, /* Cancel is default action */
                                         replace_merge_actionItems};

  static ActionAreaItem multicollide_actionItems[] = {
      {"Ok", 9, 27, NULL, NULL}, /* changed later based on Mode */
      {"Cancel", 9, 28, multicollide_cancel_callback,
       NULL}, /* changed below depending on mode */
      {"Help", 9, 29, HelpRequestCB, HELP_FILE_MANAGER_MULTI},
  };

  ActionAreaDefn multicollideActions = {XtNumber(multicollide_actionItems),
                                        1, /* Cancel is default action */
  ...
  switch (mode) {
  case MAKE_BUFFER:
    replace_rename_actionItems[0].callback = buffer_replace_rename_ok_callback;
    multicollide_actionItems[0].callback = buffer_multicollide_ok_callback;
    break;
  default:
    replace_rename_actionItems[0].callback = replace_rename_ok_callback;
    multicollide_actionItems[0].callback = multicollide_ok_callback;
  } /* endswitch */
  ...
  case PIPEMSG_FILEOP_ERROR:
    ...
    if (cb_data->desktop)
      FileOperationError(cb_data->file_view_data->widget, err_msg, err_arg);
    else {
      /* routine can be called with a NULL file_mgr_rec, use the
       * top level widget if this is the case.
       */
      if (cb_data->file_mgr_rec)
        FileOperationError(cb_data->file_mgr_rec->file_window, err_msg,
                           err_arg);
      else
  ...
  case PIPEMSG_CONFIRM:
    err_msg = PipeReadString(*fd);
    title =
        XtNewString(GETMESSAGE(9, 11, "File Manager - Move/Copy/Link Warning"));
    filop_confirm_fd = cb_data->pipe_m2s;
    _DtMessageDialog(toplevel, title, err_msg, NULL, TRUE, moveCopyLinkCancel,
                     moveCopyLinkOK, NULL, HelpRequestCB, False,
                     QUESTION_DIALOG);
    XtFree(err_msg);
    XtFree(title);
    break;
  ...
  case PIPEMSG_REPLACE_RENAME_SAME:
    ...
    if (cb_data->file_mgr_rec)
      create_replace_rename_dialog(cb_data->file_mgr_rec->shell, mode,
                                   directory, file, cb_data->pipe_m2s,
                                   replace_renameActions, True);
    else
      create_replace_rename_dialog(toplevel, mode, directory, file,
                                   cb_data->pipe_m2s, replace_renameActions,
                                   True);
    XtFree(directory);
    XtFree(file);
    break;
  ...
  case PIPEMSG_REPLACE_MERGE:
     ...
     */
    if (cb_data->file_mgr_rec)
      create_replace_merge_dialog(cb_data->file_mgr_rec->shell, mode, directory,
                                  file, cb_data->pipe_m2s,
                                  replace_mergeActions);
    else
      create_replace_merge_dialog(toplevel, mode, directory, file,
                                  cb_data->pipe_m2s, replace_mergeActions);
    XtFree(directory);
    XtFree(file);
    break;
  ...
  case PIPEMSG_MULTICOLLIDE_SAME:
    ...
    fileList = (String *)XtMalloc(
        nCollisions *
        sizeof(String)); /* de- allocated in dialog's callback functions */
    ...
    if (cb_data->file_mgr_rec)
      create_multicollide_dialog(cb_data->file_mgr_rec->shell, mode, nSelected,
                                 nCollisions, directory, fileList,
                                 cb_data->pipe_m2s, multicollideActions, True);
    else
      create_multicollide_dialog(toplevel, mode, nSelected, nCollisions,
                                 directory, fileList, cb_data->pipe_m2s,
  ...
  case PIPEMSG_MULTICOLLIDE:
    ...
    if (cb_data->file_mgr_rec)
      create_multicollide_dialog(cb_data->file_mgr_rec->shell, mode, nSelected,
                                 nCollisions, directory, fileList,
                                 cb_data->pipe_m2s, multicollideActions, False);
    else
      create_multicollide_dialog(toplevel, mode, nSelected, nCollisions,
                                 directory, fileList, cb_data->pipe_m2s,
  ...
  if (done) {
    ...
    if (!cb_data->finish_callback && cb_data->file_mgr_data) {
      ...
      /* Do this only if it is the current directory and Random placement
         is ON */
      if (cb_data->file_mgr_data->positionEnabled != RANDOM_OFF &&
          strcmp(cb_data->directory,
                 cb_data->file_mgr_data->current_directory) == 0) {
        ...
        RepositionIcons(cb_data->file_mgr_data, file_set, actual_count, G_dropx,
                        G_dropy, True);
    ...
    for (i = 0; i < cb_data->file_count; i++) {
      ...
      if (cb_data->mode == MOVE_FILE) {
        snprintf(fileName, sizeof(fileName), "%s/%s", tmpDir,
                 cb_data->updates[i].file);
        if (cb_data->updates[i].operationStatus == True) {
    ...
    if (cb_data->finish_callback)
      (*cb_data->finish_callback)(cb_data->callback_data, NULL, rc);
...
static Boolean _FileMoveCopy(XtPointer data, char *to_file, char *directory,
                             char *host, char **host_set, char **file_set,
                             BufferInfo *buffer_set, int file_count,
  ...
  if (cb_data->desktop) {
    ...
    if (data != NULL) {
      ...
      cb_data->file_mgr_rec = (FileMgrRec *)((FileMgrData *)data)->file_mgr_rec;
    } else {
      cb_data->file_mgr_data = NULL;
      cb_data->file_mgr_rec = NULL;
    }
  ...
  cb_data->updates = updates =
      (DirUpdate *)XtMalloc(file_count * sizeof(DirUpdate));
  ...
  if (pid == -1) {
    ...
    fprintf(stderr, "%s:  fork failed, ppid %d, pid %d: error %d=%s\n", pname,
            getppid(), getpid(), errno, strerror(errno));
    return False;
  }
  ...
  if (pid == 0) {
    ...
    if (desktopWindow != NULL)
      rc = FileMoveCopyProcessDesktop(pipe_s2m[1], pipe_m2s[0], directory, host,
  ...
  for (;;) {
    ...
    if ((int)select_timeout.tv_sec < 0) {
      ...
      XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_s2m[0],
                    (XtPointer)XtInputReadMask, FileOpPipeCB,
                    (XtPointer)cb_data);
      break;
    }
...
Boolean FileMoveCopy(FileMgrData *file_mgr_data, char *to_file, char *directory,
                     char *host, char **host_set, char **file_set,
                     int file_count, unsigned int modifiers,
                     void (*finish_callback)(XtPointer, char *, int),
                     XtPointer callback_data) {
...
Boolean FileMoveCopyDesktop(FileViewData *file_view_data, char *directory,
                            char **host_set, char **file_set, int file_count,
                            unsigned int modifiers, DesktopRec *desktopWindow,
                            void (*finish_callback)(XtPointer, char *, int),
                            XtPointer callback_data) {
  return _FileMoveCopy((XtPointer)file_view_data, NULL, directory,
                       home_host_name, host_set, file_set, NULL, file_count,
                       modifiers, desktopWindow, finish_callback,
                       callback_data);
}
...
static int ChangeIconNameProcess(int pipe_fd, char *host_name,
                                 char *directory_name, char *old_name,
                                 char *new_name) {
  ...
  full_name = ResolveLocalPathName(host_name, directory_name, new_name,
                                   home_host_name, &tt_status);
  if (TT_OK != tt_status) {
  ...
  modify_time = 0;
  dir_path = ResolveLocalPathName(host_name, directory_name, NULL,
                                  home_host_name, &tt_status);
  if (TT_OK != tt_status) {
  ...
  /* rename the file */
  old_full_name = ResolveLocalPathName(host_name, directory_name, old_name,
                                       home_host_name, &tt_status);
  if (TT_OK != tt_status) {
    DPRINTF(("ChangeIconNameProcess: sending exist error\n"));
    pipe_msg = PIPEMSG_FILEOP_ERROR;
  ...
  success = FileManip((Widget)(intptr_t)pipe_fd, MOVE_FILE, old_full_name,
                      full_name, TRUE, FileOpError, True, NOT_DESKTOP);
...
static void ChangeIconPipeCB(XtPointer client_data, int *fd, XtInputId *id) {
    ...
    if (rc == 0) {
      ...
      for (i = 0; i < desktop_data->numIconsUsed; i++) {
        ...
        if (strcmp(cb_data->host_name, desktopWindow->host) == 0 &&
            strcmp(cb_data->directory_name, desktopWindow->dir_linked_to) ==
                0 &&
...
static void _ChangeIconName(Widget w, Boolean desktop,
                            FileViewData *file_view_data,
                            DesktopRec *desktopWindow, char *host_name,
                            char *directory_name) {
  ...
  /* Ensure the new name has length less than or equal to the maximum
     ...
     If maxFileNameLength == -1 the file system is not supporting POSIX, use
     MAXNAMLEN
  */
  else if (maxFileNameLength < -1 ||
           (maxFileNameLength == -1 &&
            (length > MAXNAMLEN || length + dirNameLength > MAX_PATH)) ||
           (maxFileNameLength > 0 && length > maxFileNameLength)) {
  ...
  if (desktopWindow == NULL) {
    for (i = 0; i < desktop_data->numIconsUsed; i++) {
      if (strcmp(host_name, desktop_data->desktopWindows[i]->host) == 0 &&
  ...
  if (desktopWindow != NULL) {
    ...
    for (j = 0; j < desktop_data->numIconsUsed; j++) {
      if (strcmp(new_name, desktop_data->desktopWindows[j]
        ...
        title = XtNewString(GetSharedMessage(FILE_RENAME_ERROR_TITLE));
        if (desktop)
          tmpStr = GETMESSAGE(28, 9,
                              "An object with this name already exists on the "
                              "Workspace.\nPlease choose a different name.");
        else
          tmpStr = GETMESSAGE(
              9, 90,
              "Name Conflict.\nThis object is out on the Workspace back "
              "drop.\nAnother object on the back drop already has the name "
              "you\nare trying to enter.\nPlease choose a different name.");
  ...
  if (pid == 0) {
    ...
    rc = ChangeIconNameProcess(pipe_fd[1], host_name, directory_name, old_name,
                               new_name);
    close(pipe_fd[1]);
  ...
  XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_fd[0],
                (XtPointer)XtInputReadMask, ChangeIconPipeCB,
                (XtPointer)cb_data);
}

void ChangeIconName(Widget w, XtPointer client_data,
  ...
  FileMgrRec *file_mgr_rec = (FileMgrRec *)file_mgr_data->file_mgr_rec;
  DirectorySet *directory_set =
      (DirectorySet *)(file_mgr_data->renaming->directory_set);
      ...
      if (XtIsManaged(file_mgr_rec->horizontal_scroll_bar)) {
        ...
        */
        ...
        if ((x2 - x3) > 0) {
          ...
          if ((value < (max - size)) && ((x1 + x2) - value > 0) &&
            ...
            if ((value + (x2 - x3)) > (max - size))
              ...
              value += (x2 - x3);
            XmScrollBarSetValues(file_mgr_rec->horizontal_scroll_bar, value,
                                 size, increment, page, True);
          }
        }
  ...
  _ChangeIconName(w, False, file_mgr_data->renaming, NULL, file_mgr_data->host,
                  directory_set->name);
}

void ChangeIconNameDT(Widget w, XtPointer client_data, XtPointer call_data) {
...
void GetTarget(char *from_host, char *from_dir, char *new_name, char *to_host,
               char *to_dir, char *to_file) {
  char *ptr;
...
static int MakeFileProcess(int pipe_fd, char *to_host, char *to_dir,
                           char *to_file, unsigned char type) {
  char *to;
  struct stat stat_buf;
  char *dir_path;
  long modify_time;
  ...
  to = ResolveLocalPathName(to_host, to_dir, to_file, home_host_name,
                            &tt_status);
  if (to == NULL)
    rc = EINVAL;
  ...
  /* now create the file/dir */
    ...
    modify_time = 0;
    dir_path =
        ResolveLocalPathName(to_host, to_dir, NULL, home_host_name, &tt_status);
    if (stat(dir_path, &stat_buf) == 0)
      modify_time = stat_buf.st_mtime;
    XtFree(dir_path);
...
static void MakeFilePipeCB(XtPointer client_data, int *fd, XtInputId *id) {
  ...
  if (pipe_msg == PIPEMSG_FILE_MODIFIED) {
    ...
    DirectoryFileModified(cb_data->to_host, cb_data->to_dir, cb_data->to_file);
    return;
  }
...
void MakeFile(Widget w, char *host_name, char *directory_name, char *new_name,
              unsigned char type,
              void (*finish_callback)(XtPointer, char *, int),
              XtPointer callback_data) {
  ...
  XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_fd[0],
                (XtPointer)XtInputReadMask, MakeFilePipeCB, (XtPointer)cb_data);
}
...
Boolean MakeFilesFromBuffers(FileMgrData *file_mgr_data, char *directory,
                             char *host, char **file_set, char **host_set,
                             BufferInfo *buffer_set, int num_of_buffers,
                             void (*finish_callback)(XtPointer, char *, int),
                             XtPointer callback_data) {
  return _FileMoveCopy((XtPointer)file_mgr_data, NULL, directory, host,
                       host_set, file_set, buffer_set, num_of_buffers, 0, NULL,
                       finish_callback, callback_data);
}

Boolean MakeFilesFromBuffersDT(FileViewData *file_view_data, char *directory,
                               char **file_set, char **host_set,
                               BufferInfo *buffer_set, int num_of_buffers,
                               DesktopRec *desktopWindow,
                               void (*finish_callback)(XtPointer, char *, int),
                               XtPointer callback_data) {
  return _FileMoveCopy((XtPointer)file_view_data, NULL, directory,
                       home_host_name, host_set, file_set, buffer_set,
                       num_of_buffers, 0, desktopWindow, finish_callback,
                       callback_data);
}
...
static Boolean CreateFileFromBuffer(int pipe_s2m, char *directory,
  ...
  if ((fnew = open(fully_qualified_name, O_CREAT | O_WRONLY, mode)) < 0) {
    DPRINTF(
        ("CreateBufferFromFile: Could not create %s\n", fully_qualified_name));
    savedError = errno;
    rc = -1;
  } else {
...

{
  ...
  if (cb_data->mode == MOVE_FILE) {
    ...
    if (cb_data->callback_data == NULL) {
      ...
      err_msg = GETMESSAGE(11, 45,
                           "No Copy Operation performed on object %s.\nYou "
                           "must change either the Destination Folder\nor the "
                           "Name for Copy before a copy can be created");
    }
  } else {
    ...
    if (cb_data->callback_data == NULL) {
      ...
      err_msg = GETMESSAGE(11, 46,
                           "No Link Operation performed on object %s.\nYou "
                           "must change either the Destination Folder\nor the "
                           "Name for Copy before a link can be created");
    }
  }
  ...
  msgptr =
      XtCalloc(1, strlen(err_msg) + strlen(cb_data->updates[index].file) + 10);
...
static void DisplayErrorMessage(int pipe_s2m, char *errMsg, char *from,
  ...
  localstr =
      (char *)XtMalloc(strlen(errMsg) + strlen(from) + strlen(tdir) + 10);
  sprintf(localstr, errMsg, tdir, from);
  PipeWriteErrmsg(pipe_s2m, -1, localstr, NULL);
  XtFree(localstr);
}
static Boolean IsInParentDir(char *source_path, char *dest_path) {
  char filename[MAX_PATH];
  char *cptr;
