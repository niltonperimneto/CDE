...
static DialogResource resources[] = {
    {"matchFilter", XmRBoolean, sizeof(Boolean),
     XtOffset(FilterDataPtr, match_flag), (XtPointer)True,
     (WriteResourceProc)_DtBooleanToString},

    {"name", XmRString, sizeof(String), XtOffset(FilterDataPtr, filter),
     (XtPointer) "", (WriteResourceProc)_DtStringToString},

    {"showHidden", XmRBoolean, sizeof(Boolean),
     XtOffset(FilterDataPtr, show_hidden), (XtPointer)False,
     (WriteResourceProc)_DtBooleanToString},

    {"filetypesFilteredCount", XmRInt, sizeof(int),
     XtOffset(FilterDataPtr, filetypesFilteredCount), (XtPointer)3,
     (WriteResourceProc)_DtIntToString},

    {"filetypesFiltered", XmRString, sizeof(String),
     XtOffset(FilterDataPtr, filetypesFiltered), (XtPointer)DEFAULT_FILTER,
     (WriteResourceProc)_DtStringToString},
};
...
static void Create(Display *display, Widget parent, Widget *return_widget,
                   XtPointer *dialog);
static void InstallChange(FilterRec *filter_rec, XtCallbackProc callback,
                          XtPointer client_data);
static void InstallClose(FilterRec *filter_rec, XtCallbackProc callback,
                         XtPointer client_data);
static void Destroy(FilterRec *filter_rec);
static XtPointer GetValues(FilterRec *filter_rec);
static XtPointer GetDefaultValues(void);
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list);
static void SetValues(FilterRec *filter_rec, FilterData *filter_data);
static void WriteResourceValues(DialogData *values, int fd, char **name_list);
static void FreeValues(FilterData *filter_data);
static void DefaultCallback(Widget w, XtPointer client_data,
                            XtPointer call_data);
...
static void ApplyCallback(Widget w, XtPointer client_data, XtPointer call_data);
static Boolean GetFilterValues(FilterRec *filter_rec, FilterData *filter_data);
static void filterTypeChanged(Widget w, XtPointer client_data,
                              XtPointer call_data);
static int FileTypeAscending(const void *t1, const void *t2);
static void SelectAllFilter(Widget w, XEvent *event, String *params,
                            Cardinal *num_params);
static void DeselectAllFilter(Widget w, XEvent *event, String *params,
                              Cardinal *num_params);
...
static XtActionsRec FilterAction[] = {{"SelectAllFilter", SelectAllFilter},
...
static void Create(Display *display, Widget parent, Widget *return_widget,
  ...
  XtAddCallback(form, XmNhelpCallback, HelpRequestCB, HELP_FILTER_DIALOG_STR);

  XtAppAddActions(XtWidgetToApplicationContext(form), FilterAction,
                  XtNumber(FilterAction));
  resolvedFilterTranslationString =
      ResolveTranslationString(FilterTranslationString, (char *)filter_rec);
  filterTranslationTable =
      XtParseTranslationTable(resolvedFilterTranslationString);
  ...
  label_string =
      XmStringCreateLocalized((GETMESSAGE(13, 31, "Select Datatypes to be:")));
  n = 0;
  ...
  n++;
  XtSetArg(args[n], XmNsubMenuId, hiddenPulldown);
  n++;
  XtSetArg(args[n], XmNmenuHistory, filter_rec->hidArray[HIDDEN]);
  n++;
  hidden = XmCreateOptionMenu(form, "file_name_label", args, n);
  XtManageChild(hidden);
  XmStringFree(label_string);
  XtAddCallback(hidden, XmNhelpCallback, HelpRequestCB, HELP_FILTER_DIALOG_STR);
  ...
  n++;
  XtSetArg(args[n], XmNtopWidget, hidden);
  n++;
  ...
  n++;
  file_window = XmCreateDrawingArea(fileTypeScrollWin, "file_window", args, n);
  ...
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, fileTypeScrollWin);
  n++;
  XtSetArg(args[n], XmNtopOffset, 10);
  n++;
  ...
  n++;
  XtSetArg(args[n], XmNleftPosition, 15);
  n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
  n++;
  XtSetArg(args[n], XmNtopOffset, 5);
  ...
  label_string = XmStringCreateLocalized(
      ((char *)GETMESSAGE(13, 26, "Also hide (Optional): ")));
  n = 0;
  XtSetArg(args[n], XmNlabelString, label_string);
  n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
  ...
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, sel_form);
  n++;
  XtSetArg(args[n], XmNtopOffset, 18);
  n++;
  ...
  n++;
  XtSetArg(args[n], XmNleftWidget, labelPt2);
  n++;
  ...
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, fileNameFilter);
  n++;
  XtSetArg(args[n], XmNtopOffset, 10);
  n++;
  ...
  XtSetArg(args[n], XmNmarginHeight, 4);
  n++;
  XtSetArg(args[n], XmNmultiClick, XmMULTICLICK_DISCARD);
  n++;
  XtSetArg(args[n], XmNuserData, flag);
  n++;
  ok = XmCreatePushButtonGadget(form, "ok", args, n);
  XtManageChild(ok);
  XtAddCallback(ok, XmNhelpCallback, HelpRequestCB, HELP_FILTER_DIALOG_STR);
  ...
  XtManageChild(apply);
  XtAddCallback(apply, XmNhelpCallback, HelpRequestCB, HELP_FILTER_DIALOG_STR);

  XtSetArg(args[0], XmNdefaultButton, apply);
  XtSetValues(form, args, 1);

  label_string =
      XmStringCreateLocalized(((char *)GETMESSAGE(13, 11, "Defaults")));
  n = 0;
  XtSetArg(args[n], XmNlabelString, label_string);
  n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
  ...
  XtManageChild(reset);
  XtAddCallback(reset, XmNactivateCallback, DefaultCallback,
                (XtPointer)filter_rec);
  XtAddCallback(reset, XmNhelpCallback, HelpRequestCB, HELP_FILTER_DIALOG_STR);
  XmStringFree(label_string);
  ...
  XtAddCallback(close, XmNactivateCallback, CancelCallback,
                (XtPointer)filter_rec);
  XtManageChild(close);
  XtAddCallback(close, XmNhelpCallback, HelpRequestCB, HELP_FILTER_DIALOG_STR);
  ...
  XtAddCallback(help, XmNactivateCallback, HelpRequestCB,
                HELP_FILTER_DIALOG_STR);
  XtAddCallback(help, XmNhelpCallback, HelpRequestCB, HELP_FILTER_DIALOG_STR);
...
static void InstallChange(FilterRec *filter_rec, XtCallbackProc callback,
  ...
  XtAddCallback(filter_rec->apply, XmNactivateCallback, ApplyCallback,
                (XtPointer)apply_data);

  XtAddCallback(filter_rec->ok, XmNactivateCallback, OkCallback,
                (XtPointer)apply_data);
}
...
static void InstallClose(FilterRec *filter_rec, XtCallbackProc callback,
  ...
  delete_window_atom =
      XmInternAtom(XtDisplay(filter_rec->shell), "WM_DELETE_WINDOW", True);
...
static XtPointer GetValues(FilterRec *filter_rec) {
  ...
  if (filter_rec->cancel && filter_rec->previous_filter_data) {
...
static FilterData *CopyFilterData(FilterData *filter_data) {
  ...
  CopiedFilterData->filetypesFilteredCount =
      filter_data->filetypesFilteredCount;
  CopiedFilterData->filetypesFiltered =
      XtNewString(filter_data->filetypesFiltered);
  CopiedFilterData->count = filter_data->count;
  CopiedFilterData->user_data =
      (FTData **)XtMalloc(sizeof(FTData *) * filter_data->count);
  for (i = 0; i < filter_data->count; ++i) {
    ...
    CopiedFilterData->user_data[i]->index = filter_data->user_data[i]->index;
...
static void SetValues(FilterRec *filter_rec, FilterData *filter_data) {
  ...
  for (i = 0, x = 5, count = 0; i < filter_data->count; i++) {
    ...
    pixmapData = _DtRetrievePixmapData(user_data->filetype, NULL, NULL,
                                       filter_rec->file_window, LARGE);
    ...
    if (i - count < num_children) {
      ...
      if (mgr->composite.num_children == 0) {
        ...
        icon_widget =
            _DtDuplicateIcon((Widget)mgr, mgr->composite.children[0],
    ...
    _DtCheckAndFreePixmapData(user_data->filetype, filter_rec->file_window,
...
static void WriteResourceValues(DialogData *values, int fd, char **name_list) {
  ...
  _DtDialogPutResources(fd, name_list, FILE_FILTER, values->data, resources,
                        filterClass->resource_count);
}
...
static void ApplyCallback(Widget w, XtPointer client_data,
                          XtPointer call_data) {
  FilterRec *filter_rec;
  DialogData *dialog_data;
  FilterData *filter_data;
  ...
  for (i = 0, count = 0; i < num_children && i + count < new_filter_data->count;
       i++) {
    XtSetArg(args[0], XmNuserData, new_filter_data->user_data[i + count]);
    XtSetValues(mgr->composite.children[i], args, 1);
  }
  (*(((FilterApply *)client_data)->callback))(
      w, ((FilterApply *)client_data)->client_data, NULL);
}
...
static Boolean GetFilterValues(FilterRec *filter_rec, FilterData *filter_data) {
  ...
  if (dialog_data != NULL) {
    ...
    for (i = 0; i < filter_data->count; i++) {
      ...
      filter_data->user_data[i]->index = old_filter_data->user_data[i]->index;
      filter_data->user_data[i]->selected =
...
void ShowHiddenFiles(Widget w, XtPointer client_data, XtPointer callback)
...
int ReadInFiletypes(FilterData *filter_data) {
  ...
  while (file_types[i]) {
    ...
    if (count == count_size) {
      ...
      filter_data->user_data = (FTData **)XtRealloc(
          (char *)filter_data->user_data, sizeof(FTData *) * count_size);
    }
  ...
  qsort(filter_data->user_data, count, sizeof(FTData *), FileTypeAscending);
...
void UpdateFilterAfterDBReread(DialogData *dialog_data)

{
  ...
  if (filter_rec && result) {
    ...
    mgr = (XmManagerWidget)filter_rec->file_window;
    num_children = mgr->composite.num_children;
    for (i = 0, count = 0; i < num_children && i + count < filter_data->count;
         i++) {
...
static void filterTypeChanged(Widget w, XtPointer client_data,
  ...
  if (menuHistory == filter_rec->hidArray[HIDDEN]) {
    ...
    filter_data->match_flag = False;
    label_string = XmStringCreateLocalized(
        ((char *)GETMESSAGE(13, 29, "Also show (Optional): ")));
  }
...
static int FileTypeAscending(const void *t1, const void *t2) {
  const FTData **ft1 = (const FTData **)t1;
  const FTData **ft2 = (const FTData **)t2;
  return strcmp((*ft1)->filetype, (*ft2)->filetype);
}
