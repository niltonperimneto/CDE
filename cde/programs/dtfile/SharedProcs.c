...
static void SetBufferFileNames(char **file_set, DtDndBuffer *buffers,
                               int num_of_buffers);
static char *BuildBufferFileName(char *file_name, int postfix_index,
                                 void *buffer, int buffer_size);
static char *RetrieveAndUseNameTemplateInfo(void *buffer, int buffer_size,
                                            char *template_input);
static void RenameEntry(char **name, unsigned int addIndex);
static void RenameCollisions(char **list, int count);
...
Boolean _DtStringsAreEquivalent(char *in_str, char *test_str) {
#ifdef NLS16
  wchar_t c1;
  wchar_t c2;
  int c1size, c2size;
...
void _DtDuplicateDialogNameList(char **nameList, char **newNameList,
                                int newNameListSize, int *nameCount)
...
void _DtLoadSubdialogArray(char **nameList, int nameCount,
                           DialogData ***dialogArray, int dialogCount,
                           int dialogId, XrmDatabase dataBase, int firstId)
...
void _DtSaveSubdialogArray(char **nameList, int nameCount,
                           DialogData **dialogArray, int dialogCount, int fd,
                           int firstId)
...
void _DtDestroySubdialogArray(DialogData **dialogArray, int dialogCount)
...
void _DtDestroySubdialog(DialogData *dialogData)
...
void _DtHideOneSubdialog(DialogData *dialogData, DialogData ***dialogArray,
                         int *dialogCountPtr)

{
  ...
  if (*dialogCountPtr == 0) {
    ...
    *dialogArray = (DialogData **)XtRealloc(
        (char *)*dialogArray, sizeof(DialogData *) * (*dialogCountPtr));
  }
}

void _DtAddOneSubdialog(DialogData *dialogData, DialogData ***dialogArray,
                        int *dialogCountPtr)
...
void _DtMappedCB(Widget w, XtPointer client_data, XtPointer call_data) {
  if (!disableDialogAutoPlacement)
    _DtmapCB(w, client_data, call_data);

  XtRemoveCallback(w, XmNpopupCallback, (XtCallbackProc)_DtMappedCB,
                   client_data);
}
...
void _DtGenericMapWindow(Widget parent, XtPointer recordPtr) {
...
void _DtGenericDestroy(XtPointer recordPtr)
...
void _DtGenericUpdateWindowPosition(DialogData *dataPtr)

{
  ...
  if (genericData->displayed == True) {
    ...
    XTranslateCoordinates(XtDisplay(genericRecord->shell),
                          ...
                          RootWindowOfScreen(XtScreen(genericRecord->shell)), 0,
                          0, &t_x, &t_y, &junkWindow);
...
PixmapData *_DtRetrievePixmapData(char *dataType, char *fileName,
                                  char *directory, Widget shell, int size)

{
  ...
  pixmapData->hostPrefix =
      DtDtsDataTypeToAttributeValue(dataType, DtDTS_DA_DATA_HOST, NULL);
  ...
  */
  if (path[0] != 0x0) {
    pixmapData->instanceIconName =
        DtDtsDataTypeToAttributeValue(dataType, DtDTS_DA_INSTANCE_ICON, path);
  } else
    pixmapData->instanceIconName = NULL;

  if (pixmapData->instanceIconName == NULL) {
    pixmapData->iconName =
        DtDtsDataTypeToAttributeValue(dataType, DtDTS_DA_ICON, NULL);
  ...
  if (pixmapData->size == LARGE)
    pixmapData->iconFileName = _DtGetIconFileName(
        XtScreen(shell), pixmapData->instanceIconName, pixmapData->iconName,
        pixmapData->hostPrefix, DtMEDIUM);
  else
    pixmapData->iconFileName = _DtGetIconFileName(
        XtScreen(shell), pixmapData->instanceIconName, pixmapData->iconName,
        pixmapData->hostPrefix, DtTINY);
...
void _DtCheckAndFreePixmapData(char *dataType, Widget shell,
                               DtIconGadget iconGadget, PixmapData *pixmapData)
  ...
  */
  if (pixmapData->instanceIconName) {
    ...
    if (!pixmapData->iconFileName &&
        iconGadget->icon.pixmap == 0) { /* Try to get pixmap name
                                         */
      char *tmp, *ptr;
      ...
      if (pixmapData->size == LARGE)
        pixmapData->iconFileName =
            _DtGetIconFileName(XtScreen(shell), pixmapData->instanceIconName,
                               NULL, pixmapData->hostPrefix, DtMEDIUM);
      else
        pixmapData->iconFileName =
            _DtGetIconFileName(XtScreen(shell), pixmapData->instanceIconName,
                               NULL, pixmapData->hostPrefix, DtTINY);
    ...
    if (iconGadget->icon.pixmap == 0 || iconGadget->icon.pixmap_width == 0 ||
...
Boolean _DtCheckForDataTypeProperty(char *dataType, char *property)

{
  ...
  properties =
      DtDtsDataTypeToAttributeValue(dataType, DtDTS_DA_PROPERTIES, NULL);
...
char **_DtCompileActionVector(char *dataType)

{
  char *actions;
  char **vector = NULL;

  actions = DtDtsDataTypeToAttributeValue(dataType, DtDTS_DA_ACTION_LIST, NULL);
  if (actions)
...
char *_DtRetrieveDefaultAction(char *dataType)
...
char *_DtBuildFMTitle(FileMgrData *file_mgr_data)

{
  ...
  if (file_mgr_data->title) {
    if (file_mgr_data->toolbox &&
      ...
      title = (char *)XtMalloc(strlen(file_mgr_data->title) + strlen(ptr) +
    ...
    if (strcmp(file_mgr_data->current_directory, "/") == 0 && !fileLabel) {
      title =
          (char *)XtMalloc(strlen((GETMESSAGE(12, 7, "File Manager"))) +
...
char *_DtGetSelectedFilePath(FileViewData *selected_file)
...
void _DtBuildActionArgsWithSelectedFiles(FileViewData **selection_list,
                                         int selected_count,
...

{
  ...
  *action_args =
      (DtActionArg *)XtCalloc(1, sizeof(DtActionArg) * selected_count);
...
void _DtBuildActionArgsWithDTSelectedFiles(DesktopRec **selection_list,
                                           int selected_count,
                                           DtActionArg **action_args,
                                           int *arg_count)
...
void _DtBuildActionArgsWithDroppedFiles(
    FileViewData *dropped_on_obj, DtDndDropCallbackStruct *drop_parameters,
    DtActionArg **action_args, int *arg_count)
...
void _DtBuildActionArgsWithDroppedBuffers(
    FileViewData *dropped_on_obj, DtDndDropCallbackStruct *drop_parameters,
    DtActionArg **action_args, int *arg_count)
...
void _DtFreeActionArgs(DtActionArg *action_args, int arg_count)
...
void _DtSetDroppedBufferInfo(char **file_set, BufferInfo *buffer_set,
                             char **host_set,
...
static void SetBufferFileNames(char **file_set, DtDndBuffer *buffers,
  ...
  for (i = 0; i < num_of_buffers; i++) {
    if (buffers[i].name == NULL) {
      ...
      file_set[i] = BuildBufferFileName(DEFAULT_BUFFER_FILENAME, -1,
                                        buffers[i].bp, buffers[i].size);
    } else {
...
static char *BuildBufferFileName(char *file_name, int postfix_index,
  ...
  new_file_name =
      RetrieveAndUseNameTemplateInfo(buffer, buffer_size, new_file_name);
...

{
  ...
  name_template = DtDtsBufferToAttributeValue(buffer, buffer_size,
                                              DtDTS_DA_NAME_TEMPLATE, NULL);
  if (name_template)
    buffer_name =
        (char *)XtMalloc(strlen(name_template) + strlen(template_input) + 1);
  if (buffer_name) {
...
#define MAX_INT_SIZE 15
  ...
  if (*name == 0x0)
    ...
    newName =
        (char *)XtCalloc(1, strlen(DEFAULT_BUFFER_FILENAME) + MAX_INT_SIZE);
...
void _DtFreeDroppedBufferInfo(char **file_set, BufferInfo *buffer_set,
                              char **host_set, int num_of_buffers) {
  int i;
...
Boolean _DtIsBufferExecutable(void *buffer, int buffer_size)

{
  ...
  exe_attribute =
      DtDtsBufferToAttributeValue(buffer, buffer_size, "IS_EXECUTABLE", NULL);
  if (exe_attribute) {
...
void _DtSetDroppedFileInfo(DtDndDropCallbackStruct *drop_parameters,
...
void _DtCopyDroppedFileInfo(int num_files, char **orig_file_set,
                            char **orig_host_set, char ***new_file_set,
                            char ***new_host_set) {
  int i;
...
void _DtFreeDroppedFileInfo(int num_files, char **file_set, char **host_set) {
  int i;
...
void _DtPathFromInput(char *input_string, char *current_dir, char **host,
                      char **rel_path)

{
  ...
  *host = XtNewString(home_host_name);
  ...
  if (path) {
    ...
    true_path =
        ResolveLocalPathName(*host, path, NULL, home_host_name, &tt_status);
    XtFree(path);
...
char *_DtChangeTildeToHome(char *input_string) {
  char *path;
  char *full_path;
    ...
    if (path) {
      ...
      *path = '/';
      ...
      if (strcmp(pwInfo->pw_dir, "/") == 0) {
        ...
        full_path = (char *)XtMalloc(strlen(pwInfo->pw_dir) + strlen(path) + 1);
...
Boolean _DtSpacesInFileNames(char **fileNames, int fileCount) {
  int i;
...
char *_DtResolveAppManPath(char *path, char *restricted_dir) {
