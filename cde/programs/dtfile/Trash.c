...
static String CreateTrashFilename(String baseName, Boolean uniqueTest);
static void MessageToFileList(Tt_message msg, char ***file_list,
                              int *file_count);
#ifdef SUN_PERF
static void MessagesToFileList(Tt_message *msg_list, int msg_cnt,
                               char ***file_list, int *file_count);
#endif /* SUN_PERF */
static int WriteEntry(FILE *id, String external, String internal);
static Boolean MatchesSacredDirectory(String file);
static void VerifyCleanup(Widget mbox, Boolean completeDelete);
static void VerifyCancel(Widget w, XtPointer client_data, XtPointer call_data);
static void VerifyOk(Widget w, XtPointer client_data, XtPointer call_data);
static Boolean WriteTrashEntries(void);
static void Select_All(Widget w, XtPointer client_data, XtPointer call_data);
static void Unselect_All(Widget w, XtPointer client_data, XtPointer call_data);
void CloseTrash(Widget w, XtPointer client_data, XtPointer call_data);
static String GetBasePath(String fullPath);
static Boolean ReadTrashList(void);
static void RemoveOkCB(Widget w, XtPointer client_data, XtPointer call_data);
static void RemoveCancelCB(Widget w, XtPointer client_data,
                           XtPointer call_data);
static void ConfirmOk(Widget w, XtPointer client_data, XtPointer call_data);
static void ConfirmCancel(Widget w, XtPointer client_data, XtPointer call_data);
static void Noop(Widget w, XtPointer clientData, XtPointer callData);
static void AddToDeleteList(DeleteList *deleteList, int i, char *filename);
static void MoveToTrash(char **file_list, int file_count,
                        Boolean do_verify_checks, Tt_message msg);
static void RestoreFromTrash(char **file_list, int file_count,
                             char *target_host, char *target_dir,
                             Tt_message msg, Boolean CheckedAlready);
static void EmptyTrash(DeleteList *del_list, int del_count, int removeType,
                       Tt_message msg);
static int CheckDeletePermissionRecur(char *dir);
static void RestoreVerifyOk(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void RestoreVerifyCancel(Widget w, XtPointer client_data,
                                XtPointer call_data);
static int RestoreObject(Widget w, int mode, char *source, char *target,
                         Boolean isContainer,
...
Boolean TrashIsInitialized(void) { return (TrashInitialized); }
...
Boolean InitializeTrash(Boolean enableVerifyPrompt) {
  ...
  if (stat(trash_dir, &statInfo) < 0)
    mkdir(trash_dir, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP |
  ...
  while (ptr = DtStrchr(ptr, '/')) {
    ...
    *ptr = '\0';
    if (sacred_dir_count >= sacred_dir_list_size) {
      /* Grow the list */
      sacred_dir_list_size += 5;
      sacred_dir_list = (char **)XtRealloc(
          (char *)sacred_dir_list, sizeof(char *) * sacred_dir_list_size);
    }
    sacred_dir_list[sacred_dir_count++] = XtNewString(TrashInfoFileName);
    *ptr = '/';
    ptr++;
...
void TrashCreateDialog(Display *display) {
  char *tmpStr;
  char *title;
  ...
  _DtBuildDialog(NULL, NULL, NULL, dialog_data, NULL, NULL, NULL, NULL, NULL,
                 False, special_view, title, NULL);
  TrashView = False;
  ...
  if (trashInfoFileId == 0) {
    ...
    msg = XtMalloc(
        strlen(GETMESSAGE(
            ...
            "trash operations will not be performed.\n   The most common "
      ...
      */
      {
        ...
        */
        ...
        internal = (char *)XtMalloc(strlen(users_home_dir) + strlen(TRASH_DIR) +
                                    strlen(intName) + 2);
        ...
        if (lstat(internal, &statbuf) < 0) {
          ...
          if (numTrashItems >= trashListSize) {
            trashListSize += 10;
            trashCan = (TrashEntry *)XtRealloc(
                (char *)trashCan, sizeof(TrashEntry) * trashListSize);
          }
...
static Boolean WriteTrashEntries(void) {
  /* static String path = NULL; - Unused */
  FILE *newFile;
  int i;
...
static int WriteEntry(FILE *id, String external, String internal) {
  return (fprintf(id, "%ld %ld %s %s\n", (long)strlen(external),
                  (long)strlen(internal), external, internal));
}
...
Widget CreateTrashMenu(Widget mainw, FileMgrRec *file_mgr_rec) {
  int j, i;
  Widget menu_bar;
  Arg args[2];
  ...
  menu_bar = _DtCreateMenuSystem(mainw, "menu_bar_trash", TrashHelpRequestCB,
                                 HELP_TRASH_DIALOG_STR, True, trashMenu,
                                 trashMenuItemCount, NULL, Noop, Noop);
...
void TrashDisplayHandler(Tt_message msg) {
  Atom current_ws;
  Window root;
  ...
  if (!TrashInitialized) {
    ...
    tmpStr = GETMESSAGE(27, 90,
                        "The 'Trash Can' cannot be initialized.\nPlease "
  ...
  if (trashFileMgrData->file_mgr_rec == 0) {
    ...
    if (trash_icon == XmUNSPECIFIED_PIXMAP) {
      ...
      pixmap = XmGetPixmap(XtScreen(file_mgr_rec->main), trashIcon, foreground,
                           background);
    ...
    if (DtWsmGetCurrentWorkspace(XtDisplay(trashShell), root, &current_ws) ==
        Success)
      DtWsmSetWorkspacesOccupied(XtDisplay(trashShell), XtWindow(trashShell),
                                 &current_ws, 1);
...
static void ConfirmEmptyCancel(Widget w, XtPointer client_data,
                               XtPointer call_data) {
...
static void ConfirmEmptyOk(Widget w, XtPointer client_data,
                           XtPointer call_data) {
  /* destroy dialog */
  XtUnmanageChild((Widget)client_data);
  XmUpdateDisplay((Widget)client_data);
...
void TrashEmptyHandler(Tt_message msg) {
  if (msg != 0) {
    tt_message_reply(msg);
    tttk_message_destroy(msg);
  }
...
void SensitizeTrashBtns(void) {
#ifdef DEFER_TRASH_CREATION
  if (trashFileMgrData && removeBtn && restoreBtn)
#endif
  {
...
void TrashRemoveHandler(Tt_message msg) {
  /* char *ptr; - Unused */
  char *str;
  int number;
  char **file_list;
  ...
  global_msg_list =
      (Tt_message *)XtRealloc((char *)global_msg_list, msglistsize);
  memcpy((char *)&(global_msg_list[global_msg_cnt - 1]), (char *)&msg,
         sizeof(Tt_message));
  ...
  if (global_msg_cnt > 0) {
    MessagesToFileList(global_msg_list, global_msg_cnt, &file_list,
                       &file_count);
#endif /* SUN_PERF */
  ...
  if (number == 0) {
    ...
    if (number == 1) {
      if (widget_dragged)
        template = (GETMESSAGE(
            27, 85,
            "The Workspace object you are deleting is a linked copy of\n   an "
            "object inside the File Manager at\n   %s\n   If you click OK, "
            "both the linked copy and the original object\n   will be "
            "deleted.\n   If you just want to delete the Workspace object,\n   "
            "   1) click Cancel on this dialog\n      2) then select 'Remove "
            "From Workspace'\n         from the Workspace object's popup "
            "menu."));
      else
        template = (GETMESSAGE(
            27, 102,
            "A linked copy of the following object is on\nthe Workspace "
            "backdrop:\n%s\nMoving the object to the trash will remove it "
      ...
      if (widget_dragged)
        template = (GETMESSAGE(
            27, 86,
            "The Workspace objects you are deleting are linked copies of\n   "
            "objects inside the File Manager at\n%s\n   If you click OK, both "
            "the linked copies and the original objects\n   will be deleted.\n "
            "  If you just want to delete the Workspace objects,\n      1) "
            "click Cancel on this dialog\n      2) then select 'Remove From "
            "Workspace'\n         from the Workspace objects' popup menu."));
      else
        template = (GETMESSAGE(
            27, 103,
            "A linked copy of the following objects are also on\nthe Workspace "
            "backdrop:\n%s\nMoving the objects to the trash will remove them "
            "from\nboth the File Manager and Workspace backdrop."));
      message = (char *)XtMalloc(strlen(template) + strlen(str) + 1);
      sprintf(message, template, str);
    }
...
#ifndef SUN_PERF
    global = msg;
    _DtMessageDialog(toplevel, title, message, NULL, TRUE, RemoveCancelCB,
                     RemoveOkCB, NULL, HelpRequestCB, False, QUESTION_DIALOG);
#else
      {
        Widget dlog;
        Arg args[2];
        ...
        msg_cache->msg_list =
            (Tt_message *)XtMalloc(sizeof(Tt_message) * msg_cache->msg_cnt);
        ...
        dlog = (Widget)_DtMessageDialog(toplevel, title, message, NULL, TRUE,
                                        RemoveCancelCB, RemoveOkCB, NULL,
...
#else
    for (i = 0; i < current_msg_cache->msg_cnt; i++)
      tttk_message_fail(current_msg_cache->msg_list[i], TT_DESKTOP_ECANCELED, 0,
                        1);
    XtFree((char *)current_msg_cache->msg_list);
    XtFree((char *)current_msg_cache);
#endif /* SUN_PERF */
...
void TrashRemoveNoConfirmHandler(Tt_message msg) {
  char **file_list;
  int file_count;
  ...
  */
  ...
  if (uniqueTest && (lstat(trashName, &statInfo) == 0)) {
    ...
    for (i = 1; True; i++) {
      ...
      if (extension)
        sprintf(trashName, "%s%s/%s_%d.%s", users_home_dir, TRASH_DIR, baseName,
                i, extension);
      else
        sprintf(trashName, "%s%s/%s_%d", users_home_dir, TRASH_DIR, baseName,
                i);
 ...
 ************************************************************************/
static XtPointer trash_popup_client_data;
static void ConfirmCancel(Widget w, XtPointer client_data,
                          XtPointer call_data) {
  XtUnmanageChild((Widget)client_data);
  XmUpdateDisplay((Widget)client_data);
  XtDestroyWidget((Widget)client_data);
...
void ConfirmRemove(Widget w, XtPointer client_data, XtPointer call_data) {
  char *tmpStr, *title, *msg;
...
void Remove(Widget w, XtPointer client_data, XtPointer call_data) {
  FileViewData *file_view_data;
  Boolean match = False;
...
void TrashRestoreHandler(Tt_message msg) {
  char **file_list;
  int file_count;
...
void Restore(Widget w, XtPointer client_data, XtPointer call_data) {
  FileViewData *file_view_data;
  Boolean match = False;
  char **file_list;
  int file_count;
...
void DropOnTrashCan(int file_count, char **host_set, char **file_set,
  ...
  _DtBuildActionArgsWithDroppedFiles(NULL, drop_parameters, &action_args,
                                     &arg_count);

  DtActionInvoke(file_mgr_rec->shell, TRASH_ACTION, action_args, arg_count,
                 NULL, NULL, trashFileMgrData->current_directory, True, NULL,
                 NULL);

  _DtFreeActionArgs(action_args, arg_count);

  RepositionIcons(trashFileMgrData, file_set, file_count, drop_x, drop_y, True);
}
...
void MoveOutOfTrashCan(FileMgrData *file_mgr_data, FileMgrRec *file_mgr_rec,
                       Window w, int file_count, char **host_set,
                       char **file_set, Position drop_x, Position drop_y) {
  int i;
  char *target_host;
  char *target_dir;
  char **file_list;
...
Boolean FileFromTrash(char *filename) {
  Boolean IsTrash = False;

  if (strncmp(users_home_dir, filename, strlen(users_home_dir)) == 0) {
    if (strncmp((filename + strlen(users_home_dir)), TRASH_DIR,
                strlen(TRASH_DIR)) == 0)
      IsTrash = True;
  }
...
static void UpdateDirectoryOf(char *path) {
  char *ptr;
  char host[256];
...
int EraseObject(char *file_name) {
...
static void MoveToTrashProcess(int pipe_fd, char **file_list, int file_count,
...
static void MoveToTrashPipeCB(XtPointer client_data, int *fd, XtInputId *id) {
  ...
  char *dir_error = NULL, *file_error = NULL, *no_file_error = NULL,
       *sacred_error = NULL;
  ...
  default:
    fprintf(stderr, "Internal error in MoveToTrashPipeCB: bad pipe_msg %d\n",
            pipe_msg);
  ...
  for (i = 0; i < cb_data->file_count; i++) {
    if (cb_data->rc[i] == BAD_TRASH_DIRECTORY ||
        cb_data->rc[i] == BAD_TRASH_FILE) {
      if (badCount < 8) {
        char *tmpmsg;
        /* Boolean errflg = False; - Unused */
        if (cb_data->rc[i] == BAD_TRASH_DIRECTORY) {
          tmpmsg = GETMESSAGE(
              27, 113,
          ...
          if (dir_error) {
            dir_error = (char *)XtRealloc(
                dir_error,
                strlen(dir_error) + strlen(cb_data->file_list[i]) + 5);
            strcat(dir_error, cb_data->file_list[i]);
            strcat(dir_error, "\n");
          } else {
            dir_error =
                XtMalloc(strlen(tmpmsg) + strlen(cb_data->file_list[i]) + 5);
            sprintf(dir_error, "%s%s\n", tmpmsg, cb_data->file_list[i]);
          }
        } else {
          tmpmsg = GETMESSAGE(27, 114,
                              "The following file(s) cannot be deleted.  The "
                              "most likely cause \nis that you do not have "
                              "permissions to delete the file(s).\n\n");
          if (file_error) {
            file_error = (char *)XtRealloc(
                file_error,
                strlen(file_error) + strlen(cb_data->file_list[i]) + 5);
      ...
      if (badCount < 8) {
        char *tmpmsg = GETMESSAGE(
            27, 115,
            "The following object(s) cannot be deleted.\nThe desktop cannot "
            "function properly without these object(s).\n\n");
        if (sacred_error) {
          sacred_error = (char *)XtRealloc(
              sacred_error,
              strlen(sacred_error) + strlen(cb_data->file_list[i]) + 5);
      ...
      tmpmsg = GETMESSAGE(
          ...
          "is that these object(s)\nhave already been deleted.\n\n");

      if (badCount < 8) {
        if (no_file_error) {
          no_file_error = (char *)XtRealloc(
              no_file_error,
              strlen(no_file_error) + strlen(cb_data->file_list[i]) + 5);
      ...
      badCount++;
    } else if (cb_data->rc[i] == VERIFY_FILE || cb_data->rc[i] == VERIFY_DIR) {
      if (verifyCount < 6)
  ...
  if (fileCount > 0) {
    ...
    if (trashInfoFileId != NULL) {
      for (i = fileCount; i > 0; i--) {
        if (WriteEntry(trashInfoFileId, trashCan[numTrashItems - i].external,
  ...
  if (buf) {
    ...
    if (badCount > 8) {
      ...
      AddString(&buf, &bufsize, extraFiles,
                GETMESSAGE(27, 97,
                           "The following objects could not be placed in the "
                           "trash can:     \n"));
    }
  ...
  if (verifybuf) {
    ...
    if (vfiles && vdirs)
      header =
          GETMESSAGE(27, 6,
                     "   Each of the following is either a non-empty folder,   "
                     " \n   or a file for which you do not have write "
                     "permission.    \n   Do you want to proceed?\n");
    else if (vfiles)
      header = GETMESSAGE(27, 104,
                          "You do not have write permission for the following "
                          "file(s):\nDo you want to proceed?\n");
    else
      header = GETMESSAGE(
          27, 100,
          "The following folder(s) are not empty.\nDo you want to proceed?\n");
...
static void MoveToTrash(char **file_list, int file_count,
  ...
  if (global_msg_cnt > 0) {
    ...
    cb_data->msg_list =
        (Tt_message *)XtMalloc(sizeof(Tt_message) * cb_data->msg_cnt);
    memcpy((char *)cb_data->msg_list, (char *)global_msg_list,
           sizeof(Tt_message) * cb_data->msg_cnt);
  }
#endif /* SUN_PERF */

  /* create a pipe */
  if (-1 == pipe(pipe_fd)) {
    fprintf(stderr, "%s:  pipe failed error %d=%s\n", pname, errno,
            strerror(errno));
    return;
  }
  ...
  XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_fd[0],
                (XtPointer)XtInputReadMask, MoveToTrashPipeCB,
                (XtPointer)cb_data);
}
...
static void RestoreProcess(int pipe_fd, char **file_list, int file_count,
                           char *target_host, char *target_dir, int *rc,
                           Boolean CheckedAlready) {
  int i, j, rv;
  char *full_dirname;
  char *from, *to;
  ...
  short pipe_msg;
  int status;
  /* char **RestoreList = NULL; - Unused */

  /* get full path name of target directory */
  if (target_dir) {
    Tt_status tt_status;
    full_dirname = ResolveLocalPathName(target_host, target_dir, NULL,
                                        home_host_name, &tt_status);
    if (TT_OK != tt_status) {
  ...
  for (i = 0; i < file_count; i++) {
    ...
    if (to != NULL) {
      ...
      status =
          RestoreObject((Widget)(intptr_t)pipe_fd, MOVE_FILE, from, to, TRUE,
                        (void (*)(Widget, XtPointer, XtPointer))FileOpError,
                        False, NOT_DESKTOP, CheckedAlready);
...
static void RestorePipeCB(XtPointer client_data, int *fd, XtInputId *id) {
  ...
  default:
    fprintf(stderr, "Internal error in RestorePipeCB: bad pipe_msg %d\n",
            pipe_msg);
  }
  ...
  for (i = 0; i < cb_data->file_count; i++) {
    ...
    if (cb_data->rc[i] == SKIP_FILE) {
      ...
      FromRestoreList = (char **)XtRealloc((char *)FromRestoreList,
                                           sizeof(char *) * RestoreIndex);
    ...
    /* Check the return code from the restore */
      ...
      char *restore_header,
          *tmpStr = GETMESSAGE(
              27, 101, "The following object(s) could not be put back:\n");
...
static void RestoreFromTrash(char **file_list, int file_count,
  ...
  if (pid == 0) {
    ...
    RestoreProcess(pipe_fd[1], file_list, file_count, target_host, target_dir,
                   cb_data->rc, CheckedAlready);
    close(pipe_fd[1]);
  ...
  XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_fd[0],
                (XtPointer)XtInputReadMask, RestorePipeCB, (XtPointer)cb_data);
}
...
static void EmptyTrashProcess(int pipe_fd, DeleteList *del_list, int del_count,
                              int *rc) {
  int i, rv;
  short pipe_msg;
...
static void EmptyTrashPipeCB(XtPointer client_data, int *fd, XtInputId *id) {
  ...
  default:
    fprintf(stderr, "Internal error in EmptyTrashPipeCB: bad pipe_msg %d\n",
            pipe_msg);
  }
  ...
  if (cb_data->removeType == TRASH_FILE) {
    for (i = 0; i < cb_data->del_count; i++) {
      ...
      if (cb_data->rc[i] == 0) {
        ...
        if (itemCount < 8) {
          char *msg =
              XtNewString(GETMESSAGE(27, 96,
                                     "The following objects could not be "
    ...
    if (cb_data->rc[0] == 0) {
      ...
      title = XtNewString(tmpStr);
      tmpStr = GETMESSAGE(27, 82,
                          "   The following file could not be removed from the "
                          "file system:  \n");
...
static void EmptyTrash(DeleteList *del_list, int del_count, int removeType,
  ...
  XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_fd[0],
                (XtPointer)XtInputReadMask, EmptyTrashPipeCB,
                (XtPointer)cb_data);
}

int CheckDeletePermission(char *parentdir, char *destinationPath) {
  ...
  if (getuid() == 0) {
...
#endif
    {
      ...
      /* Create template: parentdir/dtfileXXXXXX */
      if (snprintf(tmp_template, sizeof(tmp_template), "%s/dtfileXXXXXX",
                   parentdir) >= sizeof(tmp_template))
        return -1;

      if ((fd = mkstemp(tmp_template)) < 0) /* Create a temporary file */
      {
        return -1;
      }
      close(fd);
      if (unlink(tmp_template) < 0) /* Delete the created file */
      {
        return -1;
      }
    }
...
static int CheckDeletePermissionRecur(char *destinationPath) {
  struct stat statbuf;
  DIR *dirp;
  struct dirent *dp;
  Boolean first_file;
...
static void CreateRestoreDialog(char *source, char *target) {
  ...
  char *header = GETMESSAGE(
      27, 110,
      "There is already an object named\n\n%s\n\nSelect Ok if you want to "
      "replace the object\nwith the object from the Trash.\n\nSelect Cancel if "
      "you want to keep the object.\nIf you then rename the object, you will "
  ...
  dw = (Widget)_DtMessageDialog(toplevel, title, tmpbuf, NULL, True,
                                RestoreVerifyCancel, RestoreVerifyOk, NULL,
                                HelpRequestCB, False, WARNING_DIALOG);

  XtSetArg(args[n], XmNuserData, dirs);
  n++;
...
static void RestoreVerifyOk(Widget w, XtPointer client_data,
  ...
  if (DirectoryBusy(dirs[1])) {
    ...
    int status; /* j, k - Unused */
    struct stat s1;
    /* char *realTarget, *tptr; - Unused */
    /* Boolean moveop; - Unused */
    char **FileList;
