...
extern int ReadInFiletypes(FilterData *filter_data);
...
static XmFontListEntry FontListDefaultEntry(XmFontList font_list);
static void Create(Display *display, Widget parent, Widget *return_widget,
                   XtPointer *dialog);
static XtPointer GetDefaultValues(void);
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list);
static void SetValues(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data);
static void WriteResourceValues(DialogData *values, int fd, char **name_list);
static void FreeValues(FileMgrData *file_mgr_data);
static void Destroy(XtPointer recordPtr);
static void GetSessionDir(FileMgrData *file_mgr_data);
static void BranchListToString(int fd, void *value, char *out_buf);
static void SelectionListToString(int fd, void *value, char *out_buf);
static Boolean StringToBranchList(Display *display, XrmValue *args,
                                  Cardinal num_args, XrmValue *from_val,
                                  XrmValue *to_val, XtPointer *converter_data);
static void StringToSelectionList(XrmValue *args, Cardinal num_args,
                                  XrmValue *from_val, XrmValue *to_val);
static void GetFileData(FileMgrData *file_mgr_data, Boolean valid,
                        char **branch_list);
static FileViewData *GetTopInfo(FileMgrData *file_mgr_data, char *host_name,
                                char *directory_name, char **branch_list);
static void CountDirectories(FileViewData *ip, int *np);
static DirectorySet *NewDirectorySet(char *name, FileViewData *ip,
                                     FileMgrData *file_mgr_data);
static void MakeDirectorySets(FileMgrData *file_mgr_data, FileViewData *ip,
                              DirectorySet **directory_set, int *index);
static void ReadTreeDirectory(Widget w, char *host_name, char *directory_name,
                              FileMgrData *file_mgr_data, char **branch_list,
                              DirectorySet ***directory_set,
                              int *directory_count);
static Bool IsShown(FileMgrData *fmd, FileViewData *ip);
static void SetDisplayedRecur(FileMgrData *fmd, FileViewData *dp, int level);
static void SelectVisible(FileMgrData *file_mgr_data);
static void FreeDirectorySet(DirectorySet **directory_set, int directory_count);
static void FreeDirectoryData(FileMgrData *file_mgr_data);
static void SystemClose(Widget w, XtPointer data);
static void SetIconAttributes(FileMgrRec *file_mgr_rec,
                              FileMgrData *file_mgr_data, char *directory_name);
static void SetFocus(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data);
static void MoveOkCB(Widget w, XtPointer client_data, XtPointer call_data);
static void MoveCancelCB(Widget w, XtPointer client_data, XtPointer call_data);
static void CreateFmPopup(Widget w);
static void DoTheMove(int type);
static void FMInput(Widget wid, XEvent *event, String *params,
                    Cardinal *num_params);
static void ActivateClist(Widget wid, XEvent *event, String *params,
                          Cardinal *num_params);
...
/* Action for osfMenu */
static XtActionsRec FMAction[] = {{"FMInput", FMInput},
...
static DialogResource resources[] = {
    {"show_type", SHOW_TYPE, sizeof(unsigned char),
     XtOffset(FileMgrDataPtr, show_type), (XtPointer)SINGLE_DIRECTORY,
     ShowTypeToString},

    {"tree_files", TREE_FILES, sizeof(unsigned char),
     XtOffset(FileMgrDataPtr, tree_files), (XtPointer)TREE_FILES_NEVER,
     TreeFilesToString},

    {"view_single", VIEW, sizeof(unsigned char),
     XtOffset(FileMgrDataPtr, view_single), (XtPointer)BY_NAME_AND_ICON,
     ViewToString},

    {"view_tree", VIEW, sizeof(unsigned char),
     XtOffset(FileMgrDataPtr, view_tree), (XtPointer)BY_NAME_AND_SMALL_ICON,
     ViewToString},

    {"order", ORDER, sizeof(unsigned char), XtOffset(FileMgrDataPtr, order),
     (XtPointer)ORDER_BY_ALPHABETICAL, OrderToString},

    {"direction", DIRECTION_RESRC, sizeof(unsigned char),
     XtOffset(FileMgrDataPtr, direction), (XtPointer)DIRECTION_ASCENDING,
     DirectionToString},

    {"positionEnabled", RANDOM, sizeof(unsigned char),
     XtOffset(FileMgrDataPtr, positionEnabled), (XtPointer)RANDOM_OFF,
     RandomToString},

    {"host", XmRString, sizeof(String), XtOffset(FileMgrDataPtr, host),
     (XtPointer)NULL, _DtStringToString},

    {"current_directory", XmRString, sizeof(String),
     XtOffset(FileMgrDataPtr, current_directory), (XtPointer) "~",
     _DtStringToString},

    {"branch_list", BRANCH_LIST, sizeof(XtPointer),
     XtOffset(FileMgrDataPtr, branch_list), (XtPointer)NULL,
     BranchListToString},

    {"restricted_directory", XmRString, sizeof(String),
     XtOffset(FileMgrDataPtr, restricted_directory), (XtPointer)NULL,
     _DtStringToString},

    {"title", XmRString, sizeof(String), XtOffset(FileMgrDataPtr, title),
     (XtPointer)NULL, _DtStringToString},

    {"helpVol", XmRString, sizeof(String), XtOffset(FileMgrDataPtr, helpVol),
     (XtPointer)NULL, _DtStringToString},

    {"selection_list", SELECTION_LIST, sizeof(XtPointer),
     XtOffset(FileMgrDataPtr, selection_list), (XtPointer)NULL,
     SelectionListToString},

    {"show_iconic_path", XmRBoolean, sizeof(Boolean),
     XtOffset(FileMgrDataPtr, show_iconic_path), (XtPointer)True,
     _DtBooleanToString},

    {"show_current_directory", XmRBoolean, sizeof(Boolean),
     XtOffset(FileMgrDataPtr, show_current_dir), (XtPointer)False,
     _DtBooleanToString},

    {"show_status_line", XmRBoolean, sizeof(Boolean),
     XtOffset(FileMgrDataPtr, show_status_line), (XtPointer)True,
     _DtBooleanToString},

    {"fast_cd_enabled", XmRBoolean, sizeof(Boolean),
     XtOffset(FileMgrDataPtr, fast_cd_enabled), (XtPointer)False,
     _DtBooleanToString},

    {"toolbox", XmRBoolean, sizeof(Boolean), XtOffset(FileMgrDataPtr, toolbox),
     (XtPointer)False, _DtBooleanToString},

    {"show_hid_enabled", XmRBoolean, sizeof(Boolean),
     XtOffset(FileMgrDataPtr, show_hid_enabled), (XtPointer)False,
     _DtBooleanToString},

    {"secondaryHelpDialogCount", XmRInt, sizeof(int),
     XtOffset(FileMgrDataPtr, secondaryHelpDialogCount), (XtPointer)0,
     _DtIntToString},

    {"attr_dialog_count", XmRInt, sizeof(int),
     XtOffset(FileMgrDataPtr, attr_dialog_count), (XtPointer)0, _DtIntToString},

    {"trashcan", XmRBoolean, sizeof(Boolean),
     XtOffset(FileMgrDataPtr, IsTrashCan), (XtPointer)False,
     _DtBooleanToString},
};
...
static void Create(Display *display, Widget parent, Widget *return_widget,
  ...
  trans_table = XtParseTranslationTable(translations_sp_esc);
  {
    ...
    resolvedTranslationString =
        ResolveTranslationString(translations_da, (char *)file_mgr_rec);
  ...
  n = 0;
  XtSetArg(args[n], XmNdeleteResponse, XmUNMAP);
  n++;

  if (first == False || TrashView) {
    XtSetArg(args[n], XmNgeometry, NULL);
    n++;
  } else
    first = False;
  ...
  XmGetColors(XtScreen(mainWidget), colormap, background, &foreground,
  ...
  if (TrashView) {
    ...
    file_mgr_rec->menuStates =
        (RENAME | MOVE | DUPLICATE | LINK | TRASH | MODIFY | CHANGEDIR |
         PREFERENCES | FILTER | FIND | CREATE_DIR | CREATE_FILE | SETTINGS |
         CLEAN_UP | MOVE_UP | HOME | CHANGE_DIR | TERMINAL);
  ...
  n++;
  XtSetArg(args[n], XmNtextTranslations, trans_table);
  n++;
  file_mgr_rec->header_frame = header_frame =
      XmCreateForm(mainWidget, "header_frame", args, n);

  XtAddCallback(header_frame, XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
                HELP_FILE_MANAGER_VIEW_STR);
  ...
  if (showFilesystem && !TrashView) {
    ...
    n = 0;
    XtSetArg(args[n], DtNfileMgrRec, file_mgr_rec);
    n++;
    ...
    n++;
    XtSetArg(args[n], XmNtopWidget, iconic_path_da);
    n++;
    XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
    ...
    XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
    n++;
    XtSetArg(args[n], XmNtopWidget, header_separator);
    n++;
    XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
    ...
    XtAddEventHandler(current_directory_frame,
                      ...
                      (XtEventHandler)CurrentDirectoryIconMotion, file_mgr_rec);

    /* Create the change directory drop zone. */
    if (showDropZone) {
      if (change_view_pixmap == XmUNSPECIFIED_PIXMAP) {
        change_view_pixmap =
            _DtGetPixmap(XtScreen(current_directory_frame), CHANGE_VIEW_ICON_S,
                         foreground, background);
      }
      n = 0;
      XtSetArg(args[n], XmNstring, NULL);
      n++;
      XtSetArg(args[n], XmNshadowThickness, 2);
      n++;
      XtSetArg(args[n], XmNfillOnArm, False);
      n++;
      XtSetArg(args[n], XmNhighlightThickness, 0);
      n++;
      XtSetArg(args[n], XmNpixmap, change_view_pixmap);
      n++;
      XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
      n++;
      ...
      XtAddCallback(current_directory_drop, XmNdropCallback, DropOnChangeView,
                    (XtPointer)file_mgr_rec);
    } else
    ...
    XtSetArg(args[n], XmNstring, NULL);
    n++;
    XtSetArg(args[n], XmNimageName, SMALL_DIRECTORY_ICON);
    n++;
    ...
    if (showDropZone) {
      ...
      n++;
      XtSetArg(args[n], XmNleftWidget, current_directory_drop);
      n++;
    ...
    file_mgr_rec->current_directory_icon = current_directory_icon =
        _DtCreateIcon(current_directory_frame, "current_directory_icon", args,
                      n);
    ...
    n++;
    XtSetArg(args[n], XmNpushButtonEnabled, False);
    n++;
    XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
    n++;
    ...
    n++;
    file_mgr_rec->current_directory = XmCreateDrawnButton(
        current_directory_frame, "current_directory", args, n);
    ...
    n++;
    XtSetArg(args[n], XmNleftWidget, file_mgr_rec->current_directory);
    n++;
    XtSetArg(args[n], XmNleftOffset, 0);
    n++;
    XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
    n++;
    XtSetArg(args[n], XmNbottomWidget, file_mgr_rec->current_directory);
    n++;
    XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNrightOffset, 1);
    ...
    file_mgr_rec->current_directory_text =
        XmCreateTextField(current_directory_frame, FAST_RENAME, args, n);
    ...
    XtAddCallback(file_mgr_rec->current_directory_text, XmNhelpCallback,
                  (XtCallbackProc)HelpRequestCB, HELP_FILE_MANAGER_VIEW_STR);
#ifdef FOO
  ...
  XmMainWindowSetAreas(mainWidget, menu, file_mgr_rec->header_frame, NULL, NULL,
                       work_frame);
  ...
  if (TrashView)
    XtAddCallback(file_mgr_rec->scroll_window, XmNhelpCallback,
                  (XtCallbackProc)HelpRequestCB, HELP_TRASH_OVERVIEW_TOPIC_STR);
  else
    XtAddCallback(file_mgr_rec->scroll_window, XmNhelpCallback,
                  (XtCallbackProc)HelpRequestCB, HELP_FILE_MANAGER_VIEW_STR);
  ...
  XtSetArg(args[0], XmNhorizontalScrollBar,
  ...
  XtAddEventHandler(file_mgr_rec->scroll_window, StructureNotifyMask, False,
                    (XtEventHandler)FileWindowResizeCallback, file_mgr_rec);
  XtAddEventHandler(shell, StructureNotifyMask, False,
                    (XtEventHandler)FileWindowMapCallback, file_mgr_rec);
  ...
  XtOverrideTranslations(
      file_mgr_rec->file_window,
      (XtTranslations)((XmManagerClassRec *)XtClass(file_mgr_rec->file_window))
  ...
  XtAddEventHandler(file_mgr_rec->file_window,
                    Button1MotionMask | Button2MotionMask, False,
                    (XtEventHandler)FileIconMotion, file_mgr_rec);
...
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list) {
  ...
  if (first) {
    ...
    XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XmRString, VIEW,
                      StringToView, NULL, 0);
    XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XmRString, ORDER,
                      StringToOrder, NULL, 0);
    XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XmRString,
                      DIRECTION_RESRC, StringToDirection, NULL, 0);
    XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XmRString, RANDOM,
                      StringToRandom, NULL, 0);
    XtAppSetTypeConverter(XtWidgetToApplicationContext(toplevel), XmRString,
                          BRANCH_LIST, (XtTypeConverter)StringToBranchList,
                          NULL, 0, XtCacheNone, NULL);
    XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XmRString,
                      SELECTION_LIST, (XtConverter)StringToSelectionList, NULL,
                      0);
  }
  ...
  if (restoreType == NORMAL_RESTORE &&
    ...
    _DtLoadSubdialogArray(new_name_list, name_list_count,
                          ...
                          file_mgr_data->secondaryHelpDialogCount, help_dialog,
                          data_base, 1);
  } else {
    file_mgr_data->secondaryHelpDialogList = NULL;
    file_mgr_data->secondaryHelpDialogCount = 0;
  ...
  if (restoreType == NORMAL_RESTORE) {
    ...
    if (((DialogInstanceData *)(file_mgr_data->primaryHelpDialog->data))
            ->displayed == False) {
  ...
  if (restoreType == NORMAL_RESTORE && file_mgr_data->attr_dialog_count > 0) {
    /* Get properties dialogs */
    _DtLoadSubdialogArray(
        new_name_list, name_list_count, &(file_mgr_data->attr_dialog_list),
        file_mgr_data->attr_dialog_count, mod_attr_dialog, data_base, 1);
  } else {
    file_mgr_data->attr_dialog_list = NULL;
    file_mgr_data->attr_dialog_count = 0;
  }
   ...
   */
  if (restoreType != NORMAL_RESTORE) {
    ((DialogInstanceData *)(file_mgr_data->change_dir->data))->displayed =
        False;

    ((DialogInstanceData *)(file_mgr_data->preferences->data))->displayed =
        False;
  }
  ...
  if (restoreType != NORMAL_RESTORE) {
    ((DialogInstanceData *)(file_mgr_data->filter_edit->data))->displayed =
        False;

    ((DialogInstanceData *)(file_mgr_data->find->data))->displayed = False;
  }
...
static void SetValues(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data) {
  ...
  if (file_mgr_data->cd_inited == False) {
    ...
    file_mgr_data->tree_solid_thin_gc = XtGetGC(widg, gc_mask, &gc_values);
    ...
    file_mgr_data->tree_dash_thick_gc = XtGetGC(
        widg, gc_mask | GCLineWidth | GCLineStyle | GCDashList, &gc_values);
  }
  ...
  if (showFilesystem && !TrashView) {
    ...
    pixmapData = GetPixmapData(file_mgr_rec, file_mgr_data,
                               file_mgr_data->current_directory, False);
    if (pixmapData)
    ...
    _DtCheckAndFreePixmapData(
        ...
        file_mgr_rec->shell, (DtIconGadget)cdi, pixmapData);
  ...
  if (file_mgr_data->selection_table != NULL) {
    ...
    while (file_mgr_data->selection_table[i] != NULL) {
      for (j = 0; j < directory_count; j++) {
        if (strncmp(directory_set[j]->name, file_mgr_data->selection_table[i],
          ...
          for (k = 0; k < directory_set[j]->file_count; k++) {
            if (strcmp(file_name, order_list[k]->file_data->file_name) == 0) {
  ...
  if (file_mgr_data != trashFileMgrData && file_mgr_data != NULL) {
  ...
  if (((DialogInstanceData *)(file_mgr_data->change_dir->data))->displayed ==
      True) {
    ShowChangeDirDialog((Widget)NULL, (XtPointer)file_mgr_rec, (XtPointer)NULL);
  } else
    file_mgr_rec->menuStates |= CHANGEDIR;

  if (((DialogInstanceData *)(file_mgr_data->preferences->data))->displayed ==
      True) {
  ...
  if (((DialogInstanceData *)(file_mgr_data->find->data))->displayed == True) {
    ShowFindDialog((Widget)NULL, (XtPointer)file_mgr_rec, (XtPointer)NULL);
  } else
    file_mgr_rec->menuStates |= FIND;

  if (((DialogInstanceData *)(file_mgr_data->filter_edit->data))->displayed ==
      True) {
  ...
  for (i = 0; i < file_mgr_data->attr_dialog_count; i++) {
    if (((DialogInstanceData *)(file_mgr_data->attr_dialog_list[i]->data))
            ->displayed == False) {
      continue;
    }
    loadOk = True;
    ...
    if (loadOk) {
       ...
       */
      ((ModAttrData *)(file_mgr_data->attr_dialog_list[i]->data))->main_widget =
          file_mgr_rec->main;

      _DtShowDialog(file_mgr_rec->shell, NULL, (XtPointer)file_mgr_rec,
                    file_mgr_data->attr_dialog_list[i], ModAttrChange,
                    (XtPointer)file_mgr_rec, ModAttrClose,
      ...
      if (file_mgr_data->title != NULL &&
        ...
        tempStr =
            (char *)XtMalloc(strlen(tmpStr) + strlen(file_mgr_data->title) + 5);
  ...
  for (i = 0; i < file_mgr_data->secondaryHelpDialogCount; i++) {
    ShowHelpDialog(file_mgr_rec->shell, (XtPointer)file_mgr_rec,
                   HYPER_HELP_DIALOG, file_mgr_data->secondaryHelpDialogList[i],
                   NULL, NULL, NULL, NULL, 0);
  }

  /* Display the primary help dialog, if active */
  if (file_mgr_data->primaryHelpDialog &&
      ((DialogInstanceData *)(file_mgr_data->primaryHelpDialog->data))
              ->displayed == True) {
    ShowHelpDialog(file_mgr_rec->shell, (XtPointer)file_mgr_rec,
                   MAIN_HELP_DIALOG, file_mgr_data->primaryHelpDialog, NULL,
                   NULL, NULL, NULL, 0);
  }
}
...
static void WriteResourceValues(DialogData *values, int fd, char **name_list) {
  ...
  if (file_mgr_data->current_directory && file_mgr_data->toolbox) {
    ...
    _DtDialogPutResources(fd, name_list, FILEMGR, values->data, resources,
                          fileMgrClass->resource_count);
...
static void FreeValues(FileMgrData *file_mgr_data) {
  ...
  if (showFilesystem && file_mgr_data != trashFileMgrData &&
    ...
    if (file_mgr_data->cd_normal_gc != NULL)
      XtReleaseGC(file_mgr_rec->current_directory, file_mgr_data->cd_normal_gc);
...
static void ActivateClist(Widget wid, XEvent *event, String *params,
  ...
  if (*(params[0]) != '@') {
    ...
    for (i = 0; i < len; ++i) {
      ((isalpha(tmp[i])) ? (message[i] = (int)(tmp[i]) - 1)
                         : (message[i] = tmp[i]));
    };
...
static void GetSessionDir(FileMgrData *file_mgr_data) {
  ...
  } else /* session restore */
  {
    if ((file_mgr_data->current_directory) && (file_mgr_data->toolbox) &&
      ...
      len = strlen(root_toolbox) + strlen(user_session_str) +
            strlen(toolbox_dir) + 1;
      current_dir = XtMalloc(len);
      if (current_dir) {
        snprintf(current_dir, len, "%s%s%s", root_toolbox, user_session_str,
                 toolbox_dir);
      ...
      if (file_mgr_data->restricted_directory) {
        snprintf(file_mgr_data->restricted_directory, len, "%s%s", root_toolbox,
                 user_session_str);
      }
    } else {
      file_mgr_data->current_directory =
...
PixmapData *GetPixmapData(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
                          char *path, Boolean large) {
  char *full_name;
  char *short_name;
  char *ftype;
  char *icon_name;
  ...
  full_name = ResolveLocalPathName(file_mgr_data->host, path, NULL,
  ...
  if (large)
    pixmapData = _DtRetrievePixmapData(ftype, short_name, full_name,
                                       file_mgr_rec->shell, LARGE);
  else
    pixmapData = _DtRetrievePixmapData(ftype, short_name, full_name,
                                       file_mgr_rec->shell, SMALL);
...
static void BranchListToString(int fd, void *val_ptr, char *out_buf) {
  char ***value = (char ***)val_ptr;
  int i;
  Boolean first = True;
  char *branch_name;
  int rv;
...
static void SelectionListToString(int fd, void *val_ptr, char *out_buf) {
  FileViewData ***value = (FileViewData ***)val_ptr;
  int i;
  Boolean first = True;
...
static Boolean StringToBranchList(Display *display, XrmValue *args,
  ...
  while (start != NULL) {
    ...
    table_size++;
    table =
        (char **)XtRealloc((char *)table, sizeof(char *) * (table_size + 1));
 ...
 */
static void UpdateStatusLine(FileMgrRec *file_mgr_rec,
                             FileMgrData *file_mgr_data) {
 ...
 */
static void MsgTimerEvent(FileMgrData *file_mgr_data, XtIntervalId *id) {
  FileMgrRec *file_mgr_rec;
 ...
 */
void SetSpecialMsg(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
                   char *msg) {
  if (file_mgr_data->special_msg) {
    XtFree(file_mgr_data->special_msg);
    file_mgr_data->special_msg = NULL;
  ...
  if (msg) {
    ...
    file_mgr_data->msg_timer_id = XtAppAddTimeOut(
        XtWidgetToApplicationContext(file_mgr_rec->shell), 4000,
        (XtTimerCallbackProc)MsgTimerEvent, (XtPointer)file_mgr_data);
  } else {
    file_mgr_data->special_msg = NULL;
    file_mgr_data->msg_timer_id = 0;
  }
 ...
 */
void UpdateHeaders(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
  ...
  if (!showFilesystem) {
    if (file_mgr_data->show_status_line)
      UpdateStatusLine(file_mgr_rec, file_mgr_data);
    return;
  } else if (file_mgr_data == trashFileMgrData && file_mgr_data) {
    UpdateStatusLine(file_mgr_rec, file_mgr_data);
    return;
  }
  ...
  if ((file_mgr_data->show_iconic_path == 0) !=
    ...
    DPRINTF(("UpdateHeaders: iconic_path %d, current_dir %d, status_line %d\n",
             file_mgr_data->show_iconic_path, file_mgr_data->show_current_dir,
    ...
    XtUnmanageChildren(((XmManagerWidget)file_mgr_rec->current_directory_frame)
                           ->composite.children,
    ...
    if (file_mgr_data->show_iconic_path && file_mgr_data->show_current_dir) {
      ...
      XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
      n++;
      XtSetArg(args[n], XmNtopWidget, file_mgr_rec->iconic_path_da);
      n++;
      XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
      n++;
      XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
    ...
    if (file_mgr_data->show_current_dir) {
      ...
      if (file_mgr_data->show_iconic_path) {
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, file_mgr_rec->header_separator);
        n++;
      } else {
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
        n++;
      ...
      */
      ...
      if (file_mgr_data->show_iconic_path) {
        ...
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, file_mgr_rec->current_directory_icon);
        n++;
        XtSetArg(args[n], XmNleftOffset, 0);
        n++;
        XtSetValues(file_mgr_rec->current_directory, args, n);
    ...
    if (icons_changed) {
      ...
      _DtCheckAndFreePixmapData(
          GetDirectoryLogicalType(file_mgr_data,
                                  file_mgr_data->current_directory),
          file_mgr_rec->shell,
          (DtIconGadget)file_mgr_rec->current_directory_icon, pixmapData);
...
void FileMgrRedisplayFiles(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
...
void ShowNewDirectory(FileMgrData *file_mgr_data, char *host_name,
                      char *directory_name) {
  FileMgrRec *file_mgr_rec;
  Arg args[1];
  ...
  tmp_type =
      DtDtsDataToDataType(directory_name, NULL, 0, NULL, NULL, NULL, NULL);
  if (((strcmp(tmp_type, LT_AGROUP) == 0) ||
    ...
    DtActionInvoke(((FileMgrRec *)file_mgr_data->file_mgr_rec)->shell,
                   "OpenAppGroup", action_args, 1, NULL, NULL, pwd_dir, True,
                   NULL, NULL);
    ...
    pwd_dir = XtNewString(file_mgr_data->current_directory);
    DtActionInvoke(((FileMgrRec *)file_mgr_data->file_mgr_rec)->shell, "Trash",
                   NULL, 0, NULL, NULL, pwd_dir, True, NULL, NULL);
    DtDtsFreeDataType(tmp_type);
    XtFree(pwd_dir);
    return;
  }
  ...
  else if (file_mgr_data == trashFileMgrData && file_mgr_data != NULL)
    UpdateStatusLine(file_mgr_rec, file_mgr_data);
...
void FileMgrReread(FileMgrRec *file_mgr_rec) {
  DialogData *dialog_data;
  FileMgrData *file_mgr_data;
  DirectorySet **directory_set;
  int i;
...
void FileMgrBuildDirectories(FileMgrData *file_mgr_data, char *host_name,
                             char *directory_name) {
...
Boolean DropOnGadget(FileMgrData *file_mgr_data, Widget w, Position x,
                     Position y) {
...
FileViewData *DropOnPrimaryHotspot(FileMgrData *file_mgr_data, Position drop_x,
                                   Position drop_y,
 ...
 *
 ************************************************************************/
static void ProcessDropOnFileWindow(Widget w, DtDndDropCallbackStruct *dropInfo,
                                    FileMgrData *file_mgr_data) {
  char *command = NULL;
  char *fileType;
  ...
  if (dropInfo->reason != DtCR_DND_DROP_ANIMATE) {
    ...
    */
    if (dropInfo->dropData->protocol == DtDND_BUFFER_TRANSFER) {
      if (file_mgr_data == trashFileMgrData && file_mgr_data != NULL) {
        ...
        DPRINTF(
            ("ProcessDropOnFileWindow: Rejecting buffer drop on Trash Can\n"));
        return;
      }
    }
    ...
    */
    ...
    DPRINTF(("DropOnFileWindow: Transfer Callback - Setting Complete move flag "
             "to False\n"));
    ...
    if (command) {
      ...
      for (i = 0; i < directory_set->file_count; ++i) {
        if (strcmp(directory_set->order_list[i]->file_data->file_name, ".") ==
            0) {
          RunCommand(command, file_mgr_data, directory_set->order_list[i], NULL,
                     dropInfo, w);
          break;
        }
      }
...
void DropOnFileWindow(Widget w, XtPointer client_data, XtPointer call_data) {
 ...
 *
 ************************************************************************/
static void ProcessDropOnObject(Widget w, DtDndDropCallbackStruct *dropInfo,
                                FileViewData *file_view_data) {
  char *fileType;

  DirectorySet *directory_data = (DirectorySet *)file_view_data->directory_set;
  ...
  if (dropInfo->reason != DtCR_DND_DROP_ANIMATE) {
    ...
    if (FileFromTrash(dropInfo->dropData->data.files[0])) {
      if (InvalidTrashDragDrop(
              dropInfo->operation, FROM_TRASH,
  ...
  /******************************************/
    ...
    command = TypeToAction(dropInfo->operation,
                           file_view_data->file_data->logical_type);
    if (command) {
      RunCommand(command, file_mgr_data, file_view_data, NULL, dropInfo, NULL);
...
void DropOnObject(Widget w, XtPointer client_data, XtPointer call_data) {
  DtDndDropCallbackStruct *dropInfo = (DtDndDropCallbackStruct *)call_data;
...
void FileMgrPropagateSettings(FileMgrData *src_data, FileMgrData *dst_data) {
  ...
  dst_data->view_tree = dst_preferences_data->view_tree = src_data->view_tree;
...
void UpdateBranchList(FileMgrData *file_mgr_data, DirectorySet *directory_set) {
  int i;
  char **table = NULL;
  int table_size = 0;
  char *branch_name;
...
static void GetFileData(FileMgrData *file_mgr_data, Boolean valid,
  ...
  ReadTreeDirectory(file_mgr_rec->shell, file_mgr_data->host,
                    file_mgr_data->current_directory, file_mgr_data,
                    branch_list, &new_dir_set, &new_dir_count);
    ...
    for (j = 0; j < directory_set[i]->file_count; j++) {
      ...
      for (k = 0; k < new_file_count; k++) {
        if (new_view_data[k]->file_data == file_view_data->file_data) {
          /* Fix for defect 5029    */
          if (file_mgr_data->popup_menu_icon &&
...
Boolean QueryBranchList(FileMgrData *file_mgr_data, char **branch_list,
...
static void ReadTreeDirectory(Widget w, char *host_name, char *directory_name,
  ...
  rc = ReadDir(w, file_mgr_data, host_name, directory_name, ip, 0,
               file_mgr_data->tree_preread_level, branch_list);
  if (file_mgr_data->busy_status != not_busy) {
  ...
  *directory_set = (DirectorySet **)XtMalloc((*directory_count + 1) *
                                             sizeof(DirectorySet *));
...
static void
SetDisplayedRecur(FileMgrData *fmd,
 ...
 *   a tree branch.
 */
void UpdateBranchState(FileMgrData *file_mgr_data, FileViewData *ip, int op,
                       Boolean busy) {
  ...
  else if (op == BRANCH_EXPAND) /* show more */
  {
    if (file_mgr_data->tree_files == TREE_FILES_NEVER) {
      ...
      if (ip->ndir == 0 && ip->nfile == 0) {
        ...
        ip->ts = tsNone;
      else if (ip->ts == tsNone && ip->ndir > 0 && ip->nfile > 0 &&
  ...
  else if (op == BRANCH_COLLAPSE) /* show less */
  {
    ...
    if (file_mgr_data->tree_files == TREE_FILES_NEVER) {
      ...
      if (ip->ndir == 0 && ip->nfile == 0) {
        ...
        ip->ts = tsAll;
      else if (ip->ts == tsAll && ip->ndir > 0 && ip->nfile > 0 &&
               file_mgr_data->tree_files == TREE_FILES_CHOOSE)
        ip->ts = tsDirs;
      else
        ip->ts = tsNone;
 ...
 */
void DirTreeExpand(FileMgrData *file_mgr_data, FileViewData *ip,
                   Boolean expand) {
  FileMgrRec *file_mgr_rec = (FileMgrRec *)file_mgr_data->file_mgr_rec;
  DirectorySet *directory_set;
  ...
  if (expand) {
    ...
    if (new_branch) {
      ...
      rc = ReadDir(file_mgr_rec->shell, file_mgr_data, file_mgr_data->host,
                   path, ip, level, level, NULL);
      ...
      if (ip->ts == tsNotRead) {
        file_mgr_data->directory_set =
            ((DirectorySet **)XtRealloc(
                (char *)(file_mgr_data->directory_set - 1),
                (file_mgr_data->directory_count + 2) *
                    sizeof(DirectorySet *))) +
            1;
...
static void FreeDirectorySet(DirectorySet **directory_set,
                             int directory_count) {
  int i, j;
...
static void SetIconAttributes(FileMgrRec *file_mgr_rec,
  ...
  if (tool_icon == XmUNSPECIFIED_PIXMAP) {
    ...
    XmGetColors(XtScreen(file_mgr_rec->main), colormap, background, &foreground,
                &top_shadow, &bottom_shadow, &select);

    /* first get the File Manager's Icon */
    pixmap = XmGetPixmap(XtScreen(file_mgr_rec->main), fileMgrIcon, foreground,
                         background);
    ...
    pixmap = XmGetPixmap(XtScreen(file_mgr_rec->main), appMgrIcon, foreground,
                         background);
    if (pixmap != XmUNSPECIFIED_PIXMAP)
      tool_icon = pixmap;
  ...
  if (file_mgr_data->title) {
    if (file_mgr_data->toolbox) {
      if (strcmp(directory_name, file_mgr_data->restricted_directory) == 0) {
        new_directory_name = (char *)XtMalloc(strlen(file_mgr_data->title) + 1);
 ...
 * field up and down.
 */
void ShowChangeDirField(Widget w, XtPointer client_data, XtPointer callback) {
  FileMgrRec *file_mgr_rec;
  DialogData *dialog_data;
  FileMgrData *file_mgr_data;
  Arg args[1];
...
void FreePositionInfo(FileMgrData *file_mgr_data) {
  int i, j;
  FileViewData **file_view_data;
...
Boolean PositioningEnabledInView(FileMgrData *file_mgr_data)
...
Boolean PositionFlagSet(FileMgrData *file_mgr_data)
...
void InheritPositionInfo(FileMgrData *src_file_mgr_data,
  ...
  temp_stack = (ObjectPosition **)XtMalloc(sizeof(ObjectPosition *) *
                                           dest_file_mgr_data->num_objects);

  for (i = 0; i < dest_file_mgr_data->num_objects; i++) {
    ptr = dest_file_mgr_data->object_positions[i] =
...
void SavePositionInfo(FileMgrData *file_mgr_data)

{
  ...
  if (file_mgr_data->num_objects <= 0)
    ...
    position_info = (PositionInfo *)XtMalloc(file_mgr_data->num_objects *
                                             sizeof(PositionInfo));
    for (i = 0; i < file_mgr_data->num_objects; i++) {
...
void LoadPositionInfo(FileMgrData *file_mgr_data) {
  PositionInfo *position_info;
  ObjectPosition *ptr;
  int numObjects;
  int i;
  ...
  numObjects = GetDirectoryPositionInfo(
      file_mgr_data->host, file_mgr_data->current_directory, &position_info);
  ...
  if (numObjects > 0) {
    ...
    file_mgr_data->object_positions =
        (ObjectPosition **)XtMalloc(sizeof(ObjectPosition *) * numObjects);
...
static void RemoveIconInWorkspace(char *fileName, char *workspaceName) {
  ...
  for (i = 0; i < desktop_data->numIconsUsed; i++) {
    ...
    if (strcmp(fileName, iconName) == 0 &&
      ...
      RemoveDT(desktopWin->shell, (XtPointer)desktopWin, (XtPointer)NULL);
    }
  }
}
 ...
 *
 **************************************************************************/
void CheckMoveType(FileMgrData *file_mgr_data, FileViewData *file_view_data,
  ...
  if (_DtSpacesInFileNames(file_set, file_count)) {
    char *tmpStr = (GETMESSAGE(
        27, 94,
        "The File Manager does not support objects\nwith spaces in their "
        "names, so we cannot delete them.\nTo delete these objects:\n  - "
        "select 'Open Terminal' from the File Manager menu\n  - then issue the "
        "remove command to delete them."));
  ...
  if (number > 0 && modifiers != ControlMask && modifiers != ShiftMask &&
      widget_dragged != NULL) {
    char *message = NULL;
    char *title;
    ...
    _DtCopyDroppedFileInfo(file_count, file_set, host_set, &global_file_set,
                           &global_host_set);
    G_dropx = drop_x;
    G_dropy = drop_y;
    fv = file_view_data;
    ...
    case DESKTOP:
      ...
      sprintf(to, "%s/%s", directory_data->name, fv->file_data->file_name);
      DtEliminateDots(to);
      break;
    ...
    default: /* view == NOT_DESKTOP */
      ...
      fm = file_mgr_data;
      dd = (DirectorySet *)NULL;
      to = (char *)XtMalloc(strlen(file_mgr_data->current_directory) + 1);
    ...
    if (number == 1) {
      ...
      if (file_mgr_data && file_mgr_data->toolbox) {
        ...
        ptr1 = _DtResolveAppManPath(ptr1, file_mgr_data->restricted_directory);
      ...
      tmpStr = (GETMESSAGE(
          12, 4,
          "The object %s is currently in folder %s.\nYou are moving the object "
          "to folder %s.\nIs this what you want to do?"));
#else
      tmpStr = (GETMESSAGE(
          12, 5,
          "The object %1$s is currently in folder %2$s.\nYou are moving the "
          "object to folder %3$s.\nIs this what you want to do?"));
#endif
...

      {
        ...
        if (file_mgr_data && file_mgr_data->toolbox) {
          to = _DtResolveAppManPath(to, file_mgr_data->restricted_directory);
          for (i = 0; i < number; ++i) {
            char *ptr, *ptr1;
            ...
            *ptr = 0x0;
            from =
                _DtResolveAppManPath(from, file_mgr_data->restricted_directory);
      ...
      if (notHere) {
        tmpStr = (GETMESSAGE(12, 6,
                             "At least one of the files you dropped is from a "
                             "different\nfolder.  You are moving all these "
                             "files to %s.\nIs this what you want to do?"));
    ...
    if (message) {
      _DtMessageDialog(toplevel, title, message, NULL, TRUE, MoveCancelCB,
                       MoveOkCB, NULL, HelpRequestCB, False, QUESTION_DIALOG);
  ...
  case DESKTOP:
    ...
    value =
        FileMoveCopyDesktop(file_view_data, directory, host_set, file_set,
                            file_count, modifiers, desktopWindow, NULL, NULL);
    break;
  ...
  case NOT_DESKTOP_DIR:
    ...
    sprintf(directory, "%s/%s", directory_data->name,
            file_view_data->file_data->file_name);
    DtEliminateDots(directory);
    value = FileMoveCopy(file_mgr_data, NULL, directory, target_host, host_set,
                         file_set, file_count, modifiers, NULL, NULL);
    break;
   ...
   */
  if (number > 0 && value && modifiers != ControlMask &&
      modifiers != ShiftMask) {
    Tt_status tt_status;
    ...
    for (j = 0; j < file_count; j++) {
      ...
      for (i = 0; i < desktop_data->numIconsUsed; i++) {
        desktopWin = desktop_data->desktopWindows[i];
        fileName = ResolveLocalPathName(
            desktopWin->host, desktopWin->dir_linked_to, desktopWin->file_name,
            home_host_name, &tt_status);
...
void SetPWD(char *viewHost, char *viewDir, char **pwdHostRet, char **pwdDirRet,
            char *type)
...
static void CreateFmPopup(Widget w) {
  ...
  n = 0;
  XtSetArg(args[n], XmNmenuAccelerator, "   ");
  n++;
  XtSetArg(args[n], XmNwhichButton, bMenuButton);
  n++;
  fileMgrPopup.menu = XmCreatePopupMenu(w, "FMPopup", args, n);
  XtAddCallback(fileMgrPopup.menu, XmNhelpCallback,
                (XtCallbackProc)HelpRequestCB, HELP_POPUP_MENU_STR);
  ...
  popupBtns[i++] = XmCreateSeparatorGadget(fileMgrPopup.menu, "sep2", NULL, 0);

  /* Create 'Properties' option - white space popup */
  label_string =
      XmStringCreateLocalized((GETMESSAGE(20, 150, "Change Permissions...")));
  XtSetArg(args[0], XmNlabelString, label_string);
  ...
  XtAddCallback(popupBtns[i++], XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
                HELP_POPUP_MENU_STR);
  ...
  XtAddCallback(popupBtns[i], XmNactivateCallback, ShowFindDialog,
                (XtPointer)FM_POPUP);
  XtAddCallback(popupBtns[i++], XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
                HELP_FILE_MENU_STR);
  ...
  XtAddCallback(popupBtns[i], XmNactivateCallback, CleanUp,
                (XtPointer)FM_POPUP);
  XtAddCallback(popupBtns[i++], XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
  ...
  XtAddCallback(popupBtns[i], XmNactivateCallback, SelectAll,
                (XtPointer)FM_POPUP);
  XtAddCallback(popupBtns[i++], XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
  ...
  XtAddCallback(popupBtns[i], XmNactivateCallback, UnselectAll,
                (XtPointer)FM_POPUP);
  XtAddCallback(popupBtns[i++], XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
  ...
  label_string =
      XmStringCreateLocalized((GETMESSAGE(20, 156, "Show Hidden Objects")));
  XtSetArg(args[0], XmNlabelString, label_string);
  mnemonic = ((char *)GETMESSAGE(20, 102, "H"));
  XtSetArg(args[1], XmNmnemonic, mnemonic[0]);

  fileMgrPopup.wsPopup[BTN_SHOWHIDDEN] = popupBtns[i] =
      XmCreateToggleButtonGadget(fileMgrPopup.menu, "showHiddenObjects", args,
                                 2);
  XtAddCallback(popupBtns[i], XmNvalueChangedCallback, ShowHiddenFiles,
                (XtPointer)FM_POPUP);
  XtAddCallback(popupBtns[i++], XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
  ...
  label_string =
      XmStringCreateLocalized((GETMESSAGE(20, 84, "Put in Workspace")));
  XtSetArg(args[0], XmNlabelString, label_string);
...
static void DoTheMove(int type) {
  ...
  case NOT_DESKTOP:
    ...
    */
    result = FileMoveCopy(fm, NULL, fm->current_directory, fm->host, host_set,
                          file_set, file_count, (unsigned int)0, NULL, NULL);
#ifdef FOO
    CheckDesktop();
    if (type == DESKTOP) {
      for (j = 0; j < file_count; j++) {
  ...
  case NOT_DESKTOP_DIR:
    ...
    result = FileMoveCopy(fm, NULL, directory, fm->host, host_set, file_set,
                          file_count, (unsigned int)0, NULL, NULL);
  ...
  default: {
    ...
    sprintf(directory, "%s/%s", directory_data->name, fv->file_data->file_name);

    result = FileMoveCopyDesktop(fv, directory, host_set, file_set, file_count,
                                 mod, dtWindow, NULL, NULL);
  } break;
  }
