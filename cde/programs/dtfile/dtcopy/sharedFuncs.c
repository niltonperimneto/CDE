...
void split_path(const String path, String folder, String object)
...
String build_path(const String folder, const String object)
...
int auto_rename(const String path)
...
void generate_NewPath(String newPath, String oldPath)
...
XImage *CreateDefaultImage(Display *display, char *bits, unsigned int width,
                           unsigned int height) {
  XImage *image;

  image =
      XCreateImage(display, DefaultVisual(display, DefaultScreen(display)), 1,
                   XYBitmap, 0, bits, width, height, 8, (width + 7) >> 3);
...
void ImageInitialize(Display *display) {
  XImage *image;
  ...
  image = CreateDefaultImage(display, (char *)xm_warning_bits, xm_warning_width,
                             xm_warning_height);
  XmInstallImage(image, "default_xm_warning");

  image = CreateDefaultImage(display, (char *)xm_working_bits, xm_working_width,
                             xm_working_height);
...
static int CopyCheckDeletePermissionRecur(char *destinationPath) {
  struct stat statbuf;
  DIR *dirp;
  struct dirent *dp;
  Boolean first_file;
...
static int CopyCheckDeletePermission(char *parentdir, char *destinationPath) {
...
void CloseTopLevel(Widget w, void *client_data, void *call_data) {
  XtDestroyWidget((Widget)G_toplevel);
  exit(-1);
}

void CheckDeleteAccess(XtAppContext app_context, int delay, Boolean checkPerms,
                       Boolean move, char *source_name) {
  if (checkPerms && move) {
    ...
    tmpmsg = GETMESSAGE(
        4, 8,
        "You do not have permission to put the object \n\n%s\n\ninto "
        "trash.\n\nUse the Change Permissions choice from the object's\npopup "
        "menu or from the Selected menu to turn on your\nRead permission on "
        "the object.\n\nNote: If this object is a folder, you must also "
        "have\nRead permission for each of the objects inside the\nfolder "
        "before you can put the folder in the trash.");

    msg = XtMalloc(strlen(tmpmsg) + strlen(source_name) + 2);
    sprintf(msg, tmpmsg, source_name);
    _DtMessageDialog(G_toplevel, title, msg, 0, FALSE, NULL, CloseTopLevel,
                     NULL, NULL, False, ERROR_DIALOG);
    /*
...
void TimeoutHandler(XtPointer client_data, XtIntervalId *id) { exit(0); }
...
#endif
{
  ...
  if ((input_string[1] != '/')) {
    ...
    full_path =
        (char *)XtMalloc(strlen(homedir) + strlen(input_string + 2) + 1);
...
int Check(char *spth, char *dpth, int mode) {
