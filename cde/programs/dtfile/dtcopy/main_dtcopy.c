...
static char *DefaultLongFileOpNames[] = {
    "processing",   "opening",       "opening", "opening",
    "reading link", "deleting",      "copying", "creating directory",
    "copying link", "creating link", "working", "is a copy",
    "is a link",    "renaming"};
...
static XtResource resources[] = {
...

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(AppArgsPtr, source_name),
        XmRImmediate,
        (XtPointer)NULL,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(AppArgsPtr, target_name),
        XmRImmediate,
        (XtPointer)NULL,
    },
...

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.verbose),
        XmRImmediate,
        (XtPointer)True,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.quiet),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.dontdoit),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.keepnew),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(AppArgsPtr, sync.keepold),
        XmRImmediate,
        (XtPointer)NULL,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.dontdelete),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.dontadd),
        XmRImmediate,
        (XtPointer)False,
    },
...

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.dontrecur),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.keeplinks),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.keepcopies),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.forcecopies),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.listlinks),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.listcopies),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.linkdirs),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.linkfiles),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.copydirs),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.copyfiles),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBool,
        sizeof(int),
        XtOffset(AppArgsPtr, sync.copytop),
        XmRImmediate,
        (XtPointer)False,
    },
...

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(AppArgsPtr, confirmreplace),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(AppArgsPtr, confirmerrors),
        XmRImmediate,
        (XtPointer)False,
    },
...

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(AppArgsPtr, use_toggle_buttons),
        XmRImmediate,
        (XtPointer)True,
    },
    {
...

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(AppArgsPtr, checkPerms),
        XmRImmediate,
        (XtPointer)False,
    },
};
...
static void UpdateSummary(void) {
  ...
  XtVaSetValues(G_summary_text, XmNlabelString, xs, NULL);
  XmStringFree(xs);
}

static void UpdateStatus(char *msg1, char *msg2) {
...
static void UpdateErrorCount(void) {
  char msg[128];
  ...
  if (nerr == 1) {
    XtVaSetValues(G_status_text, XmNbottomAttachment, XmATTACH_NONE, NULL);
    /*
        XtVaSetValues(G_status_text2,
  ...
  XtVaSetValues(G_error_count, XmNvalue, msg, XmNcursorPosition, 0, NULL);
}

static void AppendErrorMsg(char *msg) {
  XmTextPosition pos;
...
/* ARGSUSED */
static void CheckForMap(Widget w, XtPointer client_data, XEvent *event,
                        Boolean *cont)
...
/* process X events until there are no more pending events queued */
{
  ...
  /* if the user clicked on the Cancel button and Warning 'yes' btn, abort the
   * copy */
  if (G_do_copy)
    return 0;
  else {
...
static int ErrorHandler(FileOp op, char *fname, int errnum) {
  char msg[2048];
...
static int ConfirmHandler(FileOp op, char *sname, int stype, char *tname,
                          int ttype, char *link) {
  char msg[1024];
  Arg args[5];
...
static int moveDeleteCallback(char *fname) {
  return ConfirmHandler(op_delete, "", ft_noent, fname, 0, "");
}

static int moveErrorCallback(char *fname, int errnum) {
...
static void InitNlsMessage(void) {
  int i;
  char *msg;
...
int main(int argc, char *argv[]) {
  ...
  G_toplevel = XtAppInitialize(
      &app_context, "Dtfile", /* Dtcopy is considered part of Dtfile */
      option_list, XtNumber(option_list), &argc, argv, NULL, args, n);

  InitNlsMessage();

  XtGetApplicationResources(G_toplevel, &app_args, resources,
                            XtNumber(resources), NULL, 0);

  /* add support for editres debugging tool */
  /* XtAddEventHandler(G_toplevel, (EventMask) 0, True, _XEditResCheckMessages,
   * NULL); */
  ...
  if (G_move) {
    XtSetArg(args[n], XmNtitle,
             GETMESSAGE(2, 11, "File Manager - Folder Move"));
    n++;
  } else {
    XtSetArg(args[n], XmNtitle,
             GETMESSAGE(2, 12, "File Manager - Folder Copy"));
    n++;
  }
  ...
  nfiles = 0;
  ndirs =
      -1; /* -1 so the source folder does not get counted, only subfolders */
  UpdateSummary();
  nerr = 0;
  UpdateErrorCount();
  ...
  XtVaSetValues(G_error_msgs, XmNcursorPosition, 0, XmNtopCharacter, 0, NULL);
