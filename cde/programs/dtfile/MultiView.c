...
extern void _DtGlobalRestoreMenuSensitivity(Widget w, XtPointer client_data,
                                            XtPointer call_data);
static Widget CreateMenuBtn(Widget parent, Boolean isPushButton,
                            MenuDesc *menuDesc, int btnIndex);
...
Widget _DtCreateMenuSystem(Widget parent, char *menuBarName,
                           XtCallbackProc helpCallback, XtPointer helpData,
                           Boolean createSharedComponents, MenuDesc *menuDesc,
                           int numMenuComponents,
                           DtGetMenuDataProc getMenuDataProc,
                           XtCallbackProc setMenuSensitivity,
                           XtCallbackProc restoreMenuSensitivity)
  ...
  for (i = 0; i < numMenuComponents; i++) {
    ...
    case MENU_PULLDOWN_BUTTON: {
      ...
      if (menuDesc[i].mnemonic) {
        XtSetArg(args[n], XmNmnemonic, menuDesc[i].mnemonic[0]);
        n++;
      }
      XtSetArg(args[n], XmNmarginWidth, 6);
      n++;
      child = XmCreateCascadeButtonGadget(menubar, menuDesc[i].name, args, n);
      if (menuDesc[i].helpCallback) {
        XtAddCallback(child, XmNhelpCallback, menuDesc[i].helpCallback,
                      menuDesc[i].helpData);
      }
    ...
    case MENU_PANE: {
      if (createSharedComponents || (menuDesc[i].type == MENU_PANE)) {
        ...
        if (firstMenuPane) {
          XtAddCallback(XtParent(child), XmNpopupCallback, setMenuSensitivity,
                        (XtPointer)getMenuDataProc);
          XtAddCallback(XtParent(child), XmNpopdownCallback,
                        restoreMenuSensitivity, (XtPointer)getMenuDataProc);
          firstMenuPane = False;
        }
    ...
    case MENU_BUTTON: {
      ...
      if ((createSharedComponents) || (lastMenuPaneType == MENU_PANE)) {
        child = CreateMenuBtn(lastMenuPane, (menuDesc[i].type == MENU_BUTTON),
    ...
    case MENU_SEPARATOR: {
      ...
      if ((createSharedComponents) || (lastMenuPaneType == MENU_PANE)) {
        child =
            XmCreateSeparatorGadget(lastMenuPane, menuDesc[i].name, NULL, 0);
        XtManageChild(child);
        menuDesc[i].widget = child;
      }
      break;
...
/* ARGSUSED */
void _DtGlobalSetMenuSensitivity(Widget w, XtPointer client_data,
                                 XtPointer call_data)

{
  ...
  DtGetMenuDataProc getMenuData = (DtGetMenuDataProc)client_data;
  ...
  for (i = 0; i < menuItemCount; i++) {
    ...
    case MENU_BUTTON: {
      if ((menuDesc[i].maskBit) && ((*globalMenuStates & menuDesc[i].maskBit) !=
                                    (*viewMenuStates & menuDesc[i].maskBit))) {
        _DtGlobalUpdateMenuItemState(menuDesc[i].widget, menuDesc[i].maskBit,
                                     *viewMenuStates & menuDesc[i].maskBit,
                                     globalMenuStates);
      }
      break;
...
/* ARGSUSED */
void _DtGlobalRestoreMenuSensitivity(Widget w, XtPointer client_data,
                                     XtPointer call_data)
  ...
  for (i = 0; i < menuItemCount; i++) {
    if (menuDesc[i].type == MENU_BUTTON) {
      if ((menuDesc[i].maskBit) &&
        ...
        _DtGlobalUpdateMenuItemState(menuDesc[i].widget, menuDesc[i].maskBit,
                                     True, globalMenuStates);
      }
    }
  }
}

void _DtGlobalUpdateMenuItemState(Widget btn, unsigned int mask,
                                  unsigned int on,
                                  unsigned int *globalMenuStates)

{
  if (on != 0) {
    ...
    *globalMenuStates |= mask;
    ...
    if (btn != *findBtn && btn != *change_directoryBtn &&
        btn != *preferencesBtn && btn != *defaultEnvBtn && btn != *filterBtn) {
...
static Widget CreateMenuBtn(Widget parent, Boolean isPushButton,
  ...
  XtSetArg(args[n], XmNlabelString, labelString);
  n++;
  XtSetArg(args[n], XmNmnemonic, menuDesc->mnemonic[0]);
  n++;
  if (isPushButton)
  ...
  if (menuDesc->activateCallback) {
    if (isPushButton) {
      ...
      XtAddCallback(child, XmNvalueChangedCallback, menuDesc->activateCallback,
                    menuDesc->activateData);
    }
  }
  XmStringFree(labelString);
