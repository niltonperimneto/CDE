...
static void ScheduleActivity(Directory *directory);
static int WritePosInfoProcess(int pipe_fd, Directory *directory,
                               ActivityStatus activity);
static int ReadDirectoryProcess(int pipe_fd, Directory *directory,
                                ActivityStatus activity);
static int UpdateAllProcess(int pipe_fd, Directory *directory,
                            ActivityStatus activity);
static int UpdateSomeProcess(int pipe_fd, Directory *directory,
                             ActivityStatus activity);
static int TimerEventProcess(int pipe_fd, Directory *directory,
                             ActivityStatus activity);
static int CheckDesktopProcess(int pipe_fd, Directory *directory,
                               ActivityStatus activity);
static void WritePosInfoPipeCallback(XtPointer client_data, int *fd,
                                     XtInputId *id);
static void ReaddirPipeCallback(XtPointer client_data, int *fd, XtInputId *id);
static void TimerPipeCallback(XtPointer client_data, int *fd, XtInputId *id);
static void CheckDesktopPipeCallback(XtPointer client_data, int *fd,
                                     XtInputId *id);
...
static Directory dummy_dir_struct = {
    "dummy_host", "dummy_directory", "dummy_path", NULL, False, activity_idle};
...
static struct {
  ...
  StickyProcDesc *sticky_procs;
    ...
    {WritePosInfoProcess, WritePosInfoPipeCallback, False,
     NULL}, /* writing_posinfo*/
    ...
    {CheckDesktopProcess, CheckDesktopPipeCallback, False,
     NULL},                                             /* checking_desktop */
...
static void InitializePositionFileName(void) {
  ...
  if (positionFileName == NULL) {
    ...
    positionFileName =
        XtMalloc(strlen(pwInfo->pw_name) + strlen(POSITION_FILE_PREFIX) + 1);
...
void InitializeDirectoryRead(Widget widget)

{
  ...
  if (desktop_data->numIconsUsed > 0 && checkBrokenLink != 0) {
    checkBrokenLinkTimerId = XtAppAddTimeOut(
        app_context, checkBrokenLink * 1000, TimerEventBrokenLinks, NULL);
  } else {
...
static Directory *FindDirectory(char *host_name, char *directory_name) {
  int i;
...
static Boolean DirectoryGone(Directory *directory) {
  int i;
...
void FreeFileData(FileData *file_data, Boolean free_all) {
...
void PipeWriteFileData(int fd, FileData *file_data) {
...
static FileData *PipeReadFileData(int fd) {
  FileData *file_data;
  int n;
...
FileData *FileData2toFileData(FileData2 *file_data2, int *l) {
  FileData *file_data;
  int n;
  ...
  file_data->action_name =
      file_data2->action_name ? XtNewString(action_name_buf) : NULL;
  ...
  file_data->link = file_data2->link ? XtNewString(link_buf) : NULL;

  file_data->final_link =
      file_data2->final_link ? XtNewString(final_link_buf) : NULL;
  file_data->is_subdir = file_data2->is_subdir;
  file_data->is_broken = file_data2->is_broken;

  *l = sizeof(*file_data2) - sizeof(file_data2->text) + file_data2->file_name +
       file_data2->action_name + file_data2->logical_type + file_data2->link +
...
void PipeWritePositionInfo(int fd, PositionInfo *position_info) {
...
static void PipeReadPositionInfo(int fd, PositionInfo *position_info) {
...
FileData *ReadFileData(char *full_directory_name, char *file_name) {
  FileData *file_data;
    ...
    if ((stat_buf.st_mode & S_IFMT) == S_IFLNK) {
      ...
      if (file_data->link)
        file_data->logical_type =
            (char *)DtDtsDataToDataType(file_data->link, NULL, 0, &stat_buf,
                                        file_data->final_link, NULL, NULL);
      else
        file_data->logical_type = (char *)DtDtsDataToDataType(
            full_file_name, NULL, 0, &stat_buf, NULL, NULL, NULL);
...
int ReadFileData2(FileData2 *file_data2, char *full_directory_name,
  ...
  if (stat_result == 0) {
    ...
    if ((stat_buf.st_mode & S_IFMT) == S_IFLNK) {
      ...
      if (link_buf[0] == NILL)
        ptr = (char *)DtDtsDataToDataType(full_file_name, NULL, 0, &stat_buf,
                                          NULL, NULL, NULL);
      else
        ptr = (char *)DtDtsDataToDataType(link_buf, NULL, 0, &stat_buf,
                                          final_link_buf, NULL, NULL);
      ...
      */
      if (!IsToolBox && S_ISDIR(stat_buf.st_mode) &&
        ...
        if (strcmp(file_name, "..") != 0 && strcmp(file_name, ".") != 0) {
          ...
          if (access(fullPathName, R_OK) != 0) {
            free(ptr); /* Don't use XtFree. This pointer is being kept by
                          tooltalk */
        ...
        strcpy(logical_type_buf, ptr);
        free(
            ptr); /* Don't use XtFree. This pointer is being kept by tooltalk */
      }
#endif
    }
    ...
    if (DtActionExists(logical_type_buf)) {
      ...
      char *ptr =
          DtDtsDataTypeToAttributeValue(logical_type_buf, DtDTS_DA_LABEL, NULL);
      if (ptr) {
        strcpy(action_name_buf, ptr);
        DtDtsFreeAttributeValue(ptr);
      }
  ...
  i = sizeof(*file_data2) - sizeof(file_data2->text) + file_data2->file_name +
      file_data2->action_name + file_data2->logical_type + file_data2->link +
      file_data2->final_link;
...
static int ReadDirectoryProcess(int pipe_fd, Directory *directory,
  ...
  for (;;) {
    ...
    full_directory_name = ResolveLocalPathName(host_name, namePtr, NULL,
    ...
    path_logical_types[path_count] = (char *)DtDtsDataToDataType(
        full_directory_name, NULL, 0, NULL, NULL, NULL, NULL);
#if defined(DATATYPE_IS_FIXED)
#else
    {
        ...
        if (S_ISDIR(stat_buf.st_mode) &&
          ...
          if (access(full_directory_name, R_OK) != 0) {
            ...
            path_logical_types[path_count] =
                XtNewString(LT_NON_WRITABLE_FOLDER);
          }
        }
      }
    }
  ...
  {
    Tt_status tt_status;
    full_directory_name = ResolveLocalPathName(host_name, directory_name, NULL,
                                               home_host_name, &tt_status);
  ...
  DPRINTF(
      ("ReadDirectoryProcess: sending %d path_logical_types\n", path_count));
   ...
   */
  if ((ptr = strrchr(full_directory_name, '/')) && (strcmp(ptr, "/.dt") == 0))
    inDtDir = True;
  else
    inDtDir = False;
...
#define PIPEMSG_HDR_LEN (2 * sizeof(short) + sizeof(int))
  ...
  do {
    ...
    if ((dp = readdir(dirp)) != NULL) {
      ...
      if (directory->directoryView && directory->directoryView->file_mgr_data)
        ...
        IsToolBox = False;
      len = ReadFileData2((FileData2 *)file_data_buf_ptr, full_directory_name,
                          dp->d_name, IsToolBox);
    ...
    if (file_data_count == FILEDATABUF ||
      ...
      *(int *)(file_data_buffer + 2 * sizeof(short)) = len;
      write(pipe_fd, file_data_buffer, file_data_buf_ptr - file_data_buffer);
  ...
  if ((fptr = fopen(file_name, "r")) != NULL) {
    ...
    if (position_count > 0) {
      ...
      position_info =
          (PositionInfo *)XtMalloc(position_count * sizeof(PositionInfo));
      ...
      while (i < position_count) {
        if (fgets(file_name, MAX_PATH, fptr) != NULL &&
          ...
          position_info[i].x = x, position_info[i].y = y,
    ...
    DPRINTF(
        ("ReadDirectoryProcess: sending %d position_info\n", position_count));
    write(pipe_fd, &pipe_msg, sizeof(short));
...
static int UpdateAllProcess(int pipe_fd, Directory *directory,
  ...
  if (rc) {
    ...
    for (;;) {
      ...
      DtEliminateDots(full_directory_name);
      path_logical_types = (char **)XtRealloc(
          (char *)path_logical_types, (path_count + 1) * sizeof(char *));
      path_logical_types[path_count] = (char *)DtDtsDataToDataType(
          full_directory_name, NULL, 0, NULL, NULL, NULL, NULL);
#if defined(DATATYPE_IS_FIXED)
#else
  ...
  while (dp = readdir(dirp)) {
    ...
    for (old_pp = &directory->file_data; (old_data = *old_pp) != NULL;
...
static int UpdateSomeProcess(int pipe_fd, Directory *directory,
  ...
  for (i = 0; i < directory->modified_count; i++) {
    ...
    ReadFileData2(&file_data2, full_directory_name, directory->modified_list[i],
                  IsToolBox);
...
static void ReaddirPipeCallback(XtPointer client_data, int *fd, XtInputId *id) {
  ...
  case PIPEMSG_PATH_LOGICAL_TYPES:
    ...
    if (directory->path_count != n) {
      ...
      directory->path_logical_types = (char **)XtRealloc(
          (char *)directory->path_logical_types, n * sizeof(char *));
    }
  ...
  case PIPEMSG_FILEDATA3:
    ...
    if (msg == PIPEMSG_FILEDATA3) {
      ...
      for (new_nextp = &directory->new_data; *new_nextp;
      ...
      for (i = 0; i < file_data_count; i++) {
        ...
        new_data = FileData2toFileData((FileData2 *)file_data_buf_ptr, &n);
        file_data_buf_ptr += n;
  ...
  if (done) {
     ...
     */
    for (new_nextp = &directory->new_data; (new_data = *new_nextp) != NULL;
         new_nextp = &new_data->next) {
      for (old_nextp = &directory->file_data; (old_data = *old_nextp) != NULL;
     ...
     */
    for (new_data = directory->file_data; new_data != NULL;
...
static void ReadDirectoryFiles(Widget w, Directory *directory) {
  FileMgrData *file_mgr_data;
  FileMgrRec *file_mgr_rec;
  int i;
...
static Boolean ReadDirectory(Widget w, char *host_name, char *directory_name,
                             FileData **file_data, int *file_count,
                             FileMgrData *file_mgr_data) {
  Directory *directory;
  int i;
  char *err_msg;
  ...
  if ((directory != NULL) &&
    ...
    if (i == directory->numOfViews) {
      directory->directoryView = (DirectoryView *)XtRealloc(
          (char *)directory->directoryView, sizeof(DirectoryView) * (i + 1));
    ...
    if (directory->errmsg_needed && !directory->busy[activity_reading] &&
        w != NULL) {
    ...
    if (directory_count == directory_set_size) {
      ...
      directory_set = (Directory **)XtRealloc(
          (char *)directory_set, sizeof(Directory **) * directory_set_size);
    }
    ...
    directory->path_name = ResolveLocalPathName(host_name, directory_name, NULL,
                                                home_host_name, &tt_status);
    ...
    directory->directoryView = (DirectoryView *)XtMalloc(sizeof(DirectoryView));
...
static int _ReadDir(Widget w, FileMgrData *file_mgr_data, char *host_name,
                    char *directory_name, FileViewData *dp, /* directory info */
 ...
 */
{
  ...
  DPRINTF2(("_ReadDir(\"%s\", \"%s\"): level %d, read_level %d\n", host_name,
            directory_name, level, read_level));
  ...
  busy_reading = ReadDirectory(w, host_name, directory_name, &file_data, &n,
                               file_mgr_data);
  if (busy_reading) {
    file_mgr_data->busy_status = busy_readdir;
    return 0;
  ...
  for (i = 0, fp = file_data; i < n && fp; i++, fp = fp->next) {
    ...
    if (fp->is_subdir) {
      ...
      if (level < read_level || ts != tsNotRead) {
        ...
        rc = _ReadDir(w, file_mgr_data, host_name, subdir_name, ip, level,
...
int ReadDir(Widget w, FileMgrData *file_mgr_data, char *host_name,
            char *directory_name, FileViewData *dp, /* directory info */
            int level,          /* tree level of this directory */
            int read_level,     /* deepest level to be read */
            char **branch_list) /* list of tree branches to expand */
{
  ...
  _ReadDir(w, file_mgr_data, host_name, directory_name, NULL, level, read_level,
           branch_list);
...
void FileWindowMapUnmap(FileMgrData *file_mgr_data)
...
void RereadDirectory(Widget w, char *host_name, char *directory_name) {
  Directory *directory;
...
void UpdateDirectory(Widget w, char *host_name, char *directory_name) {
  Directory *directory;
...
void DirectoryAbortModify(char *host_name, char *directory_name) {
  Directory *directory;
...
void DirectoryBeginModify(char *host_name, char *directory_name) {
  Directory *directory;
...
void DirectoryModifyTime(char *host_name, char *directory_name,
                         long modify_time) {
  Directory *directory;
...
void DirectoryFileModified(char *host_name, char *directory_name,
                           char *file_name) {
  Directory *directory;
  int i;
  ...
  if (directory != NULL) {
    ...
    directory->modified_list = (char **)XtRealloc(
        (char *)directory->modified_list, (i + 1) * sizeof(char *));
...
void DirectoryEndModify(char *host_name, char *directory_name) {
  Directory *directory;
...
void UpdateCachedDirectories(View **view_set, int view_count) {
    ...
    for (k = 0; k < file_mgr_data->directory_count; k++) {
      ...
      if (directory == NULL) {
        fprintf(stderr, "Warning: %s:%s not found in directory set.\n",
                view_set[j]->host_name, file_mgr_data->directory_set[k]->name);
        continue;
      }
      ...
      directory->directoryView = (DirectoryView *)XtRealloc(
          (char *)directory->directoryView, sizeof(DirectoryView) * (n + 1));
...
char *GetLongName(FileData *file_data) {
#ifdef NLS16
  struct tm *tms;
  ...
  if (file_data->errnum == 0) {
    ...
    strftime(time_string, 100, GetSharedMessage(DIRECTORY_DATE_FORMAT), tms);
#else
  ...
  if (file_data->stat.st_mode & S_IRUSR)
    usr_read_priv = READ_PRIV;
  else
    usr_read_priv = OPTION_OFF;

  if (file_data->stat.st_mode & S_IWUSR)
    usr_write_priv = WRITE_PRIV;
  else
    usr_write_priv = OPTION_OFF;

  if (file_data->stat.st_mode & S_IXUSR)
    usr_exec_priv = EXEC_PRIV;
  else
    usr_exec_priv = OPTION_OFF;

  if (file_data->stat.st_mode & S_IRGRP)
    grp_read_priv = READ_PRIV;
  else
    grp_read_priv = OPTION_OFF;

  if (file_data->stat.st_mode & S_IWGRP)
    grp_write_priv = WRITE_PRIV;
  else
    grp_write_priv = OPTION_OFF;

  if (file_data->stat.st_mode & S_IXGRP)
    grp_exec_priv = EXEC_PRIV;
  else
    grp_exec_priv = OPTION_OFF;

  if (file_data->stat.st_mode & S_IROTH)
    oth_read_priv = READ_PRIV;
  else
    oth_read_priv = OPTION_OFF;

  if (file_data->stat.st_mode & S_IWOTH)
    oth_write_priv = WRITE_PRIV;
  else
    oth_write_priv = OPTION_OFF;

  if (file_data->stat.st_mode & S_IXOTH)
    oth_exec_priv = EXEC_PRIV;
  else
    oth_exec_priv = OPTION_OFF;
...
#define NAME_PRECISION 28
    if (!is_multibyte) {
      ...
      if (len > NAME_PRECISION) {
        ...
        sprintf(long_name,
                "%-28.28s  %s  %9ld  %c%c%c%c%c%c%c%c%c%c  %-9s  %-9s  %s",
                name, time_string, (long)file_data->stat.st_size, permission,
                usr_read_priv, usr_write_priv, usr_exec_priv, grp_read_priv,
                grp_write_priv, grp_exec_priv, oth_read_priv, oth_write_priv,
                oth_exec_priv, user_name, group_name, link_path);
      } else {
        sprintf(long_name,
                "%-28.28s  %s  %9ld  %c%c%c%c%c%c%c%c%c%c  %-9s  %-9s  %s",
                file_data->file_name, time_string,
                (long)file_data->stat.st_size, permission, usr_read_priv,
                usr_write_priv, usr_exec_priv, grp_read_priv, grp_write_priv,
                grp_exec_priv, oth_read_priv, oth_write_priv, oth_exec_priv,
       ...
       * a valid multibyte string (at least the glibc version), but we can't
       * fail to display a file because it's name has some invalid characters).
       * So it looks that instead of using sprintf() we have to format the file
       * name part manually.
       */
      int len = DtCharCount(file_data->file_name);
      int copy_len =
          len > NAME_PRECISION ? NAME_PRECISION - sizeof(ELLIPSIS) + 1 : len;
      int byte_len = 0;
      int count;
      ...
      for (count = 0; (count < copy_len) && *(file_data->file_name + byte_len);
           count++) {
        int chr_bytes = mblen(file_data->file_name + byte_len, MB_CUR_MAX);
        if (chr_bytes > 0) {
          strncpy(long_name + byte_len, file_data->file_name + byte_len,
                  chr_bytes);
        } else if (chr_bytes < 0) { /* invalid char */
          chr_bytes = 1;
          long_name[byte_len] = '?';
        } else {
      ...
      sprintf(long_name + byte_len,
              "  %s  %9ld  %c%c%c%c%c%c%c%c%c%c  %-9s  %-9s  %s", time_string,
              (long)file_data->stat.st_size, permission, usr_read_priv,
              usr_write_priv, usr_exec_priv, grp_read_priv, grp_write_priv,
              grp_exec_priv, oth_read_priv, oth_write_priv, oth_exec_priv,
              user_name, group_name, link_path);
...
Boolean DirectoryBusy(char *path) {
  FileMgrData *file_mgr_data;
      ...
      for (j = 0; j < directory_set[i]->numOfViews; j++) {
        ...
        if (sub_root->displayed &&
            (sub_root->ts == tsDirs && sub_root->ndir > 0 ||
             sub_root->ts == tsAll && sub_root->ndir + sub_root->nfile > 0)) {
          return True;
        }
      }
    }
...
char *GetDirectoryLogicalType(FileMgrData *file_mgr_data, char *path) {
  int len;
  int n;
  Directory *directory;
  char *ptr;
  ...
  if (strncmp(file_mgr_data->current_directory, path, len) != 0 ||
      (len > 1 && file_mgr_data->current_directory[len] != '/' &&
  ...
  /*  Find the directory set entry.  */
  directory =
      FindDirectory(file_mgr_data->host, file_mgr_data->current_directory);
  if ((directory != NULL) && (strcmp(file_mgr_data->current_directory,
    ...
    DPRINTF2(("GetDirectoryLogicalType(%s): n %d, type %s\n", path, n,
              directory->path_logical_types[n]));
...
int GetDirectoryPositionInfo(char *host_name, char *directory_name,
                             PositionInfo **position_info) {
  Directory *directory;
...
static int WritePosInfoProcess(int pipe_fd, Directory *directory,
  ...
  fileName =
      ResolveLocalPathName(directory->host_name, directory->directory_name,
                           positionFileName, home_host_name, &tt_status);
  ...
  */
  DPRINTF(
      ("WritePosInfoProcess: count %d, file %s\n", position_count, fileName));
  ...
  if (position_count <= 0)
    ...
    if (f == NULL) {
      ...
      fprintf(f, "%d\n", position_count);
      for (i = 0; i < position_count; i++) {
        fprintf(f, "%s\n%d %d %d\n", position_info[i].name, position_info[i].x,
                position_info[i].y, position_info[i].stacking_order);
      }
...
int SetDirectoryPositionInfo(char *host_name, char *directory_name,
                             int position_count, PositionInfo *position_info) {
  Directory *directory;
  Boolean unchanged;
  int i, j;
  ...
  if (directory->position_count == position_count) {
    ...
    for (i = 0; i < position_count && unchanged; i++) {
      for (j = 0; j < position_count; j++)
        if (strcmp(position_info[i].name, directory->position_info[j].name) ==
            0) {
          break;
        }
...
static Boolean SkipRefresh(Directory *directory) {
  int i;
...
static int TimerEventProcess(int pipe_fd, Directory *directory,
  ...
  if (rc == 0 && activity == activity_checking_links) {
    ...
    /* check all links */
    for (file_data = directory->file_data; file_data && !link_changed;
...
static void StickyProcIdle(ActivityStatus activity, StickyProcDesc *sticky_proc,
                           int max_procs) {
  StickyProcDesc *p, **lp;
  int i, n;
...
static void TimerPipeCallback(XtPointer client_data, int *fd, XtInputId *id) {
  ...
  DPRINTF2((
      ...
      rc, directory->errnum, modify_time, (long)directory->modify_time,
      link_changed));
  ...
  if (directory->busy[activity_reading] || directory->busy[activity_update_all])
    return;
  ...
  if (rc == 0) {
    if (link_changed) {
      DPRINTF(
          ("TimerPipeCallback: %s link changed\n", directory->directory_name));
      ReadDirectoryFiles(NULL, directory);
    } else if (modify_time != directory->modify_time ||
               directory->errnum != 0) {
...
static int CheckDesktopProcess(int pipe_fd, Directory *directory,
  ...
  for (i = 0; i < desktop_data->numIconsUsed; i++) {
    ...
    full_path = ResolveLocalPathName(
        desktopWindow->host, desktopWindow->dir_linked_to,
        desktopWindow->file_name, home_host_name, &tt_status);

    /* Check if the file still exists */
    errno = 0;
    if (lstat(full_path, &stat_buf) < 0) {
      /* the real file no longer exists */
      DPRINTF2(("CheckDesktopProcess: sending PIPEMSG_DESKTOP_REMOVED for %s\n",
                full_path));
      ...
      if (new_data->physical_type != old_data->physical_type ||
        ...
        DPRINTF2(
            ("CheckDesktopProcess: sending PIPEMSG_DESKTOP_CHANGED for %s\n",
             full_path));
        DPRINTF2(("  old type %d %s, new type %d %s\n", old_data->physical_type,
                  old_data->logical_type, new_data->physical_type,
                  new_data->logical_type));
...
static void CheckDesktopPipeCallback(XtPointer client_data, int *fd,
  ...
  if (msg == PIPEMSG_DESKTOP_REMOVED || msg == PIPEMSG_DESKTOP_CHANGED) {
    ...
    DPRINTF2(("CheckDesktopPipeCallback: msg %d: host %s, dir %s, name %s\n",
...
static int CheckListCmp(const void *p1, const void *p2) {
  const int *ip1 = (const int *)p1;
  const int *ip2 = (const int *)p2;
  return directory_set[*ip1]->last_check - directory_set[*ip2]->last_check;
}
...
static void TimerEvent(XtPointer client_data, XtIntervalId *id) {
   ...
   */
  qsort(check_list, n, sizeof(int), CheckListCmp);
...
void TimerEventBrokenLinks(XtPointer client_data, XtIntervalId *id) {
  int i;
...
static void ScheduleDirectoryActivity(Directory *directory) {
  ...
  StickyProcDesc *p;
  int pipe_s2m_fd[2] = {
      -1, -1}; /* for msgs from backgroundnd proc (slave to master) */
  int pipe_m2s_fd[2] = {
      -1, -1}; /* for msgs to backgroundnd proc (master to slave) */
  pid_t pid = 0;
  char *s;
  int rc;
  ...
  if (n_active >= maxDirectoryProcesses || n_checking >= maxRereadProcesses) {
  ...
  if (!p) {
    ...
    if (pid == 0) {
      ...
      for (;;) {
        rc = (*ActivityTable[activity].main)(pipe_s2m_fd[1], directory,
                                             activity);
        if (!sticky || rc != 0)
          break;
    ...
    DBGFORK(("%s:  forked child<%d> for activity %d, s2m %d, m2s %d\n", pname,
             pid, activity, pipe_s2m_fd[0], pipe_m2s_fd[1]));
  ...
  XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_s2m_fd[0],
                (XtPointer)XtInputReadMask, ActivityTable[activity].callback,
                (XtPointer)pipe_data);
}
...
static void SelectDesktopFile(FileMgrData *file_mgr_data) {
