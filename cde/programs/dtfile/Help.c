...
static DialogResource resources[] = {
    {"idString", XmRString, sizeof(String), XtOffset(HelpDataPtr, idString),
     (caddr_t)NULL, _DtStringToString},

    {"volString", XmRString, sizeof(String), XtOffset(HelpDataPtr, volString),
     (caddr_t)NULL, _DtStringToString},

    {"topicTitle", XmRString, sizeof(String), XtOffset(HelpDataPtr, topicTitle),
     (caddr_t)NULL, _DtStringToString},

    {"fileType", XmRString, sizeof(String), XtOffset(HelpDataPtr, fileType),
     (caddr_t)NULL, _DtStringToString},

    {"helpType", XmRInt, sizeof(int), XtOffset(HelpDataPtr, helpType),
...
static void Create(Display *display, Widget parent, Widget *return_widget,
                   XtPointer *dialog);
static void InstallClose(HelpRec *helpRec, XtCallbackProc callback,
                         XtPointer clientData);
static XtPointer GetValues(HelpRec *helpRec);
static XtPointer GetDefaultValues(void);
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list);
static void SetValues(HelpRec *helpRec, HelpData *helpData);
static void WriteResourceValues(DialogData *values, int fd, char **name_list);
static void FreeValues(HelpData *helpData);
...
{
  XtAddCallback(helpRec->helpDialog, DtNcloseCallback, callback, clientData);
}
...

{
  ...
  n = 0;
  XtSetArg(args[n], XmNx, &helpData->x);
  n++;
  XtSetArg(args[n], XmNy, &helpData->y);
  n++;
  XtSetArg(args[n], XmNwidth, &helpData->width);
  n++;
  XtSetArg(args[n], XmNheight, &helpData->height);
  n++;
...

{
  ...
  n = 0;
  XtSetArg(args[n], DtNhelpVolume, helpData->volString);
  n++;
  XtSetArg(args[n], DtNhelpType, (unsigned char)helpData->helpType);
  n++;
  if (helpData->helpType == DtHELP_TYPE_TOPIC) {
    XtSetArg(args[n], DtNlocationId, helpData->idString);
    n++;
  } else {
    helpString =
        MapFileTypeToHelpString(helpData->idString, helpData->fileType);
    XtSetArg(args[n], DtNstringData, helpString);
    n++;
    XtSetArg(args[n], DtNtopicTitle, helpData->topicTitle);
    n++;
  }
...

{
  ...
  if (helpData->displayed == True) {
    ...
    n = 0;
    XtSetArg(args[n], DtNhelpVolume, &volString);
    n++;
    XtSetArg(args[n], DtNlocationId, &idString);
    n++;
    XtSetArg(args[n], DtNtopicTitle, &topicTitle);
    n++;
    XtSetArg(args[n], DtNstringData, &stringData);
    n++;
    XtSetArg(args[n], DtNhelpType, &helpType);
    n++;
    XtGetValues(helpRec->helpDialog, args, n);
    ...
    _DtDialogPutResources(fd, name_list, HELP_DIALOG, values->data, resources,
                          helpClass->resource_count);
  }
}
...
XtPointer MapFileTypeToHelpString(String filetype, String miscData)

{
  ...
  if (strcmp(filetype, openNewView) == 0) {
    tmpStr = (GETMESSAGE(29, 7,
                         "The `%s` action opens a new File Manager view "
  ...
  if (strcmp(filetype, openInPlace) == 0) {
    tmpStr = (GETMESSAGE(29, 8,
                         "The `%s` action displays the requested folder "
  ...
  desc = DtDtsDataTypeToAttributeValue(filetype, DtDTS_DA_DESCRIPTION, NULL);
  ...
  if (isAction) {
    tmpStr =
        (GETMESSAGE(29, 5, "There is no help available for the %s action"));
  ...
  label = DtDtsDataTypeToAttributeValue(filetype, DtDTS_DA_LABEL, NULL);
  if (label) {
