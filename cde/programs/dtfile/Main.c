...
extern Tt_status InitializeToolTalkProcid(int *ttFd, Widget topLevel,
                                          Boolean sendStarted);
...
static void RestrictModeUsage(char **argv);
static void Stop(int);
static void RestoreSettingsFile(void);
static void MoveDefaultSettings(int mode);
static void SaveDefaultCancelCB(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void SaveDefaultOkCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void SaveSession(char *path);
static int RestoreSession(char *path, int type_of_restore, char *directory);
static void GetPWD(char current_directory[]);
static void OpenDirectories(char *directory_set, char *type);
static Tt_callback_action ObserveTtNotice(Tt_message msg, Tt_pattern pat);
static void ViewSessionHandler(Tt_message msg);
static void ViewDirectoryHandler(Tt_message msg);
static void ViewHomeDirectoryHandler(Tt_message msg);
static void ViewToolsDirectoryHandler(Tt_message msg);

static void ReloadDatabases(void);
static void ViewAccept(View *view, Tt_message msg);
static void LoadViews(int num_views, XrmDatabase db, char *host_name,
                      char *directory_name, char *type, Tt_message msg);
static void RemoveTextFields(XtPointer client_data, DialogData *old_dialog_data,
                             DialogData *new_dialog_data);
static void DtfileCvtStringToObjPlace(XrmValue *args, Cardinal numArgs,
                                      XrmValue *fromVal, XrmValue *toVal);
static unsigned char *_DtNextToken(unsigned char *pchIn, int *pLen,
                                   unsigned char **ppchNext);
static void DtfileCvtStringToOpenDir(XrmValue *args, Cardinal numArgs,
                                     XrmValue *fromVal, XrmValue *toVal);
static void DtfileCvtStringToDTIcon(XrmValue *args, Cardinal numArgs,
                                    XrmValue *fromVal, XrmValue *toVal);
static void DtfileCvtStringToTree(XrmValue *args, Cardinal numArgs,
                                  XrmValue *fromVal, XrmValue *toVal);
static void DtfileCvtStringToTreeFiles(XrmValue *args, Cardinal numArgs,
                                       XrmValue *fromVal, XrmValue *toVal);
static void DtfileCvtStringToView(XrmValue *args, Cardinal numArgs,
                                  XrmValue *fromVal, XrmValue *toVal);
static void DtfileCvtStringToOrder(XrmValue *args, Cardinal numArgs,
                                   XrmValue *fromVal, XrmValue *toVal);
static void DtfileCvtStringToDirection(XrmValue *args, Cardinal numArgs,
                                       XrmValue *fromVal, XrmValue *toVal);
static void DtfileCvtStringToGrid(XrmValue *args, Cardinal numArgs,
                                  XrmValue *fromVal, XrmValue *toVal);
static void DtfileStringToTree(char *str, int *type);
static void DtfileStringToTreeFiles(char *str, int *type);
static void DtfileStringToView(char *str, int *type);
static void DtfileStringToOrder(char *str, int *type);
static void DtfileStringToDirection(char *str, int *type);
static void DtfileStringToGrid(char *str, int *type);
static void SetupSendRequestArgs(ApplicationArgs application_args,
                                 Tt_message msg);
static void BuildAndShowIconName(char *file_type_name, unsigned char view,
                                 unsigned char show_type, Widget widget);
static void MarqueeSelect(Widget w, int type, Position x, Position y,
                          Dimension width, Dimension height,
                          XtPointer client_data);
static void WaitForResponseAndExit(void);

static Widget post_dialog(Widget parent, char *title, char *msg,
                          XtCallbackProc callback);
static void DtErrExitCB(Widget widget, XtPointer client_data,
                        XtPointer call_data);
static Tt_callback_action ExitAppTT(Tt_message msg, Tt_pattern p);
...
unsigned int currentMenuStates =
    (RENAME | MOVE | DUPLICATE | LINK | TRASH | MODIFY | CHANGEDIR |
     PREFERENCES | FILTER | FIND | CREATE_DIR | CREATE_FILE | PUT_ON_DESKTOP |
     PUTBACK | CLEAN_UP | MOVE_UP | HOME | CHANGE_DIR | TERMINAL);
...
static XtResource resources[] = {
    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, no_view),
        XmRImmediate,
        (XtPointer)NULL,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, session),
        XmRImmediate,
        (XtPointer)NULL,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, directories),
        XmRImmediate,
        (XtPointer)NULL,
    },

    {
        ...
        "Tree",
        sizeof(int),
        XtOffset(ApplicationArgsPtr, tree_view),
        XmRImmediate,
...

    {
        ...
        "TreeFiles",
        sizeof(int),
        XtOffset(ApplicationArgsPtr, tree_files),
        XmRImmediate,
        (XtPointer)UNSET_VALUE,
    },

    {
        ...
        "View",
        sizeof(int),
        XtOffset(ApplicationArgsPtr, view),
        XmRImmediate,
        (XtPointer)UNSET_VALUE,
    },

    {
        ...
        "Order",
        sizeof(int),
        XtOffset(ApplicationArgsPtr, order),
        XmRImmediate,
        (XtPointer)UNSET_VALUE,
    },

    {
        ...
        "Direction",
        sizeof(int),
        XtOffset(ApplicationArgsPtr, direction),
        XmRImmediate,
        (XtPointer)UNSET_VALUE,
    },

    {
        ...
        "Grid",
        sizeof(int),
        XtOffset(ApplicationArgsPtr, grid),
        XmRImmediate,
        (XtPointer)UNSET_VALUE,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, instanceIconWidth),
        XmRImmediate,
        (XtPointer)256,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, instanceIconHeight),
        XmRImmediate,
        (XtPointer)256,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, restricted),
        XmRImmediate,
        (XtPointer)NULL,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, title),
        XmRImmediate,
        (XtPointer)NULL,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, help_volume),
        XmRImmediate,
        (XtPointer)NULL,
    },

    {
        ...
        XmRFontList,
        sizeof(XmFontList),
        XtOffset(ApplicationArgsPtr, user_font),
        XmRString,
        (XtPointer) "Fixed",
    },

    {
        ...
        sizeof(Dimension),
        XtOffset(ApplicationArgsPtr, tool_width),
        XmRImmediate,
        (XtPointer)700,
    },

    {
        ...
        sizeof(Dimension),
        XtOffset(ApplicationArgsPtr, tool_height),
        XmRImmediate,
        (XtPointer)250,
    },

    {
        ...
        XmRHorizontalDimension,
        sizeof(Dimension),
        XtOffset(ApplicationArgsPtr, dir_width),
        XmRImmediate,
        (XtPointer)555,
    },

    {
        ...
        XmRVerticalDimension,
        sizeof(Dimension),
        XtOffset(ApplicationArgsPtr, dir_height),
        XmRImmediate,
        (XtPointer)400,
    },

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, prompt_user),
        XmRImmediate,
        (XtPointer)True,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, root_title),
        XmRImmediate,
        (XtPointer) "ROOT",
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, dragThreshold),
        XmRImmediate,
        (XtPointer)4,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, rereadTime),
        XmRImmediate,
        (XtPointer)3,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, checkBrokenLink),
        XmRImmediate,
        (XtPointer)180,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, maxDirectoryProcesses),
        XmRImmediate,
        (XtPointer)10,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, maxRereadProcesses),
        XmRImmediate,
        (XtPointer)5,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, maxRereadProcsPerTick),
        XmRImmediate,
        (XtPointer)1,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, trashWait),
        XmRImmediate,
        (XtPointer)1,
    },

    {
        ...
        "DesktopIcon",
        sizeof(int),
        XtOffset(ApplicationArgsPtr, desktopIconType),
        XmRImmediate,
        (XtPointer)LARGE,
    },

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, showFilesystem),
        XmRImmediate,
        (XtPointer)True,
    },

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, showDropZone),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, showEmptySet),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, showEmptyMsg),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        "OpenFolder",
        sizeof(int),
        XtOffset(ApplicationArgsPtr, openDirType),
        XmRImmediate,
        (XtPointer)CURRENT,
    },

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, restrictMode),
        XmRImmediate,
        (XtPointer)False,
    },

    {
        ...
        "ObjectPlacement",
        sizeof(int),
        XtOffset(ApplicationArgsPtr, desktopPlacement),
        XmRImmediate,
...

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, freezeOnConfig),
        XmRImmediate,
        (XtPointer)True,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, fileMgrIcon),
        XmRImmediate,
...

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, appMgrIcon),
        XmRImmediate,
...

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, trashIcon),
        XmRImmediate,
...

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, retryLoadDesktop),
        XmRImmediate,
        (XtPointer)10,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, smallIconWidth),
        XmRImmediate,
        (XtPointer)24,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, smallIconHeight),
        XmRImmediate,
        (XtPointer)24,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, largeIconWidth),
        XmRImmediate,
        (XtPointer)38,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, largeIconHeight),
        XmRImmediate,
        (XtPointer)38,
    },

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, emptyTrashOnExit),
        XmRImmediate,
        (XtPointer)False,
    },
...
#if defined(sun)
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, follow_links),
        XmRImmediate,
        (XtPointer)False,
    },
#endif
...
int main(int argc, char **argv) {
...
#ifdef HAVE_EDITRES
  XtAddEventHandler(toplevel, 0, True, (XtEventHandler)_XEditResCheckMessages,
                    (XtPointer)NULL);
#endif
  ...
  if (DtInitialize(display, toplevel, argv[0], FILE_MANAGER_TOOL_CLASS) ==
      False) {
  ...
  XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                    "ObjectPlacement", (XtConverter)DtfileCvtStringToObjPlace,
                    NULL, 0);
  XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                    "OpenFolder", (XtConverter)DtfileCvtStringToOpenDir, NULL,
                    0);
  XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                    "DesktopIcon", (XtConverter)DtfileCvtStringToDTIcon, NULL,
                    0);
  XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString, "Tree",
                    (XtConverter)DtfileCvtStringToTree, NULL, 0);
  XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                    "TreeFiles", (XtConverter)DtfileCvtStringToTreeFiles, NULL,
                    0);
  XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString, "View",
                    (XtConverter)DtfileCvtStringToView, NULL, 0);
  XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString, "Order",
                    (XtConverter)DtfileCvtStringToOrder, NULL, 0);
  XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                    "Direction", (XtConverter)DtfileCvtStringToDirection, NULL,
                    0);
  XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString, "Grid",
  ...
  XtGetApplicationResources(toplevel, &application_args, resources,
  ...
  if (XrmGetResource(db, "dttypes.defaultActions", "Dttypes.DefaultActions",
                     &rep_type, &resource_value)) {
  ...
  if (_DtGetLock(display, DTFILE_CLASS_NAME) == 0) {
    ...
    if (TT_OK != status) {
      ...
      errfmt =
          GETMESSAGE(18, 40, "Could not connect to ToolTalk:\n%s\nExiting ...");
    ...
    if (application_args.session != NULL) {
      msg = tttk_message_create(0, TT_REQUEST, TT_SESSION, 0,
                                "DtFileSession_Run", ExitAppTT);
      tt_message_file_set(msg, application_args.session);
      tt_message_send(msg);
    } else if (application_args.directories != NULL) {
      msg = tttk_message_create(0, TT_REQUEST, TT_SESSION, 0, "DtFolder_Show",
                                ExitAppTT);
      ...
      if (current_directory[0] != '\0') {
        ...
        msg = tttk_message_create(0, TT_REQUEST, TT_SESSION, 0, "DtHome_Show",
                                  ExitAppTT);
      }
      SetupSendRequestArgs(application_args, msg);
      tt_message_send(msg);
  ...
  _DtWsmAddMarqueeSelectionCallback(
      toplevel, (DtWsmMarqueeSelectionProc)MarqueeSelect, NULL);
  ...
  if (dt_path == NULL) {
    ...
    tmpStr =
        GETMESSAGE(18, 2, "Could not create the ~/.dt folder or sub folders.");
  ...
  */
  ...
  if ((strcmp(application_args.no_view, "-noview") != 0) &&
      ((application_args.session == NULL) || (session_flag != 0))) {
    if (view_count == 0) {
  ...
  printf("    Finish, time: %ld.%ld\n\n",
         update_time_fs.tv_sec - update_time_ss.tv_sec,
         update_time_fs.tv_usec - update_time_ss.tv_usec);
#endif
...
static void Usage(char **argv) {
  ...
  char *message_string2 =
      "\toff    = Files are displayed in folder tree mode.\n\n"
      ...
      "        The default display area size for large is 38 and small is "
      "24\n\n"
      "\n";
...
static void RestoreSettingsFile(void) {
  ...
  XGetWindowProperty(display, RootWindow(display, 0), save_mode, 0L,
                     (long)BUFSIZ, False, XA_STRING, &actualType, &actualFormat,
  ...
  toolSavePath = (char *)XtMalloc(
      strlen(dtPath) + strlen(TOOL_SETTINGS_FILENAME) + strlen(dirName) + 4);

  homeSavePath = (char *)XtMalloc(
      strlen(dtPath) + strlen(HOME_SETTINGS_FILENAME) + strlen(dirName) + 4);
  ...
  if ((fd = open(homeSavePath, O_RDONLY)) != -1) {
    ...
    homeHomePath =
        (char *)XtMalloc(strlen(dtPath) + strlen(HOME_SETTINGS_FILENAME) + 2);
    ...
    if (status != 0) {
      ...
      tmpStr = GETMESSAGE(18, 27,
                          "Unable to recover the saved default home settings "
                          "file, will use default.\n");
      msg = XtNewString(tmpStr);
  ...
  if ((fd = open(toolSavePath, O_RDONLY)) != -1) {
    ...
    /* create where the saved file is going to go */
    toolHomePath =
        (char *)XtMalloc(strlen(dtPath) + strlen(TOOL_SETTINGS_FILENAME) + 2);
    ...
    if (status != 0) {
      ...
      tmpStr = GETMESSAGE(18, 28,
                          "Unable to recover the saved default tool settings "
                          "file, will use default.\n");
      msg = XtNewString(tmpStr);
      _DtSimpleErrnoError(application_name, DtWarning, NULL, msg, NULL);
      XtFree(msg);
...
static void MoveDefaultSettings(int mode) {
  ...
  toolMovePath =
      (char *)XtMalloc(strlen(dtPath) + strlen(TOOL_SETTINGS_FILENAME) + 3);

  homeMovePath =
      (char *)XtMalloc(strlen(dtPath) + strlen(HOME_SETTINGS_FILENAME) + 3);
 ...
 *
 ************************************************************************/
void SaveSettingsCB(Widget w, XtPointer client_data, XtPointer call_data) {
  Widget dlog;
  Widget mbar;
  Arg args[1];
  char *message;
  ...
  if ((file_mgr_rec->menuStates & SETTINGS) == 0) {
    XSetInputFocus(XtDisplay(w), XtWindow(file_mgr_rec->defaultEnvBtn_child),
  ...
  if (file_mgr_data->restoreKind == HOME_RESTORE) {
    tmpStr = (GETMESSAGE(
        18, 29,
        "Save the current File Manager View window size,\npreferences, and "
        "filter settings?\n\nThey will be used for any new File Manager View "
        "windows\nstarted from the Front Panel."));
  } else {
    tmpStr = (GETMESSAGE(
        18, 35,
        "Save the current Application Manager\nView window size, preferences, "
        "and filter settings?\n\nThey will be used for any new Application "
  ...
  if (file_mgr_data->title != NULL &&
    ...
    title = (char *)XtMalloc(strlen(tmpStr) + strlen(file_mgr_data->title) + 5);
...
static void SaveDefaultOkCB(Widget w, XtPointer client_data,
  ...
  if ((fd = creat(full_path, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP)) == -1) {
    char *title;
    tmpStr = GETMESSAGE(
        18, 17, "Unable to create a file to store the default options.");
...
void SaveSessionCallback(Widget w, XtPointer client_data, XtPointer call_data) {
  char *full_path = NULL;
  char *file_name = NULL;
  char *strPtr;
  ...
  if (view_count == 0 && desktop_data->numIconsUsed == 0) {
    XChangeProperty(display, XtWindow(toplevel), command_atom, XA_STRING, 8,
...
static void SaveSession(char *path) {
  static char *name_list[] = {DTFILE_CLASS_NAME, NULL, NULL, NULL, NULL, NULL};
  char view_number[12];
  char number[5];
  ...
  for (i = -1; i < view_count; i++) {
    ...
    */
    ...
    XGetWindowProperty(display, XtWindow(file_mgr_rec->shell), wm_state_atom,
                       0L, (long)BUFSIZ, False, wm_state_atom, &actual_type,
    ...
    if (i == -1)
      ...
      _DtWriteDialogData((DialogData *)view_set[i]->dialog_data, fd, name_list);
    (void)write(fd, "#\n", strlen("#\n"));
  ...
  for (i = 0; i < desktop_data->numWorkspaces; i++) {
    ...
    for (j = 0; j < workspaceInfo->secondaryHelpDialogCount; j++) {
      ...
      _DtWriteDialogData(workspaceInfo->secondaryHelpDialogList[j], fd,
                         name_list);
    }
    ...
    if (workspaceInfo->primaryHelpDialog) {
      ...
      _DtWriteDialogData(workspaceInfo->primaryHelpDialog, fd, name_list);
    }
    write(fd, "#\n", strlen("#\n"));
  }
  (void)close(fd);
...
void ForceMyIconOpen(char *host_name, char *directory_name) {
  ...
  if (directory_name == NULL) {
    for (i = 0; i < view_count; i++) {
      ...
      for (j = 0; j < fmd->directory_count; j++) {
        ForceMyIconOpen(view_set[i]->host_name, fmd->directory_set[j]->name);
      }
    }
    return;
  }
  ...
  for (i = 0; i < view_count; i++) {
    ...
    if (file_view_data) {
      ...
      if (fmd->view != BY_NAME)
        BuildAndShowIconName(file_type_name, fmd->view, fmd->show_type,
  ...
  for (i = 0; i < desktop_data->numIconsUsed; i++) {
    ...
    if (strcmp(buf, directory_name) == 0 &&
      ...
      file_type_name = file_view_data->file_data->logical_type;
      if (desktopIconType == LARGE)
        BuildAndShowIconName(file_type_name, BY_NAME_AND_ICON, SINGLE_DIRECTORY,
...
static int RestoreSession(char *path, int type_of_restore, char *directory) {
  ...
  if (type_of_restore == NORMAL_RESTORE) {
    ...
    if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
      if ((temp = (char *)value.addr) != NULL && strcmp(temp, "True") == 0) {
        showFilesystem = True;
      } else
    ...
    xrm_name[2] = 0;
    if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
      if ((temp = (char *)value.addr) != NULL && strcmp(temp, "NEW") == 0) {
        openDirType = NEW;
      } else
        openDirType = CURRENT;
    } else
  ...
  if (type_of_restore == NORMAL_RESTORE) {
    for (i = 0; i < desktop_data->numWorkspaces; i++) {
        ...
        if (num_sec_help_dialogs > 0) {
          ...
          for (j = 0; j < num_sec_help_dialogs; j++) {
            ...
            desktop_data->workspaceData[i]->secondaryHelpDialogList[j] =
                dialogData;
            ShowDTHelpDialog(NULL, i, HYPER_HELP_DIALOG, dialogData, NULL, NULL,
                             NULL, NULL, DtHELP_TYPE_TOPIC);
          }
        }
      }
      ...
      if (dialogData) {
        ShowDTHelpDialog(NULL, i, MAIN_HELP_DIALOG, dialogData, NULL,
                         DTFILE_HELP_NAME, NULL, NULL, 0);
      }
    }
  }
...
static void ViewAccept(View *view, Tt_message msg) {
  extern Tt_message FileCallback(Tt_message msg, Tttk_op op, char *file,
                                 void *clientdata, int trust, int self);
  extern Tt_message SessionCallback(Tt_message msg, void *client_data,
                                    Tt_message contract);
   ...
   */
  view->pats =
      ttdt_file_join(view->directory_name, TT_SESSION, 0, FileCallback, view);
  ...
  ttdt_message_accept(msg, SessionCallback,
                      _DtGetDialogShell((DialogData *)view->dialog_data), view,
                      1, 1);
}
...

{
  ...
  for (i = 0; i < num_views; i++) {
    ...
    if (trashFileMgrData && (file_mgr_data->IsTrashCan == True) &&
    ...
    if (view_count == view_set_size) {
      view_set_size += 10;
      view_set =
          (View **)XtRealloc((char *)view_set, sizeof(View **) * view_set_size);
    }
    ...
    if (directory_name == NULL) {
      ...
      if (host_name == NULL) {
        ...
        if (type == NULL)
          ...
          file_mgr_data->restricted_directory = XtNewString(type);
      } else {
        ...
        if (special_view && special_restricted != NULL)
          file_mgr_data->restricted_directory = XtNewString(special_restricted);
        else
          file_mgr_data->restricted_directory = NULL;
      }
      FileMgrBuildDirectories(file_mgr_data, view_set[view_count]->host_name,
                              directory_name);
    }
    ...
    _DtShowDialog(NULL, NULL, NULL, dialog_data, NULL, NULL, RemoveTextFields,
                  NULL, workspaces, iconify_window, special_view, title,
                  &classHints);
 ...
 *
 ************************************************************************/
DialogData *GetNewView(char *host_name, char *directory_name, char *type,
  ...
  if (stat(directory_name, &stat_buf) == 0) {
    if ((stat_buf.st_mode & S_IFMT) != S_IFDIR) {
      tmpStr =
          GETMESSAGE(18, 19, "The folder specification,\n%s\nis not a folder.");
      errMsg = XtNewString(tmpStr);
    }
  } else {
    tmpStr =
        GETMESSAGE(18, 20, "The folder specification,\n%s\ndoes not exist.");
    errMsg = XtNewString(tmpStr);
  }
  ...
  if (errMsg) {
    ...
    tmpStr = GETMESSAGE(32, 2, "File Manager Open Directory Error");
    title = XtNewString(tmpStr);
    dmsg = XtMalloc(strlen(errMsg) + strlen(directory_name) + 1);
        ...
        if (DtWsmGetCurrentWorkspace(display, rootWindow, &pCurrent) ==
          ...
          if (DtWsmGetWorkspacesOccupied(display, XtWindow(file_mgr_rec->shell),
            ...
            if (k >= num_workspaces) {
              ...
              ws_presence = (Atom *)XtRealloc(
                  (char *)ws_presence, sizeof(Atom) * (num_workspaces + 1));

              ws_presence[num_workspaces] = pCurrent;
              DtWsmSetWorkspacesOccupied(display, XtWindow(file_mgr_rec->shell),
  ...
  if (file_mgr_data->find != NULL && file_mgr_data->current_directory != NULL) {
    ...
    if (file_mgr_data->restricted_directory == NULL) {
      if (restrictMode)
        ...
        find_data->directories = XtNewString(file_mgr_data->current_directory);
    } else {
      if (strcmp(file_mgr_data->current_directory,
                 file_mgr_data->restricted_directory) == 0)
        find_data->directories = XtNewString("/");
...
void CloseView(DialogData *dialog_data) {
  int i;
  int j;
  FileMgrData *file_mgr_data;
  ...
  for (i = 0; i < view_count; i++) {
    if (dialog_data == (DialogData *)(view_set[i]->dialog_data)) {
      ...
      directory_name =
          (char *)XtMalloc(strlen(view_set[i]->directory_name) + 1);
...
void DirectoryChanged(XtPointer file_mgr_data, char *old_host_name,
...
Tt_callback_action HandleTtRequest(Tt_message msg, Tt_pattern pat) {
  char *op;
  Tt_status status;
...
static void ViewSessionHandler(Tt_message msg) {
  ...
  if (!tt_is_err(tt_ptr_error(file))) {
    if (view_count == 0 && desktop_data->numIconsUsed == 0) {
      ...
      tmpStr =
          GETMESSAGE(18, 39,
                     "Cannot start a second File Manager session because a "
                     "File Manager\nsession is already open on the desktop.  "
                     "If you want to start a new File\nManager session, you "
                     "must first close all current File Manager windows\nand "
                     "remove all File Manager objects from the workspaces.");
      msg = XtNewString(tmpStr);
...
DialogData *ViewDirectoryProc(char *root_dir, int restricted, Tt_message msg) {
  DialogData *return_data;

  if (restrictMode) {
    if (root_dir && strncmp(root_dir, "~", 1) != 0) {
      if ((strcmp(users_home_dir, "/") != 0) &&
          (strncmp(root_dir, users_home_dir, strlen(users_home_dir) - 1) !=
           0)) {
        char *tmpStr, *errTitle, *errMsg, *dmsg;
...
static void ExitHandler(Tt_message msg, XtPointer clientData,
                        String *messageFields, int numFields) {
  tt_message_reply(msg);
  tttk_message_destroy(msg);
  FinalizeToolTalkSession();
  exit(0);
...
FileMgrData *ReturnDesktopPtr(Widget w) {
  int i;
  DialogData *dialog_data;
  FileMgrData *file_mgr_data;
...
PixmapData *CheckForOpenDirectory(FileViewData *order_list,
  ...
  if (strcmp(directory_set->name, "/") != 0)
    sprintf(directory_name, "%s/%s", directory_set->name,
            order_list->file_data->file_name);
  else
    sprintf(directory_name, "%s%s", directory_set->name,
            order_list->file_data->file_name);
  ...
  for (i = 0; i < view_count; i++) {
    if (strcmp(real_dir_name, view_set[i]->directory_name) == 0) {
      ...
      pixmapData = _DtRetrievePixmapData(new_file_type_name, NULL, NULL,
                                         file_mgr_rec->file_window, icon_size);
  ...
  if (pixmapData == NULL || pixmapData->iconFileName == NULL) {
    ...
    pixmapData = _DtRetrievePixmapData(logical_type, NULL, NULL,
                                       file_mgr_rec->file_window, icon_size);
  }
...
void CleanUp(Widget w, XtPointer client_data, XtPointer call_data) {
  Widget mbar;
  Arg args[2];
  FileMgrRec *file_mgr_rec;
  DialogData *dialog_data;
    ...
    case 'b':
      if (_DtStringsAreEquivalent((char *)pch, (char *)OBJ_PLACE_BOTTOM_STR)) {
        if (!fPrimarySet) {
          ...
          if (!(cval & (OBJ_PLACE_BOTTOM_PRIMARY | OBJ_PLACE_TOP_PRIMARY))) {
    ...
    case 'r':
      if (_DtStringsAreEquivalent((char *)pch, (char *)OBJ_PLACE_RIGHT_STR)) {
        if (!fPrimarySet) {
          ...
          if (!(cval & (OBJ_PLACE_RIGHT_PRIMARY | OBJ_PLACE_LEFT_PRIMARY))) {
    ...
    case 't':
      if (_DtStringsAreEquivalent((char *)pch, (char *)OBJ_PLACE_TOP_STR)) {
        if (!fPrimarySet) {
          ...
          if (!(cval & (OBJ_PLACE_TOP_PRIMARY | OBJ_PLACE_BOTTOM_PRIMARY))) {
...
FileMgrData *CheckOpenDir(char *directory, char *host) {
  int i;
  FileMgrData *file_mgr_data;
  DialogData *dialog_data;
...
char *GetRestrictedDirectory(Widget widget) {
  int i;
  FileMgrData *file_mgr_data;
  FileMgrRec *file_mgr_rec;
  DialogData *dialog_data;
...
static void BuildAndShowIconName(char *file_type_name, unsigned char view,
  ...
  new_file_type_name =
      (char *)XtMalloc(strlen(file_type_name) + strlen(ICON_OPEN_PREFIX) + 1);
  sprintf(new_file_type_name, "%s%s", ICON_OPEN_PREFIX, file_type_name);

  if (view == BY_NAME_AND_ICON && show_type != MULTIPLE_DIRECTORY)
    pixmapData =
        _DtRetrievePixmapData(new_file_type_name, NULL, NULL, widget, LARGE);
  else
    pixmapData =
        _DtRetrievePixmapData(new_file_type_name, NULL, NULL, widget, SMALL);
  ...
  */
  ...
  _DtCheckAndFreePixmapData(new_file_type_name, widget, (DtIconGadget)widget,
                            pixmapData);
...
void ForceMyIconClosed(char *host_name, char *directory_name) {
  int i;
  int j;
  int k;
  Arg args[1];
    ...
    for (j = 0; j < file_mgr_data->directory_count; j++) {
      if (strcmp(parent, file_mgr_data->directory_set[j]->name) == 0) {
        for (k = 0; k < file_mgr_data->directory_set[j]->file_count; k++) {
          file_view_data = file_mgr_data->directory_set[j]->file_view_data[k];
          if (strcmp(file_view_data->file_data->file_name, fname) == 0)
            goto search_done;
        }
        break;
...
search_done:
  ...
  if ((file_view_data) && (file_mgr_data->view != BY_NAME)) {
    if (file_mgr_data->view == BY_NAME_AND_ICON &&
      ...
      pixmapData =
          _DtRetrievePixmapData(file_view_data->file_data->logical_type, fname,
                                parent, file_view_data->widget, LARGE);
    else
      pixmapData =
          _DtRetrievePixmapData(file_view_data->file_data->logical_type, fname,
                                parent, file_view_data->widget, SMALL);
    ...
    _DtCheckAndFreePixmapData(file_view_data->file_data->logical_type,
                              file_view_data->widget,
                              (DtIconGadget)file_view_data->widget, pixmapData);
  }
  ...
  for (j = 0; j < desktop_data->numIconsUsed; j++) {
    ...
    if ((strcmp(buf, directory_name) == 0) &&
      ...
      pixmapData =
          _DtRetrievePixmapData(file_view_data->file_data->logical_type, fname,
                                NULL, desktopWindow->shell, desktopIconType);
      ...
      _DtCheckAndFreePixmapData(
          ...
          desktopWindow->shell, (DtIconGadget)desktopWindow->iconGadget,
          pixmapData);
    }
  }
}
...
static Widget post_dialog(Widget parent, char *title, char *msg,
  ...
  n++;
  XtSetArg(args[n], XmNmessageString, message_text);
  n++;
  ...
  XtSetArg(args[0], XmNx, (WidthOfScreen(XtScreen(dialog)) - dialogWd) / 2);
  XtSetArg(args[1], XmNy, (HeightOfScreen(XtScreen(dialog)) - dialogHt) / 2);
...
static void DtErrExitCB(Widget dialog, XtPointer client_data,
                        XtPointer call_data) {
