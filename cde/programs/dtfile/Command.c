...
static void ActionCallback(Widget w, XtPointer client_data,
                           XtPointer call_data);
static void TimerEvent(Widget widget, XtIntervalId *id);
void ProcessBufferDropOnFolder(char *command, FileMgrData *file_mgr_data,
...
void UpdateActionMenuPane(XtPointer client_data, FileMgrRec *file_mgr_rec,
                          char *file_type, int type, int number, Widget widget,
  ...
  */
  ...
  if (action_count + menu_offset > num_children) {
    ...
    for (i = num_children; i < action_count + menu_offset; i++) {
      child = XmCreatePushButtonGadget((Widget)action_pane, "action_button",
                                       args, 0);
      if (type == DESKTOP) {
        XtAddCallback(child, XmNactivateCallback, DTActionCallback,
                      (XtPointer)desktopWindow);
        XtAddCallback(child, XmNhelpCallback, (XtCallbackProc)DTHelpRequestCB,
                      NULL);
      } else if (type == FM_POPUP) {
        XtAddCallback(child, XmNactivateCallback, ActionCallback, NULL);
        XtAddCallback(child, XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
                      NULL);
      } else {
    ...
    for (i = menu_offset; i < num_children; i++) {
      if (i < action_count + menu_offset) {
        XtRemoveAllCallbacks(action_pane->composite.children[i],
                             XmNactivateCallback);
        if (type == DESKTOP)
          XtAddCallback(action_pane->composite.children[i], XmNactivateCallback,
                        DTActionCallback, (XtPointer)desktopWindow);
        else if (type == FM_POPUP)
          XtAddCallback(action_pane->composite.children[i], XmNactivateCallback,
                        ActionCallback, NULL);
        else
          XtAddCallback(action_pane->composite.children[i], XmNactivateCallback,
                        ActionCallback, (XtPointer)file_mgr_rec);
        XtManageChild(action_pane->composite.children[i]);
      } else
  ...
  for (i = 0; i < action_count; i++) {
      ...
      if (physical_type == DtDIRECTORY && i == 0) {
        ...
        if (physical_type == DtDIRECTORY) {
          if (strcmp(command_list[i - 1], openNewView) == 0 ||
            ...
            del_count++;
            XtUnmanageChild(
                action_pane->composite.children[num_children - del_count]);
            continue;
          }
          ...
          XtSetArg(args[1], XmNuserData, XtNewString(command_list[i - 1]));
    ...
    if (type != NOT_DESKTOP) {
      ...
      if (type == DESKTOP) {
        ...
        XtAddCallback(action_pane->composite.children[count],
                      XmNactivateCallback, ActionCallback, (XtPointer)NULL);
    }
    XmStringFree(string);
    XtFree(action_label);
  }
...
static void ActionCallback(Widget w, XtPointer client_data,
  ...
  if (strcmp(command, openNewView) == 0) {
    ...
    if (popup)
      ...
      RunCommand(command, file_mgr_data, file_mgr_data->selection_list[0], NULL,
                 NULL, NULL);
...
void RunCommand(char *command, FileMgrData *file_mgr_data,
                FileViewData *file_view_data, WindowPosition *position,
                DtDndDropCallbackStruct *drop_parameters, Widget drop_window)

{
  if ((strcmp(command, openInPlace) == 0) ||
    ...
    if (((strcmp(file_view_data->file_data->logical_type, LT_AGROUP) == 0) &&
      ...
      ProcessAction(command, file_view_data, drop_parameters,
    ...
    if (drop_parameters->dropData->protocol == DtDND_FILENAME_TRANSFER)
      ProcessMoveCopyLink(command, file_mgr_data, file_view_data,
                          drop_parameters, drop_window);
    else if (drop_parameters->dropData->protocol == DtDND_BUFFER_TRANSFER)
    ...
    ProcessAction(command, file_view_data, drop_parameters, file_mgr_data->host,
...
void ProcessNewView(char *command, FileMgrData *file_mgr_data,
                    FileViewData *file_view_data, WindowPosition *position) {
  ...
  if (trashFileMgrData != NULL && file_mgr_data == trashFileMgrData) {
    tmpStr = GETMESSAGE(27, 3, "Trash Can Error");
    title = XtNewString(tmpStr);
    tmpStr = GETMESSAGE(
        27, 87,
        "Object in the Trash cannot be opened.\n\nTo open an object use 'Put "
        "Back' to return it to the\nFile Manager then open it there.");
    msg = XtNewString(tmpStr);

    _DtMessage(((FileMgrRec *)file_mgr_data->file_mgr_rec)->file_window, title,
...
void ProcessMoveCopyLink(char *command, FileMgrData *file_mgr_data,
                         FileViewData *file_view_data,
                         DtDndDropCallbackStruct *drop_parameters,
                         Widget drop_window)

{
  ...
  if (trashFile) {
    if (file_mgr_data != trashFileMgrData)
      if (InvalidTrashDragDrop(
              drop_parameters->operation, FROM_TRASH,
  ...
  if (drop_window) {
    ...
    if (file_mgr_data == trashFileMgrData && !trashFile) {
      DPRINTF(("DropOnFileWindow:Dragging File(s) to Trash Can from NonTrash "
               "Window\n"));
    ...
    /****************************************************************/
      ...
      MoveOutOfTrashCan(file_mgr_data,
                        ...
                        XtWindow(drop_window), numFiles, host_set, file_set,
      ...
      */
      {
        if (file_mgr_data == (FileMgrData *)initiating_view) {
          DPRINTF(("DropOnFileWindow: Dragging and Dropping File within same "
                   "window\n"));

          /* Simple reposition in the same window */
          XmDropSiteStartUpdate(file_mgr_rec->file_window);
          RepositionIcons(file_mgr_data, file_set, numFiles, drop_parameters->x,
                          drop_parameters->y, False);
          LayoutFileIcons(file_mgr_rec, file_mgr_data, False, True);
          XmDropSiteEndUpdate(file_mgr_rec->file_window);
        } else {
          DPRINTF(("DropOnFileWindow: Dragging file(s) and dropping from other "
                   "folders\n"));

          CheckMoveType(file_mgr_data, (FileViewData *)NULL,
                        (DirectorySet *)NULL, (DesktopRec *)NULL, file_set,
                        host_set, modifiers, numFiles, drop_parameters->x,
                        drop_parameters->y, NOT_DESKTOP);
        }
      }
      /*
               else
               {
                  ...
                  if (FileMoveCopy (file_mgr_data,
                                    NULL, file_mgr_data->current_directory,
         file_mgr_data->host, host_set, file_set, numFiles, modifiers, NULL,
         NULL))
                  {
  ...
  /*****************************/
    ...
    CheckMoveType(file_mgr_data, file_view_data,
                  ...
                  (DesktopRec *)NULL, file_set, host_set, modifiers, numFiles,
                  drop_parameters->x, drop_parameters->y, NOT_DESKTOP_DIR);
  }
...

{
  ...
  if (drop_window) {
...

    {
      ...
      DPRINTF(("ProcessDropOnBufferFolder: Dropping buffers on single "
               "directory view: %s\n",
               file_mgr_data->current_directory));
      ...
      MakeFilesFromBuffers(file_mgr_data, file_mgr_data->current_directory,
                           file_mgr_data->host, file_set, host_set, buffer_set,
                           num_of_buffers, NULL, NULL);
      ...
      /* Do Tree View Stuff      */
      {
        ...
        for (i = 0; i < num_of_buffers; i++) {
          ...
          for (j = 0; j < directory_data->file_count; j++) {
            file_view_data = directory_data->file_view_data[j];
            if ((file_view_data->filtered != True) &&
                (strcmp(directory_name, file_view_data->file_data->file_name) ==
                 0)) {
              SelectFile(file_mgr_data, file_view_data);
              break;
            }
    ...
    MakeFilesFromBuffers(file_mgr_data, directory, file_mgr_data->host,
                         file_set, host_set, buffer_set, num_of_buffers, NULL,
                         NULL);
  }
...
Boolean InvalidTrashDragDrop(int drag_op, int trash_context, Widget w) {
  Boolean rc = False;

  if ((drag_op == XmDROP_COPY) || (drag_op == XmDROP_LINK)) {
    char *tmpStr, *title, *msg;
    ...
    case WITHIN_TRASH:
      tmpStr = (GETMESSAGE(
          18, 36,
          "You can't copy or link a file or folder out of the Trash Can.\nMove "
          "the object out of the Trash and put it into the File Manager.\nYou "
          "can then copy or link it from there."));
      break;
    case FROM_TRASH:
      tmpStr = (GETMESSAGE(
          18, 37,
...
void ProcessAction(char *action, FileViewData *file_view_data,
                   DtDndDropCallbackStruct *drop_parameters, char *cur_host,
                   char *cur_dir, char *restricted_dir, Widget w)

{
  ...
  if (trashFileMgrData != NULL && file_mgr_data == trashFileMgrData) {
    ...
    tmpStr = GETMESSAGE(
        27, 105,
        "Default action of a trash object will not be executed.\n\nTo execute "
        "the default action of this object\n use 'Put Back' to return it to "
  ...
  if (drop_parameters) {
    ...
    if (first_arg)
      _DtBuildActionArgsWithSelectedFiles(&first_arg, 1, &action_args,
                                          &arg_count);
  }
  ...
  /* Invoke action */
  DtActionInvoke(w, action, action_args, arg_count, NULL, NULL, pwd_dir, True,
                 NULL, NULL);
...
void MoveCopyLinkHandler(Tt_message ttMsg, int opType) {
  ...
  if (tt_is_err(tt_ptr_error(toName))) { /* No file name */
  ...
  while (1) {
    ...
    if (file_count == file_set_size) {
      file_set_size += 10;
      file_set =
          (char **)XtRealloc((char *)file_set, sizeof(char **) * file_set_size);
    }
  ...
  if (stat(toName, &fileInfo) != 0 &&
      lstat(toName, &fileInfo) != 0) { /* to directory does not exist */
    char *dialogTitle;
  ...
  for (i = 0; i < file_count; i++) {
    if (stat(file_set[i], &fileInfo) != 0 &&
        lstat(file_set[i], &fileInfo) != 0) { /* File does not exist */
      char *dialogTitle;
      ...
      if (opType == MOVE_FILE) {
        ...
        if (file_count == 1)
          ...
          sprintf(
              ...
              GETMESSAGE(
                  33, 13,
                  "One of the objects you are trying to Move:\n\n   %s\n\ndoes "
        ...
        if (file_count == 1)
          sprintf(title,
                  GETMESSAGE(33, 14,
                             "The object you are trying to Copy:\n\n   "
                             "%s\n\ndoes not exist in the file system."),
                  file_set[i]);
        else
          sprintf(
              ...
              GETMESSAGE(
                  33, 15,
                  "One of the objects you are trying to Copy:\n\n   %s\n\ndoes "
        ...
        if (file_count == 1)
          sprintf(title,
                  GETMESSAGE(33, 16,
                             "The object you are trying to Link:\n\n   "
                             "%s\n\ndoes not exist in the file system."),
                  file_set[i]);
        else
          sprintf(
              ...
              GETMESSAGE(
                  33, 17,
                  "One of the objects you are trying to Link:\n\n   %s\n\ndoes "
  ...
  FileMoveCopy(NULL, NULL, toName, home_host_name, host_set, file_set,
               file_count, modifiers, NULL, NULL);
