...
static int build_obj_array(ABObj **objArrayInOut, int *objArraySizeInOut,
                           ABObj root);
static int verify_the_silly_index(ABObj nameScopeObj, ABObj *objArray,
                                  int objArraySize);
...
int ab_string_list_count(STRING *list) {
  int i = 0;
  if (list != NULL) {
    while (list[i] != NULL) {
      ++i;
 ...
 */
int obj_ensure_num_children(ABObj obj, int requested_num_children) {
...
ABObj obj_find_child_by_label(ABObj root, STRING label) {
  ABObj child;
...
ABObj obj_find_child_by_type(ABObj obj, AB_OBJECT_TYPE childType) {
  ABObj child;
...
ABObj obj_find_by_name_and_type(ABObj root, STRING name, AB_OBJECT_TYPE type) {
  ABObj obj;
  AB_TRAVERSAL trav;
  ...
  for (trav_open(&trav, root, AB_TRAV_ALL); (obj = trav_next(&trav)) != NULL;) {
    if ((obj->type == type) && (istr_equalstr(obj->name, name))) {
      break;
    }
  }
  trav_close(&trav);
...
ABObj obj_find_child_by_name(ABObj obj, STRING name) {
  ABObj child;
  if (name == NULL) {
    return NULL;
  }
...
ABObj obj_find_menu_by_name(ABObj root, STRING name) {
  return obj_find_by_name_and_type(root, name, AB_TYPE_MENU);
}

ABObj obj_find_module_by_name(ABObj root, STRING name) {
  AB_TRAVERSAL trav;
  ABObj module = NULL;
...
ABObj obj_scoped_find_or_create_undef(ABObj parent, STRING scopedName,
                                      AB_OBJECT_TYPE objType) {
  ABObj obj = obj_scoped_find_by_name(parent, scopedName);

  if (obj == NULL) {
    STRING moduleName = obj_scoped_name_get_module_name(scopedName);
    STRING objName = obj_scoped_name_get_obj_name(scopedName);
    ABObj project = NULL;
    ABObj module = NULL;
 ...
 */
BOOL obj_is_project_action_target(ABObj obj) {
  ABObj project = NULL;
 ...
 */
int obj_scoped_name_split(STRING complex_name, STRING module_name_buf,
                          int module_name_buf_size, STRING obj_name_buf,
...
static int obj_tree_print_indented(ABObj obj, int spaces, int verbosity);
static int indent(int spaces);
...
#define print_flag(flag, flag_str)                                             \
  (obj_has_flag(obj, flag) ? util_dprintf(0, " %s", flag_str) : 0)
  ...
  *namebuf = 0;
  ...
  if (obj_is_action(obj)) {
    ...
    case AB_FUNC_BUILTIN: /* builtin */
      sprintf(
          namebuf, "builtin:%s",
          util_builtin_action_to_string(obj->info.action.func_value.builtin));
      break;
    }
  } else {
  ...
  if (verbosity >= 4) {
    ...
    if (obj_is_action(obj)) {
      ...
      util_dprintf(0, "(%#08lx)%s\n", thatObj,
                   obj_get_safe_name(thatObj, namebuf, 256));
 ...
 * builds.
 */
int objP_update_verify(ABObj obj) {
#ifndef DEBUG
  return 0;
#else
  int return_value = 0;
...
#define field_err(_fieldName)                                                  \
  (return_value = -1,                                                          \
   util_dprintf(0, "ERROR: Obj %s, bad value in field: %s\n", obj_name,        \
                (_fieldName)))

#define check_str(obj, _field)                                                 \
  (((long)(last_field = #_field)),                                             \
   (istr_verify(obj->_field) >= 0) ? (0) : (field_err(#_field)))
  ...
  if (!objP_is_accessible(obj)) {
    ...
    return_value = -1;
    util_dprintf(0, "ERROR - bad object ptr in obj_verify(): %#lx\n", obj);
    goto epilogue;
  }
  ...
  if (sigsetjmp(sigjmp_env, TRUE) == 0) {
    ...
    if (ok) {
      ...
      if (obj_str_ptr_name != NULL) {
        snprintf((STRING)obj_name, sizeof(obj_name), "(ABObj %#lx = %s)",
                 (unsigned long)obj, obj_str_ptr_name);
      } else {
        snprintf((STRING)obj_name, sizeof(obj_name), "(ABObj %#lx)",
                 (unsigned long)obj);
      }
      if (!ok) {
        field_err("name");
  ...
  if (obj->impl_flags == ObjFlagAlreadyFreedValue) {
    ...
    util_dprintf(0, "\n");
    sleep(5);
    util_dprintf(0, "        (Actually, it's just a reference to a previously "
                    "destroyed object)\n");
  ...
  if ((obj->type != AB_TYPE_PROJECT) && (obj->name != NULL)) {
    ...
    if (parent != NULL) {
      ...
      if (namesList == NULL) {
        util_dprintf(1, "No names index found containing %s\n", obj_name);
        return_value = -1;
        goto epilogue;
      } else {
        /*
        ...
        if (obj_has_impl_flags(obj, ObjFlagDestroyed)) {
          if (tmpObj != NULL) {
            util_dprintf(1, "Destroyed object is in names index: %s\n",
                         obj_name);
            return_value = -1;
            goto epilogue;
          }
...
static void sig_fault_handler(int signo) {
...
int obj_tree_verify(ABObj root) {
  int return_value = 0;
...
static int verify_the_silly_index(ABObj nameScopeObj, ABObj *objArray,
  ...
  *nameBuf3 = 0;
  ...
  for (nameCount = 0; nameCount < numNames; ++nameCount) {
    ...
    for (objCount = 0; (!namedObjFound) && (objCount < objArraySize);
         ++objCount) {
      /*
      ...
      if (curNames == NULL) {
        util_dprintf(1, "No names list found for object %s in module %s!\n",
        ...
        util_dprintf(
            1, "Object %s should be in scope for %s, but is in scope for %s\n",
            obj_get_safe_name(curObj, nameBuf1, 1024),
        ...
        util_dprintf(1, "Object %s has incorrect reference in index!\n",
                     obj_get_safe_name(curObj, nameBuf1, 1024));
      } else {
        namedObjFound = TRUE;
      }
    ...
    if (!namedObjFound) {
#ifdef DEBUG
      util_dprintf(1, "Name is in index '%s', but no such object exists!\n",
...
static int build_obj_array(ABObj **objArrayPtr, int *objArraySizePtr,
                           ABObj root) {
   ...
   */
  {
    ...
    *name = 0;
    for (i = 0; i < objArraySize; ++i) {
      if (objArray[i] == root) {
        util_dprintf(
            1, "INTERNAL ERROR: some sort of cycle detected involving %s\n",
...
ABObj obj_get_parent_of_type(ABObj obj, AB_OBJECT_TYPE type) {
