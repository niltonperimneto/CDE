...
static Widget panedwin_editor_init(DtbPanedwinEdDialogInfo pw_cgen,
                                   Widget parent);
static int panedwin_editor_load(ABObj project);
static int panedwin_editor_clear();
static int panedwin_editor_clear_geom();

static int panedwin_editor_activate(BOOL active);
static int panedwin_editor_apply();
static BOOL panedwin_editor_pending();
static void panedwinEdP_init();
static void pw_objlist_load(Widget list);
static BOOL pw_objlist_test_func(ABObj test_obj);
static BOOL pw_child_test_func(ABObj test_obj);
 ...
 * Callbacks
 */
static int pw_obj_reparentedOCB(ObjEvReparentInfo info);
static int pw_obj_updateOCB(ObjEvUpdateInfo info);
static int pw_obj_destroyedOCB(ObjEvDestroyInfo info);
static int pw_obj_renamedOCB(ObjEvAttChangeInfo info);
static void pw_objlist_selectCB(Widget widget, XtPointer client_data,
                                XmListCallbackStruct *listdata);
static void pw_panelist_selectCB(Widget widget, XtPointer client_data,
                                 XmListCallbackStruct *listdata);
...
void panedwin_ed_show_dialog() {
  ...
  if (AB_panedWinEd_dialog == (Widget)NULL) {
    ...
    XtVaSetValues(XtParent(dtb_panedwin_ed_dialog.panelist_scrolledwin),
                  XmNrightAttachment, XmATTACH_FORM, XmNrightOffset, 100, NULL);
    ...
    XtAddCallback(dtb_panedwin_ed_dialog.objlist, XmNbrowseSelectionCallback,
                  (XtCallbackProc)pw_objlist_selectCB, (XtPointer)NULL);

    /* Add Callback to load panes when a paned window obj is
     * selected from list. */
    XtAddCallback(dtb_panedwin_ed_dialog.panelist, XmNbrowseSelectionCallback,
                  (XtCallbackProc)pw_panelist_selectCB, (XtPointer)NULL);

    /*
     * Setup dialog to participate in dtbuilder window protocol
     */
    ab_register_window(AB_panedWinEd_dialog, AB_WIN_DIALOG, WindowHidden,
                       AB_toplevel, AB_WPOS_TILE_HORIZONTAL, NULL, NULL);
  }
...
static Widget panedwin_editor_init(DtbPanedwinEdDialogInfo pw_cgen,
  ...
  if (dtb_panedwin_ed_dialog_initialize(pw_cgen, parent) == 0) {
    ...
    prop_geomfield_init(&(pws->geometry), pw_cgen->geo_label, NULL, NULL, NULL,
                        NULL, NULL /*pw_cgen->width_field_label*/,
                        pw_cgen->width_field,
                        NULL /*pw_cgen->height_field_label*/,
                        pw_cgen->height_field, pw_cgen->geo_cb);
    ...
    prop_geomfield_init(&(pws->min_max_geo), pw_cgen->pane_height_lbl, NULL,
                        NULL, NULL, NULL, pw_cgen->min_height_field_label,
                        pw_cgen->min_height_field,
                        pw_cgen->max_height_field_label,
                        pw_cgen->max_height_field, pw_cgen->height_cb);
 ...
 * height values.
 */
static int panedwin_editor_load(ABObj obj) {
  PanedWinEditorSettingsRec *pws = &pw_editor_settings_rec;
...
static void pw_objlist_selectCB(Widget widget, XtPointer client_data,
  ...
  if (name) {
    ...
    if (selected_obj) {
      ...
      prop_field_set_value(&(pws->name), obj_get_name(selected_obj), False);
...
static BOOL pw_objlist_test_func(ABObj test_obj) {
  ...
  if (obj_is_paned_win(test_obj) && obj_has_flag(module, MappedFlag)) {
    return (True);
  } else
    return (False);
}
 ...
 */
static BOOL module_test_func(ABObj test_obj) {
...
static int pw_obj_destroyedOCB(ObjEvDestroyInfo info) {
  ...
  if (AB_panedWinEd_dialog != NULL) {
    ...
    if (obj_is_project(info->obj)) {
      ...
      ret = abobj_list_obj_destroyed(list, info->obj, pw_objlist_test_func);
    } else /* Object is a pane OR a layer */
    {
      ...
      if (((parent = obj_get_parent(info->obj)) != NULL) &&
        ...
        ret = abobj_list_obj_destroyed(panelist, info->obj, pw_child_test_func);
      }
    }
  }
  return (ret);
...
static int pw_obj_renamedOCB(ObjEvAttChangeInfo info) {
  Widget list = dtb_panedwin_ed_dialog.objlist;
  Widget panelist = dtb_panedwin_ed_dialog.panelist;
  ABObj parent = NULL, sel_module = NULL, selected_obj = NULL;
  XmStringTable sel_item = NULL;
  ...
  if (AB_panedWinEd_dialog != NULL) {
    if (obj_is_module(info->obj)) {
      ...
      if (info->old_name != NULL) {
        ...
        abobj_list_obj_renamed(list, info->obj, istr_string(info->old_name),
                               module_test_func);

        /* Change the module prefix in the panes list */
        abobj_list_obj_renamed(panelist, info->obj, istr_string(info->old_name),
                               module_test_func);
      }
    } else if (obj_is_paned_win(info->obj)) {
       ...
       */
      if (info->old_name != NULL) {
        ret = abobj_list_obj_renamed(
            list, info->obj, istr_string(info->old_name), pw_objlist_test_func);
      }
    } else /* obj is a pane OR a layer */
    {
      if (info->old_name == NULL) {
        ...
        if ((parent != NULL) && obj_is_paned_win(parent)) {
          XtVaGetValues(list, XmNselectedItems, &sel_item, NULL);
          if (sel_item != NULL) {
            abobj_moduled_name_extract(objxm_xmstr_to_str(sel_item[0]),
                                       &sel_module, &selected_obj);
            if (selected_obj == parent)
              ret = abobj_list_update(panelist, parent, pw_child_test_func);
          }
        }
      }
...
static int pw_obj_reparentedOCB(ObjEvReparentInfo info) {
  ...
  if (AB_panedWinEd_dialog != NULL) {
    ...
    if (obj_get_name(info->obj) != NULL) {
       ...
       */
      if ((info->old_parent != NULL) && obj_is_paned_win(info->old_parent)) {
        if (current_pw_child == info->obj)
          panedwin_editor_clear_geom();
        ret = abobj_list_obj_reparented(panelist, info, pw_child_test_func);
      }
    }
  }
  return (ret);
