...
static XtResource resources[] = {
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, useBMS),
        XmRImmediate,
        (XtPointer)False,
    },
    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationDataPtr, session),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationDataPtr, bmSuffix),
        XmRImmediate,
        (XtPointer) ".bm",
    },
    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationDataPtr, pmSuffix),
        XmRImmediate,
        (XtPointer) ".pm",
    },
    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationDataPtr, maxIconWidth),
        XmRImmediate,
    ...
    },
    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationDataPtr, maxIconHeight),
        XmRImmediate,
    ...
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, useFileFilter),
        XmRImmediate,
        (XtPointer)True,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, useFileLists),
        XmRImmediate,
        (XtPointer)True,
    },
};
...
static XrmOptionDescRec option_list[] = {
    ...
    {"-maxIconHeight", "maxIconHeight", XrmoptionSepArg,
     (caddr_t)MAX_ICON_HEIGHT},
...
/*-----------------------------------------------------------*/
static void CloseCB(Widget w, XtPointer clientData, XtPointer callbackArg) {
  DialogFlag = QUIT;
...
/*-----------------------------------------------------------*/
static void SaveSessionCB(Widget w, XtPointer clientData,
...
int main(int argc, char *argv[]) {
  ...
  TopLevel = XtAppInitialize(&AppContext, CLASS_NAME, option_list,
                             XtNumber(option_list), &argc, argv, NULL, NULL, 0);

  XtGetApplicationResources(TopLevel, &xrdb, resources, XtNumber(resources),
                            NULL, 0);
  ...
  */
  ...
  XmAddWMProtocolCallback(dtIconShell, xa_WM_DELETE_WINDOW, CloseCB, NULL);

  /*------------------------------------------------------------------*/
  /*   Register for save and restore.                                 */
  /*------------------------------------------------------------------*/
  xa_WM_SAVE_YOURSELF = XInternAtom(dpy, "WM_SAVE_YOURSELF", False);
  XmAddWMProtocolCallback(dtIconShell, xa_WM_SAVE_YOURSELF, SaveSessionCB,
                          NULL);
  ...
  ttRc = ttmedia_ptype_declare(dticon_ptype, 0, ProcessToolTalkMediaMessage,
                               NULL, 0);
  DieFromToolTalkError(dticonShell, "ttmedia_ptype_declare: %s", ttRc);

  ttRc = ttmedia_ptype_declare(dticon_ptype, 1000, ProcessToolTalkMediaMessage,
                               NULL, 0);
  DieFromToolTalkError(dticonShell, "ttmedia_ptype_declare: %s", ttRc);

  ttRc = ttmedia_ptype_declare(dticon_ptype, 2000, ProcessToolTalkMediaMessage,
                               NULL, 1);
  DieFromToolTalkError(dticonShell, "ttmedia_ptype_declare: %s", ttRc);

  tttk_Xt_input_handler(NULL, 0, 0);

  ttRc = tt_ptr_error(ttdt_session_join(
      tt_default_session(), ProcessToolTalkMessage, (Widget)0, NULL, 1));
...
static void ExitCB(Widget dialog, XtPointer client_data, XtPointer call_data) {
...
void DieFromToolTalkError(Widget parent, char *errfmt, Tt_status status) {
  ...
  n++;
  XtSetArg(args[n], XmNokLabelString, xms_ok);
  n++;
  XtSetArg(args[n], XmNdialogTitle, xms_title);
  n++;
  XtSetArg(args[n], XmNmessageString, xms_errmsg);
  n++;
  ...
  n = 0;
  XtSetArg(args[n], XmNmwmDecorations, MWM_DECOR_ALL | MWM_DECOR_MENU);
  n++;
...
Tt_message ProcessToolTalkMediaMessage(Tt_message msg, void *cdata, Tttk_op op,
                                       Tt_status diag, unsigned char *contents,
                                       int len, char *file, char *doc) {
  int mark = tt_mark();
  int tmplen = 0;
  ...
  case TTME_EDIT:
    ...
    if (tt_message_status(msg) != TT_WRN_START_MESSAGE) {
      ...
      if (file) {
        ...
        if (tt_message_arg_bval(msg, 0, &buf, &blen) != TT_OK) {
          tt_message_fail(msg);
          edit_notifier(NULL, 0, 1);
        } else {
          strcpy(start_file, "/tmp/dticon_XXXXXX");
          int fd = mkstemp(start_file);
          if (fd != -1)
            close(fd);
          if ((buf) && (!strncmp((char *)buf, "/* XPM */", 9))) {
            /* Format XPM */
            tt_tmpfile_fd =
                open(start_file, O_CREAT | O_WRONLY | O_NDELAY, 0666);
            if (tt_tmpfile_fd &&
                (wlen = write(tt_tmpfile_fd, buf, blen)) == blen) {
            ...
            if (tt_tmpfile_fd &&
                (wlen = write(tt_tmpfile_fd, buf, tmplen)) == tmplen) {
              (void)close(tt_tmpfile_fd);
              param_flag[AUTO_FILE] = True;
              argsNeedProcessed = True;
            } else {
            ...
            if (tmp1) {
              ...
              tt_tmpMaskfile_fd =
                  open(mask_file, O_CREAT | O_WRONLY | O_NDELAY, 0666);
              /* Read the mask fro the seconf buffer. */
              if (tt_tmpMaskfile_fd &&
                  (wlen = write(tt_tmpMaskfile_fd, tmp, strlen(tmp))) ==
                      strlen(tmp)) {
...
Tt_message ProcessToolTalkMessage(Tt_message msg, void *cdata,
                                  Tt_message contract) {
  int mark = tt_mark();
...
static int handle_dialog_child(Widget wgt, void (*manage_func)()) {
  ...
  XtVaGetValues(wgt, XmNnumChildren, &num_children, XmNchildren, &children,
                NULL);
