...
void LoadFile(Editor *pPad, char *include) {
  char *localName;
  ...
  if (localName && *localName) {
    ...
    if (include != (char *)NULL) {
      errorCode = DtEditorInsertFromFile(pPad->editor, localName);
    } else {
      pPad->fileStuff.fileExists = True;
      errorCode = DtEditorSetContentsFromFile(pPad->editor, localName);
    }
    ...
    case DtEDITOR_READ_ONLY_FILE:
      ...
      if (include == (char *)NULL) {
        ...
        if (pPad->xrdb.readOnlyWarning && !pPad->xrdb.statusLine &&
            !pPad->xrdb.viewOnly) {
          Warning(pPad,
                  (char *)GETMESSAGE(
                      6, 1, "This file has been opened for reading only."),
                  XmDIALOG_WARNING);
        }
      }
      break;
    case DtEDITOR_NONEXISTENT_FILE:
      if (include != (char *)NULL) {
        Warning(pPad, (char *)GETMESSAGE(6, 2, "This file does not exist."),
                XmDIALOG_ERROR);
        fileLoadError = True;
    ...
    case DtEDITOR_DIRECTORY:
      Warning(pPad, (char *)GETMESSAGE(6, 3, "Unable to edit a Directory."),
              XmDIALOG_ERROR);
      fileLoadError = True;
      break;
    case DtEDITOR_CHAR_SPECIAL_FILE:
      Warning(pPad,
              (char *)GETMESSAGE(
                  6, 4, "Unable to edit a character special device file."),
              XmDIALOG_ERROR);
      fileLoadError = True;
      break;
    case DtEDITOR_BLOCK_MODE_FILE:
      Warning(
          pPad,
          (char *)GETMESSAGE(6, 5, "Unable to edit a block mode device file."),
          XmDIALOG_ERROR);
      fileLoadError = True;
      break;
    case DtEDITOR_NULLS_REMOVED:
      Warning(pPad,
              (char *)GETMESSAGE(6, 6, "Embedded Nulls stripped from file."),
              XmDIALOG_WARNING);
      fileLoaded = True;
      break;
    ...
    case DtEDITOR_NO_FILE_ACCESS:
      Warning(pPad,
              (char *)GETMESSAGE(
                  6, 8, "Unable to access file (can't determine its status)."),
              XmDIALOG_ERROR);
      fileLoadError = True;
      break;
    ...
    default:
      Warning(pPad,
              (char *)GETMESSAGE(
                  6, 12, "Unable to read from the file (unknown reason)."),
              XmDIALOG_ERROR);
      fileLoadError = True;
      break;
    } /* switch (errorCode) */
  ...
  if (fileLoaded) {
    ...
    if (errorCode != DtEDITOR_NO_ERRORS)
      Warning(pPad, "Internal Error: Unable to NULL Editor Widget.",
              XmDIALOG_ERROR);
...
char *GetTempFile(void) {
  char *tempname = (char *)XtMalloc(256); /* Temporary file name. */
  FILE *tfp;
  int fd;
 ...
 ************************************************************************/
static char *AddPound(char *nameBuf) {
  char tempBuf[512];
...
void PanicSave(void) {
  ...
  for (pPad = pPadList; pPad != (Editor *)NULL; pPad = pPad->pNextPad) {
    ...
    addNewLines =
        pPad->xrdb.wordWrap == True && pPad->fileStuff.saveWithNewlines == True;
    errorCode = DtEditorSaveContentsToFile(
        pPad->editor, fileName, False, /* don't overwrite existing file */
 ...
 ************************************************************************/
/* ARGSUSED */
void SetStatusLineMsg(Editor *pPad) {
  char *message = "";
