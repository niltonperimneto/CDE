...
static const char *defaultTitle12 =
    "<TYPE serif><WEIGHT bold><SIZE 12><ANGLE "
    "italic>\n%s\n</ANGLE></SIZE></WEIGHT></TYPE>\n";
static const char *defaultTitle14 =
    "<TITLE><TYPE serif><WEIGHT bold><SIZE "
 ...
 *****************************************************************************/
void MyExit(int exit_val, pid_t pid) {
  if (pid != ((pid_t)-1))
 ...
 *****************************************************************************/
char *GetMessage(int set, int n, char *s) { return (s); }

/*****************************************************************************
 * Boolean *GetPath(filename)
 *****************************************************************************/
Boolean GetPath(char *filename, short strip, char ***list) {
  char *ptr;
  char **next = *list;
  ...
  if (next == NULL || *next == NULL)
    *list = (char **)_DtHelpCeAddPtrToArray((void **)(*list), strdup(filename));
 ...
 *
 *****************************************************************************/
int CreateVolumeLink(CanvasHandle canvas, FILE *outTopic, char *volume_name) {
  int result = -1;
  char *title = NULL;
 ...
 *
 *****************************************************************************/
int CreateFamily(CanvasHandle canvas, char *family_name, FILE *out_volume,
                 FILE *out_header, FILE *out_topic) {
  int result = -1;
  int count = 0;
  long filepos;
  char *charSet = NULL;
  ...
  if (db) {
     ...
     * get the title
     */
    if (XrmGetResource(db, "Family.Title", "family.title", &resType,
                       &resValue)) {
       ...
       * get the abstract
       */
      if (XrmGetResource(db, "Family.Abstract", "family.abstract", &resType,
                         &resValue)) {
         ...
         */
        if (XrmGetResource(db, "Family.Volumes", "family.volumes", &resType,
                           &resValue)) {
           ...
           */
          if (XrmGetResource(db, "Family.CharSet", "family.charSet", &resType,
                             &resValue)) {
             ...
             */
            if (XrmGetResource(db, "Family.Bitmap", "family.bitmap", &resType,
                               &resValue))
              bitmap = (char *)resValue.addr;
          } else {
            fprintf(stderr,
                    (GetMessage(1, 14, "%s: character set resource missing\n")),
                    family_name);
            return -1;
          }
        } else {
          fprintf(stderr, (GetMessage(1, 13, "%s: volumes resource missing\n")),
                  family_name);
          return -1;
        }
      } else {
        fprintf(stderr, (GetMessage(1, 12, "%s: abstract resource missing\n")),
                family_name);
        return -1;
      }
    } else {
      fprintf(stderr, (GetMessage(1, 11, "%s: title resource missing\n")),
              family_name);
      return -1;
    }
    ...
    if (title && abstract && list && charSet) {
       ...
       * write out the <TOPIC>
       */
      fprintf(out_topic, (GetMessage(3, 1, (char *)defaultTopic)), charSet,
              title);

      /*
       * write out the <TITLE>
       */
      fprintf(out_topic, (GetMessage(3, 2, (char *)defaultTitle14)), title);
      fprintf(out_topic, "%s",
              (GetMessage(3, 3, "<P before 1 first 1 left 1>\n")));
      fprintf(out_topic, "%s\n", abstract);
      fprintf(out_topic, "</P>\n");
      ...
      if (count) {
         ...
         */
        fprintf(out_header, (GetMessage(3, 4, "<CHARACTERSET %s>\n")), charSet);
        fprintf(out_header, "<LINK 0 %s>\n", familyName);
        fprintf(out_header, (GetMessage(3, 5, (char *)defaultTitle12)), title);
        ...
        if (NULL != bitmap && *bitmap != '/') {
          ...
          if (ptr) {
            ptr++;
            *ptr = '\0';
            snprintf(bitmapNameTemp, sizeof(bitmapNameTemp), "%s%s", bitmapName,
                     bitmap);
        ...
        if (NULL != bitmap) {
          fprintf(out_header,
                  (GetMessage(3, 6,
                              "<P before 1 first 1 left 1 graphic %s glink %s "
                              "gtypelink 0>\n")),
                  bitmap, familyName);
        } else
        ...
        TopicList = (char **)_DtHelpCeAddPtrToArray((void **)TopicList,
                                                    strdup(familyName));
      } else {
        /*
 ...
 *
 *****************************************************************************/
char **AddFamilyToList(char *name) {

  FamilyList =
      (char **)_DtHelpCeAddPtrToArray((void **)FamilyList, strdup(name));
  return FamilyList;
}
 ...
 *
 *****************************************************************************/
void ScanDirectory(char *directory, long *ret_time) {
  DIR *pDir;
  struct stat buf;
    ...
    if (strcmp(ext, Family_ext) == 0) {
      if (CheckFamilyList(pDirent->d_name) == False) {
        ...
        FullFamilyName = (char **)_DtHelpCeAddPtrToArray(
            (void **)FullFamilyName, strdup(fullName));
      }
...
char *FindFile(char *filename) {
  ...
  while (VolumeList != NULL && VolumeList[i] != NULL) {
    ...
    if (!different && access(FullVolName[i], R_OK) == 0 &&
        stat(FullVolName[i], &status) == 0 && S_ISDIR(status.st_mode) == 0)
 ...
 *
 *****************************************************************************/
void ExpandPaths(char *lang, char *type, char *env_var, char *default_str,
                 char ***list) {
  short strip;
  char *ptr;
  char *hPtr;
 ...
 *
 *****************************************************************************/
int CheckTimeStamps(XrmDatabase db, char **dir_list) {
  long timeVal;
  char *value;
  struct stat buf;
  ...
  while (*dir_list != NULL) {
    ...
    value = _DtHelpCeGetResourceString(db, *dir_list, "TimeStamp", "timeStamp");
    timeVal = atol(value);
...
int CheckInfo(char *file) {
  ...
  if (db != NULL) {
    volDirList =
        _DtHelpCeGetResourceStringArray(db, NULL, "DirList", "dirList");
    if (volDirList != NULL) {
      result = 0;
      list1 = volDirList;
      list2 = FUserList;
      while (result == 0 && *list1 != NULL && list2 != NULL && *list2 != NULL) {
...
int main(int argc, char *argv[]) {
  ...
  if (App_args.lang != NULL) {
    ...
    if (setlocale(LC_ALL, Lang) == NULL) {
      fprintf(
          stderr,
          (GetMessage(1, 20, "%s: Invalid system language specified - %s\n")),
          myName, Lang);
      exit(1);
    }
    _DtEnvControl(DT_ENV_SET);
  ...
  if (App_args.dir[0] != '/') {
    if (getcwd(baseName, MAXPATHLEN) == NULL) {
      fprintf(stderr,
              (GetMessage(1, 18,
                          "%s: Unable to access current working directory - "
                          "error status number %d\n")),
              myName, errno);
      exit(1);
    }
    snprintf(baseNameTemp, sizeof(baseNameTemp), "%s/%s", baseName,
             App_args.dir);
  ...
  ptr = _DtHelpCeExpandPathname(baseName, NULL, "help", NULL, Lang,
                                (_DtSubstitutionRec *)NULL, 0);
  if (ptr == NULL || *ptr == '\0') {
    fprintf(stderr, (GetMessage(1, 15, "%s: Destination directory missing\n")),
            myName);
    exit(1);
  }
  ...
  if (tmpVolume[strlen(tmpVolume) - 1] != '/') {
    snprintf(tmpVolumeTemp, sizeof(tmpVolumeTemp), "%s%s", tmpVolume,
             SlashString);
    ...
    if (access(tmpVolume, F_OK) == -1) {
      ...
      case ENOENT:
        if (mkdir(tmpVolume,
          ...
          case ENOENT:
            ptr = GetMessage(1, 4, "%s: Element of %s does not exist\n");
            break;

          case ENOSPC:
            ptr = GetMessage(1, 5, "%s: File system containing %s is full\n");
            break;
  ...
  if (result == 0) {
    if (access(tmpVolume, W_OK) == -1) {
      if (errno == EROFS)
        ptr = GetMessage(1, 7, "%s: File system containing %s is read only\n");
      else if (errno == EACCES)
        ptr = GetMessage(1, 8, "%s: Requires root permission to write to %s\n");
      else
  ...
  ExpandPaths(Lang, "families", DtSYS_FILE_SEARCH_ENV, DtDEFAULT_SYSTEM_PATH,
              &FSysList);
  ExpandPaths(Lang, "volumes", DtSYS_FILE_SEARCH_ENV, DtDEFAULT_SYSTEM_PATH,
              &VSysList);
   ...
   */
  if (childPid == 0) {
    execlp("dthelpgen.ds", "dthelpgen.ds", ((char *)0), ((char *)0),
           ((char *)0));
    _exit(1);
  }
    ...
    if (!ret) {
      ...
      if (!ret) {
        ret = _DtLcxXlateOpToStd(db, plat, execver, DtLCX_OPER_SETLOCALE,
                                 setlocale(LC_MESSAGES, NULL), &ret_stdLocale,
                                 &ret_stdLangTerr, &ret_stdCodeset,
                                 &ret_stdModifier);
        if (!ret) {
  ...
  fprintf(outHeader, (GetMessage(3, 1, (char *)defaultTopic)), charSet,
          topicTitle);
  ...
  if (foundFamily == 0)
    fprintf(stderr, GetMessage(1, 16, "%s: Zero Family files found\n"), myName);
  else if (foundVolumes == 0)
    fprintf(stderr, GetMessage(1, 17, "%s: Zero Volume files found\n"), myName);
