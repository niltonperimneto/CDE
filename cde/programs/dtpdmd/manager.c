      ...
      if (statBuffer.st_size > 0) {
        if (g.serviceRecs[i]->message_string) {
          ...
          g.serviceRecs[i]->message_string =
              Xrealloc((char *)g.serviceRecs[i]->message_string,
                       statBuffer.st_size + inc + 1);
        } else {
          inc = 0;

          g.serviceRecs[i]->message_string = Xmalloc(statBuffer.st_size + 1);
        }
...
void mgr_initialize(XEvent *report, XpPdmServiceRec *rec) {
  ...
  if (XGetWindowProperty(selection_display, requestor, prop_atom, 0, 100000,
                         True, AnyPropertyType, &text_prop.encoding,
                         &text_prop.format, &text_prop.nitems, &tafter,
  ...
  if (XmbTextPropertyToTextList(selection_display, &text_prop, &list,
                                &list_cnt) < 0) {
    /*
     * Error
     */
   ...
   * open.
   */
  if (!strcmp(XDisplayString(rec->selection_display), rec->print_display_str)) {
...
void mgr_launch_pdm(XpPdmServiceRec *rec) {
  ...
  rec->message_xtid =
      XtAppAddInput(g.context, rec->message_pipe[0], (XtPointer)XtInputReadMask,
  ...
  if (rec->cookie_cnt) {
    ...
    strcpy(rec->auth_filename, "/tmp/dtpdmd_auth_XXXXXX");
...
void mgr_launch_reply(XpPdmServiceRec *rec) {
  ...
  XChangeProperty(rec->selection_display, rec->requestor, rec->prop_atom,
                  XA_ATOM, 32, PropModeReplace,
                  (unsigned char *)&(rec->pdm_exec_errorcode), 1);
  ...
  if ((rec->pdm_exec_errormessage) && (g.log_file)) {
    if ((errlog = fopen(g.log_file, "a+")) != NULL) {
      ...
      fprintf(errlog, PDMD_MSG_11, g.prog_name, ctime(&now),
              rec->pdm_exec_errormessage, eec, rec->pdm_exec_argvs[0],
              rec->print_display_str, rec->video_display_str);
  ...
  if (setjmp(xio_quickie_jmp_buf) == 0) {
    ...
    if (rec->seldpy_as_printdpy) {
      ...
      if (pdpy) {
         ...
         */
        cme.xclient.message_type = XInternAtom(pdpy, "PDM_REPLY", False);
        cme.xclient.data.l[0] = (long)XInternAtom(pdpy, mess, False);
...
void mgr_shutdown_scan(void) {
  ...
  for (i = 0; i < g.serviceRecNum; i++) {
    ...
    if ((g.serviceRecs[i]->exit_received) &&
       ...
       */
      if (((g.serviceRecs[i]->message_string) ||
           (g.serviceRecs[i]->message_string2)) &&
          (g.log_file)) {
        if ((errlog = fopen(g.log_file, "a+")) != NULL) {
          now = time((time_t)0);
