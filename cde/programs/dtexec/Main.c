...
Tt_message dtexec_tttk_message_create(Tt_message context, Tt_class theClass,
                                      Tt_scope theScope, const char *handler,
                                      const char *op,
...
Tt_status dtexec_tttk_message_destroy(Tt_message msg) {
  Tt_status status;
...
int dtexec_tttk_message_am_handling(Tt_message msg) {
  char *handler;
  int am_handling;
...
Tt_status dtexec_tttk_message_abandon(Tt_message msg) {
  int fail;
  Tt_status status;
...
static void Help(char *argv[]) {
  ...
  (void)fprintf(stderr,
                "\t\t 0 exit as soon as cmd terminates, thus allowing\n");
  (void)fprintf(stderr, "\t\t   the terminal window to close.\n");
  (void)fprintf(stderr,
                "\t\t n continue to execute if cmd terminates within n\n");
...
static int ExecuteCommand(char **commandArray) {
  int i, index1;
  int exitStatus;
  struct sigaction svec;
...
static char **ParseCommandLine(int argc, char **argv) {
  ...
  for (argc--, argv++; argc > 0; argc--, argv++) {
    if (!strcmp(argv[0], "-open")) {
      ...
      tmpFileCntG++;
      tmpFilesG =
          (char **)realloc((char *)tmpFilesG, tmpFileCntG * sizeof(char *));
 ...
 *
 *****************************************************************************/
void DetachFromTooltalk(unsigned long *nocare1) /* if Xt - XtInputId *id; */
{
  char *sessid;
 ...
 *
 *****************************************************************************/
void input_handler(char *nocare1,          /* if Xt - XtPointer w */
 ...
 *
 *****************************************************************************/
static void ToolTalkError(char *errfmt, Tt_status status) {
  char *statmsg;
 ...
 */
Tt_callback_action IdSelfToCallerReplyCB(Tt_message msg, Tt_pattern pattern) {
  Tt_state state;
  Tt_status status;
  char *errorMsg;
 ...
 * Routine to catch identification reply.
 */
Tt_callback_action DoneRequestReplyCB(Tt_message msg, Tt_pattern pattern) {
  Tt_state state;
    ...
    if (nfound > 0) {
      ...
      if (ttfdG != -1) {
        if (GETBIT(readfds, ttfdG)) {
           ...
           * ever used.
           */
          input_handler((char *)NULL, (int *)&junki, (unsigned long *)&junki);
        }
      }
        ...
        if (GETBIT(readfds, errorpipeG[0])) {
          ...
          while (1) {
            ...
            if (tmpi > 0) {
              ...
              firstPass = 0;
              if (tmpBuffer == NULL)
                tmpBuffer = (char *)malloc(tmpi + 1);
              else
                tmpBuffer = (char *)realloc(tmpBuffer, errorBytes + tmpi + 1);
              /*
               * Drain error pipe.
               */
              ...
              if (errorBytes < 65535) {
                 ...
                 */
                select(0, FD_SET_CAST(NULL), FD_SET_CAST(NULL),
                       FD_SET_CAST(NULL), &timeoutShort);
              } else {
                /*
          ...
          if (tmpBuffer) {
            ...
            if (!tmpProgName) {
              tmpProgName =
                  (char *)malloc(strlen(argv[0]) + strlen(cmdLine[0]) + 5);
              if (!tmpProgName)
                tmpProgName = argv[0];
              else {
                /*
            ...
            DtMsgLogMessage(tmpProgName, DtMsgLogStderr, "%s", tmpBuffer);
            free(tmpBuffer);
          }
