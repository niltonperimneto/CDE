...
DtMail::MailRc::MailRc(DtMailEnv &error, Session *session) : _vars(20) {
  _session = session;
...
void DtMail::MailRc::getValue(DtMailEnv &error, const char *var,
                              const char **value, DtMailBoolean decrypt) {
  MutexLock lock_scope(_obj_mutex);
...
void DtMail::MailRc::setVar(const char *variable, const char *value) {
  MutexLock lock_scope(_obj_mutex);
...
char *DtMail::MailRc::getVar(const char *var) {
  for (int i = 0; i < _vars.length(); i++) {
...
void DtMail::MailRc::parse_var(const char *line) {
  const char *cur_c;
  ...
  // Oh well, more work to do. It is easy though. The value is the next
  // character after the = to the end of the line.
...
const char *DtMail::MailRc::getAlias(DtMailEnv &error, const char *name) {
