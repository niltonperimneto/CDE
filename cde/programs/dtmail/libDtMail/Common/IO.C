...
//
//
int SafeOpen(const char *path, int oflag, mode_t mode) {
  int status;
...
int SafeWaitpid(pid_t proc, int *p_stat, int options) {
  pid_t status;
  do {
...
size_t SafeStrftime(char *buf, size_t buf_size, const char *format,
                    const tm *timeptr) {
#if defined(POSIX_THREADS)
...
char *SockGets(char *buf, int len, void *sockfp) {
  ...
  do {
    bufp++;
    read((int)(long)sockfp, (void *)bufp, 1);
    n++;
  } while (*bufp != '\n');
  *(bufp + 1) = '\0';
  return buf;
...
int SockRead(char *buf, int size, int len, void *sockfp) {
  ...
  fseek((FILE *)sockfp, 0L, SEEK_CUR); /* required by POSIX */
#else
  int n = (int)read((int)(long)sockfp, (void *)buf, (size_t)size * len);
#endif
  return n;
}
...
int SockWrite(char *buf, int size, int len, void *sockfp) {
  ...
  fseek((FILE *)sockfp, 0L, SEEK_CUR); /* required by POSIX */
#else
  n = write((int)(long)sockfp, buf, size * len);
#endif
 ...
 *              *vmountpp.
 */
int get_stat(struct vmount **vmountpp) /* place to tell where buffer is */
{
  size_t size;
  struct vmount *vm;
  int nmounts;
...
struct vmount *get_vmount(fsid_t *fsid) {
  ...
  for (vm = inu_vmount_p; nmount;
       nmount--, vm = (struct vmount *)((char *)vm + vm->vmt_length)) {
...
int FileSystemSpace(const char *file_path, size_t bytes, char **fsname) {
  ...
  if (statfs(file_path, &statfs_buf) < 0)
    return 0;
  ...
  if (bytes != 0) {
    ...
    if (!fserror) {
      req_space =
          (size_t)((bytes > stat_buf.st_size) ? (bytes - stat_buf.st_size) : 0);
#if !defined(__linux__)
