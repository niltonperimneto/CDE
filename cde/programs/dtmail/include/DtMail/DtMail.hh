...
public:
  ...
  public:
    ...
    void queryImpl(DtMailEnv &error, const char *impl, const char *capability,
                   ...);
    void queryImplV(DtMailEnv &error, const char *impl, const char *capability,
                    va_list args);

    // Create a mail box object, after selecting an implementation.
    MailBox *mailBoxConstruct(DtMailEnv &error, DtMailObjectSpace space,
                              void *arg, DtMailCallback open_callback,
                              void *client_data, const char *impl = NULL);

    Message *messageConstruct(DtMailEnv &error, DtMailObjectSpace space,
                              void *arg, DtMailCallback open_callback,
                              void *client_data, const char *impl = NULL);

    // Create a transport object after selecting an implementation.
    Transport *transportConstruct(DtMailEnv &error, const char *impl,
    ...
    void addEventRoutine(DtMailEnv &, DtMailEventFunc, void *client_data,
                         time_t interval);

    // Remove an event routine.
    void removeEventRoutine(DtMailEnv &, DtMailEventFunc, void *client_data);

    // Push some data into the event queue.
    //
    void writeEventData(DtMailEnv &error, const void *buf,
    ...
    typedef void (*BusyApplicationCallback)(DtMailEnv &error,
                                            ...
                                            void *client_data);
    void registerBusyCallback(DtMailEnv &, BusyApplicationCallback,
                              void *client_data);
    ...
    void
    registerLastInteractiveEventTimeCallback(LastInteractiveEventTimeCallback,
                                             void *client_data);
    ...
    void DtXlateStdToOpCodeset(char *op, char *std, char *dflt, char **ret);
    void DtXlateMimeToIconv(const char *, const char *, const char *, char **,
                            char **);
    void DtXlateLocaleToMime(const char *, const char *, const char *, char **);
  ...
  public:
    ...
    static void hm_add(struct hash **table, char *key, void *value, int size);
  ...
  public:
    ...
    virtual BodyPart *getNextBodyPart(DtMailEnv &, BodyPart *last) = 0;

    virtual BodyPart *newBodyPart(DtMailEnv &, BodyPart *after) = 0;

#ifdef DEAD_WOOD
    virtual void newBodyPartOrder(DtMailEnv &, BodyPart *new_order,
                                  const int bodypart_count) = 0;
#endif /* DEAD_WOOD */

    virtual void setFlag(DtMailEnv &, const DtMailMessageState) = 0;

    virtual void resetFlag(DtMailEnv &, const DtMailMessageState) = 0;

    virtual DtMailBoolean flagIsSet(DtMailEnv &, const DtMailMessageState) = 0;
  ...
  protected:
    Message(DtMailEnv &, MailBox *parent);
  ...
  public:
    virtual DtMailHeaderHandle getFirstHeader(DtMailEnv &, char **name,
    ...
    virtual void getHeader(DtMailEnv &, const char *name,
                           const DtMailBoolean abstract,
                           DtMailValueSeq &value) = 0;

    virtual void setHeaderSeq(DtMailEnv &, const char *header_name,
    ...
    //
    virtual void setHeader(DtMailEnv &, const char *header_name,
                           const DtMailBoolean replace, const char *) = 0;

    virtual void removeHeader(DtMailEnv &, const char *header_name) = 0;
    //
  ...
  public:
    ...
    virtual void getContents(DtMailEnv &, const void **contents,
                             unsigned long *length, char **type, char **name,
                             int *mode, char **description) = 0;

    virtual void getContentType(DtMailEnv &, char **content_type) = 0;

    virtual void setContents(DtMailEnv &, const void *contents,
                             const unsigned long length, const char *type,
                             const char *name, const int mode,
                             const char *description) = 0;

    virtual void setFlag(DtMailEnv &, DtMailBodyPartState) = 0;

    virtual void resetFlag(DtMailEnv &, DtMailBodyPartState) = 0;

    virtual DtMailBoolean flagIsSet(DtMailEnv &, DtMailBodyPartState) = 0;
    ...
    virtual void DtXlateMimeToIconv(const char *, const char *, const char *,
                                    char **, char **) = 0;
    virtual void DtXlateLocaleToMime(const char *, const char *, const char *,
                                     char **) = 0;
  ...
  public:
    ...
    //   None.
    //
    MailBox(DtMailEnv &, Session *session, DtMailObjectSpace space, void *arg,
            DtMailCallback cb, void *clientData);
    ...
    //
    virtual void open(DtMailEnv &error, DtMailBoolean auto_create = DTM_TRUE,
    ...
    virtual DtMailMessageHandle
    getFirstMessageSummary(DtMailEnv &error,
                           const DtMailHeaderRequest &header_list,
                           DtMailHeaderLine &header_line) = 0;

    virtual DtMailMessageHandle
    getNextMessageSummary(DtMailEnv &error, DtMailMessageHandle last,
                          const DtMailHeaderRequest &header_list,
                          DtMailHeaderLine &header_line) = 0;

    virtual void getMessageSummary(DtMailEnv &error, DtMailMessageHandle handle,
                                   const DtMailHeaderRequest &,
                                   DtMailHeaderLine &) = 0;

    virtual void clearMessageSummary(DtMailHeaderLine &) = 0;
    ...
    virtual void startAutoSave(DtMailEnv &error,
    ...
    void hideAccessEvents(DtMailBoolean onoff) { _hide_access_events = onoff; }

    DtMailObjectKey getObjectKey(void) { return _key; }
  ...
  public:
    virtual DtMailOperationId submit(DtMailEnv &, Message *msg,
    ...
    virtual void initTransportData(int fds[2], SubProcessFinishedProc proc,
                                   void *ptr) = 0;
...
#if defined(sun) || defined(CSRG_BASED)
template <typename T>
size_t iconv(iconv_t i, const T inbuf, size_t *inleft, char **outbuf,
             size_t *outleft) {
