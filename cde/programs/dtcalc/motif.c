...
static XtResource resources[] = {
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationArgsPtr, menuBar),
        XmRImmediate,
        (XtPointer)True,
    },

    {
        ...
        XmRInt,
        sizeof(int),
        XtOffset(ApplicationArgsPtr, accuracy),
        XmRImmediate,
        (XtPointer)2,
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, base),
        XmRImmediate,
        (XtPointer) "decimal",
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, display),
        XmRImmediate,
        (XtPointer) "fixed",
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, mode),
        XmRImmediate,
        (XtPointer) "scientific",
    },

    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationArgsPtr, trigType),
        XmRImmediate,
        (XtPointer) "degrees",
    },
};
...
int main(int argc, char **argv) {
  ...
  X->kframe = XtVaAppInitialize(&X->app, DTCALC_CLASS_NAME, /* app class */
                                NULL,                       /* options list */
                                0,                          /* num options */
                                &argc, argv, NULL, /* fallback resources */
                                XtNiconName, "dtcalc", XtNiconPixmap, X->icon,
                                XtNiconMask, X->iconmask, NULL);
  ...
  XtGetApplicationResources(X->kframe, &application_args, resources, 6, NULL,
                            0);
...
void beep(void) { ds_beep(X->dpy); }

static Widget button_create(Widget owner, int row, int column, int maxrows,
                            int maxcols) {
  ...
  if (colorSrv) {
    ...
    button = XtVaCreateManagedWidget(
        buttons[n].resname, xmPushButtonWidgetClass, owner, XmNtopAttachment,
        XmATTACH_POSITION, XmNtopPosition, row * maxcols, XmNleftAttachment,
        XmATTACH_POSITION, XmNleftPosition, column * maxrows,
        XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition,
        (column + 1) * maxrows, XmNbottomAttachment, XmATTACH_POSITION,
        XmNbottomPosition, (row + 1) * maxcols, XmNlabelString, lstr,
        XmNbackground, bg, XmNtraversalOn, TRUE, XmNalignment,
        XmALIGNMENT_CENTER, XmNrecomputeSize, False, XmNnavigationType, XmNONE,
        NULL);
  } else
    button = XtVaCreateManagedWidget(
        buttons[n].resname, xmPushButtonWidgetClass, owner, XmNtopAttachment,
        ...
        (column + 1) * maxrows, XmNbottomAttachment, XmATTACH_POSITION,
        XmNbottomPosition, (row + 1) * maxcols, XmNlabelString, lstr,
        XmNtraversalOn, TRUE, XmNalignment, XmALIGNMENT_CENTER,
        XmNrecomputeSize, False, XmNnavigationType, XmNONE, NULL);

  XmStringFree(lstr);

  if (mtype != M_NONE)
    create_menu(mtype, button, n);
  val = (v->curwin << 16) + n;
  XtAddCallback(button, XmNactivateCallback, do_button, (XtPointer)val);
  XtAddCallback(button, XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                (XtPointer)val);
  XtAddEventHandler(button, KeyPressMask | KeyReleaseMask, FALSE, event_proc,
                    NULL);

  if (funBtn == NULL && strcmp(v->pstr, GETMESSAGE(3, 5, "Functions")) == 0)
    funBtn = button;
  else if (constBtn == NULL &&
           strcmp(v->pstr, GETMESSAGE(3, 6, "Constants")) == 0)
    constBtn = button;
  else if (accBtn == NULL &&
           strcmp(v->pstr, GETMESSAGE(3, 12, "Accuracy")) == 0)
    accBtn = button;
  else if (stoBtn == NULL && strcmp(v->pstr, GETMESSAGE(3, 13, "Store")) == 0)
    stoBtn = button;
  else if (rclBtn == NULL && strcmp(v->pstr, GETMESSAGE(3, 14, "Recall")) == 0)
    rclBtn = button;
  else if (exchBtn == NULL &&
           strcmp(v->pstr, GETMESSAGE(3, 15, "Exchange")) == 0)
    exchBtn = button;

  return (button);
}

static void dtcalc_initialize_rframe(Widget owner, int type) {
  char str[MAXLINE];
  int i;
  Arg args[15];
  ...
  if (type == MEM) {
    ...
    XtVaSetValues(form, XmNmarginWidth, 5, XmNmarginHeight, 5,
                  XmNallowShellResize, TRUE, XmNdefaultPosition, FALSE, NULL);

    sprintf(str, "register%1d", 0);
    X->registers[0] = XtVaCreateManagedWidget(
        str, xmLabelWidgetClass, form, XmNtopAttachment, XmATTACH_FORM,
        XmNtopOffset, 2, XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 2,
        XmNmarginWidth, 5, XmNmarginHeight, 1, NULL);

    for (i = 1; i < MAXREGS; i++) {
      sprintf(str, "register%1d", i);
      X->registers[i] = XtVaCreateManagedWidget(
          str, xmLabelWidgetClass, form, XmNtopAttachment, XmATTACH_WIDGET,
          XmNtopWidget, X->registers[i - 1], XmNtopOffset, 2, XmNleftAttachment,
          XmATTACH_FORM, XmNleftOffset, 2, XmNmarginWidth, 5, XmNmarginHeight,
          1, NULL);
    }
    ...
    XtAddCallback(button, XmNactivateCallback, close_reg,
                  (XtPointer)(intptr_t)type);
    ...
    X->fregisters[0] = XtVaCreateManagedWidget(
        str, xmLabelWidgetClass, form, XmNtopAttachment, XmATTACH_FORM,
        XmNtopOffset, 2, XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 2,
        XmNmarginWidth, 5, XmNmarginHeight, 1, NULL);
    ...
    for (i = 1; i < FINREGS; i++) {
      ...
      X->fregisters[i] = XtVaCreateManagedWidget(
          str, xmLabelWidgetClass, form, XmNtopAttachment, XmATTACH_WIDGET,
          XmNtopWidget, X->fregisters[i - 1], XmNtopOffset, 2,
          XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 2, XmNmarginWidth, 5,
          XmNmarginHeight, 1, NULL);
    }

    sprintf(str, "fregistervals%1d", 0);
    X->fregistersvals[0] = XtVaCreateManagedWidget(
        str, xmLabelWidgetClass, form, XmNtopAttachment, XmATTACH_FORM,
        XmNtopOffset, 2, XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
        X->fregisters[0], XmNleftOffset, 0, XmNrightAttachment, XmATTACH_FORM,
        XmNrightOffset, 2, XmNmarginWidth, 5, XmNmarginHeight, 1, NULL);

    for (i = 1; i < FINREGS; i++) {
      sprintf(str, "fregistervals%1d", i);
      X->fregistersvals[i] = XtVaCreateManagedWidget(
          str, xmLabelWidgetClass, form, XmNtopAttachment, XmATTACH_WIDGET,
          XmNtopWidget, X->fregistersvals[i - 1], XmNtopOffset, 2,
          XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, X->fregisters[i],
          XmNleftOffset, 0, XmNrightAttachment, XmATTACH_FORM, XmNrightOffset,
          2, XmNmarginWidth, 5, XmNmarginHeight, 1, NULL);
    }
...
static void dtcalc_kkeyboard_create(Widget owner) {
  int column, row;
  Widget buttonFrame;

  buttonFrame = XtVaCreateManagedWidget(
      "buttonFrame", xmFrameWidgetClass, owner, XmNshadowThickness, 0,
      XmNmarginWidth, 0, XmNmarginHeight, 0, XmNtopAttachment, XmATTACH_WIDGET,
      XmNtopWidget, X->modeFrame, XmNrightAttachment, XmATTACH_FORM,
      XmNleftAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM,
      NULL);

  X->kkeyboard = XtVaCreateManagedWidget(
      "kkeyboard", xmFormWidgetClass, buttonFrame, XmNfractionBase,
      BROWS * BCOLS, XmNnavigationType, XmSTICKY_TAB_GROUP, NULL);

  v->curwin = FCP_KEY;
  for (row = 0; row < BROWS; row++)
    for (column = 0; column < BCOLS; column++) {
      X->kbuttons[row][column] =
          button_create(X->kkeyboard, row, column, BROWS, BCOLS);
      XtManageChild(X->kbuttons[row][column]);
    }
  grey_buttons(v->base);
}

static void dtcalc_kpanel_create(Widget owner) {
  static char *mnames[] = {"base", "ttype", "num", "hyp", "inv", "op", "mode"};
  int i, n, val;
  ...
  X->mainWin =
      XtVaCreateManagedWidget("mainWin", xmMainWindowWidgetClass, owner, NULL);

#ifdef HAVE_EDITRES
  XtAddEventHandler(owner, 0, True, (XtEventHandler)_XEditResCheckMessages,
                    (XtPointer)NULL);
#endif
  ...
  X->kFrame = XtVaCreateManagedWidget(
      "kFrame", xmFrameWidgetClass, X->mainWin, XmNshadowThickness, 1,
      XmNshadowType, XmSHADOW_OUT, XmNmarginWidth, 5, XmNmarginHeight, 5, NULL);

  X->kpanel = XtVaCreateManagedWidget(
      "kpanel", xmFormWidgetClass, X->kFrame, XmNshadowThickness, 0,
      XmNbackground, tmp_pixelbg, XmNforeground, tmp_pixelfg, NULL);

  create_popup(X->kpanel);

  X->textFrame = XtVaCreateManagedWidget(
      "textFrame", xmFrameWidgetClass, X->kpanel, XmNshadowThickness, 2,
      XmNshadowType, XmSHADOW_IN, XmNmarginWidth, 0, XmNmarginHeight, 0,
      XmNtopAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
      XmNleftAttachment, XmATTACH_FORM, XmNnavigationType, XmTAB_GROUP, NULL);
  XtAddCallback(X->textFrame, XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                (XtPointer)HELP_DISPLAY);

  X->textForm = XtVaCreateManagedWidget(
      "textForm", xmFormWidgetClass, X->textFrame, XmNshadowThickness, 0,
      XmNbackground, tmp_pixelbg, XmNforeground, tmp_pixelfg, NULL);
  XtAddCallback(X->textForm, XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                (XtPointer)HELP_DISPLAY);

  X->modevals[(int)DISPLAYITEM] = XtVaCreateManagedWidget(
      "display", xmTextWidgetClass, X->textForm, XmNtopAttachment,
      XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment,
      XmATTACH_FORM, XmNresizeWidth, TRUE, XmNshadowThickness, 0,
      XmNhighlightThickness, 0, XmNeditable, FALSE, XmNverifyBell, FALSE,
      XmNbackground, tmp_pixelbg, XmNforeground, tmp_pixelfg, NULL);
  XtAddCallback(X->modevals[(int)DISPLAYITEM], XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                (XtPointer)HELP_DISPLAY);
  XtAddEventHandler(X->modevals[(int)DISPLAYITEM],
                    KeyPressMask | KeyReleaseMask, FALSE, event_proc, NULL);

  X->modeFrame = XtVaCreateManagedWidget(
      "modeFrame", xmFrameWidgetClass, X->kpanel, XmNshadowThickness, 0,
      XmNmarginWidth, 0, XmNmarginHeight, 0, XmNtopAttachment, XmATTACH_WIDGET,
      XmNtopWidget, X->textFrame, XmNrightAttachment, XmATTACH_FORM,
      XmNleftAttachment, XmATTACH_FORM, XmNnavigationType, XmTAB_GROUP, NULL);
  XtAddCallback(X->modeFrame, XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                (XtPointer)HELP_MODELINE);

  X->modeline = XtVaCreateManagedWidget(
      "modeline", xmFormWidgetClass, X->modeFrame, XmNshadowThickness, 0, NULL);
  XtAddCallback(X->modeline, XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                (XtPointer)HELP_MODELINE);

  label_string = XmStringCreateLocalized("                        ");
  i = (int)OPITEM;
  X->modevals[i] = XtVaCreateManagedWidget(
      mnames[i], xmLabelWidgetClass, X->modeline, XmNtopAttachment,
      XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNrecomputeSize, False,
      XmNalignment, XmALIGNMENT_CENTER, XmNlabelString, label_string, NULL);
  XtAddCallback(X->modevals[i], XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                (XtPointer)HELP_MODELINE);

  i = (int)HYPITEM;
  X->modevals[i] = XtVaCreateManagedWidget(
      mnames[i], xmLabelWidgetClass, X->modeline, XmNrightAttachment,
      XmATTACH_WIDGET, XmNrightWidget, X->modevals[(int)OPITEM],
      XmNtopAttachment, XmATTACH_FORM, XmNrecomputeSize, False, XmNalignment,
      XmALIGNMENT_CENTER, XmNlabelString, label_string, NULL);
  XtAddCallback(X->modevals[i], XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                (XtPointer)HELP_MODELINE);
  ...
  X->modevals[i] = XtVaCreateManagedWidget(
      mnames[i], xmLabelWidgetClass, X->modeline, XmNrightAttachment,
      XmATTACH_WIDGET, XmNrightWidget, X->modevals[(int)HYPITEM],
      XmNtopAttachment, XmATTACH_FORM, XmNrecomputeSize, False, XmNalignment,
      XmALIGNMENT_CENTER, XmNlabelString, label_string, NULL);
  XtAddCallback(X->modevals[i], XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
...

  {
    ...
    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_MODE].mindex + 1].val;
    ...
    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_MODE].mindex + 2].val;
    ...
    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_MODE].mindex + 3].val;
    ...
    n++;
    XtSetArg(args[n], XmNtopWidget, X->modevals[(int)HYPITEM]);
    n++;
    XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNsubMenuId, modePulldown);
    n++;
    XtSetArg(args[n], XmNmenuHistory, modeArry[(int)v->modetype]);
    n++;
    ...
    XtAddEventHandler(X->modevals[i], KeyPressMask | KeyReleaseMask, FALSE,
                      event_proc, NULL);

    i = (int)BASEITEM;
    basePulldown = XmCreatePulldownMenu(X->modeline, "basePD", args, 0);

    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_BASE].mindex].val;
    ...
    XmStringFree(label_string);
    XtAddCallback(X->baseWidgArry[0], XmNactivateCallback, modelineValueChanged,
                  (XtPointer)M_BASE);

    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_BASE].mindex + 1].val;
    label_string = XmStringCreateLocalized(GETMESSAGE(2, 2, "Oct"));
    XtSetArg(args[2], XmNlabelString, label_string);
    XtSetArg(args[3], XmNuserData, val);
    X->baseWidgArry[1] = XmCreatePushButtonGadget(basePulldown, "oct", args, 4);
    XmStringFree(label_string);
    XtAddCallback(X->baseWidgArry[1], XmNactivateCallback, modelineValueChanged,
                  (XtPointer)M_BASE);

    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_BASE].mindex + 2].val;
    label_string = XmStringCreateLocalized(GETMESSAGE(2, 3, "Dec"));
    XtSetArg(args[2], XmNlabelString, label_string);
    XtSetArg(args[3], XmNuserData, val);
    X->baseWidgArry[2] = XmCreatePushButtonGadget(basePulldown, "dec", args, 4);
    XmStringFree(label_string);
    XtAddCallback(X->baseWidgArry[2], XmNactivateCallback, modelineValueChanged,
                  (XtPointer)M_BASE);

    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_BASE].mindex + 3].val;
    label_string = XmStringCreateLocalized(GETMESSAGE(2, 4, "Hex"));
    XtSetArg(args[2], XmNlabelString, label_string);
    XtSetArg(args[3], XmNuserData, val);
    X->baseWidgArry[3] = XmCreatePushButtonGadget(basePulldown, "hex", args, 4);
    XmStringFree(label_string);
    XtAddCallback(X->baseWidgArry[3], XmNactivateCallback, modelineValueChanged,
                  (XtPointer)M_BASE);
    ...
    XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNsubMenuId, basePulldown);
    n++;
    XtSetArg(args[n], XmNmenuHistory, X->baseWidgArry[(int)v->base]);
    n++;
    ...
    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_NUM].mindex].val;
    ...
    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_NUM].mindex + 1].val;
    ...
    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_NUM].mindex + 2].val;
    ...
    XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNsubMenuId, numPulldown);
    n++;
    XtSetArg(args[n], XmNmenuHistory, X->numWidgArry[(int)v->dtype]);
    n++;
    ...
    XtSetArg(args[0], XmNmarginHeight, 0);
    XtSetArg(args[1], XmNmarginWidth, 0);
    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_TRIG].mindex].val;
    ...
    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_TRIG].mindex + 1].val;
    ...
    val = (v->curwin << 16) + menu_entries[cmenus[(int)M_TRIG].mindex + 2].val;
    ...
    XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNsubMenuId, trigPulldown);
    n++;
    XtSetArg(args[n], XmNmenuHistory, X->ttypeWidgArry[(int)v->ttype]);
    n++;
...
/*ARGSUSED*/
static void confirm_callback(Widget widget, XtPointer client_data,
                             XtPointer call_data) {
  update_cf_value();
}

static void create_cfframe(void) /* Create auxiliary frame for CON/FUN key. */
{
  int j;
  XmString tstr;
  Arg args[15];
  ...
  XtVaSetValues(X->CFframe, XmNdialogTitle, tstr, XmNautoUnmanage, FALSE,
                XmNallowShellResize, FALSE, XmNdefaultPosition, FALSE, NULL);
  XmStringFree(tstr);

  /*  Adjust the decorations for the dialog shell of the dialog  */
  j = 0;
  XtSetArg(args[j], XmNmwmFunctions, MWM_FUNC_MOVE);
  j++;
  XtSetArg(args[j], XmNmwmDecorations, MWM_DECOR_BORDER | MWM_DECOR_TITLE);
  j++;
  ...
  j = 0;
  XtSetArg(args[j], XmNtopAttachment, XmATTACH_FORM);
  j++;
  XtSetArg(args[j], XmNtopOffset, 5);
  j++;
  XtSetArg(args[j], XmNrightAttachment, XmATTACH_FORM);
  j++;
  XtSetArg(args[j], XmNrightOffset, 5);
  j++;
  XtSetValues(X->CFpi_cftext->manager, args, j);

  j = 0;
  XtSetArg(args[j], XmNtopAttachment, XmATTACH_WIDGET);
  j++;
  XtSetArg(args[j], XmNtopWidget, X->CFpi_cftext->manager);
  j++;
  XtSetArg(args[j], XmNtopOffset, 5);
  j++;
  XtSetArg(args[j], XmNrightAttachment, XmATTACH_FORM);
  ...
  XtSetArg(args[j], XmNtopAttachment, XmATTACH_WIDGET);
  j++;
  XtSetArg(args[j], XmNtopWidget, X->CFpi_dtext->manager);
  j++;
  XtSetArg(args[j], XmNtopOffset, 5);
  j++;
  XtSetArg(args[j], XmNrightAttachment, XmATTACH_FORM);
  ...
  XtSetArg(args[j], XmNtopAttachment, XmATTACH_WIDGET);
  j++;
  XtSetArg(args[j], XmNtopWidget, X->CFpi_vtext->manager);
  j++;
  XtSetArg(args[j], XmNtopOffset, 3);
  j++;
  XtSetArg(args[j], XmNleftAttachment, XmATTACH_FORM);
  j++;
  XtSetArg(args[j], XmNrightAttachment, XmATTACH_FORM);
  j++;
  ...
  XtSetArg(args[j], XmNmarginHeight, 2);
  j++;
  XtSetArg(args[j], XmNmarginWidth, 15);
  j++;
  XtSetArg(args[j], XmNlabelString, label_string);
  j++;
  XtSetArg(args[j], XmNtopAttachment, XmATTACH_WIDGET);
  j++;
  XtSetArg(args[j], XmNtopWidget, sep);
  j++;
  XtSetArg(args[j], XmNtopOffset, 5);
  j++;
  XtSetArg(args[j], XmNleftAttachment, XmATTACH_FORM);
  j++;
  XtSetArg(args[j], XmNleftOffset, 15);
  j++;
  XtSetArg(args[j], XmNbottomAttachment, XmATTACH_FORM);
  j++;
  XtSetArg(args[j], XmNbottomOffset, 5);
  j++;
  XtSetArg(args[j], XmNshowAsDefault, True);
  j++;
  XtSetArg(args[j], XmNtraversalOn, True);
  j++;
  ...
  XtSetArg(args[j], XmNmarginHeight, 2);
  j++;
  XtSetArg(args[j], XmNmarginWidth, 10);
  j++;
  XtSetArg(args[j], XmNlabelString, label_string);
  j++;
  XtSetArg(args[j], XmNtopAttachment, XmATTACH_WIDGET);
  ...
  XtSetArg(args[j], XmNtopWidget, sep);
  j++;
  XtSetArg(args[j], XmNtopOffset, 10);
  j++;
  XtSetArg(args[j], XmNleftAttachment, XmATTACH_WIDGET);
  j++;
  XtSetArg(args[j], XmNleftWidget, X->CFpi_butOK);
  j++;
  XtSetArg(args[j], XmNleftOffset, 33);
  j++;
  XtSetArg(args[j], XmNbottomAttachment, XmATTACH_FORM);
  j++;
  XtSetArg(args[j], XmNbottomOffset, 10);
  j++;
  XtSetArg(args[j], XmNtraversalOn, True);
  j++;
  ...
  XtSetArg(args[j], XmNrightAttachment, XmATTACH_FORM);
  j++;
  XtSetArg(args[j], XmNrightOffset, 15);
  j++;
  XtSetArg(args[j], XmNleftAttachment, XmATTACH_WIDGET);
  j++;
  XtSetArg(args[j], XmNleftWidget, X->CFpi_butClose);
  j++;
  XtSetArg(args[j], XmNleftOffset, 33);
  j++;
  XtSetArg(args[j], XmNbottomAttachment, XmATTACH_FORM);
  ...
  XtAddCallback(X->CFpi_dtext->textfield, XmNactivateCallback, FocusInCB,
                (XtPointer)NULL);
  j = 0;
  XtSetArg(args[j], XmNcancelButton, X->CFpi_butClose);
  j++;
  XtSetValues(X->CFframe, args, j);

  j = 0;
  XtSetArg(args[j], XmNdefaultButton, X->CFpi_butOK);
  j++;
...
static void create_menu(enum menu_type mtype, Widget button, int n) {
  ...
  if ((mtype != M_CON && mtype != M_FUN) && X->menus[(int)mtype] != NULL)
    return; /* Already created? */

  X->menus[(int)mtype] = menu = XmCreatePopupMenu(button, "menu", NULL, 0);

  X->mrec[(int)mtype] = n;

  XtCreateManagedWidget(cmenus[(int)mtype].title, xmLabelWidgetClass, menu,
                        NULL, 0);
  ...
  for (i = 0; i < cmenus[(int)mtype].total; i++) {
    ...
    switch (mtype) {
    case M_CON:
      if (strlen(v->con_names[i])) /* Constants. */
        mstr = v->con_names[i];
      else
        invalid = 1;
      break;
    case M_FUN:
      if (strlen(v->fun_names[i])) /* Functions. */
      {
        isAFunction = True;
        mstr = v->fun_names[i];
      } else
      ...
      break;
    default:
      mstr = menu_entries[cmenus[(int)mtype].mindex + i].str;
    }
    if (!invalid) {
      ...
      mi = XtCreateManagedWidget(mstr, xmPushButtonWidgetClass, menu, NULL, 0);
      val = (v->curwin << 16) + menu_entries[cmenus[(int)mtype].mindex + i].val;
...
/*ARGSUSED*/
static void do_button(Widget widget, XtPointer client_data,
                      XtPointer call_data) {
  char *str;
  XmString cstr;
  Arg args[3];
  ...
  str = (char *)_XmStringUngenerate(cstr, XmFONTLIST_DEFAULT_TAG,
...
/*ARGSUSED*/
static void do_continue_notice(Widget parent, char *str) {
  XmString contstr, message, cstr;
  char *tmpStr;

  X->notice = (Widget)XmCreateInformationDialog(X->kframe, "continue", NULL, 0);
  contstr = XmStringCreateLocalized(vstrs[(int)V_CONTINUE]);
  message = XmStringCreateLocalized(str);
  XtVaSetValues(X->notice, XmNautoUnmanage, FALSE, XmNcancelLabelString,
                contstr, XmNmessageString, message, XmNdialogStyle,
                XmDIALOG_FULL_APPLICATION_MODAL, NULL);
  XmStringFree(contstr);
  XmStringFree(message);
  XtUnmanageChild((Widget)XmMessageBoxGetChild(X->notice, XmDIALOG_OK_BUTTON));
  XtUnmanageChild(
      (Widget)XmMessageBoxGetChild(X->notice, XmDIALOG_HELP_BUTTON));
...
/*ARGSUSED*/
static void do_confirm_notice(Widget parent, char *str) {
  ...
  message = XmStringCreateLocalized(str);
  XtVaSetValues(X->notice, XmNautoUnmanage, FALSE, XmNcancelLabelString, cancel,
                XmNokLabelString, confirm, XmNmessageString, message,
...
/*ARGSUSED*/
static void close_reg(Widget widget, XtPointer client_data,
                      XtPointer call_data) {
 ...
 *
 */
void draw_button(int n, enum fcp_type fcptype, int row, int column,
                 int inverted) {
  char str[10];
  Widget widget;
...
static int event_is_keypad(XEvent *xevent) {
 ...
 */
/*ARGSUSED*/
static void event_proc(Widget widget, XtPointer client_data, XEvent *event,
  ...
  */
  ...
  str = (char *)_XmStringUngenerate(label, XmFONTLIST_DEFAULT_TAG,
                                    XmCHARSET_TEXT, XmCHARSET_TEXT);
  ...
  */
  if (X->CFframe != NULL && XtIsManaged(X->CFframe)) {
    ...
    if (w == X->CFpi_butOK) {
      if (v->cur_ch == KEY_EQ) {
        ...
        XtCallCallbacks(X->CFpi_butOK, XmNactivateCallback, (XtPointer)NULL);
      }
      ...
      if (v->cur_ch == KEY_EQ) {
        ...
        XtCallCallbacks(X->CFpi_butClose, XmNactivateCallback, (XtPointer)NULL);
      }
      if (v->event_type == ARROW || v->event_type == TAB) {
  ...
  if (v->pending == 0) {
    ...
    switch (v->base) {
    case BIN:
      if (v->cur_ch == KEY_2 || v->cur_ch == KEY_3 || v->cur_ch == KEY_4 ||
          v->cur_ch == KEY_5 || v->cur_ch == KEY_6 || v->cur_ch == KEY_7)
        return;
      /* intentional fall through */
    case OCT:
      if (v->cur_ch == KEY_8 || v->cur_ch == KEY_9)
        return;
      /* intentional fall through */
    case DEC:
      if (v->cur_ch == KEY_A || v->cur_ch == KEY_B || v->cur_ch == KEY_C ||
          v->cur_ch == KEY_D || v->cur_ch == KEY_E || v->cur_ch == KEY_F)
        return;
      /* intentional fall through */
...
static void ProcessMotifSelection(Widget W) {
  char key;
  int index;
  long i, bound;
  char *display = NULL;
...
/*ARGSUSED*/
static void frame_interpose(Widget widget, XtPointer clientData, XEvent *event,
                            Boolean *continue_to_dispatch) {
  if (!v->started)
    return;
...
/*ARGSUSED*/
static int get_next_event(Widget widget, int ev_action, XEvent *xevent) {
  char *tmpStr, chs[2];
  ...
  if (nextc == KeyPress || nextc == KeyRelease) {
     ...
     */
    if (!XLookupString(key_event, chs, 1, &ksym, (XComposeStatus *)NULL)) {
    ...
    */
    if (ksym == XK_F4 && down)
      ...
      if (!event_is_keypad(xevent))
        return (ARROW);
    } else if (ksym == XK_F10 && down)
      return (F4_PRESS);
    else if (ksym == XK_F10 && up)
      return (F4_PRESS);
    else if (ksym == XK_Tab && down)
      return (TAB);
    else if (ksym == XK_Tab && up)
      return (TAB);
    else if (ksym == XK_Return && down)
      cval = KEY_EQ;
    else if (ksym == XK_Return && up)
      cval = KEY_EQ;
    else if (ksym == XK_Escape && down)
      return (ESCAPE);
    else if (ksym == XK_Escape && up)
      return (ESCAPE);
    else if (ksym == XK_Control_L && down)
      return (CONTROL);
    else if (ksym == XK_Control_L && up)
      return (CONTROL);
    else if (ksym == XK_Control_R && down)
      return (CONTROL);
    else if (ksym == XK_Control_R && up)
      return (CONTROL);
    else if (ksym == XK_Meta_L && down)
      return (META);
    else if (ksym == XK_Meta_L && up)
      return (META);
    else if (ksym == XK_Meta_R && down)
      return (META);
    else if (ksym == XK_Meta_R && up)
      return (META);
    else if (ksym == XK_Alt_L && down)
      return (ALT);
    else if (ksym == XK_Alt_L && up)
      return (ALT);
    else if (ksym == XK_Alt_R && down)
      return (ALT);
    else if (ksym == XK_Alt_R && up)
      return (ALT);
    else if (ksym == XK_Select && down)
      return (SPACE);
    else if (ksym == XK_Select && up)
      return (SPACE);
    else if (ksym == XK_space && down)
      return (SPACE);
    else if (ksym == XK_space && up)
      return (SPACE);
...
char *get_resource(enum res_type rtype) {
  char str[MAXLINE];
...
void grey_button(int row, int column, int state) {
...
void init_graphics(void) {
  ...
  XtVaSetValues(X->kframe, XmNiconName, tmpStr1, XmNtitle, tmpStr1, NULL);
...
static int is_window_showing(Widget widget) { return (XtIsManaged(widget)); }
...
static KeySym keypad_keysym(XEvent *xevent) {
  int i;
...
void load_resources(void) { X->rDB = ds_load_resources(X->dpy); }
...
void make_frames(void) {
  ...
  XtAddEventHandler(X->kframe, StructureNotifyMask, FALSE, frame_interpose,
                    NULL);
  XtVaSetValues(X->kframe, XmNtitle, tool_label, NULL);

  XtVaGetValues(X->kframe, XmNdepth, &depth, NULL);

  if (v->titleline == NULL)
    free(tool_label);
}

void make_modewin(void) /* Draw special mode frame plus buttons. */
{
  ...
  if (type == MEM) {
    ...
    for (i = 0; i < FINREGS; i++) {
      ...
      */
      ...
      XtVaSetValues(X->fregistersvals[i], XmNlabelString, numStr, XmNalignment,
...
/*ARGSUSED*/
static void menu_handler(Widget widget, XtPointer client_data, XEvent *event,
                         Boolean *continue_to_dispatch) {
  int button;
  Widget menu;
...
/*ARGSUSED*/
void menu_proc(Widget widget, XtPointer client_data, XtPointer call_data) {
...
/*ARGSUSED*/
static void new_cf_value(Widget widget, XtPointer client_data,
                         XtPointer call_data) {
  /*
  ...
  if (mtype == M_CON) {
    ...
    XtAddCallback(X->CFpi_butHelp, XmNactivateCallback,
                  (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                  (XtPointer)HELP_CONSTANT);
  } else {
    X->CFtype = M_FUN;
...
void put_resource(enum res_type rtype, char *value) {
...
void save_resources(char *filename) {
  int reply = ds_save_resources(X->dtcalcDB, filename);

  if (reply)
    _DtSimpleError(v->appname, DtWarning, NULL, vstrs[(int)V_NORSAVE]);
}

void ErrorDialog(char *string) { ErrDialog(string, X->mainWin); }

void set_item(enum item_type itemno, char *str) {
  Widget w;
  XmString cstr;
...
void set_title(enum fcp_type fcptype, char *str) {
  Widget w;
  XmString cstr;
...
/*ARGSUSED*/
void show_ascii(Widget widget, XtPointer client_data, XtPointer call_data) {
  char *str;
  int val;
...
void show_ascii_frame(void) /* Display ASCII popup. */
{
  int j;
  XmString tstr;
  Widget sep;
  ...
  if (X->Aframe == NULL) {
    ...
    XtVaSetValues(X->Aframe, XmNdialogTitle, tstr, XmNautoUnmanage, FALSE,
                  XmNallowShellResize, TRUE, XmNdefaultPosition, FALSE, NULL);
    XmStringFree(tstr);
    ...
    XtAddCallback(X->Api_text->textfield, XmNactivateCallback, show_ascii,
                  NULL);
    ...
    XtAddCallback(X->Api_butClose, XmNactivateCallback, close_ascii,
                  (XtPointer)NULL);
    XtAddCallback(X->Api_butHelp, XmNactivateCallback,
                  (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
                  (XtPointer)HELP_ASCII);

    j = 0;
    XtSetArg(args[j], XmNcancelButton, X->Api_butClose);
    j++;
    XtSetArg(args[j], XmNdefaultButton, X->Api_butOK);
    j++;
...
void clear_buttons(int start) {
  ...
  if (first) {
    ...
    XtSetArg(args[n], XmNlabelString, lstr);
    n++;
    if (colorSrv) {
      XtSetArg(args[n], XmNbackground, pixels[1].bg);
      n++;
      XtSetArg(args[n], XmNforeground, pixels[1].fg);
      n++;
      XtSetArg(args[n], XmNtopShadowColor, pixels[1].ts);
      n++;
      XtSetArg(args[n], XmNbottomShadowColor, pixels[1].bs);
      n++;
      XtSetArg(args[n], XmNarmColor, pixels[1].sc);
      n++;
    }
...
void make_buttons_fin(void) {
  ...
  for (i = 12; i < 24; i++) {
    ...
    n++;
    if (colorSrv) {
      XtSetArg(args[n], XmNbackground, pixels[7].bg);
      n++;
      XtSetArg(args[n], XmNforeground, pixels[7].fg);
      n++;
      XtSetArg(args[n], XmNtopShadowColor, pixels[7].ts);
      n++;
      XtSetArg(args[n], XmNbottomShadowColor, pixels[7].bs);
      n++;
      XtSetArg(args[n], XmNarmColor, pixels[7].sc);
      n++;
    }
    XtSetValues(X->kbuttons[row][column], args, n);
    XmStringFree(lstr);
...
static void switch_mode(enum mode_type curmode) {
  Arg args[2];
...
static void update_cf_value(void) {
  ...
  case M_CON:
    ...
    if (n != 1 || ptr != NULL || v->error == TRUE) {
      sprintf(message, "%s\n%s", vstrs[(int)V_INVCON], vstrs[(int)V_NOCHANGE]);
    ...
    if (strncmp(result, "0.", 2) == 0 || strncmp(result, "-0.", 3) == 0) {
      len = strlen(result);
    ...
    sprintf(v->con_names[X->cfno], "%1d: %s [%s]", X->cfno, result, X->dval);
    break;
  case M_FUN:
    tmpStr = GETMESSAGE(3, 45, ".");
    if (strcmp(tmpStr, ".") != 0) {
    ...
    if (strcmp(X->vval, "") != 0) {
      sprintf(v->fun_names[X->cfno], "%1d: %s [%s]", X->cfno, X->vval, X->dval);
    } else
  ...
  switch (X->CFtype) {
  case M_CON:
    write_rcfile(X->CFtype, X->cfexists, X->cfno, result, X->dval);
    break;
  case M_FUN:
    write_rcfile(X->CFtype, X->cfexists, X->cfno, X->vval, X->dval);
    break;
  default:
    break;
  }
...
void win_display(enum fcp_type fcptype, int state) {
  Widget widget = NULL;
  Position newX, newY;
  Arg args[3];
  ...
  if (fcptype == FCP_REG)
    ...
    widget = X->mframe[(int)v->modetype];
  else if (fcptype == FCP_FIN_REG)
    widget = X->frframe;
...
/*ARGSUSED*/
void write_cf_value(Widget widget, XtPointer client_data, XtPointer call_data) {
  char message[MAXLINE];
  char str[MAXLINE]; /* Temporary buffer for various strings. */
  Widget focus_widget;
  Arg args[1];
  ...
  */
  ...
  if ((strcmp(X->cfval, "") == 0) || X->cfval[0] < '0' || X->cfval[0] > '9' ||
      X->cfno < 0 || X->cfno > 9) {
    snprintf(str, MAXLINE, "%s",
             (X->CFtype == M_CON) ? vstrs[(int)V_LCON] : vstrs[(int)V_LFUN]);
  ...
  switch (X->CFtype) {
  case M_CON:
    X->cfexists = 1; /* Always the default constants. */
    break;
  case M_FUN:
    if (strlen(v->fun_vals[X->cfno]))
      X->cfexists = 1;
    break;
  default:
    break;
  ...
  if (X->cfexists) {
    snprintf(str, MAXLINE, mess[(int)MESS_CON],
             (X->CFtype == M_CON) ? vstrs[(int)V_UCON] : vstrs[(int)V_UFUN],
             X->cfno);
...
static int xerror_interpose(Display *display, XErrorEvent *error) {
  ...
  sprintf(
      ...
      msg1, error->request_code, error->minor_code, error->resourceid,
      error->serial);
  _DtSimpleError(v->appname, DtError, NULL, msg);
  abort();
}

static void modelineValueChanged(Widget widget, XtPointer client_data,
                                 XtPointer call_data) {
  Arg args[1];
  int choice;
  XtArgVal val;
...
static void create_menu_bar(Widget parent) {
  ...
  XtAddCallback(X->menubar, XmNhelpCallback,
                (void (*)(Widget, XtPointer, XtPointer))HelpRequestCB,
  ...
  n++;
  XtSetArg(args[n], XmNmnemonic, XStringToKeysym(mnemonic));
  n++;
  ...
  mnemonic = XtNewString(mnemonic);
  labelString = XmStringCreateLocalized(GETMESSAGE(2, 15, "Memory Registers"));
  ...
  XtSetArg(args[n], XmNlabelString, labelString);
  n++;
  WidgList[count++] = XmCreatePushButton(helpPulldown, "introduction", args, n);
  ...
  WidgList[count++] =
      (Widget)XmCreateSeparatorGadget(helpPulldown, "separator", args, 0);
  ...
  XtSetArg(args[n], XmNmnemonic, XStringToKeysym(mnemonic));
  n++;
  labelString = XmStringCreateLocalized(GETMESSAGE(2, 39, "Table of Contents"));
  XtSetArg(args[n], XmNlabelString, labelString);
  n++;
  ...
  XtSetArg(args[n], XmNlabelString, labelString);
  n++;
  WidgList[count++] = XmCreatePushButton(helpPulldown, "usingHelp", args, n);
  XtAddCallback(WidgList[count - 1], XmNactivateCallback,
  ...
  XtSetArg(args[n], XmNmnemonic, XStringToKeysym(mnemonic));
  n++;
  labelString = XmStringCreateLocalized(GETMESSAGE(2, 30, "About Calculator"));
  XtSetArg(args[n], XmNlabelString, labelString);
  n++;
...
static void init_colors(void) {
  ...
  XmeGetPixelData(X->screen, &colorUse, pixels, &act, &inact, &prim, &second);
...
static void do_memory(Widget w, XtPointer client_data, XtPointer call_data) {
  intptr_t type = (intptr_t)client_data;
...
void read_resources(void) /* Read all possible resources from the database. */
{
  char str[MAXLINE];
  char *msg;
...
void close_cf(Widget widget, XtPointer client_data, XtPointer call_data) {
  Arg args[1];
...
void close_ascii(Widget widget, XtPointer client_data, XtPointer call_data) {
...
void FocusInCB(Widget widget, XtPointer client_data, XtPointer call_data) {
  Arg args[1];
...
void move_cf(Widget widget, XtPointer client_data, XtPointer call_data) {
  char *input;
  intptr_t value;
  Arg args[3];
...
static void create_popup(Widget parent) {
  ...
  XtCreateManagedWidget("separator", xmSeparatorWidgetClass, X->popupMenu, NULL,
                        0);
  XtCreateManagedWidget("separator", xmSeparatorWidgetClass, X->popupMenu, NULL,
                        0);
  ...
  /* 1.  Memory Registers ...  */
  memRegs = XtVaCreateManagedWidget(
      "memRegs1", xmPushButtonWidgetClass, X->popupMenu, XmNlabelString,
      XmStringCreateLocalized(GETMESSAGE(2, 15, "Memory Registers")),
      XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 40, "R")), NULL);
  XtAddCallback(memRegs, XmNactivateCallback, (XtCallbackProc)do_memory,
                (XtPointer)MEM);

  /* 2.  Finacial Registers ...  */
  memRegs = XtVaCreateManagedWidget(
      "memRegs2", xmPushButtonWidgetClass, X->popupMenu, XmNlabelString,
      XmStringCreateLocalized(GETMESSAGE(3, 138, "Financial Registers")),
      XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 45, "g")), NULL);
  XtAddCallback(memRegs, XmNactivateCallback, (XtCallbackProc)do_memory,
                (XtPointer)FIN);

  /* 3.  Ascii Converter ...  */
  X->asciiConv = XtVaCreateManagedWidget(
      "asciiConv", xmPushButtonWidgetClass, X->popupMenu, XmNlabelString,
      XmStringCreateLocalized(GETMESSAGE(2, 16, "ASCII Convert")), XmNmnemonic,
      XStringToKeysym(GETMESSAGE(2, 41, "A")), NULL);
  ...
  /* 4.  Enter Functions ...  */
  X->enterFun = XtVaCreateManagedWidget(
      "enterFun", xmPushButtonWidgetClass, X->popupMenu, XmNlabelString,
      XmStringCreateLocalized(vstrs[(int)V_FUNWNAME]), XmNmnemonic,
      XStringToKeysym(GETMESSAGE(2, 42, "F")), NULL);
  XtAddCallback(X->enterFun, XmNactivateCallback, (XtCallbackProc)new_cf_value,
                (XtPointer)(int)M_FUN);

  /* 5.  Enter Constants ...  */
  X->enterConst = XtVaCreateManagedWidget(
      "enterConst", xmPushButtonWidgetClass, X->popupMenu, XmNlabelString,
      XmStringCreateLocalized(vstrs[(int)V_CONWNAME]), XmNmnemonic,
      XStringToKeysym(GETMESSAGE(2, 43, "C")), NULL);
  XtAddCallback(X->enterConst, XmNactivateCallback,
                (XtCallbackProc)new_cf_value, (XtPointer)(int)M_CON);
  ...
  dummyHelp1 =
      XtVaCreatePopupShell("dummyHelp1", xmMenuShellWidgetClass, X->popupMenu,
                           XmNwidth, 10, XmNheight, 10, NULL);

  dummyHelp2 =
      XtVaCreateWidget("dummyHelp2", xmRowColumnWidgetClass, dummyHelp1,
                       XmNrowColumnType, XmMENU_PULLDOWN, NULL);

  label = XmStringCreateLocalized(GETMESSAGE(2, 20, "Overview"));
  helpI = XtVaCreateManagedWidget(
      "introduction", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString,
      label, XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 19, "v")), NULL);

  XtAddCallback(helpI, XmNactivateCallback, (XtCallbackProc)HelpRequestCB,
                (XtPointer)HELP_INTRODUCTION);
  XmStringFree(label);

  XtCreateManagedWidget("separator", xmSeparatorWidgetClass, dummyHelp2, NULL,
                        0);

  label = XmStringCreateLocalized(GETMESSAGE(2, 39, "Table of Contents"));
  helpToc = XtVaCreateManagedWidget(
      "toc", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString, label,
      XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 38, "C")), NULL);

  XtAddCallback(helpToc, XmNactivateCallback, (XtCallbackProc)HelpRequestCB,
                (XtPointer)HELP_TABLEOFCONTENTS);

  XmStringFree(label);

  label = XmStringCreateLocalized(GETMESSAGE(2, 22, "Tasks"));
  helpT = XtVaCreateManagedWidget(
      "tasks", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString, label,
      XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 21, "T")), NULL);

  XtAddCallback(helpT, XmNactivateCallback, (XtCallbackProc)HelpRequestCB,
                (XtPointer)HELP_TASKS);

  XmStringFree(label);

  label = XmStringCreateLocalized(GETMESSAGE(2, 24, "Reference"));
  helpR = XtVaCreateManagedWidget(
      "reference", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString, label,
      XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 23, "R")), NULL);

  XtAddCallback(helpR, XmNactivateCallback, (XtCallbackProc)HelpRequestCB,
  ...
  helpO = XtVaCreateManagedWidget(
      "onItem", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString, label,
      XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 25, "O")), NULL);

  XtAddCallback(helpO, XmNactivateCallback, (XtCallbackProc)HelpModeCB,
                (XtPointer)NULL);
  ...
  helpU = XtVaCreateManagedWidget(
      "useHelp", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString, label,
      XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 27, "U")), NULL);

  XtAddCallback(helpU, XmNactivateCallback, (XtCallbackProc)HelpRequestCB,
  ...
  helpV = XtVaCreateManagedWidget(
      "version", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString, label,
      XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 29, "A")), NULL);

  XtAddCallback(helpV, XmNactivateCallback, (XtCallbackProc)HelpRequestCB,
  ...
  XtVaCreateManagedWidget("help", xmCascadeButtonGadgetClass, X->popupMenu,
                          XmNsubMenuId, dummyHelp2, XmNmnemonic,
                          XStringToKeysym(mnemonic), XmNlabelString, label,
                          NULL);
  XmStringFree(label);
  ...
  X->Close = XtVaCreateManagedWidget(
      "close", xmPushButtonWidgetClass, X->popupMenu, XmNlabelString, label,
      XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 44, "X")), NULL);
  XmStringFree(label);
...
static void popupHandler(Widget widget, XtPointer client_data, XEvent *event,
                         Boolean *continue_to_dispatch) {
  ...
  if (event->type == ButtonPress && event->xbutton.button == Button3) {
    if (bevent->x >= funBtn->core.x &&
        ...
        bevent->y >= funBtn->core.y + X->modeline->core.height +
                         X->textForm->core.height &&
      ...
      menu_handler(widget, (XtPointer)M_FUN, event, continue_to_dispatch);
               ...
               bevent->y >= constBtn->core.y + X->modeline->core.height +
                                X->textForm->core.height &&
               bevent->y <= constBtn->core.y + constBtn->core.height +
                                X->modeline->core.height +
                                X->textForm->core.height) {
      menu_handler(widget, (XtPointer)M_CON, event, continue_to_dispatch);
               ...
               bevent->y >= accBtn->core.y + X->modeline->core.height +
                                X->textForm->core.height &&
      ...
      menu_handler(widget, (XtPointer)M_ACC, event, continue_to_dispatch);
               ...
               bevent->y >= rclBtn->core.y + X->modeline->core.height +
                                X->textForm->core.height &&
      ...
      menu_handler(widget, (XtPointer)M_RCL, event, continue_to_dispatch);
               ...
               bevent->y >= stoBtn->core.y + X->modeline->core.height +
                                X->textForm->core.height &&
      ...
      menu_handler(widget, (XtPointer)M_STO, event, continue_to_dispatch);
               ...
               bevent->y >= exchBtn->core.y + X->modeline->core.height +
                                X->textForm->core.height &&
...
void set_option_menu(int type, int base) {
  Arg args[1];
...
void map_popup(Widget widget, XtPointer client_data, XtPointer call_data) {
...
/*ARGSUSED*/
static void save_state(Widget widget, XtPointer client_data,
                       XtPointer call_data) {
  char *full_path = NULL;
  char *file_name = NULL;
  ...
  restart_argv[i] = XtNewString("-session");
  i++;
  restart_argv[i] = XtNewString(sessionFileName);
  i++;
...
static void SaveSession(char *path, char *file_name) {
  int fd;
  ...
  XGetWindowProperty(X->dpy, XtWindow(X->kframe), wm_state_atom, 0L,
                     (long)BUFSIZ, False, wm_state_atom, &actual_type,
  ...
  if (DtWsmGetWorkspacesOccupied(X->dpy, XtWindow(X->kframe), &ws_presence,
  ...
  XTranslateCoordinates(XtDisplay(X->kframe), XtWindow(X->kframe),
                        RootWindowOfScreen(XtScreen(X->kframe)), 0, 0, &t_x,
                        &t_y, &junkWindow);
  x = (Position)t_x;
  y = (Position)t_y;
...
void RestoreSession(void) {
  ...
  status =
      DtSessionRestorePath(X->kframe, &full_path, application_args.session);
  if (!status)
    return;
  ...
  if (get_bool_resource(R_ICON, &boolval))
    v->iconic = boolval;
  ...
  /* Get the keys state */
  if (get_bool_resource(R_KEYS, &boolval))
    v->tstate = boolval;
...
static void setCalcHints(void) {
  ...
  if (v->workspaces) {
    ...
    DtWsmSetWorkspacesOccupied(X->dpy, XtWindow(X->kframe), workspace_atoms,
                               num_workspaces);
...
void TimerEvent(XtPointer client_data, XtIntervalId *id) {
  Arg args[1];
...
static char *_DtcalcStripSpaces(char *file) {
  int i, j;
