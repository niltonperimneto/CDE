...
#define CLOSE_FILES_ON_EXEC()                                                  \
  {                                                                            \
    int ifx;                                                                   \
    for (ifx = 3; ifx < _NFILE; ifx++)                                         \
      (void)fcntl(ifx, F_SETFD, FD_CLOEXEC);                                   \
  }
...
Boolean F_Action(String actionName, ClientData *pCD, XEvent *event) {
  ...
  DtActionInvoke(wmGD.pActiveSD->screenTopLevelW1, pAP->actionName, pAP->aap,
                 pAP->numArgs, pAP->szExecParms, NULL, NULL, 1, NULL, NULL);
...
Boolean ForceLowerWindow(ClientData *pcd) {
  ...
  mask = CWStackMode;
  while (pCLE != NULL) {
    if ((pCLE->pCD != pcd) && (ClientInWorkspace(ACTIVE_WS, pCLE->pCD))) {
...
Boolean F_Lower(String args, ClientData *pCD, XEvent *event) {
  ...
  if (string) {
    ...
    if (string[0] == '-') {
      ...
      while (pNextEntry && (pEntry = FindClientNameMatch(
                                pNextEntry, False, string, F_GROUP_ALL))) {
...
void Do_Lower(ClientData *pCD, ClientListEntry *pStackEntry, int flags) {
  ...
  if ((pcdLeader->clientState == MINIMIZED_STATE) && !P_ICON_BOX(pcdLeader)) {
    ...
    if (pStackEntry) {
      if (pStackEntry->prevSibling != &pcdLeader->iconEntry) {
        StackWindow(pWS, &pcdLeader->iconEntry, True /*above*/, pStackEntry);
        MoveEntryInList(pWS, &pcdLeader->iconEntry, True /*above*/,
                        pStackEntry);
      }
    } else {
      if (ACTIVE_PSD->lastClient != &pcdLeader->iconEntry) {
        StackWindow(pWS, &pcdLeader->iconEntry, False /*on bottom*/,
                    (ClientListEntry *)NULL);
        MoveEntryInList(pWS, &pcdLeader->iconEntry, False /*on bottom*/,
                        (ClientListEntry *)NULL);
      }
    }
  } else /* NORMAL_STATE, MAXIMIZED_STATE, adoption */
  {
    ...
    if ((pcdLeader->transientChildren) && (!pcdLeader->secondariesOnTop) &&
      ...
      if (pCD == pcdLeader) {
         ...
         */
        for (pcdLdr = pCD->transientLeader; pcdLdr;
    ...
    if (pCD->transientLeader) {
      ...
      if (flags & STACK_FREE_FAMILY) {
        ...
        if ((dirtyStackEntry) && (dirtyStackEntry != pCD)) {
          RestackTransients(dirtyStackEntry);
        }
    ...
    if (pStackEntry) {
      if ((pStackEntry->prevSibling != &pcdLeader->clientEntry) &&
          !(wmGD.systemModalActive && (pcdLeader == wmGD.systemModalClient))) {
        StackWindow(pWS, &pcdLeader->clientEntry, True /*above*/, pStackEntry);
      ...
      if ((pWS->pSD->lastClient != &pcdLeader->clientEntry) &&
        ...
        StackWindow(pWS, &pcdLeader->clientEntry, False /*on bottom*/,
                    (ClientListEntry *)NULL);
        MoveEntryInList(pWS, &pcdLeader->clientEntry, False /*on bottom*/,
                        (ClientListEntry *)NULL);
      } else if ((restackTransients) || (bLeaderRestacked)) {
        RestackTransients(pCD);
      }
  ...
  while (pCD) {
    if (pCD->iconWindow && (pCD->clientFlags & ICON_REPARENTED) &&
      ...
      XReparentWindow(DISPLAY, pCD->iconWindow, ROOT_FOR_CLIENT(pCD),
                      pCD->pWsList->iconX, pCD->pWsList->iconY);
    }
  ...
  while (pNextEntry) {
    ...
    if (!wmGD.systemModalActive || (wmGD.systemModalClient == pCD)) {
      if ((pNextEntry->type != MINIMIZED_STATE) &&
          ...
          (ClientInWorkspace(ACTIVE_WS, pCD)) && (pCD != pcdNoFocus)) {
        if (pCD->transientChildren) {
  ...
  if ((pCD) && (ClientInWorkspace(ACTIVE_WS, pCD))) {
    if (pCD->clientState == MINIMIZED_STATE) {
      ...
      if (pcdFocus) {
        ...
        focusWindow = (wmGD.keyboardFocus) ? wmGD.keyboardFocus->client
                                           : ACTIVE_PSD->wmWorkspaceWin;
      }
    } else {
      /*
  ...
  if ((pcdFocus != wmGD.keyboardFocus) || (flags & ALWAYS_SET_FOCUS)) {
    if (pcdFocus) {
      ...
      if ((flags & CLIENT_AREA_FOCUS) &&
          ...
          !pcdFocus->inputFocusModel && (pcdFocus == pCD) &&
        ...
        /* just send a message.                                    */
        SendClientMsg(pcdFocus->client, (long)wmGD.xa_WM_PROTOCOLS,
                      (long)wmGD.xa_WM_TAKE_FOCUS, focusTime, NULL, 0);
      } else {
        if ((pcdFocus->protocolFlags & PROTOCOL_WM_TAKE_FOCUS) &&
            !(pcdFocus->clientState == MINIMIZED_STATE)) {
          /*
           * Locally Active Input Model - Send a take focus message to the
           * client.
           */
        ...
        if (wmGD.enforceKeyFocus || /* res - default == True. */
            ...
            (pcdFocus->clientState == MINIMIZED_STATE)) {
          if (!(flags & CLIENT_AREA_FOCUS) && !pcdFocus->inputFocusModel &&
              !(pcdFocus->clientState == MINIMIZED_STATE)) {
            ...
            XSetInputFocus(DISPLAY, focusWindow, RevertToParent, focusTime);
          }
        } else {
          /*
      ...
      XSetInputFocus(DISPLAY, focusWindow, RevertToPointerRoot, focusTime);
    }
...
Boolean F_Goto_Workspace(String args, ClientData *pCD, XEvent *event) {
  ...
  if (iwsx >= pSD->numWorkspaces) {
    Warning(((char *)GETMESSAGE(
        26, 4, "Invalid workspace name specified for f.goto_workspace")));
  } else {
...
Boolean F_Post_FpMenu(String args, ClientData *pCD, XEvent *event) {
  ...
  if (!fpMenuSpec) {
    ...
    for (oldSpec = pSD->menuSpecs; oldSpec != (MenuSpec *)NULL;
    ...
    (void)MAKE_MENU(pSD, pCD, newMenuName, F_CONTEXT_NORMAL, F_CONTEXT_NORMAL,
                    (MenuItem *)NULL, FALSE);
  ...
  _XmGetMenuState(XtParent(fpMenuSpec->menuWidget))->MS_LastManagedMenuTime =
      event->xbutton.time;
...
Boolean F_Push_Recall(String args, ClientData *pCD, XEvent *event) {
  ...
  if (pPRP->ixReg < pSD->numPushRecallClients) {
    ...
    if (pPRCD->pCD) {
      ...
      if (!(ClientInWorkspace(pSD->pActiveWS, pPRCD->pCD))) {
         ...
         */
        wsRemoveList = GetListOfOccupiedWorkspaces(pPRCD->pCD, &sizeRemoveList);
        RemoveClientFromWorkspaces(pPRCD->pCD, wsRemoveList, sizeRemoveList);
        XtFree((char *)wsRemoveList);
        AddClientToWorkspaces(pPRCD->pCD, &(pSD->pActiveWS->id), 1);
        SetClientWsIndex(pPRCD->pCD);
        SetClientState(pPRCD->pCD, pPRCD->pCD->clientState & ~UNSEEN_STATE,
                       CurrentTime);
      }
      ...
      if (!bWaiting) {
        ...
        if (pPanelist && panel.busy_light_data) {
          /* set timeout value */
          ac = 0;
          XtSetArg(al[ac], XmNclientTimeoutInterval, &clientTimeout);
          ac++;
        ...
        pPRCD->tvTimeout.tv_usec += (clientTimeout % 1000) * 1000;
...
Boolean F_Menu(String args, ClientData *pCD, XEvent *event) {
  ...
  if (pCD) {
    ...
    if (P_ICON_BOX(pCD) && event->xany.window == ICON_FRAME_WIN(pCD)) {
      if (pCD->clientState == MINIMIZED_STATE) {
  ...
  pSD = (pCD) ? PSD_FOR_CLIENT(pCD) : ACTIVE_PSD;
  if ((menuSpec = MAKE_MENU(pSD, pCD, args, menuContext, menuContext,
                            (MenuItem *)NULL, FALSE)) != NULL) {
...
Boolean F_Normalize_And_Raise(String args, ClientData *pCD, XEvent *event) {
  ...
  if (args) {
    ...
    if (pSD->useIconBox && wmGD.useFrontPanel && pSD->iconBoxControl &&
  ...
  if (pCD) {
    if (pCD->clientState == MINIMIZED_STATE) {
      /* normalize window  */
      SetClientStateWithEventMask(
          pCD, NORMAL_STATE,
          (Time)(event ? GetFunctionTimestamp((XButtonEvent *)event)
                       : GetTimestamp()),
          GetEventInverseMask(event));
    } else {
      ...
      if (wmGD.raiseKeyFocus) {
        F_Focus_Key(NULL, pCD, (event ? ((XEvent *)event) : ((XEvent *)NULL)));
      }
    }
...
Boolean F_Post_SMenu(String args, ClientData *pCD, XEvent *event) {
  ...
  if (event && pCD && pCD->systemMenuSpec) {
    ...
    if ((event->type == KeyPress) || (event->type == KeyRelease)) {
      ...
      PostMenu(pCD->systemMenuSpec, pCD, 0, 0, NoButton, menuContext, 0, event);
      ...
      PostMenu(pCD->systemMenuSpec, pCD, event->xbutton.x_root,
               event->xbutton.y_root, event->xbutton.button, menuContext,
               POST_AT_XY, event);
...
Boolean F_Kill(String args, ClientData *pCD, XEvent *event) {
  if (pCD && (pCD->clientFunctions & MWM_FUNC_CLOSE)) {
    Boolean do_delete_window = pCD->protocolFlags & PROTOCOL_WM_DELETE_WINDOW;
    Boolean do_save_yourself = pCD->protocolFlags & PROTOCOL_WM_SAVE_YOURSELF;
 ...
 *
 *************************************<->***********************************/
static void RefreshByClearing(Window win) {
  Status status;
  int i;
  Window root, parent;
  ...
  status = XQueryTree(DISPLAY, win, &root, &parent, &winChildren, &nchildren);
  if (status != 0) {
...
Boolean F_Refresh(String args, ClientData *pCD, XEvent *event) {
  ...
  if (wmGD.refreshByClearing) {
    ...
    win = XCreateWindow(DISPLAY, ACTIVE_ROOT, 0, 0,
                        (unsigned int)DisplayWidth(DISPLAY, ACTIVE_SCREEN),
                        (unsigned int)DisplayHeight(DISPLAY, ACTIVE_SCREEN), 0,
                        0, InputOutput, CopyFromParent, 0,
        ...
        if (pNextEntry->type == NORMAL_STATE) {
          if (pNextEntry->pCD->clientFlags & CLIENT_WM_CLIENTS) {
            if (pNextEntry->pCD->clientState != MINIMIZED_STATE) {
              XUnmapWindow(DISPLAY, pNextEntry->pCD->clientFrameWin);
            }
          } else {
...
Boolean F_Toggle_Front_Panel(String args, ClientData *pCD, XEvent *event) {
  ...
  if (pCD && pCD->mwmMessagesCount) {
    ...
    for (i = 0; i < pCD->mwmMessagesCount; i++) {
      if (pCD->mwmMessages[i] == (long)args) {
        SendClientMsg(pCD->client, (long)wmGD.xa_MWM_MESSAGES, (long)args,
                      CurrentTime, NULL, 0);
        return (True);
      }
    }
...
Boolean ForceRaiseWindow(ClientData *pcd) {
  ...
  XConfigureWindow(DISPLAY, pcd->clientFrameWin, CWStackMode, &changes);
...
Boolean F_Raise(String args, ClientData *pCD, XEvent *event) {
  ...
  if (string) {
    ...
    if (string[0] == '-') {
      ...
      pNextEntry = ACTIVE_PSD->clientList;
      while (pNextEntry && (pEntry = FindClientNameMatch(
                                pNextEntry, True, string, F_GROUP_ALL))) {
...
void Do_Raise(ClientData *pCD, ClientListEntry *pStackEntry, int flags) {
  ...
  if (wmGD.systemModalActive && (pcdLeader != wmGD.systemModalClient)) {
    ...
    if (pStackEntry) {
      if (pStackEntry->nextSibling != &pcdLeader->iconEntry) {
        StackWindow(pWS, &pcdLeader->iconEntry, False /*below*/, pStackEntry);
        MoveEntryInList(pWS, &pcdLeader->iconEntry, False /*below*/,
                        pStackEntry);
      }
    } else {
      if (ACTIVE_PSD->clientList != &pcdLeader->iconEntry) {
        StackWindow(pWS, &pcdLeader->iconEntry, True /*on top*/,
                    (ClientListEntry *)NULL);
        MoveEntryInList(pWS, &pcdLeader->iconEntry, True /*on top*/,
                        (ClientListEntry *)NULL);
      }
    }
  } else /* NORMAL_STATE, MAXIMIZED_STATE, adoption */
  {
     ...
     */
    bLeaderRestacked = False;
    if ((pcdLeader->transientChildren) && (!pCD->secondariesOnTop) &&
        (!wmGD.bSuspendSecondaryRestack) && (!IS_APP_MODALIZED(pCD))) {
      if (pCD != pcdLeader) {
        /*
    ...
    if (!pCD->transientLeader || !(flags & STACK_WITHIN_FAMILY)) {
      if (pStackEntry) {
        ...
        if (ACTIVE_PSD->clientList != &pcdLeader->clientEntry) {
          StackWindow(pWS, &pcdLeader->clientEntry, True /*on top*/,
                      (ClientListEntry *)NULL);
          MoveEntryInList(pWS, &pcdLeader->clientEntry, True /*on top*/,
                          (ClientListEntry *)NULL);
        }
      }
    }
  ...
  if (pCD) {
    ...
    if (CheckIfClientObscuredByAny(pcdLeader)) {
      ...
      if ((pcdLeader->secondariesOnTop == False) &&
          (pCD->transientLeader != NULL) && (!IS_APP_MODALIZED(pcdLeader))) {
        ...
        RestackTransients(pcdLeader);
               ...
               (!wmGD.systemModalActive) && (!IS_APP_MODALIZED(pcdLeader))) {
...
Boolean F_Refresh_Win(String args, ClientData *pCD, XEvent *event) {
  ...
  if (pCD && ((pCD->clientState == NORMAL_STATE) ||
    ...
    if (wmGD.refreshByClearing) {
      ...
      /* default background_pixmap is None */
      win = XCreateWindow(DISPLAY, pCD->clientBaseWin, pCD->matteWidth,
                          pCD->matteWidth, w, h, 0, 0, InputOutput,
                          CopyFromParent, 0, (XSetWindowAttributes *)NULL);
...
Boolean F_Set_Behavior(String args, ClientData *pCD, XEvent *event) {
  ...
  if (wmGD.showFeedback & WM_SHOW_FB_BEHAVIOR) {
    ConfirmAction(ACTIVE_PSD, (wmGD.useStandardBehavior)
                                  ? CUSTOM_BEHAVIOR_ACTION
                                  : DEFAULT_BEHAVIOR_ACTION);
  } else {
    RestartWm((long)((wmGD.useStandardBehavior) ? MWM_INFO_STARTUP_CUSTOM
...
Boolean F_Screen(String args, ClientData *pCD, XEvent *event) {
  ...
  if (ACTIVE_PSD->screen == scr)
    return (False); /* already on that screen */

  PreviousScreen = ACTIVE_PSD->screen;
  XQueryPointer(DISPLAY, ACTIVE_ROOT, &dumwin, &dumwin, &x, &y, &dumint,
                &dumint, &dummask);
...
Boolean F_ZZ_Debug(String subFcn, ClientData *pCD, XEvent *event) {
  ...
  if (subFcn) {
    if (!(strcmp(subFcn, "dump_resources"))) {
      ...
      for (scr = 0; scr < wmGD.numScreens; scr++) {
        ...
        XrmPutFileDatabase(XtScreenDatabase(XScreenOfDisplay(DISPLAY, scr)),
                           szRes);
      }
    }
  }
...
void DumpWindowList() {
  ...
  while (pCLE) {
    ...
    fprintf(stdout, "%08lx\t%s\n", pCLE->pCD->client, pCLE->pCD->clientName);
