...
#define NOFRZ_GRAB_MASK (KeyPressMask | ButtonPressMask | ButtonReleaseMask)
...
void SetupPlacement(ClientData *pcd) {
  ...
  XQueryPointer(DISPLAY, ACTIVE_ROOT, &junk_win, &junk_win, &cX, &cY, &junk,
                &junk, (unsigned int *)&junk);
...
void StartInteractiveSizing(ClientData *pcd, Time time) {
  ...
  XChangeActivePointerGrab(DISPLAY, gmask, wmGD.sizePlacementCursor, time);

  /* put cursor at lower-right corner */
  if (wmGD.enableWarp) {
    XWarpPointer(DISPLAY, None, ACTIVE_ROOT, 0, 0, 0, 0,
                 (int)(placeX + placeWidth - 1),
                 (int)(placeY + placeHeight - 1));
  }
...
void HandlePlacementKeyEvent(ClientData *pcd, XKeyEvent *pev) {
  ...
  FixFrameValues(pcd, &placeX, &placeY, &placeWidth, &placeHeight, placeResize);

  MoveOutline(placeX, placeY, placeWidth, placeHeight);

  if (wmGD.showFeedback & WM_SHOW_FB_PLACEMENT) {
    DoFeedback(pcd, placeX, placeY, placeWidth, placeHeight, 0, placeResize);
  }
...
void DoPlacement(ClientData *pcd) {
  ...
  placementDone = FALSE;
  while (!placementDone) {
    GetConfigEvent(DISPLAY, ACTIVE_ROOT, GRAB_MASK, placePointerX,
                   placePointerY, placeX, placeY, placeWidth, placeHeight,
                   &event);
...
void PlaceWindowInteractively(ClientData *pcd) {
   ...
   */
  if (!DoGrabs(ACTIVE_ROOT, wmGD.movePlacementCursor, gmask, CurrentTime, pcd,
               True)) {
    return;
  }
