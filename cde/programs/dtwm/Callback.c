...
void CallWmFunction(WmFunction wm_function, String args, Widget client,
                    XEvent *event)
...
void ArrowCB(Widget w, XtPointer client_data, XtPointer call_data)

{
  ...
  if (!XtIsManaged(subpanel_data->shell)) {
    ...
    ac = 0;
    XtSetArg(al[ac], XmNimageName, unpost_arrow_image);
    ac++;
    XtSetValues(w, al, ac);
  } else {
...
XtTranslations HandleInputTranslations(void)
...
void HandleInputCB(Widget w, XtPointer client_data, XtPointer call_data)
...
void MinimizeInputCB(Widget w, XtPointer client_data, XtPointer call_data)

{
  ...
  if (event->type == ButtonPress) {
    ...
    if (bevent->x >= 0 && bevent->x <= (Position)XtWidth(w) && bevent->y >= 0 &&
        bevent->y <= (Position)XtHeight(w)) {
...
void MenuInputCB(Widget w, XtPointer client_data, XtPointer call_data)
...
void WorkspaceAdjustPanelPosition(Position x, Position y, Dimension width,
                                  Dimension height)
...
void WorkspaceModifyCB(Widget w, Atom atom_name, int type,
...

{
  ...
  case DtWSM_REASON_ADD: {
    ...
    if (switch_data->switch_count <=
        (intptr_t)(switch_data->element_values[SWITCH_NUMBER_OF_ROWS]
                       .parsed_value)) {
      Arg al[1];
  ...
  case DtWSM_REASON_DELETE: {
    ...
    if (panel.switch_row_count >
      ...
      if ((int)(screen_width - (width + button_width + button_spacing)) > 0) {
        panel.switch_row_count--;
        XtSetArg(al[0], XmNnumColumns, panel.switch_row_count);
        XtSetValues(switch_data->rc, al, 1);
      }
  ...
  case DtWSM_REASON_TITLE: {
    ...
    for (i = 0; i < switch_data->switch_count; i++) {
      if (atom_name == switch_data->atom_names[i]) {
        ...
        toggle_string = XmStringCreateLocalized(workspace_info->pchTitle);
  ...
  case DtWSM_REASON_CURRENT: {
    ...
    for (i = 0; i < switch_data->switch_count; i++) {
      if (switch_data->atom_names[i] == atom_name) {
        ...
        _DtIconSetState(switch_data->buttons[switch_data->active_switch], False,
                        False);
        switch_data->active_switch = i;
        _DtIconSetState(switch_data->buttons[switch_data->active_switch], True,
                        False);
        break;
      }
    }
...
void SwitchButtonCB(Widget w, XtPointer client_data, XtPointer call_data)

{
  ...
  if (callback->reason != XmCR_VALUE_CHANGED)
    return;
  ac = 0;
  XtSetArg(al[ac], XmNuserData, &box_data);
  ac++;
  ...
  if (switch_button != old_switch_button) {
    ...
    if (XtIsManaged(box_data->switch_edit)) {
      ...
      SwitchRenameCB(box_data->switch_edit, (XtPointer)edit_switch_button,
                     (XtPointer)NULL);
...
static void SwitchRenameCancel(Widget w, XEvent *event, String *params,
                               Cardinal *num_params)
...
static void SwitchRenameCB(Widget w, XtPointer client_data, XtPointer call_data)

{
  ...
  XtRemoveCallback(w, XmNactivateCallback, SwitchRenameCB,
                   (XtPointer)client_data);
  ...
  if (!valid) {
    ...
    _DtWsmSetWorkspaceTitle(
        panel.shell, switch_data->atom_names[current_switch], edit_string);
...
void SwitchRenameLabel(Widget switch_button, BoxData *box_data)

{
  ...
  ac = 0;
  XtSetArg(al[ac], XmNx, &toggle_x);
  ac++;
  XtSetArg(al[ac], XmNy, &toggle_y);
  ac++;
  XtSetArg(al[ac], XmNwidth, &toggle_width);
  ac++;
  XtSetArg(al[ac], XmNheight, &toggle_height);
  ac++;
  XtSetArg(al[ac], XmNstring, &toggle_string);
  ac++;
  XtSetArg(al[ac], XmNfontList, &toggle_font_list);
  ac++;
  XtGetValues(switch_button, al, ac);
  ...
  if ((edit_string =
           (char *)_XmStringUngenerate(toggle_string, NULL, XmCHARSET_TEXT,
                                       XmCHARSET_TEXT)) != (char *)NULL) {

    /*  Set the switch editing field to the new resource set  */

    ac = 0;
    XtSetArg(al[ac], XmNleftOffset, toggle_x + switch_rc_x);
    ac++;
    XtSetArg(al[ac], XmNleftAttachment, XmATTACH_FORM);
    ac++;
    XtSetArg(al[ac], XmNtopOffset, toggle_y + switch_rc_y);
    ac++;
    ...
    ac++;
    XtSetArg(al[ac], XmNvalue, edit_string);
    ac++;
    XtSetArg(al[ac], XmNuserData, switch_button);
    ac++;
    ...
    XtAddCallback(switch_edit, XmNactivateCallback, SwitchRenameCB,
...
Boolean CheckControlTypeFile(ControlData *control_data)
...
static void FreeFunctionArg(WmFunction wmFunc, void *pArg) {
  if (pArg != NULL) {
    if (wmFunc == F_Action) {
...
Boolean CheckOtherMonitorsOn(SubpanelData *subpanel_data) {
  int i;
...

{
  ...
  control_behavior =
      (intptr_t)(panel.element_values[PANEL_CONTROL_BEHAVIOR].parsed_value);
  ...
  for (i = 0; i < panel.box_data_count; i++) {
    if (panel.box_data[i]->switch_edit != NULL) {
      if (XtIsManaged(panel.box_data[i]->switch_edit)) {
        ...
        SwitchRenameCB(panel.box_data[i]->switch_edit, (XtPointer)switch_button,
                       (XtPointer)NULL);
        XmProcessTraversal(w, XmTRAVERSE_CURRENT);
        break;
      }
  ...
  if (subpanel_data != NULL && event != NULL) {
    if (event->xany.type == KeyPress) {
      ...
      if (keysym == XK_Escape) {
        if (first) {
          ArrowCB(main_control_data->arrow, (XtPointer)main_control_data,
                  (XtPointer)NULL);
          first = False;
        } else
          first = True;
  ...
  if (callback->reason == XmCR_BUSY_START) {
    ...
    for (i = 0, pWid = M_PopupList(panel.shell); i < M_NumPopups(panel.shell);
         i++)
    ...
    push_action =
        (PanelActionData *)(control_data->element_values[CONTROL_PUSH_ACTION]
                                .parsed_value);
    push_recall =
        (Boolean)(intptr_t)(control_data->element_values[CONTROL_PUSH_RECALL]
                                .parsed_value);
      ...
      if (CheckControlTypeFile(control_data) && push_action != NULL) {
        ...
        if (push_recall == False) {
          ...
          if (control_type == CONTROL_FILE && !control_data->is_action) {
            ...
            aap->u.file.name =
                (char *)control_data->element_values[CONTROL_FILE_NAME]
                    .parsed_value;

            DtActionInvoke(panel.shell, push_action->action_name, aap, 1, NULL,
            ...
            if (strcmp(push_action->action_name, "FPOnItemHelp") == 0) {
              ...
              DtActionInvoke(panel.shell, push_action->action_name, NULL, 0,
          ...
          if (client_name == NULL)
            client_name = (char *)control_data->element_values[CONTROL_LABEL]
                              .parsed_value;
          ...
          if (control_type == CONTROL_FILE && !control_data->is_action) {
            ...
            file_name = (char *)control_data->element_values[CONTROL_FILE_NAME]
                            .parsed_value;

            s1 = XtMalloc(strlen(push_action->action_name) + strlen(file_name) +
                          2);
            snprintf(s1,
                     strlen(push_action->action_name) + strlen(file_name) + 2,
                     "%s %s", push_action->action_name, file_name);
          } else
            s1 = XtNewString(push_action->action_name);

          ParseWmFunctionArg((unsigned char **)&s1, function_index, wm_func,
                             (void **)&func_arg, client_name, client_name);
          ...
          if (subpanel_data != NULL)
            XFindContext(
                wm_global_data->display, XtWindow(subpanel_data->shell),
                wm_global_data->windowContextType, (XtPointer)&wm_client_data);
          else
            XFindContext(wm_global_data->display, XtWindow(panel.shell),
                         wm_global_data->windowContextType,
                         (XtPointer)&wm_client_data);

          F_Push_Recall((String)&push_argument, wm_client_data,
                        callback->event);
    ...
    if (subpanel_data != NULL && unpost_subpanel &&
        (Boolean)(intptr_t)panel.element_values[PANEL_SUBPANEL_UNPOST]
            .parsed_value &&
...
void DropCB(Widget w, XtPointer client_data, XtPointer call_data)

{
  PanelActionData *drop_action = (PanelActionData *)client_data;
  DtDndDropAnimateCallback animate_data = (DtDndDropAnimateCallback)call_data;
  ...
  if (CheckControlTypeFile(control_data) &&
    ...
    drop_action =
        (PanelActionData *)control_data->element_values[CONTROL_DROP_ACTION]
            .parsed_value;
    ...
    DtActionInvoke(w, drop_action->action_name, aap, numItems, NULL, NULL, NULL,
                   1, NULL, NULL);
...
void TransferDropCB(Widget w, XtPointer client_data, XtPointer call_data)
...
void SubpanelUnmapCB(Widget w, XtPointer client_data, XtPointer call_data)

{
  ...
  ac = 0;
  XtSetArg(al[ac], XmNimageName, post_arrow_image);
  ac++;
...
void SubpanelTornEventHandler(Widget subpanel_shell, XtPointer client_data,
...
void StartHelpClient(Widget widget, char *help_volume, char *help_topic)
...
void InvokeHelp(Widget widget, char *help_topic, char *help_volume,
                char *help_string) {
...
void SubpanelTopicHelpCB(Widget widget, XtPointer client_data,
                         XtPointer call_data)
...
void SwitchTopicHelpCB(Widget widget, XtPointer client_data,
                       XtPointer call_data)
...
void ControlTopicHelpCB(Widget widget, XtPointer client_data,
                        XtPointer call_data)
...
void GeneralTopicHelpCB(Widget widget, XtPointer client_data,
                        XtPointer call_data)
...
static void GetValuesFromDataType(char *data_type, char *file_name,
                                  SubpanelData *subpanel_data,
...

{
  ...
  *element_values =
      (ElementValue *)XtMalloc(sizeof(ElementValue) * CONTROL_KEYWORD_COUNT);
  ...
  (*element_values)[CONTROL_POSITION_HINTS].string_value = XtNewString("last");
...
void CustomizeDropCB(Widget w, XtPointer client_data, XtPointer call_data)

{
  ...
  for (i = 0; i < file_count; i++) {
    ...
    if (!strcmp(data_type, "FP")) {
      ...
      element_values[CONTROL_CONTAINER_NAME].string_value = XtNewString(
          subpanel_data->element_values[SUBPANEL_NAME].parsed_value);
      element_values[CONTROL_CONTAINER_NAME].parsed_value = XtNewString(
          subpanel_data->element_values[SUBPANEL_NAME].parsed_value);

      element_values[CONTROL_CONTAINER_TYPE].use_default = False;
      element_values[CONTROL_CONTAINER_TYPE].string_value =
          XtNewString("SUBPANEL");
      element_values[CONTROL_CONTAINER_TYPE].parsed_value = (char *)SUBPANEL;

      element_values[CONTROL_POSITION_HINTS].use_default = False;
      element_values[CONTROL_POSITION_HINTS].string_value = XtNewString("last");
      element_values[CONTROL_POSITION_HINTS].parsed_value = (char *)100;
    } else {
      drop_of_fp = False;
      GetValuesFromDataType(data_type, file_list[i], subpanel_data,
                            &element_values);
    }
    ...
    for (j = 0; j < subpanel_data->control_data_count; j++) {
      ...
      if (strcmp(control_name, new_control_name) == 0) {
        ...
        snprintf(
            message,
            (strlen(del_ctrl) + strlen(new_control_name) + strlen(del_msg) + 4),
    ...
    if ((intptr_t)element_values[CONTROL_MONITOR_TYPE].parsed_value !=
        MONITOR_NONE)
    ...
    position_hints =
        (intptr_t)element_values[CONTROL_POSITION_HINTS].parsed_value;

    /*  Initialize the subpanel layout and processing attributes.  */

    if ((intptr_t)main_control_data->element_values[CONTROL_MONITOR_TYPE]
            .parsed_value != MONITOR_NONE) {
      control_monitor = True;
    }
    ...
    for (j = 0; j < subpanel_data->control_data_count; j++) {
      if ((intptr_t)subpanel_data->control_data[j]
              ->element_values[CONTROL_MONITOR_TYPE]
              .parsed_value != MONITOR_NONE) {
        control_monitor = True;
        break;
      }
    ...
    subpanel_data->control_data = (ControlData **)XtRealloc(
        (char *)subpanel_data->control_data, sizeof(ControlData *) * count);

    for (j = count - 2; j >= 0; j--) {
      if (position_hints >= (intptr_t)subpanel_data->control_data[j]
    ...
    SubpanelControlCreate(subpanel_data, main_control_data, control_data,
                          attach_widget, False, control_monitor);
...
void CustomizeTransferDropCB(Widget w, XtPointer client_data,
                             XtPointer call_data)
