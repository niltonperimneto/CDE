...
static void UnmapCallback(Widget w, XtPointer client_data, XtPointer call_data);
...
MenuSpec *MakeMenu(WmScreenData *pSD, String menuName, Context initialContext,
  ...
  menuSpec->menuWidget = CREATE_MENU_WIDGET(
      pSD, pCD, menuName, pSD->screenTopLevelW, TRUE, menuSpec, moreMenuItems);
...
/*************************************<->***********************************/
void CheckTerminalSeparator(MenuSpec *menuSpec, Widget buttonWidget,
                            Boolean manage) {
  CompositeWidget cw;
  WidgetList children;
  Cardinal wPos;
...
static MenuItem *DuplicateMenuItems(MenuItem *menuItems) {
  ...
  for (curMenuItem = menuItems; curMenuItem != (MenuItem *)NULL;
    ...
    {
      newMenuItem->nextMenuItem = (MenuItem *)XtMalloc(sizeof(MenuItem));
...
static Boolean AdjustPBs(MenuSpec *menuSpec, ClientData *pCD,
  ...
  for (n = 0, menuButton = menuSpec->menuButtons; n < menuSpec->menuButtonCount;
       n++, menuButton++) {
        ...
        /* function applies -- manage it */
        {
          ...
          /* unmanaged -> managed */
          {
            ...
            if (n == menuSpec->menuButtonCount - 1) {
              ...
              CheckTerminalSeparator(menuSpec, menuButton->buttonWidget, True);
            }
          }
        } else
        /* function does not apply -- unmanage it */
        {
          ...
          /* managed -> unmanaged */
          {
            ...
            if (n == menuSpec->menuButtonCount - 1) {
               ...
               */
              CheckTerminalSeparator(menuSpec, menuButton->buttonWidget, False);
            }
          }
        }
...
static Boolean SavePBInfo(MenuSpec *topMenuSpec, MenuItem *menuItem,
  ...
  {
    topMenuSpec->menuButtonSize += MENU_BUTTON_INC;
    topMenuSpec->menuButtons = (MenuButton *)XtRealloc(
        (char *)topMenuSpec->menuButtons,
...
Widget CreateMenuWidget(WmScreenData *pSD, String menuName, Widget parent,
   ...
   */
  i = 0;
  XtSetArg(args[i], XmNwidth, (XtArgVal)5);
  i++;
  XtSetArg(args[i], XmNheight, (XtArgVal)5);
  i++;
  XtSetArg(args[i], XmNallowShellResize, (XtArgVal)TRUE);
  i++;
  XtSetArg(args[i], XtNoverrideRedirect, (XtArgVal)TRUE);
  i++;
  XtSetArg(args[i], XtNdepth,
           (XtArgVal)DefaultDepth(XtDisplay(parent), pSD->screen));
  i++;
  XtSetArg(args[i], XtNscreen,
           (XtArgVal)ScreenOfDisplay(XtDisplay(parent), pSD->screen));
  i++;
  ...
  menuShellW = XtCreatePopupShell(SHELL_NAME, xmMenuShellWidgetClass, parent,
                                  (ArgList)args, i);
  ...
  i = 0;
  XtSetArg(args[i], XmNborderWidth, (XtArgVal)0);
  i++;
  XtSetArg(args[i], XmNwhichButton, (XtArgVal)SELECT_BUTTON);
  i++;
  XtSetArg(args[i], XmNadjustMargin, (XtArgVal)TRUE);
  i++;

  if (fTopLevelPane) {
    XtSetArg(args[i], XmNrowColumnType, (XtArgVal)XmMENU_POPUP);
    i++;
    XtSetArg(args[i], XmNpopupEnabled, (XtArgVal)TRUE);
    i++;
    menuW = XtCreateWidget(menuName, xmRowColumnWidgetClass, menuShellW,
                           (ArgList)args, i);
    XtAddCallback(menuW, XmNunmapCallback, UnmapCallback, (XtPointer)menuSpec);
  } else {
    XtSetArg(args[i], XmNrowColumnType, (XtArgVal)XmMENU_PULLDOWN);
    i++;
    menuW = XtCreateWidget(menuName, xmRowColumnWidgetClass, menuShellW,
                           (ArgList)args, i);
  }
  ...
  while (menuItem) {
     ...
     */
    {
      {
        children[n] =
            XmCreateSeparatorGadget(menuW, SEPARATOR_NAME, (ArgList)NULL, 0);
        fUseTitleSep = FALSE;
      }
    } /* F_Separator */
     ...
     */
    {
      ...
      if ((menuItem->labelType == XmPIXMAP) &&
          (labelPixmap =
               MakeCachedLabelPixmap(pSD, menuW, menuItem->labelBitmapIndex))) {
        XtSetArg(args[i], XmNlabelType, (XtArgVal)XmPIXMAP);
        i++;
        XtSetArg(args[i], XmNlabelPixmap, (XtArgVal)labelPixmap);
        i++;
        XtSetArg(args[i], XmNlabelInsensitivePixmap, (XtArgVal)labelPixmap);
        i++;
      } else {
        XtSetArg(args[i], XmNlabelType, (XtArgVal)XmSTRING);
        i++;
        XtSetArg(args[i], XmNlabelString,
                 (XtArgVal)(tmpstr = XmStringCreateLocalized(menuItem->label)));
        i++;
        sPtr = (StrList *)XtMalloc(sizeof(StrList));
        if (sPtr == NULL) {
          MWarning(
              ((char *)GETMESSAGE(48, 2, "Insufficient memory for menu %s\n")),
              menuName);
          return (NULL);
        } else {
          sPtr->string = tmpstr;
       ...
       */
      {
        if (fUseTitleSep) {
          children[n] =
              XmCreateSeparatorGadget(menuW, SEPARATOR_NAME, sepArgs, 1);
          n++;
        }

        XtSetArg(args[i], XmNalignment, XmALIGNMENT_CENTER);
        i++;
        children[n] = XmCreateLabelGadget(menuW, TITLE_NAME, (ArgList)args, i);
        n++;
        children[n] =
            XmCreateSeparatorGadget(menuW, SEPARATOR_NAME, sepArgs, 1);
       ...
       */
      {
         ...
         */
        XtSetArg(args[i], XmNalignment, XmALIGNMENT_BEGINNING);
        i++;

        if (menuItem->mnemonic) {
          XtSetArg(args[i], XmNmnemonic, (XtArgVal)menuItem->mnemonic);
          i++;
        }
         ...
         */
        {
          subMenuW = CREATE_MENU_WIDGET(pSD, pCD, menuItem->wmFuncArgs, menuW,
                                        FALSE, topMenuSpec, (MenuItem *)NULL);
          if (subMenuW)
          /*
           ...
           */
          {
            ...
            i++;
            children[n] = XmCreateCascadeButtonGadget(menuW, CASCADE_BTN_NAME,
                                                      (ArgList)args, i);
          } else
          /*
         ...
         */
        {
          ...
          if (menuItem->accelText) {
            XtSetArg(args[i], XmNacceleratorText,
                     (XtArgVal)(tmpstr = XmStringCreateLocalized(
                                    menuItem->accelText)));
            i++;
            sPtr = (StrList *)XtMalloc(sizeof(StrList));
            if (sPtr == NULL) {
              MWarning(((char *)GETMESSAGE(
                           48, 2, "Insufficient memory for menu %s\n")),
                       menuName);
              return (NULL);
            } else {
              sPtr->string = tmpstr;
            ...
            if ((accelKeySpec = (KeySpec *)XtMalloc(sizeof(KeySpec))) == NULL)
            /* Handle insufficent memory */
            {
              MWarning(((char *)GETMESSAGE(
                           48, 6, "Insufficient memory for menu %s\n")),
                       menuName);
          ...
          children[n] =
              XmCreatePushButtonGadget(menuW, PUSH_BTN_NAME, (ArgList)args, i);
          ...
          if ((menuItem->greyedContext) || (menuItem->mgtMask)) {
            if (!SavePBInfo(topMenuSpec, menuItem, children[n])) {
              MWarning(((char *)GETMESSAGE(
                           48, 7, "Insufficient memory for menu %s\n")),
                       menuName);
              menuSpec->currentContext &= ~CR_MENU_MARK;
              return (NULL);
            }
          ...
          XtAddCallback(children[n], XmNactivateCallback,
                        (XtCallbackProc)ActivateCallback, (XtPointer)menuItem);
  ...
  if (fTopLevelPane) {
    i = 0;
    XtSetArg(args[i], XtNheight, &menuHeight);
    i++;
  ...
  if (menuHeight > (Dimension)scr->height) {
    i = 0;
    XtSetArg(args[i], XmNresizeHeight, (XtArgVal)FALSE);
    i++;
    XtSetArg(args[i], XmNpacking, (XtArgVal)XmPACK_TIGHT);
    i++;
    XtSetArg(args[i], XmNorientation, (XtArgVal)XmVERTICAL);
    i++;
    XtSetArg(args[i], XmNheight, scr->height);
    i++;
...
void PostMenu(MenuSpec *menuSpec, ClientData *pCD, int x, int y,
  ...
  if (whichButton != menuSpec->whichButton) {
    i = 0;
    XtSetArg(args[i], XmNwhichButton, (XtArgVal)whichButton);
    i++;
  ...
  if (passedInEvent) {
    ...
    if ((passedInEvent->type == ButtonPress) ||
      ...
      XUngrabPointer(passedInEvent->xany.display, passedInEvent->xbutton.time);
  }
...
void ActivateCallback(Widget w, caddr_t client_data, caddr_t call_data) {
  ...
  ((MenuItem *)client_data)
      ->wmFunction(((MenuItem *)client_data)->wmFuncArgs, wmGD.menuClient,
                   NULL);
