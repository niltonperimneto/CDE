...
Boolean StringsAreEqual(unsigned char *, unsigned char *, int);

void WmCvtStringToAMBehavior(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToCFocus(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToCDecor(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToCFunc(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToFrameStyle(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToIDecor(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToIPlace(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToKFocus(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToSize(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToShowFeedback(XrmValue *, Cardinal, XrmValue *, XrmValue *);
void WmCvtStringToUsePPosition(XrmValue *, Cardinal, XrmValue *, XrmValue *);
...
void WmCvtStringToCFocus(XrmValue *args, Cardinal numArgs, XrmValue *fromVal,
                         XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  int len;
...
void WmCvtStringToCDecor(XrmValue *args, Cardinal numArgs, XrmValue *fromVal,
                         XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  int len;
...
#define WM_DECOR_RESIZE_STR (unsigned char *)"resize"
   ...
   */
  if (*pch && (NextToken(pch, &len, &pchNext)) && (*pch == '-')) {
    cval = WM_DECOR_ALL;
    fHit = True;
  } else {
    cval = WM_DECOR_NONE;
  ...
  while (*pch && NextToken(pch, &len, &pchNext)) {
    ...
    if (*pch == '+') {
      ...
      if (StringsAreEqual(pch, WM_DECOR_ALL_STR, len)) {
        cval = fAddNext ? (cval | WM_DECOR_ALL) : (cval & ~WM_DECOR_ALL);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, WM_DECOR_TITLE_STR, len)) {
        cval = fAddNext ? (cval | WM_DECOR_TITLE) : (cval & ~WM_DECOR_TITLEBAR);
        fHit = True;
      } else if (StringsAreEqual(pch, WM_DECOR_TITLEBAR_STR, len)) {
        cval =
            fAddNext ? (cval | WM_DECOR_TITLEBAR) : (cval & ~WM_DECOR_TITLEBAR);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, WM_DECOR_MINIMIZE_STR, len)) {
        ...
        cval = fAddNext ? (cval | WM_DECOR_SYSTEM) : (cval & ~MWM_DECOR_MENU);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, WM_DECOR_RESIZE_STR, len) ||
        ...
        cval =
            fAddNext ? (cval | WM_DECOR_RESIZEH) : (cval & ~MWM_DECOR_RESIZEH);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, WM_DECOR_BORDER_STR, len)) {
        cval = fAddNext ? (cval | WM_DECOR_BORDER) : (cval & ~WM_DECOR_BORDER);
        fHit = True;
      }
    }
...
void WmCvtStringToCFunc(XrmValue *args, Cardinal numArgs, XrmValue *fromVal,
                        XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  int len;
  ...
  while (*pch && NextToken(pch, &len, &pchNext)) {
    ...
    if (*pch == '+') {
      ...
      if (StringsAreEqual(pch, WM_FUNC_ALL_STR, len)) {
        cval = fAddNext ? (cval | WM_FUNC_ALL) : (cval & ~WM_FUNC_ALL);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, WM_FUNC_RESIZE_STR, len)) {
        cval = fAddNext ? (cval | MWM_FUNC_RESIZE) : (cval & ~MWM_FUNC_RESIZE);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, WM_FUNC_MINIMIZE_STR, len)) {
        cval =
            fAddNext ? (cval | MWM_FUNC_MINIMIZE) : (cval & ~MWM_FUNC_MINIMIZE);
        fHit = True;
      } else if (StringsAreEqual(pch, WM_FUNC_MAXIMIZE_STR, len)) {
        cval =
            fAddNext ? (cval | MWM_FUNC_MAXIMIZE) : (cval & ~MWM_FUNC_MAXIMIZE);
        fHit = True;
      } else if (StringsAreEqual(pch, WM_FUNC_MOVE_STR, len)) {
        cval = fAddNext ? (cval | MWM_FUNC_MOVE) : (cval & ~MWM_FUNC_MOVE);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, WM_FUNC_CLOSE_STR, len)) {
        cval = fAddNext ? (cval | MWM_FUNC_CLOSE) : (cval & ~MWM_FUNC_CLOSE);
        fHit = True;
      }
    }
...
void WmCvtStringToFrameStyle(XrmValue *args, Cardinal numArgs,
                             XrmValue *fromVal, XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  int len;
...
void WmCvtStringToIDecor(XrmValue *args, Cardinal numArgs, XrmValue *fromVal,
                         XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  int len;
...
void WmCvtStringToIPlace(XrmValue *args, Cardinal numArgs, XrmValue *fromVal,
                         XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  int len;
    ...
    if ((*pch == 'B') || (*pch == 'b')) {
      if (StringsAreEqual(pch, ICON_PLACE_BOTTOM_STR, len)) {
        if (!fPrimarySet) {
          ...
          if (!(cval & (ICON_PLACE_BOTTOM_PRIMARY | ICON_PLACE_TOP_PRIMARY))) {
      ...
      if (StringsAreEqual(pch, ICON_PLACE_RIGHT_STR, len)) {
        if (!fPrimarySet) {
          ...
          if (!(cval & (ICON_PLACE_RIGHT_PRIMARY | ICON_PLACE_LEFT_PRIMARY))) {
      ...
      if (StringsAreEqual(pch, ICON_PLACE_TOP_STR, len)) {
        if (!fPrimarySet) {
          ...
          if (!(cval & (ICON_PLACE_TOP_PRIMARY | ICON_PLACE_BOTTOM_PRIMARY))) {
...
void WmCvtStringToKFocus(XrmValue *args, Cardinal numArgs, XrmValue *fromVal,
                         XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  int len;
...
void WmCvtStringToSize(XrmValue *args, Cardinal numArgs, XrmValue *fromVal,
                       XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  static WHSize cval;
...
void WmCvtStringToShowFeedback(XrmValue *args, Cardinal numArgs,
                               XrmValue *fromVal, XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  int len;
  ...
  while (*pch && NextToken(pch, &len, &pchNext)) {
    ...
    if ((*pch == 'A') || (*pch == 'a')) {
      if (StringsAreEqual(pch, SHOW_FB_ALL_STR, len)) {
        cval = fAddNext ? (cval | WM_SHOW_FB_ALL) : (cval & ~WM_SHOW_FB_ALL);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, SHOW_FB_KILL_STR, len)) {
        cval = fAddNext ? (cval | WM_SHOW_FB_KILL) : (cval & ~WM_SHOW_FB_KILL);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, SHOW_FB_MOVE_STR, len)) {
        cval = fAddNext ? (cval | WM_SHOW_FB_MOVE) : (cval & ~WM_SHOW_FB_MOVE);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, SHOW_FB_QUIT_STR, len)) {
        cval = fAddNext ? (cval | WM_SHOW_FB_QUIT) : (cval & ~WM_SHOW_FB_QUIT);
        fHit = True;
      }
    }
      ...
      if (StringsAreEqual(pch, SHOW_FB_RESIZE_STR, len)) {
        cval =
            fAddNext ? (cval | WM_SHOW_FB_RESIZE) : (cval & ~WM_SHOW_FB_RESIZE);
        fHit = True;
...
void WmCvtStringToUsePPosition(XrmValue *args, Cardinal numArgs,
                               XrmValue *fromVal, XrmValue *toVal) {
  unsigned char *pch = (unsigned char *)(fromVal->addr);
  unsigned char *pchNext;
  int len;
...
Boolean StringsAreEqual(unsigned char *pch1, unsigned char *pch2, int len) {
  ...
  while (len && ((chlen1 = mbtowc(&wch1, (char *)pch1, MB_CUR_MAX)) > 0) &&
