...
extern Widget DtCreateClock(Widget parent, String name, ArgList arglist,
                            Cardinal argcount);
...
static void ClockTick(XtPointer client_data, XtIntervalId *id);
static void DrawLine(DtClockGadget w, Dimension blank_length, Dimension length,
                     double fraction_of_a_circle);
static void DrawHand(DtClockGadget w, Dimension length, Dimension width,
                     double fraction_of_a_circle);
static void SetSegment(DtClockGadget w, int x1, int y1, int x2, int y2);
static int wmround(double x);
static void DrawClockFace(DtClockGadget g);
static void Initialize(Widget request_w, Widget new_w);
static void Destroy(Widget w);
static void Resize(Widget widget);
static void Draw(DtClockGadget g, Drawable drawable, Position x, Position y,
                 Dimension w, Dimension h, Dimension h_t, Dimension s_t,
                 unsigned char s_type, unsigned char fill_mode);
static void UpdateGCs(DtClockGadget g);
...
**	Resource List
*/
#define R_Offset(field) XtOffset(DtClockGadget, clock.field)

static XtResource resources[] = {
    {XmNclockInterval, XmCInterval, XmRInt, sizeof(int),
     R_Offset(clock_interval), XmRImmediate, (XtPointer)60},
    {XmNchime, XmCBoolean, XmRBoolean, sizeof(Boolean), R_Offset(chime),
...
static void ClockTick(XtPointer client_data, XtIntervalId *id) {
  ...
  if (id || !w->clock.interval_id)
    w->clock.interval_id =
        XtAppAddTimeOut(XtWidgetToApplicationContext((Widget)w),
                        G_ClockInterval(w) * 1000, ClockTick, (XtPointer)w);
  ...
  if ((G_Width(w) < (Dimension)((G_ClockWidth(w) / 2) + w->clock.centerX)) ||
      (G_Height(w) < (Dimension)((G_ClockHeight(w) / 2) + w->clock.centerY)))
    return;
  ...
  XDrawLines(dpy, win, G_ClockHandGC(w), w->clock.segbuff, VERTICES_IN_HANDS,
             CoordModeOrigin);
  XFillPolygon(dpy, win, G_ClockHandGC(w), w->clock.segbuff, VERTICES_IN_HANDS,
  ...
  XFillPolygon(dpy, win, G_ClockHandGC(w), w->clock.hour, VERTICES_IN_HANDS,
               Convex, CoordModeOrigin);
  XDrawLines(dpy, win, G_ClockHandGC(w), w->clock.hour, VERTICES_IN_HANDS,
             CoordModeOrigin);
...
static void EraseHands(DtClockGadget w, struct tm *tm) {
  ...
  if (!tm || tm->tm_min != w->clock.otm.tm_min ||
    ...
    XDrawLines(dpy, win, gc, w->clock.segbuff, VERTICES_IN_HANDS,
               CoordModeOrigin);
    XDrawLines(dpy, win, gc, w->clock.hour, VERTICES_IN_HANDS, CoordModeOrigin);
    XFillPolygon(dpy, win, gc, w->clock.segbuff, VERTICES_IN_HANDS, Convex,
                 CoordModeOrigin);
    XFillPolygon(dpy, win, gc, w->clock.hour, VERTICES_IN_HANDS, Convex,
                 CoordModeOrigin);
  }
}
...
static void DrawClockFace(DtClockGadget g) {
  ...
  int i;
  int delta = (int)(g->clock.radius - g->clock.tick_spacing) / 3;
  ...
  for (i = 0; i < 60; i++) {
    if (draw_minute_ticks) {
      if ((i % 5) == 0)
        DrawLine(g, g->clock.tick_spacing, g->clock.radius, ((double)i) / 60.);
      else
        DrawLine(g, g->clock.radius - delta, g->clock.radius,
                 ((double)i) / 60.);
    } else if ((i % 15) == 0)
      DrawLine(g, g->clock.radius - 1, g->clock.radius, ((double)i) / 60.);
  }

  /*	Draw clock face.
   */
  XDrawSegments(XtDisplay(g), XtWindow(g), G_ClockHandGC(g),
                (XSegment *)&(g->clock.segbuff[0]), g->clock.numseg / 2);
...
/* ARGSUSED */
static void Initialize(Widget request_w, Widget new_w) {
  DtClockGadget request = (DtClockGadget)request_w, new = (DtClockGadget)new_w;
  Dimension w, h, h_t = G_HighlightThickness(new), s_t = G_ShadowThickness(new);
...
static void Destroy(Widget w) {
  ...
  if (g->clock.interval_id)
    XtRemoveTimeOut(g->clock.interval_id);
...
static void Resize(Widget widget) {
  ...
  Dimension s_t = G_ShadowThickness(g), h_t = G_HighlightThickness(g),
            p_w = G_PixmapWidth(g), p_h = G_PixmapHeight(g),
            m_w = G_MarginWidth(g), m_h = G_MarginHeight(g),
            s_w = G_StringWidth(g), s_h = G_StringHeight(g),
            v_pad = 2 * (s_t + h_t + m_h), h_pad = 2 * (s_t + h_t + m_w),
            spacing = G_Spacing(g), w = G_Width(g), h = G_Height(g);
  int radius;
  ...
  G_GetPositions((DtIconGadget)g, w, h, h_t, s_t, &pix_x, &pix_y, &str_x,
                 &str_y);
  ...
  if (G_ClockWidth(g) < (Dimension)(SIZE_DEFAULT / 2)) {
    g->clock.tick_spacing = ((Dimension)(TICK_FRACT * g->clock.radius) / 100);
...
static void Draw(DtClockGadget g, Drawable drawable, Position x, Position y,
  ...
  if ((fill_mode != XmFILL_NONE) && (fill_mode != XmFILL_TRANSPARENT))
    XFillRectangle(d, drawable, gc, x + h_t, y + h_t, w - 2 * h_t, h - 2 * h_t);

  G_GetPositions((DtIconGadget)g, w, h, h_t, s_t, &p_x, &p_y, &s_x, &s_y);

  if (G_Pixmap(g)) {
    width =
        ((Dimension)(p_x + s_t + h_t) >= G_Width(g))
            ? 0
            : min(G_PixmapWidth(g), (Dimension)(G_Width(g) - p_x - s_t - h_t));
    height = ((Dimension)(p_y + s_t + h_t) >= G_Height(g))
                 ? 0
                 : min(G_PixmapHeight(g),
    ...
    if ((gc != NULL) && (pix != XmUNSPECIFIED_PIXMAP))
      XCopyArea(d, pix, drawable, gc, 0, 0, width, height, x + p_x, y + p_y);
  }
  ...
  if ((behavior == XmICON_BUTTON || behavior == XmICON_DRAG) &&
    ...
    gc = G_ArmedGC(g);
           ...
           ((G_Armed(g) && !G_Set(g)) || (!G_Armed(g) && G_Set(g))))
    gc = G_ArmedGC(g);
  else
    gc = G_NormalGC(g);
    ...
    if (clip.width > 0 && clip.height > 0) {
      if (bMono) {
        if ((s_x - 2) >= (Position)(h_t + s_t))
          XFillRectangle(d, drawable, G_ArmedBackgroundGC(g), x + s_x - 2,
                         y + s_y, 2, G_StringHeight(g));
        XmStringDrawImage(d, drawable, G_FontList(g), G_String(g),
                          G_NormalGC(g), x + s_x, y + s_y, clip.width,
        ...
        XmStringDraw(d, drawable, G_FontList(g), G_String(g), G_NormalGC(g),
                     x + s_x, y + s_y, clip.width, XmALIGNMENT_BEGINNING,
                     XmSTRING_DIRECTION_L_TO_R, &clip);
      }
    }
  }
...
static void UpdateGCs(DtClockGadget g) {
  ...
  XFontStruct *font;
  Boolean font_rtn;
  DtIconGadgetClass super = (DtIconGadgetClass)XtSuperclass(g);
...
**-------------------------------------------------------------
*/
Widget DtCreateClock(Widget parent, String name, ArgList arglist,
                     Cardinal argcount) {
  return (XtCreateWidget(name, dtClockGadgetClass, parent, arglist, argcount));
}
