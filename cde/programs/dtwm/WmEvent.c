...
typedef struct {
  ...
  Boolean (*parseProc)(unsigned char **, WmFunction, String *);
} FunctionTableEntry;
  ...
  for (scr = 0; scr < wmGD.numScreens; scr++) {
    ...
    if (pSD->managed) {
      ...
      if (pSD->buttonBindings) {
         ...
         * release events.
         */
        setAttributes.event_mask |= (ButtonPressMask | ButtonReleaseMask);
      }

      XChangeWindowAttributes(DISPLAY, pSD->rootWindow, CWEventMask,
                              &setAttributes);
      ...
      if (pSD->keySpecs) {
        SetupKeyBindings(pSD->keySpecs, pSD->rootWindow, GrabModeSync,
                         F_CONTEXT_ALL);
      }
...
static unsigned int *_WmGrabMasks(unsigned int modifiers, int *pnum_masks) {
  ...
  if ((pRetMasks == NULL) || (len_ret_masks < num_masks + 2)) {
    ...
    pRetMasks = (unsigned int *)XtCalloc(len_ret_masks, sizeof(unsigned int));
  }
...
void WmGrabKey(Display *display, int keycode, unsigned int modifiers,
  ...
  for (i = 0; i < num_masks; i++, pGrabMasks++) {
    XGrabKey(display, keycode, *pGrabMasks, grab_window, owner_events,
             pointer_mode, keyboard_mode);
  }
}
...
void WmGrabButton(Display *display, unsigned int button, unsigned int modifiers,
                  Window grab_window, unsigned int event_mask,
  ...
  for (i = 0; i < num_masks; i++, pGrabMasks++) {
    XGrabButton(display, button, *pGrabMasks, grab_window, event_mask,
                owner_events, pointer_mode, keyboard_mode, confine_to, cursor);
  }
}
...
int SetupKeyBindings(KeySpec *keySpecs, Window grabWindow, int keyboardMode,
                     long context) {
  KeySpec *keySpec;
  int setCount = 0;
  Boolean iconContext;
...
#endif
      if (((F_CONTEXT_ICON ==
            (keySpec->context ^ (F_CONTEXT_ICONBOX | F_SUBCONTEXT_IB_IICON |
        ...
        WmGrabKey(DISPLAY, keySpec->keycode, keySpec->state, grabWindow, False,
...
Boolean WmDispatchMenuEvent(XButtonEvent *event) {
  ...
  if (event->type == KeyPress) {
    if (wmGD.menuActive->accelKeySpecs) {
      ...
      menuBtnPtr =
          wmGD.menuActive->menuButtons + (wmGD.menuActive->menuButtonSize - 1);
      ...
      while (keySpecs) {
        if ((keyEvent->keycode == keySpecs->keycode) &&
          ...
          doXtDispatchEvent = XtIsSensitive(menuBtnPtr->buttonWidget);
          break;
        }
      ...
      doXtDispatchEvent =
          doXtDispatchEvent &&
          HandleKeyPress((XKeyEvent *)event, wmGD.menuActive->accelKeySpecs,
                         checkContext, context, TRUE, (ClientData *)NULL);
    }
    ...
    if (wmGD.menuActive && wmGD.menuUnpostKeySpec) {
      if ((wmGD.menuUnpostKeySpec->keycode == event->button) &&
        ...
        doXtDispatchEvent = False;
           ...
           (event->x_root <
            (wmGD.hotspotRectangle.x + (short)wmGD.hotspotRectangle.width)) &&
           (event->y_root <
            (wmGD.hotspotRectangle.y + (short)wmGD.hotspotRectangle.height)) &&
    ...
    if (event->type == ButtonRelease) {
      if (pCD) {
        ...
        if (((XButtonEvent *)event)->time > wmGD.clickData.time) {
          timeDiff = ((XButtonEvent *)event)->time - wmGD.clickData.time;
        } else {
          timeDiff = ~wmGD.clickData.time + ((XButtonEvent *)event)->time + 1;
        }
      ...
      if (wmGD.clickData.doubleClickContext == F_SUBCONTEXT_W_SYSTEM) {
      ...
      } else if (wmGD.clickData.doubleClickContext == F_SUBCONTEXT_I_ALL) {
        /*
         * Normalize the icon.
         */
        int newState;
        ...
        doXtDispatchEvent = False;
      } else if ((wmGD.clickData.doubleClickContext == F_SUBCONTEXT_IB_IICON) ||
        ...
        /*		F_Normalize_And_Raise ((String)NULL, pCD, (XEvent
         * *)NULL);
         */
        doXtDispatchEvent = False;
      }
...
Boolean WmDispatchWsEvent(XEvent *event) {
  ...
  case UnmapNotify: {
    /* BEGIN CR 5183 */
    if ((!XFindContext(DISPLAY, event->xunmap.window, wmGD.windowContextType,
                       (XPointer *)&pCD)) &&
...
Boolean HandleWsKeyPress(XKeyEvent *keyEvent) {
  ...
  if (checkKeyEvent) {
    ...
    if (HandleKeyPress(keyEvent, ACTIVE_PSD->keySpecs, TRUE, context, FALSE,
                       (ClientData *)NULL) &&
        ACTIVE_PSD->acceleratorMenuCount) {
      for (n = 0;
           ((keyEvent->keycode != 0) && (n < ACTIVE_PSD->acceleratorMenuCount));
           n++) {
    ...
    /*
     * Fix for CR 3117 - Do the XAllowEvents after calling HandleKeyPress so
     * that keys meant for an application can be sent to it.
     */
    XAllowEvents(DISPLAY, AsyncKeyboard, CurrentTime);
    /*
     * End Fix for CR 3117
...
Boolean HandleKeyPress(XKeyEvent *keyEvent, KeySpec *keySpecs,
                       Boolean checkContext, Context context, Boolean onlyFirst,
                       ClientData *pCD) {
  ...
  while (processKey && keySpecs) {
    if (((keyEvent->state == keySpecs->state) ||
      ...
      if ((!checkContext) || (context & keySpecs->context)) {
        ...
        if (!(keySpecs->wmFunction(keySpecs->wmFuncArgs, functionClient,
                                   (XEvent *)keyEvent))) {
          /*
...
Boolean CheckForButtonAction(XButtonEvent *buttonEvent, Context context,
                             Context subContext, ClientData *pCD) {
    ...
    if ((buttonEvent->button == buttonSpec->button) &&
      ...
      if ((buttonEvent->type == buttonSpec->eventType) &&
        ...
        if (buttonEvent->type == ButtonRelease) {
          ...
          if ((buttonSpec->subContext | wmGD.clickData.doubleClickContext) !=
              buttonSpec->subContext) {
            /* click binding and event contexts do not match */
            buttonSpec = buttonSpec->nextButtonSpec;
            continue;
...
void IdentifyEventContext(XButtonEvent *event, ClientData *pCD,
  ...
  eventOnRoot = (event->window == ACTIVE_ROOT) ? True : False;
  ...
  if (pCD) {
    ...
    if (actionWindow == pCD->clientFrameWin) {
      *pContext = F_CONTEXT_WINDOW;
      ...
      if (eventOnRoot) {
        clientX = event->x - (pCD->maxConfig ? pCD->maxX : pCD->clientX) +
                  pCD->clientOffset.x;
        clientY = event->y - (pCD->maxConfig ? pCD->maxY : pCD->clientY) +
                  pCD->clientOffset.y;
      } else {
        clientX = event->x;
        clientY = event->y;
    ...
    if (((*pContext & F_CONTEXT_WINDOW) && (pCD->clientState != NORMAL_STATE) &&
...
void ProcessClickBPress(XButtonEvent *buttonEvent, ClientData *pCD,
  ...
  if (wmGD.clickData.doubleClickPending &&
      ...
      (pCD == wmGD.clickData.pCD) && (context == wmGD.clickData.context)) {
    /*
    ...
    if (timeDiff < wmGD.doubleClickTime) {
      ...
      wmGD.clickData.doubleClickContext =
          subContext | wmGD.clickData.subContext;
    }
  }
  ...
  if ((buttonEvent->button == SELECT_BUTTON) &&
      ((buttonEvent->state == 0) || (NOLOCKMOD(buttonEvent->state) == 0))) {
  ...
  if (!(pCD && (buttonEvent->window == pCD->clientBaseWin) && passButton)) {
    ...
    wmGD.clickData.releaseState =
        buttonEvent->state | buttonModifierMasks[buttonEvent->button];
...
void HandleDtWmRequest(WmScreenData *pSD, XEvent *pev) {
  Boolean more = True;
  char *pchReq = NULL;
  ...
  while (more) {
    ...
    if (pchReq) {
      ...
      iFuncIndex = ParseWmFunction(&lineP, CRS_BUTTON | CRS_KEY, &wmFunction);
      ...
      if (iFuncIndex != F_NOP_INDEX) {
         ...
         */
        if ((*(functionTable[iFuncIndex].parseProc))(&lineP, wmFunction,
                                                     (void *)&wmFuncArgs)) {
          /*
           ...
           */
          if (wmFuncArgs) {
            if ((functionTable[iFuncIndex].parseProc == ParseWmFuncStrArg) ||
              ...
              while (pAP->numArgs > 0) {
                XtFree((char *)pAP->aap[--(pAP->numArgs)].u.file.name);
              }
              XtFree((char *)pAP);
            }
          }
        ...
        if (pch) {
          ...
          Warning(((char *)GETMESSAGE(
              32, 3, "Missing path name for backdrop change request.")));
        }
        if (pchFile) {
          strcpy(pchFile, pch);
          ...
          if (pch) {
            ...
            if (pch) {
              ...
              Warning(((char *)GETMESSAGE(32, 5,
                                          "Missing backdrop image style number "
                                          "for backdrop change request.")));
            }
          } else {
            Warning(((char *)GETMESSAGE(
                32, 4, "Missing pixmap id for backdrop change request.")));
          }
          XtFree(pchFile);
        } else {
          Warning(((char *)GETMESSAGE(
              32, 2, "Insufficient memory to handle backdrop change.")));
        }
      }
      if (sRequest) {
        XtFree((char *)sRequest);
  ...
  if (!wmGD.menuActive && ((enterEvent->mode == NotifyNormal) ||
    ...
    if (wmGD.keyboardFocusPolicy == KEYBOARD_FOCUS_POINTER) {
      Do_Focus_Key((ClientData *)NULL, enterEvent->time, ALWAYS_SET_FOCUS);
               ...
               (wmGD.keyboardFocus == NULL) && enterEvent->focus) {
      /*
...
void HandleWsLeaveNotify(XLeaveWindowEvent *leaveEvent) {
  ...
  if (!wmGD.menuActive && ((leaveEvent->detail == NotifyNonlinear) ||
  ...
  if (XFindContext(DISPLAY, configureEvent->window, wmGD.windowContextType,
    ...
    if (WmGetWindowAttributes(configureEvent->window)) {
      ...
      if (!configChanged) {
        ...
        XSendEvent(DISPLAY, configureEvent->window, False, StructureNotifyMask,
                   (XEvent *)&notifyEvent);
      }
    }
  } else {
    ...
    if (wmGD.keyboardFocus && (focusEvent->detail != NotifyInferior)) {
      if (sameScreen) {
        ...
        if (wmGD.keyboardFocusPolicy == KEYBOARD_FOCUS_EXPLICIT) {
          Do_Focus_Key(wmGD.keyboardFocus, GetTimestamp(), ALWAYS_SET_FOCUS);
        } else {
      ...
      if (sameScreen) {
        if (wmGD.keyboardFocusPolicy == KEYBOARD_FOCUS_EXPLICIT) {
          if (((focusEvent->detail == NotifyInferior) ||
            ...
            Do_Focus_Key((ClientData *)pCD, GetTimestamp(), ALWAYS_SET_FOCUS);
          }
...
Time GetTimestamp(void) {
   ...
   */
  XSync(DISPLAY, False);
  if (XCheckWindowEvent(DISPLAY, pSD->wmWorkspaceWin, PropertyChangeMask,
                        &event)) {
...
void PullExposureEvents(void) {
   ...
   */
  while (
      XCheckMaskEvent(DISPLAY, ExposureMask | VisibilityChangeMask, &event) ||
      XCheckMaskEvent(DISPLAY1, ExposureMask | VisibilityChangeMask, &event)) {
    /*
...
Boolean ReplayedButtonEvent(XButtonEvent *pevB1, XButtonEvent *pevB2) {
  ...
  if ((pevB1->type == pevB2->type) &&
      ...
      (pevB1->display == pevB2->display) && (pevB1->window == pevB2->window) &&
      (pevB1->root == pevB2->root) && (pevB1->subwindow == pevB2->subwindow) &&
      (pevB1->time == pevB2->time) && (pevB1->x == pevB2->x) &&
      (pevB1->y == pevB2->y) && (pevB1->x_root == pevB2->x_root) &&
      (pevB1->y_root == pevB2->y_root) && (pevB1->state == pevB2->state) &&
