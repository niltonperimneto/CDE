...
static void BuildLockMaskSequence(void) {
   ...
   */
  wmGD.pLockMaskSequence =
      (unsigned int *)XtCalloc(num_masks + 1, sizeof(unsigned int));
  ...
  while (mask && thisbit) {
    ...
    for (j = 0, k = run - 1; j < num_masks; j++, k--) {
      ...
      if (bit_on)
        wmGD.pLockMaskSequence[j] |= thisbit;
    }
  }
}
 ...
 * this set.)
 */
static KeySym pksLockingMods[] = {XK_Scroll_Lock, XK_Kana_Lock, XK_Num_Lock,
                                  XK_Mode_switch};
...
static void MappingEventHandler(Widget w, XtPointer client_data, XEvent *event,
...
void InitWmGlobal(int argc, char *argv[], char *environ[]) {
  ...
  wmGD.display =
      XtOpenDisplay(wmGD.mwmAppContext, NULL, wmGD.mwmName, res_class, NULL, 0,
...
#if defined(sun) && defined(ALLPLANES)
  {
    ...
    wmGD.allplanes = XAllPlanesQueryExtension(wmGD.display, &dummy, &dummy);
  }
  ...
  argnum = 0;
  XtSetArg(args[argnum], XtNgeometry, NULL);
  argnum++;
  XtSetArg(args[argnum], XtNx, 10000);
  argnum++;
  XtSetArg(args[argnum], XtNy, 0);
  argnum++;
  XtSetArg(args[argnum], XtNwidth, 10);
  argnum++;
  XtSetArg(args[argnum], XtNheight, 10);
  argnum++;
  XtSetArg(args[argnum], XtNmappedWhenManaged, False);
  argnum++;
  XtSetArg(args[argnum], XtNjoinSession, True);
  argnum++;
  XtSetArg(args[argnum], XtNrestartStyle, SmRestartNever);
  argnum++;
  XtSetArg(args[argnum], XtNargc, savedArgc);
  argnum++;
  XtSetArg(args[argnum], XtNargv, wmGD.argv);
  argnum++;

  /* create topmost shell (application shell) */
  wmGD.topLevelW = XtAppCreateShell(NULL, res_class, sessionShellWidgetClass,
                                    DISPLAY, args, argnum);

  XtAddEventHandler(wmGD.topLevelW, NoEventMask, True, MappingEventHandler,
                    NULL);
   ...
   */
  {
    ...
    _DtWsmSetBackdropSearchPath(XScreenOfDisplay(DISPLAY, sNum), sBdPath,
                                useMultiColorIcons);
  ...
  if (!(wmGD.Screens =
            (WmScreenData *)XtCalloc(wmGD.numScreens, sizeof(WmScreenData)))) {
    ShowWaitState(FALSE);
    ...
    for (scr = 0; scr < wmGD.numScreens; scr++) {
      ...
      XChangeWindowAttributes(DISPLAY, RootWindow(DISPLAY, sNum), CWEventMask,
                              &sAttributes);
      /*
      ...
      if ((wmGD.errorFlag) &&
        ...
        sprintf(pch,
                ((char *)GETMESSAGE(
                    40, 3, "Another window manager is running on screen %d")),
                sNum);
        Warning((char *)&pch[0]);
        ...
        if (!processedGlobalResources) {
          enum {
            XA_DT_SESSION_HINTS,
            XA_DT_SM_WM_PROTOCOL,
            XA_DT_SM_START_ACK_WINDOWS,
            XA_DT_SM_STOP_ACK_WINDOWS,
            XA_DT_WM_WINDOW_ACK,
            XA_DT_WM_EXIT_SESSION,
            XA_DT_WM_LOCK_DISPLAY,
            XA_DT_WM_READY,
            NUM_ATOMS
          };
          ...
          InitOLCompat();
          wmGD.hasShape = XShapeQueryExtension(DISPLAY, &wmGD.shapeEventBase,
                                               &wmGD.shapeErrorBase);
          ...
          wmGD.iconDefaultTitle = XmStringCreateLocalized(DEFAULT_ICON_TITLE);
           ...
           */
          wmGD.display1 =
              XtOpenDisplay(wmGD.mwmAppContext, NULL, wmGD.mwmName, res_class,
                            NULL, 0, &dpy2Argc, dpy2Argv);
          if (!wmGD.display1) {
            ShowWaitState(FALSE);
            Warning(((char *)GETMESSAGE(
                40, 4, "Could not open second display connection.")));
            ExitWM(WM_ERROR_EXIT_VALUE);
          }

          _DtGetSmWindow(DISPLAY, RootWindow(DISPLAY, 0), &wmGD.dtSmWindow);
          ...
          wmGD.topLevelW1 =
              XtAppCreateShell(NULL, res_class, applicationShellWidgetClass,
                               DISPLAY1, args, argnum);
  ...
  argnum = 0;
  XtSetArg(args[argnum], XmNuseAsyncGeometry, True);
  argnum++;
  ...
  {
    enum {
      XA_DT_WORKSPACE_HINTS,
      XA_DT_WORKSPACE_PRESENCE,
      XA_DT_WORKSPACE_INFO,
      XA_WmNall,
      XA_DT_WORKSPACE_EMBEDDED_CLIENTS,
      XA_DT_WM_REQUEST,
      XA_DT_WORKSPACE_LIST,
      XA_DT_WORKSPACE_CURRENT,
      NUM_ATOMS
    };
    static char *atom_names[] = {_XA_DT_WORKSPACE_HINTS,
                                 _XA_DT_WORKSPACE_PRESENCE,
                                 _XA_DT_WORKSPACE_INFO,
                                 WmNall,
                                 _XA_DT_WORKSPACE_EMBEDDED_CLIENTS,
                                 _XA_DT_WM_REQUEST,
                                 _XA_DT_WORKSPACE_LIST,
  ...
  wmGD.xa_SM_CLIENT_ID = XmInternAtom(DISPLAY, _XA_DT_SM_CLIENT_ID, False);
  wmGD.xa_WMSAVE_HINT = XmInternAtom(DISPLAY, _XA_DT_WMSAVE_HINT, False);
  ...
  XChangeWindowAttributes(DISPLAY, XtWindow(wmGD.topLevelW), CWOverrideRedirect,
                          &sAttributes);
    ...
    for (scr = 0; scr < wmGD.numScreens; scr++) {
      ...
      if (pSD->managed) {
        ...
        if (wmGD.useFrontPanel && (pSD == wmGD.dtSD)) {
          ...
          wmGD.dtSD->wPanelist = WmPanelistAllocate(
              pSD->screenTopLevelW1, (XtPointer)&wmGD, (XtPointer)pSD);
          ...
          if (pPanelist != NULL && O_Shell(pPanelist)) {
             ...
             */
            iconBitmap = XCreateBitmapFromData(
                DISPLAY, pSD->rootWindow, (char *)fntpl_i_bm_bits,
                fntpl_i_bm_width, fntpl_i_bm_height);

            ac = 0;
            XtSetArg(al[ac], XmNiconPixmap, iconBitmap);
            ac++;
            XtSetValues(O_Shell(pPanelist), al, ac);
          }
        }

        if (wmGD.useFrontPanel && pSD->wPanelist && (pSD == wmGD.dtSD)) {
          /*
...
#define LENCBUFFER 256
  ...
  if (!(strcmp((char *)wmGD.screenNames[sNum], UNSPECIFIED_SCREEN_NAME))) {
    ...
    if ((wmGD.screenNames[sNum] = (unsigned char *)XtRealloc(
             (char *)wmGD.screenNames[sNum], buf_size)) == NULL) {
      Warning(((char *)GETMESSAGE(
          40, 7, "Cannot create enough memory for the screen names")));
      ExitWM(WM_ERROR_EXIT_VALUE);
    } else {
   ...
   * Save screen context
   */
  XSaveContext(DISPLAY, pSD->rootWindow, wmGD.screenContextType, (caddr_t)pSD);
  /*
  ...
  XtSetArg(args[argnum], XtNx, 10000);
  argnum++;
  XtSetArg(args[argnum], XtNy, 10000);
  argnum++;
  XtSetArg(args[argnum], XtNwidth, 10);
  argnum++;
  XtSetArg(args[argnum], XtNheight, 10);
  argnum++;
  XtSetArg(args[argnum], XtNoverrideRedirect, True);
  argnum++;

  XtSetArg(args[argnum], XtNdepth, DefaultDepth(DISPLAY, sNum));
  argnum++;
  XtSetArg(args[argnum], XtNscreen, ScreenOfDisplay(DISPLAY, sNum));
  argnum++;
  XtSetArg(args[argnum], XtNcolormap, DefaultColormap(DISPLAY, sNum));
  argnum++;

  pSD->screenTopLevelW =
      XtCreatePopupShell((String)wmGD.screenNames[sNum], vendorShellWidgetClass,
                         wmGD.topLevelW, args, argnum);
  ...
  XtSetArg(args[argnum], XtNdepth, DefaultDepth(DISPLAY1, sNum));
  argnum++;
  XtSetArg(args[argnum], XtNscreen, ScreenOfDisplay(DISPLAY1, sNum));
  argnum++;
  XtSetArg(args[argnum], XtNcolormap, DefaultColormap(DISPLAY1, sNum));
  argnum++;

  pSD->screenTopLevelW1 =
      XtCreatePopupShell((String)wmGD.screenNames[sNum], vendorShellWidgetClass,
                         wmGD.topLevelW1, args, argnum);
   ...
   */
  for (wsnum = 0, pwsI = pSD->pWS; wsnum < pSD->numWorkspaces;
       wsnum++, pwsI++) {
    /*
  ...
  if ((token2 = (char *)strtok(NULL, ".")) || /* parse off dpy & scr # */
      (token2 = (char *)strtok(NULL, "")) || (displayName[0] == ':')) {
    if (displayName[0] == ':') /* local dpy (special case) */
    {
      ...
      snprintf(buffer, LENCBUFFER, "DISPLAY=%s:%s.%d", token1, token2, sNum);
    }
    ...
    if ((pSD->displayString =
      ...
      Warning(
          ((char *)GETMESSAGE(40, 9, "Insufficient memory for displayString")));
    } else {
...
void InitWmWorkspace(WmWorkspaceData *pWS, WmScreenData *pSD) {
   ...
   */
  argnum = 0;
  XtSetArg(args[argnum], XtNdepth, DefaultDepth(DISPLAY, pSD->screen));
  argnum++;
  XtSetArg(args[argnum], XtNscreen, ScreenOfDisplay(DISPLAY, pSD->screen));
  argnum++;
  XtSetArg(args[argnum], XtNcolormap, DefaultColormap(DISPLAY, pSD->screen));
  argnum++;
  XtSetArg(args[argnum], XtNwidth, 5);
  argnum++;
  XtSetArg(args[argnum], XtNheight, 5);
  argnum++;

  pWS->workspaceTopLevelW = XtCreateWidget(pWS->name, xmPrimitiveWidgetClass,
                                           pSD->screenTopLevelW, args, argnum);
...
void SetupWmWorkspaceWindows(void) {
  ...
  for (scr = 0; scr < wmGD.numScreens; scr++) {
    ...
    if (pSD->managed) {
      ...
      pSD->wmWorkspaceWin = XCreateWindow(
          DISPLAY, pSD->rootWindow, -100, -100, 10, 10, 0, 0, InputOnly,
          CopyFromParent, (CWOverrideRedirect | CWEventMask), &sAttributes);

      XMapWindow(DISPLAY, pSD->wmWorkspaceWin);

      SetMwmInfo(pSD->rootWindow,
                 (long)((wmGD.useStandardBehavior) ? MWM_INFO_STARTUP_STANDARD
                                                   : MWM_INFO_STARTUP_CUSTOM),
                 pSD->wmWorkspaceWin);
      XSaveContext(DISPLAY, pSD->wmWorkspaceWin, wmGD.mwmWindowContextType,
                   (caddr_t)pSD);
    }
  }
...
void MakeWorkspaceCursors(void) {
  ...
  wmGD.stretchCursors[STRETCH_NORTH] = XCreateFontCursor(DISPLAY, XC_top_side);
  wmGD.stretchCursors[STRETCH_NORTH_EAST] =
      XCreateFontCursor(DISPLAY, XC_top_right_corner);
  wmGD.stretchCursors[STRETCH_EAST] = XCreateFontCursor(DISPLAY, XC_right_side);
...
void MakeWmFunctionResources(WmScreenData *pSD) {
  ...
  while (buttonSpec) {
    if (buttonSpec->wmFunction == F_Menu) {
      ...
      menuSpec = MAKE_MENU(pSD, NULL, buttonSpec->wmFuncArgs, menuContext,
                           buttonSpec->context, (MenuItem *)NULL, FALSE);
  ...
  while (keySpec) {
    if (keySpec->wmFunction == F_Menu) {
      ...
      menuSpec = MAKE_MENU(pSD, NULL, keySpec->wmFuncArgs, menuContext,
                           keySpec->context, (MenuItem *)NULL, FALSE);
...
void CopyArgv(int argc, char *argv[]) {
  ...
  if ((wmGD.argv = (char **)XtMalloc((argc + 1) * sizeof(char *))) == NULL) {
    Warning(((char *)GETMESSAGE(
        40, 10, "Insufficient memory for window manager data")));
    wmGD.argv = argv;
    dpy2Argv = argv;
  } else {
    for (i = 0; i < argc; i++) {
    ...
    if ((dpy2Argv = (char **)XtMalloc((argc + 1) * sizeof(char *))) == NULL) {
      Warning(((char *)GETMESSAGE(
          40, 11, "Insufficient memory for window manager data")));
      dpy2Argv = argv;
    } else {
      for (i = 0; i < argc; i++) {
        dpy2Argv[i] = argv[i];
...
void InitScreenNames(void) {
  ...
  if (!(wmGD.screenNames =
    ...
    ShowWaitState(FALSE);
    Warning(
        ((char *)GETMESSAGE(40, 12, "Insufficient memory for screen names")));
    ExitWM(WM_ERROR_EXIT_VALUE);
  }
  ...
  for (num = 0; num < numScreens; num++) {
    if (!(wmGD.screenNames[num] =
      ...
      ShowWaitState(FALSE);
      Warning(
          ((char *)GETMESSAGE(40, 13, "Insufficient memory for screen names")));
      ExitWM(WM_ERROR_EXIT_VALUE);
    }
...
#ifndef NO_HP_KEY_REMAP
static str_xref GetReplacementList(Display *dsp, str_xref std_xref, int count) {
  int min_kc;
  int max_kc;
  int ks_per_kc;
  int kc_count;
...
static Boolean GetBindingsProperty(Display *dsp, Atom property,
                                   String *binding) {
  Atom actual_type;
  int actual_format;
  ...
  XGetWindowProperty(dsp, RootWindow(dsp, 0), property, 0, 1000000L, FALSE,
                     XA_STRING, &actual_type, &actual_format, &num_items,
                     &bytes_after, &prop);
...
static void SetBindingsProperty(Display *dsp, Atom property, String binding) {
  XChangeProperty(dsp, RootWindow(dsp, 0), property, XA_STRING, 8,
                  PropModeReplace, (unsigned char *)binding, strlen(binding));
}

static String FixupBindingsString(String bindingsString, str_xref repl_xref) {
...
Boolean VirtKeys4DIN(Display *dsp) {
   ...
   */
  static char *prop_names[] = {"_MOTIF_BINDINGS", "_MOTIF_DEFAULT_BINDINGS"};
  static str_xref_rec std_xref[] = {
      {"hpInsertChar", "Insert"},
      {"hpDeleteChar", "Delete"},
      {"End", "F7"},
  };
  ...
  while (i < num_props) {
    if (GetBindingsProperty(dsp, prop_atoms[i], &bindingsString)) {
      String new_bstring = FixupBindingsString(bindingsString, vkeysym_xref);
      prop_existed = TRUE;
  ...
  if (!prop_existed) {
    ...
    extern void _XmVirtKeysLoadFallbackBindings(Display * dsp,
                                                String * binding);
