...
void GetClipDimensions(ClientData *pCD, Boolean fromRoot) {
  ...
  i = 0;
  XtSetArg(getArgs[i], XmNwidth, (XtArgVal)&clipWidth);
  i++;
  XtSetArg(getArgs[i], XmNheight, (XtArgVal)&clipHeight);
  i++;
  XtSetArg(getArgs[i], XmNx, (XtArgVal)&tmpX);
  i++;
  XtSetArg(getArgs[i], XmNy, (XtArgVal)&tmpY);
  i++;

  XtGetValues(P_ICON_BOX(pCD)->clipWidget, getArgs, i);

  if (fromRoot) {
    XtTranslateCoords(P_ICON_BOX(pCD)->scrolledWidget, tmpX, tmpY, &clipX,
                      &clipY);
  } else {
    clipX = tmpX;
    clipY = tmpY;
...
void HandleClientFrameMove(ClientData *pcd, XEvent *pev) {
  ...
  while (!moveDone) {
    ...
    if (firstTime) {
      ...
      pev = &event;
      GetConfigEvent(DISPLAY, grab_win, CONFIG_MASK, moveLastPointerX,
                     moveLastPointerY, moveX, moveY, moveWidth, moveHeight,
                     &event);
    }
    ...
    if (pev->type == KeyPress) {
      keyMultiplier = 1;
      while (
          keyMultiplier <= big_inc &&
          XCheckIfEvent(DISPLAY, &KeyEvent, IsRepeatedKeyEvent, (char *)pev)) {
        keyMultiplier++;
      }
    ...
    if (tmpX || tmpY) {
      ...
      if (!wmGD.movingIcon && (wmGD.showFeedback & WM_SHOW_FB_MOVE)) {
        DoFeedback(pcd, moveX, moveY, moveWidth, moveHeight, (unsigned long)0,
                   FALSE /* no size checks */);
      }
    }
  }
...
void UpdateAndDrawResize(ClientData *pcd) {
  ...
  case FRAME_RESIZE_NE:
    ...
    } else if (pcd->pSD->limitResize && (tmpHeight > (int)maxHeight) &&
  ...
  case FRAME_RESIZE_SW:
    ...
    } else if (pcd->pSD->limitResize && (tmpWidth > (int)maxWidth) &&
               (!(pcd->clientFlags & ICON_BOX))) {
  ...
  FixFrameValues(pcd, &resizeX, &resizeY, &resizeWidth, &resizeHeight,
                 TRUE /* do size checks */);
...
void HandleClientFrameResize(ClientData *pcd, XEvent *pev) {
  ...
  while (!resizeDone) {
    if (!pev) /* first time through will already have event */
    {
      ...
      GetConfigEvent(DISPLAY, grab_win, CONFIG_MASK, pointerX, pointerY,
                     resizeX, resizeY, resizeWidth, resizeHeight, &event);
    }
...
Boolean HandleResizeKeyPress(ClientData *pcd, XEvent *pev) {
  ...
  case XK_Left:
    ...
    case FRAME_NONE:
      ...
      warpX = resizeX + ((control) ? (-resizeBigWidthInc) : (-pcd->widthInc));
      break;
    ...
    default:
      warpX = pointerX + ((control) ? (-resizeBigWidthInc * keyMult)
  ...
  case XK_Up:
    ...
    case FRAME_NONE:
      ...
      warpY = resizeY + ((control) ? (-resizeBigHeightInc) : (-pcd->heightInc));
      ReGrabPointer(pcd->clientFrameWin, pev->xkey.time);
      break;
    ...
    default:
      warpX = pointerX;
      warpY = pointerY + ((control) ? (-resizeBigHeightInc * keyMult)
  ...
  case XK_Right:
    ...
    case FRAME_NONE:
      ...
      warpX = resizeX + resizeWidth - 1 +
              ((control) ? resizeBigWidthInc : pcd->widthInc);
      ReGrabPointer(pcd->clientFrameWin, pev->xkey.time);
      break;
  ...
  case XK_Down:
    ...
    case FRAME_NONE:
      ...
      warpY = resizeY + resizeHeight - 1 +
              ((control) ? resizeBigHeightInc : pcd->heightInc);
      ReGrabPointer(pcd->clientFrameWin, pev->xkey.time);
      break;
...
void DoFeedback(ClientData *pcd, int x, int y, unsigned int width,
                unsigned int height, unsigned long newStyle, Boolean resizing) {
  int cx = x;
  int cy = y;
  ...
  if (resizing) {
    FixWindowConfiguration(pcd, &width, &height, (unsigned int)pcd->widthInc,
   ...
   */
  cwidth =
      (width - ((pcd->widthInc == 1) ? 0 : pcd->baseWidth)) / pcd->widthInc;
  cheight =
      (height - ((pcd->heightInc == 1) ? 0 : pcd->baseHeight)) / pcd->heightInc;
...
Boolean CheckVisualPlace(ClientData *pCD, int tmpX, int tmpY) {
  ...
  XTranslateCoordinates(DISPLAY, XtWindow(P_ICON_BOX(pCD)->bBoardWidget),
                        ROOT_FOR_CLIENT(pCD), tmpX, tmpY, &newX, &newY, &child);
  ...
  if (((int)newX) > ((int)clipX + (int)clipWidth - ((int)ICON_WIDTH(pCD)))) {
    return (False);
  }
  if (((int)newY) > ((int)clipY + (int)clipHeight - ((int)ICON_HEIGHT(pCD)))) {
    return (False);
  }
...
void CompleteFrameConfig(ClientData *pcd, XEvent *pev) {
  ...
  if (wmGD.configAction == RESIZE_CLIENT) {
    ...
    AdjustPos(&resizeX, &resizeY, resizeWidth, resizeHeight, tmpWidth,
              tmpHeight);

    /* reconfigure the window(s) */
    ProcessNewConfiguration(pcd, resizeX, resizeY, resizeWidth, resizeHeight,
                            FALSE);
    ...
    if ((wmGD.configAction == MOVE_CLIENT) && (wmGD.gadgetClient == pcd) &&
    ...
    if (wmGD.movingIcon) {
      ...
      if (wmGD.iconAutoPlace || inIconBox) {
        ...
        if (place != ICON_PLACE(pcd)) {
          if (pIPD->placeList[place].pCD) {
            ...
            if (place == NO_ICON_PLACE) {
              ...
              if (pcd->pSD->moveOpaque && !inIconBox) {
                ...
                XMoveWindow(DISPLAY, ICON_FRAME_WIN(pcd), ICON_X(pcd),
                            ICON_Y(pcd));
          ...
          if ((place != NO_ICON_PLACE) && (place != ICON_PLACE(pcd))) {
            if (inIconBox) {
              ...
              if ((CheckIconBoxSize(P_ICON_BOX(pcd))) &&
                ...
                CvtIconPlaceToPosition(pIPD, place, &ICON_X(pcd), &ICON_Y(pcd));

                XtMoveWidget(pIPD->placeList[ICON_PLACE(pcd)].theWidget,
      ...
      ProcessNewConfiguration(
          pcd, moveX, moveY, (unsigned int)(moveWidth - 2 * offsetX),
          (unsigned int)(moveHeight - offsetX - offsetY), FALSE);
    }
    ...
    dtSendMarqueeSelectionNotification(pSD, DT_MARQUEE_SELECT_END, marqueeX,
                                       marqueeY, marqueeWidth, marqueeHeight);
  }
 ...
 *
 *************************************<->***********************************/
void MoveOpaque(ClientData *pcd, int x, int y, unsigned int width,
                unsigned int height)
...
static void DrawSegments(Display *dpy, Window win, GC gc, XSegment *outline,
                         int nsegs) {
...
void FlashOutline(int x, int y, unsigned int width, unsigned int height) {
   ...
   */
  DrawSegments(DISPLAY, ACTIVE_ROOT, ACTIVE_PSD->xorGC, outline,
               SEGS_PER_FLASH);
  XSync(DISPLAY, FALSE);
 ...
 *
 *************************************<->***********************************/
static void CreateOutlineWindows(WmScreenData *pSD) {
  ...
  xswa.background_pixmap = XmGetPixmap(
      XtScreen(pSD->screenTopLevelW), "50_foreground",
      pSD->clientAppearance.foreground, pSD->clientAppearance.background);

  xswamask = (CWOverrideRedirect | CWBackingStore | CWBackPixmap | CWSaveUnder);

  pSD->woN = XCreateWindow(DISPLAY, pSD->rootWindow, x, y, width, height, 0,
                           XDefaultDepth(DISPLAY, pSD->screen), CopyFromParent,
                           CopyFromParent, xswamask, &xswa);

  pSD->woS = XCreateWindow(DISPLAY, pSD->rootWindow, x, y, width, height, 0,
                           XDefaultDepth(DISPLAY, pSD->screen), CopyFromParent,
                           CopyFromParent, xswamask, &xswa);

  pSD->woE = XCreateWindow(DISPLAY, pSD->rootWindow, x, y, width, height, 0,
                           XDefaultDepth(DISPLAY, pSD->screen), CopyFromParent,
                           CopyFromParent, xswamask, &xswa);

  pSD->woW = XCreateWindow(DISPLAY, pSD->rootWindow, x, y, width, height, 0,
                           XDefaultDepth(DISPLAY, pSD->screen), CopyFromParent,
                           CopyFromParent, xswamask, &xswa);
...
void ProcessNewConfiguration(ClientData *pCD, int x, int y, unsigned int width,
                             unsigned int height, Boolean clientRequest) {
   ...
   */
  if (pCD->maxConfig) {
    if (newMax && (pCD->maxWidth == width) && (pCD->maxHeight == height)) {
  ...
  if (changedValues & (CWX | CWY)) {
    if (pCD->maxConfig) {
      ...
      if (clientRequest) {
        ...
        XMoveWindow(DISPLAY, pCD->clientFrameWin, pCD->maxX - offsetX,
                    pCD->maxY - offsetY);
      }
      /* End fix 5217 */
    } else {
      if (clientRequest) {
        ...
        XMoveWindow(DISPLAY, pCD->clientFrameWin, pCD->clientX - offsetX,
                    pCD->clientY - offsetY);
      }
    }
    SetFrameInfo(pCD);
  ...
  if ((pCD->clientFlags & ICON_BOX) && (changedValues & (CWWidth | CWHeight))) {
...
Boolean StartResizeConfig(ClientData *pcd, XEvent *pev) {
  ...
  if (wmGD.showFeedback & WM_SHOW_FB_RESIZE) {
    DoFeedback(pcd, resizeX, resizeY, resizeWidth, resizeHeight, FB_SIZE,
               TRUE /* do size checks */);
  }
...
void StartClientResize(ClientData *pcd, XEvent *pev) {
  ...
  if (!pev || pev->type == KeyPress) {
    ...
    if (wmGD.enableWarp) {
      XWarpPointer(DISPLAY, None, ROOT_FOR_CLIENT(pcd), 0, 0, 0, 0, pointerX,
                   pointerY);
    }
  }
...
Boolean StartClientMove(ClientData *pcd, XEvent *pev) {
  ...
  if (!configGrab) {
    ...
    if (pev) {
      grabbed = DoGrabs(grab_win, wmGD.configCursor, PGRAB_MASK,
                        pev->xbutton.time, pcd, False);
    } else {
      grabbed = DoGrabs(grab_win, wmGD.configCursor, PGRAB_MASK, CurrentTime,
                        pcd, False);
    }
    if (!grabbed) {
      wmGD.movingIcon = False;
  ...
  if (pev && ((pev->type == ButtonPress) || (pev->type == ButtonRelease))) {
    ...
    if (!XQueryPointer(DISPLAY, ROOT_FOR_CLIENT(pcd), &junk_win, &junk_win,
                       &(wmGD.preMoveX), &(wmGD.preMoveY), &junk, &junk,
                       (unsigned int *)&junk)) {
      CancelFrameConfig(pcd);
      return (False);
    }
  ...
  if (wmGD.movingIcon) {
    ...
    if (pcd->pSD->useIconBox && P_ICON_BOX(pcd)) {
      ...
      XTranslateCoordinates(DISPLAY, XtWindow(P_ICON_BOX(pcd)->bBoardWidget),
                            ROOT_FOR_CLIENT(pcd), 0, 0, &moveIBbbX, &moveIBbbY,
                            &child);
  ...
  if (!pev || pev->type == KeyPress) {
    ...
    ForceOnScreen(SCREEN_FOR_CLIENT(pcd), &moveLastPointerX, &moveLastPointerY);
    if (wmGD.enableWarp) {
  ...
  /* pop up feedback window */
  if (!wmGD.movingIcon && (wmGD.showFeedback & WM_SHOW_FB_MOVE)) {
    DoFeedback(pcd, moveX, moveY, moveWidth, moveHeight, FB_POSITION,
               FALSE /* no size checks */);
  }
...
Boolean DoGrabs(Window grab_win, Cursor cursor, unsigned int pmask,
  ...
  if (pCD && pCD->pSD->useIconBox && wmGD.movingIcon && P_ICON_BOX(pCD)) {
     ...
     */
    if (XGrabPointer(DISPLAY, grab_win, FALSE, /* owner_events */
                     pmask, GrabModeAsync,     /* pointer_mode */
                     GrabModeAsync,            /* keyboard_mode */
                     /* confine_to window */
                     XtWindow(P_ICON_BOX(pCD)->clipWidget), cursor,
    ...
    if (XGrabPointer(DISPLAY, grab_win, FALSE, /* owner_events */
                     pmask, GrabModeAsync,     /* pointer_mode */
                     GrabModeAsync,            /* keyboard_mode */
                     root, cursor, grabTime) != GrabSuccess) {
      return (FALSE);
    }
  }
  ...
  {
    if (!pCD ||
        ((pCD->pSD->moveOpaque && alwaysGrab) || (!(pCD->pSD->moveOpaque)))) {
      XGrabServer(DISPLAY);
    }
  }
...
void CancelFrameConfig(ClientData *pcd) {
  ...
  if (pcd) {
    ...
    if ((pcd->pSD->moveOpaque) && (wmGD.configAction == MOVE_CLIENT))

    {
      ...
      {
        XMoveWindow(DISPLAY, pcd->clientFrameWin, opaqueMoveX, opaqueMoveY);
      }
    }
  }
...
void CheckEatButtonRelease(ClientData *pcd, XEvent *pev) {
  ...
  if ((pev->type == KeyPress || pev->type == KeyRelease) &&
     ...
     */
    if (XGrabPointer(DISPLAY, grab_win, False,         /* owner_events */
                     ButtonReleaseMask, GrabModeAsync, /* pointer_mode */
                     GrabModeAsync,                    /* keyboard_mode */
                     root,                             /* confine_to window */
                     wmGD.configCursor, pev->xbutton.time) == GrabSuccess) {
...
void EatButtonRelease(unsigned int releaseButtons) {
  ...
  while (releaseButtons) {
    ...
    if (event.type == ButtonRelease) {
      ...
      new_state = event.xbutton.state & ~ButtonStateBit(event.xbutton.button);
...
#define MAX_BUTTON 5
  ...
  static ButtonAssoc bmap[MAX_BUTTON] = {
      {Button1, Button1Mask}, {Button2, Button2Mask}, {Button3, Button3Mask},
      {Button4, Button4Mask}, {Button5, Button5Mask},
  };
...
void ReGrabPointer(Window grab_win, Time grabTime) {
  XGrabPointer(DISPLAY, grab_win, FALSE,  /* owner_events */
               PGRAB_MASK, GrabModeAsync, /* pointer_mode */
               GrabModeAsync,             /* keyboard_mode */
               ACTIVE_ROOT,               /* confine_to window */
               ConfigCursor((int)wmGD.configPart), grabTime);
...
int ResizeType(ClientData *pcd, XEvent *pev) {
  ...
  if ((x > resizeX) && (y > resizeY) && (x < (resizeX + resizeWidth - 1)) &&
...
void FixFrameValues(ClientData *pcd, int *pfX, int *pfY, unsigned int *pfWidth,
                    unsigned int *pfHeight, Boolean resizing) {
  ...
  if (wmGD.movingIcon && P_ICON_BOX(pcd)) {
    ...
    if (*pfX > (DisplayWidth(DISPLAY, SCREEN_FOR_CLIENT(pcd)) - (int)lswidth)) {
      *pfX = DisplayWidth(DISPLAY, SCREEN_FOR_CLIENT(pcd)) - (int)lswidth;
    }

    /* keep top border on screen */
    if (*pfY >
        (DisplayHeight(DISPLAY, SCREEN_FOR_CLIENT(pcd)) - (int)lswidth)) {
      *pfY = DisplayHeight(DISPLAY, SCREEN_FOR_CLIENT(pcd)) - (int)lswidth;
    }
  }
...
void SetPointerPosition(int newX, int newY, int *actualX, int *actualY) {
   ...
   */
  if (wmGD.enableWarp) {
    XWarpPointer(DISPLAY, None, ACTIVE_ROOT, 0, 0, 0, 0, newX, newY);
  }
   ...
   */
  if (!wmGD.enableWarp ||
      !XQueryPointer(DISPLAY, ACTIVE_ROOT, &junk_win, &junk_win, actualX,
                     actualY, &junk, &junk, (unsigned int *)&junk))
 ...
 *
 *************************************<->***********************************/
void GetConfigEvent(Display *display, Window window, unsigned long mask,
                    int curX, int curY, int oX, int oY, unsigned oWidth,
  ...
  while (!eventToReturn) {
    ...
    if (!gotEvent && (polling || !wmGD.freezeOnConfig)) {
      ...
      while (True) {
        if (XCheckWindowEvent(display, window, (mask & ~PointerMotionMask),
                              pev)) {
          gotEvent = True;
          break;
        }
        ...
        if (!XQueryPointer(display, window, &root_ret, &child_ret, &root_x,
    ...
    if (gotEvent) {
      eventToReturn = True;
      if (pev->type == MotionNotify && pev->xmotion.is_hint == NotifyHint) {
        /*
         * "Ack" the motion notify hint
         */
        if ((XQueryPointer(display, window, &root_ret, &child_ret, &root_x,
                           &root_y, &win_x, &win_y, &mask_ret)) &&
            ((root_x != curX) || (root_y != curY))) {
          /*
...
void AdjustPos(int *pX, int *pY, unsigned int oWidth, unsigned int oHeight,
...
Window GrabWin(ClientData *pcd, XEvent *pev) {
  ...
  } else if (pev && (pev->xany.window == pcd->clientFrameWin ||
                     pev->xany.window == pcd->clientBaseWin)) {
    grab_win = pcd->clientFrameWin;
  } else if (pcd->pSD->useIconBox && P_ICON_BOX(pcd) &&
...
void StartMarqueeSelect(WmScreenData *pSD, XEvent *pev) {
  ...
  if (!pSD->bMarqueeSelectionInitialized) {
     ...
     */
    dtSendMarqueeSelectionNotification(pSD, DT_MARQUEE_SELECT_END, 0, 0, 0, 0);
  ...
  if (pev && ((pev->type == ButtonPress) || (pev->type == ButtonRelease))) {
    pointerX = pev->xbutton.x_root;
    pointerY = pev->xbutton.y_root;
  } else if (!XQueryPointer(DISPLAY, pSD->rootWindow, &junk_win, &junk_win,
                            &pointerX, &pointerY, &junk, &junk,
                            (unsigned int *)&junk)) {
  ...
  dtSendMarqueeSelectionNotification(pSD, DT_MARQUEE_SELECT_BEGIN, marqueeX,
                                     marqueeY, marqueeWidth, marqueeHeight);
...
void UpdateMarqueeSelectData(WmScreenData *pSD) {
  ...
  if ((wmGD.marqueeSelectGranularity > 0) &&
    ...
    dtSendMarqueeSelectionNotification(pSD, DT_MARQUEE_SELECT_CONTINUE,
                                       marqueeX, marqueeY, marqueeWidth,
                                       marqueeHeight);
