...
static char *SessionFileGetName(void) {
  char *home_dir;
  char *file_name;
...
void SessionRestoreData(void) {
  ...
  if ((fd = fopen(session_file, "r")) != NULL) {
      ...
      for (j = 0; j < box_data->control_data_count; j++) {
        ...
        if (control_data->subpanel_data != NULL) {
          if (subpanel_count == max_subpanel_count) {
            ...
            subpanel_data = (SubpanelData **)XtRealloc((char *)subpanel_data,
                                                       sizeof(SubpanelData *) *
                                                           max_subpanel_count);
          }
    ...
    for (i = 0; i < subpanel_posted_count; i++) {
      fscanf(fd, "%s%d%d%d", name_buffer1, &subpanel_x, &subpanel_y,
             &subpanel_torn);

      for (j = 0; j < subpanel_count; j++) {
        if (strcmp(
                name_buffer1,
                subpanel_data[j]->element_values[SUBPANEL_NAME].parsed_value) ==
            0) {
          ...
          if (subpanel_torn != 0) {
            ...
            _DtWsmSetDtWmHints(XtDisplay(panel.shell),
                               XtWindow(subpanel_data[j]->shell), &vHints);
          }
      ...
      for (j = 0; j < subpanel_count; j++) {
        if (strcmp(
          ...
          for (k = 0; k < subpanel_data[j]->control_data_count; k++) {
            if (strcmp(name_buffer2, subpanel_data[j]
                                         ->control_data[k]
    ...
    if (panel.dynamic_data_count > 0) {
      panel.dynamic_data_list = (DynamicComponent **)XtMalloc(
    ...
    for (i = 0; i < panel.dynamic_data_count; i++) {
      ...
      for (j = 0; j < subpanel_count; j++) {
        ...
        if (delete)
          have_it = True;
                 ...
                 strcmp(subpanel_data[j]
                        ...
                        name_buffer2) == 0 &&
                 strcmp(subpanel_data[j]
                            ->element_values[SUBPANEL_CONTAINER_NAME]
                            .parsed_value,
                        name_buffer3) == 0) {
          have_it = True;
        } else {
          if (parent_component_type == SUBPANEL) {
            ...
            for (k = 0; k < panel.box_data_count; k++) {
              if (strcmp(
                      panel.box_data[k]->element_values[BOX_NAME].parsed_value,
                      name_buffer3) == 0) {
        ...
        if (have_it) {
          ...
          panel.dynamic_data_list[dynamic_data_count]->file_name =
              XtNewString(name_buffer1);
          panel.dynamic_data_list[dynamic_data_count]->component_name =
              XtNewString(name_buffer2);
          panel.dynamic_data_list[dynamic_data_count]->component_type =
              component_type;
          panel.dynamic_data_list[dynamic_data_count]->parent_name =
              XtNewString(name_buffer3);
          panel.dynamic_data_list[dynamic_data_count]->parent_type =
        ...
        if (XtIsManaged(subpanel_data[i]->shell))
          fprintf(fd, "%s %d %d %d\n",
                  (char *)subpanel_data[i]
                      ->element_values[SUBPANEL_NAME]
                      .parsed_value,
                  XtX(subpanel_data[i]->shell), XtY(subpanel_data[i]->shell),
        ...
        if (subpanel_data[i]->default_control !=
          ...
          fprintf(fd, "%s	%s\n",
                  (char *)subpanel_data[i]
                      ...
                      .parsed_value,
                  (char *)subpanel_data[i]
                      ->default_control->element_values[CONTROL_NAME]
                      .parsed_value);
        }
      }
    }
...
void SessionAddFileData(char *file_name, char *component_name,
                        int component_type, char *parent_name, int parent_type,
                        Boolean delete)

{
  ...
  if (count == panel.max_dynamic_data_count) {
    ...
    panel.dynamic_data_list = (DynamicComponent **)XtRealloc(
        (char *)panel.dynamic_data_list,
        sizeof(DynamicComponent *) * panel.max_dynamic_data_count);
  }
...
void SessionDeleteFileData(char *file_name)
...
char *SessionFileNameLookup(char *component_name, int component_type,
                            char *parent_name, int parent_type)

{
  ...
  for (i = 0; i < panel.dynamic_data_count; i++) {
    if (strcmp(component_name, panel.dynamic_data_list[i]->component_name) ==
            0 &&
        component_type == panel.dynamic_data_list[i]->component_type &&
        strcmp(parent_name, panel.dynamic_data_list[i]->parent_name) == 0 &&
...
void SessionDeleteAll(void) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0)
      continue;
    if (srclen < MAX_PATH)
      strlcpy(srcname + srclen, entry->d_name, MAX_PATH - srclen);
    remove(srcname);
  }
