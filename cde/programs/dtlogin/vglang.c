 ...
 */
static void *_enumLangCmdStart(void) {
 ...
 */
static Boolean _enumLangCmdNext(void *state, char **lang, char **desc) {
  char buf[200];
  Boolean rc = False;
 ...
 * Close pipe.
 */
static void _enumLangCmdEnd(void *state) { pclose((FILE *)state); }
 ...
 */
static void *_enumLanglistStart(void) {
  char *p;
 ...
 * Xresources.
 */
static Boolean _enumLanglistNext(void *state, char **lang, char **desc) {
  Boolean rc = False;
 ...
 * Free memory.
 */
static void _enumLanglistEnd(void *state) {
  struct _enumState *enumstate = (struct _enumState *)state;
 ...
 *
 */
static void *_enumLangStart(void) {
  char *p;
 ...
 *                   note: *lang and *desc must be freed by free()
 */
static Boolean _enumLangNext(void *state, char **lang, char **desc) {
  Boolean rc;
 ...
 */
static void _enumLangEnd(void *state) {
  struct _enumObject *object = (struct _enumObject *)state;
 ...
 */
static int compareLangDesc(const void *first, const void *second) {
  return (strcmp(((struct Langlist *)first)->desc,
                 ((struct Langlist *)second)->desc));
}

void MakeLangMenu(void) {
  int i, k;
  ...
  if (nlang > 0) {
    ...
    if (nlang % nlangMenus != 0)
      maxitems++; /* allow for stragglers	   */
    ...
    for (k = 0; k < nlang; k++) {
      ...
      if (nlangMenus > 1 && k % maxitems == 0) {
        i = 0;
        item_menu = XmCreatePulldownMenu(lang_menu, "item_menu", argt, i);
      }
      ...
      button = XmCreateToggleButtonGadget(item_menu, list[k].lang, argt, i);
      XtAddCallback(button, XmNvalueChangedCallback, RespondLangCB,
                    (XtPointer)list[k].lang);
      XmStringFree(xmstr);

      XtManageChild(button);
      tostr = (char *)ReadCatalog(MC_LABEL_SET, MC_TO_LABEL, MC_DEF_TO_LABEL);
      ...
      if (nlangMenus > 1 && k % maxitems == 0) {
        ...
        sprintf(cblab, "%s %s %s", list[first].desc, tostr, list[last].desc);

        xmstr = XmStringCreateLocalized(cblab);
        XtSetArg(argt[i], XmNlabelString, xmstr);
        i++;
        XtSetArg(argt[i], XmNsubMenuId, item_menu);
        i++;
        XtSetArg(argt[i], XmNrecomputeSize, True);
        i++;
