...
static int SiaManageGreeter(int timeout, int rendition, unsigned char *title,
...
static SIGVAL catchTerm(int arg) { siglongjmp(abortSession, 1); }
...
#ifdef PAM
      PamAccounting(verify.argv[0], d->name, d->utmpId, user, ttyLine,
                    clientPid, ACCOUNTING, 0);
#else
      solaris_accounting(verify.argv[0], d->name, d->utmpId, user, ttyLine,
                         clientPid, ACCOUNTING, NULL);
#endif
...
static int IOErrorHandler(Display *dpy) {
  ...
  LogError(ReadCatalog(MC_LOG_SET, MC_LOG_FATAL_IO, MC_DEF_LOG_FATAL_IO), errno,
           s);
...
void ManageSession(struct display *d) {
  ...
  if (IsVerifyName(VN_KRB)) {
    ...
    verify.systemEnviron =
        setEnv(verify.systemEnviron, "KRBTKFILE", krb_ticket_string);

    verify.userEnviron =
        setEnv(verify.userEnviron, "KRBTKFILE", krb_ticket_string);
  }
#endif /* __KERBEROS */

  /* set LOCATION env var */
  if (d->displayType.location == Local) {
    verify.systemEnviron = setEnv(verify.systemEnviron, LOCATION, "local");
    /* ITE is needed only for Local displays */
    /* set ITE env var */
    if (d->gettyLine)
      verify.systemEnviron = setEnv(verify.systemEnviron, "ITE", d->gettyLine);
  } else
    verify.systemEnviron = setEnv(verify.systemEnviron, LOCATION, "remote");

  {
    ...
    verify.systemEnviron = setEnv(verify.systemEnviron, "USER_GID", gid);

    /* set root group id (ROOT_GID) env var */
    pwd = getpwnam("root");
    if (pwd) {
      sprintf(gid, "%ld", (long)pwd->pw_gid);
      verify.systemEnviron = setEnv(verify.systemEnviron, "ROOT_GID", gid);
    }
  }
  ...
  if (!sigsetjmp(abortSession, 1)) {
    ...
    if (StartClient(&verify, d, &clientPid)) {
      ...
      LogError(
          ReadCatalog(MC_LOG_SET, MC_LOG_FAIL_START, MC_DEF_LOG_FAIL_START));
    }
  } else {
    /*
  ...
  if (d->displayType.location == Foreign && d->displayType.origin != FromXDMCP)
    AbortClient(clientPid);
...
#ifdef PAM
    PamAccounting(verify.argv[0], d->name, d->utmpId, user, ttyLine, clientPid,
                  ACCOUNTING, 0);
#else
    solaris_accounting(verify.argv[0], d->name, d->utmpId, user, ttyLine,
                       clientPid, ACCOUNTING, NULL);
...
int LoadXloginResources(struct display *d) {
  ...
  if (d->resources && d->resources[0]) {
    ...
    sprintf(cmd, "%s%s %s -display %s -load %s", auth_key, authority, d->xrdb,
            d->name, tmpname);
...
static void LoadAltDtsResources(struct display *d) {
  ...
  if (XrmGetResource(XresourceDB, "Dtlogin*altDts", "Dtlogin*AltDts", &rmtype,
                     &rmvalue)) {
      ...
      while ((dp = readdir(dirp)) != NULL) {
        ...
        if ((strcmp(dp->d_name, DOT) != 0) &&
          ...
          if (file_count + 1 > num_allocated) {
            num_allocated += list_incr;
            file_list = realloc(file_list, num_allocated * sizeof(char *));
          }
  ...
  for (j = 0; j < file_count; j++) {
    ...
    if (XrmGetResource(XresourceDB, "Dtlogin*altDtsIncrement",
      ...
      if ((strcmp(tempbuf, "True") == 0) || (strcmp(tempbuf, "TRUE") == 0)) {

        if (XrmGetResource(XresourceDB, "Dtlogin*altDtKey", "Dtlogin*AltDtKey",
                           &rmtype, &rmvalue)) {
          ++i;
        ...
        if (XrmGetResource(XresourceDB, "Dtlogin*altDtName",
                           "Dtlogin*AltDtName", &rmtype, &rmvalue)) {
          sprintf(altdtres, "Dtlogin*altDtName%d", i);
          XrmPutStringResource(&XresourceDB, altdtres, rmvalue.addr);
        }
        if (XrmGetResource(XresourceDB, "Dtlogin*altDtStart",
                           "Dtlogin*AltDtStart", &rmtype, &rmvalue)) {
          sprintf(altdtres, "Dtlogin*altDtStart%d", i);
          XrmPutStringResource(&XresourceDB, altdtres, rmvalue.addr);
        }
        if (XrmGetResource(XresourceDB, "Dtlogin*altDtLogo",
                           "Dtlogin*AltDtLogo", &rmtype, &rmvalue)) {
...
#define NOPAG 0xffffffff
  ...
  if (verify->argv) {
    ...
    for (f = verify->argv; *f; f++) {
      Debug("%s ", *f);
      if (strcmp(*f, "failsafe") == 0)
        failsafe = TRUE;
      if (strcmp(*f, "password") == 0)
        failsafe = password = TRUE;
    }
    Debug("; ");
  }
...
#ifdef PAM
      PamAccounting(verify->argv[0], d->name, d->utmpId, user, ttyLine,
                    getpid(), USER_PROCESS, 0);
#else
      solaris_accounting(verify->argv[0], d->name, d->utmpId, user, ttyLine,
                         getpid(), USER_PROCESS, NULL);
#endif
    }
#endif
...
#if defined(PAM)
    if (PamSetCred(verify->argv[0], user, verify->uid, verify->gid) > 0) {
      Debug("Can't set User's Credentials (user=%s)\n", user);
      return (0);
    }
...
#ifdef SUNAUTH
    if (solaris_setcred(verify->argv[0], user, verify->uid, verify->gid) > 0) {
      Debug("Can't set User's Credentials (user=%s)\n", user);
      return (0);
    }
#endif /* SUNAUTH */
    ...
    if (IsVerifyName(VN_AFS)) {
      ...
      if ((pagval != NOPAG) &&
        ...
        if (setgroups(ngroups, groups) == -1) {
          LogError(
              ReadCatalog(MC_LOG_SET, MC_LOG_AFS_FAIL, MC_DEF_LOG_AFS_FAIL));
          exit(1);
        }
      }
    }
...
/* setpenv() will set uid for AIX */
    ...
    if (setuid(verify->uid) != 0) {
      ...
      LogError(
          ReadCatalog(MC_LOG_SET, MC_LOG_FAIL_SETUID, MC_DEF_LOG_FAIL_SETUID),
          user, errno);
      exit(1);
    }
#endif
    ...
    if (home) {
      if (chdir(home) == -1) {
        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_HMDIR, MC_DEF_LOG_NO_HMDIR),
                 home, getEnv(verify->userEnviron, "USER"));
        if (-1 == chdir("/")) {
          perror(strerror(errno));
        }
    ...
    if (verify->argv) {
      if (!failsafe) {
        ...
        LogError(
            ReadCatalog(MC_LOG_SET, MC_LOG_SES_EXEFAIL, MC_DEF_LOG_SES_EXEFAIL),
            verify->argv[0]);
      }
    } else {
      LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_CMDARG, MC_DEF_LOG_NO_CMDARG));
    }
...
static SIGVAL waitAbort(int arg) { siglongjmp(tenaciousClient, 1); }
...
static int AbortClient(int pid) {
  ...
  for (i = 0; i < 4; i++) {
    if (killpg(pid, sig) == -1) {
      ...
      case EPERM:
        LogError(
            ReadCatalog(MC_LOG_SET, MC_LOG_NO_KILLCL, MC_DEF_LOG_NO_KILLCL));
      case EINVAL:
      case ESRCH:
        return 0;
      }
...
int source(struct verify_info *verify, char *file) {
  char *args[2];
  int pid;
  waitType result;
  ...
  if (file && file[0]) {
    ...
    case 0:
      ...
      LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_EXE, MC_DEF_LOG_NO_EXE),
               args[0]);
      exit(1);
    case -1:
      Debug("Source(): fork failed\n");
      LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_FORK, MC_DEF_LOG_NO_FORK),
               file);
      return 1;
      break;
    default:
...
int execute(char **argv, char **environ) {
...
#define U_NAMELEN sizeof(rgy_$name_t)
  ...
  siaStatus = sia_ses_init(&siaHandle, argc, argv, hostName, loginName, d->name,
                           1, NULL);
    ...
    case 0:
      ...
      if (d->verifyName) {
        if ((strcmp(d->verifyName, VN_AFS) == 0) ||
          ...
          LogError(ReadCatalog(MC_LOG_SET, MC_LOG_IMPROP_AUTH,
                               MC_DEF_LOG_IMPROP_AUTH),
                   d->verifyName);
          d->verifyName = NULL;
        }
      }
      ...
      LogError(
          ReadCatalog(MC_LOG_SET, MC_LOG_NO_DTGREET, MC_DEF_LOG_NO_DTGREET),
          msg, d->name);
      exit(NOTIFY_ABORT_DISPLAY);

    case -1:
      Debug("Fork of Greeter failed.\n");
      LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_FORKCG, MC_DEF_LOG_NO_FORKCG),
               d->name);
      close(request[0]);
      close(request[1]);
      close(response[0]);
    ...
    default:
      ...
      while (siaStatus != SIASUCCESS) {
        while (siaStatus != SIASUCCESS && siaGreeterInfo.status) {
          ...
          siaStatus = sia_ses_authent(SiaManageGreeter, NULL, siaHandle);
        ...
        if (siaStatus != SIASUCCESS) {
          ...
          siaHandle = NULL;
          siaStatus = sia_ses_init(&siaHandle, argc, argv, hostName, loginName,
                                   d->name, 1, NULL);
          if (siaStatus != SIASUCCESS) {
            Debug("sia_ses_init failure status %d\n", siaStatus);
            exit(RESERVER_DISPLAY);
      ...
      case NOTIFY_ALT_DTS:
        ...
        i = 0;
        while (i < sizeof(rgy_$name_t) && greet->name_full[i] != '.' &&
               greet->name_full[i] != '\0') {
...
static int ManageGreeter(struct display *d, struct greet_info *greet,
  ...
  case GREET_STATE_AUTHENTICATE: {
    ...
    if (state->vf == VF_OK || state->vf == VF_PASSWD_AGED ||
    ...
    case VF_OK:
      ...
      break;
    case VF_PASSWD_AGED:
      state->id = GREET_STATE_EXPASSWORD;
      break;
    case VF_BAD_HOSTNAME:
      state->id = GREET_STATE_BAD_HOSTNAME;
      break;
    case VF_CHALLENGE:
      state->id = GREET_STATE_CHALLENGE;
      break;
    default:
      state->id = GREET_STATE_ERRORMESSAGE;
      break;
...
static int AskGreeter(RequestHeader *preqhdr, char *buf, int blen) {
  int count;
  int remainder;
...
static int SiaManageGreeter(int timeout, int rendition, unsigned char *title,
  ...
  case SIAONELINER:
  case SIAFORM:
    if (rendition == SIAFORM && dt_in_sia_ses_authent && (num_prompts == 2)) {
      ...
      while (siaGreeterInfo.state->id != GREET_STATE_TERMINATEGREET &&
             ...
             (siaGreeterInfo.status =
                  ManageGreeter(siaGreeterInfo.d, siaGreeterInfo.greet,
      ...
      req_form_size = sizeof(RequestForm) + strlen((const char *)title) + 1;
      ...
      request_form->offPrompts =
          sizeof(RequestForm) + strlen((const char *)title) + 1;
  ...
  case SIAWARNING:
    ...
    siaGreeterInfo.state->request =
        (RequestHeader *)&greeter_msg_and_buffer.greeter_message;
    siaGreeterInfo.state->vf = VF_MESSAGE;
