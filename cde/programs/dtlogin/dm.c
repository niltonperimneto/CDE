...
int main(int argc, char **argv) {
  ...
  if ((oldpid = StorePid()) != 0) {
    if (oldpid == (long)-1)
      LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_CREATE, MC_DEF_LOG_NO_CREATE),
               pidFile);
    else
...
static SIGVAL RescanNotify(int arg) {
  Debug("Caught SIGHUP\n");
  Rescan = 1;
...
#define NumTypes (sizeof(acceptableTypes) / sizeof(acceptableTypes[0]))
  ...
  if (servers[0] == '/') {
    ...
    if (serversFile == NULL) {
      LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_SRVACC, MC_DEF_LOG_NO_SRVACC),
               servers);
      return 0;
    }
    if (ServersModTime == 0) {
...
static void MarkDisplay(struct display *d) { d->state = MissingEntry; }
...
static void RescanIfMod(void) {
  ...
  if (stat(config, &statb) != -1) {
    if (statb.st_mtime != ConfigModTime) {
      ...
      LogInfo(ReadCatalog(MC_ERROR_SET, MC_LOG_REREADCFG, MC_DEF_LOG_REREADCFG),
              config);
      ConfigModTime = statb.st_mtime;
      ReinitResources();
      LoadDMResources();
  ...
  if (servers[0] == '/' && stat(servers, &statb) != -1) {
    if (statb.st_mtime != ServersModTime) {
      ...
      LogInfo(ReadCatalog(MC_ERROR_SET, MC_LOG_REREADSRV, MC_DEF_LOG_REREADSRV),
              servers);
      ServersModTime = statb.st_mtime;
      ForEachDisplay(MarkDisplay);
      ScanServers();
  ...
  if (accessFile && accessFile[0] && stat(accessFile, &statb) != -1) {
    if (statb.st_mtime != AccessFileModTime) {
      ...
      LogInfo(ReadCatalog(MC_ERROR_SET, MC_LOG_REREADACC, MC_DEF_LOG_REREADACC),
              accessFile);
      AccessFileModTime = statb.st_mtime;
      ScanAccessDatabase();
    }
...
static SIGVAL StopAll(int arg) {
  if (parent_pid != getpid()) {
     ...
     */
    Debug(
        "Child dtlogin caught signal %d before it could become a real child\n",
        arg);
  ...
  if ((d = FindDisplayByPid(pid)) != 0) {
    ...
    */
    ...
    default:
      ...
      LogError(
          (unsigned char *)"Unknown session exit code %d from process %d\n",
          waitVal(status), pid);
      StopDisplay(d);
      break;
    ...
    case running:
      ...
      LogError(
          (unsigned char *)"Server for display %s terminated unexpectedly %d\n",
          d->name, waitVal(status));
      if (d->pid != -1) {
  ...
  if (d->displayType.origin == FromFile) {
    ...
    case OldEntry:
      ...
      if (d->status == suspended && wakeupTime >= 0)
        if (GettyRunning(d) ||
            (d->gettyLine && (strcmp(d->gettyLine, "??") == 0)))
          if (wakeupTime == 0) {
            Debug("Polling of suspended server %s started.\n", d->name);

            wakeupTime =
                (wakeupInterval < 10 ? 3 * wakeupInterval : 2 * wakeupInterval);
          } else {
...
void StartDisplays(void) { ForEachDisplay(CheckDisplayStatus); }

int StartDisplay(struct display *d) {
  waitType status;
  int pid;
  char *authFile_str;
...

  {
    ...
    if (NULL == d->authFile && 0 < strlen(bkup) &&
  ...
  if (d->displayType.location == Local) {
    ...
    if (!d->gettyLine || strlen(d->gettyLine) == 0) {
      ...
      if ((p = index(d->name, ':')) != NULL && (strncmp(++p, "0", 1) == 0)) {
    ...
    if (d->serverPid == -1 &&
        (d->startTries++ >= d->startAttempts || !StartServer(d))) {
  ...
  case 0:
    ...
    {
      char *line = (d->displayType.location == Local) ? d->gettyLine : d->name;
...
void StopDisplay(struct display *d) {
  ...
  bzero(&status, sizeof(waitType));
  Debug("StopDisplay(): %s, server pid = %d, manager pid = %d, dt_shutdown = "
        "%d\n",
...
#endif
  } else if ((d->displayType.location == Local) || !dt_shutdown) {
    RemoveDisplay(d);
  }
}
...
int SetTitle(char *name, char *ptr) {
#ifndef NOXDMTITLE
  char *p, *s, *t;
  int length;
...
void GettyMessage(struct display *d, int msgnum) {
  FILE *tf;
  char buf[128];
  ...
  if ((tf = fopen(buf, "a")) != NULL) {
    ...
    case 1:
      ...
      fprintf(
          tf, "%s",
          (char *)ReadCatalog(MC_LABEL_SET, MC_SUS1_LABEL, MC_DEF_SUS1_LABEL));

      fprintf(
          tf, "%s",
          (char *)ReadCatalog(MC_LABEL_SET, MC_SUS2_LABEL, MC_DEF_SUS2_LABEL));

      fprintf(
          tf, "%s",
          (char *)ReadCatalog(MC_LABEL_SET, MC_SUS3_LABEL, MC_DEF_SUS3_LABEL));
    ...
    case 2:
      ...
      fprintf(
          tf, "%s",
          (char *)ReadCatalog(MC_LABEL_SET, MC_RES_LABEL, MC_DEF_RES_LABEL));
    ...
    case 3:
      ...
      fprintf(tf,
              (char *)ReadCatalog(MC_LABEL_SET, MC_START_LBLDPY,
                                  MC_DEF_START_LBLDPY),
              d->name);

      fprintf(
          tf, "%s",
          (char *)ReadCatalog(MC_LABEL_SET, MC_WAIT_LABEL, MC_DEF_WAIT_LABEL));
...
int GettyRunning(struct display *d) { return FALSE; }
  ...
  if (stat(dir, &file_status) != 0) {
    ...
    if (force && (file_status.st_mode & dir_mode) != dir_mode) {
      ...
      if (chmod(dir, file_status.st_mode | dir_mode) == 0) {
        ...
        LogError((unsigned char *)"Unable to set permissions on %s\n", dir);
      }
    }
  }
}
