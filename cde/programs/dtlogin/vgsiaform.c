...
void SiaForm(SiaFormInfo *siaFormInfo) {
  int i;
  int j;
  ...
  /* i = 0; */
  i = InitArg(Label);
  XtSetArg(argt[i], XmNlabelString, siaFormInfo->title);
  i++;
  ...
  XtAddCallback(ok_button, XmNactivateCallback, RespondSiaFormCB, siaFormInfo);
  ...
  i++;
  XtSetArg(argt[i], XmNentryVerticalAlignment, XmALIGNMENT_CENTER);
  i++;
  XtSetArg(argt[i], XmNrowColumnType, XmWORK_AREA);
  i++;
  XtSetArg(argt[i], XmNnumColumns, siaFormInfo->num_prompts);
  i++;
  XtSetArg(argt[i], XmNpacking, XmPACK_COLUMN);
  i++;
  XtSetArg(argt[i], XmNtopAttachment, XmATTACH_WIDGET);
  i++;
  XtSetArg(argt[i], XmNtopWidget, title_label);
  i++;
  XtSetArg(argt[i], XmNbottomAttachment, XmATTACH_WIDGET);
  i++;
  XtSetArg(argt[i], XmNbottomWidget, ok_button);
  i++;
  XtSetArg(argt[i], XmNleftAttachment, XmATTACH_FORM);
  i++;
  XtSetArg(argt[i], XmNrightAttachment, XmATTACH_FORM);
  ...
  case SIAMENUONE:
    XtSetArg(argt[i], XmNradioBehavior, True);
    i++;
    /* no break */
  case SIAMENUANY:
    XtSetArg(argt[i], XmNisHomogeneous, True);
    i++;
    XtSetArg(argt[i], XmNentryClass, xmToggleButtonGadgetClass);
    i++;
    XtSetArg(argt[i], XmNentryAlignment, XmALIGNMENT_BEGINNING);
    i++;
    break;
  ...
  case SIAONELINER:
    /* no break */
  case SIAFORM:
    XtSetArg(argt[i], XmNentryAlignment, XmALIGNMENT_END);
    i++;
  }
  ...
  for (j = 0; j < siaFormInfo->num_prompts; j++) {
    ...
    case SIAMENUONE: {
      ...
      prompt_answers[j] =
          XmCreateToggleButtonGadget(prompts_row_col, answer_name, argt, i);
    } break;
    ...
    case SIAFORM: {
      ...
      prompt_labels[j] = XmCreateLabel(prompts_row_col, label_name, argt, i);
      ...
      prompt_answers[j] =
          XmCreateTextField(prompts_row_col, answer_name, argt, i);
      ...
      if (!siaFormInfo->visible[j]) {
        ...
        XtAddCallback(prompt_answers[j], XmNmodifyVerifyCallback, NoEchoCB,
                      siaFormInfo);
        XtAddCallback(prompt_answers[j], XmNmotionVerifyCallback, NoEchoCB,
                      siaFormInfo);
   ...
   */
  {
    ...
    i++;
    XtSetArg(argt[i], XmNleftPosition, 33);
    i++;
    XtSetArg(argt[i], XmNleftOffset, -(width / 2));
    i++;
    ...
    XtSetArg(argt[i], XmNleftAttachment, XmATTACH_POSITION);
    i++;
    XtSetArg(argt[i], XmNleftPosition, 66);
    i++;
    XtSetArg(argt[i], XmNleftOffset, -(width / 2));
    i++;
  ...
  if (siaFormInfo->rendition == SIAONELINER ||
    ...
    XSetInputFocus(XtDisplay(prompt_answers[0]), XtWindow(prompt_answers[0]),
                   RevertToPointerRoot, CurrentTime);

    i = 0;
    XtSetArg(argt[i], XmNinitialFocus, prompt_answers[0]);
    i++;
...
static void SiaBackspace(Widget w, XEvent *event, char **params,
                         Cardinal *num_params) {
  int i, len;
