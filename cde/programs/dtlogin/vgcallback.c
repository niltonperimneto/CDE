...
static void CenterForm(Widget w1, Widget w2) {
  ...
  i++;
  XtSetArg(argt[i], XmNleftOffset, (width1 - width2) / 2);
  i++;
...
void CleanupAndExit(Widget w, int exit_code) {
  ...
  if (exit_code == NOTIFY_OK) {
    ...
    if (toggleon)
      exit_code = NOTIFY_FAILSAFE;

    XtGetValues(options_dtlite, argt, 1);
    if (toggleon)
      exit_code = NOTIFY_DTLITE;
    ...
    for (i = 0; i < appInfo.altDts; ++i) {
      if (alt_dts[i] != NULL) {
        ...
        if (toggleon)
          exit_code = NOTIFY_ALT_DTS + i + 1; /* alt desktops start  at 1 */
      }
    }
  }
...
static void ClearDtlabel(void) {
  ...
  XtSetArg(argt[i], XmNlabelString, xms);
  i++;
...
static void ClearDtButtons(void) {
  int i;
  ...
  if (options_failsafe)
    XtSetValues(options_failsafe, argt, 1);
  if (options_dtlite)
    XtSetValues(options_dtlite, argt, 1);
  if (options_dt)
    XtSetValues(options_dt, argt, 1);
  if (options_last_dt)
...
void RespondClearCB(Widget w, XtPointer client, XtPointer call) {
...
void CopyrightCB(Widget w, XtPointer client_data, XtPointer call_data) {
...
void EditPasswdCB(Widget w, XtPointer client, XtPointer call_data) {
  ...
  vg_TRACE_EXECUTION("currInsert=%d newInsert=%d startPos=%d endPos=%d\n",
                     cbs->currInsert, cbs->newInsert, cbs->startPos,
                     cbs->endPos);
  if (cbs->text->ptr)
    vg_TRACE_EXECUTION("text->ptr=%s\n", cbs->text->ptr);
  vg_TRACE_EXECUTION("noechobuf=%s\n", textdata->noechobuf);
#endif /* VG_TRACE */

  textdata = GetLoginTextPtr(w);
  if (NULL == textdata || textdata->bEcho)
    return;
  if (cbs->reason == XmCR_MOVING_INSERT_CURSOR)
    return;

  for (i = 0, s = buffer, t = textdata->noechobuf; (*t && i < cbs->startPos);
       i++, s++, t++)
    *s = *t;
...
void FakeFocusIn(Widget focus_widget, XtPointer client_data, XEvent *eventprm,
  ...
  XSetInputFocus(XtDisplay(focus_widget), XtWindow(focus_widget), RevertToNone,
                 CurrentTime);
  ...
  XtRemoveEventHandler(focus_widget, ExposureMask, FALSE, FakeFocusIn, NULL);
}
...
void LayoutCB(Widget w, XtPointer client_data, XtPointer call_data) {
  int i, j;
...
#define HMARGIN 4 /* min sum horizontal margin of matte */
  if (mw.width + HMARGIN > dpwidth) {
    ...
    i = 0;
    XtSetArg(argt[i], XmNwidth, width1);
    i++;
    ...
    i = 0;
    XtSetArg(argt[i], XmNwidth, mw.width);
    i++;
  ...
  if (login_form) {
    i = 0;
    XtSetArg(argt[i], XmNy, &y);
    i++;
    XtSetArg(argt[i], XmNheight, &height);
    i++;
    XtGetValues(greeting, argt, i);

    i = 0;
    XtSetArg(argt[i], XmNy, &y1);
    i++;
    ...
    if (overlap > -10) {
      ...
      i++;
      XtSetArg(argt[i], XmNbottomWidget, login_form);
      i++;
  ...
  XtSetArg(argt[i], XmNheight, &height);
  i++;
  XtSetArg(argt[i], XmNshadowThickness, &shadowThickness);
  i++;
  XtGetValues(matte, argt, i);
  ...
  mw.x =
      (x > 0 ? ToPixel(matte, XmHORIZONTAL, (int)x) : (dpwidth - mw.width) / 2);

  mw.y =
      (y > 0 ? ToPixel(matte, XmVERTICAL, (int)y) : (dpheight - mw.height) / 2);
  ...
  XtSetArg(argt[i], XmNleftAttachment, XmATTACH_FORM);
  i++;
  XtSetArg(argt[i], XmNleftOffset, spacing / 2);
  i++;
  XtSetValues(ok_button, argt, i);

  i = 0;
  XtSetArg(argt[i], XmNleftAttachment, XmATTACH_WIDGET);
  i++;
  XtSetArg(argt[i], XmNleftWidget, ok_button);
  i++;
  XtSetArg(argt[i], XmNleftOffset, spacing);
  i++;
  ...
  XtSetArg(argt[i], XmNleftAttachment, XmATTACH_WIDGET);
  i++;
  XtSetArg(argt[i], XmNleftWidget, clear_button);
  i++;
  XtSetArg(argt[i], XmNleftOffset, spacing);
  i++;
  ...
  XtSetArg(argt[i], XmNleftAttachment, XmATTACH_WIDGET);
  i++;
  XtSetArg(argt[i], XmNleftWidget, options_button);
  i++;
  XtSetArg(argt[i], XmNleftOffset, spacing);
  i++;
  ...
  if (copyright_msg) {
    ...
    i = 0;
    XtSetArg(argt[i], XmNshadowThickness, &width);
    i++;
    ...
    i = 0;
    XtSetArg(argt[i], XmNdefaultPosition, False);
    i++;
    XtSetArg(argt[i], XmNx, x1);
    i++;
    XtSetArg(argt[i], XmNy, y1);
...
void MenuItemCB(Widget w, XtPointer client_data, XtPointer call_data) {
  int i;
  char *logoFile;
  char *logoName;
  char *temp_p;
  ...
  case OB_LAST_DT:
    ...
    i = 0;
    XtSetArg(argt[i], XmNuserData, &logoFile);
    i++;
    ...
    logoName = _DtGetIconFileName(DefaultScreenOfDisplay(dpyinfo.dpy), temp_p,
                                  NULL, NULL, 0);
    ...
    i = 0;
    XtSetArg(argt[i], XmNimageName, logoName);
    i++;
...
void OptionsUnmapCB(Widget wd, XtPointer client_data, XtPointer call_data) {
...
int PingServer(void) {
  ...
  if (!sigsetjmp(pingTime, 1)) {
    ...
    if (serverDead) {
      LogError(ReadCatalog(MC_LOG_SET, MC_LOG_DEADSRV, MC_DEF_LOG_DEADSRV),
               dpyinfo.name);
      alarm(0);
...
void PingServerCB(XtPointer call_data, XtIntervalId *id) {
  ...
  if (pingInterval != 0) {
    if (PingServer())
      XtAddTimeOut((unsigned long)pingInterval * 60 * 1000, PingServerCB, NULL);
    else
      exit(NOTIFY_RESTART);
  }
}
...
void PostMenuCB(Widget w, XtPointer client_data, XtPointer call_data) {
  ...
  if (p->reason == XmCR_ARM && p->event->type == ButtonPress) {
...
void RefreshEH(Widget w, XtPointer client_data, XEvent *event,
               Boolean *continue_to_dispatch) {
...
void RequestCB(XtPointer client_data, int *source, XtInputId *id) {
  char buf[512];
  int count;
  int remainder;
  ...
  case REQUEST_OP_MESSAGE: {
    ...
    if (r->idMC) {
       ...
       */
      if (r->idMC == MC_NO_LOGIN && (fp = fopen(NO_LOGIN_FILE, "r")) != NULL) {
        /*
        ...
        while (fgets(buffer, 256, fp) != NULL) {
          ...
          string = XmStringConcat(
              xmstr, XmStringCreate(buffer, XmFONTLIST_DEFAULT_TAG));
  ...
  case REQUEST_OP_CHALLENGE: {
    ...
    XGrabKeyboard(dpyinfo.dpy, XtWindow(textdata->text[textdata->bEcho]), False,
                  GrabModeAsync, GrabModeAsync, CurrentTime);

    XmTextFieldSetString(textdata->text[textdata->bEcho],
                         r->offUserNameSeed ? buf + r->offUserNameSeed : "");

    XmTextFieldSetSelection(textdata->text[1], 0,
                            XmTextFieldGetLastPosition(textdata->text[1]),
                            CurrentTime);
    ...
    i = 0;
    XtSetArg(argt[i], XmNlabelString, string);
    i++;
    ...
    if (change) {
      ...
      if (textdata->bEcho) {
        ...
        sprintf(buf, textdata->offGreetingFormat, textdata->offGreetingUname);
...
void RespondExitCB(Widget w, XtPointer client, XtPointer call) {
  char buf[REQUEST_LIM_MAXLEN];
...
void RespondLangCB(Widget w, XtPointer client, XtPointer call) {
  XtSetArg(argt[0], XmNset, True);
  XtSetValues(w, argt, 1);
...
void RespondChallengeCB(Widget w, XtPointer client, XtPointer call) {
  char buf[REQUEST_LIM_MAXLEN];
...
void RespondDialogCB(Widget w, XtPointer client, XtPointer call_data) {
  char buf[REQUEST_LIM_MAXLEN];
 ...
 **************************************************************************/
void SetDefaultDt(Widget w) { default_dt = w; }
...
void SetDtLabelAndIcon(void) {
  ...
  if (logoName == NULL) {
    LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_LOGOBIT, MC_DEF_LOG_NO_LOGOBIT),
             logoFile);
    logoFile = NULL;
  }
...
void ShowDialogCB(Widget w, XtPointer dialog, XtPointer call_data) {
...
void TextFocusCB(Widget w, XtPointer client_data, XtPointer call_data) {
  if (w == login_text)
    focusWidget = w;
}
...
LoginTextPtr GetLoginTextPtr(Widget w) {
  LoginTextPtr textdata;
  int i;
