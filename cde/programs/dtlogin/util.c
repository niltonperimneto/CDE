...
static SIGVAL MakeLangAbort(int arg);

static int MatchesFileSuffix(const char *filename, const char *suffix);

static void ScanNLSDir(char *dirname);
...
unsigned char *ReadCatalog(int set_num, int msg_num, char *def_str) {
...
#define NLS_PATH_STRING                                                        \
  CDE_INSTALLATION_TOP "/nls/msg/%L/%N.cat:" CDE_INSTALLATION_TOP              \
...
static char *makeEnv(char *name, char *value) {
  ...
  if (!result) {
    LogOutOfMem(ReadCatalog(MC_LOG_SET, MC_LOG_MAKEENV, MC_DEF_LOG_MAKEENV));
    return 0;
  }
...
char *getEnv(char **e, char *name) {
  int l = strlen(name);

  while (*e) {
    if ((int)strlen(*e) > l && !strncmp(*e, name, l) && (*e)[l] == '=')
      return (*e) + l + 1;
...
char **setEnv(char **e, char *name, char *value) {
  char **new, **old;
  char *newe;
  int envsize;
  int l;
  ...
  if (e) {
    ...
    envsize = old - e;
    new =
        (char **)realloc((char *)e, (unsigned)((envsize + 2) * sizeof(char *)));
  } else {
    envsize = 0;
...
char **parseArgs(char **argv, char *string) {
  ...
  if (!argv) {
    ...
    if (!argv) {
      LogOutOfMem(
          ReadCatalog(MC_LOG_SET, MC_LOG_PARSEARGS, MC_DEF_LOG_PARSEARGS));
      return 0;
    }
  }
  word = string;
...
char **parseEnv(char **e, char *string) {
...
/* Forward declaration for HourGlass cursor - from libDtSvc */
extern Cursor _DtGetHourGlassCursor(Display *dpy);

void SetHourGlassCursor(Display *dpy, Window w) {
  Cursor cursor;
  ...
  if (NULL != (dirp = opendir(dirname))) {
    while ((dp = readdir(dirp)) != NULL) {
      ...
      if (locale[0] != '.' &&
        ...
        (void)snprintf(locale_path, MAXPATHLEN, "%s/%s", dirname, locale);
 ...
 */
void SetDefaultLanguage(struct display *d) {
  FILE *file;
  char lineBuf[160];
  int n;
  char *p;
...
char **setLang(struct display *d, char **env, char *langptr) {
...
/*
 * waitCode - extract exit code from wait status
 */
int waitCode(int status) {
...
/*
 * waitSig - extract signal number from wait status
 */
int waitSig(int status) {
