...
int CheckAuthentication(struct protoDisplay *pdpy, ARRAY8Ptr displayID,
...
int SelectAuthorizationTypeIndex(ARRAY8Ptr authenticationName,
...
int Willing(ARRAY8Ptr addr,
#if NeedWidePrototypes
            int connectionType,
#else
            CARD16 connectionType,
#endif /* NeedWidePrototypes */
            ARRAY8Ptr authenticationName, ARRAY8Ptr status, xdmOpCode type) {
...
ARRAY8Ptr Accept(struct sockaddr *from, int fromlen,
#if NeedWidePrototypes
                 int displayNumber)
#else
...
int SelectConnectionTypeIndex(ARRAY16Ptr connectionTypes,
   ...
   *
   */
  if (loopbackAddress.length == 0 && XdmcpAllocARRAY8(&loopbackAddress, 4)) {
  ...
  for (i = 0; i < connectionAddresses->length; i++) {
    if (!XdmcpARRAY8Equal(&connectionAddresses->data[i], &loopbackAddress))
      break;
  }
...
static char *WillingMsg(void) {
  ...
  char *cp;
  char tmpfilename[] = "/tmp/dtlogin_willing_XXXXXX";
  FILE *f;
  ...
  strcat(tmpbuf, "awk '{printf(\"%s %-.5s  load: %.3s, %.3s, "
                 "%.3s\",$(NF-6),$(NF-5),$(NF-2),$(NF-1),$NF)}'");
  strcat(tmpbuf, " > ");
