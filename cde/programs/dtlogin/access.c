...
static void FreeHostEntry(HostEntry *h);
...
static HostEntry *ReadHostEntry(FILE *file);
static int HasGlobCharacters(char *s);
static DisplayEntry *ReadDisplayEntry(FILE *file);
static void ReadAccessDatabase(FILE *file);
static int scanHostlist(HostEntry *h, ARRAY8Ptr clientAddress,
                        ...
                        char *closure, int depth, int broadcast);
static int patternMatch(char *string, char *pattern);
static int indirectAlias(char *alias, ARRAY8Ptr clientAddress,
#if NeedWidePrototypes
                         int connectionType,
#else
                         CARD16 connectionType,
...
tryagain:
  ...
  if (*hostOrAlias == ALIAS_CHARACTER) {
    ...
    if (!strcmp(hostOrAlias, "root")) {
      LogError((unsigned char *)"Access file \"%s\", root bypass disallowed\n",
               accessFile);
      free((char *)h);
      return NULL;
    }
    ...
    if (!hostent) {
      ...
      LogError(ReadCatalog(MC_LOG_SET, MC_LOG_ACC_FILE, MC_DEF_LOG_ACC_FILE),
    ...
    bcopy(hostent->h_addr, (char *)h->entry.hostAddress.data,
          hostent->h_length);
  }
  return h;
}
  ...
  if (*displayOrAlias == ALIAS_CHARACTER) {
    ...
    if (HasGlobCharacters(displayOrAlias)) {
      ...
      if ((hostent = gethostbyname(displayOrAlias)) == NULL) {
        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_ACC_DPY, MC_DEF_LOG_ACC_DPY),
      ...
      bcopy(hostent->h_addr, (char *)display->clientAddress.data,
            hostent->h_length);
...
static int scanHostlist(HostEntry *h, ARRAY8Ptr clientAddress,
  ...
  for (; h; h = h->next) {
    switch (h->type) {
    case HOST_ALIAS:
      if (indirectAlias(h->entry.aliasName, clientAddress, connectionType,
                        function, closure, depth, broadcast))
        haveLocalhost = 1;
      break;
    case HOST_ADDRESS:
...
static int indirectAlias(char *alias, ARRAY8Ptr clientAddress,
  ...
  for (d = database; d; d = d->next) {
    if (d->type != DISPLAY_ALIAS || !patternMatch(alias, d->entry.aliasName))
      continue;
    if (scanHostlist(d->hosts, clientAddress, connectionType, function, closure,
                     depth + 1, broadcast)) {
      haveLocalhost = 1;
    }
  }
...
int ForEachMatchingIndirectHost(ARRAY8Ptr clientAddress,
#if NeedWidePrototypes
                                int connectionType,
#else
                                CARD16 connectionType,
#endif /* NeedWidePrototypes */
                                void (*function)(CARD16, struct _ARRAY8 *,
                                                 char *),
                                char *closure) {
  int haveLocalhost = 0;
  DisplayEntry *d;
  ...
  for (d = database; d; d = d->next) {
    ...
    } else if (scanHostlist(d->hosts, clientAddress, connectionType, function,
                            closure, 0, FALSE)) {
      haveLocalhost = 1;
    }
    break;
...
int UseChooser(ARRAY8Ptr clientAddress,
#if NeedWidePrototypes
               int connectionType)
#else
               CARD16 connectionType)
...
void ForEachChooserHost(ARRAY8Ptr clientAddress,
#if NeedWidePrototypes
                        int connectionType,
#else
                        CARD16 connectionType,
...
int AcceptableDisplayAddress(ARRAY8Ptr clientAddress,
#if NeedWidePrototypes
                             int connectionType,
#else
                             CARD16 connectionType,
...
char *BypassLogin(char *displayName) {
  DisplayEntry *d;
  HostEntry *h;
...
NameToNetworkAddress(char *name, CARD16Ptr connectionTypep,
                     ARRAY8Ptr connectionAddress, CARD16Ptr displayNumber) {
