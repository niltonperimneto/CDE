...
/* Linux compatibility */
#define ut_syslen                                                              \
  ut_host /* Stub for Linux if it doesn't match, or just unused */
/* Actually ut_syslen is not in Linux utmpx. We should just ifdef the
 * assignments. */
#undef ut_syslen

/* Map Solaris utmpx functions to Linux equivalents */
#define makeutx(u) pututxline(u)
#define modutx(u) pututxline(u)
...
int solaris_setdevperm(char *gettyLine, uid_t uid, gid_t gid) {
...
int solaris_resetdevperm(char *gettyLine) {
  char *devname = create_devname(gettyLine);
...
int solaris_authenticate(char *prog_name, char *display_name, char *user_passwd,
                         char *user, char *line) {
  int status;
  ...
  status = ia_start(prog_name, user, line_dev, display_name, &ia_conv, &iah);
...
int solaris_accounting(char *prog_name, char *display_name, char *entry_id,
                       char *user, char *line, pid_t pid, int entry_type,
                       int exitcode) {
  ...
  if (entry_type == ACCOUNTING) {
    status = ia_start(prog_name, user, line, display_name, &ia_conv, &iah);
  } else {
    status = ia_start(prog_name, user, line_dev, display_name, &ia_conv, &iah);
  }
  ...
  if (status != IA_SUCCESS) {
    ...
    case USER_PROCESS:
      ...
      status = ia_open_session(iah, session_type, entry_type, entry_id, &out);
      if (status == IA_SUCCESS)
        audit_login_success();
      break;
    ...
    case DEAD_PROCESS:
      ...
      status = ia_close_session(iah, 0, pid, exitcode, entry_id, &out);
      break;
...
int solaris_setcred(char *prog_name, char *user, uid_t uid, gid_t gid) {
...
static void logindevperm(char *ttyn, uid_t uid, gid_t gid) {
  char *field_delims = " \t\n";
  ...
  while (fgets(line, MAXPATHLEN, fp) != NULL) {
    ...
    if (mode_str == NULL) {
      (void)fprintf(stderr, "%s: line %d, invalid entry -- %s\n", permfile,
                    lineno, line);
      continue;
    }
    ...
    if (mode < 0 || mode > 0777 || *ptr != '\0') {
      (void)fprintf(stderr, "%s: line %d, invalid mode -- %s\n", permfile,
                    lineno, mode_str);
      continue;
    }
    ...
    if (dev_list == NULL) {
      (void)fprintf(stderr, "%s: line %d, %s -- empty device list\n", permfile,
                    lineno, console);
      continue;
    }
      ...
      if ((l > DIRWLDLEN) && (strcmp(ptr, DIRWILD) == 0)) {
        *ptr = '\0'; /* chop off wildcard */
        ...
        if (chown(device, uid, gid) == -1) {
          ...
          if ((chmod(device, mode) == -1) && (errno != ENOENT)) {
            (void)fprintf(stderr, "%s: ", permfile);
            perror(device);
          }
        }
...
static void dir_dev_acc(char *dir, uid_t uid, gid_t gid, mode_t mode,
                        char *permfile) {
  DIR *dirp;
...
static void resetdevperm(char *ttyn) {
...
static int login_conv(int conv_id, int num_msg, struct ia_message **msg,
  ...
  *response = (struct ia_response *)calloc(num_msg, sizeof(struct ia_response));
  if (*response == NULL)
    return (IA_CONV_FAILURE);
...
int solaris_setutmp_mgmt(char *user,  /* user */
                         char *ttyn,  /* ttyn */
 ...
 */
int solaris_reset_utmp_mgmt(char **user,  /* user */
                            char **ttyn,  /* tty name */
...
int __setproc_cred(char *user,      /* user */
                   int flags,       /* flags - see below */
...
int solaris_setutmp_mgmt(char *user, char *ttyn, char *rhost, int flags,
                         int type, char id[]) {
  int tmplen;
...

  {
    time_t tloc;
    time(&tloc);
    utmp.ut_tv.tv_sec = tloc;
  }
  utmp.ut_pid = getpid();
  if (rhost != NULL && rhost[0] != '\0') {
    (void)strncpy(utmp.ut_host, rhost, sizeof(utmp.ut_host));
    /* Ensure null termination to be safe, though utmpx might imply it */
    if (strlen(rhost) >= sizeof(utmp.ut_host))
    ...
    if (flags & SOLARIS_LOGIN) {
      while ((u = getutxent()) != NULL) {
        if ((u->ut_type == INIT_PROCESS || u->ut_type == LOGIN_PROCESS ||
          ...
          if (id == NULL) {
            (void)memcpy(utmp.ut_id, u->ut_id, sizeof(u->ut_id));
          }
...
int solaris_reset_utmp_mgmt(char **user, char **ttyn, char **rhost, int flags,
                            int status, char id[]) {
  struct utmpx *up;
  struct utmpx ut;
  int err = 0;
  ...
  if ((flags & SOLARIS_NOLOG) == SOLARIS_NOLOG) {
    ...
    if (*ttyn != NULL && **ttyn != '\0') {
      if (strstr(*ttyn, "/dev/") != NULL)
        ...
        (void)strncpy(ut.ut_line, *ttyn, sizeof(ut.ut_line));
    }
    ut.ut_pid = pid;
    ut.ut_exit.e_termination = 0;
    ut.ut_exit.e_exit = 0;
#ifdef sun
    (void)gettimeofday((struct timeval *)&ut.ut_tv, NULL);
#endif
    (void)gettimeofday((struct timeval *)&ut.ut_tv, NULL);
    ...
    while (up = getutxent()) {
      if (up->ut_pid == pid) {
        ...
        if ((*user = (char *)strdup(up->ut_user)) == NULL ||
            (*ttyn = (char *)strdup(up->ut_line)) == NULL ||
            (*rhost = (char *)strdup(up->ut_host)) == NULL)
          return (PAM_BUF_ERR);
