...
int GetChooserAddr(char *addr, socklen_t *lenp);         // socket.c
...
static int FormatARRAY8(ARRAY8Ptr a, char *buf, int buflen) {
...
static int FormatChooserArgument(char *buf, int len) {
  char addr_buf[1024];
...
static int RegisterIndirectChoice(ARRAY8Ptr clientAddress,
...
/*ARGSUSED*/
static void AddChooserHost(CARD16 connectionType, ARRAY8Ptr addr,
                           char *closure) {
  char ***argp; /* parseArgs declared in dm.h */
  char hostbuf[1024];
...
void RunChooser(struct display *d) {
  char **args; /* parseArgs and systemEnv declared in dm.h */
  char buf[1024];
  char **env;
  ...
  strlcpy(buf, "-xdmaddress ", sizeof(buf));
  if (FormatChooserArgument(buf + strlen(buf), sizeof(buf) - strlen(buf)))
    args = parseArgs(args, buf);
  strlcpy(buf, "-clientaddress ", sizeof(buf));
  if (FormatARRAY8(&d->clientAddr, buf + strlen(buf),
...
static char *Print8Address(ARRAY8Ptr Address) {
  ...
  for (i = 0; i < (int)Address->length; i++) {
    snprintf(b, sizeof(buf) - (b - buf), " %d", Address->data[i]);
    b = buf + strlen(buf);
  }
  return (buf);
}
