...
static void PutSessionManagerOnRootWindow(char *networkIds);

static void InitializeSaveState(Boolean startUp);

static Boolean SendGetWsmClientsMessage();

static int GetCurrentWorkspaceNumber();

static void ProcessInteract(ClientRecPtr client, Boolean getWsmClientOK);

static void CancelShutdown();

static void FreeProps(PropertyRecPtr pProp);
...
Status NewClientProc(SmsConn smsConn, SmPointer managerData,
                     unsigned long *maskRet, SmsCallbacks *callbacksRet,
                     char **failureReasonRet);

void NewConnectionXtProc(XtPointer client_data, int *source, XtInputId *id);

Status RegisterClientProc(SmsConn smsConn, SmPointer managerData,
                          char *previousId);

void InteractRequestProc(SmsConn smsConn, SmPointer managerData,
                         int dialogType);

void InteractDoneProc(SmsConn smsConn, SmPointer managerData,
                      Bool cancelShutdown);

void SaveYourselfReqProc(SmsConn smsConn, SmPointer managerData, int saveType,
                         Bool shutdown, int interactStyle, Bool fast,
                         Bool global);

void SaveYourselfPhase2ReqProc(SmsConn smsConn, SmPointer managerData);

void SaveYourselfDoneProc(SmsConn smsConn, SmPointer managerData, Bool success);

void CloseConnectionProc(SmsConn smsConn, SmPointer managerData, int count,
                         char **reasonMsgs);
 ...
 */
void AddClient(ClientRecPtr newClient);

/*
 * Functions
 */

Boolean InitXSMP(char *argv0) {
  ...
  if (!SmsInitialize(SM_VENDOR_NAME, SM_RELEASE_NAME, NewClientProc, NULL,
                     HostBasedAuthProc, ERRORMSGLEN, errorMsg)) {
    DtMsgLogMessage(smGD.programName, DtMsgLogError, errorMsg);
    PostXSMPFailureDialog(XSMP_FAILURE_SMS_INITIALIZE, True);
  }

  if (!IceListenForConnections(&smXSMP.numTransports, &smXSMP.listenObjs,
                               ERRORMSGLEN, errorMsg)) {
    DtMsgLogMessage(smGD.programName, DtMsgLogError, errorMsg);
    PostXSMPFailureDialog(XSMP_FAILURE_ICE_LISTEN, True);
  }

  if (!SetAuthentication(smXSMP.numTransports, smXSMP.listenObjs,
  ...
  for (i = 0; i < smXSMP.numTransports; i++) {
    XtAppAddInput(smGD.appCon,
                  ...
                  (XtPointer)XtInputReadMask, NewConnectionXtProc,
                  (XtPointer)smXSMP.listenObjs[i]);
  }

  networkIds = IceComposeNetworkIdList(smXSMP.numTransports, smXSMP.listenObjs);
  if (!networkIds)
    PostXSMPFailureDialog(XSMP_FAILURE_ICE_COMPOSE_IDS, False);

  env = (char *)XtMalloc(strlen(SM_SESSION_MANAGER) + strlen(networkIds) + 2);
  if (!env) {
    free(networkIds);
...
static void PutSessionManagerOnRootWindow(char *networkIds) {
  ...
  XChangeProperty(smGD.display, RootWindow(smGD.display, 0), sessionManager,
                  XA_STRING, 8, PropModeReplace, (unsigned char *)networkIds,
                  strlen((char *)networkIds));
}
...
static void InitializeXSMPGlobals(void) {
...
Status NewClientProc(SmsConn smsConn, SmPointer managerData,
  ...
  if (!newClient) {
    ...
    str = strdup(
        (char *)GETMESSAGE(4, 5, "Unable to malloc memory for operation."));
    if (str) {
...
void NewConnectionXtProc(XtPointer client_data, int *source, XtInputId *id) {
  ...
  if (!ice_conn) {
    ...
    message = strdup((char *)GETMESSAGE(40, 20, "IceAcceptConnection failed."));
...
Status RegisterClientProc(SmsConn smsConn, SmPointer managerData,
  ...
  (void)printf("Received REGISTER CLIENT [%d] - id = %s\n", smsConn,
               previousId ? previousId : "New Client");
#endif /* DEBUG */
  ...
  if (!previousId) {
    ...
    for (pDbRec = smXSMP.xsmpDbList; pDbRec != NULL && found == False;
         pDbRec = pDbRec->next) {
  ...
  for (i = 1, client = connectedList; client != NULL;
...
void InteractRequestProc(SmsConn smsConn, SmPointer managerData,
  ...
  if (smXSMP.saveState.interactCount == 0) {
    ...
    if (!SendGetWsmClientsMessage()) {
      ...
      pch = strdup((char *)GETMESSAGE(
          40, 17,
          "An attempt to get a client list from the 'Window Manager' failed."));
      if (pch) {
        DtMsgLogMessage(smGD.programName, DtMsgLogWarning, pch);
        free(pch);
      }
...
void InteractDoneProc(SmsConn smsConn, SmPointer managerData,
  ...
  if (cancelShutdown && smXSMP.saveState.shutdown == True &&
    ...
    pch = strdup((char *)GETMESSAGE(
        40, 22, "A session shutdown was cancelled by the application '%s'."));
    if (pch) {
      DtMsgLogMessage(smGD.programName, DtMsgLogInformation, pch,
                      GetArrayPropertyValue(client, SmProgram));
      free(pch);
    }
...
void SaveYourselfReqProc(SmsConn smsConn, SmPointer managerData, int saveType,
  ...
  case SmSaveBoth:
    ...
    break;
  default:
    printf("save type NOT supported\n");
  }
  (void)printf("\tinteractStyle = ");
  switch (saveType) {
  case SmInteractStyleNone:
    printf("SmInteractStyleNone\n");
    break;
  case SmInteractStyleErrors:
    printf("SmInteractStyleErrors\n");
    break;
  case SmInteractStyleAny:
    printf("SmInteractStyleAny\n");
    break;
  default:
    printf("interact style NOT supported\n");
  }
#endif /* DEBUG */
  ...
  if (smXSMP.saveState.inProgress) {
    ...
    pch = strdup((char *)GETMESSAGE(40, 21,
                                    "The session will not be saved because a "
                                    "Save Session is in progress."));
    if (pch) {
  ...
  for (pClientRec = connectedList; pClientRec != NULL;
    ...
    if (pClientRec->active) {
      SmsSaveYourself(pClientRec->smConn, saveType, shutdown, interactStyle,
                      fast);
#ifdef DEBUG
      (void)printf("\tSent saveyourself to %d\n", pClientRec->smConn);
#endif /* DEBUG */
...
/*ARGSUSED*/
void SaveYourselfDoneProc(SmsConn smsConn, SmPointer managerData,
                          Bool success) {
...
static void ProcessSaveYourselfResponses(void) {
  ClientRecPtr pClientRec;
  Boolean done = False;
   ...
   * the save.
   */
  for (done = True, pClientRec = connectedList; pClientRec != NULL;
       pClientRec = pClientRec->next) {

    if (pClientRec->active && pClientRec->saveYourselfDone == False) {
      done = False;
      break;
    }
  }
...
void CloseConnectionProc(SmsConn smsConn, SmPointer managerData, int count,
  ...
  if (count > 0) {
    PostReasonsDialog(GetArrayPropertyValue(tmp, SmProgram), count, reasonMsgs,
                      True);
...
void CompleteXSMPSave(void) {
  ...
  if (!smXSMP.saveState.shutdown) {
    ...
    for (pClientRec = connectedList; pClientRec != NULL;
      ...
      if (!GetCardPropertyValue(pClientRec, SmRestartStyleHint, &restartHint))
        restartHint = pClientRec->restartHint;

      if (restartHint == SmRestartAnyway)
        ExecuteCommandProperty(SmShutdownCommand, pClientRec);
    }
    /*
...
void CloseDownClient(ClientRecPtr pClientRec) {
  int restartHint;
...
/*ARGSUSED*/
static Tt_callback_action GetWsmClientsHandler(Tt_message message,
                                               Tt_pattern pattern) {
  int lenClientIds;
  ...
  tt_message_arg_ival(message, 0, &smXSMP.saveState.numClientIds);
  tt_message_arg_bval(message, 1, (unsigned char **)&smXSMP.saveState.clientIds,
                      &lenClientIds);
  ...
  for (i = 0, pchar = smXSMP.saveState.clientIds;
       i < smXSMP.saveState.numClientIds; i++, pchar += strlen(pchar) + 1) {
    (void)printf("\tclient [%2d]: workspace = %2d, id = %s\n", i + 1,
                 smXSMP.saveState.workspaceNums[i], pchar);
  }
#endif
...
static Boolean SendGetWsmClientsMessage(void) {
  Tt_message message;
  Tt_status status;
...
static int GetCurrentWorkspaceNumber(void) {
  ...
  for (pClientRec = connectedList; pClientRec != NULL;
    ...
    if (pClientRec->interactRequested) {
      ...
      for (i = 0, pchar = smXSMP.saveState.clientIds;
        ...
        if ((currentWorkspace == smXSMP.saveState.workspaceNums[i]) &&
...
void AddClient(ClientRecPtr newClient) {
   ...
   */
  for (pClient = connectedList; pClient->next != NULL; pClient = pClient->next)
    ;
...
void MyIoErrorHandler(IceConn ice_conn) {
  if (prev_handler)
...
void XSMPExit(void) {
  ...
  if (authenticationInitialized)
    FreeAuthenticationData(smXSMP.numTransports, smXSMP.authDataEntries);
