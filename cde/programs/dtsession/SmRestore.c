...
static XtResource settingsResources[] = {
    {SmNaccelNum, SmCaccelNum, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, accelNum), XtRImmediate, (XtPointer)-1},
    {SmNaccelDenom, SmCaccelDenom, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, accelDenom), XtRImmediate, (XtPointer)-1},
    {SmNthreshold, SmCthreshold, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, threshold), XtRImmediate, (XtPointer)-1},

    {SmNtimeout, SmCtimeout, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, timeout), XtRImmediate, (XtPointer)-1},
    {SmNinterval, SmCinterval, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, interval), XtRImmediate, (XtPointer)-1},
    {SmNpreferBlank, SmCpreferBlank, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, preferBlank), XtRImmediate, (XtPointer)0},
    {SmNallowExp, SmCallowExp, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, allowExp), XtRImmediate, (XtPointer)0},
    ...
    {SmNkeyClick, SmCkeyClick, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, kbdState.key_click_percent), XtRImmediate,
     (XtPointer)-1},
    {SmNbellPercent, SmCbellPercent, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, kbdState.bell_percent), XtRImmediate,
     (XtPointer)-1},
    {SmNbellPitch, SmCbellPitch, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, kbdState.bell_pitch), XtRImmediate,
     (XtPointer)-1},
    {SmNbellDuration, SmCbellDuration, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, kbdState.bell_duration), XtRImmediate,
     (XtPointer)-1},
    {SmNledMask, SmCledMask, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, kbdState.led_mask), XtRImmediate,
     (XtPointer)0},
    {SmNglobalRepeats, SmCglobalRepeats, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, kbdState.global_auto_repeat), XtRImmediate,
     (XtPointer)2},
    ...
    {SmNkeySymsPerKey, SmCkeySymsPerKey, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, keySymPerCode), XtRImmediate, (XtPointer)0},
    ...
    {SmNmaxKeyPerMod, SmCmaxKeyPerMod, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, maxKeyPerMod), XtRImmediate, (XtPointer)0},
    {SmNmodMap, SmCmodMap, XtRString, sizeof(String),
     XtOffset(SessionSettingsPtr, modSyms), XtRString, (XtPointer) ""},

    {SmNdidQuerySettings, SmCdidQuerySettings, XtRBoolean, sizeof(Boolean),
     XtOffset(SessionSettingsPtr, didQuery), XtRImmediate, (XtPointer)False},
    {SmNshutDownState, SmCshutDownState, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, confirmMode), XtRImmediate, (XtPointer)1},
    {SmNshutDownMode, SmCshutDownMode, XtRInt, sizeof(int),
     XtOffset(SessionSettingsPtr, startState), XtRImmediate, (XtPointer)4},
};
...
static char *fontclass[] = {"*SystemFont",
                            "*UserFont",
                            "*FontList",
                            "*ButtonFontList",
                            "*LabelFontList",
...
static int FillCmdBuf(unsigned char *, unsigned char **, unsigned int *,
                      unsigned int *, unsigned int, unsigned int *);
static int FillRemoteBuf(unsigned char *, unsigned char *, unsigned char *,
                         unsigned char **, unsigned int *, unsigned int *,
                         unsigned int);
static int FillHintBuf(unsigned char *, unsigned char **, unsigned int *,
                       unsigned int *, unsigned int, unsigned int *);
...
static void ResetScreenInfo(unsigned char **, int *, unsigned int *, Boolean *,
                            char *, char *);
static void RemoteRequestFailed(char *, void *);
static void RemoteRequestSucceeded(char *, void *);

static void SetTemporaryDisplay(int screenNum);

int RestorePreferences(char *filename);

static void RestoreDisplay(int screenNum);

static Boolean StartLocalClient(char *program, char **argv, char *cwd,
                                char **envp, Boolean doChdir,
                                Boolean useIgnoreEnvResource);

static Boolean StartRemoteClient(char *program, char **argv, char *hostName,
                                 char *cwd, char **envp,
                                 Boolean useIgnoreEnvResource);

static Boolean CheckRequiredProperties(XSMPClientDBRecPtr pDbRec,
                                       char *databaseName);

static Boolean CheckRequiredFields(ProxyClientDBRecPtr pDbRec,
                                   char *databaseName, int clientNum);

static void LogXSMPOpenDatabaseFailure(char *databaseName, DtMsgLogType msgType,
                                       int errorNum);

static void LogMissingPropertyMessage(XSMPClientDBRecPtr pDbRec, char *propName,
                                      char *databaseName, DtMsgLogType msgType);

static void LogCWDMessage(char *badDir, char *appName, char *goodDir);

static char **RemoveEnvironmentVars(char **envp);

static void MarkFileDescriptors(int start_fd, int cmd, int data);
...
int RestoreState(void) {
  ...
  XChangeProperty(smGD.display, RootWindow(smGD.display, 0), XaSmRestoreMode,
                  XA_STRING, 8, PropModeReplace,
                  (unsigned char *)smGD.restoreSession,
                  strlen(smGD.restoreSession));
  XFlush(smGD.display);
...
int StartWM(void) {
  ...
  if ((smGD.wmStartup == NULL) || (*smGD.wmStartup == 0)) {
    ...
    if (smExecArray[0] != NULL) {
      (void)StartClient(smExecArray[0], smExecArray, NULL, NULL, NULL, False,
                        False, -1);
    }
  }
  ...
  if (goodWmStartup == False) {
    sprintf(wmStartupErrorString,
            GETMESSAGE(16, 7,
                       "The wmStartupCommand resource is set to:\n\n"
                       "%s\n\n"
                       "This file does not exist or is not "
                       "executable.\n" CDE_INSTALLATION_TOP
                       "/bin/dtwm will be started "
                       "instead.\n"),
            smGD.wmStartup);
  ...
  if (localWmLaunched && wmTimeout) {
    ...
    sprintf(localWmErrorString,
            GETMESSAGE(16, 9,
                       ...
                       "This will only work with X terminals that support this "
                       "protocol.\n" CDE_INSTALLATION_TOP
                       "/bin/dtwm will be started instead.\n"),
            tmpExecWmFile);
...
void ReloadResources(void) {
   ...
   */
  RestoreResources(True, "-load", "-system", "-xdefaults", NULL);
...
int RestoreResources(Boolean errorHandlerInstalled, ...) {
  pid_t forkrc;
  ...
  if (forkrc == 0) {
    ...
    if (execStatus != 0) {
      ...
      snprintf(clientMessage, (MAXPATHLEN + 256) - 1,
               ((char *)GETMESSAGE(16, 1,
                                   "Unable to exec process %s.  No session "
...
static int RestoreSettings(void) {
  ...
  if (smGD.settingPath[0] != 0) {
    ...
    if (smBase == NULL) {
      PrintError(
          DtError,
          GETMESSAGE(16, 2,
                     "Invalid client settings file.  No settings restored."));
      return (-1);
    } else {
  ...
  if ((smSettings.accelNum > -1) || (smSettings.threshold > -1)) {
    XChangePointerControl(smGD.display, True, True, smSettings.accelNum,
                          smSettings.accelDenom, smSettings.threshold);
  ...
  if ((smSettings.timeout > -1) || (smSettings.interval > -1) ||
     ...
     * that was saved.
     */
    if ((smSaverRes.saverTimeout > -1) || (smSaverRes.lockTimeout > -1)) {
    ...
    XSetScreenSaver(smGD.display, screenSaverVals.smTimeout,
                    screenSaverVals.smInterval, screenSaverVals.smPreferBlank,
  ...
  *tmpRestore = strtok(smGD.fontDirs, ",");
  ...
  while (*tmpRestore != NULL) {
    ...
    *tmpRestore = strtok(NULL, ",");
    if ((numArgs >= ptrSize) && (*tmpRestore != NULL)) {
      ptrSize += 50;
      restorePtrArray = (char **)SM_REALLOC((char *)restorePtrArray,
                                            ptrSize * sizeof(char *));
      if (restorePtrArray == NULL) {
        PrintErrnoError(DtError, smNLS.cantMallocErrorString);
        SM_FREE(restoreCharArray);
  ...
  if ((smSettings.kbdState.key_click_percent > -1) ||
    ...
    if ((smSettings.kbdState.key_click_percent > -1) ||
      ...
      kbdControl.key_click_percent = smSettings.kbdState.key_click_percent;
    ...
    if ((tmpKey == NULL) &&
      ...
      keyboardVals.smGlobalAutoRepeat = smSettings.kbdState.global_auto_repeat;
    } else {
      if (tmpKey != NULL) {
        smToSet.keyboardChange = True;
        kbdControl.auto_repeat_mode = smSettings.kbdState.global_auto_repeat;
  ...
  while (tmpKey != NULL) {
    ...
    if ((numArgs >= charSize) && (tmpKey != NULL)) {
      charSize += 500;
      restoreCharArray =
          (char *)SM_REALLOC(restoreCharArray, charSize * sizeof(char));
      if (restoreCharArray == NULL) {
        PrintErrnoError(DtError, smNLS.cantMallocErrorString);
        SM_FREE((char *)restorePtrArray);
        return (-1);
  ...
  if (smSettings.numKeyCode > 0) {
    ...
    XChangeKeyboardMapping(smGD.display, (KeyCode)smGD.display->min_keycode,
                           smSettings.keySymPerCode, tmpSyms, numArgs);
  }
 ...
 *
 *************************************<->***********************************/
void RestoreIndependentResources(void) {
  Atom actualType;
  int actualFormat;
  ...
  fltYRes = ((float)DisplayHeight(smGD.display, 0) /
             (float)DisplayHeightMM(smGD.display, 0)) *
            1000;
  ...
  if ((strcmp(currentLangPtr, smGD.sessionLang)) ||
    ...
    snprintf(fontPath, MAXPATHLEN, "%s/%s/%s/%s.%s", smGD.savePath, sessionType,
    ...
    if (resIndep == True) {
      ...
      if (XGetWindowProperty(smGD.display, RootWindow(smGD.display, 0),
                             ...
                             &leftover, (unsigned char **)&data) == Success) {
      ...
      if (smBase == NULL) {
        PrintError(
            ...
            GETMESSAGE(
                16, 6,
                "Invalid display/language independent resource file.  No "
                "display/language independent resources will be restored."));
        SM_FREE((char *)data);
        return;
      } else {
        /*
         ...
         */
        resRet = XrmGetResource(smBase, "*fontList", "*FontList", &resValRet,
                                &fontResourceReturn);

        i = 0;
        XtSetArg(uiArgs[i], XmNdefaultFontList,
                 (XmFontList)fontResourceReturn.addr);
        i++;
        ...
        for (i = 0; i < XtNumber(fonttype); i++) {
          resRet = XrmGetResource(smBase, fonttype[i], fontclass[i], &resValRet,
                                  &fontResourceReturn);
          if (resRet) {
            resdata = (char *)SM_MALLOC(fontResourceReturn.size + 30);
...
static int RestoreClients(void) {
  ...
  if (dispEnv != NULL) {
    dispSav = (char *)SM_MALLOC(
        ((strlen(dispEnv) + strlen(DISPLAY_NAME_EQUAL)) * sizeof(char) + 1));
  ...
  remoteDisplay = (unsigned char *)SM_MALLOC(sizeof(unsigned char) *
                                             (strlen(displayName) + 101));
  ...
  while (GetNextLine() != NULL) {
    ...
    if (string != NULL && !strcmp((char *)string, "dtsmcmd")) {
       ...
       */
      if (hintPtr != NULL) {
        if (FillHintBuf(hintPtr, hintsBuf, maxHintsSize, actHintsSize,
                        screenNum, numHints) != 0) {
          /*
      ...
      if ((cmdPtr != NULL) && (hostPtr == NULL)) {
        if (FillCmdBuf(cmdPtr, cmdBuf, maxCmdSize, actCmdSize, screenNum,
                       &numCmd[screenNum]) != 0) {
          /*
           * Free all malloc'd buffers and exit
           */
        ...
        if ((cmdPtr != NULL) && (hostPtr != NULL)) {
          if (FillRemoteBuf(cmdPtr, hostPtr, remoteDisplay, remoteBuf,
                            maxRemoteSize, actRemoteSize, screenNum) != 0) {
            /*
             * Free all malloc'd buffers and exit
             */
            for (i = 0; i < smGD.numSavedScreens; i++) {
  ...
  for (i = 0; i < smGD.numSavedScreens; i++) {
    ...
    if (actHintsSize[i] > 0) {
      ...
      strncpy((char *)hintsBuf[i], (char *)tmpChar, strlen((char *)tmpChar));
      XChangeProperty(smGD.display, RootWindow(smGD.display, i), XaWmDtHints,
                      XA_STRING, 8, PropModeReplace, hintsBuf[i],
                      actHintsSize[i]);
      SM_FREE((char *)hintsBuf[i]);
  ...
  if ((smRes.contManagement & SM_CM_SYSTEM) &&
      ((GetMemoryUtilization() != MEM_NOT_AVAILABLE) &&
       (clientsDone == False))) {
    while ((GetMemoryUtilization() == MEM_NOT_FULL) && (clientsDone == False)) {
      GetNextLine();
      lineP = line;
      CreateExecString((char *)lineP);
      if (smExecArray[0] != NULL) {
        (void)StartClient(smExecArray[0], (char **)smExecArray, NULL, NULL,
                          NULL, False, False, -1);
      }
      numClientsExec++;
      ...
      if (numClientsExec >= numCmd[i]) {
        ResetScreenInfo(cmdBuf, &i, actCmdSize, &clientsDone, envVal,
                        displayName);
        numClientsExec = 0;
      }
    }
  ...
  if (clientsDone == False) {
    if ((smGD.dtwmRunning) && (smRes.contManagement & SM_CM_HANDSHAKE)) {
       ...
       * to start messaging
       */
      _DtGetMwmWindow(smGD.display, RootWindow(smGD.display, 0), &dtwmWin);
  ...
  for (i = 0; i < smGD.numSavedScreens; i++) {
    ...
    if (actRemoteSize[i] > 0) {
       ...
       */
      if (cmdInvInit == False) {
        _DtInitializeCommandInvoker(smGD.display, DtSM_TOOL_CLASS,
                                    SM_RESOURCE_CLASS, NULL, smGD.appCon);
        cmdInvInit = True;
      }
      ...
      while (numRemoteExecs > 0 && GetNextLine() != NULL) {
        ...
        _DtCommandInvokerExecute(
            DtSTART_SESSION, NULL, NULL, NULL, "-", (char *)hostPtr,
            (char *)cmdPtr, RemoteRequestSucceeded, NULL, RemoteRequestFailed,
        ...
        if (numRemoteDone == MAX_REMOTE_CLIENTS) {
          PrintError(
              ...
              GETMESSAGE(
                  16, 5,
                  "You have reached the maximum allowed number of remote "
                  "clients.  No further remote clients will be restored."));
          break;
        }
      }
    }
 ...
 *
 *************************************<->***********************************/
static unsigned int PeekAhead(unsigned char *currentChar,
...
static unsigned char *GetSmartString(unsigned char **linePP) {
  ...
  /* Quoted string */
  {
    ...
    while ((*endP = *curP) && ((chlen = mblen((char *)curP, MB_CUR_MAX)) > 0) &&
           ((chlen > 1) || (*curP != '"')))
...
#ifdef MULTIBYTE
  while (*linePP && (mblen((char *)*linePP, MB_CUR_MAX) == 1) &&
         isspace(**linePP))
#else
 ...
 *
 *************************************<->***********************************/
static int FillHintBuf(unsigned char *newHint, unsigned char **hintBuf,
                       unsigned int *maxSize, unsigned int *actSize,
                       unsigned int screen, unsigned int *numHints) {
  ...
  if (maxSize[screen] == 0) {
    ...
    if ((actSize[screen] + strlen((char *)newHint) + 2) >= maxSize[screen]) {
      hintBuf[screen] = (unsigned char *)SM_REALLOC(
          (char *)hintBuf[screen],
          maxSize[screen] + (hintBufSize * sizeof(char)));
      if (hintBuf[screen] == NULL) {
        actSize[screen] = 0;
 ...
 *
 *************************************<->***********************************/
static int FillCmdBuf(unsigned char *newCmd, unsigned char **cmdBuf,
                      unsigned int *maxSize, unsigned int *actSize,
                      unsigned int screen, unsigned int *numCmd) {
  ...
  if (maxSize[screen] == 0) {
    ...
    if ((actSize[screen] + strlen((char *)newCmd)) >= maxSize[screen]) {
      cmdBuf[screen] = (unsigned char *)SM_REALLOC(
          (char *)cmdBuf[screen],
          maxSize[screen] + (cmdBufSize * sizeof(char)));
      if (cmdBuf[screen] == NULL) {
        actSize[screen] = 0;
 ...
 *
 *************************************<->***********************************/
static int FillRemoteBuf(unsigned char *newCmd, unsigned char *hostName,
                         unsigned char *displayName, unsigned char *remoteBuf[],
                         unsigned int *maxSize, unsigned int *actSize,
                         unsigned int screen) {
  unsigned char *string;
  ...
  if (maxSize[screen] == 0) {
    remoteBuf[screen] =
        (unsigned char *)SM_MALLOC(remoteBufSize * sizeof(char));
    ...
    if ((actSize[screen] + strlen((char *)newCmd) + strlen((char *)hostName) +
         strlen((char *)displayName) + strlen("-display ") + 5) >=
        maxSize[screen]) {
      remoteBuf[screen] = (unsigned char *)SM_REALLOC(
          (char *)remoteBuf[screen],
          maxSize[screen] + (remoteBufSize * sizeof(char)));
      if (remoteBuf[screen] == NULL) {
        actSize[screen] = 0;
        PrintErrnoError(DtError, smNLS.cantMallocErrorString);
 ...
 *
 *************************************<->***********************************/
void CreateExecString(char *execString)
...
#define ARG_AMT 100
  ...
  while (string != NULL) {
    ...
    if (argc >= iSizeArgv) {
      iSizeArgv += ARG_AMT;
      smExecArray =
          (char **)XtRealloc((char *)smExecArray, (iSizeArgv * sizeof(char *)));
    }
    string = (char *)GetSmartString((unsigned char **)&execString);
  }
...
static void SetTemporaryDisplay(int screenNum) {
  ...
  if (!savedDisplay) {
    ...
    if ((dispEnv = getenv(DISPLAY_NAME)) == NULL)
      return;
    savedDisplay = XtMalloc(strlen(DISPLAY_NAME_EQUAL) + strlen(dispEnv) + 2);
    if (!savedDisplay)
      return;
 ...
 *
 *************************************<->***********************************/
Boolean StartClient(char *program, char *execArray[], char *hostName, char *cwd,
                    char **envp, Boolean checkCwd,
                    Boolean useIgnoreEnvironmentResource, int screen) {
  ...
  if (!ignoreEnvPtr && useIgnoreEnvironmentResource) {
    if (smRes.ignoreEnvironment)
      ignoreEnvPtr = _DtVectorizeInPlace(smRes.ignoreEnvironment, ',');
  }
  ...
  if (!hostName || (_DtIsSameHost(localHost, hostName)))
    startStatus = StartLocalClient(program, execArray, cwd, envp, checkCwd,
                                   useIgnoreEnvironmentResource);
  else
    startStatus = StartRemoteClient(program, execArray, hostName, cwd, envp,
                                    useIgnoreEnvironmentResource);
 ...
 *
 *************************************<->***********************************/
static Boolean StartLocalClient(char *program, char *execArray[], char *cwd,
                                char **envp, Boolean doChdir,
  ...
  if (clientFork == 0) {
    ...
    if (execStatus != 0) {
      sprintf(clientMessage, ((char *)GETMESSAGE(16, 3, "Unable to exec %s.")),
              execArray[0]);
      PrintErrnoError(DtError, clientMessage);
      SM_EXIT(-1);
    }
 ...
 *
 *************************************<->***********************************/
static Boolean StartRemoteClient(char *program, char *execArray[],
                                 char *hostName, char *cwd, char **envp,
                                 Boolean useIgnoreEnvironmentResource) {
  int ioMode;
  ...
  if (channel == SPC_ERROR) {
    message = strdup((char *)GETMESSAGE(
        40, 13,
        "The following application cannot be started on host '%s'\nbecause "
        "this host cannot be reached from host '%s':\n\n   %s"));

    if (message) {
      DtMsgLogMessage(smGD.programName, DtMsgLogError, message, hostName,
                      localHost, program);
      free(message);
    }
    return (False);
  ...
  if (tt_pointer_error(netfile) != TT_OK) {
    ...
    message = strdup((char *)GETMESSAGE(
        40, 14,
        "An attempt to start application '%s'\non host '%s' failed because the "
        "following directory\ncould not be translated into cannonical "
        "form:\n\n   %s\n\n[%s]"));

    if (message) {
      DtMsgLogMessage(smGD.programName, DtMsgLogError, message, program,
                      hostName, cwd,
  ...
  if ((_DtSPCSpawn(program, netfile, execArray, tmpEnv, channel, hostName, NULL,
                   cwd, errorMessage)) == SPC_ERROR) {
  ...
  if (clientFork == 0) {
    ...
    if (execStatus != 0 && (!localWmLaunched)) {
      PrintErrnoError(DtError,
                      GETMESSAGE(16, 4,
                                 "Unable to exec process /usr/dt/bin/dtwm.  No "
                                 "window manager will be started."));
      SM_EXIT(-1);
    }
  }
}
...
static void WaitForWM(void) {
  ...
  XtAddEventHandler(smGD.topLevelWid, 0, True,
                    (XtEventHandler)HandleWMClientMessage, (XtPointer)NULL);
  ...
  wmTimeout = False;
  wmTimerId =
      XtAppAddTimeOut(smGD.appCon, smRes.waitWmTimeout, WaitWMTimeout, NULL);
  ...
  XtRemoveEventHandler(smGD.topLevelWid, 0, True,
                       (XtEventHandler)HandleWMClientMessage, (XtPointer)NULL);
 ...
 *
 *************************************<->***********************************/
static void WaitWMTimeout(XtPointer client_data, XtIntervalId *id) {
  wmTimeout = True;
  return;
 ...
 *
 *************************************<->***********************************/
static void ResetScreenInfo(unsigned char **cmdBuf, int *screen,
                            unsigned int *cmdSize, Boolean *done, char *env,
                            char *displayName) {
 ...
 *
 *************************************<->***********************************/
static void RemoteRequestFailed(char *message, void *client_data) {
  ...
  tmpSize = (strlen(REMOTE_CMD_STRING) + strlen((char *)tmpReq->hostPtr) +
  ...
  if (smExecArray[0] != NULL) {
    ...
    if (tmpCmdBuf != NULL) {
      tmpString = GETMESSAGE(
          16, 8,
          "An attempt to restore the following\ncommand (using the DT remote "
          "execution process)\non host \"%s\" failed:\n\n      %s\n\nThe "
          "following execution string will be tried:\n\n   %s\n\n");

      errorString = (char *)SM_MALLOC(
          (strlen(tmpString) + strlen((char *)tmpReq->hostPtr) +
           strlen((char *)tmpReq->cmdPtr) + strlen(tmpCmdBuf) + 1) *
          sizeof(char));

      if (errorString == NULL) {
        PrintErrnoError(DtError, smNLS.cantMallocErrorString);
      } else {
        sprintf(errorString, tmpString, tmpReq->hostPtr, tmpReq->cmdPtr,
                tmpCmdBuf);
 ...
 *
 *************************************<->***********************************/
static void RemoteRequestSucceeded(char *message, void *client_data) {
  int i;
...
Boolean StartXSMPSession(char *databaseName) {
  ClientDB inputDB;
  ...
  if ((inputDB = OpenInputClientDB(databaseName, &smXSMP.dbVersion,
  ...
  for (;;) {
    ...
    if (StartXSMPClient(pXSMPRec, databaseName)) {
       ...
       */
      for (tmpRecPtr = smXSMP.xsmpDbList; tmpRecPtr && tmpRecPtr->next != NULL;
...
Boolean StartXSMPClient(XSMPClientDBRecPtr pDbRec, char *databaseName) {
  return (StartClient(pDbRec->restartCommand[0], pDbRec->restartCommand,
                      pDbRec->clientHost, pDbRec->cwd, pDbRec->environment,
                      True, True, pDbRec->screenNum));
}

Boolean StartProxyClient(ProxyClientDBRecPtr pDbRec) {
  return (StartClient(pDbRec->command[0], pDbRec->command, pDbRec->clientHost,
                      NULL, NULL, False, False, pDbRec->screenNum));
}

Boolean ExecuteCommandProperty(char *commandName, ClientRecPtr pClientRec) {
  char **argv;
  char **envp = NULL;
  char *cwd;
  ...
  argv = (char **)XtMalloc((pPropRec->prop.num_vals + 1) * sizeof(char *));
  if (!argv)
    return (False);
  ...
  if ((pPropRec = GetPropertyRec(pClientRec, SmEnvironment)) != NULL) {
    envp = (char **)XtMalloc((pPropRec->prop.num_vals + 1) * sizeof(char *));
    if (!envp)
      return (False);
  ...
  retValue = StartClient(argv[0], argv, pClientRec->clientHost, cwd, envp, True,
                         True, pClientRec->screenNum);
...
void ExecuteDiscardCommands(char *db) {
  ...
  if ((inputDB = OpenInputClientDB(db, &version, &id)) == NULL) {
    str = strdup((char *)GETMESSAGE(
        40, 26,
        "The following client database cannot be opened:\n\n   %s\n\nThe "
        "'DiscardCommand' properties cannot be run."));
    if (!str)
      return;
  ...
  for (;;) {
    ...
    if (!StartClient(pXSMPRec->discardCommand[0], pXSMPRec->discardCommand,
                     pXSMPRec->clientHost, pXSMPRec->cwd, pXSMPRec->environment,
                     True, True, pXSMPRec->screenNum)) {
      str = strdup(
          (char *)GETMESSAGE(40, 27,
                             "An attempt to execute the 'DiscardCommand' "
...
static Boolean CheckRequiredProperties(XSMPClientDBRecPtr pDbRec,
  ...
  if (!pDbRec->program) {
    LogMissingPropertyMessage(pDbRec, SmProgram, databaseName, DtMsgLogError);
    propsOK = False;
  }

  if (!pDbRec->restartCommand) {
    LogMissingPropertyMessage(pDbRec, SmRestartCommand, databaseName,
                              DtMsgLogError);
    propsOK = False;
  }
...
static Boolean CheckRequiredFields(ProxyClientDBRecPtr pDbRec,
  ...
  if (!pDbRec->command) {
    ...
    message = strdup((char *)GETMESSAGE(40, 11,
                                        "The required resource '%s' is missing "
                                        "for client '%d'\nin the file '%s'."));

    if (!message)
      return (False);

    DtMsgLogMessage(smGD.programName, DtMsgLogError, message, SmProgram,
                    clientNum, databaseName);
...
static void LogXSMPOpenDatabaseFailure(char *databaseName, DtMsgLogType msgType,
  ...
  case 2:
    ...
    msgNum = 9;
    message = strdup(
        (char *)GETMESSAGE(40, msgNum,
                           "The following session database does not "
  ...
  if (errorNum == 1)
    ...
    DtMsgLogMessage(smGD.programName, msgType, message, data, databaseName);
...
static void LogMissingPropertyMessage(XSMPClientDBRecPtr pDbRec, char *propName,
  ...
  message =
      strdup((char *)GETMESSAGE(40, 10,
                                "The required property '%s' is missing for "
                                "client\n\n   %s\n\nin the file '%s'."));

  if (!message)
    return;

  DtMsgLogMessage(smGD.programName, msgType, message, propName,
                  pDbRec->clientId, databaseName);
...
static void LogCWDMessage(char *badDir, char *appName, char *goodDir) {
  ...
  message = strdup((char *)GETMESSAGE(
      ...
      "following directory will be used:\n\n   %s"));

  if (!message)
    return;

  DtMsgLogMessage(smGD.programName, DtMsgLogWarning, message, badDir, appName,
                  goodDir);
