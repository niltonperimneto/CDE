...
static Boolean CanReAuthenticate(char *name, uid_t uid, char *passwd,
  ...
  *spent = getspnam((*pwent)->pw_name);
  ...
  fprintf(stderr, "CanReAuthenticate(): uid: %d name: '%s' errno %d %s %s %s\n",
          uid, (name) ? name : "NULL", errno, (*pwent) ? "PWENT" : "NULL",
          (*spent) ? "SPENT" : "NULL", (name) ? name : "NULL");
#endif
  ...
  if (*pwent == NULL) { /* if we can't get this, we're screwed. */
#ifdef JET_AUTHDEBUG
...
#ifdef JET_AUTHDEBUG
    fprintf(stderr,
            "CanReAuthenticate(): (*pwent)->pw_passwd == NULL - FALSE\n");
#endif
   ...
   */
  if (*spent == NULL) { /* if it's null, lets check for the NIS case */
...
void LockDisplay(Boolean lockNow) {
  ...
  if (localAuthenticate(NULL, getuid(), NULL) == False) {
     ...
     */
    msg =
        tttk_message_create(0, TT_NOTICE, TT_SESSION, 0, "DtActivity_Began", 0);
    tt_message_send(msg);
  ...
  if (((smDD.lockCoverDialog == NULL) && (smGD.coverScreen == True)) ||
    ...
    if ((smDD.lockDialog == NULL) && (smGD.coverScreen == False)) {
       ...
       */
      i = 0;
      XtSetArg(uiArgs[i], XmNtopShadowColor, &(xcolors[0]));
      i++;
      XtSetArg(uiArgs[i], XmNbottomShadowColor, &(xcolors[1]));
      i++;
   ...
   * is lost. Ah X.
   */
  XtAddEventHandler(grabWidget, VisibilityChangeMask, False, FinishLocking,
                    NULL);

  XtManageChild(lockDlg);

  if (smGD.coverScreen == True) {
    flash_id = XtAppAddTimeOut(smGD.appCon, 1000, BlinkCaret, smDD.indLabel[1]);
  } else {
    flash_id = XtAppAddTimeOut(smGD.appCon, 1000, BlinkCaret, smDD.indLabel[0]);
  }
  ...
  if (lockDelay > 0) {
     ...
     */
    lockDelayId =
        XtAppAddTimeOut(smGD.appCon, lockDelay * 1000, RequirePassword, NULL);
  ...
  XtAddEventHandler(grabWidget,
                    (KeyPressMask | ButtonPressMask | PointerMotionMask), False,
                    EventDetected, NULL);

  /** wait 90 seconds for lock dialog to come up **/
  lockTimeId = XtAppAddTimeOut(smGD.appCon, 90000, LockAttemptFailed, lockDlg);
  return;
}
 ...
 *
 *************************************<->***********************************/
static void FinishLocking(Widget wid, XtPointer ptr, XEvent *ev, Boolean *bl) {
  int i, j;
  ...
  XtRemoveEventHandler(wid, VisibilityChangeMask, False, FinishLocking, NULL);
  XSync(smGD.display, 0);

  i = 0;
  XtSetArg(uiArgs[i], XmNy, &visibleY);
  i++;
   ...
   */
  rc = XtGrabKeyboard(grabWidget, False, GrabModeAsync, GrabModeAsync,
                      CurrentTime);
  ...
  pointerGrabbed =
      (XtGrabPointer(grabWidget, False, ButtonPressMask | PointerMotionMask,
                     GrabModeAsync, GrabModeAsync, None, smGD.lockCursor,
  ...
  if ((kbdGrabbed == False) || (pointerGrabbed == False)) {
    for (j = 0; (j < 3) && ((pointerGrabbed == False) || (kbdGrabbed == False));
         j++) {
      /*
      ...
      if (kbdGrabbed == False) {
        sleep(1);
        kbdGrabbed =
            (XtGrabKeyboard(grabWidget, False, GrabModeAsync, GrabModeAsync,
                            CurrentTime) == GrabSuccess);
      }
      ...
      if (pointerGrabbed == False) {
        sleep(1);
        pointerGrabbed =
            (XtGrabPointer(grabWidget, False,
                           ButtonPressMask | PointerMotionMask, GrabModeAsync,
                           GrabModeAsync, None, smGD.lockCursor,
                           CurrentTime) == GrabSuccess);
      }
    }
  }
  ...
  if ((pointerGrabbed != True) || (kbdGrabbed != True)) {
     ...
     */
    hostList = XListHosts(smGD.display, &hostListLen, (Bool *)&hostListActive);
...
static unsigned char lock_s_bm_bits[] = {
    0x00, 0x02, 0x00, 0x04, 0xf0, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08,
    0x08, 0x00, 0x00, 0xa8, 0x1a, 0x54, 0x1d, 0xa8, 0x1a, 0x54, 0x1d,
    0xa8, 0x1a, 0x54, 0x1d, 0xa8, 0x1a, 0x54, 0x1d, 0xfe, 0x1f};

#define lock_s_m_bm_width 13
#define lock_s_m_bm_height 16
static unsigned char lock_s_m_bm_bits[] = {
    0xf8, 0x03, 0xfc, 0x07, 0xfe, 0x0f, 0x0e, 0x0e, 0x0e, 0x0e, 0x0e,
    0x0e, 0xff, 0x1f, 0xff, 0x1f, 0xff, 0x1f, 0xff, 0x1f, 0xff, 0x1f,
    0xff, 0x1f, 0xff, 0x1f, 0xff, 0x1f, 0xff, 0x1f, 0xff, 0x1f};

void CreateLockCursor(void) {
  Pixmap source, mask;
  ...
  if (InitCursorInfo() == False) {
    ...
    source = XCreateBitmapFromData(
        smGD.display, XRootWindow(smGD.display, smGD.screen),
        (char *)lock_s_bm_bits, lock_s_bm_width, lock_s_bm_height);
    mask = XCreateBitmapFromData(
        smGD.display, XRootWindow(smGD.display, smGD.screen),
        (char *)lock_s_m_bm_bits, lock_s_m_bm_width, lock_s_m_bm_height);
    ...
    smGD.padlockCursor =
        XCreatePixmapCursor(smGD.display, source, mask, &(xcolors[0]),
                            &(xcolors[1]), lock_s_hot_x, lock_s_hot_y);
    ...
    source = XCreateBitmapFromData(
        smGD.display, XRootWindow(smGD.display, smGD.screen),
        (char *)lock_m_bm_bits, lock_m_bm_width, lock_m_bm_height);
    mask = XCreateBitmapFromData(
        smGD.display, XRootWindow(smGD.display, smGD.screen),
        (char *)lock_m_m_bm_bits, lock_m_m_bm_width, lock_m_m_bm_height);
    ...
    smGD.padlockCursor =
        XCreatePixmapCursor(smGD.display, source, mask, &(xcolors[0]),
                            &(xcolors[1]), lock_m_hot_x, lock_m_hot_y);
    XFreePixmap(smGD.display, source);
    XFreePixmap(smGD.display, mask);
  }
...
static void RecolorCursor(void) {
   ...
   */
  XRecolorCursor(smGD.display, smGD.lockCursor, &(xcolors[0]), &(xcolors[1]));
}
 ...
 *
 *************************************<->***********************************/
static void EventDetected(Widget w, XtPointer client_data, XEvent *ev,
                          Boolean *bl) {
  char str[256];
  int len;
...
#define BUF_ALLOC_LEN 64
  ...
  /*
   * password rules:
   *	- If pw_length >  MAX_PASSWORD_LENGTH, we've gone over the limit and
  ...
  UpdatePasswdField(pw_length > MAX_PASSWORD_LENGTH ? MAX_PASSWORD_LENGTH
                                                    : pw_length);
}
 ...
 *
 *************************************<->***********************************/
static Boolean CheckPassword(char *passwd) {
  char *keyholderbuf;
  char *p, *q;
 ...
 *
 *************************************<->***********************************/
static void UnlockDisplay(Boolean pointerGrabbed, Boolean kbdGrabbed) {
  int i;
  Tt_message msg;
  ...
  if (RestrictingAccess) {
    ...
    if (!hostListActive)
      XDisableAccessControl(smGD.display);
 ...
 *
 *************************************<->***********************************/
static void TakeDownLogin(XtPointer client_data, XtIntervalId *id) {
  if (lockDlgVisible == True) {
    if (smGD.coverScreen == True) {
      ...
      if (hiddenY > -1) {
        int i = 0;
        XtSetArg(uiArgs[i], XmNy, hiddenY);
        i++;
 ...
 *
 *************************************<->***********************************/
static void PutUpLogin(Boolean map, Boolean timeout) {
  ...
  if (timeout == True) {
    ...
    if (smRes.alarmTime > 0) {
      timerId = XtAppAddTimeOut(smGD.appCon, (smRes.alarmTime * 1000),
                                TakeDownLogin, NULL);
    }
  }
  XSync(smGD.display, 0);
 ...
 *
 *************************************<->***********************************/
static void LockAttemptFailed(XtPointer ptr, XtIntervalId *invId) {
  Widget lockWid = (Widget)ptr;

  PrintError(DtError, smNLS.cantLockErrorString);
  smGD.lockedState = UNLOCKED;
  XtRemoveEventHandler(lockWid, VisibilityChangeMask, False, FinishLocking,
                       NULL);
 ...
 *
 *************************************<->***********************************/
static void RequirePassword(XtPointer ptr, XtIntervalId *invId) {
  smGD.lockedState = LOCKED;
}
 ...
 *
 *************************************<->***********************************/
static void BlinkCaret(XtPointer ptr, XtIntervalId *invId) {
  static int flag = 1;
  XmString tmpString;
  int i;
  ...
  if (flag) {
    ...
    i = 0;
    XtSetArg(uiArgs[i], XmNlabelString, tmpString);
    i++;
  ...
  flash_id = XtAppAddTimeOut(smGD.appCon, 1000, BlinkCaret, ptr);
}
 ...
 *
 *************************************<->***********************************/
static void CycleSaver(XtPointer ptr, XtIntervalId *invId) {
  /*
 ...
 *
 *************************************<->***********************************/
static Boolean localAuthenticate(char *name, uid_t uid, char *passwd)
#if defined(HAS_PAM_LIBRARY)
{
  ...
  if (!passwd)
    return False;
  if (!(pwent = name ? getpwnam(name) : getpwuid(uid)))
    return False;

  endpwent();

  if (!(service = strrchr(smGD.programName, '/')))
    return False;
...

{
  ...
  if (sia_ses_init(&se, 1, Argv, (char *)NULL, pw_name, NULL,
                   0 /* don't collect info */, NULL) != SIASUCCESS) {
    seteuid(real_uid);
    return FALSE;
  }
...

{
  ...
  if (done == False) {
    ...
    if (sp != NULL &&
        !strcmp(sp->sp_pwdp, crypt(passwd, sp->sp_pwdp))) { /* a shadow match */
      rc = True;
      done = True;
    } else if (pwent != NULL &&
               !strcmp(pwent->pw_passwd,
                       crypt(passwd, pwent->pw_passwd))) { /* passwd match */
      rc = True;
      done = True;
    } else { /* failure dude! */
      rc = False;
...

{
  ...
  if (name == NULL || (sp = getspnam(name)) == NULL) {
    /*
     * Can't get entry.
     */
    rc = False;
  ...
  if ((pwent = (name == NULL ? getpwuid_shadow(uid) : getpwnam_shadow(name))) ==
      NULL)
#else
