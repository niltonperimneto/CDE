...
/*
 * Initialize D-Bus Session Bus if not present
 */
static void InitializeDBus(void) {
  ...
  /* Start dbus-launch (plain output mode is easiest to parse) */
  /* We use --exit-with-session so the bus dies when dtsession (the parent)
     dies? Actually dbus-launch starts a daemon. If we want it to die with the
     session, --exit-with-session usually expects to wrap the command. But here
     we are INSIDE the session manager. dbus-daemon --session --fork
     --print-address is another option. Let's use dbus-launch as it handles X11
     autolaunching setup well.
  */
  fp = popen("dbus-launch", "r");
  if (fp == NULL) {
    perror("Warning: Failed to run dbus-launch. D-Bus session will not be "
           "available.");
    return;
  }

  /* Read output and set environment variables */
  /* Output format:
  ...
  */
  while (fgets(path, sizeof(path) - 1, fp) != NULL) {
    ...
    if (strncmp(path, "DBUS_SESSION_BUS_ADDRESS=", 25) == 0 ||
        strncmp(path, "DBUS_SESSION_BUS_PID=", 21) == 0) {

      /* putenv requires a string that persists. strdup it. */
      putenv(strdup(path));
    }
  }
  pclose(fp);
...
int main(int argc, char **argv) {
  ...
  /* Initialize D-Bus before initializing Toolkit or anything else */
  InitializeDBus();
  ...
  if (!_DtGetLock(smGD.display, SM_RUNNING_LOCK)) {
    PrintError(
        DtError,
        GETMESSAGE(2, 2, "Another dtsession is currently running - exiting."));
    SM_EXIT(-1);
  }
...
#ifdef DEBUG
  if (smGD.DtXineramaInfo == NULL) { /* No xinerama, how... sad. */
  ...
  XtSetArg(args[n], XmNbackground,
           XBlackPixel(smGD.display, XDefaultScreen(smGD.display)));
  n++;
  ...
  n++;
  smGD.topLevelWid =
      XtAppCreateShell(SM_RESOURCE_NAME, SM_RESOURCE_CLASS,
                       applicationShellWidgetClass, smGD.display, args, n);
   ...
   */
  XtAddEventHandler(smGD.topLevelWid, PropertyChangeMask, True, ProcessEvent,
                    NULL);
...
static void StopAll(int i) { ImmediateExit(-1, 0, True); }
...
#if defined(USE_X11SSEXT)
static int RegisterX11ScreenSaver(Display *display, int *pssEventType) {
  /*
