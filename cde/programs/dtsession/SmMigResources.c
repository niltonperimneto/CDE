...
static char sccsid[] = "@(#)48  1.2  src/cde/cde1/dtsession/SmMigResources.c, "
                       "desktop, cde41J, 9520A_all 5/16/95 08:31:12";
/*
...
static char *convert1[NUMBER_OF_CONVERT1] = {
    "displayResolution", "systemFont",     "userFont",
    "FontList",          "Font",           "FontSet",
    "multiClickTime",    "sessionVersion", "background",
    "foreground",        "ColorUse",       "HelpColorUse",
    "background",        "foreground",     "enableBtn1Transfer"};
...
static char *convert2[NUMBER_OF_CONVERT2] = {
    "cycleTimeout", "lockTimeout", "saverTimeout", "random", "saverList",
};
...
static char *convert31[NUMBER_OF_CONVERT3] = {
    "dtsession", "dtsession", "dtsession", "dtsession", "dtsession",
    "dtsession", "Dtstyle",   "Dtstyle",   "Dtwm",      "Dtwm",
    "Dtwm",      "Dtwm",      "wsHelp",    "wsHelp",    "wsHelp",
    "wsHelp",    "wsHelp",    "wsHelp",    "wsHelp",    "wsHelp",
    "wsHelp",    "wsHelp",    "wsHelp",    "wsHelp",    "wsHelp",
    "cachedHelp"};
static char *convert32[NUMBER_OF_CONVERT3] = {"displayResolution",
                                              "sessionLanguage",
                                              "saverTimeout",
                                              "cycleTimeout",
                                              "lockTimeout",
...
static char *convert4[NUMBER_OF_CONVERT4] = {"XmText", "XmTextField"};
...
static char *convert5[NUMBER_OF_CONVERT5] = {"Dtwm", "Mwm"};
...
static char *convert6[NUMBER_OF_CONVERT6] = {
    "x",      "y",          "columns",    "rows",       "helpType",  "vPCount",
    "tTitle", "helpVolume", "locationId", "stringData", "workspaces"};
...
static char *convert7[NUMBER_OF_CONVERT7] = {"initialWorkspace",
                                             "workspaceList", "workspaceCount"};
...
static char *convert8[NUMBER_OF_CONVERT8] = {"title", "geometry",
                                             "iconBoxGeometry"};
...
static char *convert91[NUMBER_OF_CONVERT9] = {"backdrop", "FrontPanel",
                                              "MyFrontPanel"};

static char *convert92[NUMBER_OF_CONVERT9] = {"image", "geometry", "geometry"};
...
static char *convert10[NUMBER_OF_CONVERT10] = {"MonochromePalette", "ColorUse",
                                               "ColorPalette"};
...
int check_match1(char *match[], int noelements, int match1) {
  int i;
  int matched = FALSE;
  for (i = 0; ((matched == FALSE) && (i < noelements)); i++) {
...
/* this function determines whether a fields extracted from the resource
   ...
   Returns:
      ...
      FALSE  = if string do not match.
*/
int check_match2(char *match1_str[], char *match2_str[], int noelements,
...
*/
void MigrateResources(char *inputfile, char *outputfile) {
  ...
  if ((out_fh != NULL) && (in_fh != NULL)) {
    ...
    unmodified_buffer = malloc((size_of_mbuffer + 1) * sizeof(char *));
    if ((inputbuffer != NULL) && (inputbuffer1 != NULL) &&
        ...
        *unmodified_buffer = '\0';
...

        {
          ...
          if (string != NULL) {
            ...
            */
            new_size = strlen(unmodified_buffer) + strlen(inputbuffer1) + 2;

            if (new_size > size_of_mbuffer) {
              unmodified_buffer = realloc(unmodified_buffer, new_size);
            ...
            if ((charlen == 1) && (*realend != '\n')) {
              ...
              */
              ...
              for (; ((DtIsspace(realend) != 0) && (realend > inputbuffer1));) {
                realend = DtPrevChar(inputbuffer1, realend);
              }
              if ((mblen(realend, MB_CUR_MAX) == 1) && (*realend == '\\')) {
                *realend = '\0';
            ...
            new_size = strlen(inputbuffer) + strlen(inputbuffer1) + 2;
          ...
          if (strncmp(inputbuffer, "!", 1) != 0) {
            ...
            */
            ...
            if (resource_end != NULL) {
              if ((first_space != NULL) && (first_space < resource_end))
            ...
            if (resource_end != NULL) {
              ...
              */
              field[0] = inputbuffer;
              for (i = 1; ((i < NUMBER_OF_FIELDS) && (field[i - 1] != NULL));
                   i++) {
              ...
              */
              if ((strlen(field[0]) == 0) && (number_fields == 2) &&
                  (check_match1(convert1, NUMBER_OF_CONVERT1, 1) == TRUE))

              {
                type_found = TRUE;
              }
              if ((type_found == FALSE) && (number_fields == 3) &&
                  (strcmp(field[0], CONVERT2_0) == 0) &&
                  (strcmp(field[1], CONVERT2_1) == 0) &&
                  (check_match1(convert2, NUMBER_OF_CONVERT2, 2) == TRUE)) {
                type_found = TRUE;
              }
              if ((type_found == FALSE) && (number_fields == 2) &&
                  (check_match2(convert31, convert32, NUMBER_OF_CONVERT3, 0,
                                1) == TRUE)) {
                type_found = TRUE;
              }
              if ((type_found == FALSE) && (number_fields == 3) &&
                  (strcmp(field[2], CONVERT4_2) == 0) &&
                  (check_match1(convert4, NUMBER_OF_CONVERT4, 1) == TRUE)) {
                type_found = TRUE;
              }
              if ((type_found == FALSE) && (number_fields == 3) &&
                  (strcmp(field[2], CONVERT5_2) == 0) &&
                  (check_match1(convert5, NUMBER_OF_CONVERT5, 0) == TRUE)) {
                type_found = TRUE;
              }
              if ((type_found == FALSE) && (number_fields == 2) &&
                  (strlen(field[0]) > strlen(CONVERT6_0)) &&
                  (strncmp(field[0], CONVERT6_0, strlen(CONVERT6_0)) == 0) &&
                  (check_match1(convert6, NUMBER_OF_CONVERT6, 1) == TRUE)) {
                type_found = TRUE;
              }
              if ((type_found == FALSE) && (number_fields == 3) &&
                  ((strcmp(field[0], CONVERT7_01) == 0) ||
                   (strcmp(field[0], CONVERT7_02) == 0)) &&
                  (check_match1(convert7, NUMBER_OF_CONVERT7, 2) == TRUE)) {

                type_found = TRUE;
              }
              if ((type_found == FALSE) && (number_fields == 4) &&
                  ((strcmp(field[0], CONVERT8_01) == 0) ||
                   (strcmp(field[0], CONVERT8_02) == 0)) &&
                  (check_match1(convert8, NUMBER_OF_CONVERT8, 3) == TRUE)) {
                type_found = TRUE;
              }
              if ((type_found == FALSE) && (number_fields == 5) &&
                  ((strcmp(field[0], CONVERT9_01) == 0) ||
                   (strcmp(field[0], CONVERT9_02) == 0)) &&
                  (check_match2(convert91, convert92, NUMBER_OF_CONVERT9, 3,
                                4) == TRUE)) {
                type_found = TRUE;
              }
              if ((type_found == FALSE) && (number_fields == 3) &&
                  (check_match1(convert10, NUMBER_OF_CONVERT10, 2) == TRUE)) {
                type_found = TRUE;
              }
            }
          }
