...
#define SET_ICONGADGET_ICON_AND_EXT(widIconGadget, pszIconFileName, pszBuffer) \
  {                                                                            \
    sprintf(pszBuffer, "%s%s", pszIconFileName,                                \
            bShowPixmaps ? PIXMAP_EXT : BITMAP_EXT);                           \
    XtVaSetValues(widIconGadget, XmNimageName, "", NULL);                      \
    /*printf("setting icon to = '%s'\n", pszBuffer);*/                         \
    XtVaSetValues(widIconGadget, XmNimageName, pszBuffer, NULL);               \
  }
...
#define SET_ICONGADGET_ICON(widIconGadget, pszIconFileName)                    \
  {                                                                            \
    /*printf("setting icon to = '%s'\n", pszIconFileName);*/                   \
    XtVaSetValues(widIconGadget, XmNimageName, "", NULL);                      \
    XtVaSetValues(widIconGadget, XmNimageName, pszIconFileName, NULL);         \
  }

#define FIND_ICONGADGET_ICON(pszIcon, pszFile, size)                           \
  {                                                                            \
    pszFile = (char *)NULL;                                                    \
    pszFile = _DtGetIconFileName(XtScreen(CreateActionAppShell), NULL,         \
                                 pszIcon, NULL, size);                         \
  }
...
#define SET_TOGGLEBUTTON(widToggleButton, bState)                              \
  {                                                                            \
    XtVaSetValues(widToggleButton, XmNset, bState, NULL);                      \
  }
