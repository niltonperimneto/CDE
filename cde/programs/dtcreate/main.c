...
void GetSessionInfo(void) {
  ...
  /************************************************************************/
  if (DtSessionRestorePath(UxTopLevel, &path, sessiondata.pszSessionFile) ==
      False)
  ...
  if ((db = XrmGetFileDatabase(path)) == NULL) {
    ...
    fmtPtr = "%s\n %s\n";
    errPtr = XtMalloc((strlen(msgPtr) + strlen(fmtPtr) + strlen(path) + 1) *
                      sizeof(char));
  ...
  */
  ...
  if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
    sessiondata.showExpert =
        ((value.addr != NULL) && (strcmp((char *)value.addr, "True") == 0));
  }
...
void SaveSession(void) {
...
#endif
  XGetWindowProperty(
      XtDisplay(CreateActionAppShell), XtWindow(CreateActionAppShell),
      wmStateAtom, 0L, (long)BUFSIZ, False, wmStateAtom, &actualType,
      &actualFormat, &nitems, &leftover, (unsigned char **)&wmState);
  ...
  if (DtWsmGetWorkspacesOccupied(display, XtWindow(CreateActionAppShell),
  ...
  n = 0;
  xa_CommandStr[n] = pszExecName;
  n++;
  xa_CommandStr[n] = "-s";
  n++;
  xa_CommandStr[n] = name;
  n++;

  XSetCommand(XtDisplay(CreateActionAppShell), XtWindow(CreateActionAppShell),
              xa_CommandStr, n);
  XtFree((char *)path);
  XtFree((char *)name);
...
void RemoveTmpIconFiles(void) {
  ...
  for (i = 0; i < ICON_NUMBER; i++) {
    if ((IconDataList[i]->pmDirtyBit) && (IconDataList[i]->pmFileName[0]) &&
...
Tt_callback_action IconEdit_tt_handler(Tt_message m, Tt_pattern p) {
  ...
  if (!strcmp(op, "Edit")) {
    ...
    case TT_FAILED:
      ...
      fmtPtr = "%s = %p";
      errPtr = XtMalloc((strlen(msgPtr) + strlen(fmtPtr) + 20) * sizeof(char));
    ...
    if ((pIconData) && (!strcmp(op, "Saved"))) {
      ...
      if ((pIconData->pmMsgID) && (!strcmp(pIconData->pmMsgID, msgID))) {
        if (pIconData->pmDirtyBit) {
          ...
          int fd = mkstemp(pIconData->pmFileName);
          if (fd != -1)
            close(fd);
          bIsNewFile = True;
          tmpIconFile = pIconData->pmFileName;
        ...
        if (pIconData->bmDirtyBit) {
          ...
          int fd = mkstemp(pIconData->bmFileName);
          if (fd != -1)
            close(fd);
          bIsNewFile = True;
          tmpIconFile = pIconData->bmFileName;
      ...
      if (tmpfd == -1) {
        ...
        fmtPtr = "%s\n   %s";
        errPtr = XtMalloc(
            (strlen(msgPtr) + strlen(fmtPtr) + strlen(tmpIconFile) + 1) *
            sizeof(char));
      ...
      if ((ttbuf) && (!strncmp(ttbuf, "/* XPM */", 9))) {
        ...
        if (tmp1) {
          ...
          if (tmpfd == -1) {
            ...
            fmtPtr = "%s\n   %s";
            errPtr = XtMalloc(
                (strlen(msgPtr) + strlen(fmtPtr) + strlen(tmpbuf) + 1) *
                sizeof(char));
      ...
      if (((pIconData->pmMsgID) && (!strcmp(pIconData->pmMsgID, msgID)) &&
           (bShowPixmaps)) ||
          ((pIconData->bmMsgID) && (!strcmp(pIconData->bmMsgID, msgID)) &&
           (!bShowPixmaps))) {
...
void UxDoEditPixmap(Widget wid, char *fname) {
  ...
  if (fname) {
    ...
    if (fd1 == -1) {
      ...
      msgPtr = GETMESSAGE(5, 60, "Could not open the following file:");
      fmtPtr = "%s\n   %s";
      errPtr = XtMalloc((strlen(msgPtr) + strlen(fmtPtr) + strlen(fname) + 1) *
                        sizeof(char));
      ...
      if (strncmp(tmpbuf, "/* XPM */", 9)) {
        ...
        if ((mname) && (check_file_exists(mname))) {
          ...
          if (fd2 == -1) {
            ...
            fmtPtr = "%s\n   %s";
            errPtr =
                XtMalloc((strlen(msgPtr) + strlen(fmtPtr) + strlen(mname) + 1) *
                         sizeof(char));
      ...
      if (buffer) {
        ...
        if ((rlenFile1 = read(fd1, buffer, lenFile1)) != lenFile1) {
          ...
          fmtPtr = "%s\n   %s";
          errPtr =
              XtMalloc((strlen(msgPtr) + strlen(fmtPtr) + strlen(fname) + 1) *
                       sizeof(char));
          sprintf(errPtr, fmtPtr, msgPtr, fname);
          display_error_message(XtParent(wid), errPtr);
          XtFree(errPtr);
        ...
        printf("Buffer = '%s'\n", buffer);       /* debug */
#endif
        tt_message_barg_add(m, TT_INOUT, "XPM", buffer,
                            lenFile1 + lenFile2 + 1);
        if (mname)
          XtFree(mname);
        XtFree((char *)buffer);
...
int main(int argc, char *argv[]) {
  ...
  Widget mainIface;
  /* These are now properly declared in included headers:
   * create_AddFiletype, create_FileCharacteristics - AddFiletype.h,
  ...
  UxTopLevel = XtAppInitialize(&UxAppContext, CLASS_NAME, NULL, 0, &argc, argv,
                               NULL, NULL, 0);
  ...
  errfmt = GETMESSAGE(13, 2, "Could not connect to ToolTalk:\n%s\nExiting ...");
  ...
  /*------------------------------------------------------------------*/
  xa_WM_SAVE_YOURSELF =
      XInternAtom(XtDisplay(CreateActionAppShell), "WM_SAVE_YOURSELF", False);
  ...
  ttPatt = ttdt_session_join(sessionString, ProcessToolTalkMessage, (Widget)0,
                             (void *)0, 1);
