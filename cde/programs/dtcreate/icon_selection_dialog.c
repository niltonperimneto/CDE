...
int myscandir(char *, struct dirent ***, int (*)(struct dirent *, char *),
...
static void calc_bottom_attachment(Widget UxWidget, XtPointer UxClientData,
                                   XtPointer UxCallbackArg);
...
/*                                                                         */
/***************************************************************************/
int myscandir(char *pszDirName, struct dirent ***NameList,
              int (*Select)(struct dirent *, char *),
...
#define AINCREMENT ((int)512)
  ...
  if (!(dirptr = opendir(pszDirName))) {
    ...
    fmtPtr = "%s\n   %s";
    errPtr =
        XtMalloc((strlen(msgPtr) + strlen(fmtPtr) + strlen(pszDirName) + 1) *
                 sizeof(char));
  ...
  *NameList = (struct dirent **)malloc(sizeof(void *) * 1024);
  ...
  for (dp = readdir(dirptr); dp != NULL; dp = readdir(dirptr)) {
    ...
    if ((*Select)(dp, pszDirName)) {
      dptmp = (struct dirent *)malloc(sizeof(struct dirent) +
                                      strlen(dp->d_name) + 1);
      ...
      if (count == (sizeArray - 1)) {
        adirent = (struct dirent **)realloc(
            adirent, (sizeof(void *) * (sizeArray + AINCREMENT)));
        if (adirent) {
  ...
  *filecount = 0;
  ...
  if (dirptr) {
    ...
    *filecount = myscandir(dirname, &namelist, new_name_select, NULL);
    ...
    msgPtr = "Could not open the following directory:";
    fmtPtr = "%s\n   %s";
    errPtr = XtMalloc((strlen(msgPtr) + strlen(fmtPtr) + strlen(dirname) + 1) *
                      sizeof(char));
...
void load_directories_list(char **dirlist) {
  ...
  /******************************************************************/
  XtVaSetValues(dir_scrolled_list, XmNitems, xmstringlist, XmNitemCount,
                stringcount, NULL);
...
char *initialize_filter(char *filter_end) {
  ...
  /******************************************************************/
  initial_filter =
      (char *)calloc(filter_length + filter_end_length + 1, sizeof(char));
#ifdef DEBUG
  if (!initial_filter)
    printf("Error callocing memory in initialize_filter.\n");
#endif
  initial_filter = strcpy(initial_filter, filter);
  if (add_slash)
    initial_filter[filter_length - 1] = '/';
...
char *update_filter(char *filter) {
  ...
  /******************************************************************/
  new_filter =
      (char *)calloc(strlen(filter) + strlen(filter_end) + 1, sizeof(char));
#ifdef DEBUG
  if (!new_filter)
...
#ifdef DEBUG
  if (!old_main)
    printf("Calloc error for old_main, in apply filter.\n");
#endif
...
void update_container_contents(char *filter) {
...
#ifdef DEBUG
  if (!path)
    printf("Calloc error for path, in update_container_contents.\n");
#endif
  ...
  */
  ...
  n++;
  XtSetArg(args[n], XmNfillMode, XmFILL_SELF);
  n++;
  XtSetArg(args[n], XmNbehavior, XmICON_TOGGLE);
  n++;
  XtSetArg(args[n], XmNrecomputeSize, TRUE);
  n++;
  XtSetArg(args[n], XmNfillOnArm, TRUE);
  n++;
  ...
  if (count > icon_count) {
    ...
    icons_in_container = (DtIconGadget **)realloc(
        icons_in_container, (icon_count + 1) * sizeof(DtIconGadget *));
    ...
    for (lcv = old_count; lcv < icon_count; lcv++) {
      icons_in_container[lcv] =
          ...
          (DtIconGadget *)XtCreateManagedWidget("IconGadget", dtIconGadgetClass,
                                                (Widget)icon_scrolled_container,
                                                args, n);
...
if (namecount) {
  ...
  for (lcv = 0; lcv < count; lcv++) {
    ...
    XtVaSetValues((Widget)icons_in_container[lcv], XmNstring, xmstring,
                  XmNimageName, iconfile, NULL);
  ...
  XtVaSetValues((Widget)icons_in_container[0], XmNstring, xmstring,
                XmNimageName, (char *)NULL, NULL);
...
static void calc_bottom_attachment(Widget UxWidget, XtPointer UxClientData,
      ...
      (_UxCicon_selection_dialog *)UxGetContext(UxWidget);
  {
    if (numberOfTopButtons > 0) {
      ...
      XtVaSetValues(UxWidget, XmNbottomWidget, separatorGadget1, NULL);
  }
  return;
}
...
static void activateCB_bottom_button(Widget UxWidget, XtPointer UxClientData,
                                     XtPointer UxCallbackArg) {
  _UxCicon_selection_dialog *UxSaveCtx, *UxContext;
...

{
  ...
  if (bottom_button_one_cb) {
    ...
    bottom_button_one_cb(UxWidget, (XtPointer)basename, UxCallbackArg);
    if (basename)
      XtFree(basename);
  }
  TurnOffHourGlassAllWindows();
...
void display_nofindicon_message(Widget parent) {
  ...
  XtVaSetValues(widChild, RES_CONVERT(XmNlabelString, "Change Name"), NULL);

  widChild = XmMessageBoxGetChild(dialog, XmDIALOG_CANCEL_BUTTON);
  XtVaSetValues(widChild, RES_CONVERT(XmNlabelString, "Name OK"), NULL);
  ...
  strcpy(buffer,
         "There is no set of icons with that filename in the icon folders.\n");
  strcat(buffer, "Move the icons into one of the icon folders, then select "
                 "\"Name OK\".\n");
  strcat(buffer,
         "Or, to select a different set of icons, select \"Change Name\".\n");
  strcat(
      buffer,
  ...
  XtVaSetValues(
      ...
      RES_CONVERT(XmNmessageString, buffer), NULL);
...
static void activateCB_bottom_button1(Widget UxWidget, XtPointer UxClientData,
                                      XtPointer UxCallbackArg) {
  _UxCicon_selection_dialog *UxSaveCtx, *UxContext;
...
Enter only the name of the icon without the path or extensions.";
        ...
        if ((fullname) && (check_file_exists(fullname))) {
          TurnOnHourGlassAllWindows();
          if (bottom_button_one_cb) {
            bottom_button_one_cb(UxWidget, (XtPointer)fullname, UxCallbackArg);
          }
          TurnOffHourGlassAllWindows();
          UxPopdownInterface(IconSelector);
          if (basename)
...
static void activateCB_bottom_button2(Widget UxWidget, XtPointer UxClientData,
                                      XtPointer UxCallbackArg) {
  _UxCicon_selection_dialog *UxSaveCtx, *UxContext;
...
static void activateCB_bottom_button3(Widget UxWidget, XtPointer UxClientData,
                                      XtPointer UxCallbackArg) {
  _UxCicon_selection_dialog *UxSaveCtx, *UxContext;
...
/*                                                                         */
/***************************************************************************/
void resizeCB_clipWindow(Widget UxWidget, XtPointer UxClientData,
                         XtPointer UxCallbackArg) {
...
static void mapCB_icon_selection_dialog(Widget UxWidget, XtPointer UxClientData,
                                        XtPointer UxCallbackArg) {
...
static void focusCB_icon_selection_dialog(Widget UxWidget,
  ...
  /******************************************************************/
  file_filter_global =
      (char *)calloc(strlen((char *)file_filter) + 1, sizeof(char));

#ifdef DEBUG
  if (!file_filter_global)
    printf("Calloc error for file_filter_global, in "
           "mapCB_icon_selection_dialog.\n");
#endif
...
#define TIGHTNESS 20
  ...
  _UxParent = XtVaCreatePopupShell(
      "icon_selection_dialog_shell", xmDialogShellWidgetClass, _UxParent, XmNx,
      18, XmNy, 116, XmNwidth, 610, XmNheight, 534, XmNshellUnitType, XmPIXELS,
      XmNkeyboardFocusPolicy, XmEXPLICIT, XmNtitle, "icon_selection_dialog",
      NULL);

  UxTmp0 = dialog_title ? (char *)dialog_title : "Icon Selection Window";

  icon_selection_dialog = XtVaCreateWidget(
      "icon_selection_dialog", xmFormWidgetClass, _UxParent, XmNdialogStyle,
      XmDIALOG_FULL_APPLICATION_MODAL, XmNwidth, 550, XmNheight, 534,
      XmNunitType, XmPIXELS, RES_CONVERT(XmNdialogTitle, UxTmp0),
      XmNautoUnmanage, FALSE, XmNdefaultPosition, FALSE, NULL);
  ...
  XtAddCallback(icon_selection_dialog, XmNhelpCallback,
                (XtCallbackProc)helpCB_general, (XtPointer)HELP_ICONSELECTOR);
  ...
  bottom_button_form = XtVaCreateManagedWidget(
      "bottom_button_form", xmFormWidgetClass, icon_selection_dialog,
      XmNskipAdjust, TRUE, XmNfractionBase, ((TIGHTNESS * ntotalbuttons) - 1),

      XmNleftOffset, 10, XmNleftAttachment, XmATTACH_FORM, XmNrightOffset, 10,
      XmNrightAttachment, XmATTACH_FORM, XmNbottomOffset, 20,
      XmNbottomAttachment, XmATTACH_FORM,

      NULL);
  UxPutContext(bottom_button_form, (char *)UxIcon_selection_dialogContext);

  UxTmp0 = bottom_button_one_label ? (char *)bottom_button_one_label
                                   : GETMESSAGE(6, 10, "OK");

  /* Creation of bottom_button1 = OK */
  bottom_button1 = XtVaCreateManagedWidget(
      "bottom_button1", xmPushButtonWidgetClass, bottom_button_form,
      RES_CONVERT(XmNlabelString, UxTmp0),

      XmNleftAttachment, nbutton ? XmATTACH_POSITION : XmATTACH_FORM,
      XmNleftPosition, TIGHTNESS * nbutton, XmNrightAttachment,
      nbutton != (ntotalbuttons - 1) ? XmATTACH_POSITION : XmATTACH_FORM,
      XmNrightPosition, (TIGHTNESS * nbutton) + (TIGHTNESS - 1),

      NULL);
  nbutton++;
  XtAddCallback(bottom_button1, XmNactivateCallback,
                (XtCallbackProc)activateCB_bottom_button1, (XtPointer)NULL);

  UxPutContext(bottom_button1, (char *)UxIcon_selection_dialogContext);

  UxTmp0 = bottom_button_two_label ? (char *)bottom_button_two_label
                                   : GETMESSAGE(6, 12, "Cancel");

  /* Creation of bottom_button2 = CANCEL */
  bottom_button2 = XtVaCreateManagedWidget(
      "bottom_button2", xmPushButtonWidgetClass, bottom_button_form,
      RES_CONVERT(XmNlabelString, UxTmp0),
  ...
  UxTmp0 = bottom_button_three_label ? (char *)bottom_button_three_label
                                     : GETMESSAGE(6, 13, "Help");

  /* Creation of bottom_button3 = HELP */
  bottom_button3 = XtVaCreateManagedWidget(
      "bottom_button3", xmPushButtonWidgetClass, bottom_button_form,
      RES_CONVERT(XmNlabelString, UxTmp0), XmNuserData, icon_selection_dialog,
  ...
  UxTmp0 = bottom_button_four_label ? (char *)bottom_button_four_label
                                    : GETMESSAGE(12, 10, "Filter");

  /* Creation of bottom_button4 */
  bottom_button4 = XtVaCreateManagedWidget(
      "bottom_button4", xmPushButtonWidgetClass, bottom_button_form,
      RES_CONVERT(XmNlabelString, UxTmp0),
  ...
  separatorGadget1 = XtVaCreateManagedWidget(
      "separatorGadget1", xmSeparatorGadgetClass, icon_selection_dialog, XmNx,
      0, XmNy, 490, XmNrightOffset, 1, XmNrightAttachment, XmATTACH_FORM,
      XmNleftOffset, 1, XmNleftAttachment, XmATTACH_FORM, XmNbottomOffset, 10,
      XmNbottomWidget, bottom_button_form, XmNbottomAttachment, XmATTACH_WIDGET,
      NULL);
  UxPutContext(separatorGadget1, (char *)UxIcon_selection_dialogContext);

  /* Creation of icon_name_text_field */
  icon_name_text_field = XtVaCreateManagedWidget(
      "icon_name_text_field", xmTextFieldWidgetClass, icon_selection_dialog,
      XmNx, 9, XmNy, 440, XmNbottomOffset, 10, XmNbottomWidget,
      separatorGadget1, XmNbottomAttachment, XmATTACH_WIDGET, XmNrightOffset,
      10, XmNrightAttachment, XmATTACH_FORM, XmNleftOffset, 10,
  ...
  UxTmp0 = name_field_title ? (char *)name_field_title
                            : GETMESSAGE(11, 12, "Enter Icon Filename");

  /* Creation of icon_name_textfield_label */
  icon_name_textfield_label = XtVaCreateManagedWidget(
      "icon_name_textfield_label", xmLabelWidgetClass, icon_selection_dialog,
      XmNx, 9, XmNy, 410, RES_CONVERT(XmNlabelString, UxTmp0), XmNbottomOffset,
      0, XmNbottomWidget, icon_name_text_field, XmNbottomAttachment,
      XmATTACH_WIDGET, XmNleftOffset, 0, XmNleftWidget, icon_name_text_field,
      XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET, XmNalignment,
      XmALIGNMENT_BEGINNING, NULL);
  UxPutContext(icon_name_textfield_label,
               (char *)UxIcon_selection_dialogContext);

  UxTmp0 = filter_field_title ? (char *)filter_field_title
                              : GETMESSAGE(12, 10, "Filter");

  /* Creation of filter_textfield_label */
  filter_textfield_label = XtVaCreateManagedWidget(
      "filter_textfield_label", xmLabelWidgetClass, icon_selection_dialog, XmNx,
      10, XmNy, 10, RES_CONVERT(XmNlabelString, UxTmp0), XmNalignment,
      XmALIGNMENT_BEGINNING, XmNtopOffset, 10, XmNtopAttachment, XmATTACH_FORM,
      XmNleftOffset, 10, XmNleftAttachment, XmATTACH_FORM, NULL);
  UxPutContext(filter_textfield_label, (char *)UxIcon_selection_dialogContext);

  /* Creation of filter_text_field */
  filter_text_field = XtVaCreateManagedWidget(
      "filter_text_field", xmTextFieldWidgetClass, icon_selection_dialog, XmNx,
      10, XmNy, 40, XmNtopOffset, 0, XmNtopWidget, filter_textfield_label,
      XmNtopAttachment, XmATTACH_WIDGET, XmNrightOffset, 10, XmNrightAttachment,
      XmATTACH_FORM, XmNleftOffset, 10, XmNleftAttachment, XmATTACH_FORM, NULL);
  ...
  UxTmp0 = directory_title ? (char *)directory_title
                           : GETMESSAGE(11, 30, "Icon Folders");

  /* Creation of directory_list_label */
  directory_list_label = XtVaCreateManagedWidget(
      "directory_list_label", xmLabelWidgetClass, icon_selection_dialog, XmNx,
      0, XmNy, 90, XmNlabelType, XmSTRING, RES_CONVERT(XmNlabelString, UxTmp0),
      XmNalignment, XmALIGNMENT_BEGINNING, XmNtopOffset, 10, XmNtopWidget,
      filter_text_field, XmNtopAttachment,
      use_filter_field ? XmATTACH_WIDGET : XmATTACH_FORM, XmNleftOffset, 10,
      XmNleftAttachment, XmATTACH_FORM, NULL);
  UxPutContext(directory_list_label, (char *)UxIcon_selection_dialogContext);

  UxTmp0 = container_title ? (char *)container_title
                           : GETMESSAGE(11, 11, "Icon Files");

  /* Creation of icon_container_label */
  icon_container_label = XtVaCreateManagedWidget(
      "icon_container_label", xmLabelWidgetClass, icon_selection_dialog, XmNx,
      281, XmNy, 90, XmNlabelType, XmSTRING,
      RES_CONVERT(XmNlabelString, UxTmp0), XmNalignment, XmALIGNMENT_BEGINNING,
      XmNtopOffset, 10, XmNtopWidget, filter_text_field, XmNtopAttachment,
      XmATTACH_WIDGET, XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition,
      55, NULL);
  UxPutContext(icon_container_label, (char *)UxIcon_selection_dialogContext);

  /* Creation of top_button_form */
  top_button_form = XtVaCreateManagedWidget(
      "top_button_form", xmFormWidgetClass, icon_selection_dialog,
      XmNresizePolicy, XmRESIZE_NONE, XmNx, 10, XmNy, 290, XmNbottomOffset, 10,
      XmNbottomWidget,
      use_icon_name_field ? icon_name_textfield_label : separatorGadget1,
      XmNbottomAttachment, XmATTACH_WIDGET, XmNrightOffset, 1,
      XmNrightAttachment, XmATTACH_FORM, XmNleftOffset, 1, XmNleftAttachment,
      XmATTACH_FORM, NULL);
  ...
  top_button1 = XtVaCreateManagedWidget(
      "top_button1", xmPushButtonWidgetClass, top_button_form, XmNx, 40, XmNy,
      0, XmNleftOffset, 0, XmNbottomAttachment, XmATTACH_FORM,
      XmNleftAttachment, XmATTACH_POSITION, XmNtopOffset, 0, XmNleftPosition,
      get_top_b1_position(numberOfTopButtons),
      RES_CONVERT(XmNlabelString, UxTmp0), NULL);
  ...
  top_button2 = XtVaCreateManagedWidget(
      "top_button2", xmPushButtonWidgetClass, top_button_form, XmNx, 220, XmNy,
      0, XmNleftOffset, 0, XmNtopOffset, 0, XmNleftPosition,
      get_top_b2_position(numberOfTopButtons), XmNbottomAttachment,
  ...
  top_button3 = XtVaCreateManagedWidget(
      "top_button3", xmPushButtonWidgetClass, top_button_form, XmNx, 310, XmNy,
      0, RES_CONVERT(XmNlabelString, UxTmp0), XmNleftPosition, 72,
      XmNleftOffset, 1, XmNleftAttachment, XmATTACH_POSITION, XmNbottomOffset,
      0, XmNbottomAttachment, XmATTACH_FORM, NULL);
  ...
  UxTmp0 = (char *)((numberOfTopButtons > 0) ? top_button_form
                                             : icon_name_textfield_label);

  /* Creation of dir_scrolled_list */
  XtSetArg(args[i], XmNshadowThickness, 2);
  i++;
  XtSetArg(args[i], XmNlistSizePolicy, XmCONSTANT);
  i++;

  XtSetArg(args[i], XmNleftOffset, 10);
  i++;
  XtSetArg(args[i], XmNleftAttachment, XmATTACH_FORM);
  i++;
  XtSetArg(args[i], XmNtopOffset, 0);
  i++;
  XtSetArg(args[i], XmNtopWidget, directory_list_label);
  i++;
  XtSetArg(args[i], XmNtopAttachment, XmATTACH_WIDGET);
  i++;
  XtSetArg(args[i], XmNrightOffset, 20);
  i++;
  XtSetArg(args[i], XmNrightWidget, icon_container_label);
  i++;
  XtSetArg(args[i], XmNrightAttachment, XmATTACH_WIDGET);
  i++;

  XtSetArg(args[i], XmNbottomOffset, 10);
  i++;
  if (numberOfTopButtons > 0) {
    XtSetArg(args[i], XmNbottomWidget, top_button_form);
    i++;
  } else if (use_icon_name_field) {
    XtSetArg(args[i], XmNbottomWidget, icon_name_textfield_label);
    i++;
  } else {
    XtSetArg(args[i], XmNbottomWidget, separatorGadget1);
    i++;
  }
  ...
  dir_scrolled_list =
      XmCreateScrolledList(icon_selection_dialog, "dir_scrolled_list", args, i);
  XtVaSetValues(dir_scrolled_list, XmNselectionPolicy, XmBROWSE_SELECT,
  ...
  icon_scrolled_win = XtVaCreateManagedWidget(
      "icon_scrolled_win", xmScrolledWindowWidgetClass, icon_selection_dialog,
      XmNscrollingPolicy, XmAUTOMATIC,
      /*                      XmNnavigationType, XmTAB_GROUP, */
      XmNx, 282, XmNy, 84, XmNscrollBarDisplayPolicy, XmAS_NEEDED,
      XmNrightOffset, 10, XmNrightAttachment, XmATTACH_FORM, XmNtopOffset, 0,
      XmNtopWidget, icon_container_label, XmNtopAttachment, XmATTACH_WIDGET,
      XmNleftOffset, 0, XmNleftWidget, icon_container_label, XmNleftAttachment,
      XmATTACH_OPPOSITE_WIDGET, XmNbottomOffset, 10, XmNbottomWidget, None,
  ...
  icon_scrolled_container = XtVaCreateManagedWidget(
      "icon_scrolled_container", xmRowColumnWidgetClass, icon_scrolled_win,
      XmNnavigationType, XmTAB_GROUP,
      /*XmNborderWidth, 1,*/
      /*XmNnumColumns, 2,*/
      XmNnumColumns, 1, XmNorientation, XmVERTICAL, XmNpacking, XmPACK_COLUMN,
      XmNshadowThickness, 1, NULL);

  XtVaSetValues(icon_selection_dialog, XmNcancelButton, bottom_button2, NULL);
...
Widget create_icon_selection_dialog(
    swidget _UxUxParent, unsigned char *_Uxdialog_title,
    unsigned char *_Uxfilter_field_title, int _Uxuse_filter_field,
    unsigned char *_Uxfile_filter, unsigned char *_Uxdirectory_title,
    unsigned char **_Uxdirectories_list, unsigned char *_Uxcontainer_title,
    int _UxnumberOfTopButtons, unsigned char *_Uxtop_button_one_label,
    XtCallbackProc _Uxtop_button_one_cb, unsigned char *_Uxtop_button_two_label,
    XtCallbackProc _Uxtop_button_two_cb,
    unsigned char *_Uxtop_button_three_label,
    XtCallbackProc _Uxtop_button_three_cb, int _Uxuse_icon_name_field,
    unsigned char *_Uxname_field_title, int _UxnumberOfBottomButtons,
    unsigned char *_Uxbottom_button_one_label,
    XtCallbackProc _Uxbottom_button_one_cb,
    unsigned char *_Uxbottom_button_two_label,
    XtCallbackProc _Uxbottom_button_two_cb,
    unsigned char *_Uxbottom_button_three_label,
    XtCallbackProc _Uxbottom_button_three_cb,
    unsigned char *_Uxbottom_button_four_label,
    XtCallbackProc _Uxbottom_button_four_cb)

{
  ...
  UxIcon_selection_dialogContext = UxContext =
      (_UxCicon_selection_dialog *)UxNewContext(
          sizeof(_UxCicon_selection_dialog), False);
  ...
  case 0:
    XtUnmanageChild(top_button_form);
    break;
  case 1:
    XtUnmanageChild(top_button2);
  case 2:
    XtUnmanageChild(top_button3);
  };
