...
long allocpixel(Colormap cmap, char *name, char *def) {
  XColor col;
  XColor tmp;
  ...
  if (!XAllocColor(dsp, cmap, &col)) {
    ...
    fprintf(
        stderr,
        CATGETS(scmc_catd, 2, 35, "couldn't allocate: %s, using %s instead\n"),
        name, def);
#endif
...
int main(int argc, char *argv[]) {
  ...
  if (!create) {
    ...
    XtDisplayInitialize(appContext, dsp, ProgramName, ProgramName, NULL, 0,
                        &dummyArgc, dummyArgv);
    shellWidget = XtAppCreateShell(ProgramName, ProgramName,
                                   applicationShellWidgetClass, dsp, NULL, 0);
    DtAppInitialize(appContext, dsp, shellWidget, ProgramName, ProgramName);
    ...
    Win[0].w = XCreateWindow(
        dsp, RootWindowOfScreen(scr), 0, 0, WidthOfScreen(scr) - 100,
        HeightOfScreen(scr) - 100, 0, CopyFromParent, InputOutput,
        CopyFromParent, CWBackPixel | CWEventMask, &xswa);
  ...
  for (window = 0; window < windows; window++) {
    ...
    if (Win[window].perscreen == NULL) {
      ...
      if (mono || CellsOfScreen(pscreen->screen) == 2) {
        ...
        hsbramp(0.0, saturation, 1.0, 1.0, saturation, 1.0, colorcount, red,
                green, blue);
        pscreen->npixels = 0;
    ...
    Win[window].gc =
        XCreateGC(dsp, Win[window].w, GCForeground | GCBackground, &xgcv);
  }
  if (-1 == nice(nicelevel)) {
