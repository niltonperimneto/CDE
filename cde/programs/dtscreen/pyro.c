...
typedef struct _pyrostruct pyrostruct;
...
static void shootup(perwindow *pwin, pyrostruct *pp, rocket *rp) {
  XFillRectangle(dsp, pwin->w, pp->bgGC, (int)(rp->x), (int)(rp->y), ROCKETW,
                 ROCKETH + 3);
  ...
  XFillRectangle(dsp, pwin->w, pwin->gc, (int)(rp->x), (int)(rp->y), ROCKETW,
                 ROCKETH + random() % 4);
}
...
static void burst(perwindow *pwin, pyrostruct *pp, rocket *rp) {
  ...
  if (stype == DOUBLECLOUD) {
    XSetForeground(dsp, pwin->gc, rp->color2);
    XDrawPoints(dsp, pwin->w, pwin->gc, rp->Xpoints2, nstars, CoordModeOrigin);
  }
#endif
}
