 ...
 */
/*-
 * resource.c - resource management for dtscreen, the X Window System
 * lockscreen.
 *
...
static void DumpResources(void) {
  int i;
  ...
  for (i = 0; i < NUMPROCS - 1; i++) {
    printf("%s.%s.%s: %d\n", classname, LockProcs[i].cmdline_arg, "delay",
           LockProcs[i].def_delay);
    printf("%s.%s.%s: %d\n", classname, LockProcs[i].cmdline_arg, "batchcount",
           LockProcs[i].def_batchcount);
    printf("%s.%s.%s: %g\n", classname, LockProcs[i].cmdline_arg, "saturation",
...
static void GetResource(XrmDatabase database, char *parentname,
                        char *parentclass, char *name, char *class,
                        int valueType, char *def,
  ...
  case t_Bool:
    LowerString(buffer);
    *((int *)valuep) = (!strcmp(buffer, "true") || !strcmp(buffer, "on") ||
                        !strcmp(buffer, "enabled") || !strcmp(buffer, "yes"))
                           ? True
                           : False;
    break;
  case t_Int:
...
static XrmDatabase parsefilepath(char *xfilesearchpath, char *TypeName,
                                 char *ClassName) {
...
static void open_display(void) {
  if (display != NULL) {
...
#else
    {
      fprintf(
          stderr,
          CATGETS(scmc_catd, 2, 19, "%s: Malformed -display argument:  %s.\n"),
          ProgramName, display);
      exit(1);
    }
#endif
  ...
  {
    fprintf(stderr,
            CATGETS(scmc_catd, 2, 17, "%s: Unable to open display %s.\n"),
            ProgramName, display);
    exit(1);
  }
#endif
...
void printvar(char *class, argtype var) {
  switch (var.type) {
  case t_String:
...
void GetResources(int argc, char *argv[]) {
   ...
   */
  XrmParseCommand(&nameDB, nameTable, 1, ProgramName, &argc, argv);
  GetResource(nameDB, ProgramName, "*", "name", "Name", t_String, DEF_CLASSNAME,
              &classname);
  ...
  applicationDB =
      parsefilepath(env ? env : DEF_FILESEARCHPATH, "app-defaults", classname);

  XrmParseCommand(&cmdlineDB, cmdlineTable, cmdlineEntries, ProgramName, &argc,
                  argv);
  ...
  GetResource(RDB, ProgramName, classname, "mode", "Mode", t_String, DEF_MODE,
              (caddr_t *)&mode);
  ...
  for (i = 0; i < NGENARGS; i++)
    GetResource(RDB, ProgramName, classname, genvars[i].name, genvars[i].class,
                genvars[i].type, genvars[i].def, genvars[i].var);

  for (i = 0; i < NMODEARGS; i++)
    GetResource(RDB, modename, modeclass, modevars[i].name, modevars[i].class,
...
#else
    fprintf(stderr, "%s", CATGETS(scmc_catd, 2, 7, "Unknown mode: "));
#endif
    Syntax(mode);
  }
}
