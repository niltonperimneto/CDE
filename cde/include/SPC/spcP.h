...
/* Forward declare types used in headers that might include spc-proto.h
 * recursively */
typedef struct _buffered_data *buffered_data_ptr;
typedef struct _SPC_Connection *SPC_Connection_Ptr;
...
#define IS_SPCIO_READWRITEERROR(a)                                             \
  ((a & SPCIO_SOURCE_MASK) == SPCIO_READWRITEERROR)
...
typedef struct _SPC_Connection {
  ...
  int termination_id;
  struct _SPC_Connection *next; /* The next server in the list */
...
typedef struct _SPC_Channel { /* Sub-Process Control Channel */
  ...
  SPC_TerminateHandlerType
      Terminate_Handler; /* The SIGCLD user handler routine */
...
SPC_Channel_Ptr open_noio_channel_object(SPC_Channel_Ptr channel, int iomode,
                                         XeString hostname);
int read_noio_channel_object(SPC_Channel_Ptr channel, int connector,
                             XeString buffer, int nbytes);
int write_noio_channel_object(SPC_Channel_Ptr channel, XeString buffer,
                              int nbytes);
...
void noio_channel_object_input_handler(void *client_data, int *source,
                                       SPCInputId *id);
...
SPC_Channel_Ptr open_pipe_channel_object(SPC_Channel_Ptr channel, int iomode,
                                         XeString hostname);
int read_pipe_channel_object(SPC_Channel_Ptr channel, int connector,
                             XeString buffer, int nbytes);
...
int add_input_pipe_channel_object(SPC_Channel_Ptr channel,
                                  SbInputHandlerProc handler, void *data);

/* pty.c */
void pty_channel_class_init(object_clasp c);
SPC_Channel_Ptr open_pty_channel_object(SPC_Channel_Ptr channel, int iomode,
                                        XeString hostname);
int read_pty_channel_object(SPC_Channel_Ptr channel, int connector,
                            XeString buffer, int nbytes);
...
SPC_Channel_Ptr open_remote_channel_object(SPC_Channel_Ptr channel, int iomode,
                                           XeString hostname);
...
int write_remote_channel_object(SPC_Channel_Ptr channel, XeString buffer,
                                int len);
int read_remote_channel_object(SPC_Channel_Ptr channel, int connection,
                               XeString buffer, int len);
...
int set_termio_remote_channel_object(SPC_Channel_Ptr channel, int, int,
                                     struct termios *);
...
int SPC_Newline_Filter(SPC_Channel_Ptr channel, int connector, XeString buffer,
                       int ntoread);
int SPC_Input_Handler(SPC_Channel_Ptr channel, int connector);
SPC_Channel_Ptr open_channel_object(SPC_Channel_Ptr channel, int iomode,
                                    XeString hostname);
int close_channel_object(SPC_Channel_Ptr channel);
int read_channel_object(SPC_Channel_Ptr channel, int connector, XeString buffer,
                        int nbytes);
...
int add_input_channel_object(SPC_Channel_Ptr channel,
                             SbInputHandlerProc handler, void *data);
...
void SPC_XtAddInput(SPC_Channel_Ptr channel, int *id_addr, int fd,
...
void SPC_Conditional_Packet_Handler(void *client_data, int *source,
                                    SPCInputId *id);
