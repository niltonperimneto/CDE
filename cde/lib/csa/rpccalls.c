 ...
 *****************************************************************************/
/* exposed for Rust dispatch */
CSA_return_code csa2cmsattrs(CSA_uint32 num_attrs, CSA_attribute *csaattrs,
                             CSA_enum *ops, CSA_uint32 *newnum,
                             cms_attribute **newattrs, CSA_enum **newops);
void free_cmsattrs(CSA_uint32 num_attrs, cms_attribute *attrs);
...
/* Legacy implementation for v2-v4 protocols (called from Rust) */
extern CSA_return_code _DtCm_rpc_open_calendar_legacy(Calendar *cal) {
  _DtCm_libentry *entries;
  cms_access_entry *alist;
  CSA_return_code stat;
  _DtCm_Client_Info *ci = cal->conn.ci;

  /* This function is called from Rust for v2-v4 protocols */
   ...
   * the calendar
   */
  if ((stat = _DtCm_table_lookup_range(cal, 0, 0, B_TRUE, 0, 0, 0, 0, NULL,
                                       NULL, &entries)) == CSA_SUCCESS) {
    if (entries)
      _DtCm_free_libentries(entries);

    if ((stat = _DtCm_table_get_access(cal, &alist)) == CSA_SUCCESS) {
...
extern CSA_return_code _DtCm_rpc_open_calendar(Calendar *cal) {
  DP(("rpccalls.c: _DtCm_rpc_open_calendar (Rust dispatcher)\n"));

  /* Delegate to Rust dispatcher which handles v5 and calls back to C for legacy
   */
  return rs_rpc_open_calendar(cal);
}
...
extern CSA_return_code _DtCm_rpc_create_calendar_legacy(Calendar *cal,
  ...
  /* This function is called from Rust for v2-v4 protocols */
  if (ci->vers_out == TABLEVERS_4) {

    if ((stat = _DtCm_table_create(cal)) == CSA_SUCCESS && num_attrs > 0) {
      ...
      if (attrs[i].value == NULL)
        ...
        stat =
            _DtCm_table_set_access(cal, attrs[i].value->item.access_list_value);
    }
     ...
     */
    if (stat == CSA_SUCCESS &&
        (stat = _DtCm_table_lookup_range(cal, 0, 0, B_TRUE, 0, 0, 0, 0, NULL,
                                         NULL, &entries)) == CSA_SUCCESS) {
      if (entries)
        _DtCm_free_libentries(entries);
    }
...
extern CSA_return_code _DtCm_rpc_create_calendar(Calendar *cal,
                                                 CSA_uint32 num_attrs,
                                                 CSA_attribute *attrs) {
  DP(("rpccalls.c: _DtCm_rpc_create_calendar (Rust dispatcher)\n"));

  /* Delegate to Rust dispatcher */
  return rs_rpc_create_calendar(cal, num_attrs, attrs);
}
...
extern CSA_return_code _DtCm_rpc_delete_calendar(Calendar *cal) {
  ...
  if ((stat = _DtCm_create_tcp_client(
           cal->location, TABLEVERS, _DtCM_LONG_TIMEOUT, &ci)) != CSA_SUCCESS) {
    return (stat);
  }
...
_DtCm_rpc_lookup_reminder(Calendar *cal, time_t tick, CSA_uint32 num_names,
                          char **reminder_names, CSA_uint32 *num_rems,
  ...
  if (ci->vers_out == TABLEVERS) {
    ...
    if ((stat = csa2cmsnames(num_names, reminder_names, &num_cmsnames,
                             &cmsnames)) != CSA_SUCCESS)
      return (stat);
    ...
    if (res != NULL) {
      if ((stat = res->stat) == CSA_SUCCESS)
        stat = _DtCm_cms2csa_reminder_ref(res->rems, num_rems, rems);
    } else {
      stat = (cal->conn.stat == RPC_SUCCESS)
                 ? CSA_E_SERVICE_UNAVAILABLE
                 : _DtCm_clntstat_to_csastat(cal->conn.stat);
    ...
    stat = _DtCm_table_lookup_reminder(cal, num_names, reminder_names, num_rems,
                                       rems);
  }
...
extern CSA_return_code _DtCm_rpc_lookup_entries(Calendar *cal,
  ...
  *elist_r = NULL;
  ...
  if (ci->vers_out == TABLEVERS) {
    ...
    if ((stat = csa2cmsattrs(num_attrs, attrs, ops, &num_cmsattrs, &cmsattrs,
                             &newops)) != CSA_SUCCESS)
      return (stat);
    ...
    if (res != NULL) {
      if ((stat = res->stat) == CSA_SUCCESS) {
        stat =
            _DtCmCmsentriesToLibentries(&cal->entry_tbl, res->entries, elist_r);
      }
    ...
    if ((stat = _DtCmHashCriteria(
             cal->entry_tbl, num_attrs, attrs, NULL, ops, &no_match,
             &no_start_time_range, &no_end_time_range, &start1, &start2, &end1,
             &end2, &id, &hnum, &hattrs, &hops)) == CSA_E_INVALID_ATTRIBUTE) {
      return (CSA_SUCCESS);
    ...
    if (id > 0 && no_start_time_range && no_end_time_range) {
      ...
      key.id = 0;
      if ((stat = _DtCm_table_lookup_key_range(cal, NULL, id, _DtCM_BOT,
                                               _DtCM_EOT, &key, NULL)) ==
              CSA_SUCCESS &&
          key.id > 0) {
        ...
        if ((stat = _DtCm_make_libentry(NULL, &entry)) == CSA_SUCCESS) {
          ...
          if ((stat = _DtCm_table_lookup(cal, entry)) == CSA_SUCCESS)
            *elist_r = entry;
          else
      ...
      stat =
          _DtCm_table_lookup_range(cal, start1, start2, no_end_time_range, end1,
                                   end2, id, hnum, hattrs, hops, elist_r);
    }
...
extern CSA_return_code _DtCm_rpc_enumerate_sequence(Calendar *cal,
  ...
  *elist_r = NULL;
  ...
  if (ci->vers_out == TABLEVERS) {
    ...
    stat =
        _DtCm_table_lookup_key_range(cal, entry, 0, start, end, NULL, elist_r);
  }
...
extern CSA_return_code _DtCm_rpc_lookup_entry_by_id(Calendar *cal,
  ...
  if (ci->vers_out == TABLEVERS) {
      ...
      if (res->entries) {
        if ((stat = res->entries->stat) == CSA_SUCCESS) {
          if ((stat = _DtCmUpdateAttributes(
                   res->entries->num_attrs, res->entries->attrs,
                   &entry->e->num_attrs, &entry->e->attrs, &cal->entry_tbl,
                   B_FALSE, NULL, B_TRUE)) == CSA_SUCCESS) {
            entry->filled = B_TRUE;
          }
        }
      } else
    ...
    if (res)
      xdr_free((xdrproc_t)xdr_cms_get_entry_attr_res, (char *)res);
  } else {
...
extern CSA_return_code _DtCm_rpc_get_cal_attrs(Calendar *cal, int index,
  ...
  if (ci->vers_out == TABLEVERS) {
    ...
    if ((stat = csa2cmsnames(num_names, names, &num_cmsnames, &cmsnames)) !=
        CSA_SUCCESS)
      return (stat);
    ...
    if (res != NULL) {
      if ((stat = res->stat) == CSA_SUCCESS) {
        if ((stat = _DtCmUpdateAttributes(
                 res->num_attrs, res->attrs, &cal->num_attrs, &cal->attrs,
                 &cal->cal_tbl, B_TRUE, NULL, B_TRUE)) == CSA_SUCCESS)
    ...
    case CSA_CAL_ATTR_ACCESS_LIST_I:
      if ((stat = _DtCm_table_get_access(cal, &val.item.access_list_value)) ==
          CSA_SUCCESS) {
        val.type = CSA_VALUE_ACCESS_LIST;
        stat = _DtCmUpdateAccessListAttrVal(
    ...
    case CSA_CAL_ATTR_NUMBER_ENTRIES_I:
      if ((stat = _DtCm_table_size(cal, (int *)&val.item.uint32_value)) ==
          CSA_SUCCESS) {
        val.type = CSA_VALUE_UINT32;
        stat = _DtCmUpdateSint32AttrVal(
            &val, &cal->attrs[CSA_CAL_ATTR_NUMBER_ENTRIES_I].value);
      }
      break;
    }
  }
...
extern CSA_return_code _DtCm_rpc_set_cal_attrs(Calendar *cal,
  ...
  if (ci->vers_out == TABLEVERS) {
    ...
    if ((stat = csa2cmsattrs(num_attrs, attrs, NULL, &num_cmsattrs, &cmsattrs,
                             NULL)) != CSA_SUCCESS)
      return (stat);
...
extern CSA_return_code _DtCm_rpc_insert_entry(Calendar *cal,
  ...
  if (ci->vers_out == TABLEVERS) {
    ...
    if (res != NULL) {
      if ((stat = res->stat) == CSA_SUCCESS) {
        stat =
            _DtCmCmsentriesToLibentries(&cal->entry_tbl, res->entry, entry_r);
      }
...
extern CSA_return_code
_DtCm_rpc_update_entry(Calendar *cal, _DtCm_libentry *oentry,
                       CSA_uint32 num_attrs, CSA_attribute *attrs,
                       CSA_enum scope, _DtCm_libentry **nentry) {
  _DtCm_Client_Info *ci = NULL;
  CSA_return_code stat = CSA_SUCCESS;
  ...
  if (ci->vers_out == TABLEVERS) {
    ...
    if (res != NULL) {
      if ((stat = res->stat) == CSA_SUCCESS) {
        stat = _DtCmCmsentriesToLibentries(&cal->entry_tbl, res->entry, nentry);
      }
    ...
    stat = _DtCm_table_update(cal, oentry, num_attrs, attrs, scope, nentry);
  }
...
extern CSA_return_code
_DtCm_rpc_delete_entry(Calendar *cal, _DtCm_libentry *entry, CSA_enum scope) {
  _DtCm_Client_Info *ci = NULL;
  CSA_return_code stat = CSA_SUCCESS;
...
extern CSA_return_code
_DtCm_rpc_list_calendar_attributes(Calendar *cal, CSA_uint32 *number_names,
                                   char ***names_r) {
  _DtCm_Client_Info *ci = NULL;
  CSA_return_code stat = CSA_SUCCESS;
    ...
    *names_r = NULL;
    ...
    if (res != NULL) {
      if ((stat = res->stat) == CSA_SUCCESS && res->num_names) {
        if (names = _DtCm_alloc_character_pointers(res->num_names)) {
          for (i = 0; i < res->num_names; i++) {
            if ((names[i] = strdup(res->names[i].name)) == NULL) {
              _DtCm_free(names);
...
extern CSA_return_code _DtCm_rpc_list_calendars(char *location,
  ...
  if ((stat = _DtCm_create_tcp_client(location, TABLEVERS, _DtCM_LONG_TIMEOUT,
                                      &ci)) != CSA_SUCCESS) {
    return (stat);
  }
  conn.ci = ci;
  ...
  if (ci->vers_out == TABLEVERS) {
    ...
    *names_r = NULL;
    ...
    if (res != NULL) {
      if ((stat = res->stat) == CSA_SUCCESS && res->num_names) {
        if (names = _DtCm_alloc_calendar_users(res->num_names)) {
          for (i = 0; i < res->num_names; i++) {
            if ((names[i].calendar_address = strdup(res->names[i])) == NULL) {
              _DtCm_free(names);
              return (CSA_E_INSUFFICIENT_MEMORY);
            }
          }
...
extern CSA_return_code _DtCm_do_registration(_DtCm_Connection *conn, char *cal,
                                             unsigned long update_type) {
  CSA_return_code stat;
  ...
  *newattrs = NULL;
  ...
  for (i = 0, j = 0; i < num_attrs && stat == CSA_SUCCESS; i++) {
    ...
    case CSA_VALUE_ACCESS_LIST:
      stat =
          _DtCm_csa2cms_access_list(csaattrs[i].value->item.access_list_value,
                                    &cmsattrs[j].value->item.access_list_value);
      break;
    case CSA_VALUE_CALENDAR_USER:
      if (csaattrs[i].value->item.calendar_user_value &&
          csaattrs[i].value->item.calendar_user_value->user_name) {
        cmsattrs[j].value->item.calendar_user_value =
            csaattrs[i].value->item.calendar_user_value->user_name;
      } else {
        cmsattrs[j].value->item.calendar_user_value = nullstr;
      }
      break;
    ...
    case CSA_VALUE_REMINDER:
      if ((rptr1 = csaattrs[i].value->item.reminder_value) &&
        ...
        rptr2->lead_time = rptr1->lead_time ? rptr1->lead_time : nullstr;
        rptr2->snooze_time = rptr1->snooze_time ? rptr1->snooze_time : nullstr;
...
void free_cmsattrs(CSA_uint32 num_attrs, cms_attribute *attrs) {
  ...
  for (i = 0; i < num_attrs; i++) {
    ...
    case CSA_VALUE_ACCESS_LIST:
      _DtCm_free_cms_access_entry(attrs[i].value->item.access_list_value);
      break;
    case CSA_VALUE_REMINDER:
 ...
 */
static CSA_return_code _GetV4UserAccess(Calendar *cal,
                                        cms_access_entry *alist) {
  CSA_return_code stat = CSA_SUCCESS;
  ...
  if (_DtCmIsUserName(cal->name) == B_TRUE) {
    if (strcmp(user, cal->name) == 0) {
       ...
       * the same domain
       */
      if (dom = strchr(cal->location, '.'))
        dom++;
      if (dom == NULL ||
          _DtCmIsSamePath(_DtCmGetLocalDomain(NULL), dom) == B_TRUE)
        isowner = B_TRUE;
    }
  } else {
