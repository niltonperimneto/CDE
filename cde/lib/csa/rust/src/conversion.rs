...
unsafe fn convert_attribute_value(
    ...
    let val = match type_ {
        ...
        // Case 11 is intentionally absent from the CDE CSA XDR definition (cm.x).
        // The original C header (csa.h) skips discriminant 11 in the union, so no
        // Rust variant exists for it — fall through to the default arm below.
        12 => {
        ...
        // `default` is the unit variant generated by xdrgen for the XDR
        // `default: void;` arm.  Any unknown or future discriminant maps here.
