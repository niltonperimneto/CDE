...
_DtCm_accessentry4_to_cmsaccesslist(Access_Entry_4 *a4,
...
extern CSA_return_code _DtCm_appt4_to_attrs(char *calname, Appt_4 *a4,
                                            uint num_attrs,
                                            cms_attribute *attrs,
                                            boolean_t rerule) {
  CSA_return_code stat = CSA_SUCCESS;
  ...
  while (stat == CSA_SUCCESS && reminders != NULL) {
    if ((index = _DtCm_old_reminder_name_to_index(reminders->attr)) >= 0) {
      if ((stat = _DtCmAttr4ToReminderValue(reminders, &attrs[index].value)) !=
          CSA_SUCCESS) {
        break;
      }
    }
  ...
  if (a4->period.period != single_4 && a4->ntimes == _DtCM_OLD_REPEAT_FOREVER)
  ...
  if (stat != CSA_SUCCESS ||
      (a4->appt_id.key > 0 &&
       (stat = _DtCm_set_opaque_attrval(
            &opq, &attrs[CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I].value))))
    ;
  else if ((stat = _csa_tick_to_iso8601(a4->appt_id.tick, buf)
                       ? CSA_E_INVALID_DATE_TIME
                       : CSA_SUCCESS) ||
           (stat = _DtCm_set_string_attrval(
                buf, &attrs[CSA_ENTRY_ATTR_START_DATE_I].value,
                CSA_VALUE_DATE_TIME)))
    ;
  else if (stat = eventtype4_to_attrs(a4->tag->tag, num_attrs, attrs))
    ;
  else if (stat = _DtCm_set_sint32_attrval(
               a4->tag->showtime, &attrs[CSA_X_DT_ENTRY_ATTR_SHOWTIME_I].value))
    ;
  else if ((stat = _csa_tick_to_iso8601(a4->appt_id.tick + a4->duration, buf)
                       ? CSA_E_INVALID_DATE_TIME
                       : CSA_SUCCESS) ||
           (stat = _DtCm_set_string_attrval(
                buf, &attrs[CSA_ENTRY_ATTR_END_DATE_I].value,
                CSA_VALUE_DATE_TIME)))
    ;
  else if (stat = _DtCm_set_string_attrval(
               a4->what, &attrs[CSA_ENTRY_ATTR_SUMMARY_I].value,
               CSA_VALUE_STRING))
    ;
  else if (stat = _DtCm_set_user_attrval(
               a4->author, &attrs[CSA_ENTRY_ATTR_ORGANIZER_I].value))
    ;
  else if (stat = _DtCm_set_uint32_attrval(
               _DtCm_apptstatus4_to_status(a4->appt_status),
               &attrs[CSA_ENTRY_ATTR_STATUS_I].value))
    ;
  else if (stat = _DtCm_set_uint32_attrval(
               _DtCm_privacy4_to_classification(a4->privacy),
               &attrs[CSA_ENTRY_ATTR_CLASSIFICATION_I].value))
    ;
  else if (rerule && (stat = _RepeatInfoToRule(
                          a4, &attrs[CSA_ENTRY_ATTR_RECURRENCE_RULE_I].value)))
    ;
  else if (!rerule &&
           ((stat = _DtCm_set_uint32_attrval(
                 ntimes, &attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_TIMES_I].value)) ||
            (stat = _DtCm_set_sint32_attrval(
                 _DtCm_interval4_to_rtype(a4->period.period),
                 &attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I].value)) ||
            (a4->period.period == nthWeekday_4 &&
             (stat = _DtCm_set_sint32_attrval(
                  a4->period.nth,
                  &attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_OCCURRENCE_NUM_I]
                       .value))) ||
            (a4->period.period >= everyNthDay_4 &&
             a4->period.period <= everyNthMonth_4 &&
             (stat = _DtCm_set_uint32_attrval(
                  a4->period.nth,
                  &attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_INTERVAL_I].value))) ||
            (a4->period.enddate != 0 &&
             ((stat = _csa_tick_to_iso8601(a4->period.enddate, buf)
                          ? CSA_E_INVALID_DATE_TIME
                          : CSA_SUCCESS) ||
              (stat = _DtCm_set_string_attrval(
                   buf, &attrs[CSA_X_DT_ENTRY_ATTR_SEQUENCE_END_DATE_I].value,
                   CSA_VALUE_DATE_TIME))))))
    ;
  else {
    return (CSA_SUCCESS);
...
extern CSA_return_code
_DtCmAttr4ToReminderValue(Attribute_4 *remval, cms_attribute_value **attrval) {
  CSA_reminder rval;
  char timestr[BUFSIZ];
...
static CSA_return_code eventtype4_to_attrs(Event_Type_4 tag, uint num_attrs,
  ...
  switch (tag) {
  case appointment_4:
    if ((stat = _DtCm_set_uint32_attrval(
             CSA_TYPE_EVENT, &attrs[CSA_ENTRY_ATTR_TYPE_I].value)) !=
        CSA_SUCCESS)
      return (stat);
...
static CSA_return_code _RepeatInfoToRule(Appt_4 *a4,
  ...
  if (a4->ntimes == _DtCM_OLD_REPEAT_FOREVER) {
    duration = 0;
             ...
             a4->period.period <= everyNthMonth_4) {
    duration =
        ((a4->ntimes % a4->period.nth) ? 1 : 0) + a4->ntimes / a4->period.nth;
  } else
    duration = a4->ntimes;
  ...
  case daysOfWeek_4:
    snprintf(buf, sizeof buf, "W1 #%d ", duration);
    /* XXX strcat is unsafe here - fixed using strncat */
    if (a4->period.nth & 0x1)
      strncat(buf, "SU ", sizeof(buf) - strlen(buf) - 1);
    if (a4->period.nth & 0x2)
      strncat(buf, "MO ", sizeof(buf) - strlen(buf) - 1);
    if (a4->period.nth & 0x4)
      strncat(buf, "TU ", sizeof(buf) - strlen(buf) - 1);
    if (a4->period.nth & 0x8)
      strncat(buf, "WE ", sizeof(buf) - strlen(buf) - 1);
    if (a4->period.nth & 0x10)
      strncat(buf, "TH ", sizeof(buf) - strlen(buf) - 1);
    if (a4->period.nth & 0x20)
      strncat(buf, "FR ", sizeof(buf) - strlen(buf) - 1);
    if (a4->period.nth & 0x40)
      strncat(buf, "SA ", sizeof(buf) - strlen(buf) - 1);
    break;
  }
  ...
  if (a4->period.enddate) {
    ...
    strncat(buf, datestr, sizeof(buf) - strlen(buf) - 1);
  }
