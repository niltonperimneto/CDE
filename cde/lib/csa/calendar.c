 ...
 */
extern Calendar *_DtCm_new_Calendar(const char *calendar) {
  Calendar *cal;
  int i;
  ...
  if ((cal->attrs = (cms_attribute *)calloc(
           1, sizeof(cms_attribute) * (_DtCm_cal_name_tbl->size + 1))) ==
      NULL) {
    free(cal->name);
    free(cal);
    return (NULL);
  ...
  for (i = 1; i <= _DtCm_cal_name_tbl->size; i++) {
    if ((cal->attrs[i].name.name = strdup(_DtCm_cal_name_tbl->names[i])) ==
        NULL) {
      /* clean up */
      cal->num_attrs = i - 1;
...
extern void _DtCm_free_Calendar(Calendar *cal) {
  ...
  if (cal->name)
    free(cal->name);
  if (cal->ehead)
    _DtCm_free_libentries((_DtCm_libentry *)cal->ehead);
 ...
 */
extern uint _DtCm_add_to_entry_list(Calendar *cal, caddr_t elist) {
  int i;
...
extern void _DtCm_remove_from_entry_list(Calendar *cal, caddr_t head,
                                         caddr_t tail) {
...
extern void _DtCm_reset_cal_attrs(Calendar *cal) { cal->got_attrs = B_FALSE; }
...
extern CSA_return_code _DtCm_list_old_cal_attr_names(Calendar *cal,
  ...
  if ((names = _DtCm_alloc_character_pointers(_DtCM_OLD_CAL_ATTR_SIZE)) == NULL)
  ...
  for (i = 1, j = 0; i <= _DtCM_DEFINED_CAL_ATTR_SIZE; i++) {
    if (_CSA_cal_attr_info[i].fst_vers > 0 &&
      ...
      if ((names[j] = strdup(_CSA_calendar_attribute_names[i])) == NULL) {
        _DtCm_free(names);
        return (CSA_E_INSUFFICIENT_MEMORY);
      } else
        j++;
...
extern CSA_return_code _DtCm_get_all_cal_attrs(Calendar *cal,
  ...
  if (cal->rpc_version >= _DtCM_FIRST_EXTENSIBLE_SERVER_VERSION) {
    if ((stat = _DtCm_rpc_get_cal_attrs(cal, 0, 0, NULL)) != CSA_SUCCESS)
      return (stat);
    ...
    for (i = 1, j = 0; i <= cal->num_attrs; i++) {
      if (cal->attrs[i].value != NULL) {
        ...
        if ((stat = _DtCm_cms2csa_attribute(cal->attrs[i], &attrs_r[j])) !=
            CSA_SUCCESS) {
          _DtCm_free(attrs_r);
          return (stat);
        } else
    ...
    *attrs = attrs_r;
    ...
    return (_DtCm_get_cal_attrs_by_name(cal, _DtCM_DEFINED_CAL_ATTR_SIZE + 1,
...
_DtCm_get_cal_attrs_by_name(Calendar *cal, CSA_uint32 num_names,
  ...
  for (i = 0, j = 0; i < num_names; i++) {
    ...
    case CSA_CAL_ATTR_ACCESS_LIST_I:
      ...
      stat = _get_access_list(cal, num_names, names, &attrs_r[j]);
      break;

    case CSA_CAL_ATTR_NUMBER_ENTRIES_I:

      stat = _get_number_entries(cal, num_names, names, &attrs_r[j]);
      break;
    ...
    case CSA_CAL_ATTR_CALENDAR_OWNER_I:
      ...
      stat = _get_calendar_owner(cal, num_names, names, &attrs_r[j]);
      break;
    ...
    default:
      if (cal->rpc_version >= _DtCM_FIRST_EXTENSIBLE_SERVER_VERSION) {
        ...
        if (cal->got_attrs == B_FALSE &&
          ...
          if (index == -1 && (index = _DtCm_get_index_from_table(
                                  cal->cal_tbl, names[i])) == -1)
            break;
        }

        if (stat == CSA_SUCCESS && cal->attrs[index].value)
          stat = _DtCm_cms2csa_attribute(cal->attrs[index], &attrs_r[j]);
      }
      break;
    }
 ...
 */
extern void _DtCm_count_entry_in_list(caddr_t elist) {
  int i;
  _DtCm_libentry *list;

  for (i = 0, list = (_DtCm_libentry *)elist; list != NULL;
       i++, list = list->next)
    ;
 ...
 */
static CSA_return_code _get_owner_from_old_cal(Calendar *cal, char *owner) {
  char *calname;
  ...
  if (_DtCmIsUserName(calname) == B_TRUE)
    snprintf(owner, BUFSIZ, "%s", calname);
  else
    *owner = '\0';
...
static CSA_return_code _get_calendar_owner(Calendar *cal, uint num_names,
  ...
  if (cal->rpc_version >= _DtCM_FIRST_EXTENSIBLE_SERVER_VERSION) {
    if (cal->got_attrs == B_FALSE) {
      if ((stat = _DtCm_rpc_get_cal_attrs(cal, 0, num_names, names)) !=
          CSA_SUCCESS)
        return (stat);
    }
    owner = cal->attrs[CSA_CAL_ATTR_CALENDAR_OWNER_I].value->item.string_value;
  } else {
    if ((stat = _get_owner_from_old_cal(cal, buf)) != CSA_SUCCESS)
      return (stat);
    else if (*buf == '\0')
  ...
  if (attr->name = strdup(CSA_CAL_ATTR_CALENDAR_OWNER)) {
    ...
    if ((val->item.calendar_user_value = (CSA_calendar_user *)calloc(
...
static CSA_return_code _get_product_identifier(Calendar *cal,
  ...
  if (attr->name = strdup(CSA_CAL_ATTR_PRODUCT_IDENTIFIER))
    return (_DtCm_set_csa_string_attrval(_DtCM_PRODUCT_IDENTIFIER, &attr->value,
                                         CSA_VALUE_STRING));
  else
    return (CSA_E_INSUFFICIENT_MEMORY);
}
...
static CSA_return_code _get_version_supported(Calendar *cal,
  ...
  if (attr->name = strdup(CSA_CAL_ATTR_VERSION))
    return (_DtCm_set_csa_string_attrval(_DtCM_SPEC_VERSION_SUPPORTED,
...
static CSA_return_code _get_server_version(Calendar *cal, CSA_attribute *attr) {
  if (attr->name = strdup(CSA_X_DT_CAL_ATTR_SERVER_VERSION))
    return (_DtCm_set_csa_uint32_attrval(cal->rpc_version, &attr->value));
  else
    return (CSA_E_INSUFFICIENT_MEMORY);
}
...
static CSA_return_code _get_data_version(Calendar *cal, CSA_attribute *attr) {
  if (attr->name = strdup(CSA_X_DT_CAL_ATTR_DATA_VERSION))
    return (_DtCm_set_csa_uint32_attrval(cal->file_version, &attr->value));
  else
    return (CSA_E_INSUFFICIENT_MEMORY);
}
...
static CSA_return_code _get_access_list(Calendar *cal, uint num_names,
  ...
  if (cal->rpc_version < _DtCM_FIRST_EXTENSIBLE_SERVER_VERSION)
    stat = _DtCm_rpc_get_cal_attrs(cal, CSA_CAL_ATTR_ACCESS_LIST_I, 0, NULL);
  else if (cal->got_attrs == B_FALSE)
    stat = _DtCm_rpc_get_cal_attrs(cal, 0, num_names, names);

  if (stat == CSA_SUCCESS)
    stat =
        _DtCm_cms2csa_attribute(cal->attrs[CSA_CAL_ATTR_ACCESS_LIST_I], attr);
...
static CSA_return_code _get_number_entries(Calendar *cal, uint num_names,
  ...
  if (cal->rpc_version < _DtCM_FIRST_EXTENSIBLE_SERVER_VERSION)
    stat = _DtCm_rpc_get_cal_attrs(cal, CSA_CAL_ATTR_NUMBER_ENTRIES_I, 0, NULL);
  else if (cal->got_attrs == B_FALSE)
    stat = _DtCm_rpc_get_cal_attrs(cal, 0, num_names, names);

  if (stat == CSA_SUCCESS)
    stat = _DtCm_cms2csa_attribute(cal->attrs[CSA_CAL_ATTR_NUMBER_ENTRIES_I],
                                   attr);
