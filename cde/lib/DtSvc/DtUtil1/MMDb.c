...
static void build_file_list(DtShmProtoIntList, DtDirPaths*, DtDtsMMHeader*, const char*);
...
static int write_db(DtDtsMMHeader* header, void* index, int size, const char* CacheFile);
...
int _MMWriteDb(DtDirPaths* dirs, int num_db, DtDtsDbDatabase** db_list, const char* CacheFile) {
  ...
  header.db_offset = build_new_db(shm_handle, int_handle, num_db, db_list);
  ...
  tbl_data =
      (void*)_DtShmProtoAddIntLst(int_handle, tbl_size / sizeof(int), &header.str_tbl_offset);
...
static void build_file_list(DtShmProtoIntList int_handle, DtDirPaths* dirs, DtDtsMMHeader* header,
                            const char* suffix) {
  DIR* dirp;
  ...
  for (i = 0; dirs->paths[i]; i++) {
    ...
    while ((result = _XReaddir(dirp, dirEntryBuf)) != NULL) {
      ...
      if (c && strcmp(c, suffix) == 0) {
        ...
        mtime_list = (time_t*)realloc(mtime_list, count * sizeof(time_t));
        mtime_list[count - 1] = buf.st_mtime;
        boson_list[count - 1] = _DtShmProtoAddStrtab(shm_handle, pathname, &isnew);
        free(pathname);
        continue;
      }
    }
  ...
  data = _DtShmProtoAddIntLst(int_handle, count * sizeof(time_t) / sizeof(int),
                              &header->mtimes_offset);
...
/* static int db_table_size(int num_db, DtDtsDbDatabase **db_list) Unused */
static void _DtMMSortDataTypes(DtShmProtoStrtab str_handle) {
  DtDtsDbDatabase* dc;
  DtDtsDbDatabase* da;
  int i;
  ...
  for (i = 0; i < dc->recordCount; i++) {
    if (dc->recordList[i]->compare != cde_dc_field_compare) {
      _DtDtsDbFieldSort(dc->recordList[i], cde_dc_field_compare);
    }
  }
  _DtDtsDbRecordSort(dc, cde_dc_compare);

  for (i = 0; i < da->recordCount; i++) {
    if (da->recordList[i]->compare != _DtDtsDbCompareFieldNames) {
      _DtDtsDbFieldSort(da->recordList[i], _DtDtsDbCompareFieldNames);
    }
  }
...
static void add_if_missing(DtDtsDbRecord* rec_ptr, XrmQuark name, char* value) {
...
static void _DtMMAddActionsToDataAttribute(DtDtsDbDatabase* db_ptr) {
  ...
  /* int		action_flag = 0; -- Unused */
  /* int		sort_flag = 0; -- Unused */
  /* int		found_flag = 0; -- Unused */
  /* int		n; -- Unused */
  ...
  for (rec = 0; rec < db_ptr->recordCount; rec++) {
    /* int	found_des = 0; -- Unused */
    /* int	found_icon = 0; -- Unused */
    /* int	found_label = 0; -- Unused */
    char* obj_type;

    rec_ptr = db_ptr->recordList[rec];
    obj_type = XrmQuarkToString(rec_ptr->recordName);

    if (_DtDtsDbGetFieldByName(rec_ptr, DtDTS_DA_IS_ACTION) == 0) {
      continue;
    }
    add_if_missing(rec_ptr, desc_qrk, DtActionDescription(obj_type));
    add_if_missing(rec_ptr, icon_qrk, DtActionIcon(obj_type));
    add_if_missing(rec_ptr, label_qrk, DtActionLabel(obj_type));
  }
}

static int build_new_db(DtShmProtoStrtab shm_handle, DtShmProtoIntList int_handle, int num_db,
                        DtDtsDbDatabase** db_list) {
  ...
  new_db_list = (DtDtsMMDatabase*)_DtShmProtoAddIntLst(
      int_handle, num_db * sizeof(DtDtsMMDatabase) / sizeof(int), &db_index);
  for (db = 0; db < num_db; db++) {
    int last_boson = -1;
    /* int	list_count = 0; -- Unused */
    ...
    new_rec_ptr_list = (DtDtsMMRecord*)_DtShmProtoAddIntLst(
        int_handle, db_ptr->recordCount * sizeof(DtDtsMMRecord) / sizeof(int), &index);
    ...
    for (rec = 0; rec < db_ptr->recordCount; rec++) {
      ...
      new_rec_ptr->pathId =
          _DtShmProtoAddStrtab(shm_handle, tmp = _DtDbPathIdToString(rec_ptr->pathId), &isnew);
      XtFree(tmp);
      new_rec_ptr->seq = rec_ptr->seq;
      new_rec_ptr->fieldCount = rec_ptr->fieldCount;

      /* create space for field list */
      new_fld_ptr_list = (DtDtsMMField*)_DtShmProtoAddIntLst(
          int_handle, rec_ptr->fieldCount * sizeof(DtDtsMMField) / sizeof(int), &index);
      ...
      for (fld = 0; fld < rec_ptr->fieldCount; fld++) {
        ...
        new_fld_ptr->fieldValue =
            fld_ptr->fieldValue ? _DtShmProtoAddStrtab(shm_handle, fld_ptr->fieldValue, &isnew) : 0;
      }
    }
...
static int srch(const void* a, const void* b) {
...
/* static void showtable(...) Unused */

static int build_name_list(DtDtsDbDatabase* db, DtShmProtoIntList int_handle, DtDtsMMHeader* head) {
  struct list* other;
  int i;
  char* c;
  int isnew;
  ...
  name_index = (struct list*)calloc(db->recordCount * 2, sizeof(struct list));
  other = (struct list*)calloc(db->recordCount, sizeof(struct list));

  /* step through all records */
  for (i = 0; i < db->recordCount; i++) {
    /* DtShmBoson	boson; -- Unused */
    char* attr;
    /* char	*t; -- Unused */

    /* see if a name pattern exist */
    attr = _DtDtsDbGetFieldByName(db->recordList[i], DtDTS_NAME_PATTERN);
    if (!attr) {
      /* it didn't so check path pattern */
      attr = _DtDtsDbGetFieldByName(db->recordList[i], DtDTS_PATH_PATTERN);
      if (!attr) {
    ...
    if (c && *c == '\0') {
      ...
      name_index[next].boson = _DtShmProtoAddStrtab(shm_handle, (const char*)attr, &isnew);
  ...
  */
  ...
  for (i = 0; i <= next; i++) {
    if (i != next && (last_boson == -1 || name_index[i].boson == last_boson)) {
      ...
      list_of_recs = (int*)realloc(list_of_recs, ++list_count * sizeof(int));
      ...
      */
      if (list_count == 1) {
        ...
        _DtShmProtoAddInttab(indexList, last_boson, list_of_recs[0]);
      } else {
        /* if there are multiple items in the list
           create a table for them */
        int* list = _DtShmProtoAddIntLst(int_handle, list_count, &index);

        /* write the list to the to the table */
        memcpy(list, list_of_recs, list_count * sizeof(int));

        /* then index on the negative of the boson
           so that we know it is a list */
        _DtShmProtoAddInttab(indexList, last_boson, -index);
        list_count = 0;
        list_of_recs = (int*)realloc(list_of_recs, ++list_count * sizeof(int));
      }
      if (i != next) {
  ...
  if (other_break > 0) {
    /* create the space */
    int* list = _DtShmProtoAddIntLst(int_handle, other_break, &head->no_name_offset);
  ...
  space = _DtShmProtoAddIntLst(int_handle, size / sizeof(int), &head->name_list_offset);
...
static int write_db(DtDtsMMHeader* header, void* index, int size, const char* CacheFile) {
  ...
  if ((tmpfile = malloc(sizeof(_DTDTSMMTEMPDIR) + sizeof(_DTDTSMMTEMPFILE) + 7)) == NULL) {
  ...
  if (fd == -1) {
    _DtSimpleError(DtProgName, DtError, NULL, (char*)tmpfile, NULL);
    free(tmpfile);
    return (0);
  }
  ...
  if ((write(fd, header, sizeof(DtDtsMMHeader)) != sizeof(DtDtsMMHeader)) ||
  ...
  if (rename((const char*)tmpfile, CacheFile) == -1) {
    _DtSimpleError(DtProgName, DtError, NULL, (char*)CacheFile, NULL);
...
intptr_t _DtActionCompareRecordBoson(DtDtsMMRecord* record1, DtDtsMMRecord* record2) {
