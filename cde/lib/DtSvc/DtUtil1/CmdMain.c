...

{
  ...
  if (windowType == PERM_TERMINAL || windowType == TERMINAL) {
    ...
    snprintf(theCommand, theCommandLen, "%s -open %d -ttprocid '%s' %s %s",
             cmd_Resources.dtexecPath, 0 /* wait time zero for NO_STDIO */,
             _DtActNULL_GUARD(procId), _DtActNULL_GUARD(tmpFiles), execString);
  }
  ...
  if (!localExecution) {
    ...
    (void)strlcpy(context, netfile, MAXPATHLEN);
    tt_free(netfile);
    ...
    if (!_DtCmdCheckForExecutable(commandArray2[0])) {
      (void)snprintf(errorMessage, MAX_BUF_SIZE, errorExec, commandArray2[0]);
      Cmd_FreeAllocatedStringVector(commandArray);
      _DtCmdFreeStringVector(commandArray2);
      return (_CMD_EXECUTE_FAILURE);
    }
    ...
    for (index1 = 0; (index1 < 10) && ((commandPid = fork()) < 0); index1++) {
      /* Out of resources ? */
      if (errno != EAGAIN)
        break;
