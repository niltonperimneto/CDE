...
char* _DtHostString(const char* s) {
  char* slashp;
  char* host;
  char buf[MAXPATHLEN + MAXHOSTNAMELEN + 1];
  char* p = buf;
...
char* _DtGetSessionHostName(void) {
...
char* _DtGetDisplayHostName(Display* dp) {
  ...
  if (dp) {
    ...
    strlcpy(tmpName, DisplayString(dp), MAXHOSTNAMELEN + 5 + 1);
    ...
    strlcpy(tmpName, getenv("DISPLAY"), MAXHOSTNAMELEN + 5 + 1);
    if ((tmp = DtStrrchr(tmpName, ':'))) {
      *tmp = '\0';
      displayHostName = XtNewString(tmpName);
    }
   ...
   */
  if (!(displayHostName && *displayHostName && strcmp(displayHostName, "local") &&
...
int _DtIsSameHost(const char* host1, const char* host2) {
  char hostName1[MAXHOSTNAMELEN + 1];
  char hostName2[MAXHOSTNAMELEN + 1];
  struct hostent* host_ret;
  /* _Xgethostbynameparams host_buf; -- Unused */
  char* tp;
  ...
  if (!host1) {
    ...
    strlcpy(hostName1, tp, sizeof(hostName1));
    XtFree(tp);
  } else {
    strlcpy(hostName1, host1, sizeof(hostName1));
  }

  if (!host2) {
    tp = _DtGetLocalHostName();
    strlcpy(hostName2, tp, sizeof(hostName2));
    XtFree(tp);
  } else {
    strlcpy(hostName2, host2, sizeof(hostName2));
  }
   ...
   */
  strlcpy(hostName1, host_ret->h_name, sizeof(hostName1));
  if ((tp = DtStrchr(hostName1, '.')) != NULL)
    *tp = '\0';
...
char* _DtGetActionIconDefault(void) {
  ...
  bytesNeeded = strlen(DtACTION_ICON_RESOURCE_NAME) + strlen(_DtApplicationName) + 4;
  size_t nameBufSize;
  if (bytesNeeded > _DtAct_MAX_BUF_SIZE) {
    name = XtMalloc(bytesNeeded);
    nameBufSize = bytesNeeded;
  } else {
    name = nameBuf;
    nameBufSize = _DtAct_MAX_BUF_SIZE;
  }

  snprintf(name, nameBufSize, "%s*%s", _DtActNULL_GUARD(_DtApplicationName),
           DtACTION_ICON_RESOURCE_NAME);

  bytesNeeded = strlen(DtACTION_ICON_RESOURCE_CLASS) + strlen(_DtApplicationClass) + 4;
  size_t classBufSize;
  if (bytesNeeded > _DtAct_MAX_BUF_SIZE) {
    class = XtMalloc(bytesNeeded);
    classBufSize = bytesNeeded;
  } else {
    class = classBuf;
    classBufSize = _DtAct_MAX_BUF_SIZE;
  }
  snprintf(class, classBufSize, "%s*%s", _DtActNULL_GUARD(_DtApplicationClass),
           DtACTION_ICON_RESOURCE_CLASS);
...
char* _DtGetExecHostsDefault(void) {
  ...
  snprintf(name, nameBufSize, "%s*%s", _DtActNULL_GUARD(_DtApplicationName), DtEXEC_HOSTS_NAME);
  ...
  snprintf(class, classBufSize, "%s*%s", _DtActNULL_GUARD(_DtApplicationClass), DtEXEC_HOSTS_CLASS);
...
char* _DtGetDtTmpDir(void) {
  ...
  bytesNeeded = strlen(DtACTION_DTTMPDIR_RESOURCE_NAME) + strlen(_DtApplicationName) + 4;
  size_t nameBufSize;
  if (bytesNeeded > _DtAct_MAX_BUF_SIZE) {
    name = XtMalloc(bytesNeeded);
    nameBufSize = bytesNeeded;
  ...
  snprintf(name, nameBufSize, "%s*%s", _DtActNULL_GUARD(_DtApplicationName),
           DtACTION_DTTMPDIR_RESOURCE_NAME);

  bytesNeeded = strlen(DtACTION_DTTMPDIR_RESOURCE_CLASS) + strlen(_DtApplicationClass) + 4;
  size_t classBufSize;
  if (bytesNeeded > _DtAct_MAX_BUF_SIZE) {
    class = XtMalloc(bytesNeeded);
    classBufSize = bytesNeeded;
  ...
  snprintf(class, classBufSize, "%s*%s", _DtActNULL_GUARD(_DtApplicationClass),
           DtACTION_DTTMPDIR_RESOURCE_CLASS);
...
char* _DtActGenerateTmpFile(char* dir, char* format, mode_t mode, int* fd) {
  int pid;
  ...
  do {
    _DtSvcProcessLock();
    snprintf(nameBuf, sizeof(nameBuf), "%lx_%d", nameCount++, pid);
    _DtSvcProcessUnlock();
    base = XtNewString(nameBuf);
    /*
     * Convert the base name to the desired format
     */
    snprintf(nameBuf, sizeof(nameBuf), f, base);
    XtFree(base);
    ...
    if (*DtPrevChar(d, d + strlen(d)) != '/')
      snprintf(nameBuf, sizeof(nameBuf), "%s/%s", d, base);
    else
      snprintf(nameBuf, sizeof(nameBuf), "%s%s", d, base);
 ...
 ******************************************************************************/
void _DtRemoveTrailingBlanksInPlace(char** s) {
  char* p;

  if (!s || !strlen(*s))
    return;

  for (p = DtPrevChar(*s, *s + strlen(*s)); DtIsspace(p) && (p > *s); p = DtPrevChar(*s, p))
    *p = '\0';
}
...
int _DtExecuteAccess(const char* path) {
  int i;
  uid_t euid;
  struct stat s;
  gid_t* pgid;
...
_DtActInvRecT* _DtActAllocInvRec(void) {
  ...
  if (first_time) {
    ...
    _DtActInvRecArray = (_DtActInvRecT**)XtMalloc(sizeof(_DtActInvRecT*) * _actInvRecArraySize);

    /*
     * A NULL indicates an available slot.
     */
    for (i = 0; i < _actInvRecArraySize; i++) _DtActInvRecArray[i] = NULL;
  ...
  if (newslot == -1) {
    ...
    _DtActInvRecArray = (_DtActInvRecT**)XtRealloc((char*)_DtActInvRecArray,
                                                   sizeof(_DtActInvRecT*) * _actInvRecArraySize);

    /*
     * NULL out new entries
     */
    for (i = _actInvRecArraySize - 10; i < _actInvRecArraySize; i++) _DtActInvRecArray[i] = NULL;
   ...
   */
  _DtActInvRecArray[newslot] = (_DtActInvRecT*)XtCalloc(1, sizeof(_DtActInvRecT));
...
static void _DtActFreeChildRec(_DtActChildRecT* childRecP) {
  CallbackData* data;
  ...
  if (IS_CMD(childRecP->mask)) {
    ...
    if (childRecP->u.cmd.reqMessage) {
      data = (CallbackData*)tt_message_user(childRecP->u.cmd.reqMessage, 0);
    ...
    if (childRecP->u.tt.reqMessage) {
      data = (CallbackData*)tt_message_user(childRecP->u.tt.reqMessage, 0);
...
_DtActChildRecT* _DtActAllocChildRec(_DtActInvRecT* invRec) {
  ...
  if (invRec) {
    ...
    if (invRec->numChildren == 1) {
       ...
       */
      invRec->childRec = (_DtActChildRecT**)XtMalloc(sizeof(_DtActChildRecT*));
    } else {
      invRec->childRec = (_DtActChildRecT**)XtRealloc(
          (char*)invRec->childRec, sizeof(_DtActChildRecT*) * invRec->numChildren);
    }

    /*
     * Hang a child rec off and initialize
     */
    invRec->childRec[invRec->numChildren - 1] = (_DtActChildRecT*)XtMalloc(sizeof(_DtActChildRecT));
 ...
 */
_DtActChildRecT* _DtActFindChildRec(DtActionInvocationID id, unsigned long childId) {
  int i;
...
static void _DtActFreeInvRec(_DtActInvRecT* invp) {
  int i;
  ...
  if (invp->cachedUploadCnt) {
    for (i = 0; i < invp->cachedUploadCnt; i++) {
      _DtActFreeArgArray(invp->cachedUploads[i].newArgp, invp->cachedUploads[i].newArgc);
    }
 ...
 */
static void _DtActCacheArgs(_DtActInvRecT* invRecP, DtActionArg* newArgp, int newArgc,
  ...
  if (invRecP->cachedUploadCnt)
    invRecP->cachedUploads = (_DtActUpdateCache*)XtRealloc(
        (char*)invRecP->cachedUploads, (invRecP->cachedUploadCnt + 1) * sizeof(_DtActUpdateCache));
  else
    invRecP->cachedUploads = (_DtActUpdateCache*)XtMalloc(sizeof(_DtActUpdateCache));
 ...
 */
void _DtActExecutionLeafNodeCleanup(DtActionInvocationID id, DtActionArg* newArgp, int newArgc,
  ...
  if (IS_INV_FINISHED(invRecP->state)) {
    ...
    if (CALL_INV_CB(invRecP->state)) {
      if (invRecP->cb) {
        (invRecP->cb)(id, invRecP->client_data, (DtActionArg*)NULL, 0, DtACTION_INVOKED);
      }
  ...
  if (flushCache) {
    for (i = 0; i < invRecP->cachedUploadCnt; i++) {
      if (invRecP->cb) {
        (invRecP->cb)(id, invRecP->client_data, invRecP->cachedUploads[i].newArgp,
                      invRecP->cachedUploads[i].newArgc, invRecP->cachedUploads[i].userStatus);
      } else {
        /*
         * We normally won't have cached data if no cb.
         */
        _DtActFreeArgArray(invRecP->cachedUploads[i].newArgp, invRecP->cachedUploads[i].newArgc);
      }
    }
  ...
  if (userStatus == DtACTION_STATUS_UPDATE) {
    ...
    if ((newArgc) && (invRecP->cb)) {
      if (useCache) {
        ...
        (invRecP->cb)(id, invRecP->client_data, newArgp, newArgc, userStatus);
      }
    }
  } else {
    /*
...
void* _DtActReadTmpFileToBuffer(char* fname, int* sizep) {
  /*
  ...
  for (size = 0, space = MAX_BUF_SIZE; (bytes = read(fd, buf + size, space)) != 0; size += bytes) {
    if (bytes < 0) {
...
int _DtActGetCmdReturnArgs(DtActionInvocationID invId, _DtActChildRecT* childp,
                           DtActionArg** aargv) {
  int i;
  _DtActInvRecT* invp;
  DtActionArg* newArgp;
    ...
    if (IS_BUFFER_OBJ(infop->mask)) {
      if (IS_FILE_OBJ(infop->mask)) {
        ...
        newArgp[Idx].u.buffer.bp =
            _DtActReadTmpFileToBuffer(infop->name, &newArgp[Idx].u.buffer.size);
      } else {
        /*
...
int _DtCmdCheckQForId(DtActionInvocationID id) {
  extern Cmd_RequestQueue* _DtCmdGetQueueHead(void);
  Cmd_RequestQueue* pNode;
  CallbackData* dp;

  for (pNode = _DtCmdGetQueueHead(); pNode; pNode = pNode->next) {
