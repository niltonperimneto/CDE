...
static void _DtActTimerCB(XtPointer clientData, XtIntervalId IntId) {
  _DtActExecutionLeafNodeCleanup((unsigned long)clientData, NULL, 0, True);
}
...
void _DtCreateErrorDialog(Widget w, String actionName, XmString msg)

{
  ...
  XtAddCallback(dialog, XmNokCallback, FreeErrorDialog, (XtPointer)NULL);
  ...
  if (!is_mapped) {
    ...
    XtSetArg(args[0], XmNx, (WidthOfScreen(XtScreen(dialog)) - dialogWd) / 2U);
    XtSetArg(args[1], XmNy, (HeightOfScreen(XtScreen(dialog)) - dialogHt) / 2U);
...
static void InvalidFilename(Widget w, String actionName, String filename)
...
static void HostAccessError(Widget w, String actionName, String hostName)
...
static void MultiHostAccessError(Widget w, String actionName, String hostList)
...
static void CommandInvokerError(Widget w, String actionName, String errorString)
...
static _DtActInvRecT *CreateInvocationRecord(char *actionName, Widget w,
  ...
  for (i = 0; i < aac; i++) {
    if (aap[i].argClass == DtACTION_BUFFER) {
      ...
      if (is_executable && DtDtsIsTrue(is_executable))
  ...
  ********************************************************************/
  ...
  if (typeFile) {
    if (filetype && *filetype)
      ...
      TryToTypeFile(objectData, request->hostNames[hostId], dirName,
                    objectData->u.file.baseFilename, &resolvedPath);
  ...
  if (IS_CMD(action->mask)) {
    ...
    if (childrecp) {
      ...
      for (i = 0; i < action->u.tt_msg.value_count; i++) {
        ...
        if (action->u.tt_msg.tt_argn_value[i].parsedMessage[0].keyword != ARG)
          continue;
        ...
        argNum = action->u.tt_msg.tt_argn_value[i].parsedMessage[0].argNum;

        if ((argNum > 0) && (argNum <= invp->ac)) {
          /* The ith message part must be returned */
          childrecp->argMap[i].argIdx = argNum + request->objOffset - 1;
      ...
      if (action->u.tt_msg.tt_file.numMsgParts &&
        ...
        argNum = action->u.tt_msg.tt_file.parsedMessage[0].argNum;
        if ((argNum > 0) && (argNum <= invp->ac)) {
...
void _DtFreeActionStruct(ActionPtr action)
    ...
    if (IS_FILE_OBJ(object->mask)) {
      if (processingMask & _DTAct_TT_VTYPE) {
        ...
        if (object->type != (-1)) {
          ...
          mediaAttr = DtDtsDataTypeToAttributeValue(dataType, "MEDIA", path);
          XtFree(path);
...

{
  ...
  n++;
  XtSetArg(args[n], XmNrightOffset, 20);
  n++;
  XtSetArg(args[n], XmNtopOffset, 15);
  n++;
  XtSetArg(args[n], XmNlabelString, pt1);
  n++;
  ...
  for (count = 0; count < numPrompts; count++) {
    ...
    n++;
    XtSetArg(args[n], XmNtopWidget, topAttach);
    n++;
  ...
  n++;
  XtSetArg(args[n], XmNleftPosition, 5 + 10);
  n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
  n++;
  XtSetArg(args[n], XmNrightPosition, 31 + 10);
  n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, separator);
  n++;
  XtSetArg(args[n], XmNtopOffset, 16);
  n++;
  XtSetArg(args[n], XmNbottomOffset, 16);
  n++;
  ...
  if (numPrompts <= 1) {
    n = 0;
    XtSetArg(args[n], XmNdefaultButton, ok);
    n++;
    ...
    for (i = 0; i < numPrompts; i++) {
      if (i <= (numPrompts - 2))
        XtAddCallback(promptDes[i].promptWidget, XmNactivateCallback,
                      (XtCallbackProc)ChangePromptTraversal, (XtPointer)dialog);
      else
        XtAddCallback(promptDes[i].promptWidget, XmNactivateCallback,
                      (XtCallbackProc)ProcessPromptDialog, (XtPointer)dialog);
    }
  }
  ...
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
  n++;
  XtSetArg(args[n], XmNleftPosition, 37 + 22);
  n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
  n++;
  XtSetArg(args[n], XmNrightPosition, 63 + 22);
  n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNtopWidget, separator);
  n++;
  XtSetArg(args[n], XmNtopOffset, 21);
  n++;
  XtSetArg(args[n], XmNbottomOffset, 21);
  n++;
  XtSetArg(args[n], XmNmarginHeight, 4);
  n++;
  XtSetArg(args[n], XmNlabelString, labelString);
  ...
  for (i = 0; i < numPrompts; i++) {
    n = 0;
    XtSetArg(args[n], XmNuserData, cancel);
    n++;
  ...
  if (!is_mapped) {
    ...
    XtSetArg(args[0], XmNx, (WidthOfScreen(XtScreen(bboard)) - dialogWd) / 2U);
    XtSetArg(args[1], XmNy, (HeightOfScreen(XtScreen(bboard)) - dialogHt) / 2U);
...

{
  ...
  XtSetArg(args[n], XmNokLabelString, ok);
  n++;
  XtSetArg(args[n], XmNcancelLabelString, cancel);
  n++;
...

{
  ...
  if (request->termOpts) {
    ...
    strlcat(termOpts, request->termOpts, termOptsLen);
    XtFree(cmd->termOpts.compiledMessage);
...

{
   ...
   */
  snprintf(procIdBuf, sizeof(procIdBuf), "%s_%d_%lu", _DtActNULL_GUARD(procId),
           (int)request->invocId, request->childId);
    ...
    if ((childp = _DtActFindChildRec(request->invocId, request->childId)) !=
      ...
      for (i = 0; i < childp->numObjects; i++) {
        ...
        if ((len += strlen(" -tmp ") + strlen(p)) < sizeof(tmpFileBuf)) {
           ...
           */
          strlcat(tmpFileBuf, " -tmp ", sizeof(tmpFileBuf));
          strlcat(tmpFileBuf, p, sizeof(tmpFileBuf));
        } else {
          /*
          ...
          strlcpy(tmpFiles, tmpFileBuf, len + 1);
          strlcat(tmpFiles, " -tmp ", len + 1);
          strlcat(tmpFiles, p, len + 1);
        }
      }
