...
void _DtProcessTtRequest(Widget w, ActionRequest* request, char* relPathHost, char* relPathDir);

Tt_status _DtInitializeToolTalk(Widget w);

extern void _DtCreateErrorDialog(Widget w, char* actionName, XmString msg);
extern Boolean _DtCompileMessagePiece(Widget w, ActionRequest* request, char* relPathHost,
                                      char* relPathDir, parsedMsg* piece, Boolean initialize,
                                      unsigned long processingMask, Boolean** paramUsed,
                                      int* promptDataIndex);
extern ActionRequest* _DtCloneRequest(ActionRequest* request);
extern void _DtFreeRequest(ActionRequest* request);
/********    End Public Function Declarations    ********/

/********    Static Function Declarations    ********/

static Boolean ResolveTtRequestMessagePieces(Widget w, ActionRequest* request, char* relPathHost,
                                             char* relPathDir);

static void InitiateTtRequest(Widget w, ActionRequest* request);

static Tt_callback_action TtRequestCallbackHandler(Tt_message message, Tt_pattern pattern);

static char* WrapMessageLines(char* errorMsg);

static void ReportToolTalkError(Widget w, String actionName, String errorMsg);

static Tt_callback_action _DtActReceiveExecId(Tt_message msg, Tt_pattern pat);
static Tt_callback_action _DtActReceiveDoneMsg(Tt_message msg, Tt_pattern pat);
static Tt_message _DtTtContractIgnoreMsgCB(Tt_message msg, void* clientdata, Tt_message contract);
...
{
  /* ActionPtr action = request->clonedAction; -- Unused */
...

{
  ...
  if (_DtCompileMessagePiece(w, request, relPathHost, relPathDir, &(tt->tt_op), True, 0, &paramUsed,
                             &promptDataIndex) &&
      _DtCompileMessagePiece(w, request, relPathHost, relPathDir, &(tt->tt_file), False, 0,
                             &paramUsed, &promptDataIndex)) {
    for (i = 0, success = True; (i < tt->value_count) && success; i++) {
      success = _DtCompileMessagePiece(w, request, relPathHost, relPathDir, &(tt->tt_argn_value[i]),
                                       False, _DTAct_TT_ARG, &paramUsed, &promptDataIndex);
    }
    for (i = 0; (i < tt->vtype_count) && success; i++) {
      success = _DtCompileMessagePiece(w, request, relPathHost, relPathDir, &(tt->tt_argn_vtype[i]),
                                       False, _DTAct_TT_VTYPE, &paramUsed, &promptDataIndex);
    }

    if (success) {
      /* We must have at least the op strings */
      if ((tt->tt_op.compiledMessage) && (strlen(tt->tt_op.compiledMessage) > 0)) {
        XtFree(paramUsed);
        return (True);
      }
    }
 ...
 */
Tt_message Ttmedia_to_Dt_StatusUpdateCB(Tt_message message, void* clientdata, Tttk_op op,
  ...
  int newArgc = 0;
  /* unsigned long evalStatus; -- Unused */
  /* DtActionStatus userStatus; -- Unused */
  int i, upIdx;
  char* upVType;
  ...
  if (state == TT_FAILED) {
    if (status != TT_DESKTOP_ECANCELED) {
      ...
      if ((tt_pointer_error(errorMsg) == TT_OK) && errorMsg) {
        ...
        ReportToolTalkError(data->associatedWidget, data->actionLabel, errorMsg);
        if (wrapMessageLines)
          tt_free(errorMsg);
      } else {
        ReportToolTalkError(data->associatedWidget, data->actionLabel, NULL);
      }
    }
    ...
    if (file == NULL) {
      ...
      if (upIdx != -1) {
        if ((IS_BUFFER_OBJ(invRec->info[upIdx].mask) && IS_WRITE_OBJ(invRec->info[upIdx].mask))) {
          /*
    ...
    for (i = 0; i < totalArgs; i++) {
      ...
      if (upIdx != -1) {
        if ((IS_BUFFER_OBJ(invRec->info[upIdx].mask) && IS_WRITE_OBJ(invRec->info[upIdx].mask)) ||
           ...
           */
          if ((argRepType == DtACT_TT_REP_BUFFER) && !(upArgClass == DtACTION_BUFFER))
          ...
          switch (argRepType) {
            ...
            case DtACT_TT_REP_STRING:
               ...
               * fetched using bval.
               */
              status = tt_message_arg_bval(message, i, (unsigned char**)&upttbuf, &upttbuflen);
            ...
            case DtACT_TT_REP_INT:
               ...
               * Unpack an integer.
               */
              status = tt_message_arg_ival(message, i, &ivalue);
              if (status != TT_OK) {
                upArgClass = DtACTION_NULLARG;
              } else {
                /*
          ...
          switch (upArgClass) {
            ...
            case DtACTION_BUFFER:
              ...
              newArgp[upIdx].u.buffer.bp = (void*)XtMalloc(upttbuflen);
              memcpy(newArgp[upIdx].u.buffer.bp, upttbuf, upttbuflen);
            ...
            case DtACTION_FILE:
               ...
               */
              newArgp[upIdx].argClass = upArgClass;
              newArgp[upIdx].u.file.name = XtNewString(upttbuf);
...

{
  ...
  if ((tt->tt_class == TT_REQUEST) && (tt->tt_scope == TT_SESSION) &&
     ...
     */
    if (tt->mode_count == 2) {
      if ((tt->tt_argn_mode[1] != TT_IN) || (tt->tt_argn_rep_type[1] != DtACT_TT_REP_STRING) ||
  ...
  if (ttmedia_test) {
    ...
    if (tt->tt_argn_rep_type[0] == DtACT_TT_REP_BUFFER ||
      ...
      if (actChildRecP->argMap[0].argIdx != -1) {
        if (IS_BUFFER_OBJ(actInvRecP->info[actChildRecP->argMap[0].argIdx].mask)) {
          length = actInvRecP->info[actChildRecP->argMap[0].argIdx].size;
          bufFilename = actInvRecP->info[actChildRecP->argMap[0].argIdx].name;
        }
      }
    }

    if (bufFilename) {
      contents = (unsigned char*)_DtActReadTmpFileToBuffer(bufFilename, &length);
      /* length plucked from above */
    } else {
      contents = (unsigned char*)tt->tt_argn_value[0].compiledMessage;
     ...
     */
    message =
        ttmedia_load((Tt_message)0, /* Am top level req */
                     Ttmedia_to_Dt_StatusUpdateCB, (void*)data, op,
                     tt->tt_argn_vtype[0].compiledMessage, contents, length, file, docname, FALSE);
    ...
    if (!tt_is_err(status)) {
       ...
       * and callback data.
       */
      subcon_patterns = ttdt_subcontract_manage(message, (Ttdt_contract_cb)_DtTtContractIgnoreMsgCB,
                                                w, (void*)NULL);
     ...
     */
    message = tttk_message_create(0, tt->tt_class, tt->tt_scope, 0, tt->tt_op.compiledMessage,
    ...
    for (i = 0; i < tt->mode_count; i++) {
      ...
      if (actChildRecP->argMap[i].argIdx != -1) {
        if (IS_BUFFER_OBJ(actInvRecP->info[actChildRecP->argMap[i].argIdx].mask)) {
          length = actInvRecP->info[actChildRecP->argMap[i].argIdx].size;
          bufFilename = actInvRecP->info[actChildRecP->argMap[i].argIdx].name;
        }
      }
       ...
       * Determine REP TYPE
       */
      if ((i < tt->rep_type_count) && (tt->tt_argn_rep_type[i] != DtACT_TT_REP_UNDEFINED)) {
      ...
      if (repType == DtACT_TT_REP_INT) {
        ...
        tt_message_barg_add(message, mode, vtype, (unsigned char*)value, length);
      }
...
static char* WrapMessageLines(char* errorMsg) { return errorMsg; }
