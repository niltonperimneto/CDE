...
DtDtsMMRecord* _DtActionLocateRecord(DtShmBoson actQuark, unsigned long mask, DtShmBoson arg_type,
                                     int arg_count, DtDtsMMDatabase* act_db);

/********    End Private Function Declarations    ********/

/********    Static Function Declarations    ********/

static int _DtActMMCompareObjClassMask(unsigned long objClassMask, char* actClass);
static int _DtActMMCompareMode(unsigned long objMask, char* actMode);
static int _DtActMMCompareType(DtShmBoson reqType, char* actType);
static int _DtActMMCompareCount(int reqCount, char* actCount);
static int _DtActionCompareRecordQuarks(DtDtsDbRecord** record1, DtDtsDbRecord** record2);
static void _DtActionGetRecordWeight(DtDtsDbRecord** record, long* primaryWeightPtr,
                                     long* secondaryWeightPtr);
static void ParseMessage(ActionPtr action, char* str, parsedMsg* msg);
static Boolean ValidKeyword(char* start, char* end, char** prompt, int* keywordId, int* argNum,
                            Boolean* isFile);

static unsigned long _DtActMMParseArgTypesList(char* tlist, DtShmBoson** quarkAddr, int* countp);
static unsigned long _DtActMMParseClassList(char* clist);
static unsigned long _DtActMMParseArgCountString(char* countStr, int* countp);
static unsigned long _DtActMMParseActionType(char* actionTypeStr);
static void _DtActMMGetCmdInfo(DtDtsMMRecord* actRecp, ActionPtr actp);
static void _DtActMMGetTtMsgInfo(DtDtsMMRecord* actRecp, ActionPtr actp);
/* static int _DtActInputSeqCheck(DtDtsDbRecord* r1, DtDtsDbRecord* r2); -- Unused */
static char* _DtActDbChooseLabel(DtDtsDbRecord* rec);
static char* _DtActMMChooseLabel(DtDtsMMRecord* rec);
static void _DtCheckForDuplicateRecord(DtDtsDbRecord* rec, DtDtsDbRecord* duprec);
static char* _DtActGetDtsDbField(DtDtsDbRecord* recp, char* fieldName);
static char* _DtActGetDtsMMField(DtDtsMMRecord* actRecp, char* name);
static void _DtSearchDuplicateRecord(DtDtsDbDatabase* db);

#ifdef _DT_ALLOW_DT_MSGS
static void _DtActDbGetDtNtfyInfo(DtDtsDbRecord* actRecp, ActionPtr actp);
static void _DtActDbGetDtReqInfo(DtDtsDbRecord* actRecp, ActionPtr actp);
...
char* DtActionDescription(char* s) {
  char* actionDesc;

  _DtSvcAppLockDefault();
  actionDesc = (use_in_memory_db) ? _DtActionDbDescription(s) : _DtActionMMDescription(s);
...
static char* _DtActionDbDescription(char* s) {
  ...
  for (act_rec = act_db->recordList; n && act_rec && *act_rec; act_rec++, n--) {
  ...
  if (last_rec_found) {
    _DtSvcProcessUnlock();
    return XtNewString(_DtActGetDtsDbField(*last_rec_found, _DtACTION_DESCRIPTION));
  }
...
static char* _DtActionMMDescription(char* s) {
  ...
  if (last_rec_found) {
    _DtSvcProcessUnlock();
    return _DtActGetDtsMMField(last_rec_found, _DtACTION_DESCRIPTION);
  }
  _DtSvcProcessUnlock();
  return NULL;
}
...
Boolean DtActionExists(char* s) {
  ...
  _DtSvcAppLockDefault();
  actionExists = (use_in_memory_db) ? _DtActionDbExists(s) : _DtActionMMExists(s);
  _DtSvcAppUnlockDefault();
...
char* DtActionLabel(char* s) {
  ...
  _DtSvcAppLockDefault();
  actionLabel = (use_in_memory_db) ? _DtActionDbLabel(s) : _DtActionMMLabel(s);
  _DtSvcAppUnlockDefault();
...
char* DtActionIcon(char* s) {
  char* actionIcon;

  _DtSvcAppLockDefault();
  actionIcon = (use_in_memory_db) ? _DtActionDbIcon(s) : _DtActionMMIcon(s);
  _DtSvcAppUnlockDefault();
...
static char* _DtActionDbIcon(char* s) {
  ...
  if (last_rec_found) {
     ...
     */
    if ((iconString = _DtActGetDtsDbField(*last_rec_found, _DtACTION_ICON)) != NULL) {
      _DtSvcProcessUnlock();
...
static char* _DtActionMMIcon(char* s) {
  ...
  if (last_rec_found) {
     ...
     * Obtain the associated icon name (if any).
     */
    if ((iconString = _DtActGetDtsMMField(last_rec_found, _DtACTION_ICON)) != NULL) {
      _DtSvcProcessUnlock();
      return iconString;
    }
...
static char* _DtActGetDtsDbField(DtDtsDbRecord* actRecp, char* name) {
...
static char* _DtActDbChooseLabel(DtDtsDbRecord* rec) {
  ...
  if ((label = _DtActGetDtsDbField(rec, _DtACTION_LABEL)) != NULL)
...
static void _DtActMMGetCmdInfo(DtDtsMMRecord* actRecp, ActionPtr actp) {
  ...
  strlcpy(tmp, titleStr, amtToAlloc);
  strlcat(tmp, actp->label, amtToAlloc);
  strlcat(tmp, "\"", amtToAlloc);
  if (s) {
    strlcat(tmp, " ", amtToAlloc);
    strlcat(tmp, s, amtToAlloc);
  }
  _DtDtsMMSafeFree(s);
...
static void _DtActMMGetTtMsgInfo(DtDtsMMRecord* actRecp, ActionPtr actp) {
  ...
  for (i = 0; True; i++) {
    snprintf(argNname, sizeof(argNname), "%s%d%s", _DtACTION_TTN_ARG, i, _DtACTION_TTN_MODE);
    if (!(s = _DtActGetDtsMMField(actRecp, argNname)))
      break; /* no more args */
    actp->u.tt_msg.tt_argn_mode =
        (int*)XtRealloc((char*)actp->u.tt_msg.tt_argn_mode, sizeof(int) * (i + 1));
    ...
    snprintf(argNname, sizeof(argNname), "%s%d%s", _DtACTION_TTN_ARG, i, _DtACTION_TTN_VTYPE);
    actp->u.tt_msg.tt_argn_vtype =
        (parsedMsg*)XtRealloc((char*)actp->u.tt_msg.tt_argn_vtype, sizeof(parsedMsg) * (i + 1));
    _DtDtsMMSafeFree(s);
    s = (char*)0;
    ...
    snprintf(argNname, sizeof(argNname), "%s%d%s", _DtACTION_TTN_ARG, i, _DtACTION_TTN_VALUE);
    actp->u.tt_msg.tt_argn_value =
        (parsedMsg*)XtRealloc((char*)actp->u.tt_msg.tt_argn_value, sizeof(parsedMsg) * (i + 1));
    _DtDtsMMSafeFree(s);
    s = (char*)0;
    s = _DtActGetDtsMMField(actRecp, argNname);
    ...
    snprintf(argNname, sizeof(argNname), "%s%d%s", _DtACTION_TTN_ARG, i, _DtACTION_TTN_REP_TYPE);
    actp->u.tt_msg.tt_argn_rep_type =
        (int*)XtRealloc((char*)actp->u.tt_msg.tt_argn_rep_type, sizeof(int) * (i + 1));
    _DtDtsMMSafeFree(s);
    s = (char*)0;
    if (!(s = _DtActGetDtsMMField(actRecp, argNname)))
      ...
      actp->u.tt_msg.tt_argn_rep_type[i] = DtACT_TT_REP_INT;
    else if (!(strcmp(s, _DtACTION_TT_RTYP_STR)))
      actp->u.tt_msg.tt_argn_rep_type[i] = DtACT_TT_REP_STRING;
    else if (!(strcmp(s, _DtACTION_TT_RTYP_BUF)))
      actp->u.tt_msg.tt_argn_rep_type[i] = DtACT_TT_REP_BUFFER;
    else {
      myassert(0); /* should never reach here */
      actp->u.tt_msg.tt_argn_rep_type[i] = DtACT_TT_REP_UNDEFINED;
    }
   ...
   */
  actp->u.tt_msg.mode_count = actp->u.tt_msg.vtype_count = actp->u.tt_msg.rep_type_count =
      actp->u.tt_msg.value_count = i;
}
...
static void _DtActDbGetDtNtfyInfo(DtDtsDbRecord* actRecp, ActionPtr actp) {
  ...
  for (i = 0; True; i++) {
    snprintf(argNval, sizeof(argNval), "%s%d%s", _DtACTION_DTN_ARG, i, _DtACTION_DTN_VALUE);
    if (!(s = _DtActGetDtsDbField(actRecp, argNval)))
      break; /* no more args */
    actp->u.dt_notify.argn_value =
        (parsedMsg*)XtRealloc((char*)actp->u.dt_notify.argn_value, sizeof(parsedMsg) * (i + 1));
...
static void _DtActDbGetDtReqInfo(DtDtsDbRecord* actRecp, ActionPtr actp) {
  ...
  for (i = 0; True; i++) {
    ...
    actp->u.dt_req.argn_value =
        (parsedMsg*)XtRealloc((char*)actp->u.dt_req.argn_value, sizeof(parsedMsg) * (i + 1));
...
static int _DtActMMCompareObjClassMask(unsigned long objClassMask, char* actClass) {
  ...
  if (!DtStrchr(actClass, _DtACT_LIST_SEPARATOR_CHAR)) {
    ...
    if (!strcmp(actClass, _DtACTION_FILE))
       ...
       */
      if (IS_FILE_OBJ(objClassMask) && !(IS_BUFFER_OBJ(objClassMask)))
        return True;
   ...
   * Then vectorize it in place.
   */
  strlcpy(buf, actClass, sizeof(buf));
  cvp = classVec = _DtVectorizeInPlace(buf, _DtACT_LIST_SEPARATOR_CHAR);
...
static int _DtActMMCompareMode(unsigned long objMask, char* actMode) {
  if (!actMode || !strcmp(actMode, _DtACT_ARG_MODE_ANY) || !strcmp(actMode, DtDTS_DT_UNKNOWN))
    return True; /* We don't care about the mode */

  if (!strcmp(actMode, _DtACT_ARG_MODE_WRITE) && IS_WRITE_OBJ(objMask))
    return True;

  if (!strcmp(actMode, _DtACT_ARG_MODE_NOWRITE) && !(IS_WRITE_OBJ(objMask)))
    return True;
...
static int _DtActMMCompareType(DtShmBoson reqType, char* actType) {
   ...
   * Then vectorize it in place.
   */
  strlcpy(buf, actType, sizeof(buf));
  tvp = typeVec = _DtVectorizeInPlace(buf, _DtACT_LIST_SEPARATOR_CHAR);
...
DtDtsMMRecord* _DtActionLocateRecord(DtShmBoson actQuark, unsigned long obj_mask,
                                     DtShmBoson arg_type, int arg_count, DtDtsMMDatabase* act_db) {
  int n;
  DtDtsMMRecord* act_rec;
  DtDtsMMRecord* act_rec_list;
  /* DtDtsMMRecord* last_rec_found = NULL; -- Unused */
  char* tmp = 0;
  int* start;
  ...
  for (n = *start; n < act_db->recordCount; n++) {
    ...
    _DtDtsMMSafeFree(tmp);
    if (!_DtActMMCompareType(arg_type, tmp = _DtActGetDtsMMField(act_rec, _DtACTION_ARG_TYPE)))
      continue;

    _DtDtsMMSafeFree(tmp);
    if (!_DtActMMCompareCount(arg_count, tmp = _DtActGetDtsMMField(act_rec, _DtACTION_ARG_COUNT)))
      continue;

    _DtDtsMMSafeFree(tmp);
    if (!_DtActMMCompareMode(obj_mask, tmp = _DtActGetDtsMMField(act_rec, _DtACTION_ARG_MODE)))
      continue;
    _DtDtsMMSafeFree(tmp);
    /*
     * We've found a match
...

{
  ...
  *argNum = NO_ARG;
  ...
  if ((len >= localHostLen) && (strncmp(_DtACT_LOCALHOST_STR, start, (size_t)localHostLen) == 0)) {
    *isFile = False;
  ...
  } else if ((len >= argsLen) && (strncmp(_DtACT_ARGS_STR, start, (size_t)argsLen) == 0)) {
    /* Flag that a keyword was found */
    *keywordId = ARG;
    *argNum = ALL_ARGS;
    /*
    ...
    start += argsLen;
  } else if ((len >= arg_Len) && (strncmp(_DtACT_ARG_UNDER_STR, start, (size_t)arg_Len) == 0)) {
    /* Flag that a keyword was found */
    *keywordId = ARG;
    /*
     * Args keyword should default to File unless
   ...
   */
  /* savedChar = *DtNextChar(end); -- Unused */
...
ActionPtr _DtActionFindDBEntry(ActionRequest* reqp, DtShmBoson actQuark) {
  int numSearches = 0;
  ...
  for (actRecp = _DtActionLocateRecord(actQuark, mask, type, num, act_db); actRecp;
       actRecp = _DtActionLocateRecord(_DtDtsMMStringToBoson(mapto), mask, type, num, act_db),
      numSearches++) {
    /*
  ...
  if (actRecp) {
    ...
    actp->file_name_id = XrmStringToQuark(_DtDtsMMBosonToString(actRecp->pathId));
    if (origLabel)
      actp->label = origLabel;
    else
      actp->label = _DtActMMChooseLabel(actRecp);
    actp->description = _DtActGetDtsMMField(actRecp, _DtACTION_DESCRIPTION);

    actp->mask |= _DtActMMParseClassList(tmp = _DtActGetDtsMMField(actRecp, _DtACTION_ARG_CLASS));
    _DtDtsMMSafeFree(tmp);

    actp->mask |= _DtActMMParseArgTypesList(tmp = _DtActGetDtsMMField(actRecp, _DtACTION_ARG_TYPE),
                                            &actp->arg_types, &actp->type_count);
    _DtDtsMMSafeFree(tmp);

    actp->mask |= _DtActMMParseArgCountString(
        tmp = _DtActGetDtsMMField(actRecp, _DtACTION_ARG_COUNT), &actp->arg_count);
    _DtDtsMMSafeFree(tmp);

    actp->mask |= _DtActMMParseActionType(tmp = _DtActGetDtsMMField(actRecp, _DtACTION_TYPE));
    _DtDtsMMSafeFree(tmp);
...

{
  ...
  while (1) {
    ...
    if ((keywordStart == NULL) || (keywordEnd == NULL)) {
      ...
      if (strlen(string) > 0) {
        msg->numMsgParts++;
        msg->parsedMessage = (MsgComponent*)XtRealloc(
            (char*)msg->parsedMessage, (Cardinal)(sizeof(MsgComponent) * msg->numMsgParts));
       ...
       */
      if (ValidKeyword(keywordStart + 1, keywordEnd - 1, &prompt, &keyword, &argNum, &isFile)) {
        msg->numMsgParts++;
        msg->parsedMessage = (MsgComponent*)XtRealloc(
            (char*)msg->parsedMessage, (Cardinal)(sizeof(MsgComponent) * msg->numMsgParts));
        ...
        if (keywordStart != string) {
          ...
          newBlock->precedingText = (char*)XtMalloc((Cardinal)(keywordStart - string + 1));
          (void)strncpy(newBlock->precedingText, string, (size_t)(keywordStart - string));
