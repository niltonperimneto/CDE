...
Boolean _DtStringsAreEqual(String string1, String string2) {
...
Boolean _DtEmptyString(String str) {
...
static _DtActNameAndBit _DtActNamesAndBits[] = {
    ...
    {
        _DtACTION_ARG_COUNT,
        _ActDb_ARG_COUNT_SET,
    },
    {
        _DtACTION_DESCRIPTION,
        _ActDb_DESCRIPTION_SET,
    },
    {
        _DtACTION_ICON,
        _ActDb_ICON_SET,
    ...
    *******************************************************************/
    {
        _DtACTION_MAP_ACTION,
        _ActDb_MAP_ACTION_SET,
    },
    {
        _DtACTION_EXEC_STRING,
        _ActDb_EXEC_STRING_SET,
    },
    {
        _DtACTION_EXEC_HOST,
        _ActDb_EXEC_HOST_SET,
    },
    {
        _DtACTION_CWD,
        _ActDb_CWD_SET,
...
#ifdef _DT_ALLOW_DT_MSGS
    {
        _DtACTION_DT_REQUEST_NAME,
        _ActDb_DT_REQ_NAME_SET,
    },
    {
        _DtACTION_DT_SVC,
        _ActDb_DT_SVC_SET,
    ...
    {
        _DtACTION_DT_NOTIFY_NAME,
        _ActDb_DT_NTFY_NAME_SET,
    },
    {
        _DtACTION_DT_NGROUP,
...
static unsigned long _DtActFieldBitLookup(char* name) {
  int j;
  char* np = name;
#ifdef _DT_ALLOW_DT_MSGS
  ...
  if (!strncmp("TT_ARG", np, sizeof("TT_ARG") - 1)) {
     ...
     * the table lookup.
     */
    np = strlcat(dtnamebuf, np, sizeof(dtnamebuf));
  }
#endif /* _DT_ALLOW_DT_MSGS */
 ...
 *
 ******************************************************************************/
static Boolean _DtActDupFieldNameCheck(DtDtsDbField** fields, int numFields, XrmQuark name) {
  int i;
 ...
 *
 ******************************************************************************/
static char* _DtActValidateFieldValue(long bit, char* value, char* actName, char* filename) {
  char* val;
  char* start;
  char* p = NULL;
  ...
  switch (bit) {
    case _ActDb_TYPE_SET:
      if (strcmp(_DtACTION_MAP, val) && strcmp(_DtACTION_COMMAND, val) &&
          strcmp(_DtACTION_TT_MSG, val)
#ifdef _DT_ALLOW_DT_MSGS
          && strcmp(_DtACTION_DT_REQUEST, val) && strcmp(_DtACTION_DT_NOTIFY, val)
#endif /* _DT_ALLOW_DT_MSGS */
      ) {
        /*
        ...
        (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, unrecogMsgType, actName, filename, val,
                       _DtACTION_TYPE);
    ...
    case _ActDb_ARG_CLASS_SET:
      ...
      while ((p = DtStrchr(start, _DtACT_LIST_SEPARATOR_CHAR))) {
         ...
         */
        *p = '\0';
        if (strcmp(_DtACT_ANY, start) && strcmp(_DtACTION_FILE, start) &&
            strcmp(_DtACTION_BUFFER, start)) {
          /*
           * Invalid field value -- issue error msg.
           */
          buf = malloc(_DtAct_MAX_BUF_SIZE);
          (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, unrecogMsgType, actName, filename, start,
                         _DtACTION_ARG_CLASS);
          _DtSimpleError(DtProgName, DtError, NULL, "%s", buf);
          free(val);
          free(buf);
    ...
    case _ActDb_ARG_MODE_SET:
      ...
      if (strcmp(val, _DtACT_ANY) && strcmp(val, _DtACT_ARG_MODE_WRITE) &&
    ...
    case _ActDb_ARG_COUNT_SET:
      ...
      for (p = val; *p; p = DtNextChar(p)) {
        if (p == val && ((*p == _DtACT_LT_CHAR) || (*p == _DtACT_GT_CHAR))) {
    ...
    case _ActDb_WINDOW_TYPE_SET:
      if (strcmp(_DtACTION_TERMINAL, val) && strcmp(_DtACTION_PERM_TERMINAL, val) &&
    ...
    case _ActDb_CWD_SET:
       ...
       * BY A ':'.
       */
      if (*val == '/' && !((mblen(val, MB_CUR_MAX) == 1) && DtIsspace(DtNextChar(val))))
      ...
      for (p = val; *p; p = DtNextChar(p)) {
         ...
         * followed by a slash.
         */
        if ((*p == ':') && (*(DtNextChar(p)) == '/') && (p != val))
          break; /* Accept this as good with hostname */

        if ((*p == '/') || ((mblen(p, MB_CUR_MAX) == 1) && DtIsspace(p))) {
          /* looks like an error */
          buf = malloc(_DtAct_MAX_BUF_SIZE);
          (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, unrecogMsgType, actName, filename, val,
                         _DtACTION_CWD);
    ...
    case _ActDb_TT_CLASS_SET:
      if (strcmp(_DtACTION_TT_NOTICE, val) && strcmp(_DtACTION_TT_REQUEST, val)) {
        /*
         * Invalid field value -- issue error msg.
         */
        buf = malloc(_DtAct_MAX_BUF_SIZE);
    ...
    case _ActDb_TT_SCOPE_SET:
      if (strcmp(_DtACTION_TT_SESSION, val) && strcmp(_DtACTION_TT_FILE, val) &&
          strcmp(_DtACTION_TT_BOTH, val) && strcmp(_DtACTION_TT_FILE_IN_SESSION, val)) {
        /*
         * Invalid field value -- issue error msg.
         */
        buf = malloc(_DtAct_MAX_BUF_SIZE);
    ...
    case _ActDb_TT_ARGN_MODE_SET:
       ...
       */
      if (strcmp(_DtACTION_TT_MODE_IN, val) && strcmp(_DtACTION_TT_MODE_OUT, val) &&
          strcmp(_DtACTION_TT_MODE_INOUT, val)) {
        /*
         * Invalid field value -- issue error msg.
         */
        buf = malloc(_DtAct_MAX_BUF_SIZE);
        (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, unrecogMsgType2, actName, filename, val,
                       _DtACTION_TTN_ARG, "n", _DtACTION_TTN_MODE);
        _DtSimpleError(DtProgName, DtError, NULL, "%s", buf);
        free(val);
        free(buf);
        return NULL;
    ...
    case _ActDb_TT_ARGN_RTYP_SET:
       ...
       */
      if (strcmp(_DtACTION_TT_RTYP_UND, val) && strcmp(_DtACTION_TT_RTYP_INT, val) &&
          strcmp(_DtACTION_TT_RTYP_BUF, val) && strcmp(_DtACTION_TT_RTYP_STR, val)) {
        /*
         * Invalid field value -- issue error msg.
         */
        buf = malloc(_DtAct_MAX_BUF_SIZE);
        (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, unrecogMsgType2, actName, filename, val,
                       _DtACTION_TTN_ARG, "n", _DtACTION_TTN_REP_TYPE);
        _DtSimpleError(DtProgName, DtError, NULL, "%s", buf);
        free(val);
        free(buf);
        return NULL;
...
static char* _DtActCheckRecord(DtDtsDbRecord* actp, char* actionType, unsigned long mask,
                               char* fileName) {
  int i;
  char* s;
  char argnFieldName[sizeof(_DtACTION_TTN_ARG) +    /* ARGn prefix */
                     3 +                            /* space for 3 decimal digits */
                     sizeof(_DtACTION_TTN_REP_TYPE) /* space for longest suffix */
                     + 8]; /* addition space for warning prevention incase the digits are more than
                              3 */
  char* buf;
  ...
  if (!strcmp(actionType, _DtACTION_COMMAND)) {
     ...
     */
    if (mask & (_ActDb_TT_BITS
#ifdef _DT_ALLOW_DT_MSGS
                | _ActDb_DT_REQUEST_BITS | _ActDb_DT_NOTIFY_BITS
#endif /* _DT_ALLOW_DT_MSGS */
                | _ActDb_MAP_BITS)) {
      /*
      ...
      (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, invldField, XrmQuarkToString(actp->recordName),
    ...
    if (!(mask & _ActDb_EXEC_STRING_SET)) {
      buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
      (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, noExecString, XrmQuarkToString(actp->recordName),
    ...
    if (!(mask & _ActDb_MAP_ACTION_SET)) {
      buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
      (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, missingField, XrmQuarkToString(actp->recordName),
...
#ifdef _DT_ALLOW_DT_MSGS
                | _ActDb_DT_NOTIFY_BITS | _ActDb_DT_REQUEST_BITS
#endif /* _DT_ALLOW_DT_MSGS */
                | _ActDb_MAP_BITS)) {
      /*
    ...
    if (!(mask & _ActDb_TT_OPERATION_SET)) {
      buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
      (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, missingField, XrmQuarkToString(actp->recordName),
                     fileName, _DtACTION_TT_OPERATION, _DtACTION_TT_MSG);
      return buf;
    }
    ...
    if ((mask & _ActDb_TT_ARGN_BITS)) {
      ...
      if (!(mask & _ActDb_TT_ARGN_MODE_SET)) {
        char argModeString[1 + sizeof(_DtACTION_TTN_ARG) + sizeof(_DtACTION_TTN_MODE)];
        /*
         * Missing required arg mode
         */
        snprintf(argModeString, sizeof(argModeString), "%s%s%s", _DtACTION_TTN_ARG, "n",
                 _DtACTION_TTN_MODE);
        buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
        (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, missingField, XrmQuarkToString(actp->recordName),
                       fileName, argModeString, _DtACTION_TT_MSG);
        return buf;
      }
       ...
       */
      for (i = 0; True; i++) {
        snprintf(argnFieldName, sizeof(argnFieldName), "%s%d%s", _DtACTION_TTN_ARG, i,
                 _DtACTION_TTN_MODE);
        if (!(s = _DtDtsDbGetFieldByName(actp, argnFieldName)))
        ...
        if (strcmp(s, _DtACTION_TT_MODE_OUT)) {
          ...
          if (!(s = _DtDtsDbGetFieldByName(actp, argnFieldName))) {
             ...
             */
            buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
            (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, missingField,
                           XrmQuarkToString(actp->recordName), fileName, argnFieldName, modeStr);
            return buf;
          }
        }
     ...
     */
    if (mask & (_ActDb_CMD_BITS | _ActDb_DT_NOTIFY_BITS | _ActDb_TT_BITS | _ActDb_MAP_BITS)) {
      /*
     ...
     */
    if (mask & (_ActDb_DT_REQUEST_BITS | _ActDb_CMD_BITS | _ActDb_TT_BITS | _ActDb_MAP_BITS)) {
      /*
     ...
     */
    buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
    (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, unrecogType, _DtACTION_TYPE, actionType, fileName);
    return buf;
  }
...
Boolean _DtActionConverter(DtDtsDbField* fields, DtDbPathId pathId, char* hostPrefix,
  ...
  if (fields[0].fieldValue == NULL) {
    buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
    snprintf(buf, _DtAct_MAX_BUF_SIZE, missingName, _DtACTION_NAME, fileName);
    _DtSimpleError(DtProgName, DtError, NULL, "%s", buf);
    XtFree(fileName);
    XtFree(buf);
    _DtSvcProcessUnlock();
    ...
    buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
    (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, noFields, fields[0].fieldValue, fileName);
    _DtSimpleError(DtProgName, DtError, NULL, "%s", buf);
    XtFree(fileName);
    XtFree(buf);
    _DtSvcProcessUnlock();
  ...
  for (i = 1; fields[i].fieldName; i++) {
    ...
    if ((ThisBit = _DtActFieldBitLookup(XrmQuarkToString(fields[i].fieldName))) != 0) {
      switch ((intptr_t)(fieldVal = _DtActValidateFieldValue(ThisBit, fields[i].fieldValue,
                                                             fields[0].fieldValue, fileName))) {
        case 0:
          /*
           *  invalid minor field
           *  -- reject field only
    ...
    } else /* Unrecognized Field */
    {
       ...
       */
      buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
      (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, unrecogField, fields[0].fieldValue, fileName,
                     XrmQuarkToString(fields[i].fieldName), fields[i].fieldValue);
    ...
    if (mask & ThisBit) {
      ...
      if (((ThisBit & _ActDb_ARGN_BITS) &&
           _DtActDupFieldNameCheck(act_rec->fieldList, act_rec->fieldCount, fields[i].fieldName)) ||
         ...
         */
        buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
        (void)snprintf(buf, _DtAct_MAX_BUF_SIZE, dupFields, fields[0].fieldValue, fileName,
  ...
  if ((buf = _DtActCheckRecord(act_rec, actionType, mask, fileName)) != NULL) {
    /*
...
static void InitializeLocalizedStrings(void) {
  noFields = XtNewString(((char*)Dt11GETMESSAGE(
      6, 1, "The action definition \"%s\" in the file:\n  \"%s\"\n  does not have any fields.\n")));

  unrecogField = XtNewString(
      ((char*)Dt11GETMESSAGE(6, 2,
                             "The action definition \"%s\" in the file\n  \"%s\"\n  contains the "
                             "following unrecognized field name and value:\n  \"%s\":\"%s\"\n")));

  unrecogType =
      XtNewString(((char*)Dt11GETMESSAGE(6, 4,
                                         "The \"%s\" field in the action definition \"%s\"\n  in "
                                         "the file \"%s\"\n  has an unrecognized action type.\n")));

  noMapTo = XtNewString(((char*)Dt11GETMESSAGE(
      6, 5,
      "The action definition \"%s\" in the file\n  \"%s\"\n  is not mapped properly.  The \"%s\" "
      "field\n  should be: TYPE MAP action_mapped_to.\n")));

  noExecString = XtNewString(
      ((char*)Dt11GETMESSAGE(6, 8,
                             "The action definition \"%s\" in the file\n  \"%s\"\n  is a "
                             "\"COMMAND\" action type but lacks the\n  required field \"%s\".\n")));

  unrecogMsgType =
      XtNewString(((char*)Dt11GETMESSAGE(6, 11,
                                         "The action definition \"%s\" in the file\n  \"%s\"\n  "
                                         "has the illegal value \"%s\" in the \"%s\" field.\n")));

  dupFields =
      XtNewString(((char*)Dt11GETMESSAGE(6, 12,
                                         "The action definition \"%s\" in the file\n  \"%s\"\n  "
                                         "contains duplicate fields named:\n \"%s\".\n")));

  invldField =
      XtNewString(((char*)Dt11GETMESSAGE(6, 13,
                                         "The action definition \"%s\" in the file\n  \"%s\"\n  "
                                         "contains invalid fields for \"%s\" type actions.\n")));

  missingField = XtNewString(
      ((char*)Dt11GETMESSAGE(6, 14,
                             "The action definition \"%s\" in the file\n  \"%s\"\n  is missing the "
                             "required field, \"%s\"\n for \"%s\" type actions.\n")));

  missingName =
      XtNewString(((char*)Dt11GETMESSAGE(4, 1,
                                         "A \"%s\" definition in the file:\n  \"%s\"\n  is missing "
                                         "a name.  Add the name\n  to the definition.\n")));

  unrecogMsgType2 = XtNewString(
      ((char*)Dt11GETMESSAGE(6, 16,
                             "The action definition \"%s\" in the file\n  \"%s\"\n  has the "
                             "illegal value \"%s\" in the \"%s%s%s\" field.\n")));
}
