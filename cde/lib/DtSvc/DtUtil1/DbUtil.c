...
static char DTDATABASESEARCHPATH_DEFAULT[] =
    "%s/.dt/types/%%L,"
    "%s/.dt/types," CDE_CONFIGURATION_TOP "/appconfig/types/%%L," CDE_CONFIGURATION_TOP
    "/appconfig/types," CDE_INSTALLATION_TOP "/appconfig/types/%%L," CDE_INSTALLATION_TOP
    "/appconfig/types";

/****   Substitution records used by XtFindFile() in _DtExpandLang()   ****/
static SubstitutionRec langSubstitutions[] = {
    {'L', (char*)NULL}, {'l', (char*)NULL}, {'t', (char*)NULL}, {'c', (char*)NULL}};
static int nLangSubstitutions = XtNumber(langSubstitutions);

/********    Static Function Declarations    ********/

static Boolean __testPath(String str);
static void __setupLangSubstitutions(void);
static void __freeLangSubstitutions(void);
static char* _DtExpandLang(char* string);
static char _DtIsDir(char* path, char* name);
static void _DtFreeDirVector(char** dir_vector);
static void __swap(int i, DtDirPaths* data);
static void _DtSortFiles(int low, int n, DtDirPaths* data);
...
static Boolean __testPath(String str) { return True; }
...
static void __setupLangSubstitutions(void) {
  ...
  if ((tcPtr = DtStrchr(ttPtr ? ttPtr : tlPtr, '.')) != (char*)NULL) {
    endPtr = tcPtr++;
    if (*tcPtr != '\0')
      codesetPart = XtNewString(tcPtr); /* free'd in __freeLangSubstitutions() */
  }
  ...
  if (ttPtr) {
    if (endPtr) {
      ...
      if (ttLen > 0) {
        ...
        strlcpy(territoryPart, ttPtr, ttLen + 1);
        territoryPart[ttLen] = '\0';
      }
    } else
      territoryPart = XtNewString(ttPtr); /* free'd in __freeLangSubstitutions() */
  ...
  if (endPtr) {
    ...
    if (tlLen > 0) {
      ...
      strlcpy(languagePart, tlPtr, tlLen + 1);
      languagePart[tlLen] = '\0';
    }
  } else
    languagePart = XtNewString(tlPtr); /* free'd in __freeLangSubstitutions() */
...
static void __freeLangSubstitutions(void) {
  ...
  for (i = 0; i < nLangSubstitutions; i++) XtFree(langSubstitutions[i].substitution);
}
...
static char* _DtExpandLang(char* string) {
   ...
   *  passing it along.
   */
  for (nColons = 0, tmpPtr = string; (tmpPtr = DtStrchr(tmpPtr, ':')) != (char*)NULL;
       nColons++, tmpPtr++)
    /* EMPTY */
    ;

  newString = (char*)XtCalloc(1, (strlen(string) + nColons + 1) * sizeof(char));
  for (tmpPtr = string; (tmpPtr1 = DtStrchr(tmpPtr, ':')) != (char*)NULL; tmpPtr = tmpPtr1 + 1) {
    size_t current_len = strlen(newString);
    size_t total_buf_size = (strlen(string) + nColons + 1);
    strlcat(newString, tmpPtr, current_len + (tmpPtr1 - tmpPtr) + 1);
    current_len = strlen(newString);
    strlcat(newString, "%:", total_buf_size);
  }
  strlcat(newString, tmpPtr, (strlen(string) + nColons + 1));
  ...
  newPath = (char*)NULL;
  for (thisPath = DtStrtok_r(newString, ",", &tokPtr); thisPath != (char*)NULL;
       thisPath = DtStrtok_r((char*)NULL, ",", &tokPtr)) {
    modPath = XtFindFile(thisPath, langSubstitutions, nLangSubstitutions, __testPath);
    if (modPath) {
       ...
       */
      if (*modPath == '/') {
        while (*(modPath + 1) == '/') modPath++;
      }
      modLen = strlen(modPath);

      if (pathLen + modLen + 2 > maxPathLen) {
        maxPathLen = ((pathLen + modLen + 2 + MAXPATHLEN) / MAXPATHLEN) * MAXPATHLEN;
        newPath = (char*)XtRealloc(newPath, maxPathLen * sizeof(char));
      }

      if (pathLen > 0)
        newPath[pathLen++] = ',';
      snprintf(&(newPath[pathLen]), maxPathLen - pathLen, "%s", modPath);
      pathLen += modLen;
...
static char _DtIsDir(char* path, char* name) {
  ...
  char* stat_name;
  size_t stat_name_size = (strlen(path) + strlen(name) + 2);

  stat_name = XtMalloc((Cardinal)stat_name_size);
  (void)strlcpy(stat_name, path, stat_name_size);
  (void)strlcat(stat_name, "/", stat_name_size);
  (void)strlcat(stat_name, name, stat_name_size);
...
static void _DtFreeDirVector(char** dir_vector) {
  ...
  if (dir_vector) {
    for (v = dir_vector; *v != NULL; v++) XtFree((char*)*v);
...
DtDirPaths* _DtFindMatchingFiles(DtDirPaths* dirs, char* suffix, Boolean sort_files) {
  /* LOCAL VARIABLES */
  ...
  /* _Xreaddirparams dirEntryBuf; -- Unused */
      ...
      if (strlen(result->d_name) >= strlen(suffix)) {
        ...
        if (file_suffix && (strcmp(file_suffix, suffix) == 0) &&
          ...
          if (num_found == max_files - 1) {
            files->dirs = (char**)XtRealloc(
                (char*)files->dirs, (Cardinal)(sizeof(char*) * (max_files + FILE_INCREMENT)));
            files->paths = (char**)XtRealloc(
                (char*)files->paths, (Cardinal)(sizeof(char*) * (max_files + FILE_INCREMENT)));
            max_files += FILE_INCREMENT;
          }

          /* Get some memory and copy the filename to the array. */
          Cardinal dirLen = (Cardinal)(strlen(dirs->dirs[nextIndex]) + strlen(result->d_name) + 2);
          files->dirs[num_found] = next_file = (char*)XtMalloc(dirLen);
          (void)strlcpy(next_file, dirs->dirs[nextIndex], dirLen);
          (void)strlcat(next_file, "/", dirLen);
          (void)strlcat(next_file, result->d_name, dirLen);

          Cardinal pathLen = (Cardinal)(strlen(next_path) + strlen(result->d_name) + 2);
          files->paths[num_found] = next_file = (char*)XtMalloc(pathLen);
          (void)strlcpy(next_file, next_path, pathLen);
          (void)strlcat(next_file, "/", pathLen);
          (void)strlcat(next_file, result->d_name, pathLen);
...
char* _DtDbGetDataBaseEnv(void) {
  ...
  snprintf(temp_buf, slen - 1, DTDATABASESEARCHPATH_DEFAULT, nwh_dir, nwh_dir);
  return temp_buf;
}
...
DtDirPaths* _DtGetDatabaseDirPaths(void) {
  /* XrmValue resource_value; -- Unused */
  /* char* rep_type; -- Unused */
  /* char *dir_string, *remote_hosts; -- Unused */
  char* dir_string;
  ...
  for (next_dir = dir_vector; *next_dir != NULL; next_dir++) {
    ...
    if (!_DtIsOpenableDirContext(*next_dir, &internal)) {
      ...
      if (i == valid_dirs) {
        valid_dirs++;
        ret_paths->dirs =
            (char**)XtRealloc((char*)ret_paths->dirs, (Cardinal)(sizeof(char*) * valid_dirs));
...
#endif
            (**next_dir == '/')) {
          Cardinal dirLen = (Cardinal)(strlen(nwh_host) + 2 + strlen(*next_dir));
          dir = XtMalloc(dirLen);
          (void)snprintf(dir, dirLen, "%s:%s", nwh_host, *next_dir);
        } else
          dir = XtNewString(*next_dir);

        ret_paths->dirs[valid_dirs - 1] = dir;
        ret_paths->paths =
            (char**)XtRealloc((char*)ret_paths->paths, (Cardinal)(sizeof(char*) * valid_dirs));
  ...
  ret_paths->dirs =
      (char**)XtRealloc((char*)ret_paths->dirs, (Cardinal)(sizeof(char*) * (valid_dirs + 1)));
  ret_paths->dirs[valid_dirs] = NULL;
  ret_paths->paths =
      (char**)XtRealloc((char*)ret_paths->paths, (Cardinal)(sizeof(char*) * (valid_dirs + 1)));
...
void _DtFreeDatabaseDirPaths(DtDirPaths* paths) {
