...
/*------------------------------------------------------------------------+*/
static XeString err_mnemonic(unsigned int errn, char* buff)
/*------------------------------------------------------------------------+*/
{
...
#ifndef __aix
    case ENOTEMPTY:
      s = (XeString) "ENOTEMPTY";
      break;
#endif
    case ENAMETOOLONG:
      s = (XeString) "ENAMETOOLONG";
      break;
    case ENOSYS:
      s = (XeString) "ENOSYS";
      break;
  ...
  if (s)
    snprintf(buff, 40, "%s (%d)", s, errn);
  else
    snprintf(buff, 40, "(%d)", errn);
...
/*----------------------------------------------------------------------+*/
{
  ...
  if (buffer) {
    snprintf(buffer, SPC_BUFSIZ, err->format, arg1, arg2);
  ...
  if (SPC_who_am_i == SPC_I_AM_A_DAEMON) {
    SPC_Format_Log((XeString) "DTSPCD error (%d): %s", XeSPCErrorNumber, err->text);
    if (err->use_errno) {
      XeString errname;
      XeString errmsg;
...
/*----------------------------------------------------------------------+*/
int SPC_Make_Log_Filename(XeString name, int unique) /* When TRUE, make name unique */
/*----------------------------------------------------------------------+*/
{
  ...
  if (!name || !*name) {
    ...
    strlcpy(spc_logfile, log_file_path, MAXPATHLEN + 1);
  } else {
    if (strlen(name) > MAXPATHLEN)
      _DtSimpleError(XeProgName, XeInternalError, XeString_NULL,
                     (XeString) "String too long in DTSPCD_Make_Log_Filename()");

    strlcpy(spc_logfile, name, MAXPATHLEN + 1);
  }

  if (unique) {
    /* Add the extension.  No strlen checking is done */
    strlcat(spc_logfile, TEMPLATE_EXTENSION, MAXPATHLEN + 1);

    cp = (XeString)mktemp(spc_logfile);
    if (!cp || !*cp) {
      /* Sorry, but this is the best we can do */
      strlcpy(spc_logfile, (log_file_path) ? log_file_path : name, MAXPATHLEN + 1);
    }
  }

  /* free the strings allocated for the path */
  if (log_file_path)
    XeFree(log_file_path);
  _DtSvcProcessUnlock();
  return TRUE;
}

/*----------------------------------------------------------------------+*/
int SPC_Open_Log(XeString filename, int unique) /* When TRUE, make filename unique */
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
{
  ...
  switch (errornum) {
    ...
    case SPC_Bad_Service:
      spc_error_struct.format =
          (XeString) "><Unknown internet service %s/%s.\nMake an entry in your /etc/services file";
      spc_error_struct.severity = XeError;
      spc_error_struct.use_errno = TRUE;
      break;
    ...
    case SPC_Arg_Too_Long:
      spc_error_struct.format =
          (XeString) "><Argument %.50s... to DTSPCD system call is too long, max. length is %d";
      spc_error_struct.severity = XeError;
      spc_error_struct.use_errno = FALSE;
      break;
    ...
    case SPC_Env_Too_Big:
      spc_error_struct.format =
          (XeString) "><Environment variable %.50s... too big,\nmaximum size is %d\n";
      spc_error_struct.severity = XeError;
      spc_error_struct.use_errno = FALSE;
      break;
    ...
    case SPC_Buffer_Overflow:
      spc_error_struct.format =
          (XeString) "><Attempted Buffer Overflow from host %s.\nConnection dropped.";
      spc_error_struct.severity = XeError;
      spc_error_struct.use_errno = FALSE;
      break;
