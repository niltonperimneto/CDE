...
#include <langinfo.h>
/* On Linux/glibc, RADIXCHAR is defined in langinfo.h (or indirectly).
   MON_DECIMAL_POINT is an enum in newer glibc.
   If RADIXCHAR is not defined, we might need it.
*/
#ifndef RADIXCHAR
#define RADIXCHAR _NL_NUMERIC_DECIMAL_POINT_WC
#endif
#else
#include <langinfo.h>
#endif
...
#if DOC
/*========================================================*/
$PTYPEBEG$ : __DtXlateDbRec $1LINER$
    : A private object used to represent translation dbs $SUMMARY$
    ...
    : xrmDb : Xrm database used to hold specs initGuard
    : used to test whether Db initialized
/*================================================$SKIP$==*/
#endif
...
#if DOC
/*========================================================*/
$PFUNBEG$ : ExpandPath() $1LINER$
    : adds current working directory to front of path if its relative $SUMMARY$
    : If path is absolute,
      returns a malloced copy.If path is relative,
      inserts the CWD in front of the relative path and returns a mallocedd
      memory
          .

      The caller must free the memory when no longer needed.$ARGS$ : filespec
    : the pathname $RETURNS$
    : ptr to mallocedd memory or NULL
/*================================================$SKIP$==*/
#endif

      char *ExpandPath(const char *filespec) { /* $CODE$ */
  ...
  if (getcwd(tmpPath, MAXPATHLEN) == 0)
  ...
  pathName = malloc(sizeof(char) * (strlen(filespec) + (eos - tmpPath) +
                                    5)); /* 5: arbitrary */
  if (NULL == pathName)
    return NULL; /* RETURN: error */
...
#if DOC
/*========================================================*/
$PFUNBEG$ : DeleteDbMem() $1LINER$ : Zeros out the db mem &frees it $SUMMARY$
    : Zeros out the db mem &frees it The xrmDb should have already been
      destroyed before calling this function.$ARGS$ : io_db
    : db to delete $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static void DeleteDbMem(_DtXlateDb *io_db) { /*$CODE$*/
...
{   /*$CODE$*/
  XrmValue value;
  char *str_type;
  if (XrmGetResource(dbRec->xrmDb, DEBUG_XLATE_NAME, DEBUG_XLATE_CLASS,
                     &str_type, &value) == True)
...
{        /*$CODE$*/
  ...
  for (*nxt = *old;              /* xfer but don't advance */
       *old != EOS; *nxt = *old) /* xfer but don't advance */
  {
    if (*old == MATCHALL_CHAR) /* if an escaped char follows */
    {
      ...
      {                  /* get the refNum and advance the ptr */
        ...
        if (refNum >= 0 && refNum < MAXSPECQUARKS &&
          ...
          *pStr = realloc(*pStr, strLen + repLen + 1);
          ...
          newTmp =
              *pStr + strLen - (oldOff - newOff) + repLen; /*pts to new EOS*/
          while (oldTmp >= old)
            *newTmp-- = *oldTmp--;
...
#if DOC
/*========================================================*/
$PFUNBEG$ : StripMetaAndReplaceEscSubex() $1LINER$
    : Strips off meta chars and replaces escaped subex(e.g. \1) values $SUMMARY$
    : Strip is performed in place if replaceValues is NULL.

      If replaceValues are specified and referenced,
    the routine assumes that the string pointed to by pStr was allocated using
    malloc() and can be resized using realloc()
        .The value and size of pStr may be different after the call.

    $WARNING$ : This routine assumes it is working on a value
                expression(e.g.from the RHS of a spec),
    that uses meta chars and regex(5) - style subexpression
                                        replacement specs.$ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static void StripMetaAndReplaceEscSubex(
        char **pStr, const Boolean keepEscChar,
        const char **replaceValues) { /*$CODE$*/
  char *nxt = *pStr;
  char *old = *pStr;
       ...
       *old != EOS; *nxt = *old) /* xfer but don't advance */
  {
    if (*old == ESCAPE_CHAR) /* if an escaped char follows */
    {
      /* if ESCAPE_CHAR is not followed by a digit, e.g. \1
         or no replacement values exist, ignore it */
      if (NULL == replaceValues || keepEscChar || isdigit(*(old + 1)) == 0) {
        if (!keepEscChar)
          old++; /* go past the escape char */
        else
          *nxt++ = *old++; /* copy esc char over */
        *nxt++ = *old++;   /* keep just the char that was escaped
                              and assign here to avoid tranlating
                              that character, then move on to the
                              next one */
      ...
      {                    /* get the refNum and advance the ptr */
        ...
        /*  printf("%x=%s\n", replaceValues[refNum], replaceValues[refNum]); **
         * DBG */
    ...
    else { /* a non-escaped char; make further checks */
      if (*old == COMMENT_CHAR) {
        *old = EOS; /* end the string */
        continue;   /* CONTINUE */
      } else if (*old == QUOTE_CHAR) {
        if (!inQuote)
          inQuote = True; /* start quote */
        else
...
#if DOC
/*========================================================*/
$PFUNBEG$ : strCaseiCmp $1LINER$ : case insensitive string comparison $SUMMARY$:
Rolled my own because strcasecmp() not available on all platforms.$ARGS$
    : $RETURNS$ : True : strings match False
    : they do not
/*================================================$SKIP$==*/
#endif

      static Boolean
      strCaseiCmp(const char *str1, const char *str2) { /*$CODE$*/
...
#if DOC
/*========================================================*/
$PFUNBEG$ : CheckForMatchall() $1LINER$
    : Matches search pattern to spec data $SUMMARY$ : $ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static Boolean
    CheckForMatchall(__DtXlateSrchData *srchData, const char *matchallString,
                     const char *matchingString) { /*$CODE$*/
  int refNum = -1;
  int score = 0;
  ...
  if (refNum != -1 && refNum >= 0 && refNum < XtNumber(srchData->curSpecRefs)) {
    ...
    if (matchingString[0] == MATCHALL_CHAR && matchingString[1] == EOS) {
...
#if DOC
/*========================================================*/
$PFUNBEG$ : CheckSearchPlatformMatchesSpec() $1LINER$
    : Matches search pattern to spec data $SUMMARY$ : $ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static Boolean
    CheckSearchPlatformMatchesSpec(__DtXlateSrchData *srchData,
                                   XrmQuark specPlatformQuark) { /*$CODE$*/
...
#if DOC
/*========================================================*/
$PFUNBEG$ : CheckSearchVerMatchesSpec() $1LINER$
    : Matches search pattern to spec data $SUMMARY$ : $ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static Boolean
    CheckSearchVerMatchesSpec(__DtXlateSrchData *srchData,
                              XrmQuark specVersionQuark) { /*$CODE$*/
  ...
  if ((score = 1) && sscanf(numStr, "%d-%d", &lowerBnd, &upperBnd) != 2 &&
      (score = 1) && sscanf(numStr, "%d+", &lowerBnd) != 1 && (score = 2) &&
      sscanf(numStr, "%d", &lowerBnd) != 1)
...
{        /*$CODE$*/
  ...
  do {
    ...
    if (NULL != hit && (hit == remainingOps || *(hit - 1) == OPER_SEPARATOR) &&
...
#if DOC
/*========================================================*/
$PFUNBEG$ : CheckSearchDirOpToStdMatchesSpec() $1LINER$
    : Matches search pattern to spec data $SUMMARY$ : $ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static Boolean
    CheckSearchDirOpToStdMatchesSpec(__DtXlateSrchData *srchData,
                                     XrmQuark specDirectionQuark) { /*$CODE$*/
  int score = 0;
  ...
  if (!(((type = __DtXLATE_TYPE_REGEX) &&
        ...
        ((type = __DtXLATE_TYPE_PURE) && (score = 1) &&
         (specDirectionQuark == srchData->equalsToQuark)) ||
        ((type = __DtXLATE_TYPE_CONTAINS) && (score = 1) &&
...
#if DOC
/*========================================================*/
$PFUNBEG$ : CheckSearchDirStdToOpMatchesSpec() $1LINER$
    : Matches search pattern to spec data $SUMMARY$ : $ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static Boolean
    CheckSearchDirStdToOpMatchesSpec(__DtXlateSrchData *srchData,
                                     XrmQuark specDirectionQuark) { /*$CODE$*/
  int score = 0;
  __DtXlateType type = __DtXLATE_TYPE_NONE;
...
#if DOC
/*========================================================*/
$PFUNBEG$ : CheckSearchStdValueMatchesSpec() $1LINER$
    : Matches search pattern to spec data $SUMMARY$ : $ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static Boolean
    CheckSearchStdValueMatchesSpec(__DtXlateSrchData *srchData,
                                   XrmQuark *specStdValueQuarks) { /*$CODE$*/
  int score = 0;
  int unmatched = 0;
  XrmQuark *patQuarks;
  ...
  for (unmatched = 0; *specStdValueQuarks != NULLQUARK; specStdValueQuarks++) {
    unmatched++;
  }
...
#if DOC
/*========================================================*/
$PFUNBEG$ : CheckSearchOpValueMatchesSpec() $1LINER$
    : Matches search pattern to spec data $SUMMARY$ : $ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static Boolean
    CheckSearchOpValueMatchesSpec(__DtXlateSrchData *srchData,
                                  const char *specOpValue) { /*$CODE$*/
  ...
  if (srchData->curTransType == __DtXLATE_TYPE_REGEX) {
    ...
    if ((ex = compile(opValBuf, NULL, NULL)) != NULL &&
      ...
      if (matchSize < 0 || matchSize >= sizeof(opValBuf))
        matchSize = 0;
    ...
    if (regcomp(&re, opValBuf, 0) == 0 &&
        regexec(&re, srchData->opValue, XtNumber(srchData->curSubEx),
                srchData->curSubEx, 0) == 0) {
      int matchSize;
      matches = True; /* if got this far */

      /* inc score by the size of the match after
         scaling for the maximum possible match size */
      matchSize = srchData->curSubEx[0].rm_eo - srchData->curSubEx[0].rm_so;
    ...
    regfree(&re);
#endif
  } else /* (srchData->curTransType == __DtXLATE_TYPE_PURE ||
            __DtXLATE_TYPE_CONTAINS */
  {
    char *opValueInBuf;
    ...
    if (matches)
      srchData->curScore += sizeof(opValBuf);
    ...
    if (matches) {
      if (opValueInBuf)
        srchData->curScore += strlen(srchData->opValue);
      else
        srchData->curScore += strlen(opValBuf);
    }
  }
  return matches;
} /*$END$*/
...
#if DOC
/*========================================================*/
$PFUNBEG$ : FindStdToOpMatchCB() $1LINER$
    : Matches std value of entry to search pattern;
gets op value $SUMMARY$ : $ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static Bool FindStdToOpMatchCB(XrmDatabase *database,
                                   XrmBindingList bindings, XrmQuarkList quarks,
                                   XrmRepresentation *type, XrmValue *value,
                                   XPointer client_data) { /*$CODE$*/
...
#ifdef DBG_MATCHING
  fprintf(stderr, "FindStdToOpMatch: ");
  PrintDbEntry(quarks, value);
#endif

  /* check for a match */
  if (CheckSearchPlatformMatchesSpec(srchData, quarks[PLATFORM_QUARK]) == False)
  ...
  if (CheckSearchVerMatchesSpec(srchData, quarks[VERSION_QUARK]) == False)
    return False; /* continue enumeration */
  ...
  if (CheckSearchOperMatchesSpec(srchData, quarks[OPERS_QUARK]) == False)
    return False; /* continue enumeration */
  ...
  if (CheckSearchDirStdToOpMatchesSpec(srchData, quarks[DIRECTION_QUARK]) ==
      False)
    return False; /* continue enumeration */
  ...
  if (CheckSearchStdValueMatchesSpec(srchData, &quarks[FIRSTSTD_QUARK]) ==
      False) {
...
#if DOC
/*========================================================*/
$PFUNBEG$ : FindOpToStdMatchCB() $1LINER$
    : Matches op value of entry to search pattern;
gets std value $SUMMARY$ : $ARGS$ : $RETURNS$ :
/*================================================$SKIP$==*/
#endif

    static Bool FindOpToStdMatchCB(XrmDatabase *database,
                                   XrmBindingList bindings, XrmQuarkList quarks,
                                   XrmRepresentation *type, XrmValue *value,
                                   XPointer client_data) { /*$CODE$*/
  __DtXlateSrchData *srchData = (__DtXlateSrchData *)client_data;
...
#ifdef DBG_MATCHING
  fprintf(stderr, "FindOpToStdMatch: ");
  PrintDbEntry(quarks, value);
#endif
  ...
  if (CheckSearchDirOpToStdMatchesSpec(srchData, quarks[DIRECTION_QUARK]) ==
      False)
    return False; /* continue enumeration */
  ...
  if (CheckSearchOpValueMatchesSpec(srchData, value->addr) == False) {
    if (srchData->db->debugMode) {
      fprintf(stderr, "mismatch ");
      PrintDbEntry(quarks, value);
    }
...
{       /*$CODE$*/
  ...
  if (NULL == *io_db || (*io_db)->initGuard != INIT_OCCURRED ||
      (*io_db)->xrmDb == NULL) {
...
#if DOC
/*========================================================*/
$FUNBEG$ : _DtXlateMergeDbs() $1LINER$
    : Merges two open dbs into one and closes the merged
      -
      in db.$SUMMARY$
    ...
    the io_dbToMerge database is simply moved over to io_mergeIntoDb.If
    io_mergeIntoDb is valid,
    the entries in the io_dbToMerge database are merged into it and take
    precedence over entries in the io_mergeIntoDb,
    and the io_dbToMerge database is closed.$ARGS$ : io_dbToMerge
    : database to merge into io_mergeIntoDb io_mergeIntoDb
    : database to hold merged result $RETURNS$ : 0 : no error occurred
        - 1 : if io_dbToMerge or io_mergeIntoDb is NULL
              if *io_dbToMerge is NULL or uninitialized
/*================================================$SKIP$==*/
#endif

              int _DtXlateMergeDbs(_DtXlateDb *io_dbToMerge,
                                   _DtXlateDb *io_mergeIntoDb) { /*$CODE$*/

  /* check args */
  if (NULL == io_mergeIntoDb || NULL == io_dbToMerge || NULL == *io_dbToMerge ||
...
#if DOC
/*========================================================*/
$FUNBEG$ : _DtXlateOpenAllDbs() $1LINER$
    : Open and merge all locale
      translation databases that can be found $SUMMARY$
    : DtXlateOpenAllDbs() locates all
      translation databases present in the search paths directories.$ARGS$
    : searchPaths : ':' separated list of directories databaseName
    : name of the database file in those directories ret_db
    : the reference to the open database is stored here $RETURNS$ : 0
    : at least one database was opened
    - 1
...
/*================================================$SKIP$==*/
  ...
  do {
    ...
    if (NULL == separator)
      _DtMBStrchr(workStart, EOS, -1, &separator);
    if (NULL == separator)
      break;                         /* BREAK */
...
#if DOC
/*========================================================*/
$FUNBEG$ : _DtXlateCloseDb() $1LINER$
    : Close an open translation database $SUMMARY$
    : _DtXlafteCloseDb() releases all memory associated with the
...
/*================================================$SKIP$==*/
          ...
          int _DtXlateCloseDb(_DtXlateDb *io_db) { /*$CODE$*/
  __DtXlateDbRec *dbRec;

  if (NULL == io_db || NULL == (dbRec = *io_db) /* dbRec assigned */
      || dbRec->initGuard != INIT_OCCURRED)
    return -1; /* RETURN */
  ...
  if (dbRec->debugMode)
    fprintf(stderr, "_DtXlateCloseDb: %p\n", (void *)dbRec);
...
{       /*$CODE$*/
  ...
  if (NULL == db || db->initGuard != INIT_OCCURRED || NULL == operation ||
      operation[0] == EOS)
  ...
  if (NULL == stdValue) {
    if (db->debugMode)
      fprintf(stderr, "_DtXlateStdToOpValue: NULL std value\n");
    if (ret_opValue)
      *ret_opValue = NULL;
    return -1; /* RETURN error */
  }
  ...
  if (db->debugMode)
    fprintf(stderr, "_DtXlateStdToOpValue: %s.%d.%s.%s: <op>\n", platform,
            version, operation, stdValue);

  /* scan through this Db looking for matches and put in search */
  XrmEnumerateDatabase(db->xrmDb, &empty, &empty, XrmEnumAllLevels,
                       FindStdToOpMatchCB, (XPointer)&srchData);
  ...
  if (srchData.opValue != NULL &&
    ...
    if (ret_opValue == NULL) {
      if (db->debugMode)
        fprintf(stderr, "translation exists\n");
...
{       /*$CODE$*/
  ...
  if (NULL == opValue) {
    if (db->debugMode)
      fprintf(stderr, "_DtXlateOpToStdValue: NULL op value\n");
    if (ret_stdValue)
      *ret_stdValue = NULL;
    return -1; /* RETURN error */
  }
  ...
  if (db->debugMode)
    fprintf(stderr, "_DtXlateOpToStdValue: %s.%d.%s.<std>: %s\n", platform,
            version, operation, opValue);

  /* scan through this Db looking for matches and put in search */
  XrmEnumerateDatabase(db->xrmDb, &empty, &empty, XrmEnumAllLevels,
                       FindOpToStdMatchCB, (XPointer)&srchData);
  ...
  if (srchData.stdValueQuarks[0] != NULLQUARK &&
    ...
    for (stdQ = srchData.stdValueQuarks; *stdQ != NULLQUARK; stdQ++) {
      const char *str = XrmQuarkToString(*stdQ);
      int strLen;
      if (NULL == str || str[0] == EOS)
        continue;

      strLen = strlen(str);
      if (stdValue != lhs) {
        *stdValue++ = STDVALUE_SEPARATOR;
        stdValueLen++;
      }
...
For example
    : OSMAJORVERSION &OSMINORVERSION are compile -
      time constants that must be defined as part of the build environment
...
xlateStd2Op(_DtXlateDb db,Boolean runtimeOp, char * operation,char * stdVal)
{
  ...
  if (ret == 0)
    _DtXlateStdToOpValue(db, platform, version, operation, stdVal, &opVal,
                         NULL);
...
{       /*$CODE$*/
  ...
  if (ret_AppExecEnvVersion) {
    ...
    ret = _DtXlateOpToStdValue(db, names.sysname, 0, _DtXLATE_OPER_VERSION,
                               version, &stdVer, NULL);
    if (ret == 0) {
      if (sscanf(stdVer, "%d", &verNum) != 1)
        verNum = MATCHALL_VER;
      ;
      free(stdVer);
    }
...
#define OSMINORVERSION 0
    ...
    sprintf(buf, "%s%s%s", STR(OSMAJORVERSION), nl_langinfo('.'),
            STR(OSMINORVERSION));
#else
    sprintf(buf, "%s%s%s", STR(OSMAJORVERSION), nl_langinfo(RADIXCHAR),
            STR(OSMINORVERSION));
#endif
...
/*========================================================*/
$INTROBEG$:
translation BNF syntax and semantics $1LINER$
    : _DtXlate translation specification syntax and semantics $SUMMARY$
    : BNF Syntax of Translation Specification == ==
      == == == == == == == == == == == == == == == == ==
    =

        <specfile>:: = (<xlatespec> | <comment> | <cr>)*<xlatespec>:: =
            <platform>.<version>.<operations>.<transtype>.<stdvalue>
    : <opvalue>(<comment> | <cr>)<platform>::
                = <identifier> | <matchall><version>:: =
                      <number>[+| (-<number>)] | <matchall><operations>:: =
                          <identifier>[',' < identifier >
] * | <matchall><transtype>:: =
                              '<' | '=' | '~' | '>' | '0' < stdvalue > :: =
                                  <identifier>[.<identifier>] * <langterr>:: =
                                      <identifier> | <matchall><codeset>:: =
                                          <identifier> | <matchall><modifier>:: =
                                              <identifier> | <matchall><opvalue>:: =
                                                  (<vischars> | <metachar>)+|
                                                  '"'(<anychar> |
                                                      <metachar>)+'"'
                                                                  ...
                                                                  = [0 - 9] +
                                                                    <identifier>::
                                                                        = [-_a -
                                                                            ...
                                                                            whitespace
                                                                                character;
meta and regular expression chars must be escaped with "\\" < anychar > :: =
    any printable character; meta and regular
...
<operations> : a CDE-standardized identifier for the operation(s)
to which the value applies.The operation(s) need not be supported by every
...
Use '<' if the <opvalue> contains a regular expression;
the specification may only be used to match an operation value in a
_DtLcxXlateOpToStd() translation.Use '>' if the<opvalue> contains value
    replacement expressions using values from the<stdvalue> strings; the specification may only be used by a
...
translation;
to specify that a given translation does not exist,
    the '0' translation type should be used or the specification should not
        occur
            .

    <comment> : a comment begins on an empty line with an
                    unescaped !and continues to the end of the line.

                <matchall> : The matchall character is a universal quantifier,
    meaning that it symbolizes all possible values of the field where it is
        used.The matchall character may occur only in the qualifers side of a
            translation specification.Traditionally,
    the matchall character has been '*', but because of the semantics of Xrm,
    if the '*' is used as the matchall,
    Xrm does not restrict enumeration as needed
        .

    The matchall character may be followed by a number when the<transtype>
        is '>'.When this occurs,
    the string that matches the matchall character may be referenced in
        the<opvalue>
            by using the sequence
\<number>,
    as in "\1".The occurrence is replaced with the matched string.
/*=$END$================================================*/
#endif

#if DOC
        /*========================================================*/
        $INTROBEG$ : translation specification examples $1LINER$
    : examples of _DtXlate translation specifications $EXAMPLE$ :

    !!These are examples of bi -
        directional straight translations HP - UX.900 -
        999.setlocale. =.en_US.hp -
                        roman8
    : american HP -
      UX.900 - 999.setlocale. =.en_US.iso88591 : american.iso88591 HP - UX.900 -
                               999.setlocale. =
                                   .nl_BE.hp -
                                   roman8.fold : dutch @fold HP - UX.900 -
                                   999.setlocale. =
                                   .nl_BE.hp -
                                           roman8
                                   .nofold : dutch @nofold

                                             !!These are examples of OpToStd translations
                                                 utilizing regular !!expression patterns
                                                     to match the search value.HP -
                                           UX.
                                       ?.version.< .900
                                   : "A\\.09\\..*" !!any HPUX 9.x version HP -
                                           UX.
                                       ?.version.< .807
                                   : "A\\.08\\.07.*" HP -
                                           UX.
                                       ?.version.< .800
                                   : "A\\.08\\..*" AIX.?.version.< .320
                                   : "2 3" AIX.        ?.version.< .300
                                   : "[0-9] 3" SunOS.
                                       ?.version.< .530
                                   : "5\\.3.*" SunOS.
                                   ...
                                   .
                                       ?.iso88596
                                   : arabic8 HP - UX.900 +.iconv1.>.?.iso88597
                                   : greek8 HP - UX.900 +.iconv1.>. ?.hp - kana8
                                   : kana8 HP - UX.900 +.iconv1.>.
                                       ?.hp - roman8
                                       : roman8

                                         !!The following examples use value
                                             replacement as part of
                                                 their !!specifications
                                   .Using this can lead to much sparser tables,
        but !!it depends on op -
                    specific and std values sharing the same strings.HP -
                    UX.900 - 999.setlocale. <
                    .nl_BE.hp - roman8.
            ? 1
        : [dD][uU][tT][cC][hH] @\\(.*\\) !HP - UX.900 - 999.setlocale. <
                    .nl_BE.hp - roman8.
            ? 1
        : dutch @\\(.*\\) HP - UX.1000 +.setlocale.>.? 1.hp-roman8
        :    \\1.roman8 !!all non - modif cases HP - UX.1000 +.setlocale.>.
             ? 1.hp-roman8. ? 2 : \\1.roman8
                   @\\2 !!all modif cases
...
/*=$END$================================================*/
                       ...
                       /*========================================================*/
                       $INTROBEG$
             : _DtXlate example usage $1LINER$
    : Examples of how to _DtXlate $EXAMPLE$ :
#include <XlationSvc.h>
    main() {
  _DtXlateDb db = NULL;
  int ret;
...
#define OPER_YOUROP "myop"
  ...
  ret = _DtXlateOpToStdValue(db, plat, execver, OPER_YOUROP, str = "american",
                             &val, NULL);
  if (ret == 0)
    printf("setlocale(%s) xlation=%s\n", str, val);
  else
