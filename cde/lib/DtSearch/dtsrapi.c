...
static void signal_abort(int sig) {
  ...
  fprintf(aa_stderr, CATGETS(dtsearch_catd, MS_ausapi, 216, "\n%s %s Caught signal %d.\n"),
          PROGNAME "216", (aa_argv0) ? aa_argv0 : OE_prodname, sig);
  fflush(aa_stderr);
...
static int valid_dbname(char* dbname) {
  ...
  if (db == NULL) {
    sprintf(sprintbuf,
            CATGETS(dtsearch_catd, MS_ausapi, 1, "%1$sInvalid or unavailable database '%2$s'."),
            PROGNAME "48 ", dbname);
...
static void build_dbnames_array(void) {
  ...
  ausapi_dbnamesv =
      austext_malloc(mallocsz + (sizeof(char*) * (2 + ausapi_dbnamesc)) + 4, PROGNAME "106", NULL);
 ...
 */
int DtSearchInit(char* argv0, char* userid, long switches, char* config_file, FILE* err_file,
                 char*** dbnames, int* dbcount) {
  if (aa_is_initialized) {
    sprintf(sprintbuf,
            CATGETS(dtsearch_catd, MS_ausapi, 621, "%1$s %2%s has already been initialized."),
  ...
  if (userid == NULL || *userid == 0)
    if ((userid = (char*)getenv("LOGNAME")) == NULL) {
      sprintf(sprintbuf,
              CATGETS(dtsearch_catd, MS_ausapi, 187,
  ...
  if (dbnames) {
    *dbnames = ausapi_dbnamesv;
    if (!dbcount) {
      sprintf(sprintbuf,
              CATGETS(dtsearch_catd, MS_ausapi, 7, "%s dbnames specified but not dbcount."),
              PROGNAME "304");
      DtSearchAddMessage(sprintbuf);
      return DtSrFAIL;
    }
 ...
 */
int DtSearchGetKeytypes(char* dbname,           /* 1 - 8 char database name */
...
static int both_valid_dates(char* date1, char* date2) {
  ...
  if (!convert_error && usrblk.objdate1 != 0L && usrblk.objdate2 != 0L &&
      usrblk.objdate1 >= usrblk.objdate2) {
    sprintf(sprintbuf,
            CATGETS(dtsearch_catd, MS_ausapi, 198,
 ...
 */
int DtSearchQuery(void* qry,              /* query, fzkeyi, nav string */
                  ...
                  DtSrResult** dittolist, /* put hitlist here */
                  ...
                  int* stemcount)         /* put size of stems array */
{
  ...
  DBLK* db;
  LLIST* llp;
  enum { TEXT, NAVSTRING, FZKEYI } qryarg;
#define _DATE1STR_LEN (24)
  ...
  switch (search_type) {
    ...
    default:
      sprintf(sprintbuf, CATGETS(dtsearch_catd, MS_ausapi, 20, "%1$s Invalid search_type '%2$c'."),
  ...
  QUERY_ERROR:
    sprintf(sprintbuf,
            CATGETS(dtsearch_catd, MS_ausapi, 30, "%s Null query.  No search performed."),
            PROGNAME "81");
    DtSearchAddMessage(sprintbuf);
    return DtSrFAIL;
  }
  ...
  if (usrblk.debug & USRDBG_SRCHCMPL) {
    ...
    *(--ptr) = 0;
    ...
    fprintf(aa_stderr,
            PROGNAME
            "353 DtSearchQuery(): dbname='%s' srchtype='%c'\n"
            "  maxhits=%d keytypes='%s'\n"
            "  date1='%s' -> %s.  date2='%s' -> %s.\n"
            "  query='%.60s'\n",
            db->name, search_type, db->maxhits, sprintbuf, NULLORSTR(date1), date1str,
            NULLORSTR(date2), objdate2fzkstr(usrblk.objdate2), NULLORSTR(usrblk.query));
    fflush(aa_stderr);
  }
  ...
  /* END_OF_SEARCH: -- Unused */
  if (usrblk.debug & USRDBG_SRCHCMPL) {
    /* count msgs */
    i = 0;
    for (llp = ausapi_msglist; llp != NULL; llp = llp->link) i++;
    fprintf(aa_stderr,
            PROGNAME
            "380 Return from Search:  "
 ...
 */
int DtSearchRetrieve(char* dbname,     /* 1 - 8 char database name */
 ...
 *      char   stems [DtSrMAX_STEMCOUNT] [DtSrMAXWIDTH_HWORD].
 */
int DtSearchHighlight(char* dbname,    /* database name */
  ...
  if (cleartext == NULL || cleartext[0] == 0) {
    sprintf(
        sprintbuf,
        CATGETS(dtsearch_catd, MS_ausapi, 40, "%1$s Null cleartext.  No highlighting performed."),
        PROGNAME "349 ", dbname);
    DtSearchAddMessage(sprintbuf);
    return DtSrERROR;
  }
  if (cleartext != usrblk.cleartext) {
    ...
    usrblk.cleartext = austext_malloc(usrblk.clearlen + 16, PROGNAME "267", NULL);
  ...
  if (stems) {
    if (stemcount > DtSrMAX_STEMCOUNT) {
      sprintf(sprintbuf,
              CATGETS(dtsearch_catd, MS_ausapi, 1072,
