...
static void expired(char* sprintbuf) {
  sprintf(sprintbuf, CATGETS(dtsearch_catd, MS_oe, 71, PROGNAME "71 %s has expired."), OE_prodname);
...
/* static void alarm_signal_handler (int sig) -- Unused */
...
int oe_unblob(LLIST* bloblist) {
  ...
  while (lptr != NULL) {
    ...
    if (targ + blobclearlen > stoploc) {
      DtSearchAddMessage(PROGNAME
                         "242 Logical Error in database. "
...
void oe_write_audit_rec(long numhits) {
  ...
  {
    sprintf(sprintbuf,
            CATGETS(dtsearch_catd, MS_misc, 1596, PROGNAME "1596 Cannot open audit file %s: %s"),
    ...
    time(&now_gmt);
    fprintf(
        stream, AUDIT_FORMAT "%s\n", usrblk.userid, nowstring(&now_gmt),
        now_gmt - saveusr.start_time, /* elapsed search time */
        usrblk.dblk->name, numhits,
        (saveusr.lastqry == NULL) ? CATGETS(dtsearch_catd, MS_misc, 1, "<null>") : saveusr.lastqry);
    if (saveusr.lastqry != NULL) {
      free(saveusr.lastqry);
...
static int no_keytypes(void) {
  ...
  while (--i >= 0)
    if (keytypes[i].is_selected)
      return FALSE;
  sprintf(sprintbuf,
          CATGETS(dtsearch_catd, MS_oe, 440,
...
static char* request_str(int reqnum) {
  static NUMSTR numstr[] = {{OE_INITIALIZE, "INITIALIZE"},             /*  1 */
  ...
  while (reqnum != ptr->num && ptr->num != 0) ptr++;
  return ptr->str;
} /* request_str() */
...
char* retncode_str(int num) {
  static char buf[16];
  static NUMSTR numstr[] = {{OE_OK, "OE_OK"},                   /*  1 */
  ...
  while (num != ptr->num && ptr->num != 0) ptr++;
  if (ptr->num == 0)
    sprintf(buf, "%d(?)", num);
  return ptr->str;
} /* retncode_str() */
...
void Opera_Engine(void) {
  ...
  if (usrblk.debug != 0L) {
    ...
    fprintf(aa_stderr,
            "\n" PROGNAME
            "444  Opera_Engine Request %d (%s) at %s.\n"
            "  user='%s', usrblk.flags=%ld(x%04lx), usrblk.debug=%ld(x%04lx).\n"
            "  OE_flags=%ld(x%04lx), exp=%s, sitecnfg=%s.\n",
            usrblk.request, request_str(usrblk.request), sprintbuf, usrblk.userid, usrblk.flags,
            usrblk.flags, usrblk.debug, usrblk.debug, OE_flags, OE_flags, sprintbuf + 100,
            sprintbuf + 200);
    i = 0;
  ...
  if (OE_flags & OE_PERMERR) {
    sprintf(sprintbuf,
            CATGETS(dtsearch_catd, MS_oe, 490, PROGNAME "490 %s Engine permanently disabled."),
            OE_prodname);
    DtSearchAddMessage(sprintbuf);
    usrblk.retncode = OE_ABORT;
    goto ENGINE_RETURN;
  ...
  if (usrblk.request != OE_INITIALIZE) {
    ...
    for (db = usrblk.dblist, OE_dbn = 0; db != NULL; db = db->link, OE_dbn++)
      if (strcmp(usrblk.dblk->name, db->name) == 0)
        break;
    if (db == NULL) {
      sprintf(sprintbuf,
              CATGETS(dtsearch_catd, MS_oe, 48,
  ...
  switch (usrblk.request) {
    ...
    case OE_SRCH_WORDS:
      ...
      if (no_keytypes())
        break;
      usrblk.search_type = (usrblk.request == OE_SRCH_WORDS) ? 'W' : 'S';
    ...
    case OE_SRCH_STATISTICAL:
      ...
      ve_statistical();
      if (usrblk.debug & USRDBG_SRCHCMPL)
        print_stems(usrblk.stemcount, usrblk.stems, PROGNAME "770 SRCH_STATISTICAL:");
      if (usrblk.debug & (USRDBG_SRCHCMPL | USRDBG_HITLIST))
    ...
    case OE_HILITE_STEMS:
      if (usrblk.debug & (USRDBG_RETRVL | USRDBG_HILITE)) {
        fprintf(aa_stderr,
                ...
                usrblk.search_type, usrblk.clearlen, NULLORSTR(usrblk.cleartext));
        print_stems(usrblk.stemcount, usrblk.stems, " ");
      }
      if (usrblk.cleartext == NULL || usrblk.clearlen <= 0) {
      NO_TEXT:
        DtSearchAddMessage(PROGNAME "839 Client Error: No Text to highlight.");
        usrblk.retncode = OE_BAD_QUERY;
        break;
      }
      ...
      switch (usrblk.search_type) {
        ...
        default:
          i = 0;
          DtSearchAddMessage(PROGNAME
                             "708 Word Highlighting is not available "
      ...
      if (usrblk.debug & USRDBG_RETRVL) {
        fprintf(aa_stderr, PROGNAME "820 HILITE_STEMS: hitwcount=%ld\n", usrblk.hitwcount);
      }
    ...
    case OE_GETREC_DIC:
       ...
       */
        ...
        switch (usrblk.request) {
          case OE_GETREC_WORDS:
            hilite_cleartext('W', (char*)usrblk.stems, usrblk.stemcount);
            break;

          case OE_GETREC_STEMS:
            hilite_cleartext('S', (char*)usrblk.stems, usrblk.stemcount);
            break;
        ...
        for (i = 0; i < usrblk.hitwcount; i++)
          fprintf(aa_stderr, "offset = %4ld, len=%2d, '%.10s...'\n", usrblk.hitwords[i].offset,
                  usrblk.hitwords[i].length, usrblk.cleartext + usrblk.hitwords[i].offset);
#endif
    ...
    case OE_RECKEY2DBA:
      ...
      if ((usrblk.debug & USRDBG_DELETE) != 0L)
        fprintf(aa_stderr,
                ...
                usrblk.retncode, usrblk.query, (long)((usrblk.dba) >> 24),
                (long)((usrblk.dba) & 0xffffff));
      break;
    ...
    default:
      sprintf(sprintbuf,
              CATGETS(dtsearch_catd, MS_oe, 367,
