...
static unsigned char* final_permutes = NULL;
...
void add_syntax_errmsg(int msgno) {
  switch (msgno) {
    case 1:
      ...
      sprintf(msgbuf, CATGETS(dtsearch_catd, MS_boolpars, 2, "%s Query field is empty."),
              PROGNAME "086");
    ...
    case 2:
      sprintf(msgbuf,
              CATGETS(dtsearch_catd, MS_boolpars, 5,
    ...
    case 3:
      sprintf(msgbuf,
              CATGETS(dtsearch_catd, MS_boolpars, 6, "%s Expression in parentheses is missing."),
              PROGNAME "093");
      DtSearchAddMessage(msgbuf);
      break;
    ...
    case 4:
      sprintf(msgbuf,
              CATGETS(dtsearch_catd, MS_boolpars, 7,
    ...
    case 5:
      ...
      sprintf(msgbuf,
              CATGETS(dtsearch_catd, MS_boolpars, 3,
    ...
    case 6:
      sprintf(msgbuf,
              CATGETS(dtsearch_catd, MS_boolpars, 4,
    ...
    default:
      sprintf(
          msgbuf,
          CATGETS(dtsearch_catd, MS_boolpars, 8, "%s Invalid boolean query.  Syntax Error #%d."),
          PROGNAME "100", msgno);
      DtSearchAddMessage(msgbuf);
      break;
  }
...
static TRUTHTAB* creatett(int stemno, int pmsz, unsigned char* permutes) {
  TRUTHTAB* newtt = austext_malloc(sizeof(TRUTHTAB) + pmsz + 4, PROGNAME "140", NULL);
...
static TRUTHTAB* get_stem_truthtab(char* newstem, char* origword) {
  ...
  if (stemno == saveusr.stemcount) {
    if (++saveusr.stemcount > DtSrMAX_STEMCOUNT) {
      sprintf(msgbuf, CATGETS(dtsearch_catd, MS_boolpars, 9, "%s Too many terms in boolean query."),
              PROGNAME "1513");
      DtSearchAddMessage(msgbuf);
      saveusr.stemcount--;
      return NULL;
  ...
  if (debugging_boolpars) {
    fprintf(aa_stderr, "   WORD: stem[%d]='%c%s' expr=%p pmsz=%d\n", stemno,
            (saveusr.stems[stemno][0] == STEM_CH) ? '~' : saveusr.stems[stemno][0],
            &saveusr.stems[stemno][1], (void*)newtt, newtt->pmsz);
    fflush(aa_stderr);
  }
  return newtt;
} /* get_stem_truthtab() */
...
TRUTHTAB* boolyac_OR(TRUTHTAB* tt1, TRUTHTAB* tt2) {
  ...
  while (pm1 < tt1->pmsz) new_permutes[newpm++] = permutes1[pm1++];
  while (pm2 < tt2->pmsz) new_permutes[newpm++] = permutes2[pm2++];
 ...
 */
TRUTHTAB* boolyac_COLLOC(TRUTHTAB* word1tt, int colloc_val, TRUTHTAB* word2tt) {
  TRUTHTAB* newtt;
  ...
  if (word1tt->stemno == word2tt->stemno) {
    sprintf(msgbuf,
            CATGETS(dtsearch_catd, MS_boolpars, 12,
  ...
  sprintf(wordbuf, COLLOC_STEM_FORMAT, word1tt->stemno, word2tt->stemno, colloc_val);
  if ((newtt = get_stem_truthtab(wordbuf, wordbuf)) == NULL)
    return NULL;
  freett(word1tt);
  freett(word2tt);
  if (debugging_boolpars) {
    fprintf(aa_stderr, " COLLOC: exprs=%p,%p-->expr=%p pmsz=%d\n", (void*)word1tt, (void*)word2tt,
            (void*)newtt, newtt->pmsz);
    fflush(aa_stderr);
  }
  return newtt;
...
void yyerror(char* msg) {
  if (strcmp(msg, "syntax error") == 0) {
    if (DtSearchHasMessages())
      ...
      sprintf(msgbuf,
              CATGETS(dtsearch_catd, MS_boolpars, 1,
...
GET_ANOTHER_TOKEN:
  ...
  /* Skip white space */
  while (ascii_charmap[(unsigned char)*next_lex_char] & WHITESPACE) next_lex_char++;
  ...
  switch (*next_lex_char) {
    ...
    case '@': /* COLLOCATION operator */
      ...
      if ((usrblk.dblk->dbrec.or_dbaccess & ORA_BLOB) == 0) {
        ...
        sprintf(msgbuf,
                CATGETS(dtsearch_catd, MS_boolpars, 10,
      ...
      if (yylval.int_val <= 0) {
        retn_token = ERROR_TOKEN;
        sprintf(msgbuf,
                CATGETS(dtsearch_catd, MS_boolpars, 11,
    ...
    default:
      ...
      if (stembufp == NULL) {
        ...
        retn_token = ERROR_TOKEN;
        if (!DtSearchHasMessages()) {
          sprintf(msgbuf, CATGETS(dtsearch_catd, MS_boolpars, 13, "%s Word '%.*s' is invalid."),
      ...
      if (strlen(stembufp) != strlen(yytext)) {
        retn_token = ERROR_TOKEN;
        sprintf(msgbuf,
                CATGETS(dtsearch_catd, MS_boolpars, 14, "%s String '%.*s' is not a single word."),
...
DELIVER_TOKEN:
  if (debugging_boolpars) {
    fprintf(aa_stderr, "  yylex: op?=%d parct=%d tok#=%d lval=%p%sYYTEXT='%s'\n",
            last_token_was_boolop, paren_count, retn_token, (void*)yylval.truthtab,
            (retn_token == COLLOC_TOKEN) ? "\t\t" : "\t", yytext);
    fflush(aa_stderr);
  }
  return retn_token;
...
int boolean_parse(void) {
  ...
  if (!msgbuf)
    msgbuf = austext_malloc(300 + DtSrMAXWIDTH_HWORD, PROGNAME "255", NULL);

  /* Test for empty query */
  if (usrblk.query == NULL) {
  EMPTY_QUERY:
    /* Message #2 is called in two places */
    sprintf(msgbuf, CATGETS(dtsearch_catd, MS_boolpars, 2, "%s Query is empty."), PROGNAME "289");
    DtSearchAddMessage(msgbuf);
    return FALSE;
  }
  ...
  if (debugging_boolpars || (usrblk.debug & USRDBG_SRCHCMPL)) {
    fprintf(aa_stderr,
            ...
            "  query: '%s'\n",
            (usrblk.request == OE_SRCH_STEMS), qry_is_all_ANDs, qry_has_no_NOTs, usrblk.query);
    fflush(aa_stderr);
  }
  ...
  if (debugging_boolpars || (usrblk.debug & USRDBG_SRCHCMPL)) {
    print_stems(saveusr.stemcount, saveusr.stems, PROGNAME "815 end boolean_parse, syntax ok,");
  ...
  if (final_truthtab.pmsz <= 0) {
    sprintf(msgbuf,
            CATGETS(dtsearch_catd, MS_boolpars, 15,
  ...
  if (final_truthtab.pmsz >= 256) {
    sprintf(msgbuf,
            CATGETS(dtsearch_catd, MS_boolpars, 16,
...
static void process_user_args(int argc, char* argv[]) {
  ...
  while (argc > 0) {
    ...
    switch (argptr[1]) {
      ...
      case 'd':
        for (cptr = argptr + 2; *cptr != 0; cptr++) {
          switch (*cptr) {
            ...
            default:
              oops = TRUE;
              fprintf(aa_stderr, "%s Invalid debug option %c.\a\n", PROGNAME "049", *cptr);
              break;
          }
        }
        break;
      ...
      default:
        oops = TRUE;
        fprintf(aa_stderr, "%s Invalid command line argument '%s'.\a\n", PROGNAME "059", argptr);
        break;
    } /* end switch */
...
int main(int argc, char* argv[]) {
  ...
  if (!load_language(&dblk, NULL)) {
    fprintf(aa_stderr, PROGNAME "140 load_language() failed.  Msgs:\n%s\n", DtSearchGetMessages());
    return 2;
  }
  fprintf(aa_stderr, "  lang=%d minwdsz=%d maxwdsz=%d.\n", dblk.dbrec.or_language,
          dblk.dbrec.or_minwordsz, dblk.dbrec.or_maxwordsz);

  /* Main loop.  Each line is a boolean query. */
  printf(
      "Enter an AusText boolean query.  'q' or '.' to quit.\n"
  ...
  while (fgets(linebuf, sizeof(linebuf), stdin) != NULL) {
    ...
    if (DtSearchHasMessages()) {
      printf(
          "mmmmm Messages returned to user mmmmmmmmmmmmmmmmmm\n"
