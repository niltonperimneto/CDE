...
static void dumpboth(void) {
  ...
  for (d = usrblk.dblist; d != NULL; d = d->link) {
    ...
    *ptr = 0;
    printf(
        "DBLK at %p link=%p name='%s' max=%d\n"
        "    keytypes='%s', path='%s'\n",
        d, d->link, d->name, d->maxhits, typestring, NULLORSTR(d->path));
  }
...
static void dumpoef(char* before_after) {
  OEFTAB* oef;
  fprintf(aa_stderr, "\toef_table[] %s (* = changed strings)\n", before_after);
  for (oef = oef_table; oef->id != NULL; oef++) {
    fprintf(aa_stderr, "\t%10s = %c'%s'\n", oef->id, (oef->previously_specified) ? '*' : ' ',
            *(oef->OEFptr));
  }
  fputc('\n', aa_stderr);
  return;
...
/*					*/
/****************************************/
static void set_boolbit(long* flagvar, long mask, char* flagname, _Xstrtokparams* strtok_buf) {
  int i;
  ...
  if (i > 0)
    ...
    *flagvar &= ~mask; /* switch OFF */
    ...
    sprintf(sprintbufp,
            CATGETS(dtsearch_catd, MS_loadocf, 352, "%s %s: Invalid %s value, line ignored.\n"),
...
/*					*/
/****************************************/
/* static void set_int (int *intvar, char *keyword, _Xstrtokparams *strtok_buf) -- Unused */
...
/*					*/
/****************************************/
static void set_long(long* longvar, char* keyword, _Xstrtokparams* strtok_buf) {
  long longi;
  ...
  ERR_MSG:
    sprintf(sprintbufp,
            CATGETS(dtsearch_catd, MS_loadocf, 610,
                    "%s %s: %s value is '%s'. "
...
/*					*/
/****************************************/
static void set_boolint(int* boolint, char* keyword, _Xstrtokparams* strtok_buf) {
  int i;
...
static void set_fileio(_Xstrtokparams* strtok_buf) {
  ...
  if (strcmp(token, "-ON") == 0)
    ...
    sprintf(sprintbufp,
            CATGETS(dtsearch_catd, MS_loadocf, 607,
 ...
 */
static void read_rest_of_line(char* keyword, char** passed_ptr, _Xstrtokparams* strtok_buf) {
  if ((token = _XStrtok(NULL, "\n", *strtok_buf)) == NULL) {
    sprintf(sprintbufp, CATGETS(dtsearch_catd, MS_loadocf, 1007, "%s%s: Empty %s string ignored."),
...
static void read_filepath(_Xstrtokparams* strtok_buf) {
  ...
  IGNORE_FILEPATH:
    sprintf(sprintbufp,
            CATGETS(dtsearch_catd, MS_loadocf, 903, "%s %s: FILEPATH '%s' %s. Line ignored."),
  ...
  if ((token = _XStrtok(NULL, DELIMITERS, *strtok_buf)) == NULL) {
    ...
    errp = CATGETS(dtsearch_catd, MS_loadocf, 168, "missing path specification");
    goto IGNORE_FILEPATH;
  }
  toklen = strlen(token);
...
static void read_path(_Xstrtokparams* strtok_buf) {
  ...
  if ((token = _XStrtok(NULL, DELIMITERS, *strtok_buf)) == NULL) {
    sprintf(sprintbufp,
            CATGETS(dtsearch_catd, MS_loadocf, 221,
...
/* static void obsolete_keyword (char *keyword) -- Unused */
...
static void read_maxhits(_Xstrtokparams* strtok_buf) {
  ...
  INVALID_MAXHITS:
    sprintf(
        sprintbufp,
        CATGETS(dtsearch_catd, MS_loadocf, 304, "%s %s: Database '%s', invalid MAXHITS value.\n"),
...
static int add_a_keytype(char ktchar) {
  ...
  sprintf(kt[newindex].name, CATGETS(dtsearch_catd, MS_loadocf, 457, "'%c' Records"), ktchar);
...
GOT_FULLNAME:
  ...
  if (fptr == NULL) {
    if (errno != ENOENT || is_prespecified_fname) {
      sprintf(sprintbufp,
              CATGETS(dtsearch_catd, MS_loadocf, 414,
  ...
  if (!is_prespecified_fname) {
    OE_sitecnfg_fname = austext_malloc(strlen(fullname) + 2, PROGNAME "941", NULL);
    strcpy(OE_sitecnfg_fname, fullname);
  }
  if (fstat(fileno(fptr), &statbuf) == -1) {
    sprintf(sprintbufp,
            CATGETS(dtsearch_catd, MS_loadocf, 1404,
...
int load_ocf(void) {
  ...
  if (OE_sitecnfg_fname) {
    if (debug_ocf)
      fprintf(aa_stderr, PROGNAME "1446 ocfopen '%s', prespecified file name.\n",
              OE_sitecnfg_fname);
  ...
  sprintf(
      sprintbuf,
      CATGETS(dtsearch_catd, MS_loadocf, 1643,
      ...
      PROGNAME "1643 ", FNAME_SITECONFIG,
      (p) ? p : CATGETS(dtsearch_catd, MS_misc, 16, "<no environment variable>"), inbuf,
      (q) ? q : CATGETS(dtsearch_catd, MS_misc, 16, "<no environment variable>"),
      (OE_inittab_dir) ? OE_inittab_dir : CATGETS(dtsearch_catd, MS_misc, 17, "<no server daemon>"),
      (OE_server_dir) ? OE_server_dir : CATGETS(dtsearch_catd, MS_misc, 17, "<no server daemon>"));
...
OCFOPEN_OK:
  ...
  *inbuf = '\0';
  while (fgets(inbuf, sizeof(inbuf) - 1, stream) != NULL) {
    ...
    sprintf(sprintbufp,
            CATGETS(dtsearch_catd, MS_loadocf, 495, "%s %s: '%.16s' is invalid keyword.\n"),
  ...
  if (usrblk.dblist == NULL) {
    sprintf(sprintbufp,
            CATGETS(dtsearch_catd, MS_loadocf, 1869,
    ...
    for (db = usrblk.dblist; db != NULL; db = db->link) {
      if (db->ktcount <= 0) {
        sprintf(
            sprintbufp,
            CATGETS(dtsearch_catd, MS_loadocf, 986, "%s %s: KEYTYPES missing for database '%s'."),
