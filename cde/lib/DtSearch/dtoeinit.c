 ...
 */
/* static void dump_dblk (char *msgprefix, DBLK *d) -- Unused */
...
void oe_initialize(void) {
  ...
  for (oef = oef_table; oef->id != NULL; oef++) oef->previously_specified = FALSE;
  ...
  if (usrblk.request == OE_INITIALIZE) {
    if (!is_compatible_version(usrblk.query, SCHEMA_VERSION) ||
      ...
      sprintf(sprintbuf,
              CATGETS(dtsearch_catd, MS_oeinit, 137,
  ...
  if (!load_ocf()) {
    sprintf(sprintbuf,
            CATGETS(dtsearch_catd, MS_oeinit, 202,
  ...
  if (OE_flags & OE_AUDIT || usrblk.debug & USRDBG_RARE) {
    sprintf(sprintbuf, "ENGINEINIT='%s' UVER='%s' " AUDIT_WHOWHEN "\n", AUSAPI_VERSION,
            usrblk.query, usrblk.userid, nowstring(NULL));
    if (usrblk.debug & USRDBG_RARE)
  ...
  INIT_FAILED:
    sprintf(sprintbuf,
            CATGETS(dtsearch_catd, MS_oeinit, 266,
  ...
  if (good_dblk_count <= 0) {
    sprintf(sprintbuf, CATGETS(dtsearch_catd, MS_misc, 8, "%s No valid databases remain."),
            PROGNAME "265");
    DtSearchAddMessage(sprintbuf);
    goto INIT_FAILED;
  }
  ...
  if (usrblk.debug & USRDBG_RARE) {
    ...
    fprintf(aa_stderr,
            PROGNAME
            "666  Engine Initialization Completed.\n"
            "  usrblk.flags=%ld(x%04lx), usrblk.debug=%ld(x%04lx).\n"
            "  OE_flags=%ld(x%04lx), expiration=%s, sitecnfg=%s.\n",
            usrblk.flags, usrblk.flags, usrblk.debug, usrblk.debug, OE_flags, OE_flags, sprintbuf,
            sprintbuf + 100);
  }
