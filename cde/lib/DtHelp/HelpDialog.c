...
/* Canvas Engine */
/* #include "CanvasP.h" REMOVED */ /* for AccessI.h */
#include "bufioI.h"
...
static void NavigationTypeDefault(Widget widget, int offset, XrmValue *value);
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
                         Cardinal *num_args);
static void CloseHelpCB(Widget w, XtPointer clientData, XtPointer callData);
static void BuildDisplayArea(Widget parent, DtHelpDialogWidget nw);
static void ProcessJumpBack(DtHelpDialogWidget nw);
static void VariableInitialize(DtHelpDialogWidget nw);
static void CleanUpHelpDialog(Widget nw, int cleanUpKind);
static void CatchClose(Widget widget);
static void DisplayTopLevelCB(Widget w, XtPointer client_data,
                              XtPointer call_data);
static void CopyTextCB(Widget w, XtPointer clientData, XtPointer callData);
static void DisplayPrintCB(Widget w, XtPointer client_data,
                           XtPointer call_data);
static void ProcessBMenuBackCB(Widget w, XtPointer clientData,
                               XButtonEvent *event,
                               Boolean *continueToDispatch);
static void ProcessJumpReuse(Widget nw, DtHelpHyperTextStruct *hyperData);
static Widget SetupHelpDialogMenus(Widget parent);
static Widget CreatePopupMenu(Widget parent);
static void ResizeHelpDialogCB(XtPointer clientData);
static void InitialPopupCB(Widget w, XtPointer clientData, XtPointer callData);
...
static XtResource resources[] = {
    ...
    {DtNshowNewWindowButton, DtCShowNewWindowButton, XmRBoolean,
     sizeof(Boolean), XtOffset(DtHelpDialogWidget, help_dialog.menu.showDupBtn),
     XmRImmediate, (XtPointer)False},

    {DtNshowTopLevelButton, DtCShowTopLevelButton, XmRBoolean, sizeof(Boolean),
     XtOffset(DtHelpDialogWidget, help_dialog.browser.showTopLevelBtn),
     XmRImmediate, (XtPointer)False},

    {DtNscrollBarPolicy, DtCScrollBarPolicy, DtRDtScrollBarPolicy,
     sizeof(unsigned char),
     XtOffset(DtHelpDialogWidget, help_dialog.display.scrollBarPolicy),
     XmRImmediate, (XtPointer)DtHELP_AS_NEEDED_SCROLLBARS},

    {DtNexecutionPolicy, DtCExecutionPolicy, DtRDtExecutionPolicy,
     sizeof(unsigned char),
    ...
    {DtNrows, DtCRows, XmRShort, sizeof(short),
     XtOffset(DtHelpDialogWidget, help_dialog.display.textRows), XmRImmediate,
     (XtPointer)25},

    {DtNmarginWidth, DtCMarginWidth, XmRHorizontalDimension, sizeof(Dimension),
     XtOffset(DtHelpDialogWidget, help_dialog.ghelp.marginWidth), XmRImmediate,
     (XtPointer)1},

    {DtNmarginHeight, DtCMarginHeight, XmRVerticalDimension, sizeof(Dimension),
     XtOffset(DtHelpDialogWidget, help_dialog.ghelp.marginHeight), XmRImmediate,
     (XtPointer)4},

    {DtNvisiblePathCount, DtCVisiblePathCount, XmRInt, sizeof(int),
     XtOffset(DtHelpDialogWidget, help_dialog.browser.visiblePathCount),
     XmRImmediate, (XtPointer)7},

    {DtNlocationId, DtCLocationId, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.display.locationId), XmRImmediate,
     (XtPointer)_DtHelpDefaultLocationId},

    {DtNhelpPrint, DtCHelpPrint, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.print.helpPrint), XmRImmediate,
     (XtPointer)_DtHelpDefaultHelpPrint},

    {DtNprinter, DtCPrinter, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.print.printer), XmRImmediate,
     (XtPointer)NULL},

    {DtNpaperSize, DtCPaperSize, DtRDtPaperSize, sizeof(unsigned char),
     XtOffset(DtHelpDialogWidget, help_dialog.print.paperSize), XmRImmediate,
     (XtPointer)DtHELP_PAPERSIZE_LETTER},

    {DtNsrchHitPrefixFont, DtCSrchHitPrefixFont, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.srch.hitPrefixFont), XmRImmediate,
     (XtPointer)_DtHelpDefaultSrchHitPrefixFont},

    {DtNhelpVolume, DtCHelpVolume, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.display.helpVolume), XmRImmediate,
     (XtPointer)NULL},

    {DtNmanPage, DtCManPage, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.display.manPage), XmRImmediate,
     (XtPointer)NULL},

    {DtNstringData, DtCStringData, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.display.stringData), XmRImmediate,
     (XtPointer)NULL},

    {DtNhelpFile, DtCHelpFile, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.display.helpFile), XmRImmediate,
     (XtPointer)NULL},

    {DtNtopicTitle, DtCTopicTitle, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.display.topicTitleStr),
     XmRImmediate, (XtPointer)NULL},

    {DtNhelpOnHelpVolume, DtCHelpOnHelpVolume, XmRString, sizeof(char *),
     XtOffset(DtHelpDialogWidget, help_dialog.help.helpOnHelpVolume),
     XmRImmediate, (XtPointer)_DtHelpDefaultHelp4HelpVolume},

    {DtNhelpType, DtCHelpType, DtRDtHelpType, sizeof(unsigned char),
     XtOffset(DtHelpDialogWidget, help_dialog.display.helpType), XmRImmediate,
     (XtPointer)DtHELP_TYPE_TOPIC},

    {DtNhyperLinkCallback, DtCHyperLinkCallback, XmRCallback,
     sizeof(XtCallbackList),
     XtOffset(DtHelpDialogWidget, help_dialog.display.hyperLinkCallback),
     XmRImmediate, (XtPointer)NULL},

    {DtNcloseCallback, DtCCloseCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffset(DtHelpDialogWidget, help_dialog.ghelp.closeCallback),
     XmRImmediate, (XtPointer)NULL},

    {XmNnavigationType, XmCNavigationType, XmRNavigationType,
     sizeof(unsigned char), XtOffsetOf(XmManagerRec, manager.navigation_type),
...
externaldef(dthelpdialogwidgetclassrec)
    DtHelpDialogWidgetClassRec dtHelpDialogWidgetClassRec = {
        {
            /* core_class fields  */
...
static char *HelpTypeNames[] = {
    "help_type_topic", "help_type_string",         "help_type_man_page",
    "help_type_file",  "help_type_dynamic_string", "help_type_markdown_file"};

static char *ScrollBarValueNames[] = {"help_no_scrollbars",
                                      "help_static_scrollbars",
                                      "help_as_needed_scrollbars"};

static char *ExecutionValueNames[] = {
    "help_execute_none", "help_execute_query_all",
    "help_execute_query_unaliased", "help_execute_all"};
 ...
 *
 *********************************************************************/
static void NavigationTypeDefault(Widget widget, int offset, /* unused */
                                  XrmValue *value) {
 ...
 *
 *****************************************************************************/
static void ClassPartInitialize(WidgetClass widgetClass) { return; }
...
static void VariableInitialize(DtHelpDialogWidget nw) {
  ...
  if (hw->help_dialog.display.locationId != NULL)
    hw->help_dialog.display.locationId =
        XtNewString(hw->help_dialog.display.locationId);
  ...
  if (hw->help_dialog.display.manPage != NULL)
    hw->help_dialog.display.manPage =
        XtNewString(hw->help_dialog.display.manPage);
  if (hw->help_dialog.display.stringData != NULL)
    hw->help_dialog.display.stringData =
        XtNewString(hw->help_dialog.display.stringData);
  if (hw->help_dialog.display.helpFile != NULL)
    hw->help_dialog.display.helpFile =
        XtNewString(hw->help_dialog.display.helpFile);
  ...
  if (hw->help_dialog.display.topicTitleStr != NULL)
    ...
    hw->help_dialog.display.topicTitleStr =
        XtNewString((char *)_DTGETMESSAGE(2, 70, "Nonexistent Topic Title."));
  /*
   * create the XmString version
   */
  if (hw->help_dialog.display.topicTitleStr != NULL)
    hw->help_dialog.display.topicTitleLbl =
        XmStringCreateLocalized(hw->help_dialog.display.topicTitleStr);
  ...
  if (hw->help_dialog.print.helpPrint != _DtHelpDefaultHelpPrint)
    hw->help_dialog.print.helpPrint =
        XtNewString(hw->help_dialog.print.helpPrint);
 ...
 *
 *****************************************************************************/
static void Initialize(Widget rw, Widget nw, ArgList args_init,
                       Cardinal *num_args) {
  ...
  pHelpInfo =
      _DtHelpListAdd(DtHELP_dialogShell_STR, (Widget)hw, &hw->help_dialog.help,
                     &hw->help_dialog.help.pHelpListHead);
  XtAddCallback((Widget)hw, XmNhelpCallback, _DtHelpCB, (XtPointer)pHelpInfo);

  /* Just for fun, lets make sure our sizes are correct */
  XtAddCallback(XtParent(hw), XmNpopupCallback, (XtCallbackProc)InitialPopupCB,
                (XtPointer)hw);
...
static void InitialPopupCB(Widget w, XtPointer clientData, XtPointer callData) {
  ...
  _DtHelpResizeDisplayArea(XtParent(hw), hw->help_dialog.help.pDisplayArea,
                           hw->help_dialog.display.textRows,
                           hw->help_dialog.display.textColumns);

  XtRemoveCallback(XtParent(hw), XmNpopupCallback,
                   (XtCallbackProc)InitialPopupCB, (XtPointer)hw);
}
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
  ...
  if ((current->help_dialog.display.textRows !=
       hw->help_dialog.display.textRows) ||
      (current->help_dialog.display.textColumns !=
       hw->help_dialog.display.textColumns)) {
  ...
  if (current->help_dialog.display.helpVolume !=
      hw->help_dialog.display.helpVolume) {
    hw->help_dialog.display.helpVolume =
        XtNewString(hw->help_dialog.display.helpVolume);
  ...
  if (current->help_dialog.display.locationId !=
      hw->help_dialog.display.locationId) {
    hw->help_dialog.display.locationId =
        XtNewString(hw->help_dialog.display.locationId);
  ...
  if (current->help_dialog.display.stringData !=
      hw->help_dialog.display.stringData) {
    hw->help_dialog.display.stringData =
        XtNewString(hw->help_dialog.display.stringData);
  ...
  if (current->help_dialog.display.helpFile !=
      hw->help_dialog.display.helpFile) {
    hw->help_dialog.display.helpFile =
        XtNewString(hw->help_dialog.display.helpFile);
  ...
  if (current->help_dialog.display.helpType !=
 ...
 *
 *****************************************************************************/
static void MenuBarFix(XmGeoMatrix geoSpec, int action,
 ...
 *
 *****************************************************************************/
XmGeoMatrix _DtHelpDialogWidgetGeoMatrixCreate(Widget wid, Widget instigator,
                                               XtWidgetGeometry *desired) {
  ...
  geoSpec->margin_w =
      hw->help_dialog.ghelp.marginWidth + hw->manager.shadow_thickness;
  geoSpec->margin_h =
      hw->help_dialog.ghelp.marginHeight + hw->manager.shadow_thickness;
 ...
 *
 ****************************************************************************/
void _DtHelpDisplayBackCB(Widget w, XtPointer client_data,
                          XtPointer call_data) {
 ...
 *
 ****************************************************************************/
void _DtHelpDuplicateWindowCB(Widget w, XtPointer clientData,
                              XtPointer callData) {
  DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
  ...
  if (hw->help_dialog.display.hyperLinkCallback != NULL) {
    /* Use the old one */
    callDataStruct.helpVolume = XtNewString(hw->help_dialog.display.helpVolume);
    callDataStruct.reason = DtCR_HELP_LINK_ACTIVATE;
    callDataStruct.event = NULL; /* should be hyperData->event */
    callDataStruct.locationId = XtNewString(hw->help_dialog.display.locationId);
 ...
 *
 ****************************************************************************/
void _DtHelpDisplayHistoryCB(Widget w, XtPointer client_data,
                             XtPointer call_data) {
 ...
 *
 ****************************************************************************/
void _DtHelpDisplayBrowserHomeCB(Widget w, XtPointer client_data,
                                 XtPointer call_data) {
 ...
 *
 ****************************************************************************/
void _DtHelpDisplayIndexCB(Widget w, XtPointer client_data,
                           XtPointer call_data) {
  ...
  _DtHelpGlobSrchDisplayDialog((Widget)hw, NULL,
                               hw->help_dialog.display.helpVolume);
 ...
 *
 *****************************************************************************/
Widget DtCreateHelpDialog(Widget parent, char *name, ArgList al, Cardinal ac) {
  Widget w;
 ...
 *****************************************************************************/
int _DtHelpFilterExecCmdCB(void *clientData, const char *cmdStr,
  ...
  hv_path =
      _DtHelpFileLocate(DtHelpVOLUME_TYPE, hw->help_dialog.display.helpVolume,
                        _DtHelpFileSuffixList, False, R_OK);

  return _DtHelpFilterExecCmd(
      (Widget)hw, cmdStr, hw->help_dialog.display.executionPolicy, True,
      &hw->help_dialog.help, ret_filteredCmdStr, hv_path);
}
...
static void BuildDisplayArea(Widget parent, DtHelpDialogWidget nw) {
  ...
  n = 0;
  XtSetArg(args[n], XmNfontList, &(defaultList));
  ++n;
  XtGetValues(hw->help_dialog.menu.topBtn, args, n);

  /* Build the Display Area */
  hw->help_dialog.help.pDisplayArea = _DtHelpCreateDisplayArea(
      parent, "DisplayArea", ((short)hw->help_dialog.display.scrollBarPolicy),
      ((short)hw->help_dialog.display.scrollBarPolicy), False,
      ((int)hw->help_dialog.display.textRows),
      ((int)hw->help_dialog.display.textColumns), _DtHelpDialogHypertextCB,
      ResizeHelpDialogCB, _DtHelpFilterExecCmdCB, (XtPointer)hw, defaultList);
 ...
 *
 *****************************************************************************/
void _DtHelpSetupDisplayType(DtHelpDialogWidget nw, Boolean vol_changed,
                             int updateType) {
  char *templocationId;
  ...
  case DtHELP_TYPE_TOPIC:
    ...
    else if (hw->help_dialog.display.helpVolume != NULL)
      validAccessPath = _DtHelpExpandHelpVolume(
          (Widget)hw, &hw->help_dialog.display, &hw->help_dialog.help,
    ...
    if (validAccessPath) {
      ...
      if (hw->help_dialog.display.helpVolume == NULL) {
        ...
        tmpError = XtNewString(
            (char *)_DTGETMESSAGE(2, 60, "No help volume specified."));
        _DtHelpDisplayFormatError(hw->help_dialog.help.pDisplayArea, (Widget)hw,
                                  tmpError, NULL);

        _DtHelpUpdateJumpList(tmpError, DtHELP_TYPE_DYNAMIC_STRING, (Widget)hw);
         ...
         */
        didWeHaveAnError = TRUE;
        tmpMessage = XtNewString((char *)_DTGETMESSAGE(
            2, 58,
        ...
        locTitle = XtNewString((char *)_DTGETMESSAGE(2, 62, "Location ID:"));
        volumeTitle = XtNewString((char *)_DTGETMESSAGE(2, 61, "Help Volume:"));
        tmpError = XtMalloc(strlen(tmpMessage) + strlen(locTitle) +
                            strlen(volumeTitle) +
  ...
  case DtHELP_TYPE_STRING:
    ...
    if (status >= VALID_STATUS) {
      ...
      _DtHelpDisplayAreaSetList(hw->help_dialog.help.pDisplayArea, topicHandle,
                                FALSE, hw->help_dialog.backtr.scrollPosition);
    } else {
      didWeHaveAnError = TRUE;
      tmpError = XtNewString(
          (char *)_DTGETMESSAGE(2, 50, "String data could not be formatted."));
  ...
  case DtHELP_TYPE_DYNAMIC_STRING:
    ...
    /* Set the string to the current help dialog */
    status = _DtHelpFormatAsciiStringDynamic(hw->help_dialog.help.pDisplayArea,
                                             hw->help_dialog.display.stringData,
                                             &topicHandle);
    ...
    if (status >= VALID_STATUS) {
      ...
      didWeHaveAnError = TRUE;
      tmpError = XtNewString((char *)_DTGETMESSAGE(
          2, 51, "Dynamic string data could not be formatted."));
  ...
  case DtHELP_TYPE_MAN_PAGE:
    ...
    /* Set the string to the current help dialog */
    status =
        _DtHelpFormatManPage(hw->help_dialog.help.pDisplayArea,
    ...
    if (status >= VALID_STATUS) {
      ...
      tmpError = XtNewString((char *)_DTGETMESSAGE(
          2, 52,
          "Man Page could not be formatted. The requested Man Page is either "
  ...
  case DtHELP_TYPE_MARKDOWN_FILE:
    /*
     * Call the Markdown formatting routine
     */
    _DtHelpUpdatePathArea(NULL, hw);

    status = _DtHelpFormatMarkdownFile(
        (XtPointer)hw->help_dialog.help.pDisplayArea,
    ...
    if (status >= VALID_STATUS) {
      ...
      if (updateType == DtHISTORY_AND_JUMP) {
        _DtHelpUpdateJumpList(hw->help_dialog.display.helpFile,
                              DtHELP_TYPE_MARKDOWN_FILE, (Widget)hw);

        _DtHelpUpdateHistoryList(hw->help_dialog.display.helpFile,
                                 DtHELP_TYPE_MARKDOWN_FILE, TRUE, (Widget)hw);
        _DtHelpDisplayAreaSetList(hw->help_dialog.help.pDisplayArea,
                                  topicHandle, FALSE,
                                  hw->help_dialog.backtr.scrollPosition);
      }
      ...
      didWeHaveAnError = TRUE;
      /* Use generic error or specific one if defined */
      tmpError = XtNewString("Markdown file could not be formatted.");
  ...
  case DtHELP_TYPE_FILE:
    ...
    /* Set the string to the current help dialog */
    status =
        _DtHelpFormatAsciiFile(hw->help_dialog.help.pDisplayArea,
                               hw->help_dialog.display.helpFile, &topicHandle);
    ...
    if (status >= VALID_STATUS) {
      ...
      tmpError = XtNewString((char *)_DTGETMESSAGE(
          2, 53,
          "Text file data could not be formatted. The requested text file is "
          "either not present, or corrupt."));
  ...
  default:
    ...
    didWeHaveAnError = TRUE;
    tmpError = XtNewString(
        (char *)_DTGETMESSAGE(2, 80, "The specified help type is invalid."));
  ...
  _DtHelpUpdatePrintDialog(&hw->help_dialog.print, &hw->help_dialog.display,
                           &hw->help_dialog.help, False);
 ...
 *
 *****************************************************************************/
void _DtHelpUpdateDisplayArea(char *locationId, DtHelpDialogWidget nw,
  ...
  if (locationId == NULL && hw->help_dialog.help.topLevelId != NULL) {
  ...
  if (locationId != NULL) {
    ...
    status = _DtHelpFormatTopic(hw->help_dialog.help.pDisplayArea,
                                hw->help_dialog.display.volumeHandle,
                                locationId, True, &topicHandle);
  ...
  if (status == 0) /* success */
  {
    if (hw->help_dialog.ghelp.volumeFlag == FALSE) {
      ...
      if (labelString == NULL && NULL != hw->help_dialog.display.helpVolume)
        labelString =
            XmStringCreateLocalized(hw->help_dialog.display.helpVolume);
    ...
    if (listUpdateType == DtHISTORY_AND_JUMP) {
      _DtHelpUpdateHistoryList(locationId, topicUpdateType, vol_changed,
                               (Widget)hw);
  ...
  if (status < 0) {
    ...
    if (!validlocationId) {
      ...
      if (locationId == NULL) {
        tmpMsg = (char *)_DTGETMESSAGE(2, 59, "No location ID specified.");
        userErrorStr = XtNewString(tmpMsg);
      } else {
        tmpMsg = (char *)_DTGETMESSAGE(2, 55, "Nonexistent location ID:");
      ...
      tmpMsg =
          (char *)_DTGETMESSAGE(2, 54, "Help topic could not be formatted.");
      userErrorStr = XtNewString(tmpMsg);
      sysErrorStr = XtNewString(HDMessage5);
    }

    _DtHelpUpdateJumpList(userErrorStr, DtHELP_TYPE_DYNAMIC_STRING, (Widget)hw);
 ...
 *
 *****************************************************************************/
void _DtHelpUpdateJumpList(char *topicInfo, int topicType, Widget nw) {
  DtHelpDialogWidget hw = (DtHelpDialogWidget)nw;
  ...
  _DtHelpTopicListAddToHead(topicInfo, hw->help_dialog.display.topicTitleLbl,
                            ...
                            &hw->help_dialog.backtr.totalJumpNodes, -1);
 ...
 *
 ****************************************************************************/
void _DtHelpDialogHypertextCB(DtHelpDispAreaStruct *pDisplayAreaStruct,
                              XtPointer clientData,
                              DtHelpHyperTextStruct *hyperData) {
  ...
  case _DtCvLinkType_CrossLink:
    ...
    case _DtCvWindowHint_NewWindow:
      ...
      if (hw->help_dialog.display.hyperLinkCallback != NULL) {
        ...
        if (pTempAccessPath != NULL) /* Use the New one */
          ...
          callData.helpVolume = XtNewString(hw->help_dialog.display.helpVolume);

        callData.reason = DtCR_HELP_LINK_ACTIVATE;
        callData.event = hyperData->event;
        callData.locationId = _DtHelpParseIdString(hyperData->specification);
    ...
    case _DtCvWindowHint_PopupWindow:
      ...
      _DtHelpDisplayDefinitionBox(
          (Widget)hw, (Widget **)&(hw->help_dialog.ghelp.definitionBox),
  ...
  case _DtCvLinkType_Execute:
    _DtHelpExecFilteredCmd((Widget)hw, hyperData->specification,
                           DtHELP_ExecutionPolicy_STR, &hw->help_dialog.display,
                           &hw->help_dialog.help);
    break;
  ...
  case _DtCvLinkType_TextFile:
    ...
    if (hw->help_dialog.display.hyperLinkCallback != NULL) {
      ...
      if (hyperData->hyper_type == DtHELP_LINK_APP_DEFINE) {
        tmpErrorMsg = XtNewString(
            (char *)_DTGETMESSAGE(2, 56,
                                  "The selected Hypertext link is not "
                                  "supported within this application."));
        _DtHelpErrorDialog(XtParent(hw), tmpErrorMsg);
      } else {
        tmpErrorMsg = XtNewString((char *)_DTGETMESSAGE(
            2, 57,
 ...
 *
 *****************************************************************************/
void _DtHelpTopicListFree(DtTopicListStruct *pHead) {
  DtTopicListStruct *pTemp = NULL;
...
static void CatchClose(Widget widget) {
  ...
  wm_delete_window =
      XmInternAtom(XtDisplay(XtParent(widget)), "WM_DELETE_WINDOW", FALSE);
...
static Widget SetupHelpDialogMenus(Widget parent) {
  ...
  pHelpInfo =
      _DtHelpListAdd(DtHELP_FileMenu_STR, (Widget)hw, &hw->help_dialog.help,
                     &hw->help_dialog.help.pHelpListHead);
  XtAddCallback((Widget)menupane1, XmNhelpCallback, _DtHelpCB,
                (XtPointer)pHelpInfo);
  ...
  n++;
  c[0] = hw->help_dialog.menu.printBtn =
      XmCreatePushButtonGadget(menupane1, "print", args, n);
  ...
  if (hw->help_dialog.menu.showDupBtn == True) {
    /* Duplicate button */
    labelStr =
        XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 5, "New Window...")));
  ...
  pHelpInfo =
      _DtHelpListAdd(DtHELP_EditMenu_STR, (Widget)hw, &hw->help_dialog.help,
                     &hw->help_dialog.help.pHelpListHead);
  XtAddCallback((Widget)menupane2, XmNhelpCallback, _DtHelpCB,
                (XtPointer)pHelpInfo);
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[0] = hw->help_dialog.menu.copyBtn =
      XmCreatePushButtonGadget(menupane2, "copy", args, n);
  XtAddCallback(hw->help_dialog.menu.copyBtn, XmNactivateCallback, CopyTextCB,
                (XtPointer)hw);
  ...
  pHelpInfo =
      _DtHelpListAdd(DtHELP_SearchMenu_STR, (Widget)hw, &hw->help_dialog.help,
                     &hw->help_dialog.help.pHelpListHead);
  XtAddCallback((Widget)menupane3, XmNhelpCallback, _DtHelpCB,
                (XtPointer)pHelpInfo);
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[0] = hw->help_dialog.menu.keyBtn =
      XmCreatePushButtonGadget(menupane3, "keyword", args, n);
  ...
  pHelpInfo =
      _DtHelpListAdd(DtHELP_NavigateMenu_STR, (Widget)hw, &hw->help_dialog.help,
                     &hw->help_dialog.help.pHelpListHead);
  XtAddCallback((Widget)menupane4, XmNhelpCallback, _DtHelpCB,
                (XtPointer)pHelpInfo);

  /* Back Button */
  labelStr =
      XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 13, "Backtrack")));
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[0] = hw->help_dialog.menu.backBtn =
      XmCreatePushButtonGadget(menupane4, "backTrack", args, n);
  ...
  /* Top Button */
  labelStr =
      XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 11, "Home Topic")));
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[1] = hw->help_dialog.menu.topBtn =
      XmCreatePushButtonGadget(menupane4, "homeTopic", args, n);
  ...
  /* History Button */
  labelStr =
      XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 15, "History...")));
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[3] = hw->help_dialog.menu.historyBtn =
      XmCreatePushButtonGadget(menupane4, "history", args, n);
  ...
  pHelpInfo =
      _DtHelpListAdd(DtHELP_HelpMenu_STR, (Widget)hw, &hw->help_dialog.help,
                     &hw->help_dialog.help.pHelpListHead);
  XtAddCallback((Widget)menupane5, XmNhelpCallback, _DtHelpCB,
                (XtPointer)pHelpInfo);

  /* On HELP button */
  labelStr = XmStringCreateLocalized(
      ((char *)_DTGETMESSAGE(2, 17, "Overview - Using Help")));
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[0] = helpBtn1 = XmCreatePushButtonGadget(menupane5, "usingHelp", args, n);
  pHelpInfo =
      _DtHelpListAdd(DtHELP_onHelpMenu_STR, (Widget)hw, &hw->help_dialog.help,
                     &hw->help_dialog.help.pHelpListHead);
  XtAddCallback(helpBtn1, XmNactivateCallback, _DtHelpCB, (XtPointer)pHelpInfo);
  ...
  labelStr = XmStringCreateLocalized(
      ((char *)_DTGETMESSAGE(2, 19, "Table Of Contents")));
  ...
  n++;
  c[2] = helpBtn2 =
      XmCreatePushButtonGadget(menupane5, "tableOfContents", args, n);
  pHelpInfo = _DtHelpListAdd(DtHELP_onTableOfContenseMenu_STR, (Widget)hw,
                             &hw->help_dialog.help,
                             &hw->help_dialog.help.pHelpListHead);
  XtAddCallback(helpBtn2, XmNactivateCallback, _DtHelpCB, (XtPointer)pHelpInfo);
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[3] = helpBtn3 = XmCreatePushButtonGadget(menupane5, "tasks", args, n);
  pHelpInfo =
      _DtHelpListAdd(DtHELP_onTasksMenu_STR, (Widget)hw, &hw->help_dialog.help,
                     &hw->help_dialog.help.pHelpListHead);
  XtAddCallback(helpBtn3, XmNactivateCallback, _DtHelpCB, (XtPointer)pHelpInfo);
  XmStringFree(labelStr);

  /* On Reference button */
  labelStr =
      XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 23, "Reference")));
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[4] = helpBtn4 = XmCreatePushButtonGadget(menupane5, "reference", args, n);
  pHelpInfo = _DtHelpListAdd(DtHELP_onReferenceMenu_STR, (Widget)hw,
                             &hw->help_dialog.help,
                             &hw->help_dialog.help.pHelpListHead);
  XtAddCallback(helpBtn4, XmNactivateCallback, _DtHelpCB, (XtPointer)pHelpInfo);
  XmStringFree(labelStr);

  /* On Mouse & Keyboard button */
  labelStr = XmStringCreateLocalized(
      ((char *)_DTGETMESSAGE(2, 25, "Mouse and Keyboard")));
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[5] = helpBtn5 = XmCreatePushButtonGadget(menupane5, "reference", args, n);
  pHelpInfo = _DtHelpListAdd(DtHELP_onMouseAndKeysMenu_STR, (Widget)hw,
                             &hw->help_dialog.help,
                             &hw->help_dialog.help.pHelpListHead);
  XtAddCallback(helpBtn5, XmNactivateCallback, _DtHelpCB, (XtPointer)pHelpInfo);
  XmStringFree(labelStr);
  ...
  /* On Version */
  labelStr =
      XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 27, "About Help")));
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[7] = helpBtn6 = XmCreatePushButtonGadget(menupane5, "reference", args, n);
  pHelpInfo = _DtHelpListAdd(DtHELP_onVersionMenu_STR, (Widget)hw,
                             &hw->help_dialog.help,
                             &hw->help_dialog.help.pHelpListHead);
  XtAddCallback(helpBtn6, XmNactivateCallback, _DtHelpCB, (XtPointer)pHelpInfo);
  XmStringFree(labelStr);
  ...
  /* Navigate Menu */
  labelStr =
      XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 33, "Navigate")));
  mnemonic = ((char *)_DTGETMESSAGE(2, 34, "N"));
  n = 0;
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  XtSetArg(args[n], XmNsubMenuId, menupane4);
  n++;
  XtSetArg(args[n], XmNlabelString, labelStr);
  n++;
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  XtSetArg(args[n], XmNsubMenuId, menupane5);
  n++;
  XtSetArg(args[n], XmNlabelString, labelStr);
  n++;
  ...
  n = 0;
  XtSetArg(args[n], XmNmenuHelpWidget, cascadeB5);
  n++;
 ...
 *
 **********************************************************************/
static Widget CreatePopupMenu(Widget nw) {
  Widget popup;
  Widget c[10];
  Arg args[10];
  int n;
  ...
  popup = XmCreatePopupMenu(
      _DtHelpDisplayAreaWidget(hw->help_dialog.help.pDisplayArea), "popup",
      args, n);

  /* Add our button three event handler to our newly created display area */
  XtAddEventHandler(_DtHelpDisplayAreaWidget(hw->help_dialog.help.pDisplayArea),
                    ButtonPressMask, FALSE, (XtEventHandler)ProcessBMenuBackCB,
                    hw);

  /* Add Help callback for Popup Menu */
  pHelpInfo =
      _DtHelpListAdd(DtHELP_PopupMenu_STR, (Widget)hw, &hw->help_dialog.help,
                     &hw->help_dialog.help.pHelpListHead);
  XtAddCallback((Widget)popup, XmNhelpCallback, _DtHelpCB,
                (XtPointer)pHelpInfo);
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[2] = hw->help_dialog.menu.popupBackBtn =
      XmCreatePushButtonGadget(popup, "backTrack", args, n);
  XmStringFree(labelStr);
  ...
  XtSetArg(args[n], XmNmnemonic, mnemonic[0]);
  n++;
  c[3] = hw->help_dialog.menu.popupTopBtn =
      XmCreatePushButtonGadget(popup, "homeTopic", args, n);
  XmStringFree(labelStr);
