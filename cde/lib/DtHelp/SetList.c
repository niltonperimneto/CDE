 ...
 *
 *****************************************************************************/
static void SetMaxPositions(DtHelpDispAreaStruct *pDAS, _DtCvUnit max_x,
                            _DtCvUnit max_y) {
  pDAS->maxX = max_x;
...
Boolean _DtHelpSetScrollBars(XtPointer client_data, Dimension new_width,
  ...
  do {
    ...
    if (new_height != pDAS->dispHeight) {
      ...
      pDAS->visibleCount = ((int)pDAS->dispUseHeight) / pDAS->lineHeight;
    ...
    if (new_width != pDAS->dispWidth) {
       ...
       */
      if (_DtCvSTATUS_OK ==
          _DtCanvasResize(pDAS->canvas, _DtCvFALSE, &ret_width, &ret_height))
  ...
  if (oldWidth != pDAS->dispWidth || oldHeight != pDAS->dispHeight) {
     ...
     */
    {
      XtSetArg(dispArgs[dispN], XmNwidth, pDAS->dispWidth);
      ++dispN;
      XtSetArg(dispArgs[dispN], XmNheight, pDAS->dispHeight);
      ++dispN;
    }
  ...
  if (pDAS->vertScrollWid && pDAS->vertIsMapped && doVert != DoUnmanage) {
    ...
    XtSetArg(args[n], XmNvalue, pDAS->firstVisible);
    ++n;
    XtSetArg(args[n], XmNsliderSize, slideSize);
    ++n;
    XtSetArg(args[n], XmNmaximum, maximum);
    ++n;
    XtSetArg(args[n], XmNpageIncrement, pageInc);
    ++n;
  ...
  if (pDAS->horzScrollWid && pDAS->horzIsMapped && doHorz != DoUnmanage) {
    ...
    XtSetArg(args[n], XmNvalue, pDAS->virtualX);
    n++;
    XtSetArg(args[n], XmNsliderSize, slideSize);
    n++;
    XtSetArg(args[n], XmNmaximum, maximum);
 ...
 *
 *********************************************************************/
void _DtHelpDisplayAreaSetList(XtPointer client_data, XtPointer topic_handle,
   ...
   */
  /*
   * IN THE RUST ENGINE WORLD, WE DO NOT CALL _DtCanvasSetTopic.
   * The text is set directly via dthelp_engine_set_text (which is done in
   * _DtHelpUpdatePath or by the caller). For now, we assume the content is
   * already set or will be set.
   */
  /* _DtCanvasSetTopic REMOVED */
  width = 0;
  height = 0;
  scrollY = 0; /* Default values */
...
/*****************************************************************************
 * Function: void _DtHelpDisplayAreaDimensionsReturn (DtHelpDispAreaStruct
 * *pDAS, int *ret_width,	int *ret_height)
 *
 ...
 *
 *****************************************************************************/
void _DtHelpDisplayAreaDimensionsReturn(XtPointer client_data, short *ret_rows,
                                        short *ret_columns) {
  DtHelpDispAreaStruct *pDAS = (DtHelpDispAreaStruct *)client_data;
 ...
 *
 *****************************************************************************/
int _DtHelpGetScrollbarValue(XtPointer client_data) {
  DtHelpDispAreaStruct *pDAS = (DtHelpDispAreaStruct *)client_data;
 ...
 *
 *****************************************************************************/
int _DtHelpUpdatePath(DtHelpDispAreaStruct *pDAS,
  ...
  /* _DtCanvasMoveTraversal REMOVED */
  ...
  if (pDAS->rust_engine) {
    ...
    /* LEGACY PATH */
    result =
        _DtHelpFormatToc(pDAS, volume_handle, loc_id, &realId, &topic_handle);
    if (-1 == result)
      return -1;

    _DtCanvasSetTopic(pDAS->canvas, (_DtCvTopicPtr)topic_handle,
                      pDAS->honor_size, &width, &height, NULL);
  }

  /* if (pDAS->rust_engine) block removed here as it is moved above */
  ...
  if (1 != result) {
    (void)_DtCanvasMoveTraversal(pDAS->canvas, _DtCvTRAVERSAL_ID, False,
                                 ((_DtCvStatus)XtIsRealized(pDAS->dispWid)),
                                 realId, NULL, &(pDAS->toc_y),
                                 &(pDAS->toc_base), &(pDAS->toc_height));
   ...
   * Reset the scroll bars and perhaps do an expose.
   */
  (void)_DtHelpSetScrollBars((XtPointer)pDAS, pDAS->dispWidth,
                             pDAS->dispHeight);
  if (XtIsRealized(pDAS->dispWid))
 ...
 *
 *****************************************************************************/
Widget _DtHelpDisplayAreaWidget(XtPointer client_data) {
  DtHelpDispAreaStruct *pDAS = (DtHelpDispAreaStruct *)client_data;
 ...
 *
 *****************************************************************************/
XtPointer _DtHelpDisplayAreaData(XtPointer client_data) {
  DtHelpDispAreaStruct *pDAS = (DtHelpDispAreaStruct *)client_data;
