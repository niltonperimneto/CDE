...
static Boolean get_fontsetproperty(XFontSet fontset, Atom atom,
                                   unsigned long *value_return);
/********    End Private Function Declarations    ********/

/********    Private Variables    ********/

static _DtCvVirtualInfo DefVirtFunctions = {NULL, NULL, NULL, NULL, NULL, NULL};
...
static XtResource Resources[] = {
    {"leading", "Leading", XmRInt, sizeof(int), XtOffset(DAArgsPtr, leading),
 ...
 * font.
 */
static void GetUnderLineInfo(Display *dpy, DtHelpDispAreaStruct *pDAS,
                             int *ret_underThick) {
  int idx;
  XtPointer myFont;
  ...
  if (idx < 0) {
    ...
    if (get_fontsetproperty((XFontSet)myFont, XA_UNDERLINE_POSITION,
      ...
      pDAS->underLine =
          ((int)(extents->max_ink_extent.height + extents->max_ink_extent.y)) /
          2;
    }
  } else {
 ...
 *********************************************************************/
static XtPointer
HelpCreateDA(Widget parent, char *name, short vert_flag, short horz_flag,
             Boolean traversal_flag, _DtCvValue honor_size,
             _DtCvRenderType render_type, int rows, int columns,
             unsigned short media_resolution,
             void (*hyperTextCB)(DtHelpDispAreaStruct *, void *,
  ...
  pDAS->rust_engine = dthelp_engine_new();

  /*
   * get the resources
   */
  XtGetSubresources(parent, &DA_args, name, "XmDrawnButton", Resources,
  ...
  if (1 < MB_CUR_MAX)
    _DtHelpLoadMultiInfo(&(pDAS->cant_begin_chars), &(pDAS->cant_end_chars),
                         &(pDAS->nl_to_space));
  ...
  __DtHelpFontMetrics(pDAS->font_info, __DtHelpDefaultFontIndexGet(pDAS),
                      &maxFontAscent, &maxFontDescent, &maxFontCharWidth, NULL,
                      NULL);
  ...
  if (n < 0)
    value = get_fontsetproperty(__DtHelpFontSetGet(pDAS->font_info, n),
                                xa_ave_width,
                                ((unsigned long *)&(pDAS->charWidth)));
  else
    value =
        XGetFontProperty(__DtHelpFontStructGet(pDAS->font_info, n),
  ...
  if (False == value || 0 == pDAS->charWidth) {
    ...
    if (n < 0)
      len += XmbTextEscapement(__DtHelpFontSetGet(pDAS->font_info, n), "1", 1);
    else
   ...
   */
  n = 0;
  XtSetArg(args[n], XmNmarginWidth, &margin_width);
  ++n;
  XtSetArg(args[n], XmNmarginHeight, &margin_height);
  ++n;
  ...
  if (vert_flag != _DtHelpNONE) {
    ...
    ++n;
    XtSetArg(args[n], XmNbottomOffset, margin_height);
    ++n;
    ...
    ++n;
    XtSetArg(args[n], XtNmappedWhenManaged, pDAS->vertIsMapped);
    ++n;
    pDAS->vertScrollWid =
        XmCreateScrollBar(form, "DisplayDtHelpVertScrollBar", args, n);

    XtManageChild(pDAS->vertScrollWid);
    if (vert_flag != _DtHelpSTATIC)
      pDAS->neededFlags =
          _DtHelpSET_AS_NEEDED(pDAS->neededFlags, _DtHelpVERTICAL_SCROLLBAR);

    XtAddCallback(pDAS->vertScrollWid, XmNdragCallback, _DtHelpVertScrollCB,
                  (XtPointer)pDAS);
    ...
    XtAddCallback(pDAS->vertScrollWid, XmNpageDecrementCallback,
                  _DtHelpVertScrollCB, (XtPointer)pDAS);
    XtAddCallback(pDAS->vertScrollWid, XmNtoBottomCallback, _DtHelpVertScrollCB,
                  (XtPointer)pDAS);
    XtAddCallback(pDAS->vertScrollWid, XmNtoTopCallback, _DtHelpVertScrollCB,
                  (XtPointer)pDAS);
  ...
  if (horz_flag != _DtHelpNONE) {
    ...
    XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
    ++n;
    XtSetArg(args[n], XmNrightOffset, margin_width);
    ++n;
    ...
    XtSetArg(args[n], XmNminimum, 0);
    ++n;
    XtSetArg(args[n], XmNmaximum, (pDAS->charWidth / 10));
    ++n;
    /* fake out the scrollbar manager, who will init dims to 100 */
    XtSetArg(args[n], XmNwidth, 1);
    ++n;
    XtSetArg(args[n], XmNincrement, (pDAS->charWidth / 10));
    ++n;
    XtSetArg(args[n], XmNpageIncrement, (pDAS->charWidth / 10));
    ++n;
    XtSetArg(args[n], XmNsliderSize, (pDAS->charWidth / 10));
    ++n;
    XtSetArg(args[n], XtNmappedWhenManaged, pDAS->horzIsMapped);
    ++n;
    pDAS->horzScrollWid =
        XmCreateScrollBar(form, "DisplayHorzScrollBar", args, n);
    XtManageChild(pDAS->horzScrollWid);
    if (horz_flag != _DtHelpSTATIC)
      pDAS->neededFlags =
          _DtHelpSET_AS_NEEDED(pDAS->neededFlags, _DtHelpHORIZONTAL_SCROLLBAR);

    XtAddCallback(pDAS->horzScrollWid, XmNdragCallback, _DtHelpHorzScrollCB,
                  (XtPointer)pDAS);
    ...
    XtAddCallback(pDAS->horzScrollWid, XmNpageDecrementCallback,
                  _DtHelpHorzScrollCB, (XtPointer)pDAS);
    XtAddCallback(pDAS->horzScrollWid, XmNtoBottomCallback, _DtHelpHorzScrollCB,
                  (XtPointer)pDAS);
    XtAddCallback(pDAS->horzScrollWid, XmNtoTopCallback, _DtHelpHorzScrollCB,
                  (XtPointer)pDAS);
  ...
  if (horz_flag == _DtHelpSTATIC) {
    ...
    ++n;
    XtSetArg(args[n], XmNbottomWidget, pDAS->horzScrollWid);
    ++n;
  } else {
    XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
    ++n;
  ...
  if (vert_flag == _DtHelpSTATIC) {
    ...
    ++n;
    XtSetArg(args[n], XmNrightWidget, pDAS->vertScrollWid);
    ++n;
  } else {
    XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
    ++n;
  ...
  ++n;
  XtSetArg(args[n], XmNuserData, pDAS);
  ++n;
  ...
  */
   ...
   */
  XtAddCallback(pDAS->dispWid, XmNdestroyCallback, _DtHelpDisplayAreaDestroyCB,
                (XtPointer)pDAS);

  n = 0;
  XtSetArg(args[n], XmNshadowThickness, &shadowThick);
  ++n;
  XtSetArg(args[n], XmNhighlightThickness, &highThick);
  ++n;
  XtSetArg(args[n], XmNforeground, &(pDAS->foregroundColor));
  ++n;
  XtSetArg(args[n], XmNbackground, &(pDAS->backgroundColor));
  ++n;
  XtSetArg(args[n], XmNhighlightColor, &(pDAS->traversalColor));
  ++n;
  XtSetArg(args[n], XmNdepth, &(pDAS->depth));
  ++n;
  ...
  gcMask =
      (GCFunction | GCPlaneMask | GCForeground | GCBackground | GCLineWidth |
       GCLineStyle | GCClipXOrigin | GCClipYOrigin | GCClipMask);
  ...
  pDAS->normalGC = XCreateGC(dpy, RootWindowOfScreen(XtScreen(pDAS->dispWid)),
                             gcMask, &gcValues);

  gcMask &= (~GCTile);
  pDAS->pixmapGC = XCreateGC(dpy, RootWindowOfScreen(XtScreen(pDAS->dispWid)),
                             gcMask, &gcValues);

  gcValues.foreground = pDAS->backgroundColor;
  gcValues.background = pDAS->foregroundColor;
  pDAS->invertGC = XCreateGC(dpy, RootWindowOfScreen(XtScreen(pDAS->dispWid)),
                             gcMask, &gcValues);
  ...
  if (NULL != pDAS->horzScrollWid) {
    n = 0;
    XtSetArg(args[n], XmNheight, &hBarHeight);
    n++;
    XtSetArg(args[n], XmNinitialDelay, &(pDAS->horz_init_scr));
    n++;
    XtSetArg(args[n], XmNrepeatDelay, &(pDAS->horz_rep_scr));
    n++;
  ...
  if (NULL != pDAS->vertScrollWid) {
    n = 0;
    XtSetArg(args[n], XmNwidth, &vBarWidth);
    n++;
    XtSetArg(args[n], XmNinitialDelay, &(pDAS->vert_init_scr));
    n++;
    XtSetArg(args[n], XmNrepeatDelay, &(pDAS->vert_rep_scr));
    n++;
  ...
  pDAS->formWidth = 0;
  pDAS->formHeight = 0;
  XtSetArg(args[n], XmNwidth, pDAS->dispWidth);
  ++n;
  XtSetArg(args[n], XmNheight, pDAS->dispHeight);
  ++n;
  ...
  if (pDAS->vertScrollWid != NULL) {
    n = 0;
    XtSetArg(args[n], XmNmaximum, pDAS->dispUseHeight);
    ++n;
    XtSetArg(args[n], XmNincrement, pDAS->lineHeight);
    ++n;
    XtSetArg(args[n], XmNpageIncrement, pDAS->lineHeight);
    ++n;
    XtSetArg(args[n], XmNsliderSize, pDAS->dispUseHeight);
    ++n;
  ...
  XSetClipRectangles(XtDisplay(pDAS->dispWid), pDAS->normalGC, 0, 0, rectangle,
                     1, Unsorted);
  XSetClipRectangles(XtDisplay(pDAS->dispWid), pDAS->invertGC, 0, 0, rectangle,
                     1, Unsorted);
  ...
  n = 0;
  XtSetArg(args[n], XmNcolormap, &(pDAS->colormap));
  n++;
  XtSetArg(args[n], XmNvisual, &(pDAS->visual));
  n++;
...
_DtHelpCreateDisplayArea(Widget parent, char *name, short vert_flag,
                         short horz_flag, Boolean traversal_flag, int rows,
                         int columns,
                         void (*hyperTextCB)(DtHelpDispAreaStruct *, void *,
                                             DtHelpHyperTextStruct *),
                         void (*resizeCB)(void *),
                         int (*exec_ok_routine)(void *, const char *, char **),
                         XtPointer client_data, XmFontList default_list) {
  return HelpCreateDA(parent, name, vert_flag, horz_flag, traversal_flag,
                      _DtCvIGNORE_BOUNDARY, _DtCvRENDER_PARTIAL, rows, columns,
                      100, hyperTextCB, resizeCB, exec_ok_routine, client_data,
                      default_list);
}
 ...
 *
 *********************************************************************/
XtPointer _DtHelpCreateOutputArea(
    Widget parent, char *name, short vert_flag, short horz_flag,
    Boolean traversal_flag, _DtCvValue honor_size, _DtCvRenderType render_type,
  ...
  pDAS =
      HelpCreateDA(parent, name, vert_flag, horz_flag, traversal_flag,
                   honor_size, render_type, 1, 1, media_resolution, hyperTextCB,
                   resizeCB, exec_ok_routine, client_data, default_list);
