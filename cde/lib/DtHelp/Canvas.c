...
static _DtCvUnit DrawText(_DtCanvasStruct *canvas, _DtCvDspLine *line,
                          int txt_line, _DtCvFlags old_flag,
                          _DtCvFlags new_flag);
 ...
 *
 *****************************************************************************/
static void RenderSubSet(_DtCanvasStruct *canvas, _DtCvDspLine *lines, int cnt,
                         _DtCvUnit y1, _DtCvUnit y2, _DtCvUnit *last_y) {
  int i;
  _DtCvUnit minY;
  _DtCvUnit maxY;
 ...
 *
 *****************************************************************************/
static _DtCvStatus CheckAround(_DtCvDspLine *lines, int cnt, int idx,
                               _DtCvUnit y2) {
  int i = 0;
 ...
 *
 *****************************************************************************/
static int FindChar(_DtCanvasStruct *canvas, _DtCvSegmentI *segment,
                    void *string, int max_len, _DtCvUnit x_pos,
                    _DtCvUnit *diff) {
  int myIndex;
  _DtCvUnit myDiff = 0;
  _DtCvUnit len;
   ...
   */
  _DtCvFontMetrics(canvas, _DtCvFontOfStringSeg(segment), NULL, NULL,
                   &charWidth, NULL, NULL);
 ...
 *
 *****************************************************************************/
static void DrawCanvasLines(_DtCanvasStruct *canvas, _DtCvUnit x1, _DtCvUnit y1,
                            _DtCvUnit x2, _DtCvUnit y2, _DtCvRenderType flag,
                            _DtCvUnit *ret_y, _DtCvUnit *ret_next) {
  int i;
  _DtCvUnit newY2 = y2;
  _DtCvLineSeg *pLS;
  ...
  if (canvas->line_lst != NULL && canvas->line_cnt &&
    ...
    for (i = 0, pLS = canvas->line_lst; i < canvas->line_cnt; i++, pLS++) {
       ...
       */
      if (pLS->max_y >= y1 && pLS->pos_y <= newY2 && lnInfo.x2 >= x1 &&
          pLS->pos_x <= x2 &&
        ...
        (*(canvas->virt_functions.render_elem))(
            canvas->client_data, _DtCvLINE_TYPE, pLS->pos_x, pLS->pos_y, -1, 0,
            0, _DtCvBAD_TYPE, NULL, &lnInfo);
      }
      /*
...
static _DtCvUnit DrawText(_DtCanvasStruct *canvas, _DtCvDspLine *line,
   ...
   */
  xPos =
      _DtCvAdjustForSuperSub(canvas, pSeg, xPos, &scriptX, &superWidth, &superY,
                             &subWidth, &subY, &lastWasSuper, &lastWasSub);
  /*
  ...
  while (NULL != pSeg && 0 < count) {
    ...
    _DtCvCheckLineMarks(canvas, txt_line, curIdx, count, xPos,
                        (_DtCvSELECTED_FLAG | _DtCvMARK_FLAG), &len, &old_flag,
                        &new_flag);
     ...
     */
    xPos = _DtCvDrawSegments(canvas, *line, pSeg, start, len, &lastLnk, xPos,
                             xPos, &scriptX, &superWidth, &superY, &subWidth,
                             &subY, &lastWasSub, &lastWasSuper, &lastLnkVis,
                             old_flag, new_flag, _DtCvBAD_TYPE, NULL);
    /*
    ...
    curIdx += len;
    if (0 < count)
      _DtCvSkipLineChars(canvas, pSeg, start, count + len, len, &start, &pSeg);
  }
 ...
 *****************************************************************************/
static _DtCvValue IsLineSpecial(_DtCvSelectData start, _DtCvSelectData end,
                                _DtCvDspLine line, int line_idx, int char_idx,
                                int length, _DtCvUnit dst_x, int *ret_len,
                                _DtCvFlags *ret_flag) {
 ...
 *****************************************************************************/
static void DrawTraversalIndicator(_DtCanvasStruct *canvas, _DtCvValue render,
                                   _DtCvValue draw_flag, _DtCvUnit *ret_x,
                                   _DtCvUnit *ret_y, _DtCvUnit *ret_baseline,
  ...
  if (_DtCvTraversalMark == canvas->trav_lst[travIdx].type) {
    ...
    if (True == render) {
      oldFlag =
          oldFlag | _DtCvMARK_FLAG | _DtCvTRAVERSAL_BEGIN | _DtCvTRAVERSAL_END;
      newFlag =
          newFlag | _DtCvMARK_FLAG | _DtCvTRAVERSAL_BEGIN | _DtCvTRAVERSAL_END;
      ...
      _DtCvDrawAreaWithFlags(
          canvas, canvas->marks[markIdx].beg, canvas->marks[markIdx].end,
          oldFlag, newFlag, _DtCvMARK_TYPE, canvas->marks[markIdx].client_data);
    }

    if (ret_height)
      *ret_height =
          canvas->marks[markIdx].end.y - canvas->marks[markIdx].beg.y +
  ...
  while (pSeg->link_idx != linkIndex) {
     ...
     * sub script - or not.
     */
    dstX = _DtCvAdjustForSuperSub(canvas, pSeg, dstX, &scriptX, &superWidth,
                                  &superY, &subWidth, &subY, &lastWasSuper,
                                  &lastWasSub);

    /*
     * get the width of the segment.
     */
    _DtCvGetWidthOfSegment(canvas, pSeg, start, count, &len, &tmpWidth, NULL);
    dstX += tmpWidth;
  ...
  if (ret_y)
    *ret_y =
        canvas->txt_lst[txtLine].baseline - canvas->txt_lst[txtLine].ascent;
  if (ret_baseline)
  ...
  if (True == render) {
    while (txtLine < canvas->txt_cnt && linkIndex == pSeg->link_idx) {
      ...
      count = 0;
      wrkChr = start;
      while (totCnt > 0 && tmpSeg != NULL && tmpSeg->link_idx == linkIndex) {
        _DtCvGetWidthOfSegment(canvas, tmpSeg, wrkChr, totCnt, &len, NULL,
                               NULL);
        totCnt -= len;
        count += len;
        wrkChr = 0;
      ...
      while (count > 0 && pSeg != NULL && pSeg->link_idx == linkIndex) {
        ...
        _DtCvCheckLineMarks(canvas, txtLine, curIdx, count, dstX,
                            (_DtCvSELECTED_FLAG | _DtCvMARK_FLAG), &len,
                            &oldFlag, &newFlag);
        /*
         ...
         * render the segments
         */
        dstX = _DtCvDrawSegments(
            canvas, canvas->txt_lst[txtLine], pSeg, start, len, &lstLnk, dstX,
            dstX, &scriptX, &superWidth, &superY, &subWidth, &subY, &lastWasSub,
            &lastWasSuper, &lstLnkVis, oldFlag, newFlag, _DtCvLINK_TYPE, NULL);

        count -= len;
        curIdx += len;
        if (count > 0) {
          _DtCvSkipLineChars(canvas, pSeg, start, count + len, len, &start,
                             &pSeg);
      ...
      height +=
          canvas->txt_lst[txtLine].ascent + canvas->txt_lst[txtLine].descent;
      txtLine++;
 ...
 *****************************************************************************/
int _DtCvGetSearchLineMetrics(_DtCvHandle handle, int idx, _DtCvUnit *baseline,
 ...
 *           object.
 *****************************************************************************/
_DtCvStatus _DtCvCheckInfringement(_DtCvUnit tst_top, _DtCvUnit tst_bot,
                                   _DtCvUnit obj_top, _DtCvUnit obj_bot) {
 ...
 *****************************************************************************/
void _DtCvCheckLineMarks(_DtCanvasStruct *canvas, int line_idx, int char_idx,
                         int length, _DtCvUnit dst_x, _DtCvFlags check_flags,
                         int *ret_len, _DtCvFlags *ret_old,
                         _DtCvFlags *ret_new) {
  int i;
  _DtCvFlags flag = 0;
  ...
  if ((check_flags & _DtCvSELECTED_FLAG) && canvas->select_start.y != -1) {
    ...
    *ret_new &= ~(_DtCvSELECTED_FLAG);
    ...
    if (IsLineSpecial(start, end, canvas->txt_lst[line_idx], line_idx, char_idx,
                      length, dst_x, &length, NULL)) {
      /*
  ...
  if ((check_flags & _DtCvMARK_FLAG) && 0 < canvas->mark_cnt) {
     ...
     */
    *ret_old &=
        ~(_DtCvMARK_FLAG | _DtCvMARK_BEGIN | _DtCvMARK_END | _DtCvMARK_ON);
    *ret_new &=
        ~(_DtCvMARK_FLAG | _DtCvMARK_BEGIN | _DtCvMARK_END | _DtCvMARK_ON);
    ...
    for (i = 0; i < canvas->mark_cnt; i++) {
      if (IsLineSpecial(canvas->marks[i].beg, canvas->marks[i].end,
                        canvas->txt_lst[line_idx], line_idx, char_idx, length,
                        dst_x, &length, &flag)) {
        /*
 ...
 *          the line.
 *****************************************************************************/
void _DtCvSkipLineChars(_DtCanvasStruct *canvas, _DtCvSegmentI *p_seg,
                        int start, int max_cnt, int use_len, int *ret_start,
 ...
 *****************************************************************************/
void _DtCvClearInternalUse(_DtCvSegmentI *list, _DtCvStatus flag) {
  while (NULL != list) {
    /*
 ...
 * Purpose:
 *****************************************************************************/
int _DtCvGetCharIdx(_DtCanvasStruct *canvas, _DtCvDspLine line,
                    _DtCvUnit find_x) {
  void *pChar;
  ...
  while (!done && find_x > xPos && count > 0) {
    xPos = _DtCvAdvanceXOfLine(canvas, pSeg, xPos, &lnkInd, &lastLinkVisible);

    if (xPos < find_x) {
      /*
       * advance the pointer by the width
       */
      _DtCvGetWidthOfSegment(canvas, pSeg, start, count, &len, &segWidth, NULL);
      ...
      } else /* if (xPos < find_x && find_x < xPos + segWidth) */
      {
        if (_DtCvIsSegString(pSeg)) {
          ...
          count -= FindChar(canvas, pSeg, pChar, len, find_x - xPos, NULL);
        }
        done = True;
      }
    }
 ...
 *****************************************************************************/
_DtCvUnit _DtCvGetStartXOfLine(_DtCvDspLine *line, _DtCvSegmentI **p_seg) {
  *p_seg = line->seg_ptr;
 ...
 *****************************************************************************/
_DtCvUnit _DtCvAdvanceXOfLine(_DtCanvasStruct *canvas, _DtCvSegmentI *p_seg,
                              _DtCvUnit x_pos, int *link_idx,
  ...
  *link_flag = _DtCvModifyXpos(canvas->link_info, p_seg, junk, *link_flag,
                               *link_idx, &x_pos);
  ...
  (void)_DtCvModifyXpos(canvas->traversal_info, p_seg, junk, ((_DtCvValue)True),
                        *link_idx, &x_pos);
 ...
 *
 *****************************************************************************/
void _DtCvGetWidthOfSegment(_DtCanvasStruct *canvas, _DtCvSegmentI *p_seg,
                            int start, int max_cnt, int *ret_cnt,
                            _DtCvUnit *ret_w, _DtCvValue *ret_trimmed) {
  void *pChar;
 ...
 *****************************************************************************/
_DtCvValue _DtCvModifyXpos(_DtCvSpaceMetrics info, _DtCvSegmentI *seg,
                           _DtCvValue tst_result, _DtCvValue cur_flag,
                           int last_idx, _DtCvUnit *x) {
  int addx = 0;
 ...
 *
 *****************************************************************************/
_DtCvUnit _DtCvAdjustForSuperSub(_DtCanvasStruct *canvas, _DtCvSegmentI *pSeg,
                                 _DtCvUnit start_x, _DtCvUnit *script_x,
                                 _DtCvUnit *super_width, _DtCvUnit *super_y,
                                 _DtCvUnit *sub_width, _DtCvUnit *sub_y,
  ...
  if (!(*last_was_super || *last_was_sub)) {
    if (_DtCvIsSegString(pSeg))
      _DtCvFontMetrics(canvas, _DtCvFontOfStringSeg(pSeg), NULL, NULL, NULL,
                       super_y, sub_y);
 ...
 *
 *****************************************************************************/
_DtCvUnit _DtCvDrawSegments(
    _DtCanvasStruct *canvas, _DtCvDspLine line, struct _dtCvSegment *p_seg,
    int start_char, int count, int *prev_lnk, _DtCvUnit txt_x, _DtCvUnit sel_x,
    _DtCvUnit *script_x, _DtCvUnit *super_width, _DtCvUnit *super_y,
    _DtCvUnit *sub_width, _DtCvUnit *sub_y, _DtCvValue *last_was_sub,
    _DtCvValue *last_was_super, _DtCvValue *last_link_vis, _DtCvFlags old_flag,
    _DtCvFlags new_flag, _DtCvElemType trav_type, _DtCvPointer trav_data) {
  int linkType = 0;
  int len;
  ...
  void *pChar;
  _DtCvFlags saveEnd =
      new_flag & (_DtCvTRAVERSAL_END | _DtCvLINK_END | _DtCvMARK_END);
  ...
  while (p_seg != NULL && count > 0) {
     ...
     */
    txt_x = _DtCvAdvanceXOfLine(canvas, p_seg, txt_x, prev_lnk, last_link_vis);

    /*
     * check for super and sub scripts.
     * adjust text x positioning accordingly.
     */
    txt_x = _DtCvAdjustForSuperSub(canvas, p_seg, txt_x, script_x, super_width,
                                   super_y, sub_width, sub_y, last_was_super,
                                   last_was_sub);
    if (sel_x > txt_x)
      sel_x = txt_x;
    ...
    *prev_lnk = p_seg->link_idx;
    ...
    if (-1 != elemType) {
      ...
      if (NULL != canvas->virt_functions.render_elem)
        (*(canvas->virt_functions.render_elem))(
            canvas->client_data, elemType, txt_x, yPos, linkType, old_flag,
            new_flag, trav_type, trav_data, &posInfo);

      if (cropped == _DtCvFALSE && new_flag & _DtCvSEARCH_END &&
          new_flag & _DtCvAPP_FLAG2 && !(new_flag & _DtCvSELECTED_FLAG))
...
void _DtCanvasLoadMetrics(_DtCvHandle handle) {
  _DtCanvasStruct *canvas = (_DtCanvasStruct *)handle;
  (*(canvas->virt_functions.get_metrics))(canvas->client_data, _DtCvCANVAS_TYPE,
                                          &(canvas->metrics));
  (*(canvas->virt_functions.get_metrics))(canvas->client_data, _DtCvLINK_TYPE,
                                          &(canvas->link_info));
  (*(canvas->virt_functions.get_metrics))(
      canvas->client_data, _DtCvTRAVERSAL_TYPE, &(canvas->traversal_info));
  if (MB_CUR_MAX > 1)
    (*(canvas->virt_functions.get_metrics))(
        canvas->client_data, _DtCvLOCALE_TYPE, &(canvas->locale));
}
 ...
 *		to the canvas.
 *****************************************************************************/
void _DtCanvasClean(_DtCvHandle canvas_handle) {
...
void _DtCanvasRender(_DtCvHandle canvas_handle, _DtCvUnit x1, _DtCvUnit y1,
  ...
  for (lines = canvas->txt_lst, i = 0; NULL != lines && i < canvas->txt_cnt;
       lines++, i++) {
    /*
     * get the minimum and maximum y of the next line
     */
    ...
    if (_DtCvIsNotProcessed(*lines) && maxY >= y1 && minY <= y2 &&
        (_DtCvRENDER_PARTIAL == flag ||
         (maxY <= y2 &&
          _DtCvTRUE != (sideCk = CheckAround(canvas->txt_lst, canvas->txt_cnt,
                                             i, y2))))) {
      (void)DrawText(canvas, lines, i, 0, 0);
      ...
      if (_DtCvFALSE == sideCk)
        RenderSubSet(canvas, canvas->txt_lst, canvas->txt_cnt, minY, maxY,
                     &lastY);
...
_DtCvStatus _DtCanvasMoveTraversal(_DtCvHandle canvas_handle,
    ...
    if (NULL != rid) {
      ...
      while (False == found && newIndex < canvas->trav_cnt) {
        if (_DtCvTRAVERSAL_ID == cmd &&
            _DtCvTraversalLink == canvas->trav_lst[newIndex].type) {
          idx = canvas->trav_lst[newIndex].seg_ptr->link_idx;
          lnkId = _DtLinkDbGetLinkSpec(canvas->link_data, idx);
  ...
  if (cmd == _DtCvTRAVERSAL_OFF) {
    if (-1 != canvas->cur_trav)
      DrawTraversalIndicator(canvas, render, False, NULL, NULL, NULL, NULL);
   ...
   */
    ...
    DrawTraversalIndicator(canvas, render, True, ret_x, ret_y, ret_baseline,
                           ret_height);
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCanvasGetPosLink(_DtCvHandle canvas_handle, _DtCvUnit x1,
  ...
  while (line < canvas->txt_cnt && _DtCvSTATUS_NONE == found) {
    ...
    if (topY <= y1 && y1 <= botY && startX <= x1 &&
      ...
      while (count > 0 && _DtCvSTATUS_NONE == found) {
        ...
        lstVisible = _DtCvModifyXpos(canvas->link_info, pSeg, junk, lstVisible,
                                     lnkIndx, &startX);
        /*
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCanvasGetCurLink(_DtCvHandle canvas_handle,
                                _DtCvLinkInfo *ret_info) {
  _DtCanvasStruct *canvas = (_DtCanvasStruct *)canvas_handle;
  _DtCvSegmentI *pSeg;
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCanvasGetCurTraversal(_DtCvHandle canvas_handle,
                                     _DtCvLinkInfo *ret_info,
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCanvasGetSpotInfo(_DtCvHandle canvas_handle, _DtCvUnit x,
                                 _DtCvUnit y, _DtCvSegment **ret_seg,
                                 _DtCvUnit *ret_offx, _DtCvUnit *ret_offy,
