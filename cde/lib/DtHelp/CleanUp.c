 ...
 *		Private Functions
 *****************************************************************************/
static void CheckFreePrivInfo(_DtCvSegment *seg) {
  /*
...
static void FreePrivateAndSeg(_DtCvSegment *seg) {
  if (NULL != seg) {
    /*
 ...
 *****************************************************************************/
void _DtHelpFreeSegments(_DtCvSegment *seg_list, _DtCvStatus unresolved,
  ...
  while (seg_list != NULL) {
    ...
    case _DtCvCONTAINER:
       ...
       * free the id block.
       */
      if (False == dupFlag && NULL != _DtCvContainerIdOfSeg(seg_list))
        free(_DtCvContainerIdOfSeg(seg_list));

      /*
       * free the contents of the block
       */
      _DtHelpFreeSegments(_DtCvContainerListOfSeg(seg_list), unresolved,
    ...
    case _DtCvREGION:
      if (False == dupFlag && NULL != destroy_region)
        (destroy_region)(client_data, _DtCvInfoOfRegionSeg(seg_list));
      break;

    case _DtCvSTRING:
      if (False == dupFlag) {
        free(_DtCvStringOfStringSeg(seg_list));
        if (True == unresolved && NULL != _DtCvFontOfStringSeg(seg_list))
    ...
    case _DtCvTABLE:
      ...
      while (NULL != tableSeg && NULL != *tableSeg) {
         ...
         * free the contents of the block
         */
        _DtHelpFreeSegments(_DtCvContainerListOfSeg(*tableSeg), unresolved,
                            destroy_region, client_data);
        tableSeg++;
      }
       ...
       * free the row ids.
       */
      _DtHelpCeFreeStringArray(_DtCvCellIdsOfTableSeg(seg_list));
      ...
      for (i = 0; NULL != strs && i < _DtCvNumColsOfTableSeg(seg_list);
           i++, strs++)
        free((void *)*strs);
 ...
 * Purpose:	Free all memory associated with an SDL list.
 *****************************************************************************/
void _DtHelpDestroyTopicData(_DtCvTopicInfo *topic,
                             void (*destroy_region)(void *, void *),
                             _DtCvPointer client_data) {
  if (NULL != topic) {
    /*
     ...
     */
    _DtHelpFreeSegments(topic->seg_list, _DtCvFALSE, destroy_region,
                        client_data);
