 ...
 *
 *********************************************************************/
void _DtHelpResizeDisplayArea(Widget parent, XtPointer client_data, int rows,
                              int columns) {
  int count = 0;
  int n;
  ...
  do {
     ...
     */
    n = 0;
    XtSetArg(args[n], XmNwidth, &oldWidth);
    ++n;
    XtSetArg(args[n], XmNheight, &oldHeight);
    ++n;
