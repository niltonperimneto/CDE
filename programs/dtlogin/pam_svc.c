...
int PamInit(char *prog_name, char *user, char *line_dev, char *display_name) {
  ...
  if (!pamh) {
    ...
    if (prog_name)
      pam_set_item(pamh, PAM_SERVICE, prog_name);
    if (user)
...
int PamAuthenticate(char *prog_name, char *display_name, char *user_passwd,
                    char *user, char *line) {
  int status;
...
int PamAccounting(char *prog_name, char *display_name, char *entry_id,
  ...
  if (status == PAM_SUCCESS)
    ...
    case USER_PROCESS:
      ...
      status = solaris_setutmp_mgmt(user, tty_line, display_name, session_type,
                                    entry_type, entry_id);
    ...
    case DEAD_PROCESS:
      ...
      status = solaris_reset_utmp_mgmt(&user, &tty_line, &display_name, 0,
                                       entry_type, entry_id);
...
int PamSetCred(char *prog_name, char *user, uid_t uid, gid_t gid) {
  ...
  if (status == PAM_SUCCESS && (!user) || (initgroups(user, gid) == -1))
