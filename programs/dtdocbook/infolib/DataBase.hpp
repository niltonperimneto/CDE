...
#include "object/c_codes.hpp" /* mmdb codes */
...
class DBCursor{
 public:
  ...
  void undoNext(void); /* seek back to the beginning of the last record read
		    * throw(PosixError) on unseekable device
		    */
  void local_rewind(void); /* restart at the beginning of the table file */
  int  tell(void);         /* current position of the file */
