...
static int open_hft_file(char *hft_file_name) {
  return (open(hft_file_name, O_RDONLY));
}

static void close_hft_file(int hft_file) { close(hft_file); }
...
static int process_hft(int hft_action, unsigned int hft_vtid) {
...
static int scan_hft_ring(int hft_action) {
  int hft;
  int rc = 0;
...
static int my_strpos(char *s, char c) {
  char *t;
...
void AixEnableHftRing(int enable) {
#ifdef _AIX
  if (is_local_display()) {
