...
void createbdf(Exc_data *ed) {
  ...
  i = ExpGpftoBDF(ed->fontfile, ed->bdffile, ed->code_num, ed->gpf_code_list,
...
void selcharcd(Exc_data *ed) {
  ...
  n++;
  XtSetArg(args[n], XmNtopWidget, slctLabel);
  n++;
  XtSetArg(args[n], XmNitems, xmslist);
  n++;
  XtSetArg(args[n], XmNitemCount, ld->existcode_num);
  n++;
  ...
  ok = excCreatePushButton(mainw, "ok", oklabel, (XtCallbackProc)selcharokCB,
                           (XtPointer)ld);

  cancel = excCreatePushButton(mainw, "cancel", cancellabel,
                               (XtCallbackProc)selcharcancelCB, (XtPointer)ld);
