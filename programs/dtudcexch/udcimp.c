...
extern int ImpBDFCodeList(char *, int *, int **);
extern int ImpBDFCodeListFree(int **);
extern int ImpBDFCheck(char *, char *);
extern int ImpBDFtoGpf(char *, char *, int, int *);

extern void ChatchSig(int);
void setcodelists(ListData *, int, int *);
void addcharptn(Exc_data *);
extern ListData *ld;
extern int num_gr;
...
void getcharcd(Exc_data *ed) {
  ...
  msg = GETMESSAGE(12, 2,
                   "Glyph images in this BDF file can't be added to the font.");
  msg2 = GETMESSAGE(
      12, 4,
      "Failed to open the selected BDF font. You have no right to access for "
      "the font file, or the format of the file is not consistent.");
  msg3 = GETMESSAGE(
      12, 6,
      "There are one or more glyph images being overwritten. Overwrite?");
...
void addcharptn(Exc_data *ed) {
  ...
  msg2 = GETMESSAGE(
      12, 10, "Glyph images in this BDF file can't be added to the font.");
  ...
  i = 0;
  i = ImpBDFtoGpf(ed->bdffile, ed->fontfile, ed->code_num, ed->bdf_code_list);
  FalCloseFont(fontid);
  if (i != 0) {
