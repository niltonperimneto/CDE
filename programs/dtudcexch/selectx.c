...
static Widget CreateSelectXLFD(Widget);
...
static char *char_set(char *str) /* dtex */
{
  int i, count;
  char *p;
...
static char *spc(char *str, char ch, int count) {
  char *p;
  p = str + strlen(str);
  for (; count && (str < p); p--) {
    if (*p == ch)
...
static void OpenWindowCB() /* dtex change */
{
  ...
  char *msg3;
  char err[128];
  extern void selcharcd(Exc_data *);
  char *locale;
  int mask =
      FAL_FONT_MASK_DEFINED | FAL_FONT_MASK_UNDEFINED | FAL_FONT_MASK_XLFDNAME;
  FalFontData key;
  ...
  msg2 = GETMESSAGE(
      4, 4,
      "Failed to open the selected font. You have no right to access for the "
      "font file, or the format of the file is not consistent.");
  ...
  str = (char *)_XmStringUngenerate((XmString)st[0], NULL, XmMULTIBYTE_TEXT,
                                    XmMULTIBYTE_TEXT);
  if (str == NULL) {
    return;
  }
  ...
  if (FalGetUDCGIArea(locale, fullFontData.cd_set,
                      char_set(fullFontData.xlfdname), &gr,
                      &num_gr) == FAL_ERROR) {
...
/*ARGSUSED*/
static void OpenCancelCB(Widget widget, caddr_t clientData, caddr_t callData)
/* dtex change */
{
...
static void udc_call(Widget w) {
  XmString label;
  char *moji;

  XtVaGetValues(w, XmNlabelString, &label, NULL);

  moji = (char *)_XmStringUngenerate((XmString)label, NULL, XmMULTIBYTE_TEXT,
                                     XmMULTIBYTE_TEXT);
  if (moji) {
    if (strncmp(moji, "*", 1) == 0) {
      udc_flag = False;
...
static void button_set1() {
  ...
  for (j = 0; j < udc_count; j++) {
    for (i = 0, f = fontlist->list, found = False; i < fontlist->num;
         i++, f++) {
      if (udc[j] == f->cd_set) {
        found = True;
        break;
...
void data_sort(int *data, int count) {
  int *p1, *p2, tmp, i;
...
static void font_init() /* dtex change */
{
  FalFontDataList *fontlist;
  FalFontData *f;
  Boolean found;
  ...
  if (FalGetFontList(NULL, FAL_FONT_MASK_DEFINED | FAL_FONT_MASK_UNDEFINED,
                     &fontlist) == FAL_ERROR) {
    sprintf(err, "%s", ERROR_1);
  ...
  XtAddCallback(button, XmNactivateCallback, (XtCallbackProc)udc_call, NULL);
  for (i = 0; i < udc_count; i++) {
    ...
    XtAddCallback(button1[i], XmNactivateCallback, (XtCallbackProc)udc_call,
                  NULL);
  }

  button = XmCreatePushButton(pull2, "*", NULL, 0);
  XtManageChild(button);
  XtAddCallback(button, XmNactivateCallback, (XtCallbackProc)sty_call, NULL);
  for (i = 0; i < sty_count; i++) {
    button2[i] = XmCreatePushButton(pull2, sty[i], NULL, 0);
    XtManageChild(button2[i]);
    XtAddCallback(button2[i], XmNactivateCallback, (XtCallbackProc)sty_call,
                  NULL);
  }

  button = XmCreatePushButton(pull3, "*", NULL, 0);
  XtManageChild(button);
  XtAddCallback(button, XmNactivateCallback, (XtCallbackProc)wls_call, NULL);
  for (i = 0; i < wls_count; i++) {
    sprintf(tmp, "%d", wls[i]);
    button3[i] = XmCreatePushButton(pull3, tmp, NULL, 0);
    XtManageChild(button3[i]);
    XtAddCallback(button3[i], XmNactivateCallback, (XtCallbackProc)wls_call,
                  NULL);
  }

  button = XmCreatePushButton(pull4, "*", NULL, 0);
  XtManageChild(button);
  XtAddCallback(button, XmNactivateCallback, (XtCallbackProc)hls_call, NULL);
  for (i = 0; i < hls_count; i++) {
    sprintf(tmp, "%d", hls[i]);
    button4[i] = XmCreatePushButton(pull4, tmp, NULL, 0);
    XtManageChild(button4[i]);
    XtAddCallback(button4[i], XmNactivateCallback, (XtCallbackProc)hls_call,
                  NULL);
  }
}

static Widget CreateSelectXLFD(Widget top) /* dtex change */
{
  ...
  n++;
  XtSetArg(args[n], XmNsubMenuId, pull1);
  n++;
  ...
  XtSetArg(args[n], XmNlabelString, str);
  n++;
  XtSetArg(args[n], XmNsubMenuId, pull2);
  n++;
  ...
  XtSetArg(args[n], XmNlabelString, str);
  n++;
  XtSetArg(args[n], XmNsubMenuId, pull3);
  n++;
  ...
  XtSetArg(args[n], XmNlabelString, str);
  n++;
  XtSetArg(args[n], XmNsubMenuId, pull4);
  n++;
  ...
  n++;
  XtSetArg(args[n], XmNitems, xlf);
  n++;
  XtSetArg(args[n], XmNitemCount, xlf_count);
  n++;
  ...
  /*        XtAddCallback(editW, XmNhelpCallback,
     ...
     dtex del */
  XtAddCallback(editW, XmNhelpCallback, (XtCallbackProc)quit, NULL);
