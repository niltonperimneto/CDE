...
static int impFileConvInit(struct btophead *, struct btophead *,
                           struct ptobhead *);
static int impGetGpfInf(struct btophead *, struct ptobhead *, char *, int);
static int impReadBdfHeaderAndPut(struct btophead *, struct ptobhead *, char *);
static int impMergePtn(struct btophead *, struct btophead *, char *, int,
                       int *);
/* static int impCheckHeader(); */
static int impModifyPtn(struct btophead *, struct btophead *, char *, int);
static int impInsertPtn(struct btophead *, struct btophead *, char *, int, int);
static int impWriteSnf(struct btophead *, struct ptobhead *);
static int impWritePtnToBdf();
static int impPutDefaultChars();
static int impReadBdfToMemory();

extern int expCheckCode(int, int, int *);

static struct ptobhead WriteGpf;

static char *targ_file = NULL; /* UDC_file_name */

static void sigint_out(int sig) {
...
int ImpBDFCodeList(char *bdf_name, int *code_num, int **code_list) {
  int *glyph_list;
  FILE *bdfp;
  char *p;
  char buf[2048];
  ...
  if ((glyph_list = (int *)calloc(*code_num, (unsigned int)sizeof(int))) ==
      NULL) {
    return (-1);
  }
...
int ImpBDFCodeListFree(int **code_list) {
  free(*code_list);
  return (0);
}

int ImpBDFCheck(char *bdf_name, /* BDF file name */
...
int ImpBDFtoGpf(char *bdf_name, char *gpf_name, int bdf_codenum,
                int *bdf_codelist) {
  int rtn;
  int exit_stat;
  ...
  signal(SIGHUP, sigint_out);
  signal(SIGINT, sigint_out);
  signal(SIGQUIT, sigint_out);
  signal(SIGTERM, sigint_out);
   ...
   */
  if ((rtn = impMergePtn(&ReadUdc, &ReadGpf, buf, bdf_codenum, bdf_codelist))) {
    if (WriteGpf.out_file) {
      UNLINK_TMPFILE(WriteGpf.out_file);
    }
    return (-1);
   ...
   */
  return (Make_NewFefFile(WriteGpf.snf_file, WriteGpf.out_file, FONT_FILE_PARM,
...
static int impFileConvInit(struct btophead *r_udc, struct btophead *r_gpf,
  ...
  case 0:
    ...
    /* gpf -> bdf */
    execl(oakgtobdf, oakgtobdf, r_gpf->in_file, (char *)0);
    return EXEC_ERROR;
  ...
  case 0:
    ...
    if (ChkPcfFontFile(w_gpf->snf_file) == 0) {
      execl(bdftopcf, bdftopcf, (char *)0);
      return EXEC_ERROR;
    }
    /*
...
static int impGetGpfInf(struct btophead *r_gpf, struct ptobhead *w_gpf,
                        char *buf, int bdf_codenum) {
  int nchar, rtn;
...
static int impReadBdfHeaderAndPut(struct btophead *r_gpf,
                                  struct ptobhead *w_gpf, char *buf) {
  char *p;
  ...
  while (1) {
    ...
    if (!strncmp(p, SIZE, (unsigned int)strlen(SIZE))) {
      if ((sscanf(p, "SIZE %f%d", &(r_gpf->bdf_point), &(r_gpf->bdf_xdpi))) !=
          2) {
        return BDF_INVAL;
      }
    ...
    } else if (!strncmp(p, FONTBOUNDINGBOX,
                        (unsigned int)strlen(FONTBOUNDINGBOX))) {
      if ((sscanf(p, "FONTBOUNDINGBOX %d %d %d %d", &(r_gpf->bdf_width),
                  &(r_gpf->bdf_height), &(r_gpf->bdf_x), &(r_gpf->bdf_y))) !=
          4) {
        return BDF_INVAL;
      }
      fprintf(w_gpf->output, "%s", buf);
...
static int impMergePtn(struct btophead *r_udc, struct btophead *r_gpf,
                       char *buf, int bdf_codenum, int *bdf_codelist) {
  ...
  if ((r_udc->bdf_width != r_gpf->bdf_width) ||
    ...
    if (r_udc->ptn == NULL) {
      r_udc->ptn = (char **)malloc(sizeof(char *));
    }
    r_udc->ptn[0] = ptn;
...
static int impModifyPtn(struct btophead *r_udc, struct btophead *r_gpf,
                        char *buf, int ix) {
  ...
  if (r_udc->zoomf) {
    ...
    if ((rtn =
             PtnZoom(r_gpf->ptn[ix], r_udc->ptn[0], r_udc->bdf_width,
                     r_udc->bdf_height, r_gpf->bdf_width, r_gpf->bdf_height))) {
      return rtn;
    }
  } else {
...
static int impInsertPtn(struct btophead *r_udc, struct btophead *r_gpf,
                        char *buf, int code, int ix) {
...
static int impWriteSnf(struct btophead *r_gpf, struct ptobhead *w_gpf) {
  w_gpf->zoomf = 0;
