...
static int writeBdfHeader(struct ptobhead *, int, char **);
static int readBdfToMemory(struct btophead *, char *, int, int *, int);
static void sigint_out(int);
...
int expCheckCode(unsigned int code, int code_num, int *code_list) {
  int i;
...
static int
writeBdfHeader(struct ptobhead *head, int comment_num, /* number comments */
               char **comment_list /*pointer to the list of comments */
) {
  FILE *fp;
  int fd[2];
  ...
  while (1) {
     ...
     * write user comments
     */
    if (!strncmp(p, "FONT", strlen("FONT")) && comment_list && !comflg) {
      int i;
    ...
    if (!strncmp(p, FONTBOUNDINGBOX, FONTBOUNDINGBOXsz)) {
      if ((cnt = sscanf(p, "FONTBOUNDINGBOX %d%d%d%d", &(head->bdf_width),
                        &(head->bdf_height), &(head->bdf_x), &(head->bdf_y))) !=
          4) {
        fclose(fp);
        if (head->snf_file != NULL) {
          close(fd[0]);
