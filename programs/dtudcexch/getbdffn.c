...
void filesbokCB(Widget, Exc_data *, XtPointer);
void filesbcancelCB(Widget, Exc_data *, XtPointer);
 ...
 */
{
  ...
  if (strcmp(mode, "r") == 0) {
    ...
    if (stat(file, &buf) == 0) {
      if (((buf.st_mode & S_IFMT) == S_IFREG) && (access(file, W_OK) == 0))
        return 1; /* writable existing file */
    } else {
      if (stat(dir, &buf) == 0) {
        if (((buf.st_mode & S_IFMT) == S_IFDIR) && (access(dir, W_OK) == 0)) {
...
void filesbokCB(Widget widget, Exc_data *ed, XtPointer call_data) {
  ...
  dir = (char *)_XmStringUngenerate((XmString)ptr->dir, NULL, XmMULTIBYTE_TEXT,
                                    XmMULTIBYTE_TEXT);
  if (!dir) {
    return;
  } else {
    if (*file != '/') {
      if ((tmpfile = XtMalloc(strlen(dir) + 1 + strlen(file) + 1)) == NULL) {
