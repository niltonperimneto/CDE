...
#include <locale.h>
extern void _DtEnvControl(int);
void callexpCB(Widget, Exc_data *, XtPointer);
void callimpCB(Widget, Exc_data *, XtPointer);
void exitCB(Widget, Exc_data *, XtPointer);
void helpCB(Widget, Exc_data *, XtPointer);
void udcexp(Exc_data *);
void udcimp(Exc_data *);
...
Widget excCreateRadioButton(Widget parent, String name, String label,
  ...
  XtAddCallback(widget, XmNvalueChangedCallback, (XtCallbackProc)callback_func,
                (XtPointer)client_data);
...
int main(int argc, char *argv[]) {
  ...
  n = 0;
  toplevel =
      XtAppInitialize(&app_context, "UDCexchange", (XrmOptionDescList)NULL, 0,
                      (int *)&argc, argv, (String *)NULL, args, n);
  ...
  errmsg_org[8] =
      GETMESSAGE(8, 20, "The format of fonts.list file is illegal.");
  errmsg_org[9] = GETMESSAGE(
      8, 22, "The descriptions of the fonts.list file are incorrect.");
  errmsg_org[10] =
      GETMESSAGE(8, 24, "The format of fonts.list file is illegal.");
  ...
  errmsg_org[16] = GETMESSAGE(
      8, 36, "This font file is already opened by other application.");
  errmsg_org[17] = GETMESSAGE(8, 38, "Cannot lock font file.");
  errmsg_org[18] = GETMESSAGE(8, 40, "Cannot unlock font file.");
  errmsg_org[19] =
      GETMESSAGE(8, 42, "Cannot get lock information from the font file.");
  errmsg_org[20] = GETMESSAGE(8, 44, "Cannot find the specified font file.");
  errmsg_org[21] = GETMESSAGE(8, 46, "Cannot read NLS database.");
  errmsg_org[22] =
      GETMESSAGE(8, 48, "Cannot get charset names from NLS database.");
  errmsg_org[23] =
      GETMESSAGE(8, 50, "Charset name not defined in NLS database.");
  ...
  exit = excCreatePushButton(mainw, "exit", exitlabel, (XtCallbackProc)exitCB,
                             (XtPointer)ed);

  /*    help = excCreatePushButton(mainw, "help", helplabel,
                                 (XtCallbackProc) helpCB, (XtPointer) ed);
  */
  XtVaSetValues(mainw, XmNmessageWindow, exit, NULL);
...
void exitCB(Widget widget, Exc_data *ed, XtPointer call_data) { excexit(ed); }
