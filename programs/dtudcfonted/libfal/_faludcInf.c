...
/*
   external symbols
*/
extern FontData falread_EncodingInfo(int, char **);
...
extern XLCd _fallcGenericLoader(char *);
...
int fal_get_codeset(char *locale, char *charset_str, int **codeset,
                    int *num_codeset) {
  XLCdRec lcdrec;
  XLCd lcd;
  XLCdPublicRec xlcdp;
      ...
      *ptr = 0;
      if (!_fallcCompareISOLatin1(charset_str, buf)) {
        ...
        if (num_ret == 1) {
          ...
          ret = (int *)Xrealloc(ret, num_ret * sizeof(int));
        }
        if (ret == NULL) {
          fal_utyerrno = 0x03;
          return (FAL_ERROR);
...
/*
    ...
    charset_str     :  JISX0208.1983-0
*/
int FalGetUDCGIArea(char *locale, int codeset, char *charset_str,
...
int fal_gi_to_vgi(XLCd lcd, char *locale, char *charset_str, int codeset,
                  unsigned long gi, unsigned long *vgi, char *charsetname) {
  FalGlyphRegion *udc;
  int num = 0, count, num_ret = 0;
  int i, j, k;
  char **value, buf[128], ptr;
...
Bool falnon_standard(XLCd lcd, XlcCharSet charset) {
  ...
  for (i = 0;; i++) {
    ...
    if (count > 0) {
      if (!_fallcNCompareISOLatin1(value[0], charset->name, strlen(value[0]))) {
        return (True);
      }
    } else {
      return (False);
...
void falmake_none_standard(char *from, XlcCharSet charset, char *src) {
...
int fal_glyph_to_code(char *locale, char *charset_str, int codeset,
                      unsigned long glyph_index, unsigned long *codepoint) {
  XLCd lcd;
  unsigned char *from;
  int from_left;
  unsigned char *to;
  ...
  fal_gi_to_vgi(lcd, locale, charset_str, codeset, glyph_index, &glyph_index,
                charsetname);
  ...
  */
  ...
  _fallcConvert(conv, (XPointer *)&from, &from_left, (XPointer *)&to, &to_left,
                args, 0);
...
int FalGetUDCCPArea(char *locale, int codeset, char *charset_str,
                    FalCodeRegion **cr, int *num_cr) {
  int i, num_gr, ret;
  FalGlyphRegion *gr;
  FalCodeRegion *crr;
  ...
  for (i = 0; i < num_gr; i++) {
    ret = fal_glyph_to_code(locale, charset_str, codeset, gr[i].start,
                            &(crr[i].start));
    if (ret == FAL_ERROR) {
      fal_utyerrno |= (0x0a << 8);
      Xfree(gr);
    ...
    ret = fal_glyph_to_code(locale, charset_str, codeset, gr[i].end,
                            &(crr[i].end));
    if (ret == FAL_ERROR) {
      fal_utyerrno |= (0x0a << 8);
      Xfree(gr);
 ...
 *
 */
static Bool fal_vgi_to_gi(XLCd lcd, char *locale, unsigned long vglyph,
                          unsigned long *glyph, XlcCharSet charset,
                          char *charsetname) {
  int num = 0, count, num_ret = 0;
  int i, j, k;
  char **value, buf[128], ptr;
...
int fal_code_to_glyph(char *locale, unsigned long codepoint, FalGIInf **gi,
                      int *num_gi) {
  XLCd lcd;
  unsigned char *from;
  int from_left;
  ...
  if (fal_vgi_to_gi(lcd, locale, vglyph, &glyph, charset, charsetname) ==
      False) {
    _fallcCloseConverter(conv);
    _fallcDestroyLC(lcd);
    *num_gi = 0;
