...
static CodeSet GetCodeSetFromCharSet(XLCd, XlcCharSet);
static CodeSet wc_codeset(XLCd, wchar_t);

#define BADCHAR(min_ch, c)                                                     \
  (BIT8OFF(c) < (char)min_ch && BIT8OFF(c) != 0x0 && BIT8OFF(c) != '\t' &&     \
   BIT8OFF(c) != '\n' && BIT8OFF(c) != 0x1b)
...
static int euc_mbstowcs(XlcConv conv, XPointer *from, int *from_left,
...
static int euc_wcstombs(XlcConv conv, XPointer *from, int *from_left,
                        XPointer *to, int *to_left, XPointer *args,
                        int num_args) {
...
static int euc_mbtocs(XlcConv conv, XPointer *from, int *from_left,
                      XPointer *to, int *to_left, XPointer *args,
                      int num_args) {
  XLCd lcd = (XLCd)conv->state;
  XlcCharSet charset;
...
static int euc_mbstocs(XlcConv conv, XPointer *from, int *from_left,
                       XPointer *to, int *to_left, XPointer *args,
                       int num_args) {
  XLCd lcd = (XLCd)conv->state;
  ...
  while ((ret = euc_mbtocs(conv, from, from_left, to, to_left, tmp_args, 1)) >
         0)
    unconv_num += ret;
  if (ret < 0)
    return ret;
...
static int euc_wcstocs(XlcConv conv, XPointer *from, int *from_left,
                       XPointer *to, int *to_left, XPointer *args,
                       int num_args) {
  XLCd lcd = (XLCd)conv->state;
  ...
  for (; wcstr_len > 0 && buf_len > 0; wcptr++, wcstr_len--) {
    ...
    while (length--) {
      *tmpptr-- = codeset->length == 1 && codeset->side == XlcGR ? BIT8ON(wch)
                                                                 : BIT8OFF(wch);
...
static int euc_cstombs(XlcConv conv, XPointer *from, int *from_left,
                       XPointer *to, int *to_left, XPointer *args,
                       int num_args) {
  XLCd lcd = (XLCd)conv->state;
  char *csptr = *from;
  ...
  if (bufptr) {
    while (buf_len--) {
      ...
      while (length--)
        *bufptr++ =
            codeset->side == XlcGR ? BIT8ON(*csptr++) : BIT8OFF(*csptr++);
    }
  }
...
static int euc_cstowcs(XlcConv conv, XPointer *from, int *from_left,
                       XPointer *to, int *to_left, XPointer *args,
                       int num_args) {
  XLCd lcd = (XLCd)conv->state;
  char *csptr = *from;
 ...
 */
static void initCTptr(XLCd lcd) {
    ...
    for (j = 0; j < num_charsets; j++) {
      ...
      for (ctdp = ctdata; ctdp <= ctd_endp; ctdp++)
        ...
        if (!strcmp(ctdp->name, charset->name)) {
          ...
          ctdptr[codeset->cs_num]->set_size = charset->set_size;

          ctdptr[codeset->cs_num]->min_ch =
              charset->set_size == 94 &&
                      (ctdptr[codeset->cs_num]->length > 1 ||
                       ctdptr[codeset->cs_num]->side == XlcGR)
                  ? 0x21
                  : 0x20;

          if (codeset->parse_info) {
            ctdptr[codeset->cs_num]->sshift = *codeset->parse_info->encoding;
          }
...
#define SKIP_I(str)                                                            \
  while (*(str) >= 0x20 && *(str) <= 0x2f)                                     \
    (str)++;
#define SKIP_P(str)                                                            \
  while (*(str) >= 0x30 && *(str) <= 0x3f)                                     \
    (str)++;

static int euc_ctstowcs(XlcConv conv, XPointer *from, int *from_left,
                        XPointer *to, int *to_left, XPointer *args,
                        int num_args) {
  XLCd lcd = (XLCd)conv->state;
  Ulong wc_shift_bits = XLC_GENERIC(lcd, wc_shift_bits);
  ...
  for (length = ctdata[Ascii].length; *from_left > 0; (*from_left) -= length) {
    ...
    /* The following code insures that non-standard encodings, direction,
...
static int euc_wcstocts(XlcConv conv, XPointer *from, int *from_left,
                        XPointer *to, int *to_left, XPointer *args,
                        int num_args) {
  int ct_len = *to_left;
  wchar_t *inbufptr = (wchar_t *)*from;
...
static int euc_ctstombs(XlcConv conv, XPointer *from, int *from_left,
                        XPointer *to, int *to_left, XPointer *args,
                        int num_args) {
  XPointer inbufptr = *from;
  XPointer outbufptr = *to;
...
static int euc_mbstocts(XlcConv conv, XPointer *from, int *from_left,
                        XPointer *to, int *to_left, XPointer *args,
                        int num_args) {
  int ct_len = *to_left;
  int cs_num;
  ...
  for (; *from_left > 0; (*from_left) -= length) {
    ...
    clen = length;
    do {
      *ctptr++ = charset == ct_state.GR_charset ? BIT8ON(*inbufptr++)
                                                : BIT8OFF(*inbufptr++);
    } while (--clen);
  }
...
static XlcConv open_mbtocs(XLCd from_lcd, char *from_type, XLCd to_lcd,
                           char *to_type) {
...
XLCd _fallcEucLoader(char *name) {
  XLCd lcd;
