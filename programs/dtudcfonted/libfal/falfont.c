...
static int falGetGlyph(char *, Oak_FontInf *, int);
static int falGetCharIndex(Oak_FontInf *, int);
static int falZoom(char *, char *, int, int, int, int, char *);
static int exline(char *, char *, int, int);
static char falGetMask(int);
static int chk_key_str(FalFontData *, int);
static int fal_make_fontlist(FalFontDataList *, FalFontData *, int);
static int fal_sort_fontlist(FalFontDataList *);
static int cpy_default_path(FalFontPath *);
static int comp_default_path(FalFontPath *);
static int fal_clear_font_path(FalFontPath *);
static int set_font_pathlist(FalFontPath *, int);

static int fal_split_data(char *, int, char **);
static int fal_clear_data(FalFontData *);
static int fal_cmp_data(FalFontData *, FalFontData *, int);
static int new_target(FalFontData *, FalFontData *);

static int fal_add_fidinf(FalFontID, int, int, int);
static int fal_read_fidinf(FalFontID, FontIDInfo *);
static int fal_del_fidinf(FalFontID);

extern int falReadFontProp();
extern int falReadGpfProp(int, Oak_FontInf *, int, FalFontData *);
extern int falInitReadPcfProp();
extern int fal_get_codeset(char *, char *, int **, int *);
extern int falInitReadPcf(PcfFontInf *, char *);
extern void falGetPcfGSize(PcfFontInf *, int *, int *);
extern int falPcfGlyph(char *, Oak_FontInf *, unsigned int);
extern int fal_code_to_glyph(char *, unsigned long, FalGIInf **, int *);
extern int fal_glyph_to_code(char *, char *, int, unsigned long,
                             unsigned long *);

FalFontID __FalOpenFont(char *, int, int, int);
int __FalCloseFont(FalFontID, int);

static int fal_conv_code_to_glyph(FalFontID, unsigned int, unsigned int *);
static int fal_conv_glyph_to_code(Oak_FontInf *, int, int, unsigned int,
                                  unsigned int *);
static int falReadFontInfoLists(int, FalFontData *, int, FalFontDataList *,
                                FalFontData, char *, FalFontID, FalFontData *);
static int fal_get_def_fontdata(int, char *, char *, char **, FalFontData *,
                                char *);
static int falgetfontlist(FalFontData *, FalFontData *, int, FalFontDataList *,
                          int);
static int fal_get_undef_fontdata(int, char *, char *, FalFontData *, char *,
                                  int *, int **);
static void clear_charset_info();
static int make_default_path(FalFontPath *);
static int file_lock(int);
static int file_unlock(int);
static int is_lock(int);
static int set_default_path(void);
extern int set_struct(FalFontData *, char **);
extern int fal_eq_data(FalFontData, FalFontData);
extern char *fal_get_base_name(char *);
static int CR_to_NULL(char *);
...
void set_errfile_str(char *obuf, char *ibuf) {
  char *sp;
 ...
 */
FalFontID FalOpenFont(char *file, int protect_key_data, int codeset) {
 ...
 */
FalFontID __FalOpenFont(char *file, int protect_key_data, int codeset,
                        int lockflag) {
  int fd;
  char *buf;
  char *openfontfile;
  ...
  while (1) {
    ...
    if (!(openfontfile = (char *)malloc(strlen(file) + 1))) {
      fal_seterrcode(_FAL_MALOC_ER, 0, FAL_ERR_MALLOC, FAL_FUNCNUM_OPNFNT);
    ...
    CHK_PROTECT_KEY(protect_key_data, openfontfile, fal_utyerror,
                    fal_utyderror);
    ...
    if (strcmp(FILE_SUFFIX(openfontfile), PCFSUFFIX) == 0) {
      ...
      FAL_GET_SNF_HEADER(buf, protect_key_data, fal_utyerror, fal_utyderror, fd,
                         finf, openfontfile);
    }
    finf->buf = buf;

    if (finf->isFef) {
      finf->pCinf = (CharInfoPtr)((char *)finf->pFinf + sizeof(FontInfoRec));
      finf->pGlyphs =
          (unsigned char *)((char *)finf->pCinf +
                            (sizeof(CharInfoRec) * GETNUMCHARS(finf->pFinf)));
      finf->width = finf->pFinf->maxbounds.rightSideBearing -
                    finf->pFinf->maxbounds.leftSideBearing;
      finf->height =
          finf->pFinf->maxbounds.ascent + finf->pFinf->maxbounds.descent;
    } else if (finf->isPcf) {
      finf->pCinf = NULL;
      finf->pGlyphs = NULL;
      falGetPcfGSize(&finf->pcfinf, (int *)&finf->width, (int *)&finf->height);
    } else {
      fal_utyerror = _FAL_FONT_ER;
      fal_utyderror = 0;
      fal_utyerrno = FAL_ERR_FONT;
...
int FalCloseFont(FalFontID fid) {
 ...
 * 	0: OFF --  for "FalOpenFont()"
 */
int __FalCloseFont(FalFontID fid, int lockflag) {
  Oak_FontInf *finf;
...
int FalQueryFont(FalFontID fid, FalFontinfo *fontinfo) {
  Oak_FontInf *finf;
  ...
  if ((fal_conv_glyph_to_code(finf, fontid_inf.dspcode, fontid_inf.cd_set,
                              (unsigned int)finf->start,
                              &inner_start) == FAL_ERROR) ||
      (fal_conv_glyph_to_code(finf, fontid_inf.dspcode, fontid_inf.cd_set,
                              (unsigned int)finf->end,
                              &inner_end) == FAL_ERROR)) {
    fal_utyerrno |= (FAL_FUNCNUM_QRY << 8);
    return FAL_ERROR;
  }
...
char *FalReadFont(FalFontID fid, int code, int width, int height) {
  Oak_FontInf *finf;
  int zoom_on;
  char *toglyph;
  int ptnBsize;
  ...
  if (zoom_on == TRUE) {
    ...
    falZoom(finf->dptnBuf, finf->sptnBuf, finf->width, finf->height, width,
            height, finf->dbuf);
    return (finf->dptnBuf);
  } else {
    return finf->sptnBuf;
...
static int falGetGlyph(char *glyph, Oak_FontInf *finf, int code) {
  ...
  /* code check */
  if (((code < finf->start) || (code > finf->end)) ||
  ...
  in_dwidth = (finf->width + SNF_BOUND - 1) / SNF_BOUND * (SNF_BOUND / 8);
...
static int falGetCharIndex(Oak_FontInf *finf,
...
static int falZoom(char *dmem, char *smem, int sw, int sh, int dw, int dh,
                   char *dbuf) {
  int swidth = (sw + 7) / 8;
  int dwidth = (dw + 7) / 8;
  int i, lcnt = 0;
...
static int exline(char *sp, char *dbuf, int sw, int dw) {
...
static unsigned char _Fal_Mask_Tab[8] = {0xff, 0x80, 0xc0, 0xe0,
                                         0xf0, 0xf8, 0xfc, 0xfe};

static char falGetMask(int width) {
  int ix = width % 8;
...
static FalFontDB fal_db_shape[] = {{FAL_FONT_ZENKAKU, FAL_FONT_STR_ZENKAKU},
...
static int
chk_key_str(FalFontData *key, /* a structure of saerching information */
            int mask)         /* a mask                               */
{
  int flg = 0;
...
static int
fal_make_fontlist(FalFontDataList *fls, /* a pointer of a structure of a font
                                           information list */
                  FalFontData *key, /* a structure of searching information */
                  int mask)         /* a mask for a saerch */
{
  FalFontData dummy_data = {0};
  return falReadFontInfoLists(MKFONTLIST, key, mask, fls, dummy_data, NULL,
                              NULL, NULL);
}
...
int FalFreeFontList(FalFontDataList *list) {
  int i;
...
static int fal_split_data(char *buf, int elm_num, char *elm[]) {
  int cnt;
  int strtop_flg;
...
static int fal_clear_data(FalFontData *tmp) {
  /* search a character */
  free(tmp->xlfdname);
  free(tmp->style.name);
  free(tmp->shape.name);
...
static int fal_check_already_exist(FalFontData *data, FalFontDataList *lst) {
  int i;
  int mask;
...
static int fal_sort_fontlist(FalFontDataList *lst) {
  int i, j;
  FalFontDataList srt;
  FalFontData target;
  int target_entry;
  ...
  for (i = 0; i < srt.num - 1; i++) {
    ...
    if (target_entry != i) {
      memmove(&target, &(srt.list[target_entry]), sizeof(FalFontData));
      memmove(&(srt.list[i + 1]), &(srt.list[i]),
              sizeof(FalFontData) * (target_entry - i));
      memmove(&(srt.list[i]), &target, sizeof(FalFontData));
    }
  }
  free(lst->list);
  lst->list = srt.list;
...
static int fal_atoi(char *str, int *val) {
  char *ptr;
  char *str_end;
  int tmp;
...
static int fal_set_cs(char *str, int *cs) {
  if (!strcmp(str, "CS0")) {
    *cs = FAL_FONT_CS0;
...
static int fal_set_prm(char *str, int *prm) {
  int tmp = 0;
  for (; *str != '\0'; str++) {
    if (*str == 'P') {
...
int FalGetFontPath(FalFontPath **dlist_ret) {
...
static int set_default_path(void) { return set_font_pathlist(&orgn, 0); }

static int cpy_default_path(FalFontPath *p) {
  int i;
  FalFontPath tmp;
  ...
  if ((tmp.path = (char **)malloc(sizeof(char *) * (orgn.num))) == NULL) {
    fal_utyerror = _FAL_MALOC_ER;
    fal_utyerrno = FAL_ERR_MALLOC;
    return FAL_ERROR;
  }
...
static int comp_default_path(FalFontPath *p) {
  int i;
...
static int fal_clear_font_path(FalFontPath *ls) {
  int i;
...
int set_struct(FalFontData *tmp_data, char *elm[]) {
  char *dup_p;

  /* data information */
  while (1) {
    /* a width of a character size */
    if (fal_atoi(elm[FAL_FONT_ELM_SIZE_W], &(tmp_data->size.w)) == FAL_ERROR) {
      break;
    }
    /* a height of a character size */
    if (fal_atoi(elm[FAL_FONT_ELM_SIZE_H], &(tmp_data->size.h)) == FAL_ERROR) {
      break;
    }
    /* a width of a letter size */
    if (fal_atoi(elm[FAL_FONT_ELM_LETTER_W], &(tmp_data->letter.w)) ==
        FAL_ERROR) {
      break;
    }
    /* a height of a letter size */
    if (fal_atoi(elm[FAL_FONT_ELM_LETTER_H], &(tmp_data->letter.h)) ==
        FAL_ERROR) {
      break;
    }
    /* position x of a letter size */
    if (fal_atoi(elm[FAL_FONT_ELM_LETTER_X], &(tmp_data->letter.x)) ==
        FAL_ERROR) {
      break;
    }
    /* position y of a letter size */
    if (fal_atoi(elm[FAL_FONT_ELM_LETTER_Y], &(tmp_data->letter.y)) ==
        FAL_ERROR) {
      break;
    }
    /* codeset */
    if (fal_set_cs(elm[FAL_FONT_ELM_CODE_SET], &(tmp_data->cd_set)) ==
        FAL_ERROR) {
      break;
    }
    /* a equipment of an output permission */
    if (fal_set_prm(elm[FAL_FONT_ELM_PERMISSION], &(tmp_data->prm)) ==
        FAL_ERROR) {
      break;
    }
...
int searchFontFileName(FalFontData data, char *fullPathName) {
  int rtn;
  rtn = falReadFontInfoLists(SRCHFNAME, NULL, 0, NULL, data, fullPathName, NULL,
                             NULL);
  if (fal_utyerrno != 0) {
...
static int CR_to_NULL(char *buf) {
  for (; *buf != '\0'; buf++) {
    if (*buf == '\n') {
      *buf = '\0';
      break;
...
static int fal_cmp_data(FalFontData *op1, FalFontData *key, int mask) {
  if (mask == 0) {
    return (0);
  }
  ...
  if ((mask & FAL_FONT_MASK_XLFDNAME) && strcmp(op1->xlfdname, key->xlfdname)) {
    return _FAL_TRY_NEXT;
  }
  if ((mask & FAL_FONT_MASK_SIZE_W) && (op1->size.w != key->size.w)) {
    return _FAL_TRY_NEXT;
  }
  if ((mask & FAL_FONT_MASK_SIZE_H) && (op1->size.h != key->size.h)) {
    return _FAL_TRY_NEXT;
  }
  if ((mask & FAL_FONT_MASK_LETTER_W) && (op1->letter.w != key->letter.w)) {
    return _FAL_TRY_NEXT;
  }
  if ((mask & FAL_FONT_MASK_LETTER_H) && (op1->letter.h != key->letter.h)) {
    return _FAL_TRY_NEXT;
  }
  if ((mask & FAL_FONT_MASK_LETTER_X) && (op1->letter.x != key->letter.x)) {
    return _FAL_TRY_NEXT;
  }
  if ((mask & FAL_FONT_MASK_LETTER_Y) && (op1->letter.y != key->letter.y)) {
    return _FAL_TRY_NEXT;
  }
  if ((mask & FAL_FONT_MASK_STYLE_DEF) && (op1->style.def != key->style.def)) {
    return _FAL_TRY_NEXT;
  }
  if ((op1->style.name != NULL) && (key->style.name != NULL)) {
    if ((mask & FAL_FONT_MASK_STYLE_NAME) &&
        strcmp(op1->style.name, key->style.name)) {
      return _FAL_TRY_NEXT;
    }
  }
  if ((mask & FAL_FONT_MASK_SHAPE_DEF) && (op1->shape.def != key->shape.def)) {
    return _FAL_TRY_NEXT;
  }
  if ((op1->shape.name != NULL) && (key->shape.name != NULL)) {
    if ((mask & FAL_FONT_MASK_SHAPE_NAME) &&
        strcmp(op1->shape.name, key->shape.name)) {
      return _FAL_TRY_NEXT;
    }
  }
  FAL_CMP_DATA_OPTION(mask, op1, key);
  if ((mask & FAL_FONT_MASK_CODE_SET) && (op1->cd_set != key->cd_set)) {
    return _FAL_TRY_NEXT;
  }
...
int FalFontOfFontID(FalFontID fid, FalFontData *fontdata) {
  int rtn;
  FalFontData dummy_data = {0};
  rtn = falReadFontInfoLists(FONTOFID, NULL, 0, NULL, dummy_data, NULL, fid,
                             fontdata);
  if (fal_utyerrno != 0) {
...
FalFontID FalOpenSysFont(FalFontData *open_font_data, int font_data_mask,
  ...
  retFL =
      FalGetFontList(open_font_data, font_data_mask, missing_font_list_return);
  ...
  if ((*missing_font_list_return)->num > 1) {
    ...
    if (searchFontFileName((*missing_font_list_return)->list[0], fname) ==
        FAL_ERROR) {
    ...
    if (retOF == (FalFontID)FAL_ERROR) {
      ...
      *missing_font_list_return = NULL;
      FalFreeFontList(*missing_font_list_return);
      if (fal_add_fidinf(retOF, dspcode, slct_cdset, islock) == FAL_ERROR) {
        fal_utyerrno |= (FAL_FUNCNUM_OPNFNT << 8);
        return 0;
      }
      return retOF;
 ...
 ********************************************************/
static FILE *open_fonts_list(void) {
  ...
  GETLISTFILEPATH(locale, fal_search_path, flist_d, flist_c, flist_l,
                  FONTS_LIST_FILE);
...
static int set_font_pathlist(FalFontPath *pathlist, int nodef) {
  FILE *fp;
    ...
    if (buf[0] == '#') { /* comments */
      ...
      if (font_path) {
        ...
        if (pathlist->path == NULL) {
          ...
          tmp_list =
              (char **)realloc(pathlist->path, sizeof(char *) * (num + 1));
        }
        if (tmp_list == NULL) {
          fclose(fp);
          free(font_path);
...
static int make_default_path(FalFontPath *pathlist) {
  struct stat statbuf;
...
static int falReadFontInfoLists(int func,
                                /* parameters for FalGetFontList	*/
                                FalFontData *lstkey, int mask,
                                FalFontDataList *fls,
                                /* parameters for searchFontFileName	*/
                                FalFontData fnkey, char *fullpath,
                                /* parameters for FalFontOfFontID	*/
                                FalFontID fid, FalFontData *fdata) {
  FILE *fp;
  ...
  if (execDefined == TRUE && IsDefaultPath == FALSE) {
    ...
    while (fgets(buf, FAL_LINE_MAX, fp) != (char *)NULL) {
       ...
       */
      switch (
          fal_get_def_fontdata(func, pname, buf, elm, &tmp_data, keyfname)) {
      case _FAL_TRY_NEXT:
        if (buf[0] == '/') {
          buf[strlen(buf) - 1] = '/';
          strcpy(pname, buf);
  ...
  if (execUndefined == TRUE) {
    ...
    for (i = 0; i < wpath.num; i++) {
      ...
      fonts_list_file = (char *)malloc(
          sizeof(char) * (strlen(wpath.path[i]) + strlen(FONTSDIR) + 1));
      ...
      if (func & MKFONTLIST) {
        if (fls->list == NULL) {
          p_bak =
              (FalFontData *)malloc(sizeof(FalFontData) * (fls->num + fontnum));
        } else {
          p_bak = (FalFontData *)realloc(fls->list, sizeof(FalFontData) *
                                                        (fls->num + fontnum));
        }
        if (p_bak == NULL) {
          fal_utyerror = _FAL_MALOC_ER;
      ...
      while (fgets(buf, FAL_LINE_MAX, fp) != (char *)NULL) {
        ...
        if ((ep = strchr(buf, ' ')) == (char *)NULL)
          continue;
        else
          *ep = 0;
        /* refuse font file (not gpf) */
        if ((strcmp(FILE_SUFFIX(buf), PCFSUFFIX) != 0) &&
        ...
        if ((func & SRCHFNAME) || (func & FONTOFID)) {
          fontFileName = (char *)malloc(
              sizeof(char) * (strlen(wpath.path[i]) + strlen(buf) + 1));
        ...
        xlfd = ++ep;
        switch (fal_get_undef_fontdata(func, fontFileName, xlfd, &tmp_data,
                                       keyfname, &codeset_num, &codeset_list)) {
        case _FAL_TRY_NEXT:
          if ((func & SRCHFNAME) || (func & FONTOFID))
...
/* clear code set informations */
static void clear_charset_info(void) {
  if (charset_str_buf)
    free(charset_str_buf);
  charset_str_buf = NULL;
  if (codeset_list_sav)
    free(codeset_list_sav);
  codeset_list_sav = NULL;
  codeset_list_num = 0;
}

static int fal_get_def_fontdata(int func, char *pname, /* font path */
                                char *buf,             /* buffer for fgets() */
                                char **elm, FalFontData *tmp_data,
                                char *key_fname) {
  int rtn;
...
static int falgetfontlist(FalFontData *tmp_data, FalFontData *key, int mask,
                          FalFontDataList *fls, int fontnum) {
  FalFontData *p_bak;
...
static int fal_get_undef_fontdata(int func, char *full_path, char *xlfd,
                                  FalFontData *tmp_data, char *key_fname,
                                  int *codeset_num, int **codeset_list) {
  int rtn, pix;
  char *char_set;
  ...
  if ((func & MKFONTLIST) || (func & SRCHFNAME)) {
    ...
    if (charset_str_buf && !strcmp(charset_str_buf, char_set)) {
      ...
      *codeset_list = codeset_list_sav;
      ...
      if ((rtn = fal_get_codeset(fal_locale, char_set, codeset_list,
                                 codeset_num))) {
        return rtn;
      }
...
int FalFontIDToFileName(FalFontID fid, char **file_name) {
  char *fname;
  Oak_FontInf *finf;
  if (fid == NULL) {
    fal_utyerror = _FAL_PARM_ER;
...
int FalFreeGI(FalGIInf *ginf, int num) {
  int i;
  if (ginf == NULL) {
    fal_utyerror = _FAL_PARM_ER;
    fal_utyerrno = FAL_ERR_PARM;
...
static int fal_add_fidinf(FalFontID fid, int dspcode, int cd_set, int islock) {
  int exist, i, cnt;
  FontIDInfo **flist;
  if (fid == NULL) {
    fal_utyerror = _FAL_PARM_ER;
...
static int fal_read_fidinf(FalFontID fid, FontIDInfo *fontid_inf) {
  int i;
  if (fid == NULL) {
    fal_utyerror = _FAL_PARM_ER;
    fal_utyerrno = FAL_ERR_PARM;
...
static int fal_del_fidinf(FalFontID fid) {
  int i, cnt, target;
  FontIDInfo **flist;
  ...
  if (cnt > 0) {
    if ((flist = (FontIDInfo **)realloc(mngfid.finf,
                                        sizeof(FontIDInfo *) * cnt)) == NULL) {
      fal_utyerror = _FAL_MALOC_ER;
      fal_utyerrno = FAL_ERR_MALLOC;
      return FAL_ERROR;
    }
...
static int fal_conv_code_to_glyph(FalFontID fid, unsigned int code,
                                  unsigned int *glidx) {
  Oak_FontInf *finf;
  FalFontData tmp_data;
  FalGIInf *gi;
...
static int fal_conv_glyph_to_code(Oak_FontInf *finf, int dspcode, int cd_set,
                                  unsigned int glidx, unsigned int *code) {
  FalFontData tmp_data;
  int inner_code;
  char *char_set;
  ...
  if (dspcode != FAL_FONT_GLYPH_INDEX) {
    ...
    if (COMM_SBFNT_ISSBFNT(char_set)) {
      ...
      if (fal_glyph_to_code(fal_locale, char_set, cd_set, glidx, &temp_code)) {
        fal_clear_data(&tmp_data);
        return FAL_ERROR;
      }
      inner_code = (unsigned int)temp_code;
    }
    fal_clear_data(&tmp_data);
  } else {
...
static int file_lock(int fd) /* a file descripter */
{
  struct flock flpar;
...
static int file_unlock(int fd) /* a file descripter */
{
  struct flock flpar;
...
static int is_lock(int fd) /* file descripter */
{
  struct flock flpar;
