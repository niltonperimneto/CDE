...
static CARD32 getLSB32(unsigned char *);
static int getINT32(unsigned char *, CARD32);
static int getINT16(unsigned char *, CARD32);
static Bool seekToType(PCFTablePtr, int, CARD32, CARD32 *, CARD32 *, CARD32 *);
static void getMetric(caddr_t, CARD32, xCharInfo *);
static Bool getAccel(FontInfoPtr, xCharInfo *, caddr_t, PCFTablePtr, int,
                     CARD32);
int falInitReadPcf(struct pcf_inf *, caddr_t);
static void ByteSwap(char *, int);
static void repadBits(char *, CARD32, int, int, char *);
int falPcfGlyph(char *, Oak_FontInf *, int);
void falGetPcfGSize(struct pcf_inf *, unsigned int *, unsigned int *);
int falInitReadPcfProp(int, Oak_FontInf *, FalFontData *);
int falInitReadSnfProp(Oak_FontInf *, caddr_t, FalFontData *);
int falReadGpfProp(int, Oak_FontInf *, int, FalFontData *);

extern void BitOrderInvert(char *, int);
extern void set_errfile_str(char *, char *);
...
static Bool seekToType(PCFTablePtr tables, int ntables, CARD32 type,
                       CARD32 *formatp, CARD32 *sizep, CARD32 *offsetp) {
  int i;
...
static Bool getAccel(FontInfoPtr pFontInfo, xCharInfo *maxink, caddr_t buf_top,
                     PCFTablePtr tables, int ntables, CARD32 type) {
  CARD32 format;
  CARD32 offset;
  caddr_t buffer;

  if (!seekToType(tables, ntables, (CARD32)type, &format, (CARD32 *)NULL,
                  &offset))
    return FALSE;
  ...
  /*  pFontInfo->maxOverlap = getINT32( (unsigned char *)buffer,
   * (CARD32)format); */
  buffer += 4;
...
int falInitReadPcf(struct pcf_inf *pcfinf, caddr_t buftop) {
  CARD32 format;
  CARD32 offset;
  CARD32 *bitmapSizes;
  xCharInfo maxink;
  ...
  if (!seekToType(pcfinf->tables, pcfinf->ntables, (CARD32)PCF_BITMAPS, &format,
                  (CARD32 *)NULL, &offset))
    return -1;
  ...
  pcfinf->sizebitmaps =
      getINT32((unsigned char *)&bitmapSizes[PCF_GLYPH_PAD_INDEX(format)],
               (CARD32)format);
  ...
  buffp += 2;
  /*
      pcfinf->info.defaultCh	= getINT16( (unsigned char *)buffp,
     (CARD32)format); buffp += 2;
  */
...
static void repadBits(char *src, CARD32 format, int width, int height,
                      char *dest) {
  ...
  for (i = 0; i < height; i++, src += src_bytewidth, dest += dest_bytewidth) {
...
int falPcfGlyph(char *glyph, Oak_FontInf *finf, int code) {
  int encode;
  int inner_code;
  char *bitmap;
  int encodingOffset;
  ...
  encode = (codeRow - finf->pFinf->firstRow) *
           (finf->pFinf->lastCol - finf->pFinf->firstCol + 1);
  encode += codeCol - finf->pFinf->firstCol;
  encodingOffset =
      getINT16((unsigned char *)(finf->pcfinf.encodingOffsets + encode),
               finf->pcfinf.enc_fmt);
  ...
  bitmap = finf->pcfinf.bitmaps +
           getINT32((unsigned char *)(finf->pcfinf.offsets + encodingOffset),
                    finf->pcfinf.bmp_fmt);

  bmp_height = finf->pFinf->maxbounds.ascent + finf->pFinf->maxbounds.descent;
  if ((adj_hi = finf->pFinf->maxbounds.ascent -
                finf->pcfinf.org_bounds.ascent) > 0) {
    bytewidth = 8 * PCF_GLYPH_PAD(finf->pcfinf.bmp_fmt);
    bytewidth = ((finf->width + bytewidth - 1) / bytewidth) *
                PCF_GLYPH_PAD(finf->pcfinf.bmp_fmt);
  ...
  repadBits(bitmap + bmp_adj, finf->pcfinf.bmp_fmt, finf->width, cpy_height,
            glyph + ptn_adj);
  return (0);
}

void falGetPcfGSize(struct pcf_inf *pcfinf, unsigned int *widthp,
                    unsigned int *heightp) {
  unsigned int w, h;

  w = pcfinf->org_bounds.rightSideBearing - pcfinf->org_bounds.leftSideBearing;
...
static char *getPcfFontProp(caddr_t, PCFTablePtr, int, char *);
static char *getSnfFontProp(caddr_t, int, char *);
/*
 * read properties and get font style and
 * letter size
 */

int falReadFontProp(char *file, /* name of font file */
                    int protect_key_data, FalFontData *databuff, int islock) {
  Oak_FontInf finf;
  int fd;
  char *buf;
  char *openfontfile;
 ...
 */
int falReadGpfProp(int updflg, Oak_FontInf *finf, int protect_key_data,
 ...
 */
int falInitReadPcfProp(int updflg, Oak_FontInf *finf, FalFontData *databuff) {
   ...
   * read property "FONT"
   */
  if (buffp = getPcfFontProp(buftop, pcfinf->tables, pcfinf->ntables, "FONT")) {
   ...
   */
  if (buffp = getPcfFontProp(buftop, pcfinf->tables, pcfinf->ntables,
                             "FAMILY_NAME")) {
...
static char *getPcfFontProp(caddr_t buftop, PCFTablePtr tables, int ntables,
                            char *propname) {
  caddr_t buffer;
  int name_ofs;
  int i;
  ...
  if (!seekToType(tables, ntables, (CARD32)PCF_PROPERTIES, &format,
                  (CARD32 *)NULL, &offset))
    return NULL;
  ...
  for (i = 0; i < nprops; i++) {
    ...
    if (strcmp(propstr + name_ofs, propname) == 0) {
      if (*buffer) { /* isStringProp */
        return (propstr +
                getINT32((unsigned char *)(buffer + 1), (CARD32)format));
      } else {
        return ((char *)(intptr_t)getINT32((unsigned char *)(buffer + 1),
                                           (CARD32)format));
      }
    }
...
static char *getSnfFontProp(caddr_t buftop, int nprops, char *propname) {
  caddr_t buffer;
  int name_ofs;
  int i;
  char *propstr;
