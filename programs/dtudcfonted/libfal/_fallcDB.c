...
typedef struct {
  ...
  int (*parse_proc)(char *, Token, Database *); /* parsing procedure */
} TokenTable;

static int f_newline(char *, Token, Database *);
static int f_comment(char *, Token, Database *);
static int f_semicolon(char *, Token, Database *);
static int f_double_quote(char *, Token, Database *);
static int f_left_brace(char *, Token, Database *);
static int f_right_brace(char *, Token, Database *);
static int f_white(char *, Token, Database *);
static int f_backslash(char *, Token, Database *);
static int f_numeric(char *, Token, Database *);
static int f_default(char *, Token, Database *);
...
static void clear_parse_info(void) {
  int i;
  char *ptr;
  int size;
...
static Bool realloc_parse_info(int len) {
  char *p;
...
static int realloc_line(Line *line, int size) {
  char *str = line->str;
...
static void zap_comment(char *str, int *quoted) {
  char *p = str;
  ...
  while (*p) {
    ...
    if (*p == SYM_COMMENT && !*quoted) {
      int pos = p - str;
      if (pos == 0 || iswhite(p[-1]) && (pos == 1 || p[-2] != SYM_BACKSLASH)) {
        int len = strlen(p);
...
static int read_line(FILE *fd, Line *line) {
  char buf[BUFSIZE], *p;
  int len;
...
static Token get_token(char *str) {
  switch (*str) {
  case SYM_NEWLINE:
    return T_NEWLINE;
  case SYM_COMMENT:
...
static int get_quoted_word(char *str, char *word) {
  char *p = str, *w = word;
  Token token;
  int token_len;
...
static int append_value_list(void) {
...
static int construct_name(char *name) {
  int i, len = 0;
  char *p = name;
...
static int store_to_database(Database *db) {
...
static int check_category_end(char *str) {
  char *p;
  int len;
...
static int f_newline(char *str, Token token, Database *db) {
...
static int f_comment(char *str, Token token, Database *db) {
...
static int f_white(char *str, Token token, Database *db) {
  char *p = str;
...
static int f_semicolon(char *str, Token token, Database *db) {
  switch (parse_info.pre_state) {
  case S_NULL:
  case S_CATEGORY:
  case S_NAME:
...
static int f_left_brace(char *str, Token token, Database *db) {
  switch (parse_info.pre_state) {
  case S_NULL:
  case S_CATEGORY:
    return 0;
...
static int f_right_brace(char *str, Token token, Database *db) {
...
static int f_double_quote(char *str, Token token, Database *db) {
  char word[BUFSIZE];
  int len = 0;
  ...
  case S_VALUE:
    ...
    if ((parse_info.bufsize + (int)strlen(word) + 1) >= parse_info.bufMaxSize) {
...
static int f_backslash(char *str, Token token, Database *db) {
  return f_default(str, token, db);
}

static int f_numeric(char *str, Token token, Database *db) {
  char word[BUFSIZE], *p;
  int len;
  int token_len;
...
static int f_default(char *str, Token token, Database *db) {
  char word[BUFSIZE], *p;
  int len;
...
#ifdef DEBUG
static void PrintDatabase(Database db) {
  Database p = db;
  int i = 0, j;
...
static void DestroyDatabase(Database db) {
  Database p = db;
...
static int CountDatabase(Database db) {
  Database p = db;
  int cnt = 0;
...
static Database CreateDatabase(char *dbfile) {
...
/************************************************************************/
void _fallcGetResource(XLCd lcd, char *category, char *class, char ***value,
                       int *count) {
...
/*	This function retrieves XLocale database information.		*/
/************************************************************************/
void _fallcGetLocaleDataBase(XLCd lcd, char *category, char *name,
...
/************************************************************************/
void _fallcDestroyLocaleDataBase(XLCd lcd) {
  XlcDatabase lc_db = (XlcDatabase)XLC_PUBLIC(lcd, xlocale_db);
...
/*	the specified XLCd.						*/
/************************************************************************/
XPointer _fallcCreateLocaleDataBase(XLCd lcd) {
