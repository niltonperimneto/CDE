...
static void our_wctomb(unsigned short r, char **bufptr, int *buf_len);
static int our_mbtowc(unsigned long *p, char *s, size_t n);
static void latin2rune(unsigned char c, Rune *r);
static void jis02012rune(unsigned char c, Rune *r);
static void jis02082rune(unsigned char c, Rune *r);
static void ksc2rune(unsigned char c, Rune *r);
static void gb2rune(unsigned char c, Rune *r);
...
static void set_latin_nop(long *table, long default_val) {
  int i;
  for (i = 0; i < 0x1fff; i++)
    table[i] = default_val;
  return;
...
static void set_latin_tab(FILE *fptr, long *table, long fb_default) {
  int i = 0;
  int j = 0;
  int rv = 0;
  long value;
...
extern int _fallcResolveI18NPath(char *);
...
static void set_cjk_nop(long **to_tbl, int to_max, long default_val) {
  int i;
  for (i = 0; i < to_max; i++)
...
static void set_table(FILE *fptr, long **to_tbl, long *from_tbl, int to_max,
                      long fb_default) {
  int i = 0;
  int j = 0;
  int rv = 0;
...
static UtfData make_entry(void) {
...
static int once = 0;
static int _XlcInitUTFInfo(XLCd lcd) {
  if (!once) {
    int i;
    ...
    for (i = 0; i < codeset_num; i++) {
      ...
      if (pdata->next) {
        ...
        for (j = 0; j < MAX_UTF_CHARSET; j++) {
          if (_fallcCompareISOLatin1(charset->encoding_name,
                                     default_utf_data[j].name) ||
            ...
            (*pdata->initialize)(pdata->fromtbl,
                                 default_utf_data[j].fallback_value);
...
static int utftocs(XlcConv conv, char **from, int *from_left, char **to,
  ...
  while (utf_len > 0 && buf_len > 0) {
    ...
    if ((l = getutfrune(&p, &utf_len)) == -2) {
      ...
      if (!tmpcharset)
        return (-1); /* Unknown Codepoint */
    }
  }
}

static int utf1tocs(XlcConv conv, char **from, int *from_left, char **to,
                    int *to_left, XPointer *args, int num_args) {
  char **ptr = NULL;
...
static int ucstocs(XlcConv conv, XPointer *from, int *from_left, char **to,
                   int *to_left, XPointer *args, int num_args) {
  wchar_t *ucsptr;
  char *bufptr;
  int ucs_len, buf_len;
...
static int cstoutf(XlcConv conv, char **from, int *from_left, char **to,
                   int *to_left, XPointer *args, int num_args) {
  XlcCharSet charset;
  char *csptr, *utfptr;
  int csstr_len, utf_len;
  int cmp_len = 0;
  void (*putrune)(unsigned char c, Rune *r) = NULL;
  Rune r = (Rune)0;
  UtfData pdata = utfdata_list;
  ...
  while (pdata->next) {
    if (!_fallcNCompareISOLatin1(charset->name, pdata->charset->name,
                                 cmp_len)) {
...
static int cstoucs(XlcConv conv, char **from, int *from_left, XPointer *to,
                   int *to_left, XPointer *args, int num_args) {
  XlcCharSet charset;
  char *csptr;
  wchar_t *ucsptr;
...
static void our_wctomb(Rune r, char **utfptr, int *utf_len) {
  long l = (long)r;
...
static void jis02012rune(unsigned char c, Rune *r) { /* To Be Implemented */ }
  ...
  if (c0 >= T3) {
    ...
    if (c0 >= T5) {
      ...
      if (c0 >= T6) {
        ...
        wc = ((((((((((c0 & Mask6) << Bitx) | c1) << Bitx) | c2) << Bitx) | c3)
                << Bitx) |
               c4)
              << Bitx) |
             c5;
      ...
      /* 5 bytes */
      wc = ((((((((c0 & Mask5) << Bitx) | c1) << Bitx) | c2) << Bitx) | c3)
            << Bitx) |
           c4;
      if (wc <= Wchar4) {
        errno = EILSEQ;
    ...
    if (c0 >= T4) {
      ...
      wc = ((((((c0 & Mask4) << Bitx) | c1) << Bitx) | c2) << Bitx) | c3;
      if (wc <= Wchar3) {
        errno = EILSEQ;
        return -1;
      }
...
static void close_converter(XlcConv conv) { Xfree((char *)conv); }
...
static XlcConvMethodsRec mbtocs_methods = {close_converter, utf1tocs, NULL};

static XlcConv open_mbtocs(XLCd from_lcd, char *from, XLCd to_lcd, char *to) {
  return create_conv(from_lcd, &mbtocs_methods);
}

static XlcConvMethodsRec mbstocs_methods = {close_converter, utftocs, NULL};

static XlcConv open_mbstocs(XLCd from_lcd, char *from, XLCd to_lcd, char *to) {
  return create_conv(from_lcd, &mbstocs_methods);
}

static XlcConvMethodsRec wcstocs_methods = {close_converter, ucstocs, NULL};

static XlcConv open_wcstocs(XLCd from_lcd, char *from, XLCd to_lcd, char *to) {
  return create_conv(from_lcd, &wcstocs_methods);
}

static XlcConvMethodsRec cstombs_methods = {close_converter, cstoutf, NULL};

static XlcConv open_cstombs(XLCd from_lcd, char *from, XLCd to_lcd, char *to) {
  return create_conv(from_lcd, &cstombs_methods);
}

static XlcConvMethodsRec cstowcs_methods = {close_converter, cstoucs, NULL};

static XlcConv open_cstowcs(XLCd from_lcd, char *from, XLCd to_lcd, char *to) {
  return create_conv(from_lcd, &cstowcs_methods);
}

XLCd _fallcUtfLoader(char *name) {
  XLCd lcd;
