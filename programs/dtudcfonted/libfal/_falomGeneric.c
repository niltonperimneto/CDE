...
extern int _falmbDefaultTextEscapement(XOC, const char *, int);
extern int _falwcDefaultTextEscapement(XOC, const wchar_t *, int);
extern int _falmbDefaultTextExtents(XOC, const char *, int, XRectangle *,
                                    XRectangle *);
extern int _falwcDefaultTextExtents(XOC, const wchar_t *, int, XRectangle *,
                                    XRectangle *);
extern Status _falmbDefaultTextPerCharExtents(XOC, const char *, int,
                                              XRectangle *, XRectangle *, int,
                                              int *, XRectangle *,
                                              XRectangle *);
extern Status _falwcDefaultTextPerCharExtents(XOC, const wchar_t *, int,
                                              XRectangle *, XRectangle *, int,
                                              int *, XRectangle *,
                                              XRectangle *);
extern int _falmbDefaultDrawString(Display *, Drawable, XOC, GC, int, int,
                                   const char *, int);
extern int _falwcDefaultDrawString(Display *, Drawable, XOC, GC, int, int,
                                   const wchar_t *, int);
extern void _falmbDefaultDrawImageString(Display *, Drawable, XOC, GC, int, int,
                                         const char *, int);
extern void _falwcDefaultDrawImageString(Display *, Drawable, XOC, GC, int, int,
                                         const wchar_t *, int);

extern int _falmbGenericTextEscapement(XOC, const char *, int);
extern int _falwcGenericTextEscapement(XOC, const wchar_t *, int);
extern int _falmbGenericTextExtents(XOC, const char *, int, XRectangle *,
                                    XRectangle *);
extern int _falwcGenericTextExtents(XOC, const wchar_t *, int, XRectangle *,
                                    XRectangle *);
extern Status _falmbGenericTextPerCharExtents(XOC, const char *, int,
                                              XRectangle *, XRectangle *, int,
                                              int *, XRectangle *,
                                              XRectangle *);
extern Status _falwcGenericTextPerCharExtents(XOC, const wchar_t *, int,
                                              XRectangle *, XRectangle *, int,
                                              int *, XRectangle *,
                                              XRectangle *);
extern int _falmbGenericDrawString(Display *, Drawable, XOC, GC, int, int,
                                   const char *, int);
extern int _falwcGenericDrawString(Display *, Drawable, XOC, GC, int, int,
                                   const wchar_t *, int);
extern void _falmbGenericDrawImageString(Display *, Drawable, XOC, GC, int, int,
                                         const char *, int);
extern void _falwcGenericDrawImageString(Display *, Drawable, XOC, GC, int, int,
                                         const wchar_t *, int);

extern void dbg_printValue(char *str, char **value, int num);

extern int falGetFontProperty(XFontStruct *, Atom, unsigned long *);
extern int falFreeFontInfo(char **, XFontStruct *, int);
extern int falFreeFontNames(char **);
extern int falFreeFont(Display *, XFontStruct *);
extern void falFreeStringList(char **);

/* For VW/UDC start */

static FontData init_fontdata(FontData font_data, int font_data_count) {
  FontData fd;
  int i;
...
static VRotate init_vrotate(FontData font_data, int font_data_count, int type,
                            CodeRange code_range, int code_range_num) {
  VRotate vrotate;
  int i;
...
static Bool init_fontset(XOC oc) {
  XOCGenericPart *gen;
  FontSet font_set;
  OMData data;
  int count;
  ...
  for (; count-- > 0; data++, font_set++) {
    ...
    if ((font_set->vmap = init_fontdata(data->vmap, data->vmap_num)) == NULL) {
      err += 1;
      break;
    }
    ...
    if (data->vrotate_type != VROTATE_NONE) {
      ...
      /* as initial value.                               */
      if ((font_set->vrotate = init_vrotate(
               data->font_data, data->font_data_count, data->vrotate_type,
               data->vrotate, data->vrotate_num)) == NULL) {
        err += 1;
        break;
      }
...
static char *get_prop_name(Display *dpy, XFontStruct *fs) {
  unsigned long fp;
...
static FontData check_charset(FontSet font_set, char *font_name) {
  FontData font_data;
  char *last;
...
static int check_fontname(XOC oc, char *name, int found_num) {
  ...
  for (i = 0; i < list_num; i++) {
    ...
    for (; font_set_num-- > 0; font_set++) {
      ...
      if ((data = check_charset(font_set, fname)) == NULL) {
        if ((fn2_list = falListFontsWithInfo(dpy, name, MAXFONTS, &list2_num,
                                             &fs_list)) &&
...
static Bool load_fontdata(XOC oc, FontData font_data, int font_data_num) {
  Display *dpy = oc->core.om->core.display;
...
static Bool load_font(XOC oc) {
  ...
  for (; num-- > 0; font_set++) {
    ...
    load_fontdata(oc, (FontData)font_set->vrotate, font_set->vrotate_num);
...
static Bool load_font_info(XOC oc) {
  Display *dpy = oc->core.om->core.display;
  XOCGenericPart *gen = XOC_GENERIC(oc);
  FontSet font_set = gen->font_set;
  char **fn_list;
...
static void check_fontset_extents(XCharStruct *overall, int *logical_ascent,
                                  int *logical_descent, XFontStruct *font) {
...
static void set_fontset_extents(XOC oc) {
...
static Bool init_core_part(XOC oc) {
  XOCGenericPart *gen = XOC_GENERIC(oc);
  FontSet font_set;
  int font_set_num;
...
static char *get_font_name(XOC oc, char *pattern) {
  char **list, *name;
  int count = 0;
...
static char *get_rotate_fontname(char *font_name) {
  ...
  snprintf(str_pixel, sizeof(str_pixel), "[ 0 ~%d %d 0 ]", pixel_size,
           pixel_size);
  fields[6] = str_pixel;
  ...
  for (field_num = 0; field_num < CHARSET_ENCODING_FIELD && fields[field_num];
       field_num++) {
    snprintf(rotate_font, sizeof(rotate_font), "%s-%s", rotate_font,
             fields[field_num]);
  }
...
static Bool is_match_charset(FontData font_data, char *font_name) {
  char *last;
  int length, name_len;
...
static int parse_all_name(XOC oc, FontData font_data, char *pattern) {
...
static int parse_omit_name(XOC oc, FontData font_data, char *pattern) {
...
static int parse_fontdata(XOC oc, FontData font_data, int font_data_count,
  ...
  for (; font_data_count-- > 0; font_data++) {
    ...
    case C_VROTATE:
      if (is_found == True) {
        ...
        if ((rotate_name = get_rotate_fontname(font_data->xlfd_name)) != NULL) {
...
static int parse_vw(XOC oc, FontSet font_set, char **name_list, int count) {
  ...
  if (vrotate_num > 0) {
    ret = parse_fontdata(oc, (FontData)vrotate, vrotate_num, name_list, count,
                         C_VROTATE);
    if (ret == -1) {
      return (-1);
    } else if (ret == False) {
      ...
      if (sub_num > 0) {
        vrotate = font_set->vrotate =
            (VRotate)Xmalloc(sizeof(VRotateRec) * sub_num);
...
static int parse_fontname(XOC oc) {
  XOCGenericPart *gen = XOC_GENERIC(oc);
  FontSet font_set;
  ...
  for (; font_set_num-- > 0; font_set++) {
    ...
    if (font_set->font_data_count > 0) {
      ret = parse_fontdata(oc, font_set->font_data, font_set->font_data_count,
      ...
      if (ret == -1) {
        ...
        font_set->font_name =
            (char *)Xmalloc(strlen(font_set->font_data->xlfd_name) + 1);
        ...
        ret = parse_fontdata(oc, font_set->substitute, font_set->substitute_num,
        ...
        if (ret == -1) {
          ...
          font_set->font_name =
              (char *)Xmalloc(strlen(font_set->substitute[i].xlfd_name) + 1);
          if (font_set->font_name == (char *)NULL) {
            falFreeStringList(name_list);
            return -1;
          }
...
static Bool set_missing_list(XOC oc) {
  XOCGenericPart *gen = XOC_GENERIC(oc);
  FontSet font_set;
    ...
    if (font_set->font_data_count <= 0 ||
      ...
      if (font_set->substitute_num <= 0 ||
        ...
        if (font_set->charset_list != NULL) {
          length += strlen(font_set->charset_list[0]->encoding_name) + 1;
        } else {
          length += 1;
        }
      else
  ...
  for (; font_set_num-- > 0; font_set++) {
    ...
    if (font_set->font_data_count <= 0 ||
      ...
      if (font_set->substitute_num <= 0 ||
          font_set->substitute == (FontData)NULL)
        if (font_set->charset_list != NULL) {
          strcpy(charset_buf, font_set->charset_list[0]->encoding_name);
        } else {
          strcpy(charset_buf, "");
        }
      else
...
static Bool create_fontset(XOC oc) {
...
/* For VW/UDC start */
static void free_fontdataOC(Display *dpy, FontData font_data,
                            int font_data_count) {
  for (; font_data_count--; font_data++) {
...
static void destroy_fontdata(XOCGenericPart *gen, Display *dpy) {
  ...
  if (gen->font_set) {
    ...
    for (; font_set_num--; font_set++) {
      if (font_set->font_data) {
        free_fontdataOC(dpy, font_set->font_data, font_set->font_data_count);
        Xfree(font_set->font_data);
        font_set->font_data = NULL;
      }
      if (font_set->substitute) {
        free_fontdataOC(dpy, font_set->substitute, font_set->substitute_num);
        Xfree(font_set->substitute);
      ...
      if (font_set->vrotate) {
        free_fontdataOC(dpy, (FontData)font_set->vrotate,
...
static char *set_oc_values(XOC oc, XlcArgList args, int num_args) {
...
static char *get_oc_values(XOC oc, XlcArgList args, int num_args) {
  if (oc->core.resources == NULL)
    return NULL;
...
static XOCMethodsListRec oc_methods_list[] = {{"default", &oc_default_methods},
...
static XOC create_oc(XOM om, XlcArgList args, int num_args) {
  XOC oc;
  ...
  if (_fallcSetValues((XPointer)oc, oc_resources, XlcNumber(oc_resources), args,
                      num_args, XlcCreateMask | XlcDefaultMask)) {
    destroy_oc(oc);
    return (XOC)NULL;
  }
...
static void free_fontdataOM(FontData font_data, int font_data_count) {
  for (; font_data_count--; font_data++) {
    if (font_data->name) {
      Xfree(font_data->name);
      font_data->name = NULL;
...
static char *set_om_values(XOM om, XlcArgList args, int num_args) {
...
static char *get_om_values(XOM om, XlcArgList args, int num_args) {
  if (om->core.resources == NULL)
    return NULL;
...
static XOMMethodsRec methods = {close_om, set_om_values, get_om_values,
                                create_oc};
...
static XOM create_om(XLCd lcd, Display *dpy, XrmDatabase rdb, char *res_name,
                     char *res_class) {
  XOM om;
...
/* For VW/UDC */
FontData falread_EncodingInfo(int count, char **value) {
  FontData font_data, ret;
  char *buf, *bufptr, *scp;
  FontScope scope;
  int len;
  extern FontScope falparse_scopemaps(char *, int *);
...
static CodeRange read_vrotate(int count, char **value, int *type,
                              int *vrotate_num) {
  FontData font_data, ret;
  char *buf, *bufptr, *scp;
  CodeRange range;
...
static void read_vw(XLCd lcd, OMData font_set, int num) {
...
XOM _falomGenericOpenOM(XLCd lcd, Display *dpy, XrmDatabase rdb,
