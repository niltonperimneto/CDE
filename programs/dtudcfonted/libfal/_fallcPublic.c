...
static char *default_string(XLCd);

static XLCd create(char *, XLCdMethods);
static Bool initialize(XLCd);
static void destroy(XLCd);
static char *get_values(XLCd, XlcArgList, int);

static XLCdPublicMethodsRec publicMethods = {
    {destroy, _fallcDefaultMapModifiers, NULL, NULL, _falrmDefaultInitParseInfo,
     _falmbTextPropertyToTextList, _falwcTextPropertyToTextList,
     _falmbTextListToTextProperty, _falwcTextListToTextProperty,
     _falwcFreeStringList, default_string, NULL, NULL},
    {NULL, create, initialize, destroy, get_values, _fallcGetLocaleDataBase}};

XLCdMethods _fallcPublicMethods = (XLCdMethods)&publicMethods;

static char *default_string(XLCd lcd) {
...
static Bool load_public(XLCd lcd) {
...
static Bool initialize_core(XLCd lcd) {
...
static void destroy_core(XLCd lcd) {
  if (lcd->core) {
    if (lcd->core->name)
      Xfree(lcd->core->name);
    Xfree(lcd->core);
