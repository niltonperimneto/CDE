...
static void close_converter(XlcConv conv) { (*conv->methods->close)(conv); }

static XlcConv get_converter(XLCd from_lcd, XrmQuark from_type, XLCd to_lcd,
                             XrmQuark to_type) {
...
Bool _fallcSetConverter(XLCd from_lcd, char *from, XLCd to_lcd, char *to,
...
static int indirect_convert(XlcConv lc_conv, XPointer *from, int *from_left,
...
static void close_indirect_converter(XlcConv lc_conv) {
  Conv conv = (Conv)lc_conv->state;
...
static void reset_indirect_converter(XlcConv lc_conv) {
  Conv conv = (Conv)lc_conv->state;
...
static XlcConvMethodsRec conv_methods = {
    close_indirect_converter, indirect_convert, reset_indirect_converter};

static XlcConv open_indirect_converter(XLCd from_lcd, char *from, XLCd to_lcd,
                                       char *to) {
...
XlcConv _fallcOpenConverter(XLCd from_lcd, char *from, XLCd to_lcd, char *to) {
  XlcConv conv;
  XrmQuark from_type, to_type;
...
void _fallcCloseConverter(XlcConv conv) { close_converter(conv); }

int _fallcConvert(XlcConv conv, XPointer *from, int *from_left, XPointer *to,
                  int *to_left, XPointer *args, int num_args) {
  return (*conv->methods->convert)(conv, from, from_left, to, to_left, args,
                                   num_args);
}

void _fallcResetConverter(XlcConv conv) {
