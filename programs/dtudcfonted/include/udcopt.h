...
#define CHK_PROTECT_KEY(protect_key_data, openfontfile, fal_utyerror,          \
                        fal_utyderror)
/* case of a snf former font */
#define FAL_GET_SNF_HEADER(buf, protect_key_data, fal_utyerror, fal_utyderror, \
                           fd, finf, openfontfile)                             \
  {                                                                            \
...
/* falReadGpfProp() */
#define FAL_READ_SNF_HEADER(finf, protect_key_data, fal_utyerror,              \
                            fal_utyderror)                                     \
  {                                                                            \
    (finf)->isFef = TRUE;                                                      \
    (finf)->isPcf = FALSE;                                                     \
...
#define COMM_DSP_PRM
#define COMM_HELP_MSG_STYLE                                                    \
  USAGE("style      (The default value is mincho style.)\n")
 ...
 * udflist.def
 **********************************************************************/
#define COMM_GET_DEFAULTFILE_ERROR(buf, com, ret, height, style, list_d,       \
                                   list_c, list_l, statbuf)                    \
  {                                                                            \
    buf[0] = '\0';                                                             \
  }
...
#define GETLISTFILEPATH(locale, srch_path, list_d, list_c, list_l, list_file)  \
  {                                                                            \
    ...
    if (((srch_path) = getenv(DTUDCFONTS)) == NULL) {                          \
      snprintf((list_d), sizeof(list_d), "%s/%s", DTUDCFONTSLISTDIR,           \
               (list_file));                                                   \
      snprintf((list_c), sizeof(list_c), "%s/C/%s", DTUDCFONTSLISTDIR,         \
               (list_file));                                                   \
      snprintf((list_l), sizeof(list_l), "%s/%s/%s", DTUDCFONTSLISTDIR,        \
               (locale), (list_file));                                         \
    } else {                                                                   \
      snprintf((list_d), sizeof(list_d), "%s/%s", (srch_path), (list_file));   \
      snprintf((list_c), sizeof(list_c), "%s/C/%s", (srch_path), (list_file)); \
      snprintf((list_l), sizeof(list_l), "%s/%s/%s", (srch_path), (locale),    \
               (list_file));                                                   \
    }                                                                          \
  }
  ...
  {                                                                            \
    if (!((mask) & FAL_FONT_MASK_DEFINED) &&                                   \
      ...
      if ((mask) & FAL_FONT_MASK_DEFINED)                                      \
        ...
        (def) = FALSE;                                                         \
      if ((mask) & FAL_FONT_MASK_UNDEFINED)                                    \
        (undef) = TRUE;                                                        \
      else                                                                     \
        (undef) = FALSE;                                                       \
    }                                                                          \
