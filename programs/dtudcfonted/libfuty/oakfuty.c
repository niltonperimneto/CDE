...
int ReadBdfHeader(struct btophead *head, char *buf) {
  char *p;
  ...
  while (1) {
    ...
    if (!strncmp(p, SIZE, SIZEsz)) {
      if ((sscanf(p, "SIZE %f%d", &(head->bdf_point), &(head->bdf_xdpi))) !=
          2) {
        return (BDF_INVAL);
      }
      getstat |= 0x01;
    } else if (!strncmp(p, FONTBOUNDINGBOX, FONTBOUNDINGBOXsz)) {
      if (sscanf(p, "FONTBOUNDINGBOX %d%d%d%d", &(head->bdf_width),
                 &(head->bdf_height), &(head->bdf_x), &(head->bdf_y)) != 4) {
        return BDF_INVAL;
      }
      getstat |= 0x02;
...
int ReadGpfHeader(struct ptobhead *head, char *buf) {
  char *p;
  unsigned int getstat = 0;
...
int WriteBdfHeader(struct ptobhead *head) {
  FILE *fp;
  int fd[2];
...
int WritePtnToBdf(struct ptobhead *head) {
  ...
  for (i = 0; i < head->num_chars; i++) {
    ...
    swidth = (head->bdf_width * 72270) /
             ((int)(head->bdf_point * (float)head->bdf_xdpi));
    dw = head->bdf_width;
    bbw = head->bdf_width;
    bbh = head->bdf_height;
    bbx = head->bdf_x;
    ...
    if (head->zoomf) {
      if ((rtn = PtnZoom(zoomptn, head->ptn[i], head->p_width, head->p_height,
                         head->bdf_width, head->bdf_height))) {
        return (rtn);
      }
      if ((rtn = PutBdfPtn(zoomptn, head->bdf_width, head->bdf_height,
                           head->output))) {
        return (rtn);
      }
    } else {
      if ((rtn = PutBdfPtn(head->ptn[i], head->bdf_width, head->bdf_height,
                           head->output))) {
        return (rtn);
      }
    }
...
int putDefaultChars(struct ptobhead *head) {
...
int WritePtnToGpf(struct btophead *head) {
  int msize, rtn, i;
  char *zoomptn;
  ...
  for (i = 0; i < head->num_chars; i++) {
    ...
    if (head->zoomf) {
      ...
      if ((rtn = PutGpfPtn(zoomptn, head->p_width, head->p_height,
                           head->output))) {
        return (rtn);
      }
    } else {
      if ((rtn = PutGpfPtn(head->ptn[i], head->bdf_width, head->bdf_height,
                           head->output))) {
        return (rtn);
      }
    }
...
int ReadBdfToMemory(struct btophead *head, char *buf) {
  ...
  while (1) {
    ...
    if ((code > head->end_code) || (code < head->start_code) ||
...
int GetBdfCode(struct btophead *head, char *buf, int *code) {
  char *p;
...
int GetBdfPtn(struct btophead *head, char *buf, char *ptn, int mwidth,
              int bsize) {
  int skip, i, j;
  char *p;
...
int ReadGpfToMemory(struct ptobhead *head, char *buf) {
  int code, mwidth, num_char, bsize, rtn;
  char *ptn;
...
int GetGpfCode(struct ptobhead *head, char *buf, int *code) {
  char *p;
...
int GetGpfPtn(struct ptobhead *head, char *buf, char *ptn, int mwidth,
              int bsize) {
  int skip, i, j;
  char *p;
  ...
  for (i = 0, skip = 0; i < head->p_height; i++) {
    ...
    p = buf;
    SCAN_TO_NONSP(p);
    if ((!strncmp(p, CODE, CODEsz)) || (!strncmp(p, ENDDATA, ENDDATAsz))) {
      skip = 1;
      for (j = 0; j < mwidth; j++) {
        ptn[j] = 0;
      }
...
void GetBdfLinePtn(char *mem, char *buf, int width) {
  int i, iend, len;
  char *p, str[3];
...
void GetGpfLinePtn(char *mem, char *buf, int width) {
...
int PutBdfPtn(unsigned char *ptn, int width, int height, FILE *fp) {
  int i, j, nbyte;
...
int PutGpfPtn(char *ptn, int width, int height, FILE *fp) {
  int i, j, k, nbyte, tw;
...
int PtnZoom(char *dmem, char *smem, int sw, int sh, int dw, int dh) {
  int swidth;
  int dwidth;
  int i, lcnt;
  char *dbuf, *sp, *dp;
...
#if defined(SVR4)
  if ((buf = (char *)mmap(0, statbuf.st_size, PROT_READ, MAP_SHARED, rfd, 0)) ==
      (char *)-1) {
#endif /* SVR4 */
...
void ErrMsgTable_AndExit(int er_no, char *snf_in, char *snf_out, char *gpf_in,
                         char *gpf_out, char *bdf_in, char *prog_name) {
  int rtn = 0;
  switch (er_no) {
  case 0:
    break;
  ...
  case BDF_READ:
    USAGE2("%s : Information from the font file cannot be "
           "extracted.\"%s\"\A1\A5 \n",
           prog_name, (snf_in != NULL) ? snf_in : "\0");
    rtn = READ_FAIL;
    break;
  ...
  case BDF_OPEN_HEAD:
    USAGE2("%s : The BDF file cannot be opened.\"%s\"\A1\A5\n", prog_name,
           (bdf_in != NULL) ? bdf_in : "\0");
    rtn = OPEN_FAIL;
    break;

  case BDF_READ_HEAD:
    USAGE2(
        "%s : Information of the BDF file cannot be extracted.\"%s\"\A1\A5 \n",
  ...
  case GPF_OPEN_IN:
    USAGE2(
        "%s : The input character pattern file cannot be opened.\"%s\"\A1\A5\n",
        prog_name, (gpf_in != NULL) ? gpf_in : "\0");
    rtn = OPEN_FAIL;
    break;

  case GPF_OPEN_OUT:
    USAGE2("%s : The output character pattern file cannot be "
           "opened.\"%s\"\A1\A5\n",
  ...
  case GPF_WRITE:
    USAGE2("%s : It is not possible to write to the character pattern "
           "file.\"%s\"\A1\A5\n",
           prog_name, (gpf_out != NULL) ? gpf_out : "\0");
    rtn = DEVICE_FAIL;
    break;
  ...
  case GPF_INVAL:
    USAGE2("%s : The format of the character pattern file is "
           "illegal.\"%s\"\A1\A5\n",
           prog_name, (gpf_in != NULL) ? gpf_in : "\0");
    rtn = 1;
    break;
...
void fatal_err_msg(int er_no, char *prog_name) {
...
void ChatchSig(int sig) { sig_flg = 1; }

void Put_File_Create_Err_Msg(int msg_level, char *org_name, char *tmp_name,
                             char *save_name, char *com) {
  switch (msg_level) {
  ...
  case GPF_MODIFY_FAIL:
    USAGE1(
        "%s : Failed in the renewal of the font file. Terminates abnormally.\n",
        com);
    put_file_create_err_msg = WRITE_FAIL;
    break;

  case RESTORE_FAIL:
    USAGE3("%s : The file before updating cannot be changed to former file "
           "name.\tPlease execute the following command after the inquiry the "
           "system manager.\n\t mv %s %s\n",
...
int Make_NewFefFile(char *org_name, char *tmp_name, mode_t mode, uid_t owner,
                    gid_t group, char *com) {
  struct stat statbuf;
  char *save_name = NULL;
  int ret_val;
  int msg_level;
...
#ifndef SVR4
  signal(SIGHUP, ChatchSig);
  signal(SIGINT, ChatchSig);
  signal(SIGQUIT, ChatchSig);
  signal(SIGTERM, ChatchSig);
#else
  sigset(SIGHUP, ChatchSig);
  sigset(SIGINT, ChatchSig);
  sigset(SIGQUIT, ChatchSig);
  sigset(SIGTERM, ChatchSig);
#endif

  errno = 0;
  if (sig_flg || ((save_name = GetTmpPath(org_name)) == NULL)) {
    msg_level = (sig_flg) ? CATCH_SIGNAL : GPF_BUCK_UP_FAIL;
    Put_File_Create_Err_Msg(msg_level, org_name, tmp_name, (char *)NULL, com);
  ...
  if (sig_flg || ((rtn = Link_NewFile(org_name, save_name)) != 0)) {
    ...
    Unlink_Tmpfile(tmp_name, com);
    free(save_name);
    return ((put_file_create_err_msg == DEVICE_FAIL) ? put_file_create_err_msg
                                                     : rtn);
  }
  ...
  if (sig_flg || ((rtn = Link_NewFile(tmp_name, org_name)) != 0)) {
    ...
    if ((rtn = Link_NewFile(save_name, org_name)) != 0) {
      Put_File_Create_Err_Msg(RESTORE_FAIL, org_name, tmp_name, save_name, com);
      Unlink_Tmpfile(tmp_name, com);
    } else {
      Unlink_Tmpfile(tmp_name, com);
      Unlink_Tmpfile(save_name, com);
...
int Unlink_Tmpfile(char *file, char *com) {
  errno = 0;
  if (unlink(file) != 0) {
    USAGE2("%s : The work file cannot be deleted. Please unlink the following "
...
int Chmod_File(char *fname, mode_t mode, char *com) {
  errno = 0;
  if (mode == 0)
    return (0);
  if (chmod(fname, mode) != 0) {
    USAGE3(
        "%s : Failed in the mode change of the following files. Please change "
        "the mode to %o\tafter the inquiry the system manager.\"%s\" \n",
        com, (int)mode, fname);
    return (1);
  }
  return (0);
...
int Chown_File(char *fname, uid_t owner, gid_t group, char *com) {
  errno = 0;
  if (chown(fname, owner, group) != 0) {
    USAGE2(
        "%s : Failed in setteing of the owner and the group of the following "
        "files. \tPlease change to the file of the owner and the group of "
        "following ID \tafter the inquiry the system manager.\"%s\"\n",
        com, fname);
...
int ChkPcfFontFile(char *filename) {
  char *suffix;
...
int ChkSnfFontFile(char *filename) {
  char *suffix;
...
int SetCmdPath(char *com, char **path, char *dflt_path, char *cmd) {
  struct stat statbuf;
  char *pbuf;
