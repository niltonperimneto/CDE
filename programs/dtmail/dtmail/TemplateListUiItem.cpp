 ...
 */
/* $TOG: TemplateListUiItem.C /main/6 1997/09/03 17:40:26 mgreess $ */
/*
 *+SNOTICE
 *
 *      $TOG: TemplateListUiItem.C /main/6 1997/09/03 17:40:26 mgreess $
		       ...
		       Widget value_entry_widget):ListUiItem(w, source, search_key, NULL)
{
  ...
  XtVaSetValues(w,
    ...
    XtAddCallback(w,
		  ...
		  (XtCallbackProc)handleTmplSelection,
		  (XtPointer)this);
...
//-----------------======================-----------------
void handleTmplSelection(Widget w, XtPointer, XtPointer calldata)
{
...
void TemplateListUiItem::writeFromUiToSource()
{
  ...
  if(list_items != NULL) {
      if(list_items->length() > 0)
	{
	  ...
	  mail_rc->setValue(error, "templates", cf_str);
	}
      else
...
void TemplateListUiItem::writeFromSourceToUi()
{
  ...
  char * expanded_list = d_session->expandPath(error, value);

  list_items = new DtVirtArray<PropStringPair *>(10);

  parsePropString(expanded_list, *list_items);
  free(expanded_list);
...
///////////////////////////////////////////////////////////
void TemplateListUiItem::handleAddButtonPress()
{
  char *key_str = NULL;
  char *value_str = NULL;
...
///////////////////////////////////////////////////////////
void TemplateListUiItem::handleChangeButtonPress()
{
  char *key_str = NULL;
  char *value_str = NULL;  
  PropStringPair *new_pair = NULL;
