...
// id of the node that this dlp represents
   const oid_t node_id(void);

// the prev and next dlp of this dlp
   dlp_smart_ptr* prev(void);
   dlp_smart_ptr* next(void);

   oid_t prev_node_oid(void);
   oid_t next_node_oid(void);

   oid_t prev_dlp_oid(void);
   oid_t next_dlp_oid(void);
};
