...
#include "dstr/heap.hpp"
#include "dstr/void_ptr_array.hpp"
#include "dynhash/imp_die.hpp"
#include "storage/unixf_storage.hpp"
#include "storage/heap_comp_funcs.hpp"
#include "storage/page_cache.hpp"
#include "storage/spointer.hpp"
...
public:
   ...
   void remove(void); // remove all pages in the store
...
// seek loc to (loc +/- 1) position 
   ...
   mmdb_pos_t first_loc(void); // return first loc in the store
   mmdb_pos_t last_loc(void); // return last loc in the store
...
#ifdef C_API
   friend void initialize_MMDB(void);
   friend void quit_MMDB(void);
#endif
};
