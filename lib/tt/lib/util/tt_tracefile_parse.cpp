...
#include "util/tt_trace_parser.hpp"
#include <ctype.h>
...
static _Tt_trace_tokens get_token(const _Tt_trace_tokens, _Tt_string &, int &,
                                  _Tt_string &);
static void print_error(_Tt_string &, int);
static void eat_line(_Tt_string &, int &);
static void put_value(_Tt_trace_parser *, _Tt_trace_tokens, _Tt_trace_tokens,
                      _Tt_string &);
...
int _tt_trace_parse(_Tt_string &parse_buf, _Tt_trace_parser *trace_parser) {
  int buf_ptr = 0;
  ...
  while (token = get_token(prev, parse_buf, buf_ptr, value)) {
    ...
    switch (token) {
    case _TT_TRACE_FUNCTIONS_ENUM:
      trace_parser->clear_functions();
      break;
    case _TT_TRACE_STATES_ENUM:
      trace_parser->clear_states();
      break;
    case _TT_TRACE_OPS_ENUM:
      trace_parser->clear_ops();
      break;
    case _TT_TRACE_SENDER_PTYPES_ENUM:
      trace_parser->clear_sender_ptypes();
      break;
    case _TT_TRACE_HANDLER_PTYPES_ENUM:
    ...
    if (prev == _TT_TRACE_ENDLINE) {
      ...
      if (token != _TT_TRACE_ENDLINE && token != _TT_TRACE_ENDFILE) {
        put_value(trace_parser, kywrd, token, value);
      } else {
...
static _Tt_trace_tokens get_token(const _Tt_trace_tokens prev,
                                  _Tt_string &parse_buf, int &buf_ptr,
                                  _Tt_string &value) {
  int i;
  ...
  if (parse_buf[buf_ptr] == ';' || parse_buf[buf_ptr] == '\n') {
    buf_ptr++;
    ...
    do {
      buf_ptr++;
             ...
             parse_buf[buf_ptr] != '\t' && parse_buf[buf_ptr] != ';' &&
             parse_buf[buf_ptr] != '\n');
  }
  ...
  case _TT_TRACE_ENDLINE:
    if (value == "follow")
      return _TT_TRACE_FOLLOW_ENUM;
    if (value == ">")
      return _TT_TRACE_WRITEFILE_ENUM;
    if (value == ">>")
      return _TT_TRACE_APPENDFILE_ENUM;
    if (value == "functions")
      return _TT_TRACE_FUNCTIONS_ENUM;
    if (value == "attributes")
      return _TT_TRACE_ATTRIBUTES_ENUM;
    if (value == "states")
      return _TT_TRACE_STATES_ENUM;
    if (value == "ops")
      return _TT_TRACE_OPS_ENUM;
    if (value == "sender_ptypes")
      return _TT_TRACE_SENDER_PTYPES_ENUM;
    if (value == "handler_ptypes")
      return _TT_TRACE_HANDLER_PTYPES_ENUM;
    if (value == "version")
      return _TT_TRACE_VERSION_VAL;
#ifdef _OPT_TIMERS_SUNOS
    if (value == "timers")
      return _TT_TRACE_TIMERS_ENUM;
#endif
    return _TT_TRACE_UNKNOWN;
  case _TT_TRACE_VERSION_VAL:
    for (i = 0; i < value.len(); ++i) {
      if (!isdigit(value[i]))
        return _TT_TRACE_UNKNOWN;
    }
    return _TT_TRACE_NUMBER;
  case _TT_TRACE_FOLLOW_ENUM:
#ifdef _OPT_TIMERS_SUNOS
  case _TT_TRACE_TIMERS_ENUM:
#endif
    if (value == "on") {
      return _TT_TRACE_ON_ENUM;
    } else if (value == "off") {
      return _TT_TRACE_OFF_ENUM;
    } else {
      return _TT_TRACE_UNKNOWN;
    }
  case _TT_TRACE_WRITEFILE_ENUM:
  case _TT_TRACE_APPENDFILE_ENUM:
    // Just about anything goes.  It's not up to this
    // code to validate filenames.
    return _TT_TRACE_FILENAME_ENUM;
  case _TT_TRACE_FUNCTIONS_ENUM:
  ...
  case _TT_TRACE_ATTRIBUTES_ENUM:
    return allnone_check(value);
  case _TT_TRACE_STATES_ENUM:
    if (value == "none") {
      return _TT_TRACE_NONE_ENUM;
    } else if (value == "edge") {
      return _TT_TRACE_EDGE_ENUM;
    } else if (value == "deliver") {
      return _TT_TRACE_DELIVER_ENUM;
    } else if (value == "dispatch") {
      return _TT_TRACE_DISPATCH_ENUM;
    } else {
  ...
  case _TT_TRACE_ON_ENUM:
  case _TT_TRACE_OFF_ENUM:
  case _TT_TRACE_ALL_ENUM:
  case _TT_TRACE_NONE_ENUM:
  case _TT_TRACE_FILENAME_ENUM:
...
static void print_error(_Tt_string &parse_buf, int buf_ptr) {
  int beg = buf_ptr - 1;
  int end = buf_ptr;
  ...
  while (parse_buf[beg] != ';' && parse_buf[beg] != '\n' && beg != 0) {
    beg--;
  }
  beg++;

  // Find end of line
  while (parse_buf[end] != ';' && parse_buf[end] != '\n' &&
         parse_buf[end] != '\0') {
    end++;
  }
  _tt_syslog(0, LOG_ERR,
             catgets(_ttcatd, 1, 21, "tttracefile syntax error in line:\n%s"),
             parse_buf.mid(beg, end - beg).operator const char *());

  return;
}

static void eat_line(_Tt_string &parse_buf, int &buf_ptr) {
  while (parse_buf[buf_ptr] != ';' && parse_buf[buf_ptr] != '\n') {
    buf_ptr++;
  }
}

static void put_value(_Tt_trace_parser *tp, _Tt_trace_tokens kywrd,
                      _Tt_trace_tokens token, _Tt_string &value) {
  switch (kywrd) {
  case _TT_TRACE_VERSION_VAL:
...
static _Tt_trace_tokens allnone_check(const _Tt_string &value) {
  if (value == "all") {
    return _TT_TRACE_ALL_ENUM;
  } else if (value == "none") {
    return _TT_TRACE_NONE_ENUM;
  } else {
    return _TT_TRACE_UNKNOWN;
...
static _Tt_trace_tokens state_name_to_val(const _Tt_string &value) {
  if (value == "TT_CREATED") {
...
static int _is_tt_func(const _Tt_string &fun) {
  ...
  for (arr_indx = 0; arr_indx < _tt_entries_count; ++arr_indx) {
    ...
    while (_tt_entries[arr_indx][i] == fun[i] && i < name_len)
      i++;
