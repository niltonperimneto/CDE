...
#include "tt_trace_parser.hpp"
#include "util/tt_enumname.hpp"
#include <fcntl.h>
#include <sys/param.h>
#include <sys/stat.h>
#include <sys/types.h>
...
_Tt_trace_parser::_Tt_trace_parser() : trace_stream(), cursor(0) {
  tracer_init();
...
_Tt_trace_parser::_Tt_trace_parser(int fd) : trace_stream(), sink() {
...
void _Tt_trace_parser::tracer_init() {
  follow = _TT_TRACE_OFF_ENUM;
  ifunctions = 1;
  sfunctions = new _Tt_string_list();
  _funcd = _TT_FUNS_ALLNONE;
  attributes = _TT_TRACE_ALL_ENUM;
  states = new _Tt_int_rec_list();
  states->append(new _Tt_int_rec(_TT_TRACE_DISPATCH_ENUM));
...
int _Tt_trace_parser::any_tracing() {
  int msg_tracing =
      (states->count() != 1) || (states->top()->val != _TT_TRACE_NONE_ENUM);
  int api_tracing;
  if (_funcd == _TT_FUNS_ALLNONE) {
    api_tracing = ifunctions;
  } else {
    api_tracing = (!sfunctions.is_null()) && (!sfunctions->is_empty());
  }
  return msg_tracing || api_tracing;
}

int _Tt_trace_parser::follow_val() {
  if (follow == _TT_TRACE_OFF_ENUM) {
    return 0;
  } else {
    return 1;
  }
...
_Tt_string &_Tt_trace_parser::sink_val() { return sink; }

const char *_Tt_trace_parser::mode_val() {
  ...
  if (tmp_mode == O_RDONLY)
    ...
    return "a";
  else if (tmp_mode == (O_RDONLY | O_APPEND) || tmp_mode == (O_RDWR | O_APPEND))
    return "a+";
  else
    return "w"; // default....
}

int _Tt_trace_parser::ifunction_val() {
  if (_funcd == _TT_FUNS_ALLNONE)
    return ifunctions;
  else
    return 0;
...
_Tt_string_list_ptr _Tt_trace_parser::sfunction_val() { return sfunctions; }

int _Tt_trace_parser::attributes_val() const {
  if (attributes == _TT_TRACE_NONE_ENUM) {
    return 0;
  } else {
    return 1;
  }
...
_Tt_int_rec_list_ptr _Tt_trace_parser::states_val() { return states; }

_Tt_string_list_ptr _Tt_trace_parser::ops_val() { return ops; }

_Tt_string_list_ptr _Tt_trace_parser::sender_ptypes_val() {
  return sender_ptypes;
}

_Tt_string_list_ptr _Tt_trace_parser::handler_ptypes_val() {
  return handler_ptypes;
}

#ifdef _OPT_TIMERS_SUNOS
int _Tt_trace_parser::timers_val() {
...
void _Tt_trace_parser::set_follow(int fval) { follow = fval; }

void _Tt_trace_parser::set_sink(const _Tt_string &sinkfile, int open_mode) {
  sink = sinkfile;
  mode = open_mode;
}

void _Tt_trace_parser::clear_functions() {
  if (_funcd == _TT_FUNS_EXPLICIT)
    (void)sfunctions->flush();
  _funcd = _TT_FUNS_NOTSET;
  ifunctions = 0;
}

void _Tt_trace_parser::add_function(const _Tt_string &fun) {
  _funcd = _TT_FUNS_EXPLICIT;
  sfunctions->append(fun);
}

void _Tt_trace_parser::set_function(int fun) {
  _funcd = _TT_FUNS_ALLNONE;
  ifunctions = fun;
}

void _Tt_trace_parser::set_attributes(int attr) { attributes = attr; }

void _Tt_trace_parser::clear_states() {
  if (!states->is_empty())
    states->flush();
}
...
int _Tt_trace_parser::add_state(int state_val) {
  if (!states->is_empty()) {

    if (state_val == _TT_TRACE_NONE_ENUM && states->count() > 0) {
      return 0;
    } else if (state_val != _TT_TRACE_NONE_ENUM && states->count() == 1) {
      _Tt_int_rec_list_cursor state_curs(states);
      if (state_curs.next() == _TT_TRACE_NONE_ENUM) {
        return 0;
      }
    }
  }
...
void _Tt_trace_parser::clear_ops() {
  if (!ops->is_empty())
    ops->flush();
}

void _Tt_trace_parser::add_op(const _Tt_string &newop) { ops->append(newop); }

void _Tt_trace_parser::clear_sender_ptypes() {
  if (!sender_ptypes->is_empty())
    sender_ptypes->flush();
}

void _Tt_trace_parser::add_sender_ptype(const _Tt_string &sp) {
  sender_ptypes->append(sp);
}

void _Tt_trace_parser::clear_handler_ptypes() {
  if (!handler_ptypes->is_empty())
    handler_ptypes->flush();
}

void _Tt_trace_parser::add_handler_ptype(const _Tt_string &hp) {
  handler_ptypes->append(hp);
}

#ifdef _OPT_TIMERS_SUNOS
void _Tt_trace_parser::set_timers(int tval) { timers = tval; }
#endif

int _Tt_trace_parser::is_traced(const _Tt_message &msg) const {
  _Tt_int_rec_list_cursor stateC(states);
  while (stateC.next()) {
    if (stateC->val == _TT_TRACE_NONE_ENUM) {
      return 0;
    }
  }
...
int _Tt_trace_parser::is_traced(const _Tt_message &msg,
  ...
  while (fakeStateC.next()) {
    ...
    case _TT_TRACE_NONE_ENUM:
      return 0;
    case _TT_TRACE_DISPATCH_ENUM:
    case _TT_TRACE_DELIVER_ENUM:
      return 1;
    case _TT_TRACE_EDGE_ENUM:
      ...
      break;
    default:
      realStates->append(new _Tt_int_rec(_token_to_value(fakeStateC->val)));
      break;
    }
  }
...
int _Tt_trace_parser::_is_traced(const _Tt_message &msg) const {
  // Now match on ops

  if (!ops->is_empty() && !_tt_match_string_list(msg.op(), ops))
    return 0;
...
int _Tt_trace_parser::is_traced(_Tt_entry_pt fun) const {
  if (_funcd == _TT_FUNS_ALLNONE)
    return ifunctions;
...
static int _tt_match_string_list(const _Tt_string &str,
                                 _Tt_string_list_ptr slist) {
  int matched = 0;
  if (slist->is_empty()) {
...
int _Tt_trace_parser::next_char() {
  if (cursor < trace_stream.len()) {
    return (int)(unsigned char)trace_stream[cursor++];
  } else {
    return 0; // EOF
  }
}

void _Tt_trace_parser::ungetc(int ch) {
  if (cursor > 0) {
    cursor--;
  }
}
...
int _tt_trace_input() { return trace_parser->next_char(); }
void _tt_trace_unput(int c) { trace_parser->ungetc(c); }
int _tt_trace_iserr() { return 0; } // Default implementation
}
