...
#include "util/tt_global_env.hpp"
#include "util/tt_iostream.hpp"
#include "util/tt_port.h"
#include "util/tt_trace.hpp"
#include "util/tt_trace_stream.hpp"
#include <fcntl.h>
#include <unistd.h>
...
//	0	tracing is off
//
int _Tt_trace::init(int even_if_no_envariable) {
  //
  // States:
  ...
  //
  if (_pstream == 0) {
    _pstream = (_Tt_trace_stream **)malloc(sizeof(_Tt_trace_stream *));
    *_pstream = 0;
    _toggled_off = 0;
  ...
  case TT_ERR_FILE:
    _tt_syslog(0, LOG_ERR,
               catgets(_ttcatd, 1, 22, "Could not process tttracefile in $%s"),
               TRACE_SCRIPT);
    // fall through
  case TT_WRN_STOPPED:
  default:
...
void _Tt_trace::entry(_Tt_message &msg, Tt_state old_state) {
  if (!_entry()) {
    return;
  }
...
void _Tt_trace::entry(_Tt_message &msg, const _Tt_procid &recipient) {
  if (!_entry()) {
    return;
  }
...
void _Tt_trace::entry(_Tt_message &msg, _Tt_dispatch_reason reason) {
  if (!_entry()) {
    return;
  }
...
//
void _Tt_trace::entry(const char *argskey, _Tt_entry_pt func, va_list ap) {
  _funcname = func;
  ...
  **_pstream << _tt_enumname(func) << "(";
  ...
  while (num_args--) {
    ...
    case BYTES_WITH_LEN: {
      ...
      **_pstream << (void *)cp;
      if ((cp != 0) && (_tt_pointer_error(cp) == TT_OK)) {
        **_pstream << "==";
        _Tt_string s((const unsigned char *)cp, length);
...
//
void _Tt_trace::entry(Tt_message_callback cb, Tt_message mh, Tt_pattern ph) {
  if (!_entry()) {
    return;
  }
  ...
  **_pstream << (void *)cb << " (" << (void *)mh << ", " << (void *)ph << ")";

  (*_pstream)->set_is_entered(1);
  (*_pstream)->incr_indent();
}

const _Tt_trace &_Tt_trace::operator<<(_Tt_message &msg) const {
...
const _Tt_trace &_Tt_trace::operator<<(const _Tt_pattern &pat) const {
  if ((!any_tracing()) || (!_tracing)) {
    return *this;
  }
  **_pstream << pat;
...
const _Tt_trace &_Tt_trace::operator<<(const _Tt_string &s) const {
  if ((!any_tracing()) || (!_tracing)) {
    return *this;
  }
  **_pstream << s;
...
const _Tt_trace &_Tt_trace::operator<<(Tt_status status) const {
  if ((!any_tracing()) || (!_tracing)) {
    return *this;
  }
  **_pstream << status;
...
const _Tt_trace &_Tt_trace::operator<<(const char *s) const {
  if ((!any_tracing()) || (!_tracing)) {
    return *this;
  }
  **_pstream << s;
...
const _Tt_trace &_Tt_trace::operator<<(int n) const {
  if ((!any_tracing()) || (!_tracing)) {
    return *this;
  }
  **_pstream << n;
...
const _Tt_trace &_Tt_trace::print(_Tt_object_printfn print_it,
                                  const _Tt_object *obj) const {
  if ((!any_tracing()) || (!_tracing)) {
    return *this;
  }
...
void _Tt_trace::incr_indent() const {
  if ((!any_tracing()) || (!_tracing)) {
    return;
  }
...
void _Tt_trace::decr_indent() const {
  if ((!any_tracing()) || (!_tracing)) {
    return;
  }
...
void _Tt_trace::exitq() {
  ...
  if ((*_pstream)->timers_val()) {
    ...
    sprintf(time_string, "%d.%06d", newtime.tv_sec - timestamp.tv_sec, temp);
...
void _Tt_trace::exit(Tt_message val) { exit((void *)val); }

void _Tt_trace::exit(Tt_pattern val) { exit((void *)val); }

void _Tt_trace::exit(char *val) {
  Tt_status val_status = _tt_pointer_error(val);
...
void _Tt_trace::exit(Tt_status val) {
  if (any_tracing() && _tracing) {
...
void _Tt_trace::_exit(int val, const char *(*enumname)(int), int drop) {
  Tt_status val_status = _tt_int_error(val);
...
//
int _Tt_trace::any_tracing() {
  return (_pstream != 0) && (*_pstream != 0) && (!_toggled_off);
}

int _Tt_trace::_entry() {
  _tracing = 0;
  if (!init()) {
    return 0;
  }
