 ...
 * pattern that a process can in order to receive messages. The
...
#include <mp/mp_pat_context_utils.hpp>
...
class _Tt_pattern : public _Tt_object {
	...
	void			clr_registered(void);
	void			set_in_session(void);
	void			clr_in_session(void);
...
#include <mp/mp_pattern_inlines.hpp>
