...
#include "mp/mp_rpc.h"
#include "mp/mp_desktop_utils.hpp"
#include "mp/mp_rpc_client_utils.hpp"
#include "mp/mp_session_utils.hpp"
#include "mp/mp_session_prop_utils.hpp"
...
class _Tt_session : public _Tt_object {
	...
	_Tt_string		address_string(void);
	...
	_Tt_string		Xid(_Tt_string xdisp);
        bool_t			xdr(XDR *xdrs);
	int			desktop_event_callback(void);
	_Tt_string		displayname(void);
	Tt_status		set_id(char *sid = (char *)0);
	Tt_status		client_session_init(void);
	_Tt_string 		process_tree_id(void);
	...
	int			has_id(const _Tt_string_list_ptr slist_p);
      ...
      protected:
	Tt_status		auto_start_init(void);
