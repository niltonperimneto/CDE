...
#include "eclipse.hpp"
#include "ttdesktop.h"
#include "ttdtprocid.hpp"
#include "tttkutils.h"

TtDtProcIDEclipse::TtDtProcIDEclipse(const char *procID2Install,
                                     Tt_status *status) {
  ...
  if (*status != TT_OK) {
    ...
    if ((procID2Install == 0) && (*status == TT_ERR_NOMP)) {
      ...
      *status = TT_OK;
      ...
      _ttDtPrintStatus(here, "tt_default_procid()", *status);
      return;
    }
  }
  if (procID2Install != 0) {
    *status = tt_default_procid_set(procID2Install);
    if (*status != TT_OK) {
      _ttDtPrintStatus(here, "tt_default_procid_set()", *status);
