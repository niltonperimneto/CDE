...
#include "db/tt_db_hostname_global_map_ref.hpp"
...
private:
  ...
  void           setTtDBObjectDefaults (void);
  _Tt_db_results createMemoryObject (const _Tt_string&, const _Tt_string&);
  void           getDBObjectHostnameFromID (void);
  _Tt_db_results createDBObject (void);
  ...
  _Tt_db_results internalRefresh (void);
  ...
  //
  _Tt_db_results processDBResults (void); 
