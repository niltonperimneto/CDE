...
static char root_weave_bits[] = {0x05, 0x0a, 0x05, 0x0a};

#ifdef NLS16
#endif

/********    Private Function Declarations    ********/
static void DADrawSpc(_DtCvPointer client_data, _DtCvPointer spc_handle,
                      _DtCvUnit x, _DtCvUnit y, _DtCvUnit box_x,
                      _DtCvUnit box_y, _DtCvUnit box_height, int link_type,
                      _DtCvFlags old_flags, _DtCvFlags new_flags);
...
static SpecialSymbolTable SpcTable[] = {
    ...
    {"[ap    ]",
     {'\273', '\0'},
     SymbolString},                          /*ISOtech, approximately equal   */
    ...
    {"[bull  ]",
     {'\267', '\0'},
     SymbolString}, /*ISOpub , round bullet,filled   */
    ...
    {"[cup   ]",
     {'\310', '\0'},
     SymbolString},                          /*ISOtech, union or logical sum  */
    ...
    {"[diams ]",
     {'\250', '\0'},
     SymbolString}, /*ISOpub , diamond suit symbol   */
    ...
    {"[epsiv ]",
     {'\145', '\0'},
     SymbolString},                       /*ISOgrk3, Greek,sm epsilon, var */
    ...
    {"[exist ]",
     {'\044', '\0'},
     SymbolString}, /*ISOtech, at least one exists   */
    ...
    {"[ge    ]",
     {'\263', '\0'},
     SymbolString},                          /*ISOtech, greater-than-or-equal */
    ...
    {"[hellip]",
     {'\274', '\0'},
     SymbolString}, /*ISOpub , ellipsis(horizontal)  */
    ...
    {"[image ]",
     {'\301', '\0'},
     SymbolString}, /*ISOamso,imaginary number symbol*/
    ...
    {"[lambda]",
     {'\154', '\0'},
     SymbolString}, /*ISOgrk3, Greek, small lambda   */
    ...
    {"[mldr  ]", {'.', '.', '.', '.', '\0'}, IsoString}, /*ISOpub , em leader */
    {"[mnplus]",
     {'\140', '\b', '+', '\0'},
    ...
    {"[nabla ]",
     {'\321', '\0'},
     SymbolString},                       /*ISOtech, del, Hamilton operator*/
    ...
    {"[notin ]",
     {'\317', '\0'},
     SymbolString},                          /*ISOtech, negated set membership*/
    ...
    {"[oplus ]",
     {'\305', '\0'},
     SymbolString}, /*ISOamsb, plus sign in circle   */
    ...
    {"[piv   ]",
     {'\166', '\0'},
     SymbolString},                       /*ISOgrk3, Greek, small pi, var  */
    ...
    {"[reg   ]",
     {'\322', '\0'},
     SymbolString}, /*ISOnum,circledR,registered sign*/
    ...
    {"[sub   ]",
     {'\314', '\0'},
     SymbolString}, /*ISOtech, subset/is implied by  */
    {"[sube  ]", {'\315', '\0'}, SymbolString}, /*ISOtech, subset, equals */
    /*"[sung  ]", { ??   ,'\0'},             },  *ISOnum , musical note(sung
       txt)*/
    {"[sup   ]",
     {'\311', '\0'},
     SymbolString},                          /*ISOtech, superset or implies   */
    ...
    {"[thetav]",
     {'\112', '\0'},
     SymbolString}, /*ISOgrk3, Greek, small theta,var*/
    ...
    {"[upsi  ]",
     {'\165', '\0'},
     SymbolString},                          /*ISOgrk3, Greek, small upsilon  */
    ...
    {"[Lambda]",
     {'\114', '\0'},
     SymbolString},                          /*ISOgrk3, Greek, large lambda   */
    ...
    {"[Prime ]",
     {'\262', '\0'},
     SymbolString}, /*ISOtech, double prime/second   */
    ...
    {"[Upsi  ]",
     {'\125', '\0'},
     SymbolString},                          /*ISOgrk3, Greek, large upsilon  */
 ...
 *
 *****************************************************************************/
static _DtHelpFontHints DefFontAttrs = {NULL,
                                        NULL,
                                        10,
                                        10,
                                        NULL,
...
static void GetTocIndicator(DtHelpDispAreaStruct *pDAS) {
  _DtCvUnit ascent;
...
static void ClearTocMarker(DtHelpDispAreaStruct *pDAS, _DtCvFlags flag) {
  _DtCvUnit clearHeight;
  _DtCvUnit clearWidth;
  _DtCvUnit clearY = pDAS->toc_y + pDAS->decorThickness - pDAS->firstVisible;
  ...
  if (clearWidth > 0 && clearHeight > 0) {
    if (IsSelected(flag))
      XFillRectangle(XtDisplay(pDAS->dispWid), XtWindow(pDAS->dispWid),
                     pDAS->invertGC, clearX, clearY, clearWidth, clearHeight);
    else
      XClearArea(XtDisplay(pDAS->dispWid), XtWindow(pDAS->dispWid), clearX,
                 clearY, clearWidth, clearHeight, False);
  }
}
 ...
 *
 *****************************************************************************/
static void DrawTocIndicator(DtHelpDispAreaStruct *pDAS, _DtCvFlags flag) {

  if (XtIsRealized(pDAS->dispWid) && TocIsOn(pDAS->toc_flag) &&
    ...
    DADrawSpc(((_DtCvPointer)pDAS), regInfo->handle, 0, pDAS->toc_base, 0,
              pDAS->toc_y, pDAS->toc_height, 0, 0, 0);
  }
}
 ...
 *
 *****************************************************************************/
static void DrawTraversal(DtHelpDispAreaStruct *pDAS, _DtCvUnit dst_x,
                          _DtCvUnit dst_y, _DtCvUnit width, _DtCvUnit height,
                          _DtCvFlags flag) {
  int copyY1;
  int copyY2;
  ...
  if (pDAS->neededFlags & _DT_HELP_FOCUS_FLAG) {
    ...
    if (lineX1 - onLeftTop < ((int)pDAS->dispWidth) &&
      ...
      if (copyY1 < ((int)pDAS->dispHeight) &&
          copyY1 + ((int)pDAS->lineThickness) > ((int)pDAS->decorThickness))
     ...
     * draw the left end
     */
    if (IsTraversalBegin(flag) && lineX1 - onLeftTop < ((int)pDAS->dispWidth) &&
        lineX1 + onRightBot - 1 > 0 && copyY1 < ((int)pDAS->dispHeight) &&
        copyY2 > ((int)pDAS->decorThickness))
     ...
     * draw the right end
     */
    if (IsTraversalEnd(flag) && lineX2 - onLeftTop < ((int)pDAS->dispWidth) &&
        lineX2 + onRightBot - 1 > 0 && copyY1 < ((int)pDAS->dispHeight) &&
        copyY2 > ((int)pDAS->decorThickness))
...
static void DrawSearchTraversal(DtHelpDispAreaStruct *pDAS, _DtCvUnit dst_x,
                                _DtCvUnit dst_y, _DtCvUnit width,
                                _DtCvUnit height, _DtCvFlags flag) {
 ...
 *
 *****************************************************************************/
static void MyDrawString(Display *dpy, Drawable d, GC gc, long font_index,
                         long x, long y, const void *string, int length, int wc,
                         int image, DtHelpDAFontInfo font_info) {
  ...
  if (font_index < 0) {
    ...
    if (wc) {
      if (image)
        XwcDrawImageString(dpy, d, mySet, gc, x, y, (wchar_t *)string, length);
      else
        XwcDrawString(dpy, d, mySet, gc, x, y, (wchar_t *)string, length);
    } else {
      /*
 ...
 *
 *****************************************************************************/
static int ResolveFont(_DtCvPointer client_data, char *lang,
                       const char *charset, _DtHelpFontHints font_attr,
                       long *ret_idx) {
  int result = -1;
 ...
 *
 *****************************************************************************/
static void DADrawLine(_DtCvPointer client_data, _DtCvUnit x1, _DtCvUnit y1,
                       _DtCvUnit x2, _DtCvUnit y2, _DtCvUnit line_width) {
  int offset;
  Display *dpy;
  Window win;
  DtHelpDispAreaStruct *pDAS = (DtHelpDispAreaStruct *)client_data;
 ...
 *
 *****************************************************************************/
static void DADrawString(_DtCvPointer client_data, _DtCvPointer data,
                         _DtCvUnit x, _DtCvUnit y, const void *string,
                         int byte_len, int wc, _DtCvPointer font_ptr,
                         _DtCvPointer scd, _DtCvUnit box_x, _DtCvUnit box_y,
                         _DtCvUnit box_height, _DtCvFlags old_flags,
  ...
  if (strWidth && pDAS->dtinfo &&
     ...
     */
      ...
      if (pSCD->fg_pixel == (unsigned long)-1) {
        if (XAllocNamedColor(dpy, pDAS->colormap, pSCD->fg_color, &screen,
                             &exact)) {
    ...
    if (pSCD->bg_color) {
      ...
      if (pSCD->bg_pixel == (unsigned long)-1) {
        if (XAllocNamedColor(dpy, pDAS->colormap, pSCD->bg_color, &screen,
                             &exact)) {
   ...
   */
  if (IsSelected(new_flags) || IsSelected(old_flags) || IsMarkOn(new_flags) ||
      IsMarkOn(old_flags))
    image_flag = True;
  ...
  if (image_flag) {
    ...
    XFillRectangle(dpy, win, fillGC, eff_x, (int)box_y, eff_width, box_height);
  }

  if (strWidth) {
    MyDrawString(dpy, win, drawGC, (long)font_ptr, (long)x, (long)y, string,
                 byte_len, wc, False, pDAS->font_info);
    ...
    if (IsHyperLink(new_flags)) {
      ...
      XDrawLine(dpy, win, drawGC, (int)x, (int)y, (int)(x + strWidth), (int)y);
...
static void DADrawSpc(_DtCvPointer client_data, _DtCvPointer spc_handle,
  ...
  do {
    ...
    DADrawString(client_data, (_DtCvPointer)&strInfo, x, y, spcStr, i, 0,
                 pDAS->spc_chars[spcLstIdx].font_ptr, NULL, box_x, box_y,
                 box_height, old_flags, new_flags);
 ...
 *
 *****************************************************************************/
static void DADrawGraphic(_DtCvPointer client_data, _DtCvPointer graphic_ptr,
                          _DtCvUnit x, _DtCvUnit y, _DtCvUnit box_width,
                          _DtCvUnit box_height, _DtCvFlags old_flags,
                          _DtCvFlags new_flags) {
  int width;
  int height;
   ...
   */
  if (height > 0 && width > 0)
    XCopyArea(dpy, pGS->pix, win, pDAS->normalGC, srcX, srcY, width, height, x,
              y);
  ...
  if (HasTraversal(new_flags))
    ...
    y = old_y + box_height - pDAS->lineThickness + pDAS->lineThickness / 2;
 ...
 *
 *****************************************************************************/
static _DtCvStatus DABuildSelectedString(_DtCvPointer client_data,
                                         _DtCvPointer *prev_info,
                                         const char *string, int byte_len,
                                         int wc, _DtCvPointer font_ptr,
                                         _DtCvUnit space, _DtCvFlags flags) {
  int i;
  int j;
  int mbLen;
  int totalLen;
 ...
 *
 *****************************************************************************/
static _DtCvStatus DABuildSelectedSpc(_DtCvPointer client_data,
                                      _DtCvPointer *prev_info,
                                      _DtCvPointer spc_handle, _DtCvUnit space,
                                      _DtCvFlags flags) {
 ...
 *
 *****************************************************************************/
const char *_DtHelpDAGetSpcString(int idx) {
 ...
 *
 *****************************************************************************/
void _DtHelpDAResolveFont(_DtCvPointer client_data, char *lang,
                          const char *charset, _DtHelpFontHints font_attr,
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtHelpDAResolveSpc(_DtCvPointer client_data, char *lang,
                                const char *charset, _DtHelpFontHints font_attr,
                                const char *spc_symbol,
                                _DtCvPointer *ret_handle, _DtCvUnit *ret_width,
                                _DtCvUnit *ret_height, _DtCvUnit *ret_ascent) {
  int i = 0;
  int result = -1;
  long fontIdx;
  long spcTbIdx = 0;
  ...
  if (result == 0) {
    ...
    *ret_width = 0;
     ...
     * ascent.
     */
    _DtHelpDAGetFontMetrics(client_data, (_DtCvPointer)fontIdx, &ascent,
                            &descent, NULL, NULL, NULL);
    *ret_ascent = ascent;
    ...
    result =
        _DtHelpDAResolveSpc(client_data, lang, charset, font_attr, DefaultStr,
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtHelpDALoadGraphic(_DtCvPointer client_data, char *vol_xid,
                                 char *topic_xid, char *file_xid, char *format,
                                 char *method, _DtCvUnit *ret_width,
   ...
   */
  if (fileName != NULL &&
      _DtHelpCeStrrchr(fileName, ".", MB_CUR_MAX, &extptr) != -1)
  ...
  pGS->pix = _DtHelpProcessGraphic(
      XtDisplay(pDAS->dispWid), win,
      XScreenNumberOfScreen(XtScreen(pDAS->dispWid)), pDAS->depth,
      pDAS->pixmapGC, &(pDAS->def_pix), &(pDAS->def_pix_width),
      &(pDAS->def_pix_height), pDAS->context, pDAS->colormap, pDAS->visual,
      pDAS->foregroundColor, pDAS->backgroundColor, fileName,
      pDAS->media_resolution, &(pGS->width), &(pGS->height), &(pGS->mask),
 ...
 *
 *****************************************************************************/
void _DtHelpDADestroyGraphic(_DtCvPointer client_data,
 ...
 *
 *****************************************************************************/
void _DtHelpDADestroySpc(_DtCvPointer client_data, _DtCvPointer spc_handle) {
 ...
 *
 *****************************************************************************/
void _DtHelpDADestroyRegion(_DtCvPointer client_data, _DtCvPointer reg_info) {
 ...
 *
 *****************************************************************************/
void _DtHelpDATocMarker(XtPointer client_data, Boolean draw) {
  DtHelpDispAreaStruct *pDAS = (DtHelpDispAreaStruct *)client_data;
 ...
 *
 *****************************************************************************/
void _DtHelpDAGetCvsMetrics(_DtCvPointer client_data, _DtCvElemType elem_type,
 ...
 *
 *****************************************************************************/
void _DtHelpDAGetFontMetrics(_DtCvPointer client_data, _DtCvPointer font_ptr,
                             _DtCvUnit *ret_ascent, _DtCvUnit *ret_descent,
                             _DtCvUnit *ret_width, _DtCvUnit *ret_super,
                             _DtCvUnit *ret_sub) {
  DtHelpDispAreaStruct *pDAS = (DtHelpDispAreaStruct *)client_data;

  __DtHelpFontMetrics(pDAS->font_info, (long)font_ptr, ret_ascent, ret_descent,
                      ret_width, ret_super, ret_sub);
}
 ...
 *
 *****************************************************************************/
_DtCvUnit _DtHelpDAGetStrWidth(_DtCvPointer client_data,
                               _DtCvElemType elem_type, _DtCvPointer data) {
  DtHelpDispAreaStruct *pDAS = (DtHelpDispAreaStruct *)client_data;
  int length;
  long font_index;
  _DtCvStringInfo *strInfo;
  ...
  if (font_index < 0) {
    if (0 == strInfo->wc)
      length =
          XmbTextEscapement(__DtHelpFontSetGet(pDAS->font_info, font_index),
                            (char *)strInfo->string, strInfo->byte_len);
    else
      length =
          XwcTextEscapement(__DtHelpFontSetGet(pDAS->font_info, font_index),
 ...
 *
 *****************************************************************************/
void _DtHelpDARenderElem(_DtCvPointer client_data, _DtCvElemType elem_type,
                         _DtCvUnit x, _DtCvUnit y, int link_type,
                         _DtCvFlags old_flags, _DtCvFlags new_flags,
                         _DtCvElemType trav_type, _DtCvPointer trav_data,
                         _DtCvPointer data) {
  _DtCvStringInfo *strInfo;
  ...
  if (_DtCvSTRING_TYPE == elem_type) {
    ...
    DADrawString(client_data, posInfo->info, x, y, strInfo->string,
                 strInfo->byte_len, strInfo->wc, strInfo->font_ptr,
                 strInfo->csd, posInfo->box_x, posInfo->box_y,
                 posInfo->box_height, old_flags, new_flags);
    ...
    if (_DtHelpDAGraphic == regInfo->type) {
      DADrawGraphic(client_data, regInfo->handle, posInfo->box_x,
                    posInfo->box_y, posInfo->box_width, posInfo->box_height,
                    old_flags, new_flags);
    } else if (_DtHelpDASpc == regInfo->type) {
      DADrawSpc(client_data, regInfo->handle, x, y, posInfo->box_x,
                posInfo->box_y, posInfo->box_height, link_type, old_flags,
                new_flags);
    }
  }
}
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtHelpDABuildSelection(_DtCvPointer client_data,
                                    _DtCvElemType elem_type, unsigned int mask,
                                    _DtCvPointer *prev_info, _DtCvUnit space,
                                    _DtCvUnit width, _DtCvFlags flags,
                                    _DtCvPointer data) {
  ...
  if (_DtCvSTRING_TYPE == elem_type) {
    ...
    result = DABuildSelectedString(client_data, prev_info, strInfo->string,
    ...
    if (_DtHelpDASpc == regInfo->type)
      result = DABuildSelectedSpc(client_data, prev_info, regInfo->handle,
                                  space, flags);
  }
