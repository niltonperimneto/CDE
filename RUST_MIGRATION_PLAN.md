...
### Phase 5 — Parser Migration Completion (Yacc/Flex -> Rust Parsers)

| ID | Area | Action |
|----|------|--------|
| P5-1 | Inventory | Maintain a single source of truth of all active Yacc/Flex generators in Meson and their replacement status |
| P5-2 | Strategy | For each grammar, choose one target parser strategy (pest preferred; recursive descent/nom allowed when better fit) and document rationale |
| P5-3 | Compatibility corpus | Create fixture corpus per grammar: valid, invalid, and adversarial inputs |
| P5-4 | Dual parse mode | Add `legacy`, `rust`, and `compare` modes; run both parsers and diff canonical AST output during transition |
| P5-5 | Diagnostics parity | Keep error quality stable: line/column, category (`lexical`/`syntax`/`semantic`), and stable error identifiers |
| P5-6 | Performance guardrail | Enforce no more than 10 percent median parse regression on representative corpora |
| P5-7 | Fuzz before cutover | Add fuzz targets for every grammar before removing Yacc generator paths |
| P5-8 | Cutover policy | Remove Yacc generation from Meson when parity gates pass, or explicitly keep with owner and deadline |

### Phase 6 — Meson Production Hardening and Release Gates

| ID | Area | Action |
|----|------|--------|
| P6-1 | Build determinism | Normalize Meson install paths/options and remove hardcoded install roots in compile defines and generated resources |
| P6-2 | Rust/Meson contract | Use deterministic Cargo artifact paths, lockfile enforcement, and explicit dependency modeling in Meson |
| P6-3 | CI ratchet | Stage A: fail only on configure/build breakage and test crashes |
| P6-4 | CI ratchet | Stage B: fail on sanitizer regressions, lock drift, and high-severity dependency findings |
| P6-5 | CI ratchet | Stage C: fail on warnings budget, ABI regressions, parser parity regressions, and install-path policy violations |
| P6-6 | Release sign-off | Require two consecutive clean mainline pipelines before production tag |

---

## 4.1 Parser Migration Status and Debug Plan (added 2026-04-16)

### Current status snapshot

Completed with pest:
- `cde/programs/dtwm/rust/src/parser.rs` using `cde/programs/dtwm/rust/grammar/dtwmrc.pest`
- `cde/lib/DtSvc/rust/src/parser.rs` using `cde/lib/DtSvc/rust/grammar/dtfile.pest`

Completed Yacc replacement with non-pest Rust parsers (accepted):
- `cde/lib/csa/recurrence/src/parser.rs` replacing `cde/lib/csa/reparser.y`
- `cde/lib/DtSearch/rust/src/query.rs` replacing `cde/lib/DtSearch/boolyac.y` parse path
- `cde/programs/dtappbuilder/rust/src/parser.rs` replacing `cde/programs/dtappbuilder/src/libABil/bil_parse.y`

Still generated by Yacc/Bison in active Meson targets:
- `cde/lib/DtMmdb/StyleSheet/meson.build` (`defParser.yy`, `style.yy`)
- `cde/lib/DtMmdb/schema/meson.build` (`sheet.yy`)
- `cde/programs/dtcm/server/meson.build` (`parser.y`)
- `cde/programs/dtcm/libDtCmP/meson.build` (`getdate.y`)
- `cde/lib/csa/meson.build` (`reparser.y`, currently still generated in Meson)

### Debug workflow for each parser migration

1. Build and freeze parser fixture corpora (valid, invalid, adversarial).
2. Run legacy and Rust parser side by side in compare mode.
3. Normalize to canonical AST JSON before diffing.
4. Record diagnostics parity (span, category, stable ID).
5. Run parser fuzz smoke and sanitizer-backed tests.
6. Measure performance and memory behavior against baseline.
7. Keep rollback switch until two green pipelines with Rust parser only.

### Parser-specific release gates

- 100 percent compatibility corpus pass for each migrated grammar.
- Zero parser-related crashes in ASan/UBSan test suites.
- Zero panic-through-FFI events.
- No high-severity fuzz findings at cutover time.
- Legacy Yacc generation removed from Meson for migrated targets, or documented retention with owner and deadline.

### Program-level decisions inherited from build hardening plan

- Release-blocking platform for now: Linux x86_64 glibc.
- Policy model: phased warning baseline to strict hard-fail.
- ToolTalk compatibility requirement: strict ABI compatibility for existing `libtt` symbol surface.
