...
Create Action:files created byaction name

The output from Create Action is:

An action definition that runs a command. A file`HomeFolder`/.dt/types/`action_name`.dtis created containing the
definition.

action icon

Anaction iconfor the action. The action icon is placed in your home
...
The data type(s) and Open and Print actions are also written to the file`HomeFolder`/.dt/types/`action_name`.dt.
...
use Create Action to write an action for the command:lp -d`device``filename`
...
Use1to start at the beginning of the file.
* **OK** 
...
Text field for entering thebase nameof
...
Create Action:opening an action fileaction:openingaction:editing
