...
# Summary of the App Builder Process
app builder processprocess, creating an interface

The process of creating an App Builder user interface can be summarized as follows.

Design the interface; draw a picture of it.

Perform an App Builder "spec" of the interface (determine what
App Builder objects will be used for each of the parts of the
interface design).

Start App Builder.

Drag and drop a main window onto the desktop; this will be the
starting point for all functions in the finished application.

Drag and drop a control pane onto the main window; controls
will be dropped onto the control pane.

Drag and drop the appropriate controls onto the control pane;
use the group, alignment, and attachment tools of App Builder to
position the controls properly.

Drag and drop other main windows and custom dialogs
onto the desktop to create the other parts of the interface.

Make programmatic connections between objects in the
interface; test the connections in test mode.

Refine the interface and continue testing until it is right.

Generate code by running the App Builder code generator; add
user code to complete the functionality of the application.

Compile, run, test, iteratively until the application is
complete.
# App Builder Projects and Modules
projectsmodules
...
working on aproject,which is comprised of one or more
...
A project file has a.bip(builder interface
project) suffix.
# See Also
...
A saved module file has a.bil(builder interface
language) suffix.
# See Also
...
Once you have created a message as described in, you must determine when and how it should be
...
code generator (dtcodegen) supplies two routines,
corresponding to the two modes of display. They are found indtb_utils.cand are named:

dtb_show_modal_message()

dtb_show_message()

If you want to display a particular message modally, usedtb_show_ modal_message().
...
# Coding for Modal Messages


As explained above, if a message is to be displayed modally, usedtb_show_modal_message(). This routine returns
a value which indicates which message box button the user has pressed.
The value is anenumthat is defined indtb_utils.h:/*
...
created a simple application, namedfoo. The
project is namedfoo.bipand consists of one
module,foo.bil. The modulefoo.bilconsists of a main window, control pane,
...
it will post the error message modally:void 
verify_first_nameCB(
    Widget widget,
...
# Coding for Non-Modal Messages


If you want to post a non-modal message, you should usedtb_show_message(). The callbacks for the
message box buttons should be specified via the Connections Editor, as
described in. The buttons that are specified
...
will do nothing, in this case.void 
verify_last_nameCB(
    Widget widget,
    XtPointer clientData,
    XtPointer callData
...
The two routines above,start_overCB()andcontinueCB(), are added as callbacks for the two
buttons via the call todtb_show_message(). Here
is the code fragment that adds the callback (fromdtb_utils.c):/* Add Callbacks if necessary */
...
The structurembrcontains all the necessary
...
created via thedtb_<modulename>_<messagename>_initialize()routine -- in this example,dtb_foo_message_initialize().
# See Also
